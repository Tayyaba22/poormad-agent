...
def _(rid, params: dict) -> dict:
    ...
    paste_dir = _poormad_home / "pastes"
...
def _(rid, params: dict) -> dict:
    ...
    def _profile_mention_items(prefix: str) -> list[dict]:
        ...
        under the 'poormad' alias when no real profile claims that name.
        """
        out: list[dict] = []
        try:
            ...
            if "poormad".startswith(prefix.lower()) and "poormad" not in seen:
                out.append(
                    {
                        "text": "@poormad",
                        "display": "@poormad",
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from poormad_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    try:
        from poormad_cli.inventory import build_model_options_payload
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from poormad_cli.inventory import build_models_payload
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `poormad model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.poormad/.env via the unified credential lifecycle
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from poormad_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
