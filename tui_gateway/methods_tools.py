...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.poormad/.env`` into the gateway process via
    ``poormad_cli.config.reload_env``, matching classic CLI's ``/reload``
    ...
    """
    try:
        from poormad_cli.config import reload_env
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m poormad_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "poormad_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m poormad_cli.main` (can drive the agent) →
            # needs provider credentials. Tier-1 secrets still stripped (#29157).
            env=poormad_subprocess_env(inherit_credentials=True),
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from poormad_cli.moa_config import moa_usage, normalize_moa_config
    ...
    if name == "loop":
        ...
        if not session:
            return _err(rid, 4001, "no active session")
        try:
            from poormad_cli.loops import LoopManager, dispatch_loop_command
        except Exception as exc:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind POORMAD_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_poormad_home_override(_profile_home) if _profile_home else None
        )
        try:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("POORMAD_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("POORMAD_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_poormad_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off POORMAD_HOME, so scoping the
    ...
    if profile:
        try:
            ...
            from poormad_constants import set_poormad_home_override
    ...
    try:
        ...
        if token is not None:
            try:
                from poormad_constants import reset_poormad_home_override
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``poormad journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from poormad_cli.skills_hub import do_install
        ...
        if action == "browse":
            from poormad_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from poormad_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `poormad mcp` offers, so
    ...
    try:
        ...
        from poormad_cli import mcp_catalog
...
# (poormad_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes POORMAD_HOME via set_poormad_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses poormad_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from poormad_cli.mcp_config import _get_mcp_servers
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from poormad_cli.config import load_config, save_config, save_env_value
        from poormad_cli.mcp_config import (
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from poormad_cli.mcp_config import _remove_mcp_server
...
def _(rid, params: dict) -> dict:
    ...
    (``window.poormadDesktop.openExternal``) and then polls
    ...
    the SAME interactive MCP OAuth machinery ``poormad mcp login`` uses
    ...
    try:
        ...
        poormad_home = str(get_poormad_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(poormad_home, name, cfg)
        return _ok(
            rid,
            {
                "ok": True,
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``poormad plugins`` / the dashboard, so the three surfaces
    ...
    Actions:
      ...
      - ``install`` → git-clone into ``~/.poormad/plugins/`` (non-interactive).
    ...
    plugins live under each profile's POORMAD_HOME, so a client can list or
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native PoorMad plugin.
