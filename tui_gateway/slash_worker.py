"""Persistent slash-command worker — one PoorMadCLI per TUI session.

Protocol: reads JSON lines from stdin {id, command}, writes {id, ok, output|error} to stdout.
"""

# Stop a ``utils/`` (or ``proxy/``, ``ui/``) package in the launch directory
# from shadowing PoorMad's own top-level modules.  This worker is spawned as
...
# and crash the child in a retry loop (issue #51286).  ``poormad_bootstrap``
...
def _env_float(name: str, default: float) -> float:
    ...
    import time on a typo (e.g. ``POORMAD_SLASH_WATCHDOG_POLL_S=2s``) and kill
...
_WATCHDOG_POLL_S = max(0.05, _env_float("POORMAD_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, _env_float("POORMAD_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _prepare_slash_worker_runtime() -> None:
    """Start bounded MCP discovery before PoorMadCLI snapshots tools.

    Each slash_worker child is its own process — the parent ``poormad serve``
...
def _run(cli: PoorMadCLI, command: str) -> str:
...
def main():
    ...
    os.environ["POORMAD_SESSION_KEY"] = args.session_key
    os.environ["POORMAD_INTERACTIVE"] = "1"

    # Start before the (hundreds-of-ms) PoorMadCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = PoorMadCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
