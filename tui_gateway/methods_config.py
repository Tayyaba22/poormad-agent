    ...
    try:
        with _profile_db(params) as db:
            ...
            with pdb.connect_closing() as conn:
                ...
                # zero PoorMad sessions still surface. The desktop's native scan
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from poormad_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_poormad_home), "display": display_poormad_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _poormad_home / "config.yaml"
        try:
        ...
        if (
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No PoorMad provider is configured.",
                },
            )
...
def _(rid, params: dict) -> dict:
    """Upload a redacted debug bundle to PoorMad-internal diagnostics storage.

    Desktop's "Send Diagnostics" action (error card / diagnostics UI). Same
    collection + force-redaction pipeline as ``poormad debug share --nous``
