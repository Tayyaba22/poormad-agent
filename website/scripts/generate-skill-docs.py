...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/Tayyaba22/poormad-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    poormad_meta = (fm.get("metadata") or {}).get("poormad") or {}
    tags = poormad_meta.get("tags") or []
    related = poormad_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `poormad skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with PoorMad"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "PoorMad ships with a large built-in skill library copied into `~/.poormad/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "PoorMad also syncs bundled skills on `poormad update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.poormad/skills/` tree, it is still shipped with PoorMad; restore it with `poormad skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with poormad-agent — install via poormad skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with poormad-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "poormad skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "poormad skills install official/blockchain/solana",
        "poormad skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "poormad skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
