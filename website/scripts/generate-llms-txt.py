#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the PoorMad docs site.
...
LLM reads to learn what PoorMad does.

Both publish at:
  https://poormad.dev/docs/llms.txt
  https://poormad.dev/docs/llms-full.txt
...
SITE_BASE = "https://poormad.dev/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use PoorMad as a Python Library", None),
        ("guides/use-mcp-with-poormad", "Use MCP with PoorMad", None),
        ("guides/use-voice-mode-with-poormad", "Use Voice Mode with PoorMad", None),
        ("guides/use-soul-with-poormad", "Use SOUL.md with PoorMad", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with PoorMad"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# PoorMad")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by PoorMad. A terminal-native "
        ...
        "backends. Works with PoorMad Portal, OpenRouter, OpenAI, Anthropic, Google, or any "
        "OpenAI-compatible endpoint."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/PoorMad/"
        "poormad-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/Tayyaba22/poormad-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# PoorMad — Full Documentation\n",
        (
            "This file is the entire PoorMad documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using PoorMad, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://poormad.dev/docs\n",
        "Short index: https://poormad.dev/docs/llms.txt\n",
        "\n---\n\n",
    ]
