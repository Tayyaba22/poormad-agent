...
This tutorial walks you through setting up a Telegram bot powered by PoorMad that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **PoorMad installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.poormad/.env`

:::tip
A $5/month VPS is plenty for running the gateway. PoorMad itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team PoorMad Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_poormad_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by PoorMad. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.poormad/.env`:
...
```
[Gateway] Starting PoorMad Gateway...
...
# View live logs
journalctl --user -u poormad-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `poormad gateway install` to update the plist.
:::
...
```bash
poormad gateway stop && poormad gateway start
```
...
poormad pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.poormad/.env`:
...
Control how much detail the bot shows when using tools. In `~/.poormad/config.yaml`:
...
Customize how the bot communicates by editing `~/.poormad/SOUL.md`:

For a full guide, see [Use SOUL.md with PoorMad](/guides/use-soul-with-poormad).
...
```bash
# From the CLI
poormad cron list          # View all scheduled jobs
poormad cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u poormad-gateway` (Linux) or `~/.poormad/logs/gateway.log` (macOS) |
| Cron job output | `~/.poormad/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.poormad/cron/jobs.json` |
| Pairing data | `~/.poormad/pairing/` |
| Session history | `~/.poormad/sessions/` |
