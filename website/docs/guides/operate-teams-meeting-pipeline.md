...
```bash
poormad teams-pipeline validate
```
...
```bash
poormad teams-pipeline token-health
poormad teams-pipeline token-health --force-refresh
```
...
```bash
poormad teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
poormad teams-pipeline maintain-subscriptions
poormad teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: PoorMad cron (recommended if you already run the PoorMad gateway)

PoorMad ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.poormad/scripts/`. First create the script:

```bash
mkdir -p ~/.poormad/scripts
cat > ~/.poormad/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec poormad teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.poormad/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
poormad cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/poormad-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=PoorMad Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=poormad
EnvironmentFile=/etc/poormad/env
ExecStart=/usr/local/bin/poormad teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/poormad-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run PoorMad Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now poormad-teams-pipeline-maintain.timer
systemctl list-timers poormad-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/poormad teams-pipeline maintain-subscriptions >> /var/log/poormad/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.poormad/.env` at the top of a wrapper script that crontab calls.
...
```bash
poormad teams-pipeline subscriptions   # should show expirationDateTime advanced
poormad teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
poormad teams-pipeline list
poormad teams-pipeline list --status failed
poormad teams-pipeline show <job-id>
```

### Replay a stored job

```bash
poormad teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
poormad teams-pipeline fetch --meeting-id <meeting-id>
poormad teams-pipeline fetch --join-web-url "<join-url>"
poormad teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `poormad teams-pipeline maintain-subscriptions --dry-run`
- inspect `poormad teams-pipeline list --status failed`
...
- run `poormad teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `poormad teams-pipeline run`
...
- [ ] `poormad teams-pipeline validate` returns an OK snapshot
- [ ] `poormad teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (PoorMad cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
