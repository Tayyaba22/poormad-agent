...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `poormad setup --portal`. See [PoorMad Portal](/integrations/nous-portal).
:::
...
2. **PoorMad spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.poormad/cron/output/` and you can read them anytime.
:::
...
PoorMad will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by PoorMad
```
...
Before creating cron jobs, ensure PoorMad has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell PoorMad what you want:
...
PoorMad will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell PoorMad to delegate each topic to a sub-agent:
...
PoorMad will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
