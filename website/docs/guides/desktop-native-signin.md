...
description: "How the PoorMad Desktop app signs in to a gated gateway using your system browser and PKCE — no embedded webview, no session cookies"
---

# Desktop Native Sign-In (RFC 8252)

When the PoorMad Desktop app connects to a **gated gateway** (a hosted or
...
For PoorMad specifically, native sign-in means:
...
```
Desktop app                Gateway (/auth/native/*)          PoorMad Portal (IDP)
...
desktop app* and an OAuth client *to the upstream identity provider* (PoorMad
...
dashboard-auth subsystem. OAuth providers (e.g. the bundled **PoorMad** provider)
...
- [Run PoorMad with PoorMad Portal](./run-poormad-with-nous-portal.md)
