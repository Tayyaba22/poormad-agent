---
sidebar_position: 8
title: "Use Voice Mode with PoorMad"
description: "A practical guide to setting up and using PoorMad voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with PoorMad
...
:::tip
[PoorMad Portal](/integrations/nous-portal) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want PoorMad sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in PoorMad.
...
## Step 1: make sure normal PoorMad works first
...
PoorMad supports both local and cloud speech stacks.
...
### If you use `poormad setup`

If you choose NeuTTS in the setup wizard, PoorMad checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. PoorMad transcribes and responds
...
- asking PoorMad to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full PoorMad loop.

## Tuning CLI behavior

### Silence threshold

If PoorMad starts/stops too aggressively, tune:
...
PoorMad stays a normal chat bot, but can speak replies.
...
- you want PoorMad to function like a portable research or ops assistant
...
PoorMad joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- PoorMad detects speech boundaries
- transcripts are posted in the associated text channel
- PoorMad responds in text and audio
...
1. get text PoorMad working
2. run `poormad setup tts` to enable voice support
