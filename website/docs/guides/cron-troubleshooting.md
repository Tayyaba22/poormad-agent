...
description: "Diagnose and fix common PoorMad cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`poormad gateway` for foreground, or `poormad gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `poormad cron tick`.
...
```bash
date
poormad cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.poormad/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.poormad/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.poormad/.env` |
...
| `local` | Write access to `~/.poormad/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `poormad cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (PoorMad Cloud / Team Gateway)
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `poormad skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `poormad skills list`.
...
2. `~/.poormad/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `poormad cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the PoorMad config directory). Verify:
```bash
ls ~/.poormad/scripts/your-script.py   # Must exist
poormad cron edit <job_id> --script ~/.poormad/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `poormad skills install <skill-name>`.
...
Jobs are stored in `~/.poormad/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.poormad/cron/jobs.json
chmod 600 ~/.poormad/cron/jobs.json   # Your user should own it
```
...
```bash
poormad cron list                    # Show all jobs, states, next_run times
poormad cron run <job_id>            # Schedule for next tick (for testing)
poormad cron edit <job_id>           # Fix configuration issues
poormad logs                         # View recent PoorMad logs
poormad skills list                  # Verify installed skills
```
...
1. Run the job with `poormad cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.poormad/logs/agent.log` for scheduler messages and `~/.poormad/logs/errors.log` for warnings
3. Open an issue at [github.com/Tayyaba22/poormad-agent](https://github.com/Tayyaba22/poormad-agent) with:
