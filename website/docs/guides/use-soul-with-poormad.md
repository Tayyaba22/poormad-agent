---
sidebar_position: 7
title: "Use SOUL.md with PoorMad"
description: "How to use SOUL.md to shape PoorMad's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with PoorMad

`SOUL.md` is the **primary identity** for your PoorMad instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want PoorMad to feel like the same assistant every time you talk to it — or if you want to replace the PoorMad persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm PoorMad should be
- what PoorMad should avoid stylistically
- how PoorMad should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who PoorMad is and how PoorMad speaks
...
PoorMad now uses only the global SOUL file for the current instance:

```text
~/.poormad/SOUL.md
```

If you run PoorMad with a custom home directory, it becomes:
...
PoorMad automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, PoorMad does not overwrite it
- if the file exists but is empty, PoorMad adds nothing from it to the prompt

## How PoorMad uses it

When PoorMad starts a session, it reads `SOUL.md` from `POORMAD_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how PoorMad feels.
...
PoorMad already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How PoorMad should sound.

# Avoid
What PoorMad should not do.

# Defaults
How PoorMad should behave when ambiguity appears.
```
...
Then restart PoorMad or start a new session.
...
### I edited SOUL.md but PoorMad still sounds the same

Check:
- you edited `~/.poormad/SOUL.md` or `$POORMAD_HOME/SOUL.md`
...
### PoorMad is ignoring parts of my SOUL.md
