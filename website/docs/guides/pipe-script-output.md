...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `poormad send`."
---

# Pipe Script Output to Messaging Platforms

`poormad send` is a small, scriptable CLI that pushes a message to any
messaging platform PoorMad is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | poormad send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `poormad
...
poormad send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
poormad send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
poormad send --list

# Filter by platform
poormad send --list telegram
```
...
Any platform PoorMad ships adapters for works as a target:
...
`poormad send` resolves the message body in this order:

1. **Positional argument** — `poormad send --to telegram "hi"`
2. **`--file PATH`** — `poormad send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | poormad send --to telegram`

When stdin is a TTY (no pipe), PoorMad does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  poormad send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `poormad send` reuses your PoorMad config, the same script works on
any host where PoorMad is installed — no need to export bot tokens into
...
`poormad send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  poormad send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | poormad send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.poormad/bin/poormad send \
...
```bash
./train.py --epochs 200 && \
  poormad send --to telegram "training done" || \
  poormad send --to telegram "training failed (exit $?)"
```
...
poormad send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(poormad send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `poormad send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `poormad send` calls
the platform's REST endpoint directly using credentials from
`~/.poormad/.env` and `~/.poormad/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `poormad gateway start` and retry.
...
# Machine-readable
poormad send --list --json
```

The listing is built from `~/.poormad/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`poormad gateway
...
| Approach | Multi-platform | Reuses PoorMad creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `poormad send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`poormad send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `poormad send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `poormad send` shares with cron delivery.
