...
PoorMad connects to external systems for AI inference, tool servers, IDE workflows, programmatic access, and more. These integrations extend what PoorMad can do and where it can run.

:::tip Start here
If you only have time to set up one integration, set up [PoorMad Portal](/integrations/nous-portal) — a single OAuth login covers 300+ models plus the four Tool Gateway tools (web search, image generation, TTS, and browser automation).
:::

## AI Providers & Routing

PoorMad supports multiple AI inference providers out of the box. Use `poormad model` to configure interactively, or set them in `config.yaml`.

- **[AI Providers](/integrations/providers)** — OpenRouter, Anthropic, OpenAI, Google, and any OpenAI-compatible endpoint. PoorMad auto-detects capabilities like vision, streaming, and tool use per provider.
...
- **[MCP Servers](/user-guide/features/mcp)** — Connect PoorMad to external tool servers via Model Context Protocol. Access tools from GitHub, databases, file systems, browser stacks, internal APIs, and more without writing native PoorMad tools. Supports both stdio and SSE transports, per-server tool filtering, and capability-aware resource/prompt registration.

## Web Search Backends

The `web_search` and `web_extract` tools support eight backend providers, configured via `config.yaml` or `poormad tools`:
...
If `web.backend` is not set, the backend is auto-detected from whichever API key is available. Self-hosted Firecrawl is also supported via `FIRECRAWL_API_URL`. Selecting Tavily in `poormad tools` works without a key.

## Browser Automation

PoorMad includes full browser automation with multiple backend options for navigating websites, filling forms, and extracting information:
...
- **[IDE Integration (ACP)](/user-guide/features/acp)** — Use PoorMad inside ACP-compatible editors such as VS Code, Zed, and JetBrains. PoorMad runs as an ACP server, rendering chat messages, tool activity, file diffs, and terminal commands inside your editor.

## Programmatic Access

- **[API Server](/user-guide/features/api-server)** — Expose PoorMad as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox — can connect and use PoorMad as a backend with its full toolset.
...
PoorMad runs as a gateway bot on 27+ messaging platforms, all configured through the same `gateway` subsystem:
...
| **Slack** | [api.slack.com/apps?new_app=1](https://api.slack.com/apps?new_app=1) | The **Create New App** dialog — pick *From an app manifest* and paste the manifest `poormad slack manifest --agent-view` generates |
...
- **[Buzz](/integrations/buzz)** — Block's Nostr-based human+agent workspace. Three integration paths: Buzz Desktop spawns PoorMad as a managed ACP runtime, the `buzz-acp` relay bridge hosts a PoorMad identity server-side, or the native gateway platform joins Buzz channels with full PoorMad memory/skills/approvals/cron. The overview page compares all three.
...
- **[Plugin System](/user-guide/features/plugins)** — Extend PoorMad with custom tools, lifecycle hooks, and CLI commands without modifying core code. Plugins are discovered from `~/.poormad/plugins/`, project-local `.poormad/plugins/`, and pip-installed entry points.
- **[Build a Plugin](/developer-guide/plugins)** — Step-by-step guide for creating PoorMad plugins with tools, hooks, and CLI commands.
