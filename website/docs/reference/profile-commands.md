...
This page covers all commands related to [PoorMad profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `poormad profile` without a subcommand shows help.
...
```bash
poormad profile use <name>
```

Sets `<name>` as the active profile. All subsequent `poormad` commands (without `-p`) will use this profile.
...
poormad profile use default
```

## `poormad profile create`

```bash
poormad profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `poormad profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `poormad update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.poormad`), use `poormad skills opt-out` / `poormad skills opt-in`. |
...
poormad profile create work2 --clone-from work

# Clone everything from a specific profile
poormad profile create work2-backup --clone-from work --clone-all
```

## `poormad profile describe`

```bash
poormad profile describe [<name>] [options]
```
...
poormad profile describe researcher

# Set it explicitly
poormad profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
poormad profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
poormad profile describe --all --auto
```

## `poormad profile delete`

```bash
poormad profile delete <name> [options]
```
...
```bash
poormad profile delete mybot
poormad profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.poormad`) cannot be deleted — use `poormad uninstall` to remove everything.
:::

## `poormad profile show`

```bash
poormad profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's PoorMad home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ poormad profile show work
Profile: work
Path:    ~/.poormad/profiles/work
...
## `poormad profile alias`

```bash
poormad profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your PoorMad installation.
...
```bash
poormad profile alias work
# Creates/updates ~/.local/bin/work

poormad profile alias work --name mywork
# Creates ~/.local/bin/mywork

poormad profile alias work --remove
# Removes the wrapper script
```

## `poormad profile rename`

```bash
poormad profile rename <old-name> <new-name>
```
...
```bash
poormad profile rename mybot assistant
# ~/.poormad/profiles/mybot → ~/.poormad/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `poormad profile export`

```bash
poormad profile export <name> [options]
```
...
```bash
poormad profile export work
# Creates work.tar.gz in the current directory

poormad profile export work -o ./work-2026-03-29.tar.gz
```
...
## `poormad profile import`

```bash
poormad profile import <archive> [options]
```
...
```bash
poormad profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

poormad profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `poormad -p telemetry`). |
...
poormad profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
poormad profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
poormad profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
poormad profile install ./telemetry/
```

### `poormad profile update`

```bash
poormad profile update <name> [--force-config] [--yes]
```
...
### `poormad profile info`

```bash
poormad profile info <name>
```

Prints the profile's distribution manifest — name, version, required
PoorMad version, author, env var requirements, the source URL/path, and
...
`poormad profile list` also shows the distribution name and version in a
`Distribution` column, and `poormad profile show <name>` / `delete <name>`
...
poormad profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
poormad profile install https://github.com/your-org/internal-assistant.git
```
...
`poormad_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
PoorMad version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host PoorMad can clone from.
3. Tell recipients to run `poormad profile install <your-repo-url>`.
...
## `poormad -p` / `poormad --profile`

```bash
poormad -p <name> <command> [options]
poormad --profile <name> <command> [options]
```

Global flag to run any PoorMad command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
poormad -p work chat -q "Check the server status"
poormad --profile dev gateway start
poormad -p personal skills list
poormad -p work config edit
```

## `poormad completion`

```bash
poormad completion <shell>
```
...
- `poormad profile <TAB>` — subcommands (list, use, create, etc.)
- `poormad profile use <TAB>` — profile names
- `poormad -p <TAB>` — profile names
