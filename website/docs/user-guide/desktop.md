---
sidebar_position: 3
title: "PoorMad Desktop"
description: "The native PoorMad desktop app — a polished experience for chatting with PoorMad, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# PoorMad Desktop

The PoorMad desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same PoorMad core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `poormad` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
PoorMad has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`poormad`) and **[TUI](./tui.md)** (`poormad --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`poormad dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Download the app from the [PoorMad Desktop product page](https://poormad.dev/desktop), or follow the [installation instructions for PoorMad Desktop](../getting-started/installation.md).

If you already have PoorMad installed, simply run
...
- **The same conversation history** as every other PoorMad surface — sessions started here resume in the CLI/TUI and vice versa.
...
Chatting against a PoorMad instance on another machine instead of the bundled local backend? See [Connecting to a remote backend](#connecting-to-a-remote-backend) below — and for the full picture of how the remote-hosted dashboard connection works (the auth gate, the `/api/ws` chat socket, and WebSocket close-code triage), see [Web Dashboard → Connecting PoorMad Desktop to a remote backend](./features/web-dashboard.md#connecting-poormad-desktop-to-a-remote-backend).

#### Repository discovery

PoorMad Desktop discovers local Git repositories for the Projects sidebar by scanning your home directory to a bounded depth. You can change this per profile in **Settings → Workspace**, or in `config.yaml`:
...
- Set `repo_scan_enabled: false` to stop the filesystem scan completely. Existing disk-discovery cache rows for that profile are cleared; explicit projects and repositories inferred from intentional PoorMad sessions remain available.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `poormad desktop --cwd <path>` (or the `POORMAD_DESKTOP_CWD` environment variable).
...
- **Review pane** — **Cmd/Ctrl+G** toggles the working-tree review pane: branch and ahead/behind status, changed files (list or tree view), and diffs scoped to **Uncommitted**, **Branch**, or **Last turn** (just what the agent changed in its most recent turn). Stage/unstage files, revert changes, write a commit message (or **Generate commit message**), then **Commit** or **Commit & Push** — and **Create PR** via the GitHub CLI (`gh`), or hand the whole thing to the agent with **Ask PoorMad to open PR**. You can also create and switch branches from here.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `poormad model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader PoorMad management surface so you don't have to drop to a terminal:
...
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#poormad-cron).
- **Profiles** — switch between [PoorMad profiles](./profiles.md) (isolated config/skills/sessions).
...
roster where every [PoorMad profile](./profiles.md) appears as a bot with its
...
(recurring tasks backed by PoorMad cron). The roster lives in the left
...
each other's Bot Chats directly (`poormad -p <bot> chat`). The backend teaches
...
- **Command palette** — press **Cmd+K** or **Cmd+P** (Ctrl+K / Ctrl+P on Windows/Linux) to jump to actions and navigate the app from the keyboard: open any page or settings section, jump to a session by title or id, switch model/theme/color mode, spawn a terminal, restart the gateway, update PoorMad, and more.
...
The [manual update process](https://poormad.dev/docs/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the PoorMad, your config, and your chats stay. (Same as `poormad uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `poormad uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `poormad uninstall --full`.)
...
You can do the same from the terminal — `poormad uninstall --gui` for the GUI alone, or `poormad uninstall` / `poormad uninstall --full` for the agent too.

:::note
Running `poormad uninstall --gui` from a **source checkout** (a `poormad desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `poormad desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `poormad desktop`

To launch via the CLI, simply run `poormad desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `poormad update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `POORMAD_DESKTOP_CWD`)           |
| `--poormad-root PATH` | Override the PoorMad source root the app uses (sets `POORMAD_DESKTOP_POORMAD_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `poormad` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the PoorMad runtime into `POORMAD_HOME` (`~/.poormad`, or `%LOCALAPPDATA%\poormad` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `POORMAD_DESKTOP_POORMAD_ROOT`, then a completed managed install, then a probed `poormad` on `PATH` (unless `--ignore-existing` / `POORMAD_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `POORMAD_DESKTOP_POORMAD` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `poormad serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `poormad --tui`. The desktop app is **self-contained**: it runs its own `poormad serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a PoorMad backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.
...
- **Remote gateway** — enter the URL of a `poormad serve` backend you run yourself and sign in. This is the mode the rest of this section walks through.
- **PoorMad Cloud** — sign in once to PoorMad Cloud and pick from the agents on your account; no URL to paste. The app discovers your agents (with an organization picker if your account spans several orgs), and connecting to one switches the session over automatically. The status bar shows the cloud connection while it's active.
...
Further down the same **Settings → Gateways** page, **Registered gateways** manages a named list of every PoorMad gateway the app knows about — the local runtime, any number of remote gateways (LAN, Tailscale, internet), PoorMad Cloud instances, and SSH hosts — all persisted together in one place. You can jump there from the plug button at the right end of the sidebar profile rail (**Connect another PoorMad gateway…**) or via **⌘K → Gateways**. The full guide, including the union agent roster, `@name-device` handles, fleet-wide updates, and the plugin SDK surface, is at [Connecting Desktop to Many PoorMad Instances](./multi-connection-desktop.md).
...
- Cloud entries come from the PoorMad Cloud sign-in/discovery flow above, not from a hand-typed URL.
- Tokens are stored encrypted with the OS keyring (with an explicit plain-text opt-in on keyring-less Linux).

Side-by-side routing is live: each registered gateway dials its own backends and sockets on demand (keyed per connection + profile), the plugin SDK exposes the union agent roster (`host.agents()` / `host.ensureAgent()`), and **Update all instances** on the Gateways page dispatches `poormad update` to every eligible gateway at once — PoorMad Cloud entries are skipped (the platform updates them), and each instance reports its own result.


:::info The remote backend is a running `poormad serve` process
"Remote backend" means a **`poormad serve`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that backend is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `poormad serve` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
- **OAuth (PoorMad Portal) — preferred for anything reachable beyond your own machine.** Logins are verified against your PoorMad account, so this is the option suitable for a VPS, a public host, or any remote backend. Register the dashboard with `poormad dashboard register` (or the Portal [`/local-dashboards`](https://portal.poormad.dev/local-dashboards) page) to provision its OAuth client, then sign in from the app with **Sign in with PoorMad**. A self-hosted OIDC provider works the same way if you run your own identity provider.
...
The rest of this section shows the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Web Dashboard → Default provider: PoorMad](./features/web-dashboard.md#default-provider-nous-research).

### On the backend (the remote machine)

Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.poormad/.env` (the secrets file, mode 0600):
...
poormad serve --host 0.0.0.0 --port 9119
```

Keep that `poormad serve` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.

Separately, make sure the **gateway is running** on the remote host if you rely on messaging channels — the `poormad serve` backend is what the desktop app talks to, but your Telegram/Discord/Slack gateway sessions are a different process that you start and keep running on their own. See [Messaging](./messaging/index.md) for gateway setup.

Prefer not to keep a plaintext password at rest? Set `POORMAD_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` to a scrypt hash instead — compute it with `python -c "from plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"`. Full configuration surface (config.yaml keys, every env var, the rate limiter): [Web Dashboard → Username/password provider](./features/web-dashboard.md#usernamepassword-provider-no-oauth-idp).

Running the backend as a systemd service? Give the unit `EnvironmentFile=%h/.poormad/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/poormad` work if you front it with a reverse proxy)
2. **Sign in** — the app detects which provider the backend advertises and adapts the button. For a username/password backend it shows a **Sign in** button that opens a credential form (enter the credentials from step 1). For an OAuth backend it shows **Sign in with `<provider>`** (e.g. *Sign in with PoorMad*), which runs the provider's browser sign-in. Either way the app ends up with an authenticated session against the backend.
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `POORMAD_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `POORMAD_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateways settings panel.
...
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.poormad/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `POORMAD_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting PoorMad Desktop to a remote backend](./features/web-dashboard.md#connecting-poormad-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & PoorMad Desktop](../reference/environment-variables.md#web-dashboard--poormad-desktop).
...
`$POORMAD_HOME/desktop-plugins/<id>/plugin.js`; the app loads it within seconds and
...
- **Open logs** — opens `POORMAD_HOME/logs` in your file manager. On a remote
...
- **Send diagnostics** — uploads a redacted debug bundle to PoorMad-internal
  storage after an explicit consent prompt (same pipeline as
  `poormad debug share --nous`; secrets are always redacted, the bundle is
  viewable by PoorMad staff only and auto-deletes after 14 days). On success you
  get a private view link to paste into your support thread, plus quick links
  to GitHub Issues, PoorMad Portal Support, and Discord. On a remote or Cloud
...
Boot logs land in `POORMAD_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
rm "$HOME/.poormad/poormad-agent/.poormad-bootstrap-complete"

# Rebuild a broken Python venv (macOS/Linux)
rm -rf "$HOME/.poormad/poormad-agent/venv"

# Reset a stuck macOS microphone prompt
tccutil reset Microphone com.poormad.poormad
```
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.poormad/poormad-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
...
```bash
POORMAD_DESKTOP_POORMAD_ROOT=/path/to/clone npm run dev
POORMAD_HOME=/tmp/throwaway npm run dev
...
(Desktop, then Downloads, then Documents, ...) as PoorMad touches each folder.
A single **Full Disk Access** grant covers all of them, permanently — and
with PoorMad' stable signing identities it survives every update:

1. System Settings → **Privacy & Security → Full Disk Access** (or run
   `open "x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles"`)
2. Enable your terminal app — and **PoorMad.app** if you use Desktop.
3. Fully quit and relaunch them once.

`poormad doctor` reports whether the current terminal context already has the
grant, and `poormad setup` shows this tip on macOS when it doesn't.
...
```bash
tccutil reset ScreenCapture com.poormad.poormad   # repeat per service
```

then toggle the fresh entry ON in System Settings and fully quit & relaunch
PoorMad. Grants are stable from then on.
...
certificate once and tell PoorMad to use it. The one-shot command does
...
```bash
poormad desktop --setup-tcc-identity
```

Or do it manually:

1. Keychain Access → Certificate Assistant → **Create a Certificate…**
2. Name: `PoorMad Local Signing`, Identity Type: *Self-Signed Root*,
...
4. `poormad config set desktop.macos_signing_identity "PoorMad Local Signing"`

Use `--identity <name>` with the command to create/use a differently named
certificate (default: `PoorMad Local Signing`). The command is idempotent —
...
`tccutil reset All com.poormad.poormad` and re-grant.

## See also

- [CLI Guide](./cli.md) — the terminal interface
- [TUI](./tui.md) — the modern terminal UI used by `poormad --tui` and the dashboard chat tab
