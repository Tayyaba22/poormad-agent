...
# Connecting Desktop to Many PoorMad Instances

Register every PoorMad backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and PoorMad Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another PoorMad gateway…"**) deep-links straight to
...
list of PoorMad gateways. Its intro says it plainly: *"Manage this device and
every PoorMad gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The PoorMad runtime managed by this app." | automatic |
| **Remote gateway** | "A PoorMad gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A PoorMad install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **PoorMad Cloud** | "A hosted instance discovered through your PoorMad Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the PoorMad Cloud sign-in/discovery flow at
  the top of the Gateways page — the **PoorMad Cloud** kind in the add-connection
...
3. Pick the kind: **Local**, **PoorMad Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **PoorMad Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `poormad serve` backend,
     ...
     - **Authentication** — choose **Session token** or **OAuth**:
       - **Session token** — paste the dashboard session token from the
         ...
         token."*
       - **OAuth** — sign in through the PoorMad Portal browser flow; no token
         to paste.
   - *SSH only:*
...
Gateways are machines or hosted backends; profiles are isolated PoorMads
...
connection is registered) dispatches `poormad update` to every eligible
...
- **PoorMad Cloud** instances are skipped with a *"Managed by PoorMad Cloud"*
...
panel, ⌘K **Update PoorMad**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `poormad serve` is running on the remote host, the
...
- **"Update PoorMad Desktop to chat with agents on other connections"** — the
