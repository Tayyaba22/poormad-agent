...
sidebar_position: 1
title: "CLI Interface"
description: "Master the PoorMad terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

PoorMad's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `poormad setup --portal` — and you're ready to `poormad chat`. See [PoorMad Portal](/integrations/nous-portal).
:::

:::tip
PoorMad also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `poormad --tui` — see the [TUI](tui.md) guide.
:::
...
poormad chat --query-file prompt.txt
poormad chat --query-file - < prompt.txt

# With a specific model
poormad chat --model "anthropic/claude-sonnet-4"

# With a specific provider
poormad chat --provider nous        # Use PoorMad Portal
poormad chat --provider openrouter  # Force OpenRouter

# With specific toolsets
poormad chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
poormad -s poormad-agent-dev,github-auth
poormad chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
poormad --continue             # Resume the most recent CLI session (-c)
poormad --resume <session_id>  # Resume a specific session by ID (-r)
poormad --resume latest        # Resume the most recent session (same as -c)
poormad --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
poormad chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
poormad -w                         # Interactive mode in worktree
poormad -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`poormad -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
poormad worktree list              # audit: age, size, verdict, reason per tree
poormad worktree prune             # remove safe trees + delete merged branches
poormad worktree prune --dry-run   # show the plan without changing anything
poormad worktree prune --trees-only     # leave local branches alone
poormad worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running poormad session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.poormad/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
The `poormad plugins` commands manage native PoorMad plugins and portable Agent
...
poormad plugins disable <plugin-name>
poormad plugins update <plugin-name>
poormad plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. PoorMad currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the PoorMad CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The PoorMad CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `POORMAD_YOLO_MODE` is on (either `poormad --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend PoorMad to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.poormad/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. PoorMad supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for PoorMad replies |
...
```bash
poormad -s poormad-agent-dev,github-auth
poormad chat -s github-pr-workflow -s github-auth
```

PoorMad loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.poormad/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.poormad/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain PoorMad..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. PoorMad recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches PoorMad — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while PoorMad is working, PoorMad prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend PoorMad to the background — just like any terminal process. The shell prints a confirmation:

```
PoorMad has been suspended. Run `fg` to bring PoorMad back.
```
...
Resume this session with:
  poormad --resume 20260225_143052_a1b2c3
...
```bash
poormad --continue                          # Resume the most recent CLI session
poormad -c                                  # Short form
poormad -c "my project"                     # Resume a named session (latest in lineage)
poormad --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
poormad --resume "refactoring auth"         # Resume by title
poormad --resume latest                     # Resume the most recent session (same as -c)
poormad --resume latest --in ./my-project   # Latest session for ./my-project's workspace
poormad -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `poormad sessions rename <id> <title>` from the command line. Use `poormad sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in PoorMad's SQLite state database under `~/.poormad/state.db`. The database keeps:
...
PoorMad immediately confirms the task and gives you back the prompt:
