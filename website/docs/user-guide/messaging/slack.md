...
sidebar_position: 4
title: "Slack"
description: "Set up PoorMad as a Slack bot using Socket Mode"
---

# Slack Setup

Connect PoorMad to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your PoorMad instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. PoorMad uses the modern
...
The fastest path is to paste a manifest PoorMad generates for you. It
...
### Option A: From a PoorMad-generated manifest (recommended)

1. Generate the manifest. New Slack apps must use Agent view:
   ```bash
   poormad slack manifest --agent-view --write
   ```
   This writes `~/.poormad/slack-manifest.json` and prints paste-in
   ...
   ```bash
   poormad slack manifest --agent-view \
...
4. Enter an app name (e.g., "PoorMad") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, PoorMad can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `poormad-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a PoorMad configuration issue.
:::
...
PoorMad uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
poormad gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
poormad gateway              # Foreground
poormad gateway install      # Install as a user service
sudo poormad gateway install --system   # Linux only: boot-time system service
```
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; PoorMad now
...
Every PoorMad command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
PoorMad command with its description.

Under the hood: PoorMad ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/Tayyaba22/poormad-agent/blob/main/poormad_cli/commands.py)
...
New Slack apps use Slack's **Agent** messaging experience. Existing PoorMad
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so PoorMad can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. PoorMad only supplies that context as a label; it does not read the viewed
channel's history.

### Refreshing slash commands after updates

When PoorMad adds new commands (e.g. after `poormad update`), regenerate
the manifest and update your Slack app:

```bash
poormad slack manifest --write
```
...
3. Paste the new contents of `~/.poormad/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/poormad <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/poormad btw run the tests` — PoorMad routes it the same way as `/btw
run the tests`. Free-form questions also work: `/poormad what's the
...
Slack never delivers them to PoorMad.

As a workaround, PoorMad recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
PoorMad treats it identically to the slash form and replies in the same
thread.
...
The bang form also works behind a mention (`@PoorMad !stop`) and with
...
PoorMad falls back to a text prompt, the prompt instructs you to reply
...
fails, PoorMad retries via a second ephemeral API path — a slash reply is never
posted publicly to the channel as a fallback. (Commands typed as regular
messages — `!cmd` in threads, `@PoorMad /cmd` — reply as normal visible
messages instead.)
...
```bash
poormad slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how PoorMad behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@PoorMad what time is it?`). In channels, PoorMad replies in a thread attached to that message. |
| **Threads** | If you @mention PoorMad inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.poormad/config.yaml`.
...
platforms:
  slack:
    ...
    extra:
      ...
      # If Slack rejects the native stream, PoorMad keeps one editable text
      ...
      # that message itself @mentions PoorMad, and "all" accepts every
      # other bot. PoorMad always ignores its own bot user to prevent
      # self-echoes.
      allow_bots: "none"
...
| `platforms.slack.extra.allow_bots` | `"none"` | Controls messages from other Slack bots: `"none"` ignores them, `"mentions"` accepts a bot message only when **that message itself** @mentions PoorMad, and `"all"` accepts all of them. Use `"mentions"` for the safest bot-to-bot collaboration mode. See [Accepting messages from other bots](#accepting-messages-from-other-bots-allow_bots). |
...
name in the thread. By default PoorMad sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"PoorMad is thinking…"`) —
...
`streaming.enabled` is on (transport `auto` or `draft`), PoorMad uses native
streaming automatically wherever it's available:

- The stream starts on the first frame and appends only deltas (the API is
  append-only). The streamed message **is** the final message — PoorMad seals
...
- If your Slack app doesn't have the AI features enabled (or lacks the
  `assistant:write` scope), the first failure is cached and PoorMad falls back
...
- If the native stream can't start or update, PoorMad falls back to a single
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to PoorMad in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before PoorMad will respond.
...
By default PoorMad ignores every message authored by another Slack bot or app (including Workflow Builder posts). For multi-agent workspaces — several PoorMad instances or peer bots collaborating in one channel — opt in with `allow_bots`:
...
- PoorMad always ignores its own messages, in every mode, to prevent self-echo loops.

`mentions` is the recommended mode for bot-to-bot collaboration: each agent must explicitly summon the other per turn. Avoid `all` unless every peer bot's own reply policy is loop-safe — two bots that answer everything will answer each other forever. Detection covers labeled bot messages (`bot_id`, `subtype: bot_message`), app-originated events, and unlabeled bot *users* (probed via `users.info`), so peer PoorMads are filtered consistently across workspaces.
...
manifest — regenerate with `poormad slack manifest`):
...
Set `SLACK_HOME_CHANNEL` to a channel ID where PoorMad will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @PoorMad`).
...
Delivery works even when the cron process isn't co-located with the gateway — PoorMad falls back to a standalone Web API sender using `SLACK_BOT_TOKEN`. `MEDIA:` attachments in the cron output are uploaded as native Slack file shares to the same target.
...
PoorMad can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, PoorMad also loads tokens from an **OAuth token file** at:

```
~/.poormad/slack_tokens.json
```
...
- When a message arrives, PoorMad uses the correct workspace-specific client to respond.
...
PoorMad supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @PoorMad` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @PoorMad` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. PoorMad now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @PoorMad`)
...
- Tokens should be stored in `~/.poormad/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your PoorMad config directory
