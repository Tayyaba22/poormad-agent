...
sidebar_position: 16
title: "Yuanbao"
description: "Connect PoorMad to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect PoorMad to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.poormad/.env`:
...
All standard PoorMad commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.poormad/logs/gateway.log`
...
4. Consider enabling verbose logging: `poormad gateway run -vv`
...
Yuanbao has a maximum message size. PoorMad automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
poormad chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
