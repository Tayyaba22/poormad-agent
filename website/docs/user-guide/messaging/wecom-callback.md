...
Connect PoorMad to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
PoorMad supports two WeCom integration modes:
...
> Run `poormad gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. PoorMad decrypts the message, queues it for the agent
...
(Use `poormad gateway start` only after `poormad gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in PoorMad and the token the
...
in `~/.poormad/.env` values around `=` will also break signature checks. After
fixing, restart `poormad gateway run`.
...
Check `poormad gateway run` logs for the bound host/port. If the adapter bound to
