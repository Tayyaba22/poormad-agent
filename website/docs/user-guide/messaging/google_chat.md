...
sidebar_position: 12
title: "Google Chat"
description: "Set up PoorMad as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect PoorMad to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your PoorMad
...
> Run `poormad gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only PoorMad can read (e.g.,
`~/.poormad/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a poormad restart)
...
- Principal: `poormad-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — PoorMad calls
...
- **App name**: whatever you want users to see ("PoorMad" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/poormad-chat-events`.
...
`ADDED_TO_SPACE` event that PoorMad uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure PoorMad

Add the Google Chat section to `~/.poormad/.env`:
...
Send "hola" in the test DM. The bot posts a "PoorMad is thinking…" marker, then
...
`~/.poormad/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "PoorMad is thinking…").
...
Thread support: when a user replies inside a thread, PoorMad detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate PoorMad session.
...
3. Download the JSON. Move it onto the host that runs PoorMad.
4. Register the client with PoorMad (run under the profile you want it scoped to):
...
poormad -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's PoorMad home (e.g.
`~/.poormad/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.poormad/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.poormad/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, PoorMad isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `poormad gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: PoorMad will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.poormad/google_chat_user_tokens/<sanitized_email>.json` (filesystem
