...
description: "Chat with PoorMad from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with PoorMad from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with PoorMad](/guides/use-voice-mode-with-poormad).

:::tip
Bots need both a model provider and tool providers (TTS, web). A [PoorMad Portal](/integrations/nous-portal) subscription bundles all of them.
:::
...
```mermaid
flowchart TB
    subgraph Gateway["PoorMad Gateway"]
...
For group chats, hooks, and automation flows, PoorMad supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. PoorMad keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; PoorMad does not hide failures just because the text resembles a silence token.
...
```bash
poormad gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
poormad gateway              # Run in foreground
poormad gateway setup        # Configure messaging platforms interactively
poormad gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo poormad gateway install --system   # Linux only: install a boot-time system service
poormad gateway start        # Start the default service
poormad gateway stop         # Stop the default service
poormad gateway status       # Check default service status
poormad gateway status --system         # Linux only: inspect the system service explicitly
```
...
```yaml title="~/.poormad/config.yaml"
gateway:
...
```bash
poormad gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. PoorMad sends heartbeats
...
| `/update` | Update PoorMad to the latest version |
...
with the `session_reset` section in `~/.poormad/config.yaml`:
...
Configure per-platform overrides in `~/.poormad/gateway.json`:
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.poormad/gateway-config.yaml`:
...
poormad pairing approve telegram XKGH5N7P

# Other pairing commands:
poormad pairing list          # View pending + approved users
poormad pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, PoorMad appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.poormad/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.poormad/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `POORMAD_HOME`:

- `~/.poormad/status_phrases.yaml` or any `*.yaml` in `~/.poormad/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to POORMAD_HOME
...
Off by default. When enabled, PoorMad prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.poormad/config.yaml`:
...
```bash
POORMAD_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
poormad gateway install               # Install as user service
poormad gateway start                 # Start the service
poormad gateway stop                  # Stop the service
poormad gateway status                # Check status
journalctl --user -u poormad-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo poormad gateway install --system
sudo poormad gateway start --system
sudo poormad gateway status --system
journalctl -u poormad-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit PoorMad installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit poormad-gateway` (or `sudo systemctl edit poormad-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `poormad update`. When `poormad update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart poormad-gateway` command (it never blocks on an interactive password prompt).
...
```bash
poormad gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `poormad update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo poormad update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/poormad-gateway`:

```
poormad ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed poormad-gateway*, /usr/bin/systemctl --no-ask-password start poormad-gateway*, /usr/bin/systemctl --no-ask-password restart poormad-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. PoorMad will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple PoorMad installations on the same machine (with different `POORMAD_HOME` directories), each gets its own systemd service name. The default `~/.poormad` uses `poormad-gateway`; other installations use `poormad-gateway-<hash>`. The `poormad gateway` commands automatically target the correct service for your current `POORMAD_HOME`.
:::

### macOS (launchd)

```bash
poormad gateway install               # Install as launchd agent
poormad gateway start                 # Start the service
poormad gateway stop                  # Stop the service
poormad gateway status                # Check status
tail -f ~/.poormad/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.poormad.gateway.plist`. It includes three environment variables:
...
- **POORMAD_HOME** — scopes the gateway to your PoorMad installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `poormad gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `POORMAD_HOME` directory gets its own launchd label. The default `~/.poormad` uses `ai.poormad.gateway`; other installations use `ai.poormad.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `poormad-cli` | Full access |
| Telegram | `poormad-telegram` | Full tools including terminal |
| Discord | `poormad-discord` | Full tools including terminal |
| WhatsApp | `poormad-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `poormad-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `poormad-slack` | Full tools including terminal |
| Google Chat | `poormad-google_chat` | Full tools including terminal |
| Signal | `poormad-signal` | Full tools including terminal |
| SMS | `poormad-sms` | Full tools including terminal |
| Email | `poormad-email` | Full tools including terminal |
| Home Assistant | `poormad-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `poormad-mattermost` | Full tools including terminal |
| Matrix | `poormad-matrix` | Full tools including terminal |
| DingTalk | `poormad-dingtalk` | Full tools including terminal |
| Feishu/Lark | `poormad-feishu` | Full tools including terminal |
| WeCom | `poormad-wecom` | Full tools including terminal |
| WeCom Callback | `poormad-wecom-callback` | Full tools including terminal |
| Weixin | `poormad-weixin` | Full tools including terminal |
| BlueBubbles | `poormad-bluebubbles` | Full tools including terminal |
| QQBot | `poormad-qqbot` | Full tools including terminal |
| Yuanbao | `poormad-yuanbao` | Full tools including terminal |
| Microsoft Teams | `poormad-teams` | Full tools including terminal |
| API Server | `poormad-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `poormad-webhook` | Full tools including terminal |
| Raft | `poormad-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.poormad/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
