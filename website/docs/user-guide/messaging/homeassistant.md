...
description: Control your smart home with PoorMad via Home Assistant integration.
...
PoorMad integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "PoorMad"
5. Copy the token
...
PoorMad registers four tools for smart home control:
...
Configure which events the agent sees in `~/.poormad/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "PoorMad".
...
The adapter reads credentials from `~/.poormad/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active PoorMad profile
...
running PoorMad — `curl -H "Authorization: Bearer <token>" <url>/api/` should
