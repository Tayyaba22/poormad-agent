...
PoorMad runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. PoorMad
...
1. PoorMad captures a baseline of current diagnostics for the file.
...
…). PoorMad auto-detects the binary on PATH or in
`<POORMAD_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point PoorMad at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<POORMAD_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`poormad lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — PoorMad installs both packages together when you
run `poormad lsp install typescript` or auto-install fires on first
use.

## CLI

```
poormad lsp status          # service state + per-server install status
poormad lsp list            # registry, optionally --installed-only
poormad lsp install <id>    # eagerly install one server
poormad lsp install-all     # try every server with a known recipe
poormad lsp restart         # tear down running clients
poormad lsp which <id>      # print resolved binary path
```

`poormad lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <POORMAD_HOME>/lsp/bin
...
When `install_strategy: auto`, PoorMad installs binaries into
`<POORMAD_HOME>/lsp/bin/`. NPM packages land in
`<POORMAD_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully PoorMad-owned and is
...
**`poormad lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<POORMAD_HOME>/lsp/bin/`. Run
`poormad lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `poormad lsp status`**
...
When `poormad lsp status` shows a `Backend warnings` section, install
...
`~/.poormad/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.poormad/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `poormad lsp restart` to clear the set;
