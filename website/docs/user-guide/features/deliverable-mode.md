...
When PoorMad runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.poormad/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.poormad/config.yaml` (switchable per session
via `/personality`).
...
If you use PoorMad' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.poormad/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. PoorMad's deliverable
