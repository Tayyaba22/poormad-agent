...
description: "Use your PoorMad Portal subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your PoorMad-managed provider subscription as their
...
| Use case | "Use PoorMad as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the PoorMad Portal OAuth flow. PoorMad stores
the refresh token in `~/.poormad/auth.json` — the same place all PoorMad
...
Model:      PoorMad-4-70B    # or PoorMad-4.3-36B, PoorMad-4-405B
```
...
Currently shipped: `nous` (PoorMad Portal) and `xai` (xAI / Grok). More
OAuth providers can be added by implementing the `UpstreamAdapter`
interface in `poormad_cli/proxy/adapters/`.
...
If you see `not logged in`, run `poormad portal`. If you see
...
The proxy only forwards paths the upstream actually serves. For PoorMad
Portal:
...
supports; check `portal.poormad.dev/models`.
...
[portal.poormad.dev](https://portal.poormad.dev).
...
`poormad_cli/proxy/adapters/<provider>.py` and registering it in
