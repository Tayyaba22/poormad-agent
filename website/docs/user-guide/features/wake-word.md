...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey PoorMad' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey PoorMad")

The wake word turns PoorMad into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, PoorMad listens in the background for a spoken
trigger phrase. Say it, and PoorMad starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** PoorMad backend (for example a
...
PoorMad supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey poormad"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey poormad"** — a model for it ships with PoorMad, so
...
```bash
cd ~/.poormad/poormad-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `poormad` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.poormad/config.yaml`,
...
wake_word:
  ...
  phrase: "hey poormad"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_poormad         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey poormad" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey poormad" utterances are missed.
...
default) to let PoorMad choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which PoorMad installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three PoorMad surfaces, and `surface` picks which one
...
| `cli` | Only the classic `poormad` CLI. |
| `tui` | Only `poormad --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when PoorMad surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. PoorMad does not silently fail over to another open surface.
...
"Hey PoorMad" works out of the box — the bundled openWakeWord model
(`model: hey_poormad`) is the default. To wake on something else, the easiest
...
command (`poormad -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.poormad/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("poormad" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey PoorMad" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey poormad"
  porcupine:
    keyword: ~/.poormad/wakewords/hey_poormad.ppn
```

Set your access key in `~/.poormad/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `poormad tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `poormad-agent[wake]`).
...
but the phrase never fires. PoorMad detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the PoorMad
backend (it may appear as your terminal, `python`, or PoorMad), then toggle the
...
```bash
poormad config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
poormad config set wake_word.input_device null
```
