...
sidebar_position: 8
title: "Context Files"
description: "Project context files — .poormad.md, AGENTS.md, CLAUDE.md, global SOUL.md, and .cursorrules — automatically injected into every conversation"
---

# Context Files

PoorMad automatically discovers and loads context files that shape how it behaves. Some are project-local and discovered from your working directory. `SOUL.md` is now global to the PoorMad instance and is loaded from `POORMAD_HOME` only.

## Supported Context Files

| File | Purpose | Discovery |
|------|---------|-----------| 
| **.poormad.md** / **POORMAD.md** | Project instructions (highest priority) | Walks to git root |
...
| **SOUL.md** | Global personality and tone customization for this PoorMad instance | `POORMAD_HOME/SOUL.md` only |
| **.cursorrules** | Cursor IDE coding conventions | CWD only |
| **.cursor/rules/*.mdc** | Cursor IDE rule modules | CWD only |

:::info Priority system
Only **one** project context type is loaded per session (first match wins): `.poormad.md` → `AGENTS.override.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. **SOUL.md** is always loaded independently as the agent identity (slot #1).
...
- `~/.poormad/SOUL.md`
- or `$POORMAD_HOME/SOUL.md` if you run PoorMad with a custom home directory

Important details:

- PoorMad seeds a default `SOUL.md` automatically if one does not exist yet
- PoorMad loads `SOUL.md` only from `POORMAD_HOME`
- PoorMad does not probe the working directory for `SOUL.md`
...
PoorMad is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`.poormad.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.

This means your existing Cursor conventions automatically apply when using PoorMad.
...
1. **Scan working directory** — checks for `.poormad.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules` (first match wins)
