...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of PoorMad's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple PoorMad instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
poormad memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `poormad` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — PoorMad syncs them back at session init.
...
The `poormad honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `poormad memory setup honcho` (or run `poormad memory setup` and pick it from the list); the `poormad honcho` subcommand then appears on the next invocation.

```bash
poormad memory setup honcho    # Configure Honcho directly (works before activation)
poormad honcho status          # Connection status, config, and key settings
poormad honcho setup           # Redirects to `poormad memory setup` (post-activation alias)
poormad honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
poormad honcho peer            # Show or update peer names + dialectic reasoning level
poormad honcho mode            # Show or set recall mode (hybrid/context/tools)
poormad honcho tokens          # Show or set token budget for context and dialectic
poormad honcho identity        # Seed or show the AI peer's Honcho identity
poormad honcho sync            # Sync Honcho config to all existing profiles
poormad honcho peers           # Show peer identities across all profiles
poormad honcho sessions        # List known Honcho session mappings
poormad honcho map             # Map current directory to a Honcho session name
poormad honcho enable          # Enable Honcho for the active profile
poormad honcho disable         # Disable Honcho for the active profile
poormad honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `poormad honcho`

If you previously used the standalone `poormad honcho setup`:
...
No re-login or re-setup needed. Run `poormad memory setup` and select "honcho" — the wizard detects your existing config.
