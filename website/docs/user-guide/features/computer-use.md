...
PoorMad can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended PoorMad integration. It
...
**Fresh installs already have the driver.** The PoorMad installer
...
- **`poormad tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
poormad computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `poormad computer-use
status` to verify the install.

Already have cua-driver? PoorMad reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `poormad update`, and the first
`computer_use` call of a session, PoorMad checks the local version and
...
selected with `POORMAD_CUA_DRIVER_CMD` stays
under your control, so PoorMad reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. PoorMad autodetection is a
planned cua-driver follow-up, so currently point PoorMad at that directory or
...
interface. The built-in toolset provides PoorMad actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `poormad computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the PoorMad host identity. |
...
```
poormad -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.poormad/config.yaml`.

## Permission modes and logged-in browser profiles

PoorMad maps its existing approval UX onto cua-driver's immutable runtime
...
| PoorMad session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal PoorMad approvals; Cua stops at its protected boundary | Refuses unless `computer_use.grant_existing_profile: true` (one-time config opt-in) |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit PoorMad risk acceptance; no runtime Cua prompts | Refuses unless `computer_use.grant_existing_profile: true`; YOLO does not substitute for this grant |
...
PoorMad then launches the cua-driver runtime with the trusted-launcher grant
...
computer_use:
  ...
  capability_manifest: ~/.poormad/cua-manifest.yaml
```
...
for the format). PoorMad launches a private runtime with
...
identity instead of resetting with every PoorMad build), and PoorMad verifies
...
resetting or closing the PoorMad session, cancellation cleanup, or process exit
closes that transport session. PoorMad also stops private runtimes that it
launched for bounded, unrestricted, or existing-profile access. One PoorMad
...
embedded service under the PoorMad host identity.
...
## `poormad computer-use doctor` — your first triage stop

`poormad computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ poormad computer-use doctor
...
PoorMad run declares a public cua-driver **session name** (something like
...
without it. PoorMad disables it automatically where it is a known failure
...
PoorMad keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the PoorMad-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. PoorMad
autodetection is a planned cua-driver follow-up, so currently point PoorMad at
...
These are **platform deep dives, not duplicates of the PoorMad skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **PoorMad
...
image capture also saves a bounded, shareable copy under PoorMad' image cache
...
PoorMad applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.poormad/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. PoorMad applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  PoorMad) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the PoorMad itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. PoorMad warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive PoorMad from inside the RDP / console
...
```
POORMAD_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
POORMAD_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **PoorMad disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) PoorMad sets
...
When it's on, `poormad computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point PoorMad at a binary you built from source instead
of the published release. PoorMad resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`POORMAD_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for PoorMad testing (see notes).
...
### Option B — point PoorMad straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`POORMAD_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
POORMAD_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
POORMAD_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm PoorMad is using your build

- `poormad computer-use status` prints the resolved binary path and
  version.
- `poormad computer-use doctor` confirms the binary is reachable and
...
- **PoorMad spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  PoorMad YOLO, PoorMad instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no PoorMad
  change needed either way.
- **Local builds skip the version check.** PoorMad warns when the
...
**First action when anything's off: run `poormad computer-use doctor`.**
...
Run `poormad computer-use install` to fetch the cua-driver binary, or
run `poormad tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `poormad computer-use doctor` will
...
- **PoorMad-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  PoorMad `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. PoorMad autodetection is a
  planned follow-up; currently point PoorMad at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (PoorMad among them)
