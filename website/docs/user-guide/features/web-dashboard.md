...
# PoorMad Web Dashboard

The web dashboard is a browser-based UI for managing your PoorMad installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses PoorMad Portal OAuth; if you also want the dashboard to talk to a real backend, `poormad setup --portal` wires up the model and tool gateway too. See [PoorMad Portal](/integrations/nous-portal).
:::
...
```bash
# Custom port
poormad dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
poormad dashboard --host 0.0.0.0

# Start without opening browser
poormad dashboard --no-open
```
...
child with the selected profile's `POORMAD_HOME`, so the conversation runs
...
processes (manage them via `poormad -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `poormad-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.poormad/poormad-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.poormad/poormad-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `poormad dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `poormad dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- **Disk warnings** — the volume holding `~/.poormad` is nearly full
...
- Node.js (same requirement as `poormad --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`cd ~/.poormad/poormad-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.poormad/.env
ExecStart=/path/to/venv/bin/python -m poormad_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.poormad/.env` containing:

```bash
POORMAD_DASHBOARD_BASIC_AUTH_USERNAME=admin
POORMAD_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
POORMAD_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `poormad config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated PoorMad instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `poormad profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.poormad/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `poormad skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `poormad mcp` reads from.
...
**Catalog:** browse the PoorMad-approved MCP servers (the bundled `optional-mcps/`
...
`poormad mcp catalog` / `poormad mcp install` use.
...
Connect PoorMad to any messaging platform from the browser — full parity with
`poormad setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.poormad/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and PoorMad versions, CPU core count + utilization, memory, disk usage of the PoorMad home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The PoorMad version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `poormad update` in the background. On Docker/Nix installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **PoorMad Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `poormad portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `poormad curator`.
...
![System admin page — host stats and PoorMad Portal status](/img/dashboard/admin-system-top.png)
...
This re-reads `~/.poormad/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`POORMAD_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
- **`disk`** — a live `shutil.disk_usage()` sample of the volume holding
  `~/.poormad`. Fields: `pressure`, `free_mb`, `total_mb`, `used_percent`,
...
| `GET /api/mcp/catalog` | Browse the PoorMad-approved MCP catalog |
...
| `GET /api/poormad/update/check` | Report update availability (commits behind, install method) without applying. For git installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
| `GET /api/curator` · `PUT .../paused` · `POST .../run` | Skill-curator status + pause/resume + run |
| `GET /api/portal` | PoorMad Portal auth + Tool Gateway routing (read-only) |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — PoorMad engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
- **[OAuth (PoorMad Portal)](#default-provider-nous-research)** — for hosted deployments and any dashboard reachable over the public internet, and the recommended path for a [remote PoorMad Desktop connection](#connecting-poormad-desktop-to-a-remote-backend). Every login is verified against your PoorMad account, so this is the provider suitable for internet-facing use.
- **[Self-hosted OIDC](#self-hosted-oidc-provider)** — for bringing your own identity provider via standard OpenID Connect (Keycloak, Auth0, Okta, Google, GitHub via an OIDC bridge, etc.). No PoorMad Portal involved; suitable for public-internet exposure when fronted by a conformant OIDC server.
...
|-------|-----------|----------|
| `poormad dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `poormad dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no PoorMad plugin, no custom plugin), `poormad dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

When you run `poormad dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yet, PoorMad doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds) or **OAuth** (points you at `poormad dashboard register`). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.

### Default provider: PoorMad
...
Because every login is verified against PoorMad Portal and protected by your PoorMad account, **the PoorMad provider is the one suitable for exposing a dashboard to the public internet.**

#### Registering a dashboard

To use the PoorMad provider you need an OAuth client ID (shape `agent:{id}`). There are two ways to get one:

- **CLI — `poormad dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing PoorMad login (run `poormad setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `POORMAD_DASHBOARD_OAUTH_CLIENT_ID` into `~/.poormad/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  poormad dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes POORMAD_DASHBOARD_OAUTH_CLIENT_ID to ~/.poormad/.env
  ```

- **GUI — the Local Dashboards page.** Open [`/local-dashboards`](https://portal.poormad.dev/local-dashboards) in the PoorMad Portal to register, name, manage, and revoke self-hosted dashboards from the browser. Copy the resulting `agent:{id}` client ID into `POORMAD_DASHBOARD_OAUTH_CLIENT_ID` (env) or `dashboard.oauth.client_id` (config.yaml). This is also where you revoke a dashboard registered via the CLI.
...
|---------|-----------|--------|----------------|
| `POORMAD_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `poormad dashboard register` |

Per the PoorMad convention (`~/.poormad/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • nous: POORMAD_DASHBOARD_OAUTH_CLIENT_ID is not set (and
...
Configure an auth provider before exposing the dashboard:
  ...
  • OAuth: run `poormad dashboard register` (PoorMad Portal) or install a
...
#### Worked example: PoorMad

From a logged-in PoorMad install to a PoorMad-gated dashboard in three steps.

**1. Log in and register the dashboard.** `poormad dashboard register` uses your existing PoorMad login to provision an OAuth client and writes `POORMAD_DASHBOARD_OAUTH_CLIENT_ID` into `~/.poormad/.env` for you:

```bash
poormad setup            # if you're not already logged into PoorMad Portal
poormad dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes POORMAD_DASHBOARD_OAUTH_CLIENT_ID to ~/.poormad/.env
```
...
```bash
poormad dashboard --host 0.0.0.0 --port 9119 --no-open
```

**3. Log in.** Open `http://<host>:9119/`, you'll be bounced to `/login`. Click **Sign in with PoorMad** → authenticate at the Portal → land back on the authenticated dashboard. Verify the gate from any machine:
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://poormad.example.com/auth/callback` and set `POORMAD_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
The username/password provider is intended for self-hosted / on-prem / homelab dashboards on a **trusted network**, or reachable only over a **VPN**. It protects a single shared credential with no external identity provider, MFA, or per-user accounts behind it, so it is **not suitable for exposing a dashboard directly to the public internet**. For an internet-facing dashboard, use the [PoorMad provider](#default-provider-nous-research) (or your own [self-hosted OIDC](#self-hosted-oidc-provider) / [custom OAuth](#custom-providers) provider) instead.
:::

#### Configuration

Like the PoorMad provider, it reads from `config.yaml` (canonical) with environment variables winning when set non-empty. It activates only when `username` plus either `password_hash` (preferred) or `password` are configured — otherwise it's a no-op, so OAuth users and loopback operators are unaffected.
...
|---------|-----------|-------|
| `POORMAD_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `POORMAD_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `POORMAD_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `POORMAD_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `POORMAD_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.poormad/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.poormad/.env <<EOF
POORMAD_DASHBOARD_BASIC_AUTH_USERNAME=admin
POORMAD_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
POORMAD_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.poormad/.env
```
...
`GET /api/auth/me` then returns the verified session (`provider: basic`). Keep this behind a VPN — see the warning above; for a public host use the [PoorMad](#default-provider-nous-research) or [self-hosted OIDC](#self-hosted-oidc-provider) provider instead.
...
If you run your own identity provider, the bundled `plugins/dashboard_auth/self_hosted` plugin authenticates the dashboard against it using **standard OpenID Connect** — no per-IDP code, no PoorMad Portal involved. It's verified against and works with any conformant OIDC server:

> **Authentik · Keycloak · Zitadel · Authelia · Auth0 · Okta · Google · …**

Like the PoorMad provider, it auto-loads and only registers itself once it's configured, so it's a no-op for loopback dashboards.
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/poormad/   # required
      client_id: poormad-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `POORMAD_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `POORMAD_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `POORMAD_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-poormad.json` — it defines a `poormad` realm, a **public PKCE client** (`poormad-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "poormad-dashboard",
      "name": "PoorMad Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-poormad.json:/opt/keycloak/data/import/realm-poormad.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/poormad/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/poormad`). The admin console is at
...
```bash
export POORMAD_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/poormad"
export POORMAD_DASHBOARD_OIDC_CLIENT_ID="poormad-dashboard"
export POORMAD_DASHBOARD_PUBLIC_URL="http://localhost:9119"
poormad dashboard --host 0.0.0.0 --port 9119 --no-open
```

`POORMAD_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → poormad-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `POORMAD_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/poormad"
  trusted_proxies:
    - "172.20.0.5"
```
...
first; without one, PoorMad fails closed at startup. This prevents the local SPA
...
PoorMad always preserves loopback trust and rejects `*`, `0.0.0.0/0`, and
...
poormad dashboard --host 127.0.0.1 --port 9119 --no-open
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `POORMAD_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `POORMAD_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
The provider implements the [PoorMad Portal OAuth contract v1](https://github.com/PoorMad/nous-account-service/blob/main/docs/agent-dashboard-oauth-contract.md) — authorization-code grant with PKCE (S256):

1. User hits `/` without a session cookie → gate redirects to `/login`.
2. Login page shows a "Continue with PoorMad" button → `/auth/login?provider=nous`.
3. Server stashes PKCE state in a short-lived cookie, redirects user to `https://portal.poormad.dev/oauth/authorize?…`.
4. User authenticates with Portal, lands at `/auth/callback?code=…&state=…`.
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `poormad_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `poormad_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `poormad_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip. SameSite=None + Secure over HTTPS (must survive the cross-site IDP redirect chain — Chromium drops SameSite=Lax cookies set on a 302 in a cross-site chain); SameSite=Lax on loopback HTTP |
| `poormad_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$POORMAD_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.

### Custom providers

To plug a non-PoorMad OAuth provider (e.g. Google, GitHub, custom OIDC), create a plugin that registers a `DashboardAuthProvider`:

```python
# ~/.poormad/plugins/dashboard-auth-myidp/__init__.py
from poormad_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
The bundled first consumer is the **drain** provider (`plugins/dashboard_auth/drain`): `nous-account-service` provisions a per-agent secret via `POORMAD_DASHBOARD_DRAIN_SECRET`, and the provider verifies inbound bearer tokens against it with a constant-time compare, registering `/api/gateway/drain` as token-authable. It **fails closed** — a weak/short secret (< 256 bits) is rejected at registration and the endpoint stays disabled; it's a no-op when the env var is unset. Behavioural knobs (`scope`, `min_secret_chars`) live under `dashboard.drain_auth` in `config.yaml`.
...
```bash
# Quick env-var path.
POORMAD_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  poormad dashboard --host 0.0.0.0
...
PoorMad Desktop can drive a PoorMad backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateways → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [PoorMad Desktop](/user-guide/desktop) page.)

You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (PoorMad Portal)** (register it with [`poormad dashboard register`](#registering-a-dashboard) and sign in with *Sign in with PoorMad*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.

The recipe below uses the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Default provider: PoorMad](#default-provider-nous-research).

### On the backend (the remote machine)

```bash
# 1. Set the dashboard login credentials in ~/.poormad/.env (secrets file, 0600).
cat >> ~/.poormad/.env <<'EOF'
POORMAD_DASHBOARD_BASIC_AUTH_USERNAME=admin
POORMAD_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
poormad dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `POORMAD_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.poormad/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.poormad/.env`, so the credentials are in the environment at boot.

:::warning
The dashboard reads and writes your `.env` (API keys, secrets) and can run agent commands. The **username/password** setup shown here is for a trusted network — never expose a password-protected dashboard directly to the open internet. Put it behind a VPN. [Tailscale](https://tailscale.com/) is the clean option: bind to the machine's tailscale IP (`--host <tailscale-ip>`) and use `http://<tailscale-ip>:9119` as the Remote URL. Only devices on your tailnet can reach it. To reach a backend over the public internet, use the **OAuth (PoorMad Portal)** provider instead.
:::
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/poormad` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `POORMAD_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `POORMAD_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `POORMAD_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `POORMAD_DASHBOARD_BASIC_AUTH_USERNAME` / `POORMAD_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `POORMAD_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `poormad_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `poormad update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `poormad dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **PoorMad Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **PoorMad Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
| **PoorMad Blue** (`nous-blue`) | PoorMad-branded blue accents with airy spacing |
