...
PoorMad can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<POORMAD_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), PoorMad renders the sprite at full fidelity when your
...
poormad pets list
poormad pets list cat

# Install a pet and make it active in one step
poormad pets install boba --select
...
## `poormad pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `poormad pets list [query] [--limit N]` |
| List installed pets | `poormad pets list --installed` |
| Install a pet | `poormad pets install <slug> [--select] [--force]` |
| Set the active pet | `poormad pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `poormad pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `poormad pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `poormad pets off` |
| Remove an installed pet | `poormad pets remove <slug>` |
| Diagnose setup | `poormad pets doctor` |

`poormad pets show` flags:
...
Beyond installing pre-made pets from the gallery, PoorMad can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `poormad pets` → the generate flow.
...
2. **Hatch** — the chosen base is used as a reference image to generate one grounded animation row per PoorMad state (idle, thinking, tool use, etc.), which are deterministically sliced into frames and packed into a standard petdex/Codex atlas (8×9 grid of 192×208 cells). The result is a valid spritesheet you keep — and could `petdex submit`.

### Image backend

Generation uses the active [image-generation provider](/user-guide/features/image-generation), but it requires **reference-image grounding** so each animation row stays the same character as the base. Reference-capable backends: **PoorMad Portal**, **OpenRouter**, **OpenAI** (`gpt-image-2`), and **Krea**. OpenRouter/PoorMad run a quality-first model chain by default.

- Resolution order prefers PoorMad Portal → OpenAI → OpenRouter.
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `poormad tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `POORMAD_PET_IMAGE_PROVIDER` env var (e.g. `POORMAD_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while PoorMad is
...
Run `poormad pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; PoorMad uses its own
  profile-scoped `<POORMAD_HOME>/pets/` instead — install through `poormad pets`.

## See also

- The [`poormad-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-poormad-agent.md)
