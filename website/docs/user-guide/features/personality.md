...
description: "Customize PoorMad's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

PoorMad's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `POORMAD_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who PoorMad is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

PoorMad now seeds a default `SOUL.md` automatically in:

```text
~/.poormad/SOUL.md
```

More precisely, it uses the current instance's `POORMAD_HOME`, so if you run PoorMad with a custom home directory, it will use:

```text
$POORMAD_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- PoorMad creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- PoorMad loads `SOUL.md` only from `POORMAD_HOME`
- PoorMad does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, PoorMad falls back to a built-in default identity
...
If PoorMad loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `POORMAD_HOME`, the personality belongs to the PoorMad instance itself.

That also makes it easier to teach users:
- "Edit `~/.poormad/SOUL.md` to change PoorMad' default personality."
...
- how PoorMad should handle uncertainty, disagreement, or ambiguity
...
## What PoorMad injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, PoorMad falls back to a built-in default identity ("You are PoorMad, an intelligent AI assistant created by PoorMad..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
PoorMad ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain PoorMad, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives PoorMad its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.poormad/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.poormad/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-poormad)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how PoorMad speaks
- `display.skin` and `/skin` affect how PoorMad looks in the terminal
