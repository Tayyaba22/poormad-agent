...
PoorMad ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
poormad memory setup      # interactive picker + configuration
poormad memory status     # check what's active
poormad memory off        # disable external provider
```

You can also select the active memory provider via `poormad plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.poormad/config.yaml`:
...
When a memory provider is active, PoorMad automatically:
...
**Setup Wizard:**
```bash
poormad memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `poormad honcho setup` command still works (it now redirects to `poormad memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$POORMAD_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$POORMAD_HOME/honcho.json` > `~/.poormad/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/Tayyaba22/poormad-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/poormad).
...
:::tip Migrating from `poormad honcho`
If you previously used `poormad honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per PoorMad profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All PoorMad profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per PoorMad profile. Host key `poormad` → default; `poormad.<profile>` for others. |
...
```bash
poormad profile create coder --clone
```

`--clone` creates a `poormad.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every PoorMad profile, creates host blocks for any profile without one, inherits settings from the default `poormad` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `poormad memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/Tayyaba22/poormad-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/poormad).
...
poormad memory setup    # select "openviking"
# Or manually:
poormad config set memory.provider openviking
```

`poormad memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.poormad/.env`, and for named profiles use
`~/.poormad/profiles/<profile>/.env`.
...
`OPENVIKING_AGENT` is PoorMad' peer ID in OpenViking for peer-scoped memories.
PoorMad sends `User-Agent: openviking-memory-poormad/<version>` on OpenViking
...
**Setup (Platform):**
```bash
poormad memory setup    # select "mem0" → "Platform"
# Or manually:
poormad config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.poormad/.env
```

**Setup (OSS):**
```bash
poormad memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
poormad memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
poormad memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
poormad memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
poormad memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.poormad/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.poormad/.env
```
...
**Config:** `$POORMAD_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.poormad/.env`.
...
| `user_id` | `poormad-user` | User identifier |
| `agent_id` | `poormad` | Agent identifier |
...
**Switching modes:** Re-run `poormad memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
poormad memory setup    # select "hindsight"
# Or manually:
poormad config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.poormad/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p poormad ui start`

**Config:** `$POORMAD_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `poormad` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between PoorMad and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/Tayyaba22/poormad-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
poormad memory setup    # select "holographic"
# Or manually:
poormad config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.poormad-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$POORMAD_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
poormad memory setup    # select "retaindb"
# Or manually:
poormad config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.poormad/.env
```
...
# Then configure PoorMad
poormad memory setup    # select "byterover"
# Or manually:
poormad config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$POORMAD_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=poormad), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
poormad memory setup    # select "supermemory"
# Or manually:
poormad config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.poormad/.env
```
...
Before running `poormad memory setup`, set `base_url` in
`$POORMAD_HOME/supermemory.json`:
...
Then run `poormad memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$POORMAD_HOME/supermemory.json`
...
| `container_tag` | `poormad` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `poormad-{identity}` → `poormad-coder`) to isolate memories per PoorMad profile
...
| **Requires** | `pip install poormad-memori` + `poormad-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install poormad-memori
poormad-memori install
poormad config set memory.provider memori
poormad memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `poormad-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$POORMAD_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$POORMAD_HOME/` so each profile has its own credentials
