...
Type `@` followed by a reference to inject content directly into your message. PoorMad expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- PoorMad env: `$POORMAD_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$POORMAD_HOME/skills/.hub/`
