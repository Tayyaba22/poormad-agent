...
If you only run one PoorMad, you don't need this page — see
...
[Connecting Desktop to Many PoorMad Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more PoorMads that should all
...
(`ai.poormad.gateway-<name>.plist`) or systemd user service
(`poormad-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
poormad profile create coder
poormad profile create personal-bot
poormad profile create research
...
```bash
poormad config set gateway.multiplex_profiles true
poormad gateway restart
```

Equivalently, in the default profile's `~/.poormad/config.yaml`:
...
You do **not** run `poormad gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `poormad gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.poormad/profiles/coder/.env`; the default listener key is rejected.
...
home). `poormad status` reports the multiplexer and the profiles it serves;
`poormad status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/poormad-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: poormad-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    poormad gateway "$action"
  else
    poormad -p "$profile" gateway "$action"
  fi
}
...
```bash
poormad-gateways start      # start every configured profile
poormad-gateways stop       # stop every configured profile
poormad-gateways restart    # restart all
poormad-gateways status     # status across all
poormad-gateways list       # delegates to `poormad gateway list`
```

:::tip
The `default` profile is targeted with `poormad gateway <action>` (no `-p`),
not `poormad -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `poormad -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.poormad.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/poormad-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.poormad.gateway.plist` /
`poormad-gateway.service`.
...
```bash
# Default profile
tail -f ~/.poormad/logs/gateway.log
tail -f ~/.poormad/logs/gateway.error.log

# Named profile
tail -f ~/.poormad/profiles/<name>/logs/gateway.log
tail -f ~/.poormad/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.poormad/logs/gateway.log ~/.poormad/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
poormad logs -f                  # follow default profile
poormad -p coder logs -f         # follow one profile
poormad logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
poormad profile list             # profiles + model + gateway state
poormad-gateways status          # full status across every profile
launchctl list | grep poormad    # macOS — PIDs and labels
systemctl --user list-units 'poormad-gateway-*'   # Linux — units
```
...
```
~/.poormad/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.poormad/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
poormad config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.poormad/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=poormad --why="gateways running" \
  sleep infinity &
...
`poormad-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.poormad/.env ~/.poormad/profiles/*/.env
```

## Updating the code

`poormad update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `poormad gateway start` after a previous `poormad gateway stop`. The
...
```bash
ps -ef | grep "poormad_cli.*-p <profile>"
cat ~/.poormad/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.poormad.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.poormad.gateway-<profile>.plist

# Linux
systemctl --user restart poormad-gateway-<profile>.service
```

### Health check

```bash
poormad doctor                  # default profile
poormad -p <profile> doctor     # one profile
```
