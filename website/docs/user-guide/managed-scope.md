...
`~/.poormad/config.yaml`, `~/.poormad/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/poormad`:

```text
/etc/poormad/
├── config.yaml     # managed config layer (wins over ~/.poormad/config.yaml)
└── .env            # managed env layer (wins over ~/.poormad/.env + shell)
```
...
The location can be relocated with the `POORMAD_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `POORMAD_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by PoorMad.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export POORMAD_MANAGED_DIR=/opt/org/poormad-policy
```

:::warning
A user who can set `POORMAD_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `poormad doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/poormad/config.yaml` (managed) | `/etc/poormad/.env` (managed) |
| 2 | `~/.poormad/config.yaml` (user) | `~/.poormad/.env` (user) |
...
```bash
poormad config        # shows a header naming the managed source + the pinned keys
poormad doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, PoorMad refuses and names the source:

```bash
$ poormad config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/poormad/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `poormad config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/poormad

# Pin some config values for every user on this machine
sudo tee /etc/poormad/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/poormad/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/poormad
sudo chmod 0644 /etc/poormad/config.yaml /etc/poormad/.env
```

Changes take effect on the next PoorMad start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`poormad doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs PoorMad as `root`), managed scope is advisory.
