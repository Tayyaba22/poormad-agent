...
title: "Git Worktrees"
description: "Run multiple PoorMads safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

PoorMad is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with PoorMad so each session has a clean, isolated working directory.

## Why Use Worktrees with PoorMad?

PoorMad treats the **current working directory** as the project root:

- CLI: the directory where you run `poormad` or `poormad chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.poormad/config.yaml`
...
PoorMad creates `.worktrees/my-experiment/` inside the repo (branch
`poormad/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `poormad-<id>`
...
git worktree add ../repo-feature feature/poormad-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/poormad-experiment` checked out in that directory

Now you can `cd` into the new worktree and run PoorMad there:

```bash
cd ../repo-feature

# Start PoorMad in the worktree
poormad
```
...
git worktree add ../repo-experiment-a feature/poormad-a
git worktree add ../repo-experiment-b feature/poormad-b
```
...
Each PoorMad process:

- Works on its own branch (`feature/poormad-a` vs `feature/poormad-b`).
...
- PoorMad checkpoint data under `~/.poormad/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per PoorMad experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/poormad-checkpoints-docs`, `feature/poormad-refactor-tests`.
- **Commit frequently**
...
- **Avoid running PoorMad from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `poormad -w` (Automatic Worktree Mode)

PoorMad has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `poormad/poormad-<hash>`).
...
```bash
poormad -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `poormad -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each PoorMad session its own clean checkout.
