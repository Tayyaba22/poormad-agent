...
PoorMad automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.poormad/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
PoorMad stores session history so it can resume conversations, but it does not
...
PoorMad explicitly injects for that turn.
...
For example, if a user sends an image and asks PoorMad to make a meme from it,
PoorMad may inspect that image once with vision and run an image-processing
...
`poormad sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `poormad sessions optimize` merges FTS5 index segments and
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`poormad` or `poormad chat`) |
...
A bare `-c` is terminal-aware: each CLI session drops a small breadcrumb file under `~/.poormad/terminal-sessions/` keyed by the terminal it runs in (tty device, tmux pane, kitty window, wezterm pane, Zellij pane, Windows Terminal session, ...). When you run `poormad -c` again in the *same* terminal, PoorMad resumes that terminal's own session — so two panes side by side each continue their own conversation instead of both grabbing the globally most-recent one. If there's no breadcrumb for the terminal (first use, deleted session, or a stale breadcrumb older than 30 days), `-c` falls back to the most-recent-session behavior. `-c "name"` and `--resume` are unaffected. Disable with `session.terminal_continue: false` in `config.yaml`.
...
poormad -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
poormad -c "my project"   # → resumes "my project #3"
```
...
poormad --resume 20250305_091523_a1b2c3d4
poormad -r 20250305_091523_a1b2c3d4

# Resume by title
poormad --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
poormad --resume latest

# Or with the chat subcommand
poormad chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `poormad sessions list`.
...
poormad --resume latest --in ./my-project

# Works with the TUI too
poormad --tui --resume latest --in ./my-project
```
...
```bash
poormad --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`poormad sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
poormad sessions list --workspace my-project
poormad sessions list --workspace ~/code/poormad-agent
```

### Conversation Recap on Resume

When you resume a session, PoorMad displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a PoorMad session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.poormad/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `poormad -r "<title>"` from the shell) and pick up where the platform left off.
...
PoorMad automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `poormad sessions list` or `poormad sessions browse`.
...
```bash
poormad sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), PoorMad creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`poormad -c "my project"`), it automatically picks the most recent session in the lineage.
...
PoorMad provides a full set of session management commands via `poormad sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
poormad sessions list

# Filter by platform
poormad sessions list --source telegram

# Show more sessions
poormad sessions list --limit 50
```
...
`poormad sessions export` is one surface for every export format, selected with `--format`:
...
poormad sessions export backup.jsonl

# Export sessions from a specific platform
poormad sessions export telegram-history.jsonl --source telegram

# Export a single session
poormad sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
poormad sessions export backup.jsonl --redact
```
...
poormad sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
poormad sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
poormad sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
poormad sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `poormad-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
poormad sessions export --format trace

# One session to a local trace file
poormad sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
poormad sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.poormad/session-exports`).

```bash
# Export one session to Markdown
poormad sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
poormad sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
poormad sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
poormad sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
poormad sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
poormad sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
poormad sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
poormad sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
poormad sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
poormad sessions pin 20250305_091523_a1b2c3d4
poormad sessions pin 20250305 20250306

# Remove the pin
poormad sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
poormad sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
poormad sessions pinned --json > pinned-sessions.json
```
...
poormad sessions prune

# Custom age threshold — bare numbers are days
poormad sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
poormad sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
poormad sessions prune --newer-than 5h

# Explicit window with absolute timestamps
poormad sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
poormad sessions prune --source telegram
poormad sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
poormad sessions prune --newer-than 5h --title "smoke test"   # title substring
poormad sessions prune --older-than 30 --max-messages 3        # tiny sessions
poormad sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
poormad sessions prune --model gpt-5 --older-than 1w           # by model (substring)
poormad sessions prune --provider openrouter --older-than 60   # by billing provider
poormad sessions prune --branch feature/old-experiment         # by git branch
poormad sessions prune --user 12345678 --chat-type group       # by messaging origin
poormad sessions prune --max-tokens 500 --older-than 7         # by token usage
poormad sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
poormad sessions prune --newer-than 5h --dry-run

# Skip confirmation
poormad sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `poormad sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `poormad sessions prune` keeps the 90-day cutoff. Every
...
`poormad sessions archive` takes the same filters as `prune` but soft-hides
...
poormad sessions archive --newer-than 5h

# Archive by title substring, preview first
poormad sessions archive --title "dry run" --dry-run
poormad sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `poormad sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`poormad sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
poormad sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`poormad insights`](/reference/cli-commands#poormad-insights).
...
`poormad sessions repair-routing` finds message-bearing session rows with no
...
poormad sessions repair-routing
...
poormad sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
poormad sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.poormad/state.db ~/.poormad/state.db.bak`).


## Importing Sessions from Claude Code and Codex CLI

Started a conversation in another agent CLI? You can pull it into PoorMad and
continue it here. PoorMad reads Claude Code's session logs
...
poormad sessions import

# Limit to one tool, or point at a specific file
poormad sessions import --from claude
poormad sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl

# Import-and-resume in one step
poormad --resume @claude
poormad --resume @codex
```

`poormad sessions import` creates a new PoorMad session titled
`Imported from Claude Code: <first user message>` (or Codex CLI) and prints
the id plus a ready-to-paste `poormad --resume <id>` command.
...
When PoorMad cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, PoorMad uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to PoorMad in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.poormad/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.poormad/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.poormad/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.poormad/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.poormad/sessions/sessions.json` is a **legacy mirror** of it,
...
`poormad sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.poormad/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `poormad sessions list`, the cause is
`state.db` not receiving them — run `poormad sessions repair` and watch for a
...
`*.jsonl` files in `~/.poormad/sessions/`. They are no longer written or
read by PoorMad. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every PoorMad process in the same `POORMAD_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.poormad/config.yaml`:
...
poormad sessions delete <session_id>
