...
| | |
|---|---|
| Source | Optional — install with `poormad skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by PoorMad |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during PoorMad terminal calls (desktop app flow only)
...
### Service Account (recommended for PoorMad)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${POORMAD_HOME:-~/.poormad}/.env` (the skill will prompt for this on first load).
...
## PoorMad Execution Pattern (desktop app flow)

PoorMad terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/poormad-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/poormad-op.sock"
