...
| | |
|---|---|
| Source | Optional — install with `poormad skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the PoorMad MCP catalog. The
...
Two halves, in this order: the editor side must be up before PoorMad connects.
...
### One-time, PoorMad side

    poormad mcp install unreal-engine
...
`~/.poormad/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for PoorMad — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. PoorMad connects
...
2. Start the PoorMad session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through PoorMad
they appear as:

| PoorMad tool | Purpose |
|---|---|
...
then happens at `poormad mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — PoorMad runs batched calls
...
- **Start order matters.** Editor + server up first, then the PoorMad
...
- **Timeouts on long operations.** PoorMad' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.poormad/config.yaml` for
