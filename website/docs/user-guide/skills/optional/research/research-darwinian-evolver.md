...
| | |
|---|---|
| Source | Optional — install with `poormad skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), PoorMad |
| License | MIT |
...
import upstream classes into PoorMad itself.
...
```bash
mkdir -p ~/.poormad/cache/darwinian-evolver && cd ~/.poormad/cache/darwinian-evolver
...
```bash
cd ~/.poormad/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.poormad/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.poormad/skills/research/darwinian-evolver
DE_DIR=~/.poormad/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside PoorMad core.
   Custom driver scripts under `~/.poormad/skills/...` are user-side and fine.
