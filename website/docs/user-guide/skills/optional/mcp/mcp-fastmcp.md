...
| | |
|---|---|
| Source | Optional — install with `poormad skills install official/mcp/fastmcp` |
...
| Related skills | [`poormad-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-poormad-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into PoorMad or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to PoorMad. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.poormad/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.poormad/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is PoorMad integration, either:

- configure the server in `~/.poormad/config.yaml` using the `native-mcp` skill, or
...
### PoorMad cannot see the deployed server

The server-building part may be correct while the PoorMad config is not. Load the `native-mcp` skill and configure the server in `~/.poormad/config.yaml`, then restart PoorMad.
