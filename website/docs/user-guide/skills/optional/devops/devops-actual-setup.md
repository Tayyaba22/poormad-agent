---
title: "Actual Setup — Set up Actual Computer (actual.inc) inference in PoorMad"
sidebar_label: "Actual Setup"
description: "Set up Actual Computer (actual.inc) inference in PoorMad"
---
...
Set up Actual Computer (actual.inc) inference in PoorMad.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `poormad skills install official/devops/actual-setup` |
| Path | `optional-skills/devops/actual-setup` |
| Version | `2.0.0` |
| Author | shl0ms + PoorMad |
| License | MIT |
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a PoorMad inference
...
- User has an `ac_` key and wants PoorMad routed through their Actual cluster.
...
- PoorMad has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current PoorMad — the
...
1. Put the key in `.env` (secrets only — never config.yaml):
   append `ACTUAL_API_KEY=ac_...` to `~/.poormad/.env`.
...
3. Select provider + model:
   ```bash
   poormad config set model.provider actual
   poormad config set model.default "MODEL_ID_FROM_DISCOVERY"
   ```
4. Verify end-to-end:
   ```bash
   poormad chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL_ID
   ```
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point PoorMad at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
   built-in provider into local no-auth mode automatically — no key needed:
   append `ACTUAL_BASE_URL=http://127.0.0.1:8080` to `~/.poormad/.env`, then:
   ```bash
   poormad config set model.provider actual
   poormad config set model.default "INSTALLED_MODEL_NAME"
   ```
4. Verify (reduced toolset — see context-window pitfall below):
   ```bash
   poormad chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m INSTALLED_NAME -t file,web
   ```
...
1. **reasoning_effort trap (handled by PoorMad since the first-class provider).**
   ...
   request still 400s this way on an old PoorMad, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** PoorMad' default toolset
   ...
   servers emit a bare `data: [DONE]` — PoorMad reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current PoorMad the built-in provider wins the name; stale custom blocks
...
```bash
# Relay:
poormad chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL
# Local (small model — reduced toolset):
poormad chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m MODEL -t file,web
