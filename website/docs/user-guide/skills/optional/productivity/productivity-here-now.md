...
| | |
|---|---|
| Source | Optional — install with `poormad skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${POORMAD_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${POORMAD_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${POORMAD_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client poormad
```
...
```bash
PUBLISH="${POORMAD_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client poormad
```
...
```bash
DRIVE="${POORMAD_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `poormad`)    |
