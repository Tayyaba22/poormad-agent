...
| | |
|---|---|
| Source | Optional — install with `poormad skills install official/productivity/shopify` |
...
4. Save to `${POORMAD_HOME:-~/.poormad}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"PoorMad","productType":"Apparel","tags":["test"]}}'
```
