...
| | |
|---|---|
| Source | Optional — install with `poormad skills install official/productivity/canvas` |
...
4. Name the token (e.g., "PoorMad"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${POORMAD_HOME:-~/.poormad}/.env`:
...
```bash
CANVAS="python $POORMAD_HOME/skills/productivity/canvas/scripts/canvas_api.py"
