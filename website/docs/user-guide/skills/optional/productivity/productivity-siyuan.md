...
| | |
|---|---|
| Source | Optional — install with `poormad skills install official/productivity/siyuan` |
...
3. Store it in `${POORMAD_HOME:-~/.poormad}/.env`:
   ```
...
```yaml
# In ~/.poormad/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
