---
title: "Honcho — Configure and troubleshoot Honcho memory for PoorMad"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for PoorMad"
---
...
Configure and troubleshoot Honcho memory for PoorMad.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `poormad skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for PoorMad

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every PoorMad profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/poormad#running-honcho-locally-with-poormad

### Verify

```bash
poormad honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this PoorMad profile's AI peer
...
Honcho models conversations as interactions between **peers**. PoorMad creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this PoorMad instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each PoorMad run |
| `global` | Single session across all directories |

Manual override: `poormad honcho map my-project-name`
...
Each PoorMad profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block poormad.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `poormad.coder` host block in `honcho.json`
...
```bash
poormad honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for PoorMad when Honcho memory is active.
...
Config file: `$POORMAD_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `poormad honcho setup`. Ensure `memory.provider: honcho` is in `~/.poormad/config.yaml`.

### Memory not persisting across sessions
Check `poormad honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `poormad profile create <name> --clone`. For existing profiles: `poormad honcho sync`.
...
| Command | Description |
|---------|-------------|
| `poormad honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `poormad honcho status` | Show resolved config, connection test, peer info for active profile |
| `poormad honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `poormad honcho disable` | Disable Honcho for the active profile |
| `poormad honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `poormad honcho peers` | Show peer identities across all profiles |
| `poormad honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `poormad honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `poormad honcho sessions` | List known directory-to-session-name mappings |
| `poormad honcho map <name>` | Map current working directory to a Honcho session name |
| `poormad honcho identity` | Seed AI peer identity or show both peer representations |
| `poormad honcho sync` | Create host blocks for all PoorMad profiles that don't have one yet |
| `poormad honcho migrate` | Step-by-step migration guide from OpenClaw native memory to PoorMad + Honcho |
| `poormad memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `poormad memory status` | Show active memory provider and config |
| `poormad memory off` | Disable external memory provider |
