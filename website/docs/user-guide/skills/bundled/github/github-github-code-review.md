...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _poormad_env="${POORMAD_HOME:-$HOME/.poormad}/.env"; [ -f "$_poormad_env" ] && grep -q "^GITHUB_TOKEN=" "$_poormad_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_poormad_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python3 "${POORMAD_HOME:-$HOME/.poormad}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
```bash
source "${POORMAD_HOME:-$HOME/.poormad}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by PoorMad. Code looks clean — good test coverage, no security concerns."
