...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`poormad-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-poormad-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the PoorMad terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For PoorMad itself, `model.provider: openai-codex` uses PoorMad-managed Codex
OAuth from `~/.poormad/auth.json` after `poormad auth add openai-codex`. For the
...
## PoorMad Gateway Caveat

When invoking the Codex CLI from a PoorMad gateway/service context (for example,
