...
| List/filter by metadata (age, source, cost, tokens, workspace) | `poormad sessions list` / `stats` via terminal |
| Rename | `poormad sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `poormad sessions archive <filters>` |
| Delete (destructive) | `poormad sessions delete` / `poormad sessions prune <filters>` |
| Export before deleting anything valuable | `poormad sessions export --session-id <id> --format md` |
...
`poormad sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `poormad sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `poormad sessions list`
