...
description: "Run PoorMad directly on an Android phone with Termux"
---

# PoorMad on Android with Termux
...
PoorMad can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop PoorMad from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
This APT repository is **community-maintained by `@adybag14-cyber` and is not an official PoorMad distribution**. PoorMad does not build, sign, host, or audit these packages. Enabling the repository means trusting the contributor-operated repository and its signing key. Termux itself remains a Tier 2 / best-effort platform.
:::

For users who prefer a native package-manager install rather than building Python/Rust dependencies on the phone, a community-maintained APT repository is available. The repository bootstrap and packaging sources are published in [`adybag14-cyber/termux-python`](https://github.com/adybag14-cyber/termux-python), with the PoorMad package build in [`adybag14-cyber/termux-poormad`](https://github.com/adybag14-cyber/termux-poormad).

Install the repository key/source and PoorMad with:

```bash
curl -fsSL https://raw.githubusercontent.com/adybag14-cyber/termux-python/main/scripts/setup_apt_repo.sh | bash
pkg install poormad-agent
```
...
APT-managed PoorMad installs are marked with install method `apt`. PoorMad therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade poormad-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. PoorMad runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

PoorMad now ships a Termux-aware installer path:
...
- links `poormad` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `poormad` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/poormad" "$PREFIX/bin/poormad"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `poormad` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.poormad/.env`.
...
### `poormad doctor` says ripgrep or Node is missing
