...
| Variable | Meaning |
|----------|---------|
| `POORMAD_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `POORMAD_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `POORMAD_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by PoorMad itself — they're private to these helpers. The variables PoorMad *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `poormad --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_poormad_root)" || { echo "htui: not in a PoorMad checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$POORMAD_MAIN_CHECKOUT/.venv/bin/python" -m poormad_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `POORMAD_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_poormad_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `POORMAD_TUI_DIR` are mutually exclusive
`POORMAD_TUI_DIR` points PoorMad at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `poormad --tui --dev` exits with an error. Run `unset POORMAD_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_poormad_root)" || { echo "hgui: not in a PoorMad checkout" >&2; return 1; }
  deps="${POORMAD_GUI_DEPS_CHECKOUT:-$POORMAD_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _poormad_link_deps "$desktop" "$deps/apps/desktop"
    _poormad_link_deps "$root" "$deps"
  else
  ...
  trap '_poormad_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    POORMAD_DESKTOP_POORMAD_ROOT="$root" \
    POORMAD_DESKTOP_PYTHON="$POORMAD_MAIN_CHECKOUT/.venv/bin/python" \
    POORMAD_DESKTOP_IGNORE_EXISTING=1 \
    POORMAD_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `POORMAD_DESKTOP_POORMAD_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `poormad`. |
| `POORMAD_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `POORMAD_DESKTOP_IGNORE_EXISTING` | Ignores any `poormad` on `PATH` so it can't shadow the worktree. |
| `POORMAD_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real PoorMad checkout.
_poormad_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/poormad_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_poormad_gui_cleanup() {
  local root="$1"
  [[ -n "$root" ]] && pkill -TERM -f "${root}/apps/desktop/node_modules/electron" 2>/dev/null
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'poormad_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `poormad --tui --dev` and the `POORMAD_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/Tayyaba22/poormad-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `POORMAD_*` variable PoorMad reads
