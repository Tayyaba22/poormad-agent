...
description: "How PoorMad stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `poormad_cli/cron.py` | CLI `poormad cron` subcommands |
...
Jobs are stored in `~/.poormad/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `poormad cron` commands are run or during active CLI sessions.
...
keep the process awake), it asks PoorMad infrastructure to arm exactly **one
managed one-shot per job at that job's real next-fire time**. At fire time PoorMad
...
The flow (the managed scheduler is provided by PoorMad; the agent holds no
scheduler credentials):

```
create/update a cron job
  → Chronos asks PoorMad to arm a one-shot at the job's next_run_at
      (authenticated with the agent's existing PoorMad token)
  → at fire time PoorMad calls the gateway: POST {callback_url}/api/cron/fire
      (authenticated with a short-lived, purpose-scoped PoorMad-minted JWT)
...
Config (all non-secret; on hosted agents PoorMad sets these at provision time):

| key | meaning |
|---|---|
| `cron.provider` | `chronos` to activate (empty = built-in ticker) |
| `cron.chronos.portal_url` | PoorMad base URL (arming + the fire-token issuer) |
...
If Chronos is misconfigured or the agent isn't logged into PoorMad,
...
agent↔PoorMad wire contract lives in `docs/chronos-managed-cron-contract.md`.
...
```python
# ~/.poormad/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.poormad/cron/output/` |
...
**Bot Chat** (`bot-chat`, `bot-chat:<profile>`) is a machine-local pseudo-platform, not a gateway adapter: the scheduler delivers by running `poormad [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing -Q --query-file <tmp>` — the same lane Bot Mode agent-to-agent messages use — so the output arrives as a real inbound turn in the profile's canonical Bot Chat and the bot runs a full agent turn on it (alternation-safe by construction; this is the chat command lane, not a transcript mirror). The bare token targets the job's own profile; the named form is validated against `~/.poormad/profiles/` at create time and again at fire time, and never resolves across machines. Bot-chat targets are excluded from the `all` routing token and from delivery preflight (no gateway credentials involved). The per-delivery subprocess timeout is `cron.bot_chat_delivery_timeout_seconds` (default 600).
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `poormad cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `poormad cron` CLI provides direct job management:

```bash
poormad cron list                    # Show all jobs
poormad cron create                  # Interactive job creation (alias: add)
poormad cron edit <job_id>           # Edit job configuration
poormad cron pause <job_id>          # Pause a running job
poormad cron resume <job_id>         # Resume a paused job
poormad cron run <job_id>            # Trigger immediate execution
poormad cron remove <job_id>         # Delete a job
```
