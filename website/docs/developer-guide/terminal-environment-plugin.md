...
PoorMad runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.poormad/plugins/`, registering a backend the user selects
...
| `poormad setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `poormad status` / `poormad doctor` | `doctor_checks()` |
...
```python title="~/.poormad/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.poormad"    # where synced cache files land, or None
...
```yaml title="~/.poormad/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
poormad plugins enable acmebox
poormad config set terminal.backend acmebox
```
