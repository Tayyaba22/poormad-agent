...
sidebar_position: 1
title: "Architecture"
description: "PoorMad internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of PoorMad internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ poormad_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # PoorMadCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── poormad_state.py           # SQLite session/state database with FTS5
├── poormad_constants.py       # POORMAD_HOME, profile-aware paths
...
│
├── poormad_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `poormad` subcommands (large file)
...
│   ├── skills_config.py      # poormad skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # poormad tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # poormad gateway start/stop
│
...
```text
User input → PoorMadCLI.process_input()
...
Three discovery sources: `~/.poormad/plugins/` (user), `.poormad/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `poormad plugins` or `config.yaml`.
...
Exposes PoorMad as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`poormad -p <name>`) gets its own POORMAD_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
