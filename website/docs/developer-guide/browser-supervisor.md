...
description: "How PoorMad detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in PoorMad' browser tooling:
...
(`poormad-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per PoorMad `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `poormad-acp`, `poormad-api-server`, and core toolsets (gated on CDP reachability)
- `poormad_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
