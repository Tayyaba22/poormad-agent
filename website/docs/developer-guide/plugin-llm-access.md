...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept PoorMad already has applies. The plugin never sees a
...
poormad-agent itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — PoorMad provider id (e.g. "openrouter")
...
using PoorMad built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different PoorMad provider
        # (must be one PoorMad already knows about — same names as
        # `poormad model` and config.yaml model.provider).
        ...
        # plugin sends — PoorMad does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.poormad/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through PoorMad' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing PoorMad subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/Tayyaba22/poormad-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/Tayyaba22/poormad-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/PoorMad/poormad-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/PoorMad/poormad-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
