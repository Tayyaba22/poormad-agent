...
Plugins can launch and supervise fresh PoorMad child sessions without importing
...
active agent turn fails closed with `No active PoorMad parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until PoorMad can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; PoorMad's existing unsafe-tool block remains enforced.
