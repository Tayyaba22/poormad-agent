...
The ACP adapter wraps PoorMad' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
poormad acp / poormad-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.poormad/.env
  -> configure stderr logging
  -> construct PoorMadACPAgent
...
- `allow_once` -> PoorMad `once`
- `allow_always` -> PoorMad `always`
- reject options -> PoorMad `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps PoorMad tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["poormad-acp"])
...
Instead it reuses PoorMad' runtime resolver:

- `acp_adapter/auth.py`
- `poormad_cli/runtime_provider.py`

So ACP advertises and uses the currently configured PoorMad provider/credentials. It also always advertises a terminal setup auth method (`poormad-setup`, args `--setup`) so first-run ACP clients can open PoorMad' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.poormad/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `poormad-acp` toolset definition
- `poormad_cli/main.py` — `poormad acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `poormad-acp` script
