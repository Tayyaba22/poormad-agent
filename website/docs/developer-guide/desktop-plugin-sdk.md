...
title: "Desktop Plugin SDK (@poormad/plugin-sdk)"
description: "Extend the native PoorMad Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [PoorMad Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `PoorMadPlugin`.
It imports one module — `@poormad/plugin-sdk` — and gets everything: the app's
...
`$POORMAD_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across PoorMad. This page is the **native
desktop app** (`poormad desktop`) SDK — the `@poormad/plugin-sdk` module and
`$POORMAD_HOME/desktop-plugins/`. The **web dashboard** (`poormad dashboard`) has
its own, unrelated plugin system on `window.__POORMAD_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a PoorMad Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$POORMAD_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$POORMAD_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `PoorMadPlugin` contract, appear in **Settings → Plugins**,
...
Create `$POORMAD_HOME/desktop-plugins/hello/plugin.js` (that's `~/.poormad/...`
by default, or `~/.poormad/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.poormad/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@poormad/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, PoorMad' }),
      jsx('div', {
...
The only importable specifiers are `@poormad/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `PoorMadPlugin`:

```ts
interface PoorMadPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `poormad.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@poormad/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@poormad/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@poormad/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@poormad/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@poormad/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@poormad/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@poormad/plugin-sdk'
...
`window.poormad.send('get-price eth')` (or a declarative
`<button data-poormad-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@poormad/plugin-sdk'
...
for requests; `targetProfile` is the backend PoorMad profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from PoorMad
...
ctx.os.notify({
  ...
  // Body click → focus PoorMad + navigate. Same vocabulary as OS deep links:
  activate: 'poormad://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'poormad://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `poormad://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `poormad://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@poormad/plugin-sdk'
...
```javascript
import { queryClient } from '@poormad/plugin-sdk'
...
a package into `~/.poormad/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.poormad/plugins` is not reachable as a filesystem —
...
the `poormad://` scheme — a plain anchor on your website or README:
...
`poormad-dev://`. Full link reference:
...
`dashboard/` subfolder of a regular PoorMad plugin and declare it in a
`manifest.json`:
...
poormad-agent codebase directly (`poormad_state`, `poormad_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.poormad/`)
...
(`poormad.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `PoorMadPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@poormad/plugin-sdk` alias.
2. It's still lint-fenced to `@poormad/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@poormad/plugin-sdk`, `react`,
...
| Plugin contract | `PoorMadPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `PoorMadOpenTarget`, `Contribution` |
...
### Agents: the `poormad-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`poormad-desktop-plugins`** skill — it carries the same contract as this page in
...
`$POORMAD_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `poormad logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@poormad/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.poormad/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.poormad/logs/errors.log` for
