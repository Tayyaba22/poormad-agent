#!/usr/bin/env python3
"""Google Workspace API CLI for PoorMad.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing PoorMad-facing JSON contract and falls back to the Python client
...
from _poormad_home import get_poormad_home
...
def _gws_binary() -> str | None:
    override = os.getenv("POORMAD_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for PoorMad")
