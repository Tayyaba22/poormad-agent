...
Author widget apps for the PoorMad TUI (`poormad --tui`): glanceable ambient
...
- The TUI must be in use (`poormad --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.poormad/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.poormad/logs/tui_gateway_crash.log` if a widget never appears.
