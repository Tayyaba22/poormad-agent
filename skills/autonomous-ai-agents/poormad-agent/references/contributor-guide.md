...
For occasional contributors and PR authors. Full developer docs: https://poormad.dev/docs/developer-guide/
...
Config: `~/.poormad/config.yaml` (settings), `~/.poormad/.env` (API keys) — both under `$POORMAD_HOME` when it is set.
...
`_POORMAD_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_poormad_home()` for paths,
never hardcode `~/.poormad`. For custom/local-only tools, write a plugin in
`~/.poormad/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `poormad_cli/commands.py`
...
- Tests auto-redirect `POORMAD_HOME` to temp dirs — never touch real `~/.poormad/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/poormad_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_poormad_home()` from `poormad_constants` for all paths (profile-safe)
