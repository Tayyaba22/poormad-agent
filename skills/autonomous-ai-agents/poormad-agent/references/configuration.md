...
Edit with `poormad config edit` or `poormad config set section.key value`.
Full reference: https://poormad.dev/docs/user-guide/configuration
...
`poormad config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `poormad tools` (interactive) or `poormad tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_POORMAD_CORE_TOOLS` is the default bundle most platforms inherit).
