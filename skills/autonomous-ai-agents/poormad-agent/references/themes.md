# Themes / Skins — Author a PoorMad Color Theme

Author a PoorMad **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`poormad_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.poormad/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for PoorMad itself.
...
- Write access to the PoorMad home dir — `~/.poormad` by default, or `$POORMAD_HOME`
  / the active profile's dir. Skins live in `<poormad-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `poormad config set`).
...
3. `write_file` it to `<poormad-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `poormad config set display.skin default` (or they can `/skin default`).
...
```
poormad skin set <key> <hex>      # e.g. poormad skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `poormad skin set background "#08201f"` changes
only the background; `poormad skin use <name>` / `poormad skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.poormad`** when a profile is active — resolve the real home
  from `$POORMAD_HOME` first, falling back to `~/.poormad`.
...
- **Never hand-edit `config.yaml` to activate.** Use `poormad config set
...
- **You apply it, not the user.** `poormad config set display.skin <name>` is
...
- `read_file` the written `<poormad-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `poormad config get display.skin` and confirm it reports `<name>`.
