...
Set via `poormad model` (picker) or `poormad setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://poormad.dev/docs/integrations/providers
...
| nous | OAuth device code | `poormad auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `poormad auth add openai-codex` |
| qwen-oauth | OAuth | `poormad auth add qwen-oauth` |
| minimax-oauth | OAuth | `poormad auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`poormad auth`).
Fallback chain when the primary fails: `poormad fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`poormad_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   poormad config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
