...
Write plugins for the PoorMad desktop app: statusbar items, layout panes,
...
system (`~/.poormad/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$POORMAD_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$POORMAD_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The PoorMad desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$POORMAD_HOME/desktop-plugins/` (usually
  `~/.poormad/desktop-plugins/`).

## How to Run

1. Create `$POORMAD_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.poormad/...` by default, or `~/.poormad/profiles/<profile>/...` under a
...
The ONLY import surface is `@poormad/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from PoorMad
  ...
  (`poormad://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.poormad/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@poormad/plugin-sdk`, `react`, and
