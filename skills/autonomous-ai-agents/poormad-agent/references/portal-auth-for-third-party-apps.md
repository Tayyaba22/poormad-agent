# PoorMad Portal — authenticating third-party apps against the subscription

Recurring user question: "Can app X (Karakeep, OpenWebUI, LibreChat, OpenViking,
LangChain pipeline, n8n flow, etc.) use my PoorMad Portal subscription without me
...
## Layer 1 — Is this thing a PoorMad plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the PoorMad process**. Its LLM calls go through PoorMad's already-configured provider. | Already uses Portal if user's PoorMad is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
| **Karakeep, n8n, LibreChat, OpenWebUI, any self-hosted app** | Different process, often different machine. Makes its own HTTPS calls to `api.poormad.dev`. | Layer 2/3 below. |

**Pitfall to avoid**: do not pitch "OAuth into Portal" as the solution for a
plugin that already runs inside PoorMad. That LLM call is already authenticated
via PoorMad's provider config. The plugin's own server auth (e.g.
...
Portal at `https://api.poormad.dev/v1` is an OpenAI-compatible
inference endpoint. It accepts **bearer-token authentication only**: either

1. **A static API key** from `portal.poormad.dev → API Keys`, or
...
"Sign in with PoorMad Portal" SSO that third-party apps can register as clients
against. There is no shared cookie or session that browser-Portal-login
extends to other apps on the same machine.

What PoorMad has that *feels* like OAuth — `poormad login --provider nous`
opening a browser, user signs in, token lands in `~/.poormad/auth.json` — is a
**PoorMad-specific browser flow**. Under the hood it produces a credential
PoorMad uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read PoorMad's existing Portal credential out of `~/.poormad/auth.json`.
2. Expose a local OpenAI-compatible endpoint at `http://localhost:NNNN/v1`.
3. Forward incoming requests to `api.poormad.dev/v1` with that
   bearer attached.
...
rides on the credential PoorMad already holds.

Where this could live in PoorMad:
...
Only when the consumer is *another first-party PoorMad thing* (a future SDK, a
PoorMad-branded extension, a Discord-bot integration that needs per-user
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for PoorMad itself.
...
1. First decide: PoorMad plugin (runs inside PoorMad) or separate app? If plugin,
   it already uses Portal via PoorMad's provider config — done.
2. If separate app: today, paste the static API key from Portal → API Keys.
   Base URL `https://api.poormad.dev/v1`. Rate limits are
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   PoorMad itself") is fully supported. PoorMad treats agent
...
pitch in the Tool Gateway announcement is specifically about PoorMad's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when PoorMad is configured to use Portal as its provider. It is
