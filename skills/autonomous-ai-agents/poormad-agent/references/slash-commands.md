...
Registry of record: `poormad_cli/commands.py` (`COMMAND_REGISTRY`) — every
...
/snapshot [sub]          Create/restore PoorMad config+state snapshots (CLI)
...
/subscription (/upgrade) PoorMad plan info (CLI)
/topup                   PoorMad balance / billing
/copy [N]                Copy last response to clipboard (CLI)
/paste                   Attach clipboard image (CLI)
/image <path>            Attach a local image file (CLI)
/update                  Update PoorMad to latest
