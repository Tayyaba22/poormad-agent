...
Ledger path resolution (first wins):
  --ledger PATH
  $POORMAD_CITATION_LEDGER
  $POORMAD_HOME/cache/citations/ledger.json
"""
...
from _poormad_home import get_poormad_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("POORMAD_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_poormad_home() / "cache" / "citations" / "ledger.json"
