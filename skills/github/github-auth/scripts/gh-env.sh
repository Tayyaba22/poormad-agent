#!/usr/bin/env bash
# GitHub environment detection helper for PoorMad skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _poormad_env="${POORMAD_HOME:-$HOME/.poormad}/.env"; [ -f "$_poormad_env" ] && grep -q "^GITHUB_TOKEN=" "$_poormad_env" 2>/dev/null; then
