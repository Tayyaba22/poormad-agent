...
POORMAD_HOME_DIR="${POORMAD_HOME:-$HOME/.poormad}"
POORMAD_CFG="${POORMAD_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure PoorMad config has twozero_td MCP entry ──
if [[ ! -f "$POORMAD_CFG" ]]; then
    echo -e " ${FAIL} PoorMad config not found at ${POORMAD_CFG}"
    manual_steps+=("Create ${POORMAD_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$POORMAD_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in PoorMad config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to PoorMad config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$POORMAD_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${POORMAD_CFG} manually"); }
    manual_steps+=("Restart PoorMad session to pick up config change")
fi
