...
## 5. PoorMad Configuration

### Config location

`$POORMAD_HOME/config.yaml` (defaults to `~/.poormad/config.yaml` when `POORMAD_HOME` is unset)
...
Restart the PoorMad session for changes to take effect. The MCP connection is
