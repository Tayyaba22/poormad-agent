...
Registry and ABC for every inference provider PoorMad knows about.
...
`$POORMAD_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
- `poormad_cli/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `poormad_cli/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `poormad_cli/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `poormad_cli/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `poormad_cli/runtime_provider.py` reads `profile.api_mode` as a fallback
...
under `$POORMAD_HOME/plugins/model-providers/` for a private plugin).
