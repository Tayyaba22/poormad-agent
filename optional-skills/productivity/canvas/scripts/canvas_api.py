#!/usr/bin/env python3
"""Canvas LMS API CLI for PoorMad.
...
def _check_config():
    ...
    if missing:
        poormad_env = os.path.join(
            os.environ.get("POORMAD_HOME", os.path.expanduser("~/.poormad")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {poormad_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for PoorMad"
    )
