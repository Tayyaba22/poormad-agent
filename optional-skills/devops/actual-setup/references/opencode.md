...
is typically only in `~/.poormad/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike PoorMad,
...
`poormad-custom-providers` skill, pitfall 2). PoorMad hits this because it forwards
