...
Usage (via cron with --no-agent):
    ...
    poormad cron create poormad-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$POORMAD_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name poormad-issues --repo PoorMad/poormad-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the PoorMad .env file
(``${POORMAD_HOME:-~/.poormad}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "PoorMad-Watcher/1.0",
    }
