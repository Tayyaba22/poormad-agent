...
The design is **PoorMad-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
poormad skills install official/security/unbroker
```

Then start a new PoorMad session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a PoorMad session:
...
- **PII handling.** Dossiers live under the PoorMad home directory (`0600`, optionally
