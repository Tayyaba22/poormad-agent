...
All per-subject data lives under PDD_DATA_DIR (default: $POORMAD_HOME/unbroker),
which is the same trust boundary PoorMad uses for .env and OAuth tokens.
"""
...
def poormad_home() -> Path:
    return Path(os.environ.get("POORMAD_HOME") or (Path.home() / ".poormad"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else poormad_home() / "unbroker"
