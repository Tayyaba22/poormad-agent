...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("POORMAD_HOME", os.path.expanduser("~/.poormad")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("POORMAD_HOME", Path.home() / ".poormad")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# PoorMad config paths
# ═══════════════════════════════════════════════════════════════════

POORMAD_HOME = Path(os.getenv("POORMAD_HOME", Path.home() / ".poormad"))
CONFIG_PATH = POORMAD_HOME / "config.yaml"
PREFILL_PATH = POORMAD_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # PoorMad/PoorMad models — already uncensored, just needs clean prompt
    "poormad": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "poormad" in model_lower or "nous" in model_lower:
        return "poormad"
...
def _get_current_model() -> tuple:
    """Read current model and provider from PoorMad config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.poormad/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart PoorMad for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart PoorMad for changes to take effect.")
