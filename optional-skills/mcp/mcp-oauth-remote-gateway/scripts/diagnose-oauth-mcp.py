...
Usage:
  ...
  <server_name> matches the files in $POORMAD_HOME/mcp-tokens/<server>.json etc.
...
def _poormad_home():
    # Prefer PoorMad' own resolver (profile-safe); fall back to env then ~/.poormad.
    try:
        from poormad_constants import get_poormad_home
        return str(get_poormad_home())
    except Exception:
        return os.environ.get("POORMAD_HOME") or os.path.expanduser("~/.poormad")


def _tokens_dir():
    return os.path.join(_poormad_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "poormad-diag", "version": "1.0"}},
        },
        headers={
