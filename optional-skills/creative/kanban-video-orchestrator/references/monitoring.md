...
poormad kanban watch --tenant <project-slug>

# Snapshot of the board
poormad kanban list --tenant <project-slug>
poormad kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
poormad kanban stats --tenant <project-slug>

# Visual dashboard (browser)
poormad dashboard

# Inspect a specific task (includes comments + events)
poormad kanban show <task-id>

# Follow a single task's event stream
poormad kanban tail <task-id>
```

Verify available subcommands with `poormad kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `poormad kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `poormad kanban block <id>` then `poormad kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `poormad kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `poormad kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `poormad profile list` (profile exists?), `poormad status` (gateway/dispatcher up?), and verify tenant. |
...
poormad kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
poormad kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(poormad kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
poormad kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
poormad kanban block <task-id>
poormad kanban archive <task-id>

# Diagnose what happened
poormad kanban show <task-id>      # task body, comments, recent events
poormad kanban tail <task-id>      # follow the live event stream
poormad kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `poormad kanban create` for the director
...
while true; do
    clear
    poormad kanban list --tenant <slug>
    echo "---"
    poormad kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `poormad kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `poormad profile list` and start
