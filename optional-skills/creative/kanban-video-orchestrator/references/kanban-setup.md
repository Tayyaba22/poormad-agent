...
the actual `setup.sh` that creates the project workspace, configures PoorMad
...
> [PoorMad/kanban-video-pipeline](https://github.com/PoorMad/kanban-video-pipeline).
...
2. **Create profiles** — `poormad profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.poormad/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `poormad kanban create` assigned to the director
...
```bash
poormad profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.poormad/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.poormad/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most PoorMad
...
Each profile gets a `SOUL.md` at `~/.poormad/profiles/<name>/SOUL.md` that
...
```bash
poormad kanban create "Direct production of <video title>" \
    --assignee director \
...
the PoorMad `.env` (`${POORMAD_HOME:-$HOME/.poormad}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _poormad_env="${POORMAD_HOME:-$HOME/.poormad}/.env"
    if grep -q "^${var}=" "$_poormad_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_poormad_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_poormad_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY poormad ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY poormad OPENROUTER_API_KEY || exit 1
# ...
```
