...
Maps each role archetype to the PoorMad skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public poormad-agent
...
## PoorMad skills relevant to video production

### Visual / rendering skills (`poormad-agent/skills/creative/`)
...
### Design / pre-production skills (`poormad-agent/skills/creative/`)
...
### Audio / media skills (`poormad-agent/skills/creative/` + `skills/media/`)
...
These are **not** PoorMad skills but external CLIs / APIs that profiles invoke.
...
## Built-in PoorMad tools for media review

These are native PoorMad tools — not invoked via terminal but through their own
...
key is present in `${POORMAD_HOME:-~/.poormad}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every PoorMad profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${POORMAD_HOME:-~/.poormad}/.env` → environment variable.
...
When a new PoorMad-public video skill ships:
