...
### 1. Start order: editor first, PoorMad second

PoorMad probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW PoorMad session.
...
restart the editor, restart the PoorMad session.
...
server fails to bind (Output Log) or PoorMad' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.poormad/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for PoorMad

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. PoorMad' connection
lives in `~/.poormad/config.yaml` via `poormad mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms PoorMad — just don't
mistake it for the PoorMad setup step.
...
explicitly warns against overlapping calls. PoorMad executes same-turn tool
...
### 9. Timeouts: PoorMad gives up before Unreal does

PoorMad' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If PoorMad runs elsewhere (SSH backend,
