...
POORMAD_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env POORMAD_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``poormad config get terminal.backend`` still says local.
    """
    poormad_home = tmp_path / ".poormad"
    poormad_home.mkdir()
    (poormad_home / "config.yaml").write_text(
    ...
    (poormad_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
