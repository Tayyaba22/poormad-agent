...
variant that the ``_UNSET``-strip guard does NOT catch:
    ...
    *concurrent* message A had ALREADY called ``set_session_vars``, B inherits
    ...
    ``POORMAD_SESSION_*`` identity through the subprocess-env bridge. The bridge's
...
def _spawn_view():
    ...
    env = _make_run_env({})
    return {
        "POORMAD_SESSION_CHAT_ID": env.get("POORMAD_SESSION_CHAT_ID"),
        "POORMAD_SESSION_THREAD_ID": env.get("POORMAD_SESSION_THREAD_ID"),
        "POORMAD_SESSION_KEY": env.get("POORMAD_SESSION_KEY"),
    }
...
def test_reset_session_vars_closes_inheritance_leak():
    ...
    for var in ("POORMAD_SESSION_CHAT_ID", "POORMAD_SESSION_THREAD_ID", "POORMAD_SESSION_KEY"):
    ...
    assert captured["bound"]["POORMAD_SESSION_CHAT_ID"] == "FOREIGN_CHAT"
    assert captured["bound"]["POORMAD_SESSION_KEY"] == FOREIGN["session_key"]
...
# flag read via ``async_delivery_supported()``, not a string ``POORMAD_SESSION_*``
