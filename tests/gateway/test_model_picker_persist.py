...
closure the PR changed, against a real temp ``POORMAD_HOME``.
"""
...
def _fake_switch_result():
    ...
    from poormad_cli.model_switch import ModelSwitchResult
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "poormad_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "poormad_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_poormad_home", poormad_home)
    _stub_picker_dependencies(monkeypatch)
    # save_config writes to ``get_poormad_home() / config.yaml`` — point it here.
    monkeypatch.setattr("poormad_constants.get_poormad_home", lambda: poormad_home)
    monkeypatch.setattr("poormad_cli.config.get_poormad_home", lambda: poormad_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_poormad_home", default_home)
    _stub_picker_dependencies(monkeypatch)
