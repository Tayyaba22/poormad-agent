...
class TestHandleUpdateCommand:
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _poormad_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._poormad_home", tmp_path), \
        ...
        with patch("gateway.run._poormad_home", tmp_path):
            ...
            # project_root will be the real poormad-agent dir (which HAS .git).
    ...
    @pytest.mark.asyncio
    async def test_resolve_poormad_bin_fallback(self):
        """_resolve_poormad_bin falls back to sys.executable argv when which fails."""
        import sys
        from gateway.run import _resolve_poormad_bin

        fake_spec = MagicMock()
        with patch("shutil.which", return_value=None), \
             patch("importlib.util.find_spec", return_value=fake_spec):
            result = _resolve_poormad_bin()

        assert result == [sys.executable, "-m", "poormad_cli.main"]
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._poormad_home", poormad_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/poormad" if x == "poormad" else "/usr/bin/setsid"), \
             patch("subprocess.Popen"):
            result = await runner._handle_update_command(event)

        pending_path = poormad_home / ".update_pending.json"
        ...
        assert not (poormad_home / ".update_exit_code").exists()
    ...
    async def test_fallback_when_no_setsid(self, tmp_path):
        ...
        assert "Starting PoorMad update" in result
...
class TestUpdateCommandPlatformGate:
    ...
    async def test_allows_plugin_platform_via_registry_fallback(self, monkeypatch):
        ...
        monkeypatch.setenv("POORMAD_MANAGED", "")
        ...
        # Later steps may legitimately return success ("Starting PoorMad
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (poormad_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = poormad_home / ".update_pending.claimed.json"
        claimed_path.write_text(json.dumps({
            "platform": "telegram", "chat_id": "67890", "user_id": "12345",
        }))
        (poormad_home / ".update_output.txt").write_text("done")
        (poormad_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_notification_with_output(self, tmp_path):
        ...
        (poormad_home / ".update_pending.json").write_text(json.dumps(pending))
        (poormad_home / ".update_output.txt").write_text(
    ...
    async def test_cleans_up_on_error(self, tmp_path):
        ...
        pending_path = poormad_home / ".update_pending.json"
        output_path = poormad_home / ".update_output.txt"
        exit_code_path = poormad_home / ".update_exit_code"
        pending_path.write_text(json.dumps({
    ...
    async def test_no_adapter_for_platform_preserves_markers(self, tmp_path):
        ...
        assert not (poormad_home / ".update_pending.claimed.json").exists()
    ...
    async def test_completion_notification_tolerates_invalid_utf8_output(self, tmp_path):
        ...
        assert "PoorMad update finished" in sent_text
        assert not pending_path.exists()
        assert not output_path.exists()
        assert not exit_code_path.exists()
...
class TestWatchUpdateProgress:
    ...
    async def test_invalid_utf8_update_output_does_not_crash_watcher(self, tmp_path):
        ...
        (poormad_home / ".update_pending.json").write_text(json.dumps({
            "platform": "telegram",
            "chat_id": "67890",
            "user_id": "12345",
        }))
        (poormad_home / ".update_output.txt").write_bytes(
        ...
        assert "PoorMad update finished" in sent
        assert not (poormad_home / ".update_pending.json").exists()
