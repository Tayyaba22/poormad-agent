...
def test_session_store_default_db_uses_runtime_poormad_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime POORMAD_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    poormad_state before a fixture redirected POORMAD_HOME used to pin every
    default SessionDB() at the developer's real ~/.poormad/state.db.
    """
    config = GatewayConfig(default_reset_policy=SessionResetPolicy(mode="none"))
    fake_home = tmp_path / "alt_poormad_home"
    fake_home.mkdir()
    monkeypatch.setenv("POORMAD_HOME", str(fake_home))
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "poormad sessions list" in raw["_README"]
