"""Unit tests for the generic-OIDC / PoorMad-Portal caller-identity token resolver.

Covers gateway.relay._resolve_relay_identity_token() — the canonical resolver
shared by the runtime self-provision path and the `poormad gateway enroll` CLI.
...
Three modes:
  ...
  2. PoorMad Portal (resolve_nous_access_token) otherwise — the default.

The HTTP calls and the PoorMad resolver are monkeypatched; these prove the mode
