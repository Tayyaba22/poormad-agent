...
def test_recover_closes_owned_db_when_unexpected_exception_escapes(
    ...
    db = InterruptingDB()
    monkeypatch.setattr("poormad_state.SessionDB", lambda: db)
...
def test_get_flush_dir_uses_get_poormad_home(tmp_path, monkeypatch):
    """Flush dir must use get_poormad_home(), not hardcoded Path.home()."""
    import gateway.shutdown_flush as mod

    captured = {}

    def fake_get_poormad_home():
        from pathlib import Path
        captured["called"] = True
        return tmp_path

    monkeypatch.setattr(
        "poormad_constants.get_poormad_home", fake_get_poormad_home
    )
