...
(/opt/poormad/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable POORMAD_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_poormad_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable POORMAD_HOME."""
    ...
    monkeypatch.setattr(
        "poormad_constants.get_poormad_home", lambda: poormad_home
    )
    ...
    expected = poormad_home / "scripts" / "whatsapp-bridge"
