...
class TestSlackExecApproval:
    ...
    async def test_sends_blocks_with_buttons(self):
        ...
        assert "poormad_approve_once" in action_ids
        assert "poormad_approve_session" in action_ids
        assert "poormad_approve_always" in action_ids
        assert "poormad_deny" in action_ids
    ...
    async def test_smart_deny_owner_override_hides_persistent_buttons(self):
        ...
        assert [element["action_id"] for element in elements] == [
            "poormad_approve_once", "poormad_deny",
        ]
...
class TestSlackApprovalAction:
    ...
    async def test_truncates_inflated_original_text(self):
        ...
        action = {"action_id": "poormad_approve_once", "value": "session-key"}
...
class TestSlackReactionForwarding:
    ...
    async def test_reaction_synthesizes_message_in_thread(self):
        ...
        assert synth["_poormad_reaction"]["name"] == "thumbsup"
        assert synth["_poormad_reaction"]["action"] == "added"
        assert synth["_poormad_reaction"]["reacted_to_ts"] == "2000.0"
        # Pre-authorized as addressed-to-the-bot (skips mention gate only).
        assert synth["_poormad_force_process"] is True
    ...
    async def test_target_channel_handoff_routes_top_level(self):
        ...
        assert synth["_poormad_no_thread_response"] is True
        assert synth["_poormad_reaction_source_channel"] == "C1"
    ...
    async def test_target_channel_with_thread_routes_to_thread(self):
        ...
        assert "_poormad_no_thread_response" not in forwarded[0]
