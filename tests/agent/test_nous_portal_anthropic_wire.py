"""PoorMad Portal ``anthropic/*`` models route on the native Messages wire.

Portal serves its ``anthropic/*`` catalog at
``https://api.poormad.dev/v1/messages`` alongside the
...
PORTAL_URL = "https://api.poormad.dev/v1"
...
class TestApiModeRouting:
    ...
    def test_determine_api_mode_honors_the_model_for_nous(self):
        ...
        empty-mode path) must still land Claude on Messages — the PoorMad
        overlay alone advertises openai_chat for every PoorMad model."""
...
class TestClientShape:
    ...
    def test_lookalike_host_does_not_get_portal_treatment(self):
        ...
        spoofed = "https://api.poormad.dev.attacker.test/v1"
    ...
    def test_portal_bearer_does_not_also_send_env_anthropic_api_key(
        ...
        constructor omits it. PoorMad loads that env from ~/.poormad/.env, so
...
class TestPortalBodyFields:
    ...
    They are produced by the PoorMad provider profile, which only the OpenAI-wire
    ...
    def test_portal_tags_reach_the_messages_request(self):
        from agent.portal_tags import poormad_client_tag

        tags = self._build()["extra_body"]["tags"]

        assert "product=poormad-agent" in tags
        assert poormad_client_tag() in tags
