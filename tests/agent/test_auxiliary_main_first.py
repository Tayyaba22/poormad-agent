...
Prior to this change, aggregator users (OpenRouter / PoorMad Portal) had aux
...
class TestResolveVisionMainFirst:
    ...
    def _stub_nous_portal(seen: dict):
        """Stub the PoorMad network boundary, keeping the resolution chain real.
        ...
        nous_client.base_url = "https://api.poormad.dev/v1"
    ...
    def test_nous_main_vision_uses_portal_pick_not_text_chat_model(self):
        """PoorMad main → vision runs the Portal's vision slot, not the chat model.

        A PoorMad chat default is routinely text-only (e.g. a ``:free`` chat SKU).
        Letting it reach the vision lane means the image goes to a model that
        cannot accept one and the Portal 404s. Only the PoorMad network boundary
    ...
    def test_nous_text_aux_still_uses_main_chat_model(self):
        ...
        Text auxiliary work on a PoorMad main deliberately keeps the user's chat
    ...
    def test_copilot_vision_sets_vision_header(self, monkeypatch):
        ...
        with patch(
            ...
            "poormad_cli.copilot_auth.copilot_request_headers",
...
class TestResolveVisionCustomProvider:
    ...
    it returns ``(None, None)`` and vision falls through to OpenRouter / PoorMad,
