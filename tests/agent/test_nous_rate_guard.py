"""Tests for agent/nous_rate_guard.py — cross-session PoorMad Portal rate limit guard."""
...
def rate_guard_env(tmp_path, monkeypatch):
    ...
    poormad_home = str(tmp_path / ".poormad")
    os.makedirs(poormad_home, exist_ok=True)
    monkeypatch.setenv("POORMAD_HOME", poormad_home)
    # Clear any cached module-level imports
    return poormad_home


class TestRecordPoorMadRateLimit:
...
class TestPoorMadRateLimitRemaining:
...
class TestClearPoorMadRateLimit:
...
class TestIsGenuinePoorMadRateLimit:
    """Tell a real account-level 429 apart from an upstream-capacity 429.

    PoorMad Portal multiplexes upstreams (DeepSeek, Kimi, MiMo, PoorMad).
