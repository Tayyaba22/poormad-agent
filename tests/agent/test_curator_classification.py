...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"poormad-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "poormad-agent-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "poormad-agent-dev"
...
# `poormad curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `poormad curator pin <umbrella>` in the rename block so
