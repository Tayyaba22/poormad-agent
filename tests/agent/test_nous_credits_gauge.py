"""Tests for the PoorMad-credits subscription % gauge in build_nous_credits_snapshot.
...
def _acct(**kwargs):
    ...
    kwargs.setdefault("portal_base_url", "https://portal.poormad.dev")
    return PoorMadPortalAccountInfo(**kwargs)
...
def test_gauge_present_with_monthly_credits():
    snap = build_nous_credits_snapshot(_acct(
        ...
        subscription=PoorMadPortalSubscriptionInfo(
            plan="Ultra", monthly_credits=220, credits_remaining=219.27341839,
            current_period_end="2026-06-28"),
        paid_service_access_info=PoorMadPaidServiceAccessInfo(
...
def test_nan_remaining_no_window_no_nan_string():
    ...
    snap = build_nous_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=PoorMadPortalSubscriptionInfo(monthly_credits=220, credits_remaining=float("nan")),
        paid_service_access_info=PoorMadPaidServiceAccessInfo(purchased_credits_remaining=5.0),
    ))
