...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate POORMAD_HOME under tmp_path."""
    monkeypatch.setenv("POORMAD_HOME", str(tmp_path / ".poormad"))
    (tmp_path / ".poormad").mkdir()

    # Force the constants/image cache helpers to re-read POORMAD_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("poormad_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_poormad_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under POORMAD_HOME — gateway cleanup
