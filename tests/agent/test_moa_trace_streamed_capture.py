...
path via ``poormad chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp POORMAD_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point POORMAD_HOME at a temp dir and turn moa.save_traces on."""
    poormad_home = tmp_path / ".poormad"
    poormad_home.mkdir()
    monkeypatch.setenv("POORMAD_HOME", str(poormad_home))

    # save_moa_turn reads config via poormad_cli.config.load_config; stub it to
    ...
    return poormad_home / "moa-traces"
