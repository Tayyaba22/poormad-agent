...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("poormad_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated POORMAD_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="poormad_api_content_")
    os.makedirs(os.path.join(test_home, ".poormad"))
    prev_home = os.environ.get("POORMAD_HOME")
    os.environ["POORMAD_HOME"] = os.path.join(test_home, ".poormad")
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["POORMAD_HOME"] = prev_home
