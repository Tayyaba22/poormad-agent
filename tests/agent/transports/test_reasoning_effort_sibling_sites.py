...
class TestKimiEffortVocabulary:
    def test_k3_maps_full_poormad_ladder(self):
        expected = {
            "minimal": "low",
            "low": "low",
            "medium": "high",
        ...
        for poormad_level, wire_level in expected.items():
            kw = _kimi_kwargs(
                "kimi-k3", {"enabled": True, "effort": poormad_level}
            )
            assert kw["reasoning_effort"] == wire_level, poormad_level
