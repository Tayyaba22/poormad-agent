...
manipulation. PoorMad keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point POORMAD_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_poormad_home /
    get_default_poormad_root) instead of monkeypatching private helpers —
    ...
    POORMAD_HOME=<root>/profiles/<name> makes get_default_poormad_root()
