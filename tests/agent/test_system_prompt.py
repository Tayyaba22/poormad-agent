...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_poormad_home``,
    ``get_default_poormad_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``POORMAD_HOME`` and no resolver mocks.
    """

    def test_real_poormad_home_under_profiles_renders_correct_paths(
        ...
        assert get_poormad_home() == profile_home
        assert get_default_poormad_root() == root

        agent = _make_agent(valid_tool_names=["read_file"])
        with patch("agent.coding_context._coding_mode", return_value="off"):
            prompt = "\n\n".join(_prompt_parts(agent).values())

        assert "Active PoorMad profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """POORMAD_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".poormad"
        ...
        assert "Active PoorMad profile: default." in prompt
...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "POORMAD_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "POORMAD_AGENT_HELP_GUIDANCE_NO_SKILLS", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_poormad_home", lambda: Path("/poormad"))

    expected_profile = (
        "Active PoorMad profile: default. Other profiles (if any) live "
        "under /poormad/profiles/<name>/. Each profile has its own skills/, "
    ...
    with (
        ...
        patch("poormad_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("poormad_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under POORMAD_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from poormad_cli import config as _cfgmod
