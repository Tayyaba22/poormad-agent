...
def isolated_home(monkeypatch):
    """Temp POORMAD_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="poormad_test_31179_")
    poormad_home = os.path.join(test_home, ".poormad")
    os.makedirs(poormad_home)
...
def _fresh_modules():
    """Drop cached poormad modules so each test reloads against current env."""
    _drop_reload_targets()
