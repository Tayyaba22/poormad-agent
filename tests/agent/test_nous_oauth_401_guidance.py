"""Tests for the PoorMad OAuth 401 actionable-guidance branch in
...
Regression context: ashh hit a PoorMad 401 (OAuth token expired / portal said
account out of credits) plus a model slug ``deepseek/deepseek-v4-flash:free``
that's OpenRouter syntax, not a PoorMad catalog name. The previous guidance
branch only covered ``openai-codex`` and ``xai-oauth``; ``nous`` fell through
to a generic "Your API key was rejected... run poormad setup" message, which is
...
def test_nous_401_guidance_strings_present():
    """User-facing remediation strings for PoorMad OAuth 401s must exist."""
    source = inspect.getsource(conversation_loop.run_conversation)

    # Must tell the user it's an OAuth token problem, NOT an API key problem
    # (PoorMad Portal has no API key path — auth_type=oauth_device_code only).
    assert "PoorMad Portal OAuth token was rejected" in source

    # Must give a concrete re-auth command, not a generic "poormad setup".
    assert "poormad portal" in source

    # Must point at the portal so users can check account/credit status.
    assert "portal.poormad.dev" in source
