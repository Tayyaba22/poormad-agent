"""POSIX advisory locks must survive PoorMad' own database inspection.
...
PoorMad used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `poormad sessions
...
out across PoorMad' inspection calls.
"""
...
def clean_registry():
    ...
    import poormad_cli.sqlite_safe_read as mod
...
def test_failed_close_keeps_connection_tracked(tmp_path, clean_registry):
    ...
    class ControllableConnection(sqlite3.Connection):
        def close(self):
            if getattr(self, "_poormad_fail_close", False):
    ...
    conn._poormad_fail_close = False
    conn.close()
    assert not has_live_connection(db)
    assert read_header_bytes_preopen(db, length=16) is not None
...
def test_probe_and_connect_do_not_race(tmp_path, clean_registry, monkeypatch):
    ...
    """
    import poormad_cli.sqlite_safe_read as ssr
