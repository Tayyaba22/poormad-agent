...
the shared /blueprint command handler (poormad_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated POORMAD_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("poormad://blueprint/morning-brief?")
