...
def cron_store(tmp_path, monkeypatch):
    ...
    (poormad_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("POORMAD_HOME", str(poormad_home))
    monkeypatch.setattr(J, "POORMAD_DIR", poormad_home)
    monkeypatch.setattr(J, "CRON_DIR", poormad_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", poormad_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", poormad_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_poormad_now", lambda: now):
            due = J.get_due_jobs()
