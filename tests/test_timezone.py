"""
Tests for timezone support (poormad_time module + integration points).
...
def _reset_poormad_time_cache():
    """Reset the poormad_time module cache (replacement for removed reset_cache)."""
    poormad_time._cached_tz = None
    poormad_time._cached_tz_name = None
    poormad_time._cache_resolved = False


# =========================================================================
# poormad_time.now() — core helper
# =========================================================================

class TestPoorMadTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_poormad_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["POORMAD_TIMEZONE"] = "Asia/Kolkata"
        result = poormad_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["POORMAD_TIMEZONE"] = "UTC"
        result = poormad_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["POORMAD_TIMEZONE"] = "Europe/London"
        tz = poormad_time.get_timezone()
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When POORMAD_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) POORMAD_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("POORMAD_TIMEZONE=" + os.environ.get("POORMAD_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "POORMAD_TIMEZONE=NOT_SET" in result["output"], (
            "POORMAD_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When POORMAD_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("POORMAD_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=poormad_tz) which shifted
        absolute time when system-local tz != PoorMad tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when PoorMad tz
        ...
        # Use a PoorMad timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _poormad_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["POORMAD_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_poormad_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of PoorMad timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["POORMAD_TIMEZONE"] = "US/Eastern"
        _reset_poormad_time_cache()
