...
binary download are exercised manually by `poormad secrets bitwarden
...
def poormad_home(tmp_path, monkeypatch):
    """Point PoorMad at an isolated home directory."""
    home = tmp_path / ".poormad"
    home.mkdir()
    monkeypatch.setenv("POORMAD_HOME", str(home))
    # Some modules cache get_poormad_home; clear if needed.
    import poormad_constants
    if hasattr(poormad_constants, "_POORMAD_HOME_CACHE"):
        poormad_constants._POORMAD_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(poormad_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from poormad_cli.env_loader import _apply_external_secret_sources
