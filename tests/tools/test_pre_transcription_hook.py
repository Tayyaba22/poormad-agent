...
PROMPT = "PoorMad, Teknium, PoorMad, kanban"
...
def _fake_hooks(monkeypatch, results):
    ...
    monkeypatch.setattr("poormad_cli.plugins.invoke_hook", _invoke)
    return captured
...
def _no_hooks(monkeypatch):
    ...
    monkeypatch.setattr("poormad_cli.plugins.has_hook", lambda name: False)
    monkeypatch.setattr("poormad_cli.plugins.invoke_hook", _boom)
...
class TestUnsupportedBackends:
    def test_xai_logs_debug_and_proceeds_without_prompt(
        ...
        monkeypatch.setattr(
            "tools.xai_http.poormad_xai_user_agent", lambda: "test-ua",
        )
...
def test_real_fixture_plugins_thread_prompt_in_registration_order(
    ...
    plugin_dir = poormad_home / "plugins" / "stt_vocab"
