...
the sync wire contract):
  ...
  * the access gate (PoorMad admin) making sync inert
...
# ---------------------------------------------------------------------------
# Access gate (PoorMad admin) + per-skill opt-in
# ---------------------------------------------------------------------------
...
def synced_env(tmp_path, monkeypatch):
    """A POORMAD_HOME with two opted-in skills + a token-carrying identity."""
    import poormad_constants
    home = tmp_path / "poormad"
    skills = home / "skills"
    skills.mkdir(parents=True)
    monkeypatch.setattr(poormad_constants, "get_poormad_home", lambda: home)
...
# ---------------------------------------------------------------------------
# Env-var configuration (PoorMad Cloud "on by default" via environment)
# ---------------------------------------------------------------------------

class TestEnvConfig:
    def test_base_url_env_wins(self, monkeypatch):
        monkeypatch.setenv("POORMAD_SYNC_BASE_URL", "https://plane.example/")
    ...
    def test_base_url_defaults_to_production(self, monkeypatch):
        ...
        monkeypatch.delenv("POORMAD_SYNC_BASE_URL", raising=False)
        monkeypatch.setattr("poormad_cli.config.load_config", lambda: {}, raising=False)
    ...
    def test_feature_enabled_env(self, monkeypatch):
        # Default off.
        monkeypatch.delenv("POORMAD_SYNC_ENABLED", raising=False)
        monkeypatch.setattr("poormad_cli.config.load_config", lambda: {}, raising=False)
        assert ssc.sync_feature_enabled() is False
        for truthy in ("1", "true", "YES", "on"):
            monkeypatch.setenv("POORMAD_SYNC_ENABLED", truthy)
            assert ssc.sync_feature_enabled() is True
        for falsy in ("0", "false", "off"):
            monkeypatch.setenv("POORMAD_SYNC_ENABLED", falsy)
            assert ssc.sync_feature_enabled() is False

    def test_default_opt_in_env(self, monkeypatch):
        monkeypatch.delenv("POORMAD_SYNC_DEFAULT_OPT_IN", raising=False)
        monkeypatch.setattr("poormad_cli.config.load_config", lambda: {}, raising=False)
        assert ssc.sync_default_opt_in() is False
        monkeypatch.setenv("POORMAD_SYNC_DEFAULT_OPT_IN", "true")
    ...
    def test_env_overrides_config_yaml(self, monkeypatch):
        ...
        monkeypatch.setenv("POORMAD_SYNC_ENABLED", "false")
        monkeypatch.setattr(
            "poormad_cli.config.load_config",
            lambda: {"sync": {"enabled": True}},
            raising=False,
...
class TestDeviceName:
    def test_default_is_hostname_seeded(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("POORMAD_SYNC_DEVICE_NAME", raising=False)
        monkeypatch.setattr(
    ...
    def test_existing_file_wins_over_default_and_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("POORMAD_SYNC_DEVICE_NAME", "cloud-seed")
        assert ssc.stable_device_id() == "Explicit Name"

    def test_env_seeds_first_use(self, tmp_path, monkeypatch):
        # PoorMad Cloud path: POORMAD_SYNC_DEVICE_NAME seeds the first-use label.
        monkeypatch.setattr(ssc, "_skills_dir", lambda: tmp_path)
        monkeypatch.setenv("POORMAD_SYNC_DEVICE_NAME", "poormad-cloud-ben-1")
        assert ssc.stable_device_id() == "poormad-cloud-ben-1"
        # persisted so it stays stable even if the env later changes
        assert (tmp_path / ".sync_device_id").read_text() == "poormad-cloud-ben-1"
        monkeypatch.setenv("POORMAD_SYNC_DEVICE_NAME", "changed")
        assert ssc.stable_device_id() == "poormad-cloud-ben-1"
