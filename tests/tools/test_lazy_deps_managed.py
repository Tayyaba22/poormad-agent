...
A package-manager install (NixOS, and anything else shipping PoorMad from a
...
def _missing_and_installable(monkeypatch):
    ...
    depend on the host's ~/.poormad/config.yaml (a local
...
def test_nixos_install_fails_fast_without_touching_the_installer(monkeypatch):
    monkeypatch.setattr("poormad_cli.config.get_managed_system", lambda: "nixos")
...
def test_durable_install_target_overrides_the_guard(monkeypatch, tmp_path):
    """The container deployment sets POORMAD_MANAGED *and* a writable target.

    Dockerfile sets POORMAD_LAZY_INSTALL_TARGET and the NixOS container module
    passes POORMAD_MANAGED=true; blocking there would break that deployment.
    """
    monkeypatch.setattr("poormad_cli.config.get_managed_system", lambda: "nixos")
...
def test_unreadable_config_fails_open(monkeypatch):
    ...
    monkeypatch.setattr("poormad_cli.config.get_managed_system", _raise)
