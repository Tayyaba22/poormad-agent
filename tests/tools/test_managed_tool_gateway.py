...
def test_resolve_managed_tool_gateway_derives_vendor_origin_from_shared_domain():
    with patch.dict(
        os.environ,
        {
            "TOOL_GATEWAY_DOMAIN": "poormad.com",
        },
        clear=False,
    ...
    assert result.gateway_origin == "https://firecrawl-gateway.poormad.com"
...
def test_resolve_managed_tool_gateway_is_disabled_without_subscription():
    with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "poormad.com"}, clear=False), \
...
def test_read_nous_access_token_refreshes_expiring_cached_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "poormad_cli.auth.resolve_nous_access_token",
...
def test_managed_vendor_endpoints_pin_the_deployed_gateway_url():
    ...
    with patch.dict(
        os.environ,
        {"TOOL_GATEWAY_DOMAIN": "poormad.com", "TOOL_GATEWAY_SCHEME": "https"},
        clear=False,
    ):
    ...
    assert endpoints == {
        "origin": "https://tool-gateway.poormad.com",
        "base_url": "https://tool-gateway.poormad.com/api/vendorx",
...
def test_managed_vendor_endpoints_do_not_consult_entitlement():
    ...
    assert endpoints["base_url"] == "https://tool-gateway.poormad.com/api/vendorx"
...
def test_managed_gateway_auth_headers_reflect_a_rotated_token():
    # Read fresh on every call: a PoorMad access token expires within the hour,
...
class TestManagedMediaUploader:
    ...
    def test_a_missing_credential_fails_before_any_request(self):
        ...
        with patch.object(managed_tool_gateway, "managed_nous_tools_enabled", return_value=True), \
            ...
            with pytest.raises(RuntimeError, match="no PoorMad credential"):
