"""Strict tool-provider selection: the `poormad tools` choice always wins.

Policy (owner decision): the provider string stored in config.yaml is what
runs at call time. "nous" → managed PoorMad Tool Gateway only; a vendor name →
...
selection and pointing at `poormad tools`.
...
MANAGED = SimpleNamespace(
    ...
    gateway_origin="https://gateway.poormad.com",
)
...
class TestImageFalStrictSelection:
    ...
    def test_nous_selection_unentitled_raises_selection_error(self):
        ...
        assert "poormad tools" in str(exc.value)
...
class TestSttStrictSelection:
    def test_nous_selection_beats_direct_openai_key(self):
        ...
        assert base_url.startswith("https://gateway.poormad.com")
...
class TestWriteProviderConfig:
    def test_managed_row_writes_nous_and_clears_legacy_flag(self):
        ...
        provider = {"name": "PoorMad Subscription", "tts_provider": "openai"}
    ...
    def test_managed_image_row_persists_nous_provider(self):
        ...
        config = {}
        provider = {"name": "PoorMad Subscription", "imagegen_backend": "fal"}
