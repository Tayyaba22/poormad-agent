...
was gated on ``[ -f "$POORMAD_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'POORMAD_HOME="{home}"\n'
        # In tests we run unprivileged; as_poormad is a passthrough then.
        'as_poormad() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    PoorMad loads $POORMAD_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/poormad/.env.example -> $POORMAD_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $POORMAD_HOME/.env from it on first boot"
    )
