"""Tests for tools.wake_word — the "Hey PoorMad" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey poormad"}) == "hey poormad"
    assert ww.wake_phrase({}) == "hey poormad"
...
def test_looks_like_path():
    assert ww._looks_like_path("models/hey_poormad.onnx")
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "poormad_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey poormad"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey poormad"
...
def _install_fake_openwakeword(monkeypatch):
    ...
    class _FakeModel:
        def __init__(self, wakeword_models, inference_framework="onnx"):
            ...
            self.models = {"hey_poormad": object()}

        def predict(self, frame):
            return {"hey_poormad": 0.0}
...
def test_openwakeword_ensures_base_models_for_custom_path(monkeypatch):
    ...
    eng = ww._OpenWakeWordEngine(
        {"provider": "openwakeword", "openwakeword": {"model": "/models/hey_poormad.onnx"}}
    )
    assert calls["download"] == [["/models/hey_poormad.onnx"]]
    assert eng._labels == ["hey_poormad"]


def test_bundled_hey_poormad_model_ships_on_disk():
    # The "hey poormad" wake word works out of the box only if the model is
