...
def test_output_path_rejects_poormad_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    poormad_home = tmp_path / "poormad-home"
    poormad_home.mkdir()
    monkeypatch.setattr(file_safety, "_poormad_home_path", lambda: poormad_home)
    monkeypatch.setattr(file_safety, "_poormad_root_path", lambda: poormad_home)

    target = poormad_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    poormad_home = tmp_path / "poormad-home"
    token_dir = poormad_home / "mcp-tokens"
