...
def _restore_tool_modules():
    original_poormad_home = os.environ.get("POORMAD_HOME")
    original_modules = {
        name: module
        ...
        or name == "poormad_cli"
        or name.startswith("poormad_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_poormad_home is None:
            os.environ.pop("POORMAD_HOME", None)
        else:
            os.environ["POORMAD_HOME"] = original_poormad_home
        _reset_modules(("tools", "poormad_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    poormad_cli = types.ModuleType("poormad_cli")
    poormad_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["poormad_cli"] = poormad_cli
    poormad_home = tmp_path / "poormad-home"
    os.environ["POORMAD_HOME"] = str(poormad_home)
    sys.modules["poormad_cli.config"] = types.SimpleNamespace(
        get_poormad_home=lambda: poormad_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="poormad-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": poormad_home / "modal_snapshots.json",
