...
def test_no_scrub_inherit_profile_home_bridges_context_override(tmp_path):
    ...
    token = set_poormad_home_override(str(tmp_path))
    try:
        env = build_subprocess_env(
    ...
    assert env["POORMAD_HOME"] == str(tmp_path)
...
def test_e2e_child_sees_poormad_home_and_no_planted_secret(tmp_path, monkeypatch):
    """A real child spawned with a factory-built env must see POORMAD_HOME
    ...
    code = (
        "import os, json; "
        "print(json.dumps({'home': os.environ.get('POORMAD_HOME'), "
    ...
    assert result["home"] == str(poormad_home)
...
# ---------------------------------------------------------------------------
# E2E regression (#93082): cron/no_agent children keep bare `poormad` on PATH
# ---------------------------------------------------------------------------


def test_e2e_scrubbed_env_resolves_bare_poormad_under_minimal_parent_path(monkeypatch):
    """Regression for #92998/#93082: a gateway launched by systemd/cron with a
    minimal PATH (no poormad console-script dir) must still hand cron job
    children an env whose PATH resolves bare ``poormad``.
    ...
    bin_dir = local_mod._resolve_poormad_bin_dir()
    if not bin_dir or not os.path.isfile(
        os.path.join(bin_dir, "poormad.exe" if os.name == "nt" else "poormad")
    ):
        pytest.skip("no real poormad console-script install available")

    # Simulate the service-manager minimal PATH: poormad dir absent.
    minimal_path = os.pathsep.join(["/usr/bin", "/bin"])
    monkeypatch.setenv("PATH", minimal_path)
    assert shutil.which("poormad", path=minimal_path) is None

    env = build_subprocess_env(scrub_secrets=True)  # cron _run_job_script path

    resolved = shutil.which("poormad", path=env.get("PATH", ""))
    assert resolved is not None, (
        f"bare 'poormad' must resolve from the child PATH {env.get('PATH')!r}"
    )
