...
from poormad_cli.nous_account import PoorMadPaidServiceAccessInfo, PoorMadPortalAccountInfo
...
# managed_nous_tools_enabled
# ---------------------------------------------------------------------------
class TestManagedPoorMadToolsEnabled:
    """Subscription-based gate: True for paid PoorMad subscribers."""

    def test_disabled_when_not_logged_in(self, monkeypatch):
        monkeypatch.setattr(
            "poormad_cli.nous_account.get_nous_portal_account_info",
            lambda: PoorMadPortalAccountInfo(logged_in=False, source="none", fresh=False),
        )
...
class TestPoorMadToolGatewayUnavailableMessage:
    def test_uses_entitlement_reason_for_logged_in_user(self, monkeypatch):
        monkeypatch.setattr(
            "poormad_cli.nous_account.get_nous_portal_account_info",
            lambda force_fresh=False: PoorMadPortalAccountInfo(
                logged_in=True,
                source="account_api",
                fresh=True,
