...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad POORMAD_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: POORMAD_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_poormad_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "POORMAD_HOME": "/h", "POORMAD_PROFILE": "p",
        "POORMAD_CONFIG": "/c.yaml", "POORMAD_ENV": "/e",
        "POORMAD_DELEGATED_CHILD_CONTEXT": "1",
        # other POORMAD_* → dropped (broad prefix removed)
        "POORMAD_BASE_URL": "https://x", "POORMAD_INTERACTIVE": "1",
        "POORMAD_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "POORMAD_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "POORMAD_BASE_URL", "POORMAD_INTERACTIVE", "POORMAD_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped POORMAD_* vars."""
    import logging
