...
Covers:
- :class:`PoorMadTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.PoorMadMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _POORMAD_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# PoorMadTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = PoorMadTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("POORMAD_HOME", str(tmp_path))
        storage = PoorMadTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (PoorMadMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: PoorMadTokenStorage, **context_attrs):
    ...
    provider = _POORMAD_PROVIDER_CLS.__new__(_POORMAD_PROVIDER_CLS)
    provider._poormad_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("POORMAD_HOME", str(tmp_path))
        storage = PoorMadTokenStorage("mgr-srv")
        ...
        with patch.object(
            _POORMAD_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("POORMAD_HOME", str(tmp_path))
        storage = PoorMadTokenStorage("flow-srv")
        ...
        with patch.object(
            _POORMAD_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
