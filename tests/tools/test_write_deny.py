...
class TestWriteDenyExactPaths:
    ...
    def test_poormad_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.poormad/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "poormad_root"
        ...
        # Sanity check: POORMAD_HOME does point to the profile dir, not the root.
        from poormad_constants import get_poormad_home, get_default_poormad_root
...
class TestWriteAllowed:
    ...
    def test_poormad_control_files_requested_writable(self):
