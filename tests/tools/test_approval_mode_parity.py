...
def test_mode_and_timeout_parity_across_surfaces(
    poormad_home, tui_server, yaml_text, expected_mode, expected_timeout
):
    approval_mod = _approval_module()

    _write_config(poormad_home, yaml_text)
...
def test_tui_loader_delegates_to_core(poormad_home, tui_server):
