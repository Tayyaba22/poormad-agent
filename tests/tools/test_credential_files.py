...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        poormad_home.mkdir()
        (poormad_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"POORMAD_HOME": str(poormad_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(poormad_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.poormad/token.json"
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        poormad_home = tmp_path / ".poormad"
        poormad_home.mkdir()
        (poormad_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        poormad_home = tmp_path / ".poormad"
        skills_dir = poormad_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.poormad/skills"


    def test_custom_container_base(self, tmp_path):
        poormad_home = tmp_path / ".poormad"
        (poormad_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"POORMAD_HOME": str(poormad_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.poormad")

        assert mounts[0]["container_path"] == "/home/user/.poormad/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.poormad/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.poormad/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape POORMAD_HOME."""
        monkeypatch.setenv("POORMAD_HOME", str(tmp_path / ".poormad"))
        (tmp_path / ".poormad").mkdir()

        # Create a sensitive file one level above poormad_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside poormad_home
    ...
    def test_nested_subdir_inside_poormad_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of POORMAD_HOME must be allowed."""
        poormad_home = tmp_path / ".poormad"
        poormad_home.mkdir()
        subdir = poormad_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside POORMAD_HOME pointing outside must be rejected."""
        poormad_home = tmp_path / ".poormad"
        poormad_home.mkdir()
        monkeypatch.setenv("POORMAD_HOME", str(poormad_home))

        # Create a sensitive file outside poormad_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside poormad_home pointing outside
        symlink = poormad_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes POORMAD_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, poormad_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape POORMAD_HOME."""
        poormad_home = tmp_path / ".poormad"
        poormad_home.mkdir()
        monkeypatch.setenv("POORMAD_HOME", str(poormad_home))
        ...
        sensitive.write_text("{}")
        self._write_config(poormad_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(poormad_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside POORMAD_HOME via config must still mount."""
        poormad_home = tmp_path / ".poormad"
        poormad_home.mkdir()
        monkeypatch.setenv("POORMAD_HOME", str(poormad_home))

        (poormad_home / "oauth.json").write_text("{}")
        self._write_config(poormad_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        poormad_home = tmp_path / ".poormad"
        poormad_home.mkdir()
        (poormad_home / "cache" / "documents").mkdir(parents=True)
        (poormad_home / "cache" / "audio").mkdir(parents=True)
        (poormad_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("POORMAD_HOME", str(poormad_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.poormad/cache/documents" in paths
        assert "/root/.poormad/cache/audio" in paths
        assert "/root/.poormad/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_poormad_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        poormad_home = tmp_path / ".poormad"
        poormad_home.mkdir()
        # Use legacy dir name with content — get_poormad_dir prefers
        # populated old over new.
        legacy_doc = poormad_home / "document_cache"
        legacy_img = poormad_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(poormad_home / "document_cache") in host_paths
        assert str(poormad_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.poormad/cache/documents" in container_paths
        assert "/root/.poormad/cache/images" in container_paths
    ...
    def test_empty_poormad_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.poormad/attachments" in container_paths
        assert "/root/.poormad/images" in container_paths
        assert "/root/.poormad/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``POORMAD_HOME/images``, not
        ...
        """
        poormad_home = tmp_path / ".poormad"
        (poormad_home / "images").mkdir(parents=True)
        monkeypatch.setenv("POORMAD_HOME", str(poormad_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.poormad/images" in by_container
        assert by_container["/root/.poormad/images"] == str(poormad_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        poormad_home = tmp_path / ".poormad"
        (poormad_home / "images").mkdir(parents=True)
        upload = poormad_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("POORMAD_HOME", str(poormad_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.poormad/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        poormad_home = tmp_path / ".poormad"
        img_dir = poormad_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.poormad/cache/images/generated.png"
        )
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        mapped = map_cache_path_to_container(str(poormad_home / "cache" / "images" / "x.png"))
        assert mapped == "/root/.poormad/cache/images/x.png"
        assert (poormad_home / "cache" / "images").is_dir()


class TestToAgentVisiblePathPerBackend:
    """#76577 follow-up: translation covers every backend that relocates the
    PoorMad cache — not just docker — and skips the ones where the host path
    stays correct (local; singularity auto-binds the host home)."""

    def _staged(self, tmp_path, monkeypatch):
        poormad_home = tmp_path / ".poormad"
        (poormad_home / "attachments").mkdir(parents=True)
        monkeypatch.setenv("POORMAD_HOME", str(poormad_home))
        return str(poormad_home / "attachments" / "drop.zip")

    def test_docker_maps_to_root_poormad(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "/root/.poormad/attachments/drop.zip"

    def test_ssh_maps_to_tilde_poormad(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.poormad/attachments/drop.zip"
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        poormad_home = tmp_path / ".poormad"
        doc_dir = poormad_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — POORMAD_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping POORMAD_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"POORMAD_HOME": str(home)}):
    ...
    def test_per_service_token_still_mounts(self, tmp_path):
        ...
        assert [m["container_path"] for m in mounts] == [
            "/root/.poormad/google_token.json"
        ]
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.poormad/google_token.json" in paths
        assert "/root/.poormad/.env" not in paths
