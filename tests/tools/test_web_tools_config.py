...
class TestFirecrawlClientConfig:
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "poormad.com"}):
            with patch("tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="https://firecrawl-gateway.poormad.com",
                    )
...
class TestBackendSelection:
    ...
    ``poormad tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def test_managed_gateway_does_not_preempt_explicit_tavily(self):
        """Regression: a PoorMad OAuth token (managed gateway "ready") must NOT
        beat an explicitly configured TAVILY_API_KEY in the fallback path.
        Free PoorMad tiers don't include web search, so the user's deliberate
    ...
    def test_managed_gateway_only_falls_through_to_firecrawl(self):
        """When no explicit-credential backend is configured, a PoorMad-managed
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from poormad_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.poormad/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.poormad/.env), not bare os.getenv. Parametrized over the four
    ...
    def test_tavily_request_reads_key_via_get_env_value(self, monkeypatch):
        ...
        with patch(
            ...
            assert headers["X-Client-Name"] == "poormad-agent"
