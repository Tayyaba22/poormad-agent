...
Older images made /opt/poormad/ui-tui and /opt/poormad/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV POORMAD_TUI_DIR=/opt/poormad/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R poormad:poormad /opt/poormad/ui-tui" not in text
    assert "chown -R poormad:poormad /opt/poormad/node_modules" not in text
