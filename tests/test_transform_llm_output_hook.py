...
from poormad_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(poormad_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <poormad_home>/plugins/<name> and opt it in."""
    plugin_dir = poormad_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("POORMAD_HOME", str(tmp_path / "poormad_empty"))
