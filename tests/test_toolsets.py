...
class TestToolsetConsistency:
    ...
    def test_poormad_platforms_share_core_tools(self):
        """All poormad-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on poormad-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["poormad-cli", "poormad-telegram", "poormad-discord", "poormad-whatsapp", "poormad-slack", "poormad-signal", "poormad-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_poormad_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("poormad-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("poormad-cli")
        second = resolve_toolset("poormad-cli")
        ...
        assert (
            "poormad-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("poormad-cli", include_registry=False)
        second = resolve_toolset("poormad-cli", include_registry=False)
        assert first == second
