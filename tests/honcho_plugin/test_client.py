...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "poormad"
        assert config.workspace_id == "poormad"
...
class TestFromGlobalConfig:
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"poormad": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_host_block_overrides_root_and_env(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"poormad": {"baseUrl": "http://host-block:9001"}},
        }))
    ...
    def test_base_url_full_precedence_chain(self, tmp_path):
        ...
        config_file = tmp_path / "config.json"
        layers = {
            "hosts": {"poormad": {"baseUrl": "http://host:1"}},
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="poormad-agent"
        ):
            result = config.resolve_session_name("/home/user/poormad-agent/subdir")
        assert result == "poormad-agent"


class TestResolveConfigPath:
    def test_prefers_poormad_home_when_exists(self, tmp_path):
        poormad_home = tmp_path / "poormad"
        poormad_home.mkdir()
        local_cfg = poormad_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: POORMAD_HOME points at ~/.poormad/profiles/<name>, so
        # _get_default_poormad_home() must resolve back to ~/.poormad — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".poormad"
        ...
        assert _get_default_poormad_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "poormad_coder"
        assert profile_host_key("default") == "poormad"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"POORMAD_HONCHO_HOST": "poormad.coder"}):
            assert resolve_active_host() == "poormad.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("POORMAD_HONCHO_HOST", None)
            # Temporarily remove poormad_cli.profiles to simulate import failure
            saved = sys.modules.get("poormad_cli.profiles")
            sys.modules["poormad_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "poormad"
            finally:
                if saved is not None:
                    sys.modules["poormad_cli.profiles"] = saved
                else:
                    sys.modules.pop("poormad_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="poormad_coder")
        assert config.host == "poormad_coder"
        assert config.workspace_id == "poormad"  # shared workspace
        assert config.ai_peer == "poormad_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"poormad": {"enabled": True, "aiPeer": "poormad"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_dot_form_legacy_host_key_keeps_local_api_key(self):
        """Regression for #37436: a legacy dot-form host block (poormad.work)
        ...
        cfg = HonchoClientConfig(
            ...
            host="poormad_work",
            workspace_id="poormad",
            raw={"hosts": {"poormad.work": {"apiKey": "explicit-local-key"}}},
        )
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_poormad_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("POORMAD_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("poormad_cli.profiles.get_active_profile_name", return_value="default"), \
