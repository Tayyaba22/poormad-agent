...
#
# Installs a real, earlier PoorMad the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `poormad`.
#
...
# Keep sandbox state out of the default .poormad-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".poormad-sandbox-e2e-$ROUTE"
export POORMAD_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/poormad/.poormad/poormad-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/poormad-agent.git"
...
LOG_DIR="${POORMAD_E2E_LOG_DIR:-$(mktemp -d -t poormad-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED poormad accept FLAG on `poormad update`?
#
...
# that silently rots. `poormad update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "poormad update --help 2>&1" | grep -qF -- "$flag"
}
...
require_poormad_works() {
  local when="$1" out
  out="$(in_sandbox "poormad --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "poormad --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "poormad runs $when"
}

# ── install the earlier PoorMad ─────────────────────────────────────────────
...
require_poormad_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: poormad update'
    ...
    # poormad which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="poormad update --yes"
    else
      update_cmd="poormad update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "poormad update failed ($update_cmd)"
    fi
    require_landed_on_target 'poormad update'
    require_poormad_works 'after poormad update'
    ;;
  installer)
    ...
    require_poormad_works 'after installer re-run'
    ;;
esac
