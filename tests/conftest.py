"""Shared fixtures for the poormad-agent test suite.
...
2. **Isolated POORMAD_HOME.** POORMAD_HOME points to a per-test tempdir so
   code reading ``~/.poormad/*`` via ``get_poormad_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".poormad"`` instead of the canonical
   ``get_poormad_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No POORMAD_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox POORMAD_HOME before ANY test module is imported ──────────────────
# `poormad_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_poormad_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.poormad/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes POORMAD_HOME, but fixtures run
...
# the REAL PoorMad root — capture it BEFORE the sandbox rewires POORMAD_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.poormad (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("POORMAD_KANBAN_HOME", "").strip()
_PRE_SANDBOX_POORMAD_HOME = os.environ.get("POORMAD_HOME", "")


def _poormad_home_points_at_production(value: str) -> bool:
    """True when a pre-set POORMAD_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    POORMAD_HOME=~/.poormad``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``poormad_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.poormad`` — the escape vector that landed pytest
    ...
    POORMAD_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".poormad").resolve()
    except Exception:
        return True
...
if _poormad_home_points_at_production(os.environ.get("POORMAD_HOME", "")):
    _SESSION_POORMAD_HOME = tempfile.mkdtemp(prefix="poormad-test-home-")
    os.environ["POORMAD_HOME"] = _SESSION_POORMAD_HOME
    atexit.register(shutil.rmtree, _SESSION_POORMAD_HOME, True)
...
# like a real CLI") — which used to disarm poormad_state's live-DB guard in
# the child at the same moment the child lost the POORMAD_HOME redirect.
# POORMAD_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# poormad_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export POORMAD_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["POORMAD_TEST_ISOLATION"] = os.environ.get("POORMAD_HOME", "") or "1"

#: POORMAD_HOME as it stood when conftest was imported - i.e. before any test
...
POORMAD_HOME_AT_CONFTEST_IMPORT = os.environ.get("POORMAD_HOME", "")
...
# POORMAD_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_POORMAD_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``poormad_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a PoorMad session inherits them and
    # poormad_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "POORMAD_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "POORMAD_INTERACTIVE",
    "POORMAD_QUIET",
    "POORMAD_TOOL_PROGRESS",
    "POORMAD_TOOL_PROGRESS_MODE",
    "POORMAD_MAX_ITERATIONS",
    "POORMAD_SESSION_PLATFORM",
    "POORMAD_SESSION_CHAT_ID",
    "POORMAD_SESSION_CHAT_NAME",
    "POORMAD_SESSION_CHAT_TYPE",
    "POORMAD_SESSION_THREAD_ID",
    "POORMAD_SESSION_SOURCE",
    "POORMAD_SESSION_KEY",
    "POORMAD_GATEWAY_SESSION",
    "POORMAD_CRON_SESSION",
    "_POORMAD_GATEWAY",
    "POORMAD_PLATFORM",
    "POORMAD_MODEL",
    "POORMAD_INFERENCE_MODEL",
    "POORMAD_INFERENCE_PROVIDER",
    "POORMAD_TUI_PROVIDER",
    "POORMAD_MANAGED",
    "POORMAD_MANAGED_DIR",
    "POORMAD_DEV",
    "POORMAD_CONTAINER",
    ...
    "POORMAD_TIMEZONE",
    "POORMAD_REDACT_SECRETS",
    "POORMAD_BACKGROUND_NOTIFICATIONS",
    "POORMAD_EXEC_ASK",
    "POORMAD_HOME_MODE",
    "POORMAD_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.poormad/kanban.db instead of the per-test POORMAD_HOME.
    "POORMAD_KANBAN_DB",
    "POORMAD_KANBAN_BOARD",
    "POORMAD_KANBAN_HOME",
    "POORMAD_KANBAN_WORKSPACES_ROOT",
    "POORMAD_KANBAN_LOGS_ROOT",
    "POORMAD_KANBAN_TASK",
    "POORMAD_KANBAN_WORKSPACE",
    "POORMAD_KANBAN_RUN_ID",
    "POORMAD_KANBAN_CLAIM_LOCK",
    "POORMAD_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "POORMAD_DELEGATED_CHILD_CONTEXT",
    "POORMAD_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "poormad" host.
    "POORMAD_HONCHO_HOST",
    ...
    "POORMAD_DASHBOARD_OAUTH_CLIENT_ID",
    "POORMAD_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and POORMAD_HOME to per-test tempdirs so code that
    reads ``~/.poormad/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral POORMAD_* vars that could change test semantics.
    for name in _POORMAD_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("POORMAD_HONCHO_HOST", "poormad")

    # 3. Redirect POORMAD_HOME to a per-test tempdir. Code that reads
    #    ``~/.poormad/*`` via ``get_poormad_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.poormad/*`` via
    #    ``Path.home() / ".poormad"`` instead of ``get_poormad_home()``
    #    is a bug to fix at the callsite.
    fake_poormad_home = tmp_path / "poormad_test"
    fake_poormad_home.mkdir()
    (fake_poormad_home / "sessions").mkdir()
    (fake_poormad_home / "cron").mkdir()
    (fake_poormad_home / "memories").mkdir()
    (fake_poormad_home / "skills").mkdir()
    monkeypatch.setenv("POORMAD_HOME", str(fake_poormad_home))
    ...
    # poormad_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("POORMAD_TEST_ISOLATION", str(fake_poormad_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("POORMAD_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. poormad_state computes ``DEFAULT_DB_PATH = get_poormad_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from poormad_state import ...``) that
    ...
    poormad_state_mod = sys.modules.get("poormad_state")
    if poormad_state_mod is not None and hasattr(poormad_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            poormad_state_mod, "DEFAULT_DB_PATH", fake_poormad_home / "state.db"
        )
    ...
    monkeypatch.setenv("POORMAD_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.poormad/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import poormad_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path POORMAD_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_poormad_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from poormad_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.poormad. This autouse
...
# allow-list because test-level fixtures legitimately move POORMAD_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session POORMAD_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``POORMAD_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) PoorMad root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_POORMAD_HOME and not _poormad_home_points_at_production(
        _PRE_SANDBOX_POORMAD_HOME
    ):
        # POORMAD_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_poormad_root().resolve()
    # No pre-existing POORMAD_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".poormad").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.poormad`` captured at import time. Hermetic tests that legitimately
    move POORMAD_HOME to sibling tempdirs are unaffected.

    Only patches when ``poormad_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("poormad_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import poormad_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.poormad. See #69283."
        )
...
# ``poormad_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL PoorMad root. This fixture wires the
# test-side knobs:
...
#     installs where POORMAD_HOME is not ~/.poormad) into the guard's
...
# so subprocess children that import poormad_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("poormad_state")
    ...
    if _PRE_SANDBOX_POORMAD_HOME and not _poormad_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_POORMAD_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local PoorMad home override redirects every later
    # ``get_poormad_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from poormad_constants import get_poormad_home_override, set_poormad_home_override

        if get_poormad_home_override() is not None:
            set_poormad_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal poormad config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``poormad-gateway`` process
...
# correlated with a ``tests/poormad_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> poormad-gateway``
...
def _wal_is_usable() -> bool:
    """True when PoorMad will actually put a database into WAL mode here.

    PoorMad refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running PoorMad can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a PoorMad managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``poormad_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_poormad_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_poormad_home`` fixture
    redirects ``POORMAD_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.poormad/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``poormad_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["POORMAD_VOICE_TTS"] = "1"``. The
...
#      ``poormad_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``poormad_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``poormad_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``poormad_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# PoorMad runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.poormad``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where PoorMad falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — PoorMad uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart poormad-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting poormad/python patterns are also blocked.
    """
    ...
    def _matches_poormad_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _POORMAD_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_poormad_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch poormad-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m poormad_cli.main".
                if (
                    "poormad" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live poormad-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting poormad/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `poormad update` (or the
        # equivalent `python -m poormad_cli.main update`).  These commands
        ...
        if "update" in low and (
            # poormad update / poormad update --gateway / setsid bash -c ... poormad update
            ("poormad" in low and "update" in low.split())
            or
            # python -m poormad_cli.main update --gateway
            ("poormad_cli" in low and "update" in low.split())
            or
            # venv/bin/poormad update  (absolute path variant used in tests)
            (".venv/bin/poormad" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`poormad update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_POORMAD_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import poormad_cli.voice as _voice
    except Exception:
