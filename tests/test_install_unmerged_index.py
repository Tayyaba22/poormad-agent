...
The ``poormad update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m poormad_cli.main gateway run`` — image name
    ``pythonw``, not ``poormad.exe`` — so the ``taskkill /IM poormad.exe`` guard
    ...
    # The poormad.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM poormad.exe' in text
