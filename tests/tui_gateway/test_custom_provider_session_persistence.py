...
"No LLM provider configured. Run `poormad model`..." (resume failed) or
...
def _make_agent_with_override(override, monkeypatch, config, model_cfg=None):
    ...
    # Keep credential-pool resolution off the developer's real POORMAD home.
...
# provider (e.g. PoorMad) but who switched THIS session to a self-hosted model
...
# found" (the b200/poormad-ultra-sft report). The stored MODEL NAME is the one
...
class TestModelNameRecoversEntryIdentity:
    def test_identity_by_model_from_providers_dict_models_list(self, monkeypatch):
        ...
        assert (
            rp.find_custom_provider_identity_by_model("poormad-ultra-sft")
            == "custom:poormad-ultra"
        )
...
# what left room bots stuck on a stale provider (e.g. "out of PoorMad credits"
...
#
# Regression: GH #89497 (room bots hang then report "out of PoorMad credits").
...
# "out of PoorMad credits" after the profile was switched to ollama-cloud) while
