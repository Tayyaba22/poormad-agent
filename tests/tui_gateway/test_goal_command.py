...
def poormad_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves POORMAD_HOME.
...
@pytest.fixture()
def server(poormad_home, monkeypatch):
    ...
    # Pin config resolution to the isolated POORMAD_HOME. Sibling test
    ...
    # module-level ``_poormad_home = get_poormad_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_poormad_home", poormad_home)
...
def test_moa_bare_returns_usage(server, session, poormad_home):
    _write_moa_config(poormad_home, """
moa:
