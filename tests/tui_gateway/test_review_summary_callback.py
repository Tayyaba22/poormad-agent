    ...
    with patch.dict(
        "sys.modules",
        {
            "poormad_constants": MagicMock(
                get_poormad_home=MagicMock(return_value="/tmp/poormad_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'poormad-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'poormad-release' patched"
    }
