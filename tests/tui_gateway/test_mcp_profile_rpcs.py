...
``POORMAD_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
@pytest.fixture
def poormad_root(tmp_path, monkeypatch):
    """A temp POORMAD_HOME root with two named profiles: 'work' and 'other'.

    Pointing POORMAD_HOME at a dir outside ~/.poormad makes it the profile ROOT
    (get_default_poormad_root's Docker/custom branch), so named profiles live at
...
def test_add_lands_in_named_profile_only(poormad_root):
    root = poormad_root
    resp = _call(
        "mcp.servers.add",
        {
...
def test_list_reflects_the_scoped_profile(poormad_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_writes_env_and_header_to_right_profile(poormad_root):
    root = poormad_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_stdio_references_env_block(poormad_root):
    root = poormad_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_remove_scoped_to_profile(poormad_root):
    root = poormad_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_duplicate_and_missing_errors(poormad_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_requires_transport(poormad_root):
    resp = _call("mcp.servers.add", {"profile": "work", "name": "empty", "config": {}})
    assert "error" in resp
    assert resp["error"]["code"] == 4063


def test_default_profile_add_when_profile_omitted(poormad_root):
    root = poormad_root
    _result(
        _call(
            "mcp.servers.add",
    ...
    # Omitted profile → launch/default profile == POORMAD_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")
