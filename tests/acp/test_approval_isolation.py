...
- 96vc: ACP didn't set POORMAD_EXEC_ASK, so `check_all_command_guards`
...
1. Setting POORMAD_EXEC_ASK inside _run_agent (wraps the agent call).
...
def _isolate_approval_state(monkeypatch):
    ...
    developer's real ``~/.poormad/config.yaml`` command allowlist into
...
class TestAcpExecAskGate:
    """GHSA-96vc-wcxf-jjff: ACP's _run_agent must set POORMAD_INTERACTIVE so
    ...
    (POORMAD_EXEC_ASK takes the gateway-queue path which requires a
    ...
    def test_interactive_env_var_routes_to_callback(self, monkeypatch):
        """When POORMAD_INTERACTIVE is set and an approval callback is
        ...
        # Without POORMAD_INTERACTIVE: takes auto-approve path, callback NOT called
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert result["approved"] is True
        assert called_with == [], (
            "without POORMAD_INTERACTIVE the non-interactive auto-approve "
            "path should fire without consulting the callback"
        )

        # With POORMAD_INTERACTIVE: callback IS called, approval flows through it
        ...
        assert called_with, (
            "with POORMAD_INTERACTIVE the approval path should consult the "
