...
def test_root_session_no_compression(db):
    ...
    assert prov["currentPoorMadSessionId"] == "root1"
    assert prov["rootPoorMadSessionId"] == "root1"
    assert prov["parentPoorMadSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_poormad_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentPoorMadSessionId"] == "old"
    assert prov["rootPoorMadSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousPoorMadSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootPoorMadSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"poormad"}
    assert "sessionProvenance" in meta["poormad"]
    assert meta["poormad"]["sessionProvenance"]["currentPoorMadSessionId"] == "root1"
