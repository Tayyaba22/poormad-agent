"""Tests for pip entry-point provider discovery (poormad_agent.plugins group).

Verifies that ``providers/__init__.py`` imports provider plugins exposed via a
distribution's ``poormad_agent.plugins`` entry point, supporting both a
...
class _FakeEP:
    def __init__(self, name, loader):
        self.name = name
        self.group = "poormad_agent.plugins"
        self._loader = loader
