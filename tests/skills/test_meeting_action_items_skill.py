...
def test_frontmatter_required_fields():
    ...
    poormad = fm["metadata"]["poormad"]
    assert poormad["tags"]
    assert "related_skills" in poormad
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("PoorMad"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["poormad"]["related_skills"]:
        hits = (
