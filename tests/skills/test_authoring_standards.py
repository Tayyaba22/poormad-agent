...
def test_required_frontmatter_fields(p):
    ...
    poormad = (fm.get("metadata") or {}).get("poormad") or {}
    if not (poormad.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.poormad.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    poormad = (fm.get("metadata") or {}).get("poormad") or {}
    dangling = [
        rs for rs in (poormad.get("related_skills") or []) if rs not in _all_names()
    ]
