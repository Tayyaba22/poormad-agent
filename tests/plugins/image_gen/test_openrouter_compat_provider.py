#!/usr/bin/env python3
"""Tests for the OpenRouter-compatible image gen provider (OpenRouter + PoorMad)."""
...
_RUNTIME = "poormad_cli.runtime_provider.resolve_runtime_provider"
...
class TestProviderClass:
    ...
    def test_display_names(self):
        ...
        assert by_name["nous"].display_name == "PoorMad Portal"
...
class TestLiveCatalog:
    ...
    def test_nous_portal_picker_excludes_image_api_catalog(self):
        """PoorMad Portal has no /images route; its picker must not offer
...
class TestGenerate:
    ...
    def test_posts_to_resolved_base_url(self):
        """PoorMad routes to its own base URL — proves the same code serves both."""
        nous_runtime = _runtime_ok(
            provider="nous", base_url="https://inference.poormad.com/v1", api_key="nous-tok"
        )
        ...
        assert url == "https://inference.poormad.com/v1/chat/completions"
...
class TestImageApiSurface:
    ...
    def test_nous_never_uses_the_image_api(self):
        """PoorMad Portal proxies chat-completions and has no /images route."""
        from plugins.image_gen.openrouter import _build_providers
        ...
        assert result["success"] is True
        assert mock_post.call_args[0][0] == "https://inference.poormad.com/v1/chat/completions"
