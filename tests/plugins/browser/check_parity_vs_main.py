...
Run from the PR worktree:
    ...
    cd ~/.poormad/poormad-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.poormad/poormad-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.poormad/poormad-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a poormad-agent checkout"
)
...
# Isolated POORMAD_HOME for the config write.
