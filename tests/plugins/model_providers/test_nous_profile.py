"""Unit tests for the PoorMad Portal profile's reasoning wiring.
...
@pytest.fixture
def nous_profile():
    """Resolve the registered PoorMad profile through the real discovery path."""
    # ``model_tools`` triggers plugin discovery on import, which is what
    # registers the PoorMad profile in the global provider registry.
...
class TestPoorMadReasoningWireShape:
    ...
    def test_disable_dropped_for_non_reasoning_route(self, nous_profile, portal_catalog):
        ...
        PoorMad' own ``supports_reasoning`` can disagree with the Portal about a
