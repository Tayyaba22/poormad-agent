...
def progress(tmp_path):
    ...
    status = tmp_path / "poormad-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen PoorMad to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `poormad update`, and reports the stage that was on screen
...
n="$(cat "$POORMAD_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$POORMAD_TEST_CALLS"
for f in "$TMPDIR"/poormad-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$POORMAD_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$POORMAD_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "poormad-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    poormad = install_root / "venv" / "bin" / "poormad"
    poormad.write_text(FAKE_POORMAD)
    poormad.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "POORMAD_TEST_CAPTURE": str(capture),
        "POORMAD_TEST_CALLS": str(calls),
        "POORMAD_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".poormad-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`poormad update` is the longest wait in the hand-off and the one the
