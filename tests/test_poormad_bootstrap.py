"""Tests for poormad_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every PoorMad entry point
(poormad, poormad-agent, poormad-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every PoorMad entry point imports poormad_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported poormad_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("poormad_bootstrap", None)
    import poormad_bootstrap  # noqa: WPS433
    return poormad_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the PoorMad CLI
...
class TestEntryPointsImportBootstrap:
    """Every PoorMad entry point must import poormad_bootstrap as its
    ...
    # Entry points that invoke PoorMad as a process.  Each one must
    # import poormad_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "poormad_cli/main.py",   # poormad CLI (console_script)
        "run_agent.py",          # poormad-agent (console_script)
        "acp_adapter/entry.py",  # poormad-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import poormad_bootstrap' and that
        ...
        half-finished ``poormad update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``poormad_bootstrap`` as a top-level module) leaves poormad
        ...
        """
        # Resolve relative to the poormad-agent repo root.  Tests live
        # at tests/test_poormad_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # poormad start even when poormad_bootstrap hasn't been
        ...
        assert first_import_name == "poormad_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'poormad_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import poormad_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing PoorMad's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("POORMAD_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["POORMAD_PYTHON_SRC_ROOT"] = env
            elif "POORMAD_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["POORMAD_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/poormad")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("POORMAD_PYTHON_SRC_ROOT", None)
            else:
                os.environ["POORMAD_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/poormad", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # PoorMad root.  The guard must relocate PoorMad to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/poormad"])
        assert result[0] == "/opt/poormad"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the PoorMad root.
        assert result.index("/opt/poormad") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["POORMAD_PYTHON_SRC_ROOT"] = "/env/poormad"
            hb.harden_import_path()
            assert sys.path[0] == "/env/poormad"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("POORMAD_PYTHON_SRC_ROOT", None)
