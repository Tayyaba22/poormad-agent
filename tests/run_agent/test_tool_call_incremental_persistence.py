...
def _make_agent():
    poormad_home = Path(tempfile.mkdtemp(prefix="poormad-test-home-"))
    (poormad_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._poormad_home", poormad_home),
