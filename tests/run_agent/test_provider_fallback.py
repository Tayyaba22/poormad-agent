...
class TestFallbackChainAdvancement:
    ...
    def test_nous_anthropic_fallback_uses_the_messages_wire(self):
        ...
        ``resolve_provider_client`` still returns an OpenAI client for PoorMad;
        ...
        """
        portal = "https://api.poormad.dev/v1"
        fbs = [
            {
                "provider": "nous",
        ...
        with (
            ...
            patch(
                "poormad_cli.model_normalize.normalize_model_for_provider",
...
class TestFallbackChainDedup:
    ...
    def test_skips_entry_matching_current_provider_and_model(self):
        ...
        with patch("agent.auxiliary_client.resolve_provider_client", side_effect=_resolve):
            with patch("poormad_cli.model_normalize.normalize_model_for_provider", side_effect=lambda m, p: m):
