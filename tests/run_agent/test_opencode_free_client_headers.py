...
@patch("run_agent.OpenAI")
def test_opencode_free_sends_poormad_attribution(mock_openai):
    """Keyless requests still identify as PoorMad (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "PoorMad"
    assert str(headers.get("User-Agent", "")).startswith("PoorMadAgent/")
