"""Regression guard: a genuine PoorMad 429 must re-enter the retry loop so the
top-of-loop PoorMad rate-limit guard can activate the fallback chain.
...
class TestGenuinePoorMad429ReentersLoop:
