"""Tests for PoorMad fallback local-availability suppression.

Blocker if PoorMad token material is missing locally: the fallback chain
should not repeatedly attempt PoorMad resolution; it must skip and continue
...
class TestPoorMadFallbackLocalAvailability:
    def test_missing_nous_token_is_skipped_once(self):
        """PoorMad fallback is skipped when no access/refresh token is stored."""
        agent = _make_agent(
            fallback_model=[
    ...
    def test_nous_unavailable_not_retried_in_same_session(self):
        """After PoorMad is skipped once, subsequent activations continue further."""
        agent = _make_agent(
            fallback_model=[
                {"provider": "nous", "model": "anthropic/claude-sonnet-4.6"},
                {"provider": "openai-codex", "model": "gpt-5.5"},
    ...
    def test_present_nous_token_allows_activation(self):
        """PoorMad is considered when token material exists."""
        agent = _make_agent(
            fallback_model=[
                {"provider": "nous", "model": "anthropic/claude-sonnet-4.6"},
                {"provider": "openai-codex", "model": "gpt-5.5"},
