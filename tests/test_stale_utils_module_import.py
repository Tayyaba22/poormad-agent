...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.poormad/poormad-agent/utils.py)
...
2. ``poormad update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``poormad_cli/main.py`` (the ``poormad update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``poormad_constants`` after the
