...
def _isolate(tmp_path, monkeypatch):
    """Isolate POORMAD_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal PoorMadCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = PoorMadCLI.__new__(PoorMadCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["poormad-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below PoorMad' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly PoorMad' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above PoorMad' minimum context length."""
