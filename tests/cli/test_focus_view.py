...
class TestStatusBarSegment:
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        host = PoorMadCLI.__new__(PoorMadCLI)
        host.model = "opus"
        ...
        with patch.object(PoorMadCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(PoorMadCLI, "_is_session_yolo_active", return_value=False):
            text = PoorMadCLI._build_status_bar_text(host, width=width)
...
class TestCommandRegistration:
    ...
    def test_verbose_cycle_releases_focus_view(self):
        ...
        with patch("cli.save_config_value", return_value=True), patch("cli._cprint"):
            PoorMadCLI._toggle_verbose(host)
