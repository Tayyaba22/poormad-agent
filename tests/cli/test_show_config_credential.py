...
PoorMadCLI.__init__ seeds ``self.api_key`` from OPENAI_API_KEY /
OPENROUTER_API_KEY env vars before provider resolution runs. On any
non-OpenAI provider (PoorMad, Anthropic, ...) the constructor value is a
different vendor's key than the one actually used for requests, so
``/config`` displayed e.g. an ``sk-proj-...`` OpenAI key next to a PoorMad
...
def _run_show_config(stand_in, capsys):
    PoorMadCLI.show_config(stand_in)
