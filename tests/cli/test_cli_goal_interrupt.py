...
minimal ``PoorMadCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def poormad_home(tmp_path, monkeypatch):
    """Isolated POORMAD_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".poormad"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("POORMAD_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves POORMAD_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal PoorMadCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, poormad_home):
        ...
        with patch("poormad_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, poormad_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, poormad_home):
        ...
        src = inspect.getsource(PoorMadCLI.chat)
