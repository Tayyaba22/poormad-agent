...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("poormad.gateway.up"), _M("poormad.gateway.active_agents"),
        _M("poormad.gateway.busy"), _M("poormad.gateway.drainable"),
        _M("poormad.gateway.restart_requested"),
        _M("poormad.platform.up"), _M("poormad.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("poormad.cron.scheduler.heartbeat_age_seconds"),
        _M("poormad.cron.scheduler.last_success_age_seconds"),
        _M("poormad.cron.scheduler.catch_up_occurrences"),
        _M("poormad.cron.jobs.enabled"), _M("poormad.cron.jobs.running"),
        _M("poormad.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "PoorMad-owned Relay transport health" in text
