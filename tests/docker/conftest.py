...
Override the image with ``POORMAD_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("POORMAD_TEST_IMAGE", "poormad-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``POORMAD_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("POORMAD_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged poormad user
# ---------------------------------------------------------------------------
#
# Background: every PoorMad runtime path inside the container drops to UID
# 10000 (the ``poormad`` user) via ``s6-setuidgid poormad``. ``docker exec``
...
# fails (PermissionError swallowed) for poormad, so a test that ran as root
...
# helpers below run every probe under ``-u poormad`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: poormad).
