"""Runtime smoke test for Docker $POORMAD_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the poormad user after container boot.
...
registered later runs its log service as the dropped poormad user and
...
def test_logs_gateways_seeded_and_poormad_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by poormad after boot."""
    ...
    assert "logs=poormad" in r.stdout, (
        f"logs/ not owned by poormad: {r.stdout}"
    )
    assert "gateways=poormad" in r.stdout, (
        f"logs/gateways/ not owned by poormad: {r.stdout}"
    )
