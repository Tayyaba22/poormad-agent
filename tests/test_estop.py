"""Global emergency stop (`poormad pause` / `poormad resume`) — agent/estop.py.
...
already in flight is never touched. Removing the sentinel (`poormad resume`)
...
def poormad_home(tmp_path, monkeypatch):
    """Point POORMAD_HOME at a temp dir and reset estop module log state."""
...
def test_engage_creates_sentinel_and_is_engaged(poormad_home):
    assert estop.is_engaged() is False
    estop.engage()
    assert (poormad_home / "ESTOP").exists()
    assert estop.is_engaged() is True


def test_disengage_removes_sentinel(poormad_home):
    estop.engage()
    assert estop.disengage() is True
    assert not (poormad_home / "ESTOP").exists()
    assert estop.is_engaged() is False
    # Disengaging when not engaged is a no-op that reports False.
    assert estop.disengage() is False


def test_reason_and_timestamp_stored(poormad_home):
    ...
    raw = json.loads((poormad_home / "ESTOP").read_text(encoding="utf-8"))
    assert raw["reason"] == "runaway cron fan-out"


def test_get_state_none_when_disengaged(poormad_home):
    assert estop.get_state() is None


def test_corrupt_sentinel_still_engages(poormad_home):
    """A hand-touched/corrupt ESTOP file must still pause (fail safe)."""
    (poormad_home / "ESTOP").write_text("not json", encoding="utf-8")
    assert estop.is_engaged() is True
    state = estop.get_state()
    assert state is not None
...
def test_paused_reply_none_when_disengaged(poormad_home):
    assert estop.paused_reply() is None


def test_paused_reply_surfaces_reason_and_resume_hint(poormad_home):
    ...
    assert "poormad resume" in notice


def test_paused_reply_without_reason(poormad_home):
    estop.engage()
    notice = estop.paused_reply()
    assert notice is not None
    assert "paused" in notice.lower()
...
def test_check_paused_logs_once_per_engagement(poormad_home, caplog):
...
def test_cron_tick_skips_dispatch_when_engaged(poormad_home, monkeypatch):
...
def test_cron_tick_resumes_after_disengage(poormad_home, monkeypatch):
    from cron import scheduler
...
def test_kanban_dispatch_blocked_when_engaged(poormad_home):
...
@pytest.mark.asyncio
async def test_gateway_new_turn_gets_paused_reply(poormad_home):
...
@pytest.mark.asyncio
async def test_gateway_internal_events_bypass_estop(poormad_home):
...
# ── CLI: poormad pause / poormad resume ───────────────────────────────────────


def test_cli_pause_engages_with_reason(poormad_home, capsys):
    from poormad_cli.subcommands.pause import cmd_pause
...
def test_cli_pause_idempotent(poormad_home, capsys):
    from poormad_cli.subcommands.pause import cmd_pause
...
def test_cli_resume_disengages(poormad_home, capsys):
    from poormad_cli.subcommands.pause import cmd_pause, cmd_resume
...
def test_cli_resume_when_not_paused(poormad_home, capsys):
    from poormad_cli.subcommands.pause import cmd_resume
...
def test_builtin_subcommands_include_pause_resume():
    from poormad_cli.main import _BUILTIN_SUBCOMMANDS

    assert "pause" in _BUILTIN_SUBCOMMANDS
    assert "resume" in _BUILTIN_SUBCOMMANDS


# ── poormad status surfacing ─────────────────────────────────────────────────


def test_status_line_when_paused(poormad_home):
    from poormad_cli.status import _estop_status_line
...
def test_is_engaged_fails_safe_on_stat_error(poormad_home, monkeypatch):
    """A stat failure must report ENGAGED (fail safe) — the pause has to
    hold even when POORMAD_HOME is misbehaving, matching the module's
...
@pytest.mark.asyncio
async def test_gateway_slash_commands_bypass_estop(poormad_home):
    ...
    try:
        ...
        return
    assert reply is None or "poormad is paused" not in (reply or "").lower()
...
@pytest.mark.asyncio
async def test_gateway_pause_command_engages_and_resumes(poormad_home):
    from gateway.run import GatewayRunner
...
def test_pause_command_registered_for_gateway():
    from poormad_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command
