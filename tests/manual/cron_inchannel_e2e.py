...
Uses a throwaway POORMAD_HOME so it never touches ~/.poormad. No real names.
"""
...
def _fresh_home():
    """Point POORMAD_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_poormad_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["POORMAD_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER POORMAD_HOME is set.
...
# a different get_poormad_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"POORMAD_HOME (throwaway): {HOME}")
