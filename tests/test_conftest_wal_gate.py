"""The conftest WAL gate must agree with poormad_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``poormad_state``. That is deliberate: importing
``poormad_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.poormad`` before the per-test ``POORMAD_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_poormad_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and poormad_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_poormad_state_at_collection():
    """The gate must stay import-free of poormad_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.poormad, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``poormad_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "poormad_state" or name.startswith("poormad_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported poormad_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.poormad during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("poormad_state", None)
    ...
    try:
        ...
        if saved is not None:
            sys.modules["poormad_state"] = saved
    assert not blocked
