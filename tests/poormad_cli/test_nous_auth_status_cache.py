...
The cache avoids re-validating PoorMad credentials on every menu paint —
`poormad tools` → "All Platforms" used to fire ~31 OAuth refresh POSTs
against portal.poormad.dev during one render. The cache is keyed
...
def _seed_auth_file(tmp_path):
    """Drop a placeholder auth.json into the test POORMAD_HOME.
...
def test_get_nous_auth_status_caches_failure_path(tmp_path, monkeypatch):
    ...
    Teknium's case: ~31 cache misses per `poormad tools` "All Platforms"
