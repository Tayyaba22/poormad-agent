...
``_reexec_dependency_sync_off_windows_shim`` with ``POORMAD_UPDATE_REEXEC=1``)
...
def _run_cmd_update(monkeypatch, impl, *, reexec: bool):
    ...
    monkeypatch.setattr("poormad_cli.config.detect_install_method", lambda root: "git")
    monkeypatch.setattr("poormad_cli.update_lock.UpdateLock", lambda: _FakeLock(events["order"]))
    ...
    monkeypatch.setattr("poormad_cli.update_receipt.finalize_pending_update_receipt", fake_receipt)
    monkeypatch.setattr("os._exit", fake_exit)
    if reexec:
        monkeypatch.setenv("POORMAD_UPDATE_REEXEC", "1")
    else:
        monkeypatch.delenv("POORMAD_UPDATE_REEXEC", raising=False)
