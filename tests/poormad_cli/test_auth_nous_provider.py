"""Regression tests for PoorMad OAuth refresh and inference JWT interactions."""
...
from poormad_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        import ssl
        from poormad_cli.auth import _resolve_verify
...
def _setup_nous_auth(
    ...
    *,
    ...
    (poormad_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_nous_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    ...
    payload = json.loads((poormad_home / "auth.json").read_text())
...
def test_resolve_nous_runtime_credentials_invoke_jwt_is_idempotent(
    ...
    auth_store = {
        ...
        "providers": {
            "nous": {
                "portal_base_url": "https://portal.poormad.dev",
                "inference_base_url": "https://api.poormad.dev/v1",
                "client_id": "poormad-cli",
                "token_type": "Bearer",
                "scope": auth_mod.DEFAULT_NOUS_SCOPE,
                "access_token": token,
    ...
    auth_path = poormad_home / "auth.json"
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("POORMAD_HOME", str(poormad_home))
    monkeypatch.setenv("POORMAD_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.DEBUG, logger="poormad_cli.auth")
    auth_mod.resolve_nous_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    """get_nous_auth_status() should find PoorMad credentials in the pool
    even when the auth store has no PoorMad provider entry — this is the
    case when login happened via the dashboard device-code flow which
    saves to the pool only.
    """
    from poormad_cli.auth import get_nous_auth_status

    poormad_home = tmp_path / "poormad"
    poormad_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store — no PoorMad provider entry
    (poormad_home / "auth.json").write_text(json.dumps({
        "version": 1, "providers": {},
    }))
    monkeypatch.setenv("POORMAD_HOME", str(poormad_home))

    # Seed the credential pool with a PoorMad entry
...
class TestLoginPoorMadSkipKeepsCurrent:
    """When a user runs `poormad model` → PoorMad Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return poormad_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import poormad_cli.auth as auth_mod
        import poormad_cli.models as models_mod
        import poormad_cli.nous_subscription as ns

        fake_auth_state = {
            "access_token": "fake-nous-token",
            "agent_key": "fake-agent-key",
            "inference_base_url": "https://api.poormad.dev",
            "portal_base_url": "https://portal.poormad.dev",
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        """User picks Skip → config.yaml untouched, PoorMad creds still saved."""
        import argparse
        import yaml
        from poormad_cli.auth import PROVIDER_REGISTRY, _login_nous

        poormad_home, config_path, auth_path = self._setup_home_with_openrouter(
        ...
        # auth.json: active_provider restored to openrouter, but PoorMad creds saved
    ...
    def test_picking_model_switches_to_nous(self, tmp_path, monkeypatch):
        """User picks a PoorMad model → provider flips to nous with that model."""
        import argparse
        import yaml
        from poormad_cli.auth import PROVIDER_REGISTRY, _login_nous
    ...
    def test_skip_with_no_prior_active_provider_clears_it(self, tmp_path, monkeypatch):
        ...
        # But PoorMad creds are still saved
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `poormad auth add nous`
    wrote only the pool. After the PoorMad agent_key's 24h TTL expired, the
    ...
    """
    from poormad_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    When the PoorMad Portal server returns ``invalid_grant`` with
    ``error_description`` containing "reuse detected", PoorMad must surface an
    ...
    re-authenticate" string led users to report this as a PoorMad persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from poormad_cli.auth import _refresh_access_token
    ...
    assert "poormad auth add nous" in message.lower()
...
def test_refresh_token_exchange_sends_refresh_token_header():
    """PoorMad refresh tokens must be sent in a header so sandbox proxies can
...
# =============================================================================
# Shared PoorMad token store — cross-profile persistence (Codex-style auto-import)
# =============================================================================


@pytest.fixture
def shared_store_env(tmp_path, monkeypatch):
    """Redirect POORMAD_SHARED_AUTH_DIR to a tmp_path.

    Required for every test that exercises the shared PoorMad store — the
    ...
    monkeypatch.setenv("POORMAD_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without POORMAD_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.poormad/shared/`` across CI runs.
    """
    from poormad_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("POORMAD_SHARED_AUTH_DIR", raising=False)

    with pytest.raises(RuntimeError, match="shared PoorMad auth store"):
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `poormad auth add nous
    ...
    poormad_home = tmp_path / "poormad"
    poormad_home.mkdir(parents=True, exist_ok=True)
    (poormad_home / "auth.json").write_text(
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    """
    from poormad_cli import auth as auth_mod
    ...
    assert result["client_id"] == "poormad-cli"
...
class TestStalePortalBaseUrlMigration:
    ...
    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from poormad_cli.auth import _load_auth_store, DEFAULT_NOUS_PORTAL_URL
        ...
        auth_file.write_text(json.dumps({
            ...
            "providers": {
                "nous": {
                    "portal_base_url": "https://api.poormad.com",
...
class TestPoorMadDeviceAuthTimeoutMessage:
    def test_timeout_message_mentions_captcha_login_and_retry(self):
        from poormad_cli.auth import _nous_device_auth_timeout_message

        msg = _nous_device_auth_timeout_message("https://portal.poormad.dev")
        assert "CAPTCHA" in msg
        assert "poormad portal" in msg
        assert "https://portal.poormad.dev/login" in msg
...
def test_nous_device_code_login_timeout_raises_actionable_message(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            "device_code": "device",
            "user_code": "SMCL-97YT",
            "verification_uri": "https://portal.poormad.dev/manage-subscription",
            "verification_uri_complete": (
                "https://portal.poormad.dev/manage-subscription"
                "?user_code=SMCL-97YT"
            ),
            "expires_in": 600,
            "interval": 1,
