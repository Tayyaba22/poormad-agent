"""Tests for the unified provider catalog (poormad_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``poormad model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from poormad_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
