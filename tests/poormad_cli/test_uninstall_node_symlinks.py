"""Tests for poormad_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $POORMAD_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the PoorMad-managed node dir.
"""
...
def _make_poormad_node(poormad_home: Path) -> Path:
    """Create a fake $POORMAD_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = poormad_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    poormad_home = fake_home / ".poormad"
    _make_poormad_node(poormad_home)
    ...
    removed = uninstall.remove_node_symlinks(poormad_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    poormad_home = fake_home / ".poormad"
    node_bin = _make_poormad_node(poormad_home)
