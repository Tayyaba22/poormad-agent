"""ensure_poormad_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".poormad"
    second = tmp_path / "b" / ".poormad"

    monkeypatch.setenv("POORMAD_HOME", str(first))
    cfg.ensure_poormad_home()

    # Profile switch: POORMAD_HOME moves → the new path is ensured too.
    monkeypatch.setenv("POORMAD_HOME", str(second))
    cfg.ensure_poormad_home()
