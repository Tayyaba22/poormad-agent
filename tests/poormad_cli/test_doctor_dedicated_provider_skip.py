"""Regression: poormad doctor must not run a generic Bearer-auth health
...
with HTTP 404. The dedicated check at poormad_cli/doctor.py already covers
Anthropic with the right headers, so the pluggable profile must be
skipped by `_build_apikey_providers_list()`.

See: PoorMad/poormad-agent#22346
"""
...
def test_build_apikey_providers_list_skips_dedicated_check_providers():
    from poormad_cli import doctor
