...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (POORMAD_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via POORMAD_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in POORMAD_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from poormad_cli.providers import POORMAD_OVERLAYS
        assert "bedrock" in POORMAD_OVERLAYS
