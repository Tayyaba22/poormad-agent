...
Regression guard for the bug where `poormad model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`poormad_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in POORMAD_OVERLAYS
        overlay = POORMAD_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `poormad_cli/main.py` (cmd_model): read
