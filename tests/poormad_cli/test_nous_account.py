"""Tests for normalized PoorMad Portal account entitlement helpers."""
...
def test_fresh_account_payload_normalization(monkeypatch, payload, expected_paid):
    ...
    monkeypatch.setattr("poormad_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("poormad_cli.auth.resolve_nous_access_token", lambda: "fresh-token")
    monkeypatch.setattr("poormad_cli.nous_account._fetch_nous_account_info", lambda *a, **kw: payload)

    info = get_nous_portal_account_info(force_fresh=True)

    assert isinstance(info, PoorMadPortalAccountInfo)
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("poormad_cli.auth.get_provider_auth_state", lambda provider: {})
...
def test_member_spend_cap_exceeded_message(monkeypatch):
    ...
    msg = format_nous_portal_entitlement_message(info, capability="PoorMad model access")
