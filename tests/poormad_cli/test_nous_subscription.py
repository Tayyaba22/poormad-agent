"""Tests for PoorMad subscription feature detection."""

import shutil
import sys

from poormad_cli.nous_account import PoorMadPortalAccountInfo, PoorMadToolAccessInfo
from poormad_cli import nous_subscription as ns
...
def _account(*, logged_in: bool, paid: bool | None = None) -> PoorMadPortalAccountInfo:
...
def _pool_account() -> PoorMadPortalAccountInfo:
    ...
    return PoorMadPortalAccountInfo(
        ...
        tool_access=PoorMadToolAccessInfo(enabled=True, coverage=_POOL_COVERAGE),
    )
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import poormad_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "poormad_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
def test_get_gateway_eligible_tools_treats_explicit_backend_as_configured(monkeypatch):
    ...
    single Enter during `poormad model` overwrote it to `web.backend: nous`.
    """
...
def test_get_gateway_eligible_tools_not_entitled_returns_four_empty_lists(monkeypatch):
    """A logged-in PoorMad account with no paid access and no free tool pool
...
def test_prompt_enable_tool_gateway_not_entitled_does_not_crash(monkeypatch):
    ...
    not raise when a PoorMad account is logged in but not entitled to the Tool
...
def test_prompt_enable_tool_gateway_persists_decline(monkeypatch):
    ...
    captured = _capture_checklist(monkeypatch, selected_idx=[])
    monkeypatch.setattr(
        "poormad_cli.config.save_config", lambda cfg: saved.append(dict(cfg)), raising=False
    )
...
def test_apply_nous_managed_defaults_writes_video_gen_config(monkeypatch):
    ...
    when a PoorMad subscriber selects video_gen without a direct FAL_KEY."""
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `poormad tools`
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# STT — managed-by-PoorMad detection (Phase 4 follow-up)
# ---------------------------------------------------------------------------
...
def _stt_features_stub(*, account_info):
    return ns.PoorMadSubscriptionFeatures(
        subscribed=True,
        nous_auth_present=True,
        provider_is_nous=True,
        account_info=account_info,
        features={
            key: ns.PoorMadFeatureState(
...
def _block_legacy_agent_browser_checks(monkeypatch):
    ...
    monkeypatch.setattr("poormad_constants.agent_browser_runnable", lambda path: False)
...
def test_has_agent_browser_import_failure_falls_back_to_path_check(monkeypatch):
    ...
    monkeypatch.setattr(
        "poormad_constants.agent_browser_runnable",
...
def test_has_agent_browser_import_failure_falls_back_to_poormad_managed_node_path(
    monkeypatch, tmp_path
):
    """If tools.browser_tool cannot be imported, the managed-Node rung must
    still find a runnable agent-browser under the PoorMad Node dir even when
    ...
    monkeypatch.setattr(
        "poormad_constants.with_poormad_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "poormad_constants.agent_browser_runnable",
