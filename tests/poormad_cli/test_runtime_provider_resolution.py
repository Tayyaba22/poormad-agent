...
def test_auto_detected_nous_auth_failure_falls_through_to_openrouter(monkeypatch):
    """When auto-detect picks PoorMad but credentials are revoked, fall through to OpenRouter."""
    from poormad_cli.auth import AuthError
    ...
    # PoorMad credential resolution fails with revoked token
    monkeypatch.setattr(
...
class TestProviderEntryApiKeyEnvAlias:
    ...
    def test_snake_case_api_key_env_normalizes_to_key_env(self):
        from poormad_cli.config import _normalize_custom_provider_entry
        entry = {
            "name": "vendor",
    ...
    def test_valid_fields_set_lists_key_env(self):
        ...
        from poormad_cli.config import _VALID_CUSTOM_PROVIDER_FIELDS
...
def test_minimax_oauth_runtime_returns_anthropic_messages_mode(monkeypatch):
    ...
    from poormad_cli.auth import MINIMAX_OAUTH_GLOBAL_INFERENCE
