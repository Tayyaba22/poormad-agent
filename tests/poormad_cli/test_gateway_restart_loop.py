...
Covers:
- Defense 1: gateway stop/restart refuse when _POORMAD_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "poormad gateway uninstall",
        "poormad  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "POORMAD GATEWAY RESTART",           # uppercase
    ])
    def test_poormad_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.poormad.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.poormad/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l poormad-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.poormad.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.poormad.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.poormad.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.poormad/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.poormad.gateway-apollo:/a.plist' "
            "'ai.poormad.gateway-cronus:/c.plist' 'ai.poormad.gateway-plutus:/p.plist' "
            "'ai.poormad.gateway:/Users/botuser/Library/LaunchAgents/ai.poormad.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated poormad note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.poormad.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.poormad.gateway',
        "launchctl kick'start' -k gui/501/ai.poormad.gateway",
        "launchctl kick\\start -k gui/501/ai.poormad.gateway",
        'launchctl "kickstart" -k gui/501/ai.poormad.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.poormad.gateway',
        "launchctl dis\\able gui/501/ai.poormad.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.poormad."gateway"',
        # Same class on the systemctl and poormad-CLI branches.
        'systemctl re"start" poormad-gateway',
        'poormad gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.poormad.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.poormad.update-checker',
        'systemctl re"start" poormad-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "poormad cron list",
        "poormad update",
        "poormad config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `poormad gateway start` is benign — starting a gateway from inside a
        ...
        "poormad gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway poormad
        # services must not be falsely blocked (the old `.*poormad` matched any
        # poormad token).
        "launchctl unload ai.poormad.update-checker.plist",
        "launchctl restart ai.poormad.daemon",
        ...
        "systemctl restart poormad-meta.service",
        "systemctl restart poormad-cron-helper",
        ...
        "echo after the poormad gateway restarted cleanly",
        "the poormad gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "poormad skill view gateway-notes && echo poormad gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `poormad` is a path component
        # there, not a command.
        "cat '/docs/poormad gateway restart-notes.md'",
        "less /home/user/notes/poormad gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "poormad gateway restart",
        "poormad gateway restart; echo done",
        "poormad gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;poormad gateway restart",
        "true && poormad gateway stop",
        "echo $(poormad gateway restart)",
        "echo `poormad gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.poormad.gateway"])',
        'subprocess.run(["poormad", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.poormad.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["poormad", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: poormad gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `poormad -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry POORMAD_HOME/POORMAD_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("POORMAD_PROFILE", "zeus")
        monkeypatch.delenv("POORMAD_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "poormad -p zeus gateway stop",
        "poormad -p zeus gateway restart",
        "poormad --profile zeus gateway restart",
        "poormad --profile zeus gateway stop",
        "poormad --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "poormad -v -p zeus gateway restart",
        "poormad -p zeus -v gateway restart",
        "poormad --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "poormad -p venus gateway stop",
        "poormad -p venus gateway restart",
        "poormad --profile venus gateway restart",
        "poormad --profile=venus gateway stop",
        "poormad -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "poormad -p zeus gateway start",
        "poormad -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("poormad gateway restart")
        assert _contains_gateway_lifecycle_command("poormad gateway stop")

    def test_poormad_home_derived_profile(self, monkeypatch):
        # Without POORMAD_PROFILE the guard falls back to the POORMAD_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("POORMAD_PROFILE", raising=False)
        monkeypatch.delenv("POORMAD_PROFILE_NAME", raising=False)
        import poormad_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("poormad -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("poormad -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("poormad -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("poormad gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_poormad_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade poormad then run poormad gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_poormad_gateway_once.sh). The script must live under
        # POORMAD_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("POORMAD_HOME", str(tmp_path / ".poormad"))
        scripts_dir = tmp_path / ".poormad" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_POORMAD_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _POORMAD_GATEWAY=1.

    Issue #37453: systemctl --user restart poormad-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart poormad-gateway",
        "systemctl --user restart poormad-gateway",
        "systemctl stop poormad-gateway.service",
        "poormad gateway restart",
        "poormad gateway uninstall",
        "launchctl kickstart gui/501/ai.poormad.gateway",
        "launchctl bootout gui/501/ai.poormad.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.poormad.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.poormad/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.poormad.gateway.restart-once.plist",
        "pkill -f poormad.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart poormad-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.poormad.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-poormad label: label-independent detection is the point
        # (#62891 second reproduction used `ai.poormad.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.poormad.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _POORMAD_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _POORMAD_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_POORMAD_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="poormad gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["poormad gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.poormad.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.poormad.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-poormad systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run poormad gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("poormad gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfepoormad gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under POORMAD_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("poormad gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/poormad\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 poormad gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_poormad_home()` falls back to Path.home(), which raises
        RuntimeError when neither POORMAD_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart poormad-gateway' .",
        "grep -rn 'poormad gateway restart' ./logs",
        "rg 'poormad gateway restart' ../archive",
        "grep -c 'systemctl stop poormad-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM poormad_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell poormad gateway restart"',
        'sqlite3 ./db ".system systemctl restart poormad-gateway"',
        'psql ./x -c "\\! systemctl restart poormad-gateway"',
        "grep -r 'poormad gateway restart' . | sh",
        "grep -r 'poormad gateway restart' ./logs | bash",
        "grep -r 'poormad gateway restart' . | sudo sh",
        "grep -r 'x' . ; poormad gateway restart",
        "grep -r 'x' . && systemctl restart poormad-gateway",
        'grep -r "$(poormad gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart poormad-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell poormad gateway restart"',
        'sqlite3 db ".system systemctl restart poormad-gateway"',
        'psql -c "\\! systemctl restart poormad-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    poormad_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run poormad gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("POORMAD_HOME", str(tmp_path / ".poormad"))
        (tmp_path / ".poormad").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run poormad gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop poormad-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart poormad-gateway' /var/log/syslog",
        "rg 'poormad gateway restart' /home/user/.poormad/logs/",
        "journalctl -u poormad-gateway --grep 'systemctl restart poormad-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'poormad_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell poormad gateway restart"',
        'psql -c "\\! systemctl restart poormad-gateway"',
        "grep 'systemctl restart poormad-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart poormad-gateway",
        'grep "$(systemctl restart poormad-gateway)" f',
        "grep 'restart' log; systemctl restart poormad-gateway",
        'sqlite3 db "SELECT 1"; poormad gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "poormad gateway restart",
        "sudo systemctl stop poormad-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart poormad-gateway%'\""
        )
