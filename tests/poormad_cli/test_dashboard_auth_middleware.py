...
from poormad_cli.dashboard_auth.cookies import SESSION_AT_COOKIE
...
def _complete_stub_login(client) -> None:
    ...
    client's cookie jar holds ``poormad_session_at`` / ``poormad_session_rt``
...
class _UnreachableProvider(StubAuthProvider):
    ...
    provider first, which tries to reach PoorMad Portal's JWKS; if that's
    ...
    def verify_session(self, *, access_token: str):
        from poormad_cli.dashboard_auth.base import ProviderError
...
def _mint_stub_at(stub: StubAuthProvider) -> str:
    ...
    state = dict(
        seg.split("=", 1)
        for seg in ls.cookie_payload["poormad_session_pkce"].split(";")
        if "=" in seg
    )["state"]
    verifier = dict(
        seg.split("=", 1)
