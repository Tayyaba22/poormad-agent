"""Tests for the Kanban DB layer (poormad_cli.kanban_db)."""
...
def kanban_home(tmp_path, monkeypatch):
    """Isolated POORMAD_HOME with an empty kanban DB."""
...
def test_stale_claim_reclaim_event_records_diagnostic_payload(
    ...
    timing context)."""
    import json
    import poormad_cli.kanban_db as _kb
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("POORMAD_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("POORMAD_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """PoorMad' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` PoorMad' DB/metadata/logs on task completion.
    """
...
# spawned with `poormad -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's POORMAD_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's POORMAD_HOME."""

    def _set_home(self, monkeypatch, tmp_path, poormad_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("POORMAD_HOME", str(poormad_home))
        monkeypatch.delenv("POORMAD_KANBAN_HOME", raising=False)


    def test_profile_worker_resolves_to_shared_root(
        self, tmp_path, monkeypatch
    ):
        # Reproduces the bug: dispatcher uses ~/.poormad/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.poormad/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.poormad/kanban.db.
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # POORMAD_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile POORMAD_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # POORMAD_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is POORMAD_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["POORMAD_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["POORMAD_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["POORMAD_KANBAN_TASK"] == "t_dispatch_env"
        assert env["POORMAD_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "POORMAD_SESSION_SOURCE":
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see poormad_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import poormad_state as _poormad_state
    monkeypatch.setattr(
        _poormad_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _poormad_state._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    poormad_state._wal_fallback_warned_paths.clear()
    ...
    with _patch("poormad_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="poormad_state"):
            conn = kb.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        poormad_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    ...
    with _patch(
        "poormad_cli.kanban_db.sqlite3.connect",
...
def test_sqlite_connect_closes_tracked_conn_on_setup_failure(tmp_path, monkeypatch):
    ...
    from poormad_cli import sqlite_safe_read
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'poormad kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_poormad_argv()
#
# Workers spawned by the dispatcher must use a `poormad` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["poormad", ...]`
...
def test_resolve_poormad_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m poormad_cli.main`.

    Pins the correct module name (NOT `poormad` — there is no top-level
    `poormad` package). Regression for #23198: the original PR shipped
    `python -m poormad` which fails with `No module named poormad` on every
    invocation.
    """
    import shutil
    import sys
    import poormad_cli.kanban_db as kb

    monkeypatch.delenv("POORMAD_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_poormad_argv()
    assert argv == [sys.executable, "-m", "poormad_cli.main"]


def test_resolve_poormad_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `poormad_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m poormad_cli.main --version`
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("POORMAD_BIN", None)
    ...
    assert "PoorMad" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="poormad_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from poormad_cli.kanban_db import connect
