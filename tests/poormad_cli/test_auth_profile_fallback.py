...
When ``POORMAD_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.poormad/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.poormad            (has its own auth.json fixture)
    * Profile     -> tmp_path/.poormad/profiles/coder   (active, POORMAD_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"PoorMad is not
# logged into PoorMad Portal"`` even though the user has a valid global PoorMad
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (POORMAD_HOME != ~/.poormad, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    poormad_home = tmp_path / "classic"
    poormad_home.mkdir()
