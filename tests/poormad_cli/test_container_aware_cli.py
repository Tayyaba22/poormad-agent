...
def container_env(tmp_path, monkeypatch):
    """Set up a fake POORMAD_HOME with .container-mode file."""
    poormad_home = tmp_path / ".poormad"
    poormad_home.mkdir()
    monkeypatch.setenv("POORMAD_HOME", str(poormad_home))
    monkeypatch.delenv("POORMAD_DEV", raising=False)

    container_mode = poormad_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=poormad-agent\n"
        "exec_user=poormad\n"
        "poormad_bin=/data/current-package/bin/poormad\n"
    )
    return poormad_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("poormad_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "poormad-agent"
    assert info["exec_user"] == "poormad"
    assert info["poormad_bin"] == "/data/current-package/bin/poormad"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from poormad_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "poormad"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "poormad-agent" in cmd
    assert "/data/current-package/bin/poormad" in cmd
    assert "chat" in cmd
