...
#88251: ``poormad update`` treated a failed desktop pack as non-fatal, printed
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "poormad_constants.with_poormad_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "poormad_constants.display_poormad_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_poormad_home", lambda: tmp_path)
