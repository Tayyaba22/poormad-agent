...
def poormad_home(tmp_path, monkeypatch):
    ...
    # /etc/poormad on the dev/CI box can't influence the test.
    monkeypatch.setenv("POORMAD_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(poormad_home, monkeypatch):
...
def test_env_expansion_in_user_config(poormad_home, monkeypatch):
    from poormad_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_poormad_dotenv(poormad_home=str(home))
