...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_poormad_home", lambda: poormad_home)
    monkeypatch.setattr(uninstall, "_is_default_poormad_home", lambda home: False)
    ...
    assert str(poormad_home) in output
    assert project_root.exists()
    assert poormad_home.exists()
