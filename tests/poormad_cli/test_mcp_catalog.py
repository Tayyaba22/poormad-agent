"""Tests for poormad_cli.mcp_catalog and poormad_cli.mcp_picker.
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("POORMAD_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_poormad_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp POORMAD_HOME."""
    hh = tmp_path / "poormad-home"
    hh.mkdir()
    monkeypatch.setenv("POORMAD_HOME", str(hh))
    monkeypatch.setattr(
        "poormad_cli.config.get_poormad_home", lambda: hh
    )
    monkeypatch.setattr(
        "poormad_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "poormad_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_poormad_home() lazily through poormad_constants
    monkeypatch.setattr(
        "poormad_constants.get_poormad_home", lambda: hh
    )
    return hh
...
def _entry(name: str):
    ...
    from poormad_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from poormad_cli.mcp_catalog import list_catalog
    ...
    def test_suggest_empty_block_rejected(self, catalog_dir):
        ...
        from poormad_cli.mcp_catalog import list_catalog, catalog_diagnostics
    ...
    def test_http_api_key_builds_bearer_headers_template(self, catalog_dir):
        ...
        _write_manifest(catalog_dir, "demo", body)
        from poormad_cli.mcp_catalog import _build_server_config
    ...
    def test_http_api_key_requires_matching_env_declaration(self, catalog_dir):
        ...
        from poormad_cli.mcp_catalog import CatalogError, _parse_manifest
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from poormad_cli.mcp_catalog import install_entry
    ...
    def test_install_rejects_exfil_shaped_stdio_manifest(self, catalog_dir):
        ...
        from poormad_cli.config import load_config
        from poormad_cli.mcp_catalog import CatalogError, install_entry
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from poormad_cli.mcp_catalog import install_entry, uninstall_entry
        from poormad_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from poormad_cli.mcp_catalog import uninstall_entry
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from poormad_cli import mcp_catalog
        from poormad_cli.mcp_catalog import _do_git_install
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "poormad_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no POORMAD_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("POORMAD_OPTIONAL_MCPS", raising=False)
        from poormad_cli.mcp_catalog import _catalog_root, _parse_manifest
    ...
    def test_all_shipped_manifests_are_version_locked(self, monkeypatch):
        ...
        pyproject dependencies — everything PoorMad fetches/launches is pinned
        to an exact version.
