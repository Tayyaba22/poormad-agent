...
from poormad_cli.console_engine import PoorMadConsoleEngine, run_console_repl
...
MUTATING_CONFIRMATION_SMOKE_COMMANDS = [
    ...
    "mcp picker",
    "backup --quick -o /tmp/poormad-console-test.zip",
    "import /tmp/poormad-console-test.zip",
...
def test_sessions_list_and_stats_use_isolated_session_store(_isolate_poormad_home):
...
def test_sessions_export_rejects_oversized_single_before_touching_output(
    ...
    monkeypatch.setattr(poormad_state, "resolved_max_export_messages", lambda: 2)
    materialized = []
    ...
    result = PoorMadConsoleEngine().execute(
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_poormad_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_poormad_home):
    ...
    assert code == 0
    assert "PoorMad Console" in stdout.getvalue()
    assert "poormad>" not in stdout.getvalue()
    assert stderr.getvalue() == ""


def test_capture_output_surfaces_string_exit_code_as_command_error():
    from poormad_cli.console_engine import ConsoleCommandError, _capture_output
...
def test_execute_handler_string_exit_returns_error_not_crash(_isolate_poormad_home):
    result = PoorMadConsoleEngine().execute(
...
def test_console_checkpoints_prune_does_not_reprompt_for_orphans(
    _isolate_poormad_home, monkeypatch
):
    ...
    result = PoorMadConsoleEngine().execute("checkpoints prune", confirmed=True)
