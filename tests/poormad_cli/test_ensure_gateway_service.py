"""Tests for poormad_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `poormad setup` and `poormad import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("poormad_constants.is_container", lambda: container)
