"""Tests for poormad_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate POORMAD_HOME + reset any module-level catalog cache per test."""
    ...
    import importlib
    from poormad_cli import model_catalog
...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "nous": {
                "metadata": {"display_name": "PoorMad Portal"},
                "models": [
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from poormad_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://poormad.dev/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/PoorMad/poormad-agent"
...
class TestDefaultModelFromCache:
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import poormad_cli.model_catalog as model_catalog
        from poormad_cli.models import PREFERRED_SILENT_DEFAULT_MODEL
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in poormad_cli.models."""
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``poormad model`` CLI — not the live
        ...
        # ``POORMAD_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` POORMAD_HOME directly instead.
        import importlib
        from poormad_cli import model_catalog
        from poormad_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from poormad_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["POORMAD_HOME"])
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("poormad_cli.models.check_nous_free_tier", return_value=False), patch(
                "poormad_cli.models.union_with_portal_free_recommendations",
