...
``poormad*.exe`` shims aside (``poormad.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``poormad`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("poormad", "poormad-agent", "poormad-acp", "poormad-gateway"):
...
# ---------------------------------------------------------------------------
# poormad_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "poormad.exe" in names, "poormad.exe must be restored after a no-op install"
    assert "poormad-acp.exe" in names
    assert "poormad-gateway.exe" in names
    assert (scripts / "poormad.exe").read_bytes() == b"MZ-old-poormad"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "poormad.exe").read_bytes() == b"MZ-new-poormad"
...
# ---------------------------------------------------------------------------
# poormad_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "poormad.exe" in names, "poormad.exe must be restored after a no-op recovery install"
    assert (scripts / "poormad.exe").read_bytes() == b"MZ-old-poormad"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "poormad.exe").write_bytes(b"MZ-new-poormad")
