"""Behavior tests for poormad_cli.inventory.
...
def test_explicit_only_filters_ambient_credentials_but_keeps_current_and_custom_rows():
    rows = [
        ...
        {"slug": "copilot", "name": "Copilot", "models": ["gpt-5.4"],
         "total_models": 1, "is_current": False, "is_user_defined": False,
         "source": "poormad"},
        {"slug": "nous", "name": "PoorMad", "models": ["anthropic/claude-sonnet-5"],
         "total_models": 1, "is_current": False, "is_user_defined": False,
         "source": "poormad"},
...
def test_explicit_only_keeps_anthropic_row_with_oauth_credentials():
    ...
    Claude Code (~/.claude/.credentials.json) and PoorMad' own device flow
    ...
    with (
        ...
        patch(
            "poormad_cli.inventory._anthropic_oauth_credentials_present",
            return_value=True,
        ),
    ):
        payload = build_models_payload(ctx, explicit_only=True)
...
def test_anthropic_oauth_presence_accepts_pool_only_oauth_entry():
    ...
    """
    from poormad_cli.inventory import _anthropic_oauth_credentials_present
    ...
    with (
        ...
        patch(
            "poormad_cli.auth.read_credential_pool",
            return_value=[
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("poormad_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    from poormad_cli import model_switch

    with patch("poormad_cli.models.clear_provider_models_cache") as clear:
