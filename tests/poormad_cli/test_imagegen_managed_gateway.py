...
(non-managed) routing. When a user picked FAL through the PoorMad-subscription
...
the user's personal FAL_KEY instead of the PoorMad Tool Gateway (real incident:
...
the managed "PoorMad Subscription" row, ``image_gen.provider: fal`` for the
...
def test_reconfigure_managed_fal_row_keeps_managed_selection(monkeypatch):
    ...
    branch wrote the managed one — a PoorMad Subscription user re-entering the
