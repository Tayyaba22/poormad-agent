...
2. ``poormad_cli.main._refresh_windows_gateway_launchers`` — ``poormad
...
def test_restart_spec_normalizes_legacy_pythonw_argv(tmp_path):
    ...
    argv = [str(pythonw), "-m", "poormad_cli.main", "gateway", "run"]
    with mock.patch.object(
        gateway_windows, "_stable_gateway_working_dir", return_value=str(tmp_path)
    ), mock.patch("poormad_cli.config.get_poormad_home", return_value=str(tmp_path)):
...
# ---------------------------------------------------------------------------
# _refresh_windows_gateway_launchers: poormad update regenerates launchers
# ---------------------------------------------------------------------------
