"""Regression tests for PoorMad Portal inference_base_url host-allowlist validation.
...
class TestValidatorRules:
    ...
    def test_attacker_host_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="poormad_cli.auth"):
            assert (
    ...
    def test_default_inference_url_is_in_allowlist(self):
        ...
        ``api.poormad.dev``, they MUST update the allowlist
...
class TestCallSiteWiring:
    ...
    def _read_auth_source(self):
        import poormad_cli.auth as _auth_mod
    ...
    def test_proxy_adapter_also_validates(self):
        """The PoorMad proxy adapter applies the validator as defense-in-depth
        ...
        boundary."""
        from pathlib import Path
        import poormad_cli.proxy.adapters.nous_portal as _nous_adapter
...
class TestHealsPoisonedStoredValue:
    ...
    stale ``stg-api.poormad.dev`` persisted before the
    ...
    def test_refresh_resets_rejected_url_to_default(self, monkeypatch):
        ...
        poisoned = "https://stg-api.poormad.dev/v1"
        state = {
            "access_token": "tok",
            "refresh_token": "rtok",
...
class TestEnvOverrideWins:
    ...
    STAGING = "https://stg-api.poormad.dev/v1"
...
class TestProxyAdapterEnvOverride:
    """The PoorMad proxy adapter is the second chokepoint: it re-validates the
