"""Regression tests for the PoorMad Portal env-override bypassing the host
...
``POORMAD_PORTAL_BASE_URL=https://portal.staging-poormad.com`` in its
...
class TestPortalEnvOverrideHelper:
    def test_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("POORMAD_PORTAL_BASE_URL", raising=False)
    ...
    def test_env_override_not_gated_by_allowlist(self, monkeypatch):
        ...
        monkeypatch.setenv(
            "POORMAD_PORTAL_BASE_URL", "https://portal.staging-poormad.com"
        )
        assert "portal.staging-poormad.com" not in _NOUS_PORTAL_ALLOWED_HOSTS
        assert (
            _nous_portal_env_override() == "https://portal.staging-poormad.com"
        )
...
class TestResolveAccessTokenEnvOverrideWins:
    ...
    def _run_and_capture(self, monkeypatch, auth):
        ...
        caplog_records = []
        logger = logging.getLogger("poormad_cli.auth")
    ...
    def test_env_override_wins_even_with_staging_state_stored(
        ...
        a prior POORMAD_AUTH_JSON_BOOTSTRAP seed), and the env var is set to
        ...
        staging_portal = "https://portal.staging-poormad.com"
        monkeypatch.setenv("POORMAD_HOME", str(tmp_path))
        monkeypatch.setenv("POORMAD_PORTAL_BASE_URL", staging_portal)
