...
profile's POORMAD_HOME, and the dashboard's own profile stays untouched.
"""
import pytest
import yaml
...
class TestProfileScopedHubActions:
    def test_hub_install_spawns_with_profile_flag(
        ...
        """Hub installs must go through a fresh ``poormad -p <profile>``
        ...
        monkeypatch.setattr(web_server, "_spawn_poormad_action", _fake_spawn)
        resp = client.post(
