...
"unknown" — and on that answer the PoorMad profile drops a "thinking off" disable
rather than risk a 400. A short-lived process (``poormad -p``, a cron job, a
...
def test_staging_portal_does_not_read_productions_mirror(
    ...
    cold_process()
    monkeypatch.setenv("NOUS_INFERENCE_BASE_URL", "https://staging.poormad.com")
    monkeypatch.setattr(models_mod, "_urlopen_model_catalog_request", offline)
...
def test_pricing_fetch_seeds_the_mirror(cold_process, offline, monkeypatch):
    ...
    models_mod.fetch_models_with_pricing(
        base_url="https://api.poormad.dev"
    )
