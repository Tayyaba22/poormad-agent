...
from poormad_cli.backup import copy_db_and_verify, verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    POORMAD_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def poormad_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("poormad_cli.config") or mod == "poormad_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, poormad_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, poormad_home, capsys, monkeypatch):
