...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "poormad_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # POORMAD_HOME.  For a standard install (POORMAD_HOME unset) that root is
        # the platform-native default (~/.poormad), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from poormad_constants import get_default_poormad_root
        assert env.get("POORMAD_HOME") == str(get_default_poormad_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (POORMAD_DESKTOP=1) must NOT
