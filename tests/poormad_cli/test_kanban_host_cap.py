...
1. The standalone daemon path (``poormad kanban daemon --force`` /
   :func:`poormad_cli.kanban_db.run_daemon`) never resolved
...
def test_run_daemon_resolves_and_passes_max_in_progress(
    ...
    ``poormad kanban dispatch`` paths both resolved the memory-derived
    default.
    """
    captured: dict = {}
...
def test_configured_max_in_progress_parsing(monkeypatch):
    import poormad_cli.config as cfgmod
...
def test_nonspawnable_review_does_not_tax_ready_budget(
    ...
    import poormad_cli.config as cfgmod
    import poormad_cli.profiles as profmod
