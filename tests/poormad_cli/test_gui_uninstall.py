"""Tests for poormad_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import poormad_cli.gui_uninstall as gu


def _make_agent(poormad_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = poormad_home / "poormad-agent"
    (agent_root / "poormad_cli").mkdir(parents=True)
    (agent_root / "poormad_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(poormad_home: Path) -> None:
    """Create the source-built GUI artifacts a `poormad desktop` run produces."""
    desktop = poormad_home / "poormad-agent" / "apps" / "desktop"
    ...
    (poormad_home / "poormad-agent" / "node_modules").mkdir(parents=True)
    (poormad_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(poormad_home: Path) -> None:
    (poormad_home / "config.yaml").write_text("x: 1\n")
    (poormad_home / ".env").write_text("KEY=secret\n")
    (poormad_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(poormad_home)
    _make_gui_build(poormad_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(poormad_home)
    ...
    assert summary["poormad_home"] == str(poormad_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `poormad desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    poormad_home = tmp_path / ".poormad"
    _make_agent(poormad_home)
    icon = lde.icon_path(poormad_home / "poormad-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(poormad_home)
    ...
    assert lde.icon_path(poormad_home / "poormad-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (poormad_home / "poormad-agent" / "poormad_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".poormad")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import poormad_cli.uninstall as uninstall
