"""Tests for poormad_cli/goals.py — persistent cross-turn goals."""
...
def poormad_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves POORMAD_HOME.
...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        from poormad_cli.goals import _parse_judge_response
...
class TestGoalManager:
    ...
    def test_set_then_status(self, poormad_home):
    ...
    def test_continuation_prompt_shape(self, poormad_home):
        ...
        mgr.set("port goal command to poormad")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to poormad" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from poormad_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, poormad_home):
        """N=3 consecutive parse failures → auto-pause with config pointer."""
        from poormad_cli import goals
        from poormad_cli.goals import GoalManager, DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES
...
class TestGoalStateSubgoalsBackcompat:
    def test_old_state_meta_row_loads_without_subgoals(self):
        ...
        from poormad_cli.goals import GoalState
...
class TestMigrateGoalToSession:
    ...
    def test_migrates_active_goal_to_child(self, poormad_home):
        from poormad_cli.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
    ...
    def test_does_not_clobber_existing_child_goal(self, poormad_home):
        from poormad_cli.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
...
class TestGoalManagerSubgoals:
    def test_add_subgoal(self, poormad_home):
        from poormad_cli.goals import GoalManager
    ...
    def test_remove_subgoal_out_of_range(self, poormad_home):
        import pytest
        from poormad_cli.goals import GoalManager
...
class TestJudgeGoalWithSubgoals:
    def test_judge_uses_subgoals_template_when_provided(self, poormad_home):
    ...
    def test_judge_uses_original_template_when_no_subgoals(self, poormad_home):
        from unittest.mock import patch
        from poormad_cli import goals
...
class TestStatusLineSubgoalCount:
    ...
    def test_status_line_with_subgoals(self, poormad_home):
        from poormad_cli.goals import GoalManager
...
class TestWaitBarrier:
    ...
    def test_parked_on_live_pid_does_not_continue_or_judge(self, poormad_home):
        from poormad_cli import goals
        from poormad_cli.goals import GoalManager
    ...
    def test_stop_waiting_clears_barrier(self, poormad_home):
        from poormad_cli.goals import GoalManager
...
class TestJudgeDrivenWait:
    ...
    def test_judge_wait_pid_parks_loop(self, poormad_home):
        from poormad_cli import goals
        from poormad_cli.goals import GoalManager
    ...
    def test_time_barrier_clears_after_deadline(self, poormad_home):
        from poormad_cli.goals import GoalManager
    ...
    def test_continue_verdict_still_continues_with_background(self, poormad_home):
...
class TestSessionTriggerBarrier:
    ...
    def test_registry_releases_on_watch_match_while_alive(self, poormad_home):
    ...
    def test_wait_on_session_validation(self, poormad_home):
        from poormad_cli.goals import GoalManager
...
class TestGoalContractSerialization:
    def test_roundtrip_with_contract(self):
        from poormad_cli.goals import GoalState, GoalContract
    ...
    def test_render_block_omits_empty_fields(self):
        from poormad_cli.goals import GoalContract
...
class TestGoalManagerContract:
    ...
    def test_set_contract_after_the_fact(self, poormad_home):
        from poormad_cli.goals import GoalManager, GoalContract
        ...
        # Survives reload.
        from poormad_cli.goals import GoalManager as GM2
        assert GM2(session_id="c-after").has_contract()

    def test_persistence_roundtrip(self, poormad_home):
        from poormad_cli.goals import GoalManager, GoalContract
...
class TestJudgeWithContract:
    ...
    def test_judge_uses_contract_template(self, poormad_home):
        from unittest.mock import patch
        from poormad_cli import goals
        from poormad_cli.goals import GoalContract
...
class TestDraftContract:
    def test_draft_parses_json(self, poormad_home):
        from unittest.mock import patch
        from poormad_cli import goals
    ...
    def test_draft_returns_none_when_no_client(self, poormad_home):
        from unittest.mock import patch
        from poormad_cli import goals
...
class TestContractAndBackgroundCompose:
    ...
    def test_judge_prompt_carries_contract_and_background(self, poormad_home):
        from unittest.mock import patch
        from poormad_cli import goals
        from poormad_cli.goals import GoalContract
