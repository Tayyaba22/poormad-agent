"""Dashboard PoorMad Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_poormad_home):
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from poormad_cli.console_engine import ConsoleResult, PoorMadConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.2)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(PoorMadConsoleEngine, "execute", slow_execute)
    ...
    with console_client.websocket_connect(_url()) as conn:
        ...
        assert complete["prompt"] == "poormad> "
