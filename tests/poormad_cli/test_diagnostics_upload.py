"""Tests for ``poormad_cli.diagnostics_upload`` — the PoorMad-S3 upload client.
...
class TestRequestUploadUrl:
    def test_happy_path_posts_json_and_returns_dict(self):
        from poormad_cli.diagnostics_upload import request_upload_url
        ...
        with patch(
            "poormad_cli.diagnostics_upload.urllib.request.urlopen",
            return_value=resp,
        ) as urlopen:
    ...
    def test_base_url_env_override(self, monkeypatch):
        ...
        monkeypatch.setenv("POORMAD_DIAGNOSTICS_BASE_URL", "https://staging.example.com")
        import poormad_cli.diagnostics_upload as mod
        ...
        try:
            ...
            monkeypatch.delenv("POORMAD_DIAGNOSTICS_BASE_URL", raising=False)
...
class TestPutBundle:
    def test_put_sends_exact_body_and_content_type(self):
        from poormad_cli.diagnostics_upload import put_bundle
...
class TestShareToPoorMad:
    def test_orchestrates_request_then_put(self):
        from poormad_cli import diagnostics_upload as mod
