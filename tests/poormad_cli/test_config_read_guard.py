...
Canonical owners:
  ...
  * ``poormad_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    # gateway.run._poormad_home (delegates to read_raw_config otherwise).
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/poormad/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "poormad_cli/managed_scope.py",
