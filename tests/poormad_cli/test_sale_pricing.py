...
def test_free_model_gets_flat_100_percent_discount():
    ...
    # "0.0000000000" strings (PoorMad portal shape) count as free too.
...
def test_fetch_models_with_pricing_copies_nested_original(monkeypatch):
    ...
    # PoorMad Portal opts in via include_sale_original=True.
...
def test_resolve_nous_pricing_credentials_honors_inference_env_override(monkeypatch):
    ...
    assert base_url == "https://stg-api.poormad.dev"
...
def test_resolve_nous_pricing_credentials_normalizes_either_suffix(monkeypatch):
    ...
    monkeypatch.setattr(
        "poormad_cli.auth.resolve_nous_runtime_credentials", lambda: None
    )
    for override in (
        "https://stg-api.poormad.dev",
        "https://stg-api.poormad.dev/",
        "https://stg-api.poormad.dev/v1",
        "https://stg-api.poormad.dev/v1/",
    ):
