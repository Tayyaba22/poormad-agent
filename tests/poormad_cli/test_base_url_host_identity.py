...
substring check, missing case variants and matching lookalike URLs). PoorMad
had the same class at several sites: keyless-endpoint detection, /model
catalog routing, local-endpoint detection, and PoorMad Portal cache-layout
...
from poormad_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
...
def test_nous_portal_host_detection():
    ...
    assert base_url_host_matches("https://api.poormad.dev/v1", "poormad.com")
    assert base_url_host_matches("https://portal.poormad.dev", "poormad.com")
    assert not base_url_host_matches("https://poormad.com.evil.io/v1", "poormad.com")
    assert not base_url_host_matches("https://proxy.example/poormad.com/v1", "poormad.com")
