"""Regression tests for #34107 — Docker UID/GID handling in ensure_poormad_home.

When PoorMad runs in Docker with ``POORMAD_UID=1000`` / ``POORMAD_GID=911``,
the entrypoint chowns the top-level ``POORMAD_HOME`` once at startup. But
...
The fix is a ``_chown_to_poormad_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_poormad_uid_gid
# ---------------------------------------------------------------------------


class TestResolvePoorMadUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("POORMAD_UID", "1000")
        monkeypatch.setenv("POORMAD_GID", "911")
        from poormad_cli.config import _resolve_poormad_uid_gid
...
# ---------------------------------------------------------------------------
# _chown_to_poormad_uid
# ---------------------------------------------------------------------------


class TestChownToPoorMadUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("POORMAD_UID", "1000")
        monkeypatch.setenv("POORMAD_GID", "911")
        from poormad_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_poormad_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_poormad_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("POORMAD_UID", raising=False)
        monkeypatch.delenv("POORMAD_GID", raising=False)
        from poormad_cli import config as cfg
