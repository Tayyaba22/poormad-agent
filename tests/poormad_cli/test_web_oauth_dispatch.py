"""Regression tests for the OAuth dispatcher in poormad_cli.web_server.
...
from poormad_cli.web_server import _SESSION_TOKEN, app

client = TestClient(app)
HEADERS = {"X-PoorMad-Session-Token": _SESSION_TOKEN}
...
def _fake_nous_device_data():
    return {
        ...
        "user_code": "NOUS-1234",
        "verification_uri": "https://portal.poormad.dev/device",
        "verification_uri_complete": (
            "https://portal.poormad.dev/device?user_code=NOUS-1234"
        ),
        "expires_in": 600,
        "interval": 5,
    }
...
def _invoke_scope_refusal():
    request = httpx.Request("POST", "https://portal.poormad.dev/oauth/device/code")
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "poormad_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "poormad_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "poormad_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    from poormad_cli import web_server as ws
    ...
    def fake_status():
        observed_homes.append(get_poormad_home())
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "poormad auth add fake-oauth",
...
def test_codex_dashboard_start_rewords_device_authorization_error(monkeypatch):
    ...
    try:
        ...
        assert "poormad auth" not in detail
    finally:
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `poormad model` universe) must be offered by /api/providers/oauth. This keeps
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `poormad model` but is "
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by PoorMad."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``poormad auth remove
    xai-oauth``.
    """
    from poormad_cli import auth as auth_mod
    from poormad_cli import web_server as ws
    ...
    # Prior `poormad auth remove xai-oauth` left the source suppressed.
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to poormad_cli.auth.get_auth_status (the
    ...
    with patch("poormad_cli.auth.get_auth_status", return_value=fake_status):
