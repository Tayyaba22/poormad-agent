"""Tests for IRC gateway configuration via `poormad setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh PoorMad install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new PoorMad install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#poormad")
            monkeypatch.setenv("IRC_NICKNAME", "poormad-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `poormad setup gateway` experience with IRC present."""
