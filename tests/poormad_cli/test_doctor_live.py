"""Tests for ``poormad doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from poormad_cli import doctor_live
from poormad_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `poormad doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("poormad_cli.doctor.POORMAD_HOME", tmp_path / "home")
        monkeypatch.setattr("poormad_cli.doctor.PROJECT_ROOT", tmp_path / "root")
