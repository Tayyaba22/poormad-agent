"""Tests for the top-level `./poormad` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./poormad` should use `poormad_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "poormad"
    called = []

    fake_main_module = types.ModuleType("poormad_cli.main")

    def fake_main():
        called.append("poormad_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "poormad_cli.main", fake_main_module)
    ...
    assert called == ["poormad_cli.main"]
