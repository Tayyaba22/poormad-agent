...
the SAME provider universe as ``poormad model`` (the CLI/TUI picker).
...
def test_every_poormad_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `poormad model` universe."""
    ...
    assert not missing, (
        "providers shown in `poormad model` but not configurable in the desktop "
