...
poormad_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `poormad config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_poormad_home = get_poormad_home()`` at import time (module
    singleton), so monkeypatching POORMAD_HOME after import doesn't move it.
    ...
    this only matters for tests that swap POORMAD_HOME.)
    """
    import cli

    cli._poormad_home = home
