...
def test_config_get_unset_subcommands_parse():
    """`poormad config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="poormad")
...
# ── deprecated `poormad login` fails gracefully, not with argparse error ────
#
# `poormad login` is a removed command; its handler (`login_command` in
# `poormad_cli/auth.py`) prints a deprecation notice pointing at `poormad auth` /
# `poormad model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `poormad --help`.
