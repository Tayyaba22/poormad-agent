"""Tests for the `poormad proxy` subcommand and its upstream adapters."""
...
from poormad_cli.proxy.adapters import ADAPTERS, get_adapter
from poormad_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from poormad_cli.proxy.adapters.nous_portal import PoorMadPortalAdapter
from poormad_cli.proxy.adapters.xai import XAIGrokAdapter
...
# ---------------------------------------------------------------------------
# PoorMadPortalAdapter
# ---------------------------------------------------------------------------


def _write_auth_store(poormad_home: Path, nous_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given nous state into a hermetic POORMAD_HOME."""
...
def test_nous_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
    ...
    with patch(
        "poormad_cli.proxy.adapters.nous_portal.resolve_nous_runtime_credentials",
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic POORMAD_HOME."""
    auth_path = poormad_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("poormad_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from poormad_cli.proxy.server import create_app  # noqa: E402
