"""Tests for the per-profile Projects store (poormad_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="PoorMad", folders=["/tmp/poormad"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "poormad-agent"
    assert proj.name == "PoorMad"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/poormad"
    assert [f.path for f in proj.folders] == ["/tmp/poormad"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "poormad-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' POORMAD_HOME.
