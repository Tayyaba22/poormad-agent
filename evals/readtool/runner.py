"""Run the read-tool eval through the REAL PoorMad AIAgent.

For each task: fresh temp POORMAD_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    poormad_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".poormad"
    ...
    try:
        ...
        shutil.rmtree(poormad_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.poormad/.env; set +a  — then relaunch."
        )
