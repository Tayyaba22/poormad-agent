...
The run gets a throwaway POORMAD_HOME so no local config leaks in, and the
...
# PoorMad-subscription path on boxes without an OpenRouter key. Credentials are
...
if _or_key:
    ...
    # Resolved by the orchestrator BEFORE POORMAD_HOME is redirected to the
    # throwaway home (auth state lives in the real profile). Never printed.
    _tok = os.environ.get("BUBENCH_NOUS_TOKEN", "").strip()
    if not _tok:
        raise SystemExit("no OPENROUTER_API_KEY and no PoorMad auth available")
    _agent_auth = dict(
        base_url=os.environ.get("BUBENCH_NOUS_BASE_URL", "https://api.poormad.dev/v1"),
        api_key=_tok,
        provider="nous",
    )
