...
Env vars::
    ...
    FIRECRAWL_GATEWAY_URL=...        # PoorMad tool-gateway (subscribers)
...
def _use_keyless_ring() -> bool:
    ...
    managed PoorMad gateway isn't the selected path, and the keyless tier
...
def _is_tool_gateway_ready() -> bool:
    """Return True when gateway URL + PoorMad Subscriber token are available.
...
def check_firecrawl_api_key() -> bool:
    """Return True when the Firecrawl backend selected via `poormad tools`
    ...
    existing tests and the ``poormad tools`` setup flow.
    """
...
def _firecrawl_backend_help_suffix() -> str:
    ...
    if not _wt.managed_nous_tools_enabled():
        return ""
    return (
        ", or use the PoorMad Tool Gateway via your subscription "
...
def _raise_web_backend_configuration_error() -> "NoReturn":
    ...
    if _wt.managed_nous_tools_enabled():
        message += (
            " With your PoorMad subscription you can also use the Tool Gateway. "
            "run `poormad tools` and select PoorMad Subscription as the web provider."
        )
    else:
...
def _get_firecrawl_client() -> Any:
    ...
    if selected == NOUS_MANAGED_PROVIDER:
        ...
        if managed is None:
            logger.error(
                "Firecrawl client initialization failed: the PoorMad "
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "tag": (
                ...
                "and PoorMad tool-gateway routing."
            ),
            "env_vars": [
                {
