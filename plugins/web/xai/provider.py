...
``{title, url, description, position}`` rows every other PoorMad web
provider produces.
...
prefers PoorMad-managed xAI Grok OAuth (via ``poormad auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.poormad/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``poormad tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `poormad auth` to sign in with "
