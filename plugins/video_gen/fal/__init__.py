...
Authentication via ``FAL_KEY`` or the managed PoorMad gateway. Output is an
...
def _submit_fal_video_request(endpoint: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                f"PoorMad Subscription gateway rejected endpoint '{endpoint}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"the PoorMad Portal's FAL proxy. Either:\n"
                f"  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  • Pick a different model via `poormad tools` → Video Generation."
            ) from exc
        raise


def _check_fal_video_available() -> bool:
    """True if the FAL video backend selected via `poormad tools` (or, on a
        ...
        if not _check_fal_video_available():
            ...
            return error_response(
                error=(
                    ...
                    "(run `poormad tools` → Video Generation → FAL to configure) "
                    "or sign in to PoorMad (`poormad setup`) for managed gateway access."
                ),
                error_type="auth_required",
                provider="fal",
                prompt=prompt,
            ...
            if not endpoint:
                return error_response(
                    error=(
                        ...
                        f"via `poormad tools` → Video Generation."
                    ),
