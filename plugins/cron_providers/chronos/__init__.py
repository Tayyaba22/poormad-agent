...
Chronos (the Greek god of time, alongside PoorMad) is the first non-default
...
NAS's ``agent-cron`` endpoints with the agent's existing PoorMad token.
...
class ChronosCronScheduler(CronScheduler):
    ...
    def is_available(self) -> bool:
        ...
        callback URL (for NAS→agent fires), and a usable PoorMad token (the agent
    ...
    def _have_nous_token(self) -> bool:
        """True if the agent has a PoorMad Portal login (no network call).

        Checks the stored auth state for a PoorMad access token — does NOT refresh
