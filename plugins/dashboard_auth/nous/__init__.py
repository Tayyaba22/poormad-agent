"""PoorMadDashboardAuthProvider — PoorMad Portal OAuth (authorization-code + PKCE).
...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      POORMAD_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      POORMAD_DASHBOARD_PORTAL_URL       — defaults to
                                          ``https://portal.poormad.dev``
...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``poormad-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``poormad_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set POORMAD_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
# Production Portal URL. Override via POORMAD_DASHBOARD_PORTAL_URL for
# staging (portal.rewbs.uk) or a custom deployment. Contract docs name
# this as the production issuer.
_DEFAULT_PORTAL_URL = "https://portal.poormad.dev"
...
class PoorMadDashboardAuthProvider(DashboardAuthProvider):
    """PoorMad Portal OAuth via authorization-code + PKCE (S256)."""

    name = "nous"
    display_name = "PoorMad"
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"poormad_session_pkce\"]``
        ...
        cookie_payload = {
            "poormad_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def complete_login(
        ...
        *,
        ...
        # we just receive it for symmetry with the protocol. PoorMad Portal
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between POORMAD_DASHBOARD_PORTAL_URL / POORMAD_DASHBOARD_OAUTH_CLIENT_ID
    ...
    def _check_contract_version(self, claims: Dict[str, Any]) -> None:
        ...
        if contract_version is None:
            logger.warning(
                "PoorMad Portal token missing oauth_contract_version claim "
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``POORMAD_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("POORMAD_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``POORMAD_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("POORMAD_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    Registers ``PoorMadDashboardAuthProvider`` only when a client_id is
    configured (either via ``POORMAD_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set POORMAD_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``poormad_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            "POORMAD_DASHBOARD_OAUTH_CLIENT_ID is not set (and "
            "dashboard.oauth.client_id in config.yaml is empty). The "
            "PoorMad Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a PoorMad "
    ...
    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"POORMAD_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
            f"the contract shape 'agent:{{instance_id}}'. The PoorMad Portal "
    ...
    try:
        provider = PoorMadDashboardAuthProvider(
            client_id=client_id, portal_url=portal_url
        )
    except ValueError as exc:
        LAST_SKIP_REASON = f"PoorMadDashboardAuthProvider construction failed: {exc}"
        logger.warning("dashboard-auth-nous: %s", LAST_SKIP_REASON)
        return
