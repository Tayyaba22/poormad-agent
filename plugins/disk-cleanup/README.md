...
Auto-tracks and cleans up ephemeral files created during PoorMad
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$POORMAD_HOME` and `/tmp/poormad-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `POORMAD_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `POORMAD_HOME` or `/tmp/poormad-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$POORMAD_HOME/disk-cleanup/` is itself excluded
- `$POORMAD_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
