"""disk_cleanup — ephemeral file cleanup for PoorMad.
...
Rules:
  ...
  - empty dirs    → always delete (under POORMAD_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly POORMAD_HOME and /tmp/poormad-*
Never touches: ~/.poormad/logs/ or any system directory.
"""
...
try:
    ...
    def get_poormad_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("POORMAD_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".poormad").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$POORMAD_HOME/logs/``."""
    return get_poormad_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$POORMAD_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under POORMAD_HOME or ``/tmp/poormad-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    poormad_home = get_poormad_home()
    try:
        path.resolve().relative_to(poormad_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/poormad-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("poormad-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "poormad-agent", "backups", "profiles", ".worktrees",
...
# Paths under $POORMAD_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so POORMAD_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        poormad_home = get_poormad_home()
        for parent in ("cron", "cronjobs"):
            base = poormad_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside POORMAD_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under POORMAD_HOME, but never recurse into known
    # durable state trees.  Some installs place the PoorMad checkout, venv,
    # and desktop build under POORMAD_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    poormad_home = get_poormad_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in poormad_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    poormad_home = get_poormad_home()
    try:
        rel = path.resolve().relative_to(poormad_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "poormad-agent",
        ...
        # Path isn't under POORMAD_HOME (e.g. /tmp/poormad-*) — fall through.
        pass
