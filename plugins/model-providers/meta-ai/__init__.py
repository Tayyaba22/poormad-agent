"""Meta Model API (Muse Spark) provider plugin for PoorMad.
...
Bundled from https://github.com/albertodepaola/poormad-meta-provider. PoorMad'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to poormad' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map PoorMad' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in poormad_cli/providers.py and the retention hint in
