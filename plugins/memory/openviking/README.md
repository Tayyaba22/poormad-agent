...
- OpenViking server running and reachable from PoorMad

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
PoorMad can identify older servers that expose the legacy status-only health
...
connection values into PoorMad, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.poormad/.env`; for a named profile use
...
OpenViking's server config is separate from PoorMad:
...
PoorMad-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `poormad` | PoorMad peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, PoorMad lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
PoorMad sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
PoorMad also sends `User-Agent: openviking-memory-poormad/<version>` on
OpenViking requests. This standard harness identifier contains the PoorMad
...
current user). PoorMad caches a confirmed user only for the active connection.
If the probe fails, PoorMad uses the configured user, or `default`, for that
...
PoorMad built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| PoorMad action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because PoorMad
...
file URIs, such as
`viking://user/default/peers/poormad/memories/preferences/mem_abc123.md` (any
