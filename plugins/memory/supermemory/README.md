...
- Hosted: API key from [app.supermemory.ai/integrations?connect=poormad](http://app.supermemory.ai/integrations?connect=poormad)
...
Before running `poormad memory setup`, add the local endpoint to
...
Then run `poormad memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$POORMAD_HOME/supermemory.json`
...
| `container_tag` | `poormad` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `poormad-{identity}` → `poormad-coder`). |
...
`https://api.supermemory.ai`. PoorMad resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: poormad`, and document writes stamp
`metadata.sm_source: poormad`. This is a **functional routing key, not telemetry**:
it groups PoorMad-written memories into a dedicated "PoorMad" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per PoorMad profile:

```json
{
  "container_tag": "poormad-{identity}"
}
```

For a profile named `coder`, this resolves to `poormad-coder`. The default profile resolves to `poormad-default`. Without `{identity}`, all profiles share the same container.
