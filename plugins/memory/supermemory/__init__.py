...
_DEFAULT_CONTAINER_TAG = "poormad"
...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=poormad"
...
def _load_supermemory_config(poormad_home: str) -> dict:
    config = _default_config()
    config_path = Path(poormad_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, poormad_home: str) -> None:
    config_path = Path(poormad_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "poormad"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes PoorMad writes into the "PoorMad" Space in the Supermemory
        ...
        merged = {"sm_source": "poormad", **(metadata or {})}
...
def _resolve_container_tag_for_setup(poormad_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(poormad_home)
...
def _probe_supermemory_connection(api_key: str, poormad_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(poormad_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(poormad_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `poormad memory setup`.
        # All other options are documented for $POORMAD_HOME/supermemory.json
    ...
    def save_config(self, values, poormad_home):
        ...
        _save_supermemory_config(sanitized, poormad_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, poormad_home)
    ...
    def post_setup(self, poormad_home: str, config: dict) -> None:
        ...
        from poormad_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(env_writes, poormad_home=poormad_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from poormad_constants import get_poormad_home
        self._poormad_home = kwargs.get("poormad_home") or str(get_poormad_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._poormad_home)
