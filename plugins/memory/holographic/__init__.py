"""poormad-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $POORMAD_HOME/config.yaml (profile-scoped):
  plugins:
    poormad-memory-store:
      db_path: $POORMAD_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "poormad-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, poormad_home):
        """Write config to config.yaml under plugins.poormad-memory-store."""
        from pathlib import Path
        config_path = Path(poormad_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["poormad-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_poormad_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _poormad_home = str(get_poormad_home())
        _default_db = _poormad_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $POORMAD_HOME in user-supplied paths so config values like
        # "$POORMAD_HOME/memory_store.db" or "~/.poormad/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$POORMAD_HOME", _poormad_home)
            db_path = db_path.replace("${POORMAD_HOME}", _poormad_home)
