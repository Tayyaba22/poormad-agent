...
```bash
poormad memory setup    # select "mem0"
```
...
Behavioral settings live in `$POORMAD_HOME/mem0.json` (set them via `poormad memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.poormad/.env`.
...
| `user_id` | `poormad-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh PoorMad session and call `mem0_search` — it connects to your server.
...
```bash
poormad memory setup mem0 --mode oss \
...
```bash
poormad memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$POORMAD_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.poormad/mem0_qdrant"}}
  }
}
```
...
```bash
poormad memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$POORMAD_HOME/mem0.json`).
