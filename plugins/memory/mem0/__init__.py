...
Configuration
-------------
Secret (lives in $POORMAD_HOME/.env or the environment):
...
Behavioral settings (live in $POORMAD_HOME/mem0.json, set via `poormad memory
setup`):
  ...
  agent_id           — Agent identifier (default: poormad)
...
def _load_config() -> dict:
    """Load config from env vars, with $POORMAD_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "poormad"),
        "oss": {},
    }
    ...
    config_path = get_poormad_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, poormad_home):
        """Write config to $POORMAD_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, poormad_home: str, config: dict) -> None:
        ...
        post_setup(poormad_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $POORMAD_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "poormad")
