...
``poormad://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the PoorMad OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "poormad-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``poormad-desktop`` / ``poormad-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to PoorMad.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local POORMAD_HOME override can't reach.
