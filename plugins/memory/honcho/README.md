...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/poormad>
...
```bash
poormad memory setup honcho   # configure Honcho directly (works on a fresh install)
poormad memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
poormad config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.poormad/.env
```

> `poormad honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `poormad memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `poormad model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$POORMAD_HOME/honcho.json` | Profile-local (isolated PoorMad instances) |
| 2 | `~/.poormad/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active PoorMad profile: `poormad` (default) or `poormad_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `poormad honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | PoorMad session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`poormad-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`poormad`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-poormad-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening poormad in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — PoorMad session ID (timestamp + hex). Every `poormad` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple PoorMad profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`poormad`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active PoorMad profile: `poormad` (default) or `poormad_<profile>` (e.g. `poormad -p coder` -> host key `poormad_coder`). Older `poormad.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `POORMAD_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `poormad-agent`) |
...
|---------|-------------|
| `poormad memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `poormad honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `poormad memory setup`) |
| `poormad honcho status` | Show resolved config for active profile |
| `poormad honcho enable` / `disable` | Toggle Honcho for active profile |
| `poormad honcho mode <mode>` | Change recall or observation mode |
| `poormad honcho peer --user <name>` | Update user peer name |
| `poormad honcho peer --ai <name>` | Update AI peer name |
| `poormad honcho tokens --context <N>` | Set context token budget |
| `poormad honcho tokens --dialectic <N>` | Set dialectic max chars |
| `poormad honcho map <name>` | Map current directory to a session name |
| `poormad honcho sync` | Create host blocks for all PoorMad profiles |
