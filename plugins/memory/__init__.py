...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with poormad-agent)
2. User-installed providers: ``$POORMAD_HOME/plugins/<name>/``
3. Project-local providers: ``./.poormad/plugins/<name>/``, opt-in via
   ``POORMAD_ENABLE_PROJECT_PLUGINS``
4. Pip-installed providers: ``poormad_agent.memory_providers`` entry points
...
ENTRY_POINTS_GROUP = "poormad_agent.memory_providers"
...
_USER_NAMESPACE = "_poormad_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_poormad_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_poormad_user_memory'`` — the
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$POORMAD_HOME/plugins/`` or None if unavailable."""
    try:
        from poormad_constants import get_poormad_home
        d = get_poormad_home() / "plugins"
...
def _get_project_plugins_dir() -> Optional[Path]:
    """Return ``./.poormad/plugins/`` or None if unavailable or not opted in.

    Gated on ``POORMAD_ENABLE_PROJECT_PLUGINS`` exactly as the general
    ...
    """
    try:
        from poormad_cli.plugins import _env_enabled

        if not _env_enabled("POORMAD_ENABLE_PROJECT_PLUGINS"):
            return None
        d = Path.cwd() / ".poormad" / "plugins"
        return d if d.is_dir() else None
    except Exception:
        return None
...
def _iter_provider_dirs() -> List[Tuple[str, Path]]:
    ...
    # 2. User-installed providers ($POORMAD_HOME/plugins/<name>/)
    # 3. Project-local providers (./.poormad/plugins/<name>/), opt-in
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    ``poormad <provider>`` subcommands — working, but a second-class citizen next
...
def _entry_point_package_dir(entry_point) -> Optional[Path]:
    ...
    if entry_point is None:
        return None
    try:
        from poormad_cli.plugins import resolve_module_origin
...
def load_memory_provider(
    ...
    *,
    ...
    (``$POORMAD_HOME/plugins/<name>/``), and pip entry-point providers.
...
class _ProviderCollector:
    ...
    def register_skill(self, *args, **kwargs):
        ...
        provider — ``poormad memory status``, the setup picker — leaves no
    ...
    def _plugin_context(self):
        ...
        plugin manager, which discovery touches on every poormad startup.
        """
        if self._context is None:
            from poormad_cli.plugins import PluginContext, PluginManifest, get_plugin_manager
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _poormad_user_memory.<name>.cli, usually
