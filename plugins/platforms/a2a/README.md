# A2A — Agent-to-Agent protocol for PoorMad
...
peer (another PoorMad, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
poormad gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, PoorMad serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.poormad/a2a_audit.jsonl`.
- Conversations persist to `~/.poormad/a2a_conversations/` — they survive context
