"""ntfy platform adapter (PoorMad plugin).
...
a PoorMad dependency.

This adapter ships as a PoorMad platform plugin under
``plugins/platforms/ntfy/``. The PoorMad plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "poormad-in"              # subscribe topic (incoming)
          publish_topic: "poormad-out"     # optional — defaults to topic
...
_ECHO_TAG = "poormad-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``poormad cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a PoorMad dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `poormad gateway status` without
