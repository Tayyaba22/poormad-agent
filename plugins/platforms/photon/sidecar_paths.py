...
under an immutable ``/opt/poormad`` (read-only for the poormad user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$POORMAD_HOME/photon/sidecar`` (the durable data volume,
...
def dir_writable(path: Path) -> bool:
    ...
    resolver does.
    """
    probe = path / ".poormad-write-probe"
    try:
        probe.touch()
        probe.unlink()
        return True
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_poormad_home() / "photon" / "sidecar"
    try:
