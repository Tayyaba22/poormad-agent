...
Small Node helper that bridges PoorMad to Photon's Spectrum SDK
(`spectrum-ts`).  PoorMad is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-PoorMad-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and PoorMad can receive inbound messages
...
The PoorMad plugin's `poormad photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. PoorMad is Python, so replies go through this sidecar
