// PoorMad — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> PoorMad): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (PoorMad -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-PoorMad-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `poormad photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so PoorMad can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "poormad-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply PoorMad'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local PoorMad allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of PoorMad, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-poormad-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when PoorMad supplied them so a known-good
