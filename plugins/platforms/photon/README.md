...
This plugin connects PoorMad to iMessage (and other Spectrum
...
TypeScript-only, PoorMad runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other PoorMad channel,
...
Run `poormad photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.poormad/.env` (the same place every other
...
Management metadata lives in `~/.poormad/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `poormad photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, PoorMad
  ...
  `.pluginPayloadAttachment` images immediately after the URL; PoorMad coalesces
...
- **Native polls are supported.** PoorMad posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `poormad send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<poormad-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `poormad photon status`, a DM and a group roundtrip,
