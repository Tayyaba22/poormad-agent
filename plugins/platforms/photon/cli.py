"""
``poormad photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other PoorMad
...
# Writable sidecar runtime dir (mirrors to POORMAD_HOME on immutable
...
# wiring imports this module for `poormad --help`).
...
# when called later (gateway start, poormad status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `poormad photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'PoorMad')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``poormad photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "PoorMad" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.poormad/.env,
    ...
    try:
        from poormad_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  poormad gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `poormad photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`poormad photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.poormad/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `poormad photon telemetry on` / `poormad photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.poormad/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  poormad gateway restart")
    return 0
...
#
# `poormad gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``poormad photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `poormad gateway setup` wizard."""
