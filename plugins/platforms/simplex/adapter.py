"""SimpleX Chat platform adapter (PoorMad plugin).
...
This adapter ships as a PoorMad platform plugin under
``plugins/platforms/simplex/``. The PoorMad plugin loader scans the
...
Optional environment variables:
    ...
    POORMAD_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``poormad setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside PoorMad — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("POORMAD_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``poormad setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.poormad/.env`` via
    ``poormad_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "poormad_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.poormad/.env"
        )
        return
