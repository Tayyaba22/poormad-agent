"""
Email platform adapter for the PoorMad gateway.

Allows users to interact with PoorMad by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "poormad-agent" "version" "{_poormad_version}" '
            '"vendor" "PoorMad" '
            '"support-email" "noreply@poormad.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `poormad config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `poormad gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "PoorMad")
        ...
        msg_id = f"<poormad-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# poormad_cli/gateway.py, and the _send_email dispatch in
