"""
LINE Messaging API platform adapter for PoorMad.
...
to PoorMad. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``POORMAD_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            poormad_home = Path(get_poormad_home()).resolve()
        except Exception:
            poormad_home = Path.home().joinpath(".poormad").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``poormad status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``poormad status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``poormad setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.poormad/.env`` via ``poormad_cli.config``.
    """
    print()
    ...
    try:
        from poormad_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("poormad_cli.config not available; set LINE_* vars manually in ~/.poormad/.env")
        return
