...
# ``model_tools`` import time, so ``poormad status``, ``poormad chat``, etc.
...
# ``poormad_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "PoorMad is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a poormad MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "poormad-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${POORMAD_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from poormad_constants import get_poormad_home as _get_poormad_home
            _poormad_home = _get_poormad_home()
        except (ModuleNotFoundError, ImportError):
            _poormad_home = _Path.home() / ".poormad"
        self._thread_count_store = _ThreadCountStore(
            _poormad_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "PoorMad is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("POORMAD_HOME", str(_Path.home() / ".poormad"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${POORMAD_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a poormad MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'PoorMad is thinking…' marker message.
        ...
        "PoorMad is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "PoorMad is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "PoorMad is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "PoorMad is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``poormad setup``.

    The setup wizard at ``poormad_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.poormad/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. poormad-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.poormad/.env")
    print_info("Restart the gateway: poormad gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``poormad cron`` running as a
    separate process from ``poormad gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the PoorMad plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `poormad setup` to install Google Chat support.",
        ...
        # the "PoorMad is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'PoorMad is thinking…' marker message "
