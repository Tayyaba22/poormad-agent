...
three virtual model IDs so the ``poormad tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `poormad tools` → Image "
                    "Generation → OpenAI to configure, or `poormad setup` "
                    "to add the key."
                ),
