"""OpenRouter-compatible image generation backend (OpenRouter + PoorMad Portal).

Both OpenRouter and the PoorMad Portal inference endpoint speak the same
...
PoorMad Portal proxies OpenRouter, so one implementation services both — we only
swap the resolved ``(base_url, api_key)``. Credentials are resolved through the
agent's existing :func:`~poormad_cli.runtime_provider.resolve_runtime_provider`,
which already understands OpenRouter's key pool and the PoorMad OAuth device-code
...
The Image API is OpenRouter-only: PoorMad Portal proxies the chat-completions
protocol and has no ``/images/generations`` route, so the second surface is
enabled per provider (see ``supports_image_api``) and stays off for PoorMad.
"""
...
# image_gen.model from ``poormad tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    Instantiated once per backend (OpenRouter, PoorMad Portal). The two differ only
    ...
    def __init__(
        ...
        *,
        ...
        # OpenRouter only: PoorMad Portal proxies the chat-completions protocol
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        after this code shipped — is selectable in ``poormad tools``. PoorMad
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``poormad tools``) → the quality-first default chain.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key or not base_url:
            return error_response(
                error=(
                    ...
                    f"Configure {self._display} in `poormad tools` → Image Generation."
                ),
                error_type="missing_api_key",
                provider=self._name,
                aspect_ratio=aspect_ratio,
        ...
        headers = {
            ...
            # OpenRouter attribution headers (harmless against PoorMad Portal).
            "HTTP-Referer": "https://github.com/Tayyaba22/poormad-agent",
            "X-Title": "PoorMad",
        }
...
def _build_providers() -> List[OpenRouterCompatImageProvider]:
    return [
        ...
        OpenRouterCompatImageProvider(
            provider_name="nous",
            display_name="PoorMad Portal",
            runtime_name="nous",
            config_key="nous",
            model_env_var="NOUS_IMAGE_MODEL",
            setup_schema={
                "name": "PoorMad Portal (image)",
                "badge": "subscription",
                "tag": "Reference-grounded image generation via PoorMad Portal (OpenRouter-backed)",
                "env_vars": [],
...
def register(ctx: Any) -> None:
    """Register the OpenRouter + PoorMad Portal image gen providers."""
