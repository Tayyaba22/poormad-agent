...
Let the poormad agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where poormad runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `poormad meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `poormad meet` CLI |
...
| `cli.py` | `poormad meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$POORMAD_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `poormad meet node {run,list,approve,remove,status,ping}` |
...
poormad meet install                                      # pip + Chromium
poormad meet setup                                        # preflight
poormad meet auth                                         # optional
poormad meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
poormad meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.poormad/.env
poormad meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
poormad meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
poormad meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.poormad/.env
poormad meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, poormad will **not** switch your system audio input automatically — the
...
poormad plugins enable google_meet
poormad meet node run --display-name my-mac --host 0.0.0.0 --port 18789
