"""`poormad meet node ...` subcommand tree.

Wired into the existing ``poormad meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``poormad meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="poormad-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``poormad meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             poormad meet node approve <name> ws://<host>:{args.port} {token}")
        try:
