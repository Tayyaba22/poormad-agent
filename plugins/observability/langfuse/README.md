...
This plugin ships bundled with PoorMad but is **opt-in** — it only loads when
...
poormad tools  # → Langfuse Observability
...
Set these in `~/.poormad/.env` (or via `poormad tools`):
...
poormad chat -q "hello"              # then check Langfuse for a "PoorMad turn" trace
```

Generation observations include the PoorMad system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `POORMAD_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
POORMAD_LANGFUSE_ENV=production       # environment tag
POORMAD_LANGFUSE_RELEASE=v1.0.0       # release tag
POORMAD_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
POORMAD_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
POORMAD_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
POORMAD_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`POORMAD_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
poormad plugins disable observability/langfuse
```
