...
``<poormad home>/plugins/<name>/``. That tree is the plugin *install* dir —
``poormad plugins remove`` deletes it and ``poormad plugins update`` git-pulls
...
``<poormad home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in PoorMad.
...
# Mirrors the plugin-name shape `poormad plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<poormad home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`poormad_constants.get_poormad_home` on every call. Don't cache the
    result across profile switches.
    """
    from poormad_constants import get_poormad_home

    root = get_poormad_home() / "plugin-data" / _validate_name(name)
