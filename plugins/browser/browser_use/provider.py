...
``BROWSER_USE_API_KEY`` for self-billed users, or the managed PoorMad tool
gateway (which PoorMad uses to bill Browser Use sessions to a PoorMad
...
Auth env vars (one of)::
    ...
    # OR a managed PoorMad gateway entry (configured via 'poormad setup')
"""
...
# Idempotency tracking for managed-mode session creation. The managed PoorMad
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    to the managed PoorMad tool gateway when ``tool_gateway.browser`` config
    ...
    # ------------------------------------------------------------------
    # Config resolution (direct API key OR managed PoorMad gateway)
    # ------------------------------------------------------------------

    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        # Import here to avoid a hard dependency at module-import time —
        # managed_tool_gateway pulls in the PoorMad auth stack which can be
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when PoorMad only needs a task-
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"poormad_{task_id}_{uuid.uuid4().hex[:8]}"
        external_call_id = (
    ...
    def get_setup_schema(self) -> Optional[Dict[str, Any]]:
        # Hidden from the poormad tools picker: the "Browser Use" row now
        # activates the CLI-based backend (tools/browser_use_cli.py). This
        # provider stays registered for the PoorMad gateway path and un-migrated
