...
const BACKEND = process.env.POORMAD_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `poormad dashboard` server injects a one-shot
 * session token into `index.html` (see `poormad_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__POORMAD_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function poormadDevToken(): Plugin {
  const TOKEN_RE = /window\.__POORMAD_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__POORMAD_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "poormad:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[poormad] Could not find session token in ${BACKEND} — ` +
              `is \`poormad dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__POORMAD_SESSION_TOKEN__="${match[1]}";` +
              `window.__POORMAD_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[poormad] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`poormad dashboard\` or set POORMAD_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@poormad/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `poormad dashboard` must serve these; Vite has no
