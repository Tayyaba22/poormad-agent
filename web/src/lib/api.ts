import { buildPoorMadWebSocketUrl } from "@poormad/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/poormad/). The Python backend
// injects ``window.__POORMAD_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__POORMAD_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const POORMAD_BASE_PATH = readBasePath();
const BASE = POORMAD_BASE_PATH;
...
declare global {
  interface Window {
    __POORMAD_SESSION_TOKEN__?: string;
    __POORMAD_BASE_PATH__?: string;
    ...
    __POORMAD_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-PoorMad-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__POORMAD_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``poormad update``, ``poormad gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__POORMAD_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__POORMAD_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__POORMAD_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``poormad_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__POORMAD_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-PoorMad-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``poormad_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__POORMAD_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildPoorMadWebSocketUrl({
...
export const api = {
  ...
  updatePoorMad: () =>
    fetchJSON<ActionResponse>("/api/poormad/update", { method: "POST" }),
  checkPoorMadUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/poormad/update/check${force ? "?force=true" : ""}`,
    ),
 ...
 * PoorMad Portal contract V1 (the access token has no email/name claims —
...
export interface SkillHubSource {
  ...
  /** poormad-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``poormad update``. */
  can_update_poormad?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the POORMAD_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the POORMAD_HOME volume — no staleness dimension, so no
 * sampled_at. */
