...
const SIDEBAR_COLLAPSED_KEY = "poormad-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="poormad-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="poormad-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdatePoorMad = status?.can_update_poormad === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'poormad update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "poormad update";
    return (
      t.status.updatePoorMadConfirmMessage ??
      `This will run 'poormad update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updatePoorMadConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdatePoorMad) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updatePoorMad,
      runningLabel: t.status.updatingPoorMad,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the PoorMad gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updatePoorMadConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updatePoorMadConfirmTitle ?? `${t.status.updatePoorMad}?`}
    />
    </>
  );
}
