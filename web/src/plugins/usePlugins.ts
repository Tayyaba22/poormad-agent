...
import { api, POORMAD_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "poormad:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${POORMAD_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${POORMAD_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?poormad_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-poormad-plugin", manifest.name);
