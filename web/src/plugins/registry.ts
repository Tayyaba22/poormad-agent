 ...
 *
 * Plugins call window.__POORMAD_PLUGINS__.register(name, Component)
 ...
 * Exposed at runtime as ``window.__POORMAD_PLUGIN_SDK__.sdkVersion`` so a
...
export function exposePluginSDK() {
  window.__POORMAD_PLUGINS__ = {
    register: registerPlugin,
    registerSlot,
  };

  window.__POORMAD_PLUGIN_SDK__ = {
    ...
    // window.__POORMAD_SESSION_TOKEN__ directly.
    authedFetch,
    ...
    // UI components — PoorMad DS where available, shadcn/ui primitives elsewhere.
    components: {
      Card,
      CardHeader,
      CardTitle,
