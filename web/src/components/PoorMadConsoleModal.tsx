...
interface PoorMadConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function PoorMadConsoleModal({ open, onClose }: PoorMadConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("poormad> ");
  const inputPromptRef = useRef("poormad> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "poormad> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to PoorMad Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="poormad-console-title"
    >
      <div
        className={cn(
          themedBody,
