...
class SessionSearchMixin:
    ...
    def search_messages(
        ...
        Threshold: POORMAD_SEARCH_SLOW_MS (default 1000; 0 logs every call).
        """
        started = time.time()
        rows = None
        try:
            ...
            try:
                threshold = float(os.getenv("POORMAD_SEARCH_SLOW_MS", "1000"))
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another PoorMad surface and jump
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``POORMAD_DISABLE_FTS_TRIGRAM`` or not yet created), so
