#!/usr/bin/env bash
# Canonical test runner for poormad-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.poormad/...)
#
# Usage:
...
# (POORMAD_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.poormad/poormad-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.poormad/poormad-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${POORMAD_PYTHON:-}" ] && [ -x "$POORMAD_PYTHON" ] \
    && "$POORMAD_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: POORMAD_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `poormad` binary rather
  # than the devShell hook.
  PYTHON="$POORMAD_PYTHON"
  echo "▶ no local venv — using Nix dev venv via POORMAD_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and POORMAD_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.poormad/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.poormad"
...
#
#   * POORMAD_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * POORMAD_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     POORMAD_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# POORMAD_RUN_SLOW_PET_TESTS / POORMAD_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no POORMAD_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in POORMAD_TEST_IMAGE POORMAD_TEST_WORKERS POORMAD_TEST_PATHS \
  POORMAD_TEST_FILE_TIMEOUT POORMAD_TEST_FILE_RETRIES POORMAD_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${POORMAD_RUN_SLOW_PET_TESTS:+POORMAD_RUN_SLOW_PET_TESTS="$POORMAD_RUN_SLOW_PET_TESTS"} \
  ${POORMAD_E2E_BROWSER:+POORMAD_E2E_BROWSER="$POORMAD_E2E_BROWSER"} \
