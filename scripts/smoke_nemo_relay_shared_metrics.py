"""Run a real PoorMad CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_poormad_executable(poormad_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "poormad",
        Path(".venv") / "Scripts" / "poormad.exe",
    ):
        candidate = poormad_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("poormad")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "PoorMad executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="PoorMad source checkout containing .venv/bin/poormad",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated POORMAD_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["poormad.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['poormad.client.active']}"
        )
    [model] = by_name["poormad.model_route.count"]
    expected_model = {
        "name": "poormad.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['poormad.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["poormad.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['poormad.task_run.started']}"
        )
    [terminal] = by_name["poormad.task_run.finished"]
    ...
    [tool] = by_name["poormad.tool_call.count"]
    ...
    lifecycle = by_name["poormad.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["poormad.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The PoorMad development environment requires jsonschema"
        ) from exc
    ...
    if metrics["poormad.client.active"] != [
        {
            "name": "poormad.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['poormad.client.active']}"
        )
    [model] = metrics["poormad.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['poormad.model_route.count']}"
        )
    [terminal] = metrics["poormad.task_run.finished"]
    ...
    [tool] = metrics["poormad.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["poormad.skill.lifecycle.count"]
    ...
    loads = metrics["poormad.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    poormad_repo = args.poormad_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    poormad = _resolve_poormad_executable(poormad_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="poormad-relay-shared-metrics-"))
    home = root / "poormad-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["POORMAD_HOME"] = str(home)
        python_paths = [str(poormad_repo)]
    ...
    (root / "poormad.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "poormad.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"PoorMad exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("PoorMad model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("PoorMad did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("PoorMad did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from poormad_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "poormad.shared_metrics.v2.schema.json",
    )

    print("PoorMad -> NeMo Relay shared-metrics smoke test passed")
