...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[poormad] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[poormad`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[poormad`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        PoorMadHome = $(if ($paths) { $paths.poormad_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\poormad\poormad-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-poormad'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-poormad') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
