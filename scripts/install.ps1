...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$PoorMadHome = $(if ($env:POORMAD_HOME) { $env:POORMAD_HOME } else { "$env:LOCALAPPDATA\poormad" }),
    [string]$InstallDir = $(if ($env:POORMAD_HOME) { "$env:POORMAD_HOME\poormad-agent" } else { "$env:LOCALAPPDATA\poormad\poormad-agent" }),
    ...
    # builds apps/desktop into a launchable PoorMad.exe.
    #
    # Why opt-in:
    #   * PoorMad-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched PoorMad.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live PoorMad.exe
    ...
    #     `poormad desktop` already builds on demand.
...
# POORMAD_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[poormad] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'PoorMadInstall.LongPath').Type) {
            Add-Type -Namespace 'PoorMadInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [PoorMadInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -PoorMadHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('PoorMadHome')) {
    $PoorMadHome = ConvertTo-LongPath $PoorMadHome
} else {
    $PoorMadHome = ConvertTo-LongPath $(
        if ($env:POORMAD_HOME) { $env:POORMAD_HOME } else { "$env:LOCALAPPDATA\poormad" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:POORMAD_HOME) { "$env:POORMAD_HOME\poormad-agent" } else { "$env:LOCALAPPDATA\poormad\poormad-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: PoorMadHome=$PoorMadHome InstallDir=$InstallDir"
}
...
function Write-Banner {
    ...
    Write-Host "|             * PoorMad Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by PoorMad.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $PoorMadHome)) {
        New-Item -ItemType Directory -Force -Path $PoorMadHome | Out-Null
    }
    $envFile = Join-Path $PoorMadHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `poormad update`'s npx
    ...
    $prefixDir = Join-Path $PoorMadHome "node"
...
function Install-Uv {
    # PoorMad owns its own uv at $PoorMadHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (poormad_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `poormad update` stay in sync.
    $managedUv = Join-Path $PoorMadHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $PoorMadHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $PoorMadHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $PoorMadHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $PoorMadHome "bin"
        ...
        # (poormad_cli/managed_uv.py looks only at $PoorMadHome\bin\uv.exe).
...
# Put the PoorMad-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone poormad-setup.exe run, a user typing `npm`) silently
...
# Upgrade the PoorMad-managed Node tree's bundled npm into $NpmRange.
#
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "PoorMad-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("poormad-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and POORMAD_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under PoorMad-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun PoorMad setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which PoorMad uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\poormad\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    PoorMad needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the PoorMad copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\poormad\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``POORMAD_GIT_BASH_PATH`` (User scope) so PoorMad can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a PoorMad-managed PortableGit install instead..."
    }

    # Download PortableGit into $PoorMadHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $PoorMadHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent PoorMad features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  PoorMad needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``POORMAD_GIT_BASH_PATH`` (User env scope) so PoorMad can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $PoorMadHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $PoorMadHome\git\usr\bin\bash.exe
    $candidates += "$PoorMadHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$PoorMadHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("POORMAD_GIT_BASH_PATH", $candidate, "User")
            $env:POORMAD_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set POORMAD_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- PoorMad may not find Git Bash."
    Write-Info "If needed, set POORMAD_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        Write-Warn "Node.js $version is too old (PoorMad requires Node >=26)"
    }

    # Prefer a PoorMad-managed Node from a previous run over a too-old system one.
    $managedNode = "$PoorMadHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$PoorMadHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$PoorMadHome\node"
        Write-Success "Node.js $version found (PoorMad-managed)"
        ...
        Update-ManagedNpm "$PoorMadHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing PoorMad-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $PoorMadHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $PoorMadHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\poormad-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$PoorMadHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$PoorMadHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$PoorMadHome\node") {
                    try {
                        Rename-Item "$PoorMadHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "PoorMad-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$PoorMadHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $PoorMadHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\poormad-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `poormad
                    # update` path (#4735).
                    ...
                    $stashName = "poormad-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. poormad-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``poormad update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if PoorMad behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running poormad processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $extractPath = "$env:TEMP\poormad-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `poormad update` (see the notes at the shared clone-path
    ...
    # next `poormad update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under PoorMad-Setup.exe
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running poormad process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running poormad processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*PoorMad_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (poormad.exe) plus its child tree.
            & taskkill /F /T /IM poormad.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM poormad.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m poormad_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not poormad.exe.
            ...
            # PoorMad children re-exec through .poormad-runtime, so killing only
            ...
            # user (see poormad_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-PoorMad " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `poormad update` / the
...
function Restore-VenvBackup {
    ...
    # venv so PoorMad (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `poormad.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'poormad-agent\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install poormad-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and poormad.exe broken with
    ...
    # here, not 30 seconds later when the user runs `poormad`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close PoorMad processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close PoorMad processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves PoorMad and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register poormad.exe in dist-info/RECORD but fail to
        ...
        # `poormad` command while poormad-agent.exe / poormad-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m poormad_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `poormad dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\poormad_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `poormad dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `poormad dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: poormad_cli/web_server.py"
        }
    }
...
function Install-PoorMadCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the poormad launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring poormad.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "poormad.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the poormad command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # poormad_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("poormad", "poormad-acp")) {
    ...
    $requiredExe = Join-Path $Destination "poormad.exe"
    $requiredCmd = Join-Path $Destination "poormad.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the poormad command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up poormad command..."
    
    if ($NoVenv) {
        $poormadBin = "$InstallDir"
    } else {
        # $PoorMadHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `poormad update`'s autostash
        ...
        # installer staged under poormad-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-PoorMadCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $poormadBin = "$PoorMadHome\bin"
        Install-PoorMadCommandLaunchers -Root $InstallDir -Destination $poormadBin | Out-Null
    }
    ...
    #   poormad-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The poormad-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept poormad via $poormadBin)"
        }
    }
    
    if ($currentPath -notlike "*$poormadBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$poormadBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $poormadBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set POORMAD_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\poormad instead
    # of the Unix default ~/.poormad
    $currentPoorMadHome = [Environment]::GetEnvironmentVariable("POORMAD_HOME", "User")
    if (-not $currentPoorMadHome -or $currentPoorMadHome -ne $PoorMadHome) {
        [Environment]::SetEnvironmentVariable("POORMAD_HOME", $PoorMadHome, "User")
        Write-Success "Set POORMAD_HOME=$PoorMadHome"
    }
    $env:POORMAD_HOME = $PoorMadHome
    
    # Update current session
    $env:Path = "$poormadBin;$env:Path"
    
    Write-Success "poormad command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.poormad-bootstrap-complete which tells the PoorMad
    ...
    # PoorMad-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `poormad update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".poormad-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the POORMAD_HOME directory structure ($PoorMadHome, default %LOCALAPPDATA%\poormad)
    New-Item -ItemType Directory -Force -Path "$PoorMadHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PoorMadHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PoorMadHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PoorMadHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PoorMadHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PoorMadHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PoorMadHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PoorMadHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PoorMadHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$PoorMadHome\.env"
    ...
    # Create config.yaml
    $configPath = "$PoorMadHome\config.yaml"
    ...
    # (the default PS5 behaviour), and PoorMad's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$PoorMadHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in poormad_cli/default_soul.py. The runtime
        ...
        $soulContent = @"
You are PoorMad, an intelligent AI assistant created by PoorMad. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
"@
        $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    ...
    Write-Success "Configuration directory ready: $PoorMadHome"
    
    # Seed bundled skills into $PoorMadHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $PoorMadHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $PoorMadHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$PoorMadHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $PoorMadHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (PoorMad-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'poormad setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\poormad-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\poormad-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\poormad-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `poormad tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $PoorMadHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only PoorMad' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside PoorMad' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'poormad tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `poormad
...
# paths still lazy-install via install_cua_driver() (poormad_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `poormad computer-use install` runs. Bounded
        ...
        # PoorMad' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'poormad tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: poormad computer-use install"
            }
        } else {
...
# the final `electron` -> `PoorMad` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable PoorMad.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_poormad_desktop command
    # resolves apps/desktop/release/win-unpacked/PoorMad.exe directly,
    ...
    # enforces the build floor (Node >=26) and prepends the PoorMad-managed
    ...
    # The --dir mode produces an unpacked PoorMad.exe in
    ...
    # are NOT trying to work around). The PoorMad icon + product name are
    # stamped onto PoorMad.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\poormad-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\PoorMad.exe",
        "$desktopDir\release\win-arm64-unpacked\PoorMad.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no PoorMad.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The PoorMad icon + identity are stamped onto PoorMad.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, poormad-agent#38216).
    ...
    #    PoorMad.exe. We deliberately do NOT point them at `poormad desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'PoorMad.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'PoorMad.lnk')
        )
        ...
        # with the PoorMad icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.poormad/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'poormad setup'."
        return
    }
    ...
    # Run poormad setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m poormad_cli.main setup
    } else {
        python -m poormad_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $poormadCmd = "$InstallDir\venv\Scripts\poormad.exe"
    if (-not (Test-Path $poormadCmd)) {
        $poormadCmd = "poormad"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$PoorMadHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'poormad whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: poormad gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$PoorMadHome\logs\gateway.log"
            Start-Process -FilePath $poormadCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$PoorMadHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: poormad gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: poormad gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$PoorMadHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$PoorMadHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$PoorMadHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$PoorMadHome\poormad-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   poormad              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   poormad setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   poormad config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   poormad config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   poormad gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   poormad update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning PoorMad repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (PoorMad-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding PoorMad to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://poormad.dev/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
