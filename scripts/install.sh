...
# Python-driven tool session (e.g. PoorMad terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:PoorMad/poormad-agent.git"
REPO_URL_HTTPS="https://github.com/Tayyaba22/poormad-agent.git"
POORMAD_HOME="${POORMAD_HOME:-$HOME/.poormad}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${POORMAD_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$POORMAD_INSTALL_DIR"
...
#   code at /usr/local/lib/poormad-agent, command at /usr/local/bin/poormad,
#   data still at /root/.poormad (POORMAD_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$POORMAD_HOME/.no-bundled-skills so future"
            echo "                   'poormad update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> PoorMad.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.poormad/poormad-agent"
            echo "                   default (root, Linux): /usr/local/lib/poormad-agent"
            echo "  --poormad-home PATH  Data directory (default: ~/.poormad, or \$POORMAD_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, PoorMad installs the code under"
            echo "  /usr/local/lib/poormad-agent and links the command into"
            echo "  /usr/local/bin/poormad (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$POORMAD_HOME"
            echo "  (default /root/.poormad).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$POORMAD_HOME/poormad-agent are preserved in-place."
...
print_banner() {
    ...
    echo "│             ⚕ PoorMad Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by PoorMad.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `poormad update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (PoorMad-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download PoorMad","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install poormad command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `poormad` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $POORMAD_HOME/poormad-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $POORMAD_HOME/poormad-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/poormad-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $POORMAD_HOME/poormad-agent — then preserve it)
#
# Always no-op when the user set --dir or $POORMAD_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in POORMAD_HOME.
    if is_termux; then
        INSTALL_DIR="$POORMAD_HOME/poormad-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$POORMAD_HOME/poormad-agent/.git" ]; then
            INSTALL_DIR="$POORMAD_HOME/poormad-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/poormad-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/poormad-agent"
        ...
        # /usr/local/bin/poormad wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/poormad"
        log_info "  Data:    $POORMAD_HOME (unchanged)"
...
# Point a PoorMad-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $POORMAD_HOME/node/bin — which is NOT on
...
# PoorMad's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no PoorMad-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$POORMAD_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$POORMAD_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$POORMAD_HOME/node/etc/npmrc"
}

get_poormad_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/poormad" ]; then
        echo "$link_dir/poormad"
    else
        echo "poormad"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://poormad.dev/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # PoorMad owns its own uv at $POORMAD_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (poormad_cli/managed_uv.py) looks in the same
    # place, so install.sh and `poormad update` stay in sync.
    local _managed_uv="$POORMAD_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $POORMAD_HOME/bin ..."
    mkdir -p "$POORMAD_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/poormad-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/poormad-uv-installer.$$.sh")"
    ...
    # directly into $POORMAD_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$POORMAD_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# with the PoorMad-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing PoorMad-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing PoorMad-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a PoorMad-managed Node from a previous run over a too-old system one.
    if [ -x "$POORMAD_HOME/node/bin/node" ] && [ -x "$POORMAD_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$POORMAD_HOME/node/bin/node" --version)"; then
        export PATH="$POORMAD_HOME/node/bin:$PATH"
        log_success "Node.js $("$POORMAD_HOME/node/bin/node" --version) found (PoorMad-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing PoorMad-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (PoorMad requires Node >=26) — installing PoorMad-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.poormad/node/..."
    ...
    # Place into ~/.poormad/node/ and symlink binaries into the same bin dir
    # the poormad command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$POORMAD_HOME/node"
    mkdir -p "$POORMAD_HOME"
    mv "$extracted_dir" "$POORMAD_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$POORMAD_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$POORMAD_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$POORMAD_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$POORMAD_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.poormad/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. PoorMad install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "PoorMad itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `poormad update` path
                # (#4735).
                ...
                local stash_name
                stash_name="poormad-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``poormad update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if PoorMad behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up poormad command..."

    if [ "$USE_VENV" = true ]; then
        POORMAD_BIN="$INSTALL_DIR/venv/bin/python"
        POORMAD_ENTRYPOINT="$INSTALL_DIR/poormad"
    else
        POORMAD_BIN="$(which poormad 2>/dev/null || echo "")"
        if [ -z "$POORMAD_BIN" ]; then
            log_warn "poormad not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$POORMAD_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$POORMAD_ENTRYPOINT" ]; }; then
        log_warn "PoorMad launcher prerequisites not found"
    ...
    # Create a user-facing shim for the poormad command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $POORMAD_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$POORMAD_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/poormad"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/poormad" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$POORMAD_BIN" "$POORMAD_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/poormad" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/poormad"
    log_success "Installed poormad launcher → $command_link_display_dir/poormad"

    # Also expose `poormad-agent`. The `poormad-agent` console script declared in
    ...
    rm -f "$command_link_dir/poormad-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/poormad-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$POORMAD_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/poormad-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$POORMAD_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/poormad-agent"
    log_success "Installed poormad-agent launcher → $command_link_display_dir/poormad-agent"

    # Also expose `poormad-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `poormad-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report PoorMad as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/poormad-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/poormad-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$POORMAD_BIN" "$POORMAD_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/poormad-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$POORMAD_BIN" acp "\$@"
EOF
    fi
    chmod +x "$command_link_dir/poormad-acp"
    log_success "Installed poormad-acp launcher → $command_link_display_dir/poormad-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "poormad command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v poormad' \
                >/dev/null 2>&1; then
        ...
        log_info "poormad not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# PoorMad — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# PoorMad — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# PoorMad — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so poormad works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.poormad directory structure (config at top level, code in subdir)
    mkdir -p "$POORMAD_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.poormad/.env (top level, easy to find)
    if [ ! -f "$POORMAD_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$POORMAD_HOME/.env"
            log_success "Created ~/.poormad/.env from template"
        else
            touch "$POORMAD_HOME/.env"
            log_success "Created ~/.poormad/.env"
        fi
    else
        log_info "~/.poormad/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$POORMAD_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.poormad/config.yaml (top level, easy to find)
    if [ ! -f "$POORMAD_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$POORMAD_HOME/config.yaml"
            log_success "Created ~/.poormad/config.yaml from template"
        fi
    else
        log_info "~/.poormad/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in poormad_cli/default_soul.py — the
    ...
    if [ ! -f "$POORMAD_HOME/SOUL.md" ]; then
        cat > "$POORMAD_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.poormad/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.poormad/"

    # Seed bundled skills into ~/.poormad/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `poormad update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'poormad update'." \
            > "$POORMAD_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $POORMAD_HOME/.no-bundled-skills"
        log_info "  Future 'poormad update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.poormad/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.poormad/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$POORMAD_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$POORMAD_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.poormad/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$POORMAD_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# PoorMad browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "PoorMad will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# PoorMad browser tools — explicit browser override."
...
install_node_deps() {
    ...
    # Keep the checkout clean so `poormad update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `poormad tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$POORMAD_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside PoorMad' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$POORMAD_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'poormad tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `poormad tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `poormad computer-use install` on their own).
    ...
    # Same upstream installer `poormad computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the PoorMad install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching PoorMad'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'poormad tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: poormad computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'poormad setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m poormad_cli.main setup < /dev/tty
    else
        python -m poormad_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$POORMAD_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for PoorMad to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$POORMAD_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'poormad whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                POORMAD_CMD="$(get_poormad_command_path)"
                $POORMAD_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'poormad whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'poormad gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $POORMAD_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $POORMAD_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: poormad gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: poormad gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $POORMAD_CMD gateway > "$POORMAD_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.poormad/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: poormad gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: poormad gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.poormad-bootstrap-complete, which tells the PoorMad
    ...
    local marker_path="$INSTALL_DIR/.poormad-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $POORMAD_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $POORMAD_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $POORMAD_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}poormad${NC}              Start chatting"
    echo -e "   ${GREEN}poormad setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}poormad config${NC}       View/edit configuration"
    echo -e "   ${GREEN}poormad config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}poormad gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}poormad update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'poormad' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'poormad' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'poormad' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$POORMAD_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$POORMAD_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$POORMAD_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$POORMAD_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`PoorMad` rename
...
# _purge_electron_build_cache() used by `poormad desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the PoorMad-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `poormad desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/PoorMad" ]; then
            app="$desktop_dir/release/linux-unpacked/PoorMad"
        elif [ -x "$desktop_dir/release/linux-unpacked/poormad" ]; then
            app="$desktop_dir/release/linux-unpacked/poormad"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/PoorMad.app" \
            "$desktop_dir/release/mac/PoorMad.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `poormad desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading POORMAD_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if POORMAD_HOME="$POORMAD_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from poormad_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `poormad update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the PoorMad-managed Node
            # provisioned during prerequisites/node-deps (at $POORMAD_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $POORMAD_HOME.
    # $POORMAD_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'poormad update' on this host install.
