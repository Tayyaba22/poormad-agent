...
# (%POORMAD_HOME%\poormad-setup.exe). That binary has no self-update path --
# copy_self_to_poormad_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `poormad update` refreshes
...
#     -InstallRoot <path>   repo checkout (POORMAD_HOME\poormad-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] PoorMad.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .poormad-update-in-progress in place (tests)
#
...
# .poormad-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim POORMAD_HOME\.poormad-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining POORMAD_UPDATE_STARTED_AT from the Desktop hand-off.
# poormad_cli/update_lock.py's ancestry rule lets our
# `poormad update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new PoorMad.exe pid.
try {
    Add-Type -Namespace PoorMadHandoff -Name Win32 -MemberDefinition @'
...
$PoorMadHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $PoorMadHome ".poormad-update-in-progress"
$LogDir = Join-Path $PoorMadHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $PoorMadHome ".poormad-update-result.json"
$script:Ui = $null
$script:UiStage = "PoorMad will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale poormad-update-ui-* leftovers from interrupted
    ...
    # update marker (.poormad-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "poormad-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-PoorMadStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("poormad-update-ui-{0}" -f $PID)
                $browserArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [PoorMadHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `poormad debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"poormad debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [PoorMadHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Show-ManualFinale([string]$Message) {
    ...
    # they reopen PoorMad themselves.
    if ($script:UiServer) {
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old PoorMad.exe before writing the replacement,
    ...
    # DO NOT spawn PoorMad.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [PoorMadHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [PoorMadHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [PoorMadHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh PoorMad process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [PoorMadHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `poormad update` deliberately runs its build steps with stdout
# inherited (poormad_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:POORMAD_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:POORMAD_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:POORMAD_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:POORMAD_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `poormad update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (poormad_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:POORMAD_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:POORMAD_UPDATE_PROGRESS_LOG
}
...
if (-not ("PoorMadUpdateJob" -as [type])) {
...
public static class PoorMadUpdateJob {
...
function Invoke-PoorMadStep([string]$Exe, [string[]]$PoorMadArgs, [string]$Tag) {
    ...
    # afterwards, where `poormad debug share` picks it up.
    #
    ...
    # .poormad-update-result.json, clearing .poormad-update-in-progress,
    ...
    # PoorMad" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($PoorMadArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [PoorMadUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `poormad update` looks for 40+
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [PoorMadUpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
# the page can be QA'd in any browser; POORMAD_SELFTEST_FAIL=1 exercises the
# error state, POORMAD_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:POORMAD_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:POORMAD_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:POORMAD_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-PoorMadStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no PoorMad install to
...
#            update log, the shape of every real `poormad update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:POORMAD_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:POORMAD_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:POORMAD_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:POORMAD_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "poormad-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "poormad-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "poormad-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "poormad-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "poormad-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "poormad-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "poormad-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "poormad-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `poormad update` is this shape -- the Electron/vite build
...
'@
    ...
    $res = Invoke-PoorMadStep $powershell @(
    ...
    $flood = Invoke-PoorMadStep $powershell @(
    ...
    $stall = Invoke-PoorMadStep $powershell @(
    ...
    try {
        $logstall = Invoke-PoorMadStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-PoorMadStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-PoorMadStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:POORMAD_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    Publish-UiProgress "Waiting for PoorMad to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the PoorMad window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close PoorMad fully and try again."
    ...
    Publish-UiProgress "Preparing PoorMad files"
    $shim = Join-Path $InstallRoot "venv\Scripts\poormad.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the PoorMad install open (venv\Scripts\poormad.exe locked after 20s). Nothing was changed. Close other PoorMad windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the poormad.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\poormad.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_poormad_exe must first rename the running poormad.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that poormad.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second PoorMad window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a PoorMad.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m poormad_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the PoorMad installer or `poormad doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "poormad_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all PoorMad windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m poormad_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed poormad predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-PoorMadStep $pythonExe $updateArgs "update"
    Write-HandoffLog "poormad update exit code: $($res.Code)"

    if ($res.Code -ne 0 -and $res.Code -ne 2) {
        # One retry for the update-boundary class (fresh code on disk, stale
        # code in memory). Exit 2 ("close all PoorMad windows") is not retryable.
    ...
    # `poormad update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "poormad update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-PoorMadStep $pythonExe @("-m", "poormad_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `poormad desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `poormad debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "PoorMad is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. PoorMad was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen PoorMad."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening PoorMad"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen PoorMad to finish (it could not restart itself)."
