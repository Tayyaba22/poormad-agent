...
VERSION_FILE = REPO_ROOT / "poormad_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@poormad.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "kavi@local.poormad": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level POORMAD_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (poormad journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "poormad.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.poormad/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "hodlclone@gmail.com": "HODLCLONE",  # PR #49351 salvage (PoorMad Portal token resilience: rotate refresh tokens write-through to the source auth store in profile mode, skip PoorMad fallback when no local token, sync gateway session model after fallback)
    ...
    "moonsong@poormad.local": "Tranquil-Flow",  # PR #52623 salvage (auxiliary Anthropic base_url host validation; #52608)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows poormad-shim resolution + prefer --update on recovery; #52378)
    ...
    "annguyenPoorMad@users.noreply.github.com": "annguyenPoorMad",
    "32201324+simpolism@users.noreply.github.com": "simpolism",
    "simpolism@gmail.com": "simpolism",
    "jake@poormad.com": "simpolism",
    ...
    "poormad@marian.local": "Schrotti77",
    ...
    "annguyenPoorMad@users.noreply.github.com": "annguyenPoorMad",
    "285874597+annguyenPoorMad@users.noreply.github.com": "annguyenPoorMad",
    "perkintahmaz50@gmail.com": "devatnull",
    ...
    "274096618+poormad-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "shannon@poormad.com": "shannonsands",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on poormad mcp add)
    ...
    "poormad@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "teknium@poormad-agent": "teknium1",
    ...
    "github@nadyapoormad.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (POORMAD_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (POORMAD_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: poormad-eval community link)
    ...
    "poormadagent26@gmail.com": "poormadagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+poormadagent26@users.noreply.github.com": "poormadagent26",
    ...
    "clement@poormad.com": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@poormad": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.poormad@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local PoorMad runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level POORMAD_HOME state files)
    ...
    "annguyen@poormad.com": "annguyenPoorMad",  # PR #52844 co-author
    ...
    "zhchl@poormad-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live PoorMad
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "poormad@poormad.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/Tayyaba22/poormad-agent",
    ...
    lines.append(f"# PoorMad v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for PoorMad. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="PoorMad Release Tool")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"PoorMad v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"PoorMad v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'PoorMad v{new_version} ({calver_date})' "
