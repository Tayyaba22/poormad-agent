#!/usr/bin/env python3
"""Boot-time re-seed of a terminally-dead PoorMad bootstrap session.

Background
----------
A PoorMad bootstrap session (client_id ``poormad-cli-vps``) can take a terminal
...
``stage2-hook.sh`` seeds ``auth.json`` from ``POORMAD_AUTH_JSON_BOOTSTRAP`` only
...
``POORMAD_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the PoorMad
...
Design constraints
------------------
- Pure stdlib, no poormad_cli imports: runs early in the boot hook, before the
...
# from POORMAD_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two
# paths can never be confused: BOOTSTRAP seeds a fresh volume; REBOOTSTRAP
# overwrites a terminally-dead PoorMad entry on an existing volume.
REBOOTSTRAP_ENV = "POORMAD_AUTH_JSON_REBOOTSTRAP"
BOOTSTRAP_CLIENT_ID = "poormad-cli-vps"


def _nous_entry_is_terminal(nous_state: Any) -> bool:
    """True iff the on-disk PoorMad provider entry is in the terminal/quarantined
    state AND holds no usable credential.

    Mirrors the ``terminal`` predicate in ``poormad_cli.auth.get_nous_session_validity``:
...
def _extract_nous_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.nous`` block out of a POORMAD_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    POORMAD_AUTH_JSON_BOOTSTRAP). Returns None unless it carries the expected VPS
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             POORMAD_AUTH_JSON_BOOTSTRAP path handle it)
...
def main() -> int:
    ...
    if result == "reseeded":
        print("[rebootstrap] PoorMad bootstrap session was terminal; re-seeded auth.json from "
              f"{REBOOTSTRAP_ENV}")
    elif result == "reseeded_newer":
        print("[rebootstrap] Applied newer orchestrator-issued PoorMad bootstrap session from "
              f"{REBOOTSTRAP_ENV}")
    else:
