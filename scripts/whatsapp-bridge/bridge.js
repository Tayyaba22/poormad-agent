 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.poormad/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.poormad', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (POORMAD_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.POORMAD_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.poormad', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.POORMAD_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.poormad', 'document_cache');
const AUDIO_CACHE_DIR = process.env.POORMAD_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.poormad', 'audio_cache');
...
// `poormad update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *PoorMad*\n────────────\n';
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls PoorMad itself created (tracked when
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into PoorMad and trigger
      ...
      // Ignore PoorMad' own reply messages in self-chat mode to avoid loops.
    ...
    switch (type) {
      case 'image':
        if (ext === 'gif') {
          ...
          let tmpGifMp4 = null;
          try {
            tmpGifMp4 = path.join(tmpdir(), `poormad_gif_${randomBytes(6).toString('hex')}.mp4`);
            execFileSync(
              'ffmpeg',
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `poormad_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execFileSync(
              'ffmpeg',
