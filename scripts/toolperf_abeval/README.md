...
batch (tracker: [#77056](https://github.com/Tayyaba22/poormad-agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  PoorMad home, same model, same tasks, same reps.
...
1. Create a dedicated PoorMad home with credentials for the models under test:
   ...
   integration at it; no PoorMad observability plugin needs to be enabled.
