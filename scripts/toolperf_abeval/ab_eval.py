...
measured in the production session DB) through `poormad chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    POORMAD_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured PoorMad home with credentials for the
...
(tracker: PoorMad/poormad-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "POORMAD_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "poormad_cli.main", "chat", "--query", q,
