#!/usr/bin/env python3
"""Live test harness for PoorMad's Tool Search feature.
...
# Force-isolate the test environment BEFORE any poormad imports.
ORIGINAL_HOME = os.environ.get("POORMAD_HOME")
ORIGINAL_AUTH = Path.home() / ".poormad" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.poormad/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.poormad/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="poormad_ts_live_"))
    poormad_home = home_dir / ".poormad"
    poormad_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, poormad_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".poormad" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, poormad_home / ".env")
        ...
        load_poormad_dotenv(poormad_home=str(Path.home() / ".poormad"))
    ...
    (poormad_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return poormad_home
...
def reset_module_state():
    """Drop cached modules so the new POORMAD_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "poormad_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["POORMAD_HOME"] = str(home)
...
def main():
    ...
    # Restore original POORMAD_HOME
    if ORIGINAL_HOME is not None:
        os.environ["POORMAD_HOME"] = ORIGINAL_HOME
    else:
