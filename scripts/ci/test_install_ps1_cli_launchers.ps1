# Behavioral test for install.ps1's poormad launcher staging (PR #92092,
...
#
# The test lifts the real Install-PoorMadCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $PoorMadHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-PoorMadCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-PoorMadCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'poormad-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'poormad-agent'
    ...
    $missingThrew = $false
    try {
        Install-PoorMadCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing poormad.exe fails the launcher stage'
    ...
    $poormadV1 = [byte[]](77, 90, 1)
    $poormadV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'poormad.exe'), $poormadV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-PoorMadCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $poormadV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'poormad.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'poormad-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'poormad.exe'), $poormadV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'poormad-acp.exe'), $acp)
    Install-PoorMadCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $poormadV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'poormad.exe'))) `
        'installer refreshes an existing PoorMad launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'poormad-acp.exe'))) `
    ...
    Install-PoorMadCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'poormad.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'poormad.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'poormad.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'poormad.exe')) -and $cmdBody.Contains('%*')) `
