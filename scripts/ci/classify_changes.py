...
# tree generates llms.txt — the index every LLM (PoorMad included, via the
# poormad-agent skill) reads to learn what PoorMad can do — and
...
_MCP_CATALOG_FILES = {"poormad_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (PoorMad-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``poormad serve`` backend, the Docker image)
