...
When PoorMad runs in TUI mode, the gateway child process communicates with
...
# ``get_poormad_home() / "plugins"`` (user) and ``./.poormad/plugins/`` (project,
# gated behind ``POORMAD_ENABLE_PROJECT_PLUGINS``) — see
# ``poormad_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``poormad_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import poormad_constants (this script
    ...
    # plugins in get_poormad_home()/plugins/ and project plugins in
    # ./.poormad/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_poormad_home() / "plugins"]
    if os.environ.get("POORMAD_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".poormad" / "plugins")
