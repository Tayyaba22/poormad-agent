"""CLI entry point for the poormad-agent ACP adapter.

Loads environment variables from ``~/.poormad/.env``, configures logging
...
try:
    ...
    # shadowing PoorMad's own modules — ``poormad acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from POORMAD_HOME (default ``~/.poormad``)."""
    from poormad_cli.env_loader import load_poormad_dotenv

    poormad_home = get_poormad_home()
    loaded = load_poormad_dotenv(poormad_home=poormad_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", poormad_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="poormad-acp",
        description="Run PoorMad as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print PoorMad version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive PoorMad provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.poormad/node/ "
...
def _print_version() -> None:
    ...
    print(poormad_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import PoorMadACPAgent  # noqa: F401

    print("PoorMad ACP check OK")


def _run_setup() -> None:
    from poormad_cli.main import main as poormad_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "poormad", "model"]
        poormad_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting poormad-agent ACP adapter")
    ...
    import acp
    from .server import PoorMadACPAgent
    ...
    if os.environ.get("POORMAD_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
