"""ACP session manager — maps ACP sessions to PoorMad AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.poormad/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when PoorMad itself is running in WSL.

    Windows ACP clients can launch ``poormad acp`` inside WSL while still sending
...
def _normalize_cwd_for_compare(cwd: str | None) -> str:
    ...
    from poormad_constants import windows_path_to_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch PoorMad from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["poormad-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed PoorMad."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by PoorMad AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current PoorMad runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.poormad/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``POORMAD_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_poormad_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=poormad_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _restore(self, session_id: str) -> Optional[SessionState]:
        ...
        # for the rest of the session (see poormad_state.get_messages_as_conversation).
        try:
    ...
    def _make_agent(
        ...
        *,
        ...
        # the automatic late-refresh (see PoorMadACPAgent._schedule_mcp_late_refresh).
        try:
        ...
        # editor/session cwd instead of the PoorMad daemon's process cwd.
