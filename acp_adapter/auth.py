"""ACP auth helpers — detect and advertise PoorMad authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "poormad-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active PoorMad runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if PoorMad can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for PoorMad.
    ...
    not have PoorMad provider credentials configured yet, so PoorMad always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate PoorMad using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure PoorMad provider",
            description=(
                "Open PoorMad' interactive model/provider setup in a terminal. "
                "Use this when PoorMad has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
