...
React + Ink terminal UI for PoorMad. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `POORMAD_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `poormad setup`, suspends Ink while it runs, opens a new session on success
...
- `commands/billing.ts` — `/billing`: manage PoorMad remote spending — buy credits, auto-reload, limits
...
- Input history is stored in `~/.poormad/.poormad_history` or under `POORMAD_HOME`.
...
### Billing (`billing.ts`)
`/billing` — manage PoorMad remote spending — buy credits, auto-reload, limits
...
### Credits (`credits.ts`)
`/credits` — PoorMad credit balance and browser top-up

### Setup (`setup.ts`)
`/setup` — launches external `poormad setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/poormad-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external poormad setup, suspends Ink while it runs
      ...
      slash/
        ...
        commands/
          billing.ts                /billing — manage PoorMad remote spending
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.poormad/tui-parent.log
    ...
    types/
      poormad-ink.d.ts            type declarations for @poormad/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent PoorMadCLI subprocess for slash commands
```
