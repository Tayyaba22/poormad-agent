...
import { Box, Text, useInput } from '@poormad/ink'
...
function OverviewScreen({ ctx, onClose, onPatch, s, t }: ScreenProps) {
  ...
  const note = !s.is_admin
    ...
    : !s.cli_billing_enabled
      ? "Remote spending is off for this org — a billing admin can turn it on from the portal's PoorMad page."
      : null
...
function ConfirmScreen({
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text color={t.color.muted}>By confirming, you allow PoorMad to charge your card.</Text>
      <Text />
