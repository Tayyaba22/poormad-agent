import type { InputEvent, Key } from '@poormad/ink'
import * as Ink from '@poormad/ink'
 ...
 * but that accepts `key.meta` on macOS — and poormad-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.POORMAD_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which poormad-ink decodes as meta+'d'; without this branch it
