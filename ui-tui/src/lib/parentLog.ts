...
const logDir = join(process.env.POORMAD_HOME?.trim() || join(homedir(), '.poormad'), 'logs')
...
// a real ~/.poormad (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('poormad-tui: parent lifecycle log unavailable\n')
    }
  }
}
