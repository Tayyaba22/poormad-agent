...
// Deferred @poormad/ink import: loading `@poormad/ink` at module top-level
...
async function _ensureEvictInkCaches(): Promise<(level: 'all' | 'half') => unknown> {
  ...
  _evictInkCachesPromise ??= import('@poormad/ink')
    .then(mod => {
...
export function startMemoryMonitor({
  ...
  const cooldownRaw = process.env.POORMAD_AUTO_HEAPDUMP_COOLDOWN_MS?.trim()
  ...
  const tick = async () => {
    ...
    // Deferred import keeps `@poormad/ink` off the cold-start critical path;
