 ...
 * `poormad-boot-background` / `poormad-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports POORMAD_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.poormad matches get_poormad_home()'s default.
const bootFilePath = () => join(process.env.POORMAD_HOME ?? join(homedir(), '.poormad'), 'tui-theme-boot.json')

// Never touch the user's real ~/.poormad from test runs (the TS suite has no
// POORMAD_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into POORMAD_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.POORMAD_TUI_THEME || env.POORMAD_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.POORMAD_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.POORMAD_TUI_BACKGROUND
  ) {
    env.POORMAD_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.POORMAD_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.POORMAD_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into POORMAD_TUI_THEME.
