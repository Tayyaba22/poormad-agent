...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.POORMAD_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep PoorMad on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via POORMAD_TUI_TRUECOLOR=1.
