...
import { terminalBackgroundHex } from '@poormad/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $POORMAD_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see POORMAD_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['POORMAD_TUI_BACKGROUND', process.env.POORMAD_TUI_BACKGROUND ?? '(unset)'],
            ['POORMAD_TUI_THEME', process.env.POORMAD_TUI_THEME ?? '(unset)'],
