import { driveChargeSettlement, type SettlementOutcome } from '@poormad/shared/charge-settlement'
...
const renderBillingError = (
  ...
  switch (env.error) {
    ...
    case 'remote_spending_disabled':
      ...
      // turn it on from the portal's PoorMad page; this is NOT a per-terminal stop.
      sys("Remote spending is off for this account — a billing admin can turn it on from the portal's PoorMad page.")
      ...
      ctx.gateway
        ...
        .then(
          ctx.guarded<BillingStateResponse>(s => {
            if (!s.logged_in) {
              sys('💳 Not logged into PoorMad Portal — run /portal to log in, then /topup.')
