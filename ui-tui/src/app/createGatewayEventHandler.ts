...
import { forceRedraw, onTerminalBackground, onTerminalForeground } from '@poormad/ink'
...
const commitTheme = (theme: Theme) => {
  ...
  const pin = configPinnedTheme ? process.env.POORMAD_TUI_THEME : undefined

  writeBootTheme(theme, process.env.POORMAD_TUI_BACKGROUND, pin === 'light' || pin === 'dark' ? pin : undefined)
 ...
 * to POORMAD_TUI_THEME — the priority-2 signal `detectLightMode` already
 * honors (only an explicit POORMAD_TUI_LIGHT env var outranks it); 'auto'
 ...
 */
// True once CONFIG (via light/dark) owns the POORMAD_TUI_THEME env pin, so an
...
export function applyConfiguredTuiTheme(raw: unknown): void {
  ...
  const current = process.env.POORMAD_TUI_THEME ?? ''
  ...
  if (mode === 'light' || mode === 'dark') {
    ...
    process.env.POORMAD_TUI_THEME = mode
  } else {
    // 'auto' clears only a pin CONFIG set — never a POORMAD_TUI_THEME the user
    ...
    delete process.env.POORMAD_TUI_THEME
  }
 ...
 * is cached into POORMAD_TUI_BACKGROUND — the slot `detectLightMode` already
 ...
 * POORMAD_TUI_LIGHT / POORMAD_TUI_THEME overrides still win inside
...
export function syncThemeToTerminalBackground(): void {
  ...
  onTerminalBackground(hex => {
    ...
    resolved = true
    process.env.POORMAD_TUI_BACKGROUND = hex
    reapplyTheme()
  })
  ...
  onTerminalForeground(hex => {
    if (resolved || process.env.POORMAD_TUI_THEME || process.env.POORMAD_TUI_LIGHT) {
      return
    }
    ...
    resolved = true
    process.env.POORMAD_TUI_BACKGROUND = inferred
    reapplyTheme()
  })
  ...
  setTimeout(() => {
    if (
      ...
      process.env.POORMAD_TUI_BACKGROUND ||
      process.env.POORMAD_TUI_THEME ||
      process.env.POORMAD_TUI_LIGHT ||
    ...
    execFile('defaults', ['read', '-g', 'AppleInterfaceStyle'], (error, stdout) => {
      if (resolved || process.env.POORMAD_TUI_BACKGROUND || process.env.POORMAD_TUI_THEME) {
        return
      }
      ...
      // inference).
      resolved = true
      process.env.POORMAD_TUI_BACKGROUND = dark ? '#1e1e1e' : '#ffffff'
      reapplyTheme()
    })
  }, 1500).unref?.()
}
...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: GatewayEvent) => void {
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // Arm "Hey PoorMad" if this surface owns it (server gates on config).
    ...
    // forging a brand-new one.  Mirrors classic CLI's `poormad -c` /
    // `poormad --tui` muscle memory and addresses the audit's "session
  ...
  return (ev: GatewayEvent) => {
    ...
    switch (ev.type) {
      ...
      case 'wake.detected': {
        // "Hey PoorMad": optionally open a fresh session (start_new_session),
        ...
        void (async () => {
          ...
          if (wakeProfile && wakeProfile !== ownProfile) {
            sys(`wake phrase for profile '${wakeProfile}' — run: poormad -p ${wakeProfile} --tui`)
