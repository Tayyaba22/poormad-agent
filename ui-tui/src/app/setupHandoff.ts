import type { RunExternalProcess } from '@poormad/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`poormad ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching poormad: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`poormad ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
