...
import { Box, Text } from '@poormad/ink'
...
/**
 * User widget apps — PoorMad authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$POORMAD_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.poormad/plugins/`: files under POORMAD_HOME execute
...
const widgetsDir = () => join(process.env.POORMAD_HOME?.trim() || join(homedir(), '.poormad'), 'tui-widgets')
 ...
 *  every change, so a widget PoorMad writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or PoorMad) ever writes must hot-load too — a 10s poll here read
