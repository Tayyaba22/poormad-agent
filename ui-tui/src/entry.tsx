...
import type { FrameEvent } from '@poormad/ink'
...
if (!process.stdin.isTTY) {
  console.log('poormad-tui: no TTY')
  process.exit(0)
}
...
const dumpNotice = (snap: MemorySnapshot, dump: HeapDumpResult | null) =>
  `poormad-tui: ${snap.level} memory (${formatBytes(snap.heapUsed)}) — auto heap dump → ${dump?.heapPath ?? dump?.diagPath ?? '(failed)'}\n`
...
setupGracefulExit({
  ...
  onError: (scope, err) => {
    ...
    try {
      process.stderr.write(`poormad-tui lifecycle ${scope}: ${message.slice(0, 2000)}\n`)
    } catch {
  ...
  onSignal: signal => {
    ...
    resetTerminalModes()
    process.stderr.write(`poormad-tui lifecycle: received ${signal}\n`)
  },
...
const stopMemoryMonitor = startMemoryMonitor({
  onCritical: (snap, dump) => {
    ...
    process.stderr.write(
      `poormad-tui lifecycle: memory critical exit heap=${formatBytes(snap.heapUsed)} rss=${formatBytes(snap.rss)}\n`
    )
    process.stderr.write(dumpNotice(snap, dump))
    process.stderr.write('poormad-tui: exiting to avoid OOM; restart to recover\n')
    process.exit(137)
  },
  ...
  onWarn: snap => {
    ...
    process.stderr.write(
      `poormad-tui: heap climbing fast (${formatBytes(snap.heapUsed)}) — a large tool output or long session may be straining memory\n`
    )
  }
})

if (process.env.POORMAD_HEAPDUMP_ON_START === '1') {
