...
The canonical transcript store is SQLite via `SessionDB` (from `poormad_state`). The
...
Counts consecutive restarts via a JSON file (`{POORMAD_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `poormad update`, `poormad gateway restart`,
or `/restart`.
