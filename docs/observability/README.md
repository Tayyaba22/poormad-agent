# PoorMad Observer Hooks

PoorMad observer hooks are the read-only telemetry contract for plugins that
...
They do not replace PoorMad' planner, model providers, memory, tool registry,
...
PoorMad also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "poormad.observer.v1"
```

Hook callbacks are fail-open. PoorMad catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so PoorMad only builds
...
The native NeMo Relay SDK integration maps PoorMad session, turn, LLM, and tool
