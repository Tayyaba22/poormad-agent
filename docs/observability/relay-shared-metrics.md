...
PoorMad includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into PoorMad and does not require a PoorMad observability plugin. PoorMad remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
PoorMad execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `poormad-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the PoorMad `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `POORMAD_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `POORMAD_NEMO_RELAY_ATOF_*` and `POORMAD_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, PoorMad does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

PoorMad requires NeMo Relay 0.7.1 or later within the 0.7 release line. That
...
PoorMad installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the PoorMad process so configured interceptors can
...
Collection remains off unless PoorMad policy enables it:
...
opt-in. Set `POORMAD_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, PoorMad does not invoke Relay's plugin initializer, so Relay
...
from the selected file only. If the selected file cannot be loaded, PoorMad
...
retain the same `session_id` and add `poormad.session.segment` plus
`poormad.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a PoorMad
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that PoorMad process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, PoorMad logs:

```text
Relay plugins are active process-wide and apply to all profiles hosted by this PoorMad process.
```
...
profile's Relay scope stack. Native dynamic plugins are loaded into the PoorMad
process and share the same policy boundary.

Run profiles in separate PoorMad processes when they require different trust
...
PoorMad core owns one Relay host and one isolated Relay session scope per PoorMad
session. Core lifecycle producers use
`poormad_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require PoorMad plugin registration. Shared-metrics marks must
...
```text
PoorMad turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> PoorMad shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

PoorMad sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that PoorMad used for the logical call, such as
...
New calls use `poormad.model_route.count`. The previous
`poormad.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `poormad.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; PoorMad
version, OS family, architecture, and install method remain bounded package
resources. Concurrent PoorMad processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `poormad.task_run`, parented to
the owning PoorMad session. The start counter contains only bounded execution
...
provider attempts for the same PoorMad API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their PoorMad tool-call
...
cancellations. Active task ownership follows the task ID if PoorMad rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`poormad.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. PoorMad
...
tool or plugin names. PoorMad does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `poormad.tool_approval` marks and
...
Successful skill mutations emit `poormad.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `poormad.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. PoorMad derives reuse and patch-generation
...
```text
$POORMAD_HOME/telemetry/shared_metrics/metrics.sqlite3
$POORMAD_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the PoorMad version,
...
schema field name, its current scope is one `POORMAD_HOME`, so it is more
...
Deleting `$POORMAD_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `POORMAD_HOME`. To reset it, stop PoorMad
processes and remove `$POORMAD_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real PoorMad CLI turn against the deterministic local model server:
