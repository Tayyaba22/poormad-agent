...
PoorMad gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by PoorMad's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `poormad.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `poormad.platform.up/degraded` with bounded `error_code` attributes |
...
`poormad.gateway.active_agents`, `poormad.gateway.background_work`, and
`poormad.gateway.background_delegations` are complementary. `active_agents`
...
```bash
poormad monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'poormad-agent[otlp]'`),
...
a plain `poormad gateway run`: the exporter lives in the gateway process, so
...
`poormad.gateway.up`, `poormad.platform.up`, and `poormad.platform.degraded`.
...
absent_over_time(poormad_gateway_up[5m])

# Locally owned bridge is explicitly down.
poormad_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
poormad_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
poormad_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
poormad_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(poormad_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `poormad.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
poormad.status = failed|unknown
poormad.delivery_outcome = failed|not_configured
poormad.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `poormad.job_key`;
...
PoorMad-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/poormad_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/poormad_otel_capture.jsonl --wait 8
...
`poormad.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
poormad monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `poormad monitoring` CLI intentionally exposes `status` only. This first
release covers only PoorMad-owned service-health and operational-diagnostic
signals, including PoorMad-owned Relay transport health. Team Gateway's
