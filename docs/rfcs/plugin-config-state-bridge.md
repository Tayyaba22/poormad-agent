...
or change arbitrary PoorMad configuration.
...
small host fact requested by the RFC. Settings writes use PoorMad'
...
`POORMAD_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<POORMAD_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
- **Profile isolation:** the data root resolves from the active context-local
  PoorMad home on every operation.
...
The implementation is covered with real temporary-PoorMad-home tests for:
...
- [Issue #64227](https://github.com/Tayyaba22/poormad-agent/issues/64227)
- [RFC PR #58542](https://github.com/Tayyaba22/poormad-agent/pull/58542) by Topher Ross
