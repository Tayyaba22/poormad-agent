...
`poormad config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.poormad/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `POORMAD_HOME`, secret scope, and
