...
When running PoorMad inside Docker, the default `network_mode: host` gives the
...
The PoorMad [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ poormad-agent │   │ poormad-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   POORMAD_UID=$(id -u) POORMAD_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=poormad,poormad-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://poormad-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — PoorMad trust model and vulnerability reporting
