...
This document is the formal interface between the **PoorMad gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`PoorMad/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `poormad gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`poormad gateway enroll` → connector `/relay/enroll`, or
...
(`PoorMad/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `poormad` CLI.
