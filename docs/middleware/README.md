...
PoorMad middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing PoorMad'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before PoorMad calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving PoorMad retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving PoorMad guardrails,
...
- `telemetry_schema_version`: currently `poormad.observer.v1`
- `middleware_schema_version`: currently `poormad.middleware.v1`
...
PoorMad stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, PoorMad runs
them as a nested chain in registration order. Middleware failures are fail-open:
PoorMad logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, PoorMad applies middleware in this order:
...
For each tool call, PoorMad applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal PoorMad pre-execution path against the effective arguments:
...
For isolated local testing, use one `POORMAD_HOME` for plugin enablement and the
agent run:

```bash
export POORMAD_HOME=/tmp/poormad-middleware-test
mkdir -p "$POORMAD_HOME"
poormad plugins enable <plugin-name>
poormad chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run poormad plugins enable <plugin-name>
uv run poormad chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["poormad_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/poormad-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape PoorMad expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, PoorMad treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, PoorMad preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. PoorMad does not convert downstream
