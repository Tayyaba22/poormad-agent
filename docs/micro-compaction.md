...
has to be thrown away or summarized. PoorMad has always done this in one batch:
...
PoorMad folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. PoorMad's normal session flush is
...
With `micro_compact` unset or `false` PoorMad behaves exactly as it always has:
...
Defaults to `$POORMAD_HOME/logs/agent.log`. It reports passes, outcome mix, net
