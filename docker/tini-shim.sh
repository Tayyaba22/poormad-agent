...
# PoorMad WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: POORMAD_TINI_SHIM_TARGET overrides the `/init` path (and
# POORMAD_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${POORMAD_TINI_SHIM_TARGET:-/init}"
WRAPPER="${POORMAD_TINI_SHIM_WRAPPER:-/opt/poormad/docker/main-wrapper.sh}"
