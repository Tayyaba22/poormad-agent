...
#
# Wired into the image as /etc/cont-init.d/01-poormad-setup by the
...
POORMAD_HOME="${POORMAD_HOME:-/opt/data}"
INSTALL_DIR="/opt/poormad"

# Drop to poormad via s6-setuidgid, but skip it when already non-root.
as_poormad() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid poormad "$@"; }
...
# starts non-root. The baked install tree under /opt/poormad is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $POORMAD_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass POORMAD_UID/POORMAD_GID — or the PUID/PGID aliases — which the
...
# `id -u poormad` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# poormad UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u poormad)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-poormad UID).
...
and the baked /opt/poormad install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e POORMAD_UID=\$(id -u) -e POORMAD_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the poormad user to that UID/GID at boot and chowns the data
...
# --- Bootstrap POORMAD_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid poormad mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom POORMAD_HOME paths whose parents only
# root can create (e.g. `POORMAD_HOME=/home/poormad/.poormad` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for POORMAD_UID/POORMAD_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  POORMAD_UID/
# POORMAD_GID still win when both are set.  See #15290, salvages #25872.
POORMAD_UID="${POORMAD_UID:-${PUID:-}}"
POORMAD_GID="${POORMAD_GID:-${PGID:-}}"

if [ -n "${POORMAD_UID:-}" ] && validate_uid_gid "$POORMAD_UID" && [ "$POORMAD_UID" != "$(id -u poormad)" ]; then
    echo "[stage2] Changing poormad UID to $POORMAD_UID"
    usermod -u "$POORMAD_UID" poormad
fi
if [ -n "${POORMAD_GID:-}" ] && validate_uid_gid "$POORMAD_GID" && [ "$POORMAD_GID" != "$(id -g poormad)" ]; then
    echo "[stage2] Changing poormad GID to $POORMAD_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$POORMAD_GID" poormad 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised poormad user (UID 10000)
...
# poormad process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes poormad. Idempotent across container
...
#   - poormad is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G poormad 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] poormad already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" poormad 2>/dev/null; then
        echo "[stage2] Added poormad to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group poormad failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When POORMAD_UID is remapped or the top-level $POORMAD_HOME isn't owned by
# the runtime poormad UID, restore ownership to poormad — but ONLY for the
# directories poormad actually writes to. The full $POORMAD_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of poormad-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_poormad_uid=$(id -u poormad)
...
chown_poormad_tree() {
    ...
    chown -R poormad:poormad "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_poormad_owner() {
    target="$1"
    find "$target" \( ! -user poormad -o ! -group poormad \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$POORMAD_HOME" 2>/dev/null)" != "$actual_poormad_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $POORMAD_HOME (targeted) to poormad ($actual_poormad_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $POORMAD_HOME: chown the directory itself (not its contents)
    # so poormad can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$POORMAD_HOME"; then
        :
    else
        chown poormad:poormad "$POORMAD_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $POORMAD_HOME failed (rootless container?) — continuing"
    fi
    # PoorMad-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by poormad (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$POORMAD_HOME/$sub" ] && tree_has_non_poormad_owner "$POORMAD_HOME/$sub"; then
            chown_poormad_tree "$POORMAD_HOME/$sub"
        fi
    done
fi
...
# poormad user. Hosted/container instances keep mutable state under
# $POORMAD_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# POORMAD_DISABLE_LAZY_INSTALLS=1. Keeping /opt/poormad root-owned and
...
# $POORMAD_HOME/lazy-packages dir on the data volume (Dockerfile sets
# POORMAD_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to poormad
...
# Always reset ownership of $POORMAD_HOME/profiles to poormad on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> poormad …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as poormad and walks
...
if [ -d "$POORMAD_HOME/profiles" ] && tree_has_non_poormad_owner "$POORMAD_HOME/profiles"; then
    chown_poormad_tree "$POORMAD_HOME/profiles"
fi

# Always reset ownership of $POORMAD_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged poormad runtime even
...
# gate as profiles/).
if [ -d "$POORMAD_HOME/cron" ] && tree_has_non_poormad_owner "$POORMAD_HOME/cron"; then
    chown_poormad_tree "$POORMAD_HOME/cron"
fi

# Always ensure logs/gateways is poormad-owned (#45258). Formerly healed by
...
# top-level $POORMAD_HOME is mis-owned, so a warm volume with poormad-owned
# POORMAD_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid poormad mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as poormad.
if [ -d "$POORMAD_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$POORMAD_HOME/logs/gateways"; then
        :
    else
        chown poormad:poormad "$POORMAD_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# poormad pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged poormad gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $POORMAD_HOME is
...
# and cron/.
if [ -d "$POORMAD_HOME/platforms/pairing" ] && tree_has_non_poormad_owner "$POORMAD_HOME/platforms/pairing"; then
    chown_poormad_tree "$POORMAD_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$POORMAD_HOME/pairing" ] && tree_has_non_poormad_owner "$POORMAD_HOME/pairing"; then
    chown_poormad_tree "$POORMAD_HOME/pairing"
fi

# Reset ownership of poormad-owned top-level state files on every boot.
# The targeted data-volume chown above only covers poormad-owned
# *subdirectories*; loose state files living directly under $POORMAD_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> poormad …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged poormad runtime then hits
...
# sweep so host-owned files in a bind-mounted $POORMAD_HOME are never
...
# entries of poormad_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$POORMAD_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$POORMAD_HOME/$f"; then
            :
        else
            chown poormad:poormad "$POORMAD_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the poormad runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$POORMAD_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$POORMAD_HOME/config.yaml"; then
        :
    else
        chown poormad:poormad "$POORMAD_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$POORMAD_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as poormad user ---
# Run as poormad via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $POORMAD_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_poormad mkdir -p \
    "$POORMAD_HOME/backups" \
    "$POORMAD_HOME/cron" \
    "$POORMAD_HOME/sessions" \
    "$POORMAD_HOME/logs" \
    "$POORMAD_HOME/logs/gateways" \
    "$POORMAD_HOME/hooks" \
    "$POORMAD_HOME/memories" \
    "$POORMAD_HOME/skills" \
    "$POORMAD_HOME/skins" \
    "$POORMAD_HOME/plans" \
    "$POORMAD_HOME/workspace" \
    "$POORMAD_HOME/home" \
    "$POORMAD_HOME/pairing" \
    "$POORMAD_HOME/platforms/pairing" \
    "$POORMAD_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/poormad/.install_method (see Dockerfile), NOT written here into
# $POORMAD_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $POORMAD_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.poormad:/opt/data) and sometimes shared with a
...
# 'poormad update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $POORMAD_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/poormad stamp, so deleting the data-dir copy is safe).
if [ -f "$POORMAD_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$POORMAD_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$POORMAD_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$POORMAD_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$POORMAD_HOME/$dest"; then
            :
        else
            as_poormad cp "$INSTALL_DIR/$src" "$POORMAD_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/poormad/.env.example being present
...
# not generate one. PoorMad loads $POORMAD_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$POORMAD_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$POORMAD_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $POORMAD_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$POORMAD_HOME/.env" ] && ! refuse_symlinked_path "clean" "$POORMAD_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$POORMAD_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$POORMAD_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$POORMAD_HOME/.env"; then
        :
    else
        if [ ! -f "$POORMAD_HOME/.env" ]; then
            ...
            (umask 077 && as_poormad touch "$POORMAD_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$POORMAD_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$POORMAD_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$POORMAD_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $POORMAD_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $POORMAD_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$POORMAD_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$POORMAD_HOME/.env"; then
        :
    else
        chown poormad:poormad "$POORMAD_HOME/.env" 2>/dev/null || true
        chmod 600 "$POORMAD_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `poormad` user — without this it gets EACCES on every
...
# poormad group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp poormad /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted poormad group access to the Fly Machines API socket"
    else
...
# $POORMAD_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `poormad update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set POORMAD_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$POORMAD_HOME/config.yaml" ]; then
    s6-setuidgid poormad "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$POORMAD_HOME/auth.json" ] && [ -n "${POORMAD_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$POORMAD_HOME/auth.json"; then
        :
    else
        printf '%s' "$POORMAD_AUTH_JSON_BOOTSTRAP" > "$POORMAD_HOME/auth.json"
        chown poormad:poormad "$POORMAD_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$POORMAD_HOME/auth.json"
    fi
fi

# auth.json: re-seed a TERMINALLY-DEAD PoorMad bootstrap session (self-heal).
#
# The [ ! -f ] guard above deliberately refuses to clobber an existing
# auth.json, so a container whose PoorMad bootstrap session took a terminal
...
# supply a freshly-issued session via POORMAD_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$POORMAD_HOME/auth.json" ] && [ -n "${POORMAD_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$POORMAD_HOME/auth.json"; then
        :
    else
        s6-setuidgid poormad "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$POORMAD_HOME/auth.json" \
...
# can set POORMAD_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# POORMAD_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$POORMAD_HOME/gateway_state.json" ] && \
        [ "${POORMAD_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$POORMAD_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$POORMAD_HOME/gateway_state.json"
        chown poormad:poormad "$POORMAD_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$POORMAD_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_poormad "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/poormad/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI PoorMad spawns for the
...
# main-wrapper.sh propagates it into the supervised ``poormad`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-poormad, dashboard, etc.).
