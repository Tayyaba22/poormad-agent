...
# Must match poormad_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``poormad-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``poormad-cli`` because those also contain
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on PoorMad Portal).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. poormad-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (poormad-cli,
    # poormad-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # Extract parent's API key so subagents inherit auth (e.g. PoorMad Portal).
    ...
    #
    # PoorMad Portal is dual-wire within a single provider: anthropic/* → Messages,
    # everything else → chat_completions. Same-provider inheritance would pin a
    # child PoorMad/Qwen subagent onto the parent's Claude Messages wire (or the
    ...
    elif _effective_provider_norm in {"nous", "nous-portal", "poormad"}:
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.poormad/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_poormad_dir("cache/delegation", "delegation_cache")
    ...
    if _subagent_id:
        if owner_session_id is None:
            try:
                ...
                owner_session_id = get_session_env("POORMAD_UI_SESSION_ID", "") or None
            except Exception:
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from poormad_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    # POORMAD_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-PoorMad-Session-Id on
    ...
    try:
        ...
        _origin_ui_session_id = get_session_env("POORMAD_UI_SESSION_ID", "")
    except Exception:
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-PoorMad-Session-Id once the batch completes.
            ...
            # POORMAD_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `poormad -z`, a cron job, a Kanban "
        ...
        try:
            ...
            _source = get_session_env("POORMAD_SESSION_SOURCE", "")
            ...
            _origin_ui_session_id = (
                get_session_env("POORMAD_UI_SESSION_ID", "") or _origin_ui_session_id
            )
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and POORMAD_SESSION_KEY is not in the CLI
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'poormad auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active PoorMad config.

    Prefer the shared persistent loader because it follows the active
    POORMAD_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``POORMAD_IGNORE_USER_CONFIG=1`` (``poormad chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("POORMAD_IGNORE_USER_CONFIG") == "1"
