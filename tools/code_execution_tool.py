...
Lets the LLM write a Python script that calls PoorMad tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `poormad_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `poormad_tools.py` with file-based RPC stubs
...
# available on every platform PoorMad itself runs on.
...
def _spill_full_stdout(stdout_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_poormad_dir("cache/exec", "exec_spill")
...
# match a safe prefix, the operational POORMAD_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports PoorMad
# code can lose the DB-layer Kanban mutation guard while still inheriting
# POORMAD_HOME.
#
# NB: the broad "POORMAD_" prefix was deliberately removed (#27303) — it leaked
# POORMAD_*-named config that lacks a secret substring (e.g. POORMAD_BASE_URL,
# POORMAD_KANBAN_DB, POORMAD_*_WEBHOOK).  The child only needs the few
# location/profile vars in _POORMAD_CHILD_ALLOWED below; POORMAD_RPC_SOCKET /
# POORMAD_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational POORMAD_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set poormad_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_POORMAD_CHILD_ALLOWED = frozenset({
    "POORMAD_HOME",
    "POORMAD_PROFILE",
    "POORMAD_CONFIG",
    "POORMAD_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational POORMAD_* vars (_POORMAD_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret POORMAD_* vars dropped by the tightened allowlist (#27303). The
    # broad "POORMAD_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like POORMAD_KANBAN_DB / POORMAD_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _POORMAD_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("POORMAD_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped POORMAD_* var.
            _dropped_poormad.append(k)
    if _dropped_poormad:
        logger.debug(
            "execute_code: dropped %d non-allowlisted POORMAD_* var(s) from the "
            ...
            len(_dropped_poormad),
            ", ".join(sorted(_dropped_poormad)),
        )
...
# ---------------------------------------------------------------------------
# poormad_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-PoorMad-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-PoorMad-profile soft guard."""',
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    poormad_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'poormad_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_poormad_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the poormad_tools.py stub module.
...
"""Auto-generated PoorMad tools RPC stubs."""
...
def _connect():
    ...
    POORMAD_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["POORMAD_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("POORMAD_RPC_TOKEN", ""),
    }) + "\\n"
    ...
    # re-accepts (POORMAD_RPC_PERSISTENT=1); retry once on a fresh socket.
    _attempts = 2 if os.environ.get("POORMAD_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
"""Auto-generated PoorMad tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("POORMAD_RPC_DIR") or os.path.join(tempfile.gettempdir(), "poormad_rpc")
_seq = 0
...
def _execute_remote(
    ...
    sandbox_dir = f"{temp_dir}/poormad_exec_{sandbox_id}"
    ...
    try:
        ...
        # --- Session-kernel path (poormad-agent#96873) -------------------
        ...
        tools_src = generate_poormad_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/poormad_tools.py", tools_src)
        ...
        env_prefix = (
            f"POORMAD_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"POORMAD_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("POORMAD_TIMEZONE", "").strip()
        if tz:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    child_env["POORMAD_RPC_SOCKET"] = rpc_endpoint
    child_env["POORMAD_RPC_TOKEN"] = rpc_token
    ...
    # POORMAD_TIMEZONE is an internal PoorMad setting and must not leak
    # into child processes.
    _tz_name = os.getenv("POORMAD_TIMEZONE", "").strip()
    if _tz_name:
        child_env["TZ"] = _tz_name
    child_env.pop("POORMAD_TIMEZONE", None)

    apply_subprocess_home_env(child_env)
    # ``poormad_tools.py`` always lives in the staging directory, so that
    # directory must be importable even when project mode changes CWD.
    # PoorMad's own package root is useful too, but only when the child
    # uses the same Python environment. Project mode can select an
    # external venv; exposing PoorMad's site-packages to that interpreter
    # can mix incompatible compiled extensions (for example, Python 3.12
    # NumPy with a Python 3.9 project interpreter).
    #
    # Before re-injecting PYTHONPATH, strip PoorMad-owned entries that
    # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
    # so it passes the scrub).  They are redundant for same-PoorMad-
    ...
    _strip_poormad_owned_pythonpath(child_env)
    _poormad_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_poormad_python_environment(child_python):
        _pp_parts.append(_poormad_root)
    elif child_python not in _external_env_logged:
        # Import behavior changes silently otherwise — surface it (once
        # per interpreter path) so "import poormad_constants suddenly
        # fails" reports are diagnosable without log spam.
        _external_env_logged.add(child_python)
        logger.info(
            "execute_code: child interpreter %s is outside the PoorMad "
            "environment; poormad root omitted from PYTHONPATH",
            child_python,
        )
    if _existing_pp:
...
def execute_code(
    ...
    # terminal() path refuses `launchctl bootout ai.poormad.gateway`, but the
    ...
    # --- Set up temp directory with poormad_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="poormad_sandbox_")
    ...
    # selector from POORMAD_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"poormad_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated poormad_tools module.
        ...
        tools_src = generate_poormad_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "poormad_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  POORMAD_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.poormad/.env')).
...
def _load_config() -> dict:
    ...
    ``poormad --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (poormad-agent's python). Reproducible and the
...
# Interpreter paths already reported as outside the PoorMad environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the poormad root from every
...
def _uses_poormad_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to PoorMad's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``poormad tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and poormad-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.poormad/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
            ...
            "PoorMad's own python (the common case — stdlib plus PoorMad's "
    ...
    description = (
        "Run Python that calls PoorMad tools programmatically. Use when you "
        ...
        "loses that state.\n\n"
        f"Available via `from poormad_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from poormad_tools import {import_str}` "
