...
409), and is driven by:
  ...
  * the ``poormad sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``poormad_cli/``) so the
...
**PoorMad admin**. We read that off the access token, which rides on the same
...
status conflates "may administer PoorMad" with "has Skill Sync enabled", and has
...
is toggled via ``poormad sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.poormad/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``poormad sync enable`` — it is
...
# namespace from poormad-agent's local ``content_hash`` (skills_guard.py:846),
...
# Wire claim name is NAS's; it means "this user is a PoorMad admin"
...
class SyncInertError(RuntimeError):
    ...
    not logged in, no bearer, or the caller is not a PoorMad admin.
    """
...
def resolve_identity() -> Dict[str, Any]:
    """Resolve the PoorMad bearer + owner + dev-gate flag.
    ...
    try:
        ...
        raise SyncInertError(f"no PoorMad credentials: {e}") from e
...
# sync.base_url (or the POORMAD_SYNC_BASE_URL bridge env) overrides it to point
...
DEFAULT_SYNC_BASE_URL = "https://gateway-gateway.poormad.com"

def resolve_sync_base_url() -> Optional[str]:
    """Resolve the sync-plane base URL.

    Order: POORMAD_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("POORMAD_SYNC_BASE_URL")
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a PoorMad Cloud instance can be set
...
# precedence as base_url: the POORMAD_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   POORMAD_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   POORMAD_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   POORMAD_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``POORMAD_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    switch a PoorMad Cloud deployment sets to opt its instances into sync by
    default. It is checked by the gate-and-swallow entrypoints IN ADDITION to
    the PoorMad-admin token gate and a configured base URL — all three must hold for
    background sync to run.
    """
    return _sync_config_bool("POORMAD_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``POORMAD_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``poormad sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "POORMAD_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``POORMAD_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``poormad sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    PoorMad Cloud deployment wants. Per the design notes, this default is
    ...
    """
    return _sync_config_bool("POORMAD_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.poormad/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.poormad/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (PoorMad Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``POORMAD_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.poormad/skills/ containing
...
def stable_device_id() -> str:
    ...
    Use ``set_device_name()`` / ``poormad sync device --name`` to set an explicit
    label."""
    ...
    # PoorMad Cloud (and any templated deployment) can seed the label
    # declaratively via POORMAD_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `poormad sync device --name` (or editing the
    # file) still wins on that device. An explicit file (above) always wins over
    # the env.
    import os

    env_name = (os.environ.get("POORMAD_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.poormad/skills/.sync_device_id, overwriting
...
# the caller's three-way merge. Auth is the PoorMad bearer resolved above.
# ---------------------------------------------------------------------------
...
# ~/.poormad/skills/.sync_state as JSON.
#
...
# path under ~/.poormad/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.poormad/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this PoorMad speaks "
            f"{WIRE_VERSION} — update PoorMad to sync with it"
        )
...
def push_skills(
    ...
    *,
    ...
    message: str = "poormad skill sync",
) -> Dict[str, Any]:
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (poormad sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.poormad/skills/. Only paths the user has opted into (``sync: true``) are
...
# (no push, no pull, no-op) unless the signed-in user is a PoorMad admin.
# ---------------------------------------------------------------------------

def maybe_push_skills(*, message: str = "poormad skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not identity.get("nous_admin"):
            return None  # access gate: inert unless the user is a PoorMad admin
        if not sync_feature_enabled():
            return None  # feature off for this instance (POORMAD_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``poormad sync status``. Never raises."""
    status: Dict[str, Any] = {
        "nous_admin": False,
        "logged_in": False,
...
#
# Org skills live under a DISTINCT local namespace, ~/.poormad/skills/_org/
...
#
# `poormad sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.poormad/skills/_org/<org_id>/``.
