...
_EGRESS_LABEL_KEY = "poormad-egress"
...
def _load_poormad_env_vars() -> dict[str, str]:
    """Load ~/.poormad/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active PoorMad profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale poormad-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=poormad-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling PoorMad process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=poormad-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a poormad process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed PoorMad
    process leaves its container behind permanently because there's no
    subsequent PoorMad process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=poormad-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=poormad-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``POORMAD_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("POORMAD_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using POORMAD_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'poormad'
...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``env_overrides`` — env vars to set on container creation: ``HTTPS_PROXY``,
      ...
      paths, and one ``POORMAD_PROXY_TOKEN_<NAME>`` per minted mapping
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    if not status.configured:
        msg = (
            ...
            "Run `poormad egress setup` to mint tokens and write proxy.yaml."
        )
        if enforce:
    ...
    if not (status.pid and status.listening):
        msg = (
            ...
            "Start it with `poormad egress start`."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if status.ca_cert_path is None or not status.ca_cert_path.exists():
        ...
        msg = (
            ...
            "Re-run `poormad egress setup` to regenerate it."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if not mappings:
        msg = (
            ...
            "corrupt.  Re-run `poormad egress setup` to mint provider "
    ...
    container_ca = "/etc/ssl/certs/poormad-egress-ca.crt"
    ...
    # through the proxy.  Keep the POORMAD_PROXY_TOKEN_* aliases for
    # diagnostics.
    for m in mappings:
        env_overrides[m.real_env_name] = m.proxy_token
        env_overrides[f"POORMAD_PROXY_TOKEN_{m.real_env_name}"] = m.proxy_token
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``poormad-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.poormad/sandboxes/). Non-persistent
        ...
        if egress_env_overrides:
            try:
                from poormad_cli.config import load_config as _load_cfg_for_collision
        ...
        try:
            from poormad_cli.config import load_config as _load_cfg_for_precedence
        ...
        # ``_POORMAD_EGRESS_NODE_OPTIONS_APPEND`` and merge here.
        _egress_node_append = merged_env.pop(
            "_POORMAD_EGRESS_NODE_OPTIONS_APPEND", None,
        )
        ...
        # s6-overlay images (e.g. poormad-agent:latest) already use /init as PID 1
        ...
        container_name = f"poormad-{uuid.uuid4().hex[:8]}"
        # Labels make poormad-created containers identifiable to:
        #   * the orphan reaper (`poormad-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`poormad-task-id`, `poormad-profile`)
        #   * operators running `docker ps --filter label=poormad-agent=1`
        ...
        label_args = [
            "--label", "poormad-agent=1",
            "--label", f"poormad-task-id={task_label}",
            "--label", f"poormad-profile={profile_name}",
        ...
        self._labels = {
            "poormad-agent": "1",
            "poormad-task-id": task_label,
            "poormad-profile": profile_name,
            _EGRESS_LABEL_KEY: egress_label,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior PoorMad process
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        # win over the generic PoorMad secret blocklist. Only implicit passthrough
        # keys are filtered. Also strip PoorMad-internal dynamic secrets
        # (AUXILIARY_*_API_KEY / _BASE_URL, GATEWAY_RELAY_* auth) that the
        # name-based blocklist doesn't cover — see _is_poormad_internal_secret.
        _implicit_forward = {
            k for k in passthrough_keys if not _is_poormad_internal_secret(k)
        }
        forward_keys = explicit_forward_keys | (_implicit_forward - _POORMAD_PROVIDER_ENV_BLOCKLIST)
        poormad_env = _load_poormad_env_vars() if forward_keys else {}
        unset_names: set[str] = set()
        for key in sorted(forward_keys):
            value = os.getenv(key) or poormad_env.get(key)
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("poormad-task-id", "")
        profile_label = self._labels.get("poormad-profile", "")
        existing = self._find_reusable_container(
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"poormad-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(
        ...
        matches containers that happened to be named ``poormad-*`` but were
        started by some other tool.
        """
        try:
            filters = [
                "--filter", "label=poormad-agent=1",
                "--filter", f"label=poormad-task-id={task_label}",
                "--filter", f"label=poormad-profile={profile_label}",
            ]
            if egress_label != "off":
                ...
                # poormad-egress label is present and not "off".  Without
                # this, a container created with egress=on can be silently
                # reused after the operator runs "poormad egress disable",
        ...
        # container) but can happen if a previous PoorMad process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every PoorMad exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        PoorMad process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next PoorMad startup. That covers the
        ...
        # outlives PoorMad processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"poormad-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``poormad /quit``
