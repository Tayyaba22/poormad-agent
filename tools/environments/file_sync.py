...
_FORCE_SYNC_ENV = "POORMAD_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.poormad") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.poormad to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``POORMAD_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, poormad_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.poormad/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(poormad_home=poormad_home)

    def _sync_back_transaction(self, poormad_home: Path | None = None) -> None:
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="poormad-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.poormad/skills/a.md`` →
        ``~/.poormad/skills/a.md``, a new remote file at
        ``/root/.poormad/skills/b.md`` maps to ``~/.poormad/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
