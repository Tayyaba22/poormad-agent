...
provider** — turn on a backend by enabling a plugin (``poormad plugins
enable video_gen/<name>``) and selecting it in ``poormad tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/poormad-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `poormad tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `poormad tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `poormad tools` → "
...
# `poormad tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`poormad tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `poormad tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
