...
`poormad skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.poormad.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.poormad.blueprint`` is a mapping containing
    ...
    poormad = meta.get("poormad") if isinstance(meta, dict) else None
    blueprint = poormad.get("blueprint") if isinstance(poormad, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.poormad.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.poormad.blueprint`` block) they can hand
    to ``poormad skills publish`` to share. ``body`` is the plain-language
