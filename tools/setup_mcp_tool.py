...
``poormad mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the PoorMad desktop app. Use the "
            "terminal instead: `poormad mcp install <name>` for catalog entries, "
            "`poormad mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "PoorMad desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `poormad mcp catalog` "
