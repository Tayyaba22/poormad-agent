...
teammate agent (another PoorMad profile on this install, or an agent on a
...
DM each other via a prompt-injected ``poormad -p <bot> chat ...`` shellout.
...
- Everything here is additive. The legacy protocol transports
  (``poormad -p`` / ``poormad peer dm``) keep working for older prompts.

The transports themselves are unchanged and proven:
- local teammate  → ``poormad -p <name> chat --in ~ -c "Bot Chat"
  --create-if-missing -Q --query-file <tmp>`` (one turn, reply on stdout)
- peer teammate   → ``poormad peer dm <peer>[/<name>] < <tmp>``
                ...
                "properties": {
                    "target": {
                        ...
                        "description": (
                            ...
                            "('researcher', 'poormad' for the default agent), or "
...
def _resolve_local_name(target: str, roster: list[str]) -> Optional[str]:
    """Map a target handle to a profile name ('poormad' → 'default')."""
    want = target.strip()
    if not want:
        return None
    if want.lower() == "poormad":
...
def cleanup_bot_dm_cache(
    ...
    locations: list[tuple[Path, str]] = [
        (temp_root, "poormad-dm-*.txt"),
        (temp_root, "poormad-relay-dm-*.txt"),
    ]
    try:
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    Local deliveries (``poormad -p <profile> chat …``) collide with relay
    ...
    # resolves the venv-relative poormad next to this gateway's interpreter
    ...
    if (
        stdin_file
        or len(argv) < 3
        or cli not in ("poormad", "poormad.exe")
        or argv[1] != "-p"
    ):
        return contextlib.nullcontext()
    from tools.bot_relay import acquire_turn_lock

    home = Path(os.getenv("POORMAD_HOME") or os.path.expanduser("~/.poormad"))
    return acquire_turn_lock(_poormad_root(home), argv[2])
...
def _agent_home(agent: Any) -> str:
    ...
    try:
        ...
        pass
    return os.getenv("POORMAD_HOME") or os.path.expanduser("~/.poormad")
