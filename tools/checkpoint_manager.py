...
-----------------------------------------------------------------------------
    ...
    ~/.poormad/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/poormad/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_poormad_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/poormad"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (PoorMad convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("POORMAD_CHECKPOINT_TIMEOUT", 30)))
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file`` / ``patch`` produced, so restores can tell "PoorMad wrote
...
def _git_env(
    ...
    The shared store is internal PoorMad infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``poormad checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `poormad checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "poormad@local"], store, cfg_wd)
    _run_git(["config", "user.name", "PoorMad Checkpoint"], store, cfg_wd)
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir():
        ...
        wd_marker = child / "POORMAD_WORKDIR"
        if wd_marker.exists():
            try:
...
# look for ``HEAD``/``info/exclude``/``POORMAD_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at POORMAD_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "POORMAD_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file PoorMad just successfully wrote.
        ...
        recorded hash was hand-edited by the user after PoorMad last touched
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        ...
        still matches what PoorMad last wrote (per the agent-write ledger)
        and ``skipped`` lists files the user hand-edited after PoorMad'
        last write or that PoorMad never wrote at all.
        """
        ...
        ledger = _load_ledger(store, dir_hash)
        if not ledger:
            # No agent-write ledger yet (pre-existing store, or PoorMad has
        ...
        for rel in names_out.splitlines():
            ...
            if recorded is None:
                # PoorMad never wrote this file (or the ledger predates it) —
                # do not touch it in safe mode.
                skipped.append(rel)
                continue
            current = _hash_file(abs_path)
            if current is None:
                # File deleted since PoorMad wrote it: restoring it back is
                # safe — its last content was PoorMad-authored.
                restore.append(rel)
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has PoorMad changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        PoorMad changed", not an exact per-session ledger.
    ...
    def restore(
        ...
        hand-edited after PoorMad' last write — per the agent-write ledger —
        are left untouched, and only PoorMad-authored changes are reverted.
        ...
        if restore_paths is not None:
            ...
            # PoorMad-created files absent from it (delete to restore state).
            ...
            for rel in restore_paths:
                ...
                if ok_in_commit:
                    ...
                    # PoorMad created it. Deleting it would not restore a prior
...
# ---------------------------------------------------------------------------
# Public helpers for `poormad checkpoints` CLI
# ---------------------------------------------------------------------------
