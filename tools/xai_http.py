...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``poormad tools`` repaint, tool-registration scans,
    ...
    2. ``~/.poormad/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``poormad auth add xai-oauth``
    ...
    try:
        ...
        auth_path = get_poormad_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.poormad/.env`` first, then ``os.environ``.

    Wraps :func:`poormad_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from poormad_cli.config import get_env_value as _poormad_get_env_value
    except ImportError:
        return os.environ.get(name, default)

    value = _poormad_get_env_value(name)
    return value if value is not None else default


def poormad_xai_user_agent() -> str:
    """Return a stable PoorMad-specific User-Agent for xAI HTTP calls."""
    try:
        from poormad_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"PoorMad-Agent/{__version__}"


def poormad_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as PoorMad,
    matching the direct HTTP integrations (search, TTS, STT, image, video).
    """
    return {"User-Agent": poormad_xai_user_agent()}


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level PoorMad config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per PoorMad home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_poormad_home() / "state"
...
def _resolve_explicit_xai_base_url(default: str = "https://api.x.ai/v1") -> str:
    ...
    Honors ``POORMAD_XAI_BASE_URL`` then ``XAI_BASE_URL`` (the same override
    pair the OAuth branch reads) and pins the origin with
    :func:`poormad_cli.auth._xai_validate_inference_base_url` so a tampered
    env override can't exfiltrate the bearer; on rejection it falls back to
    the default rather than raising.
    """
    override = str(
        get_env_value("POORMAD_XAI_BASE_URL")
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers PoorMad-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``poormad_cli.config.get_env_value`` so keys
    stored in ``~/.poormad/.env`` (the standard PoorMad location) are honored —
    ...
    honors ``POORMAD_XAI_BASE_URL`` / ``XAI_BASE_URL`` behind the same
