#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the PoorMad Skills Hub.
...
Used by poormad_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _poormad_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            poormad_meta = metadata.get("poormad", {})
            if isinstance(poormad_meta, dict):
                tags = poormad_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            poormad_meta = metadata.get("poormad", {})
            if isinstance(poormad_meta, dict):
                raw_tags = poormad_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by PoorMad) but not activated
    ...
    OFFICIAL_REPO = "PoorMad/poormad-agent"
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``poormad update``. Rather than telling them to update first, resolve
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                poormad_meta = meta_block.get("poormad", {})
                if isinstance(poormad_meta, dict):
                    tags = poormad_meta.get("tags", [])
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``poormad_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# PoorMad centralized index source
# ---------------------------------------------------------------------------

POORMAD_INDEX_URL = "https://poormad.dev/docs/api/skills-index.json"
POORMAD_INDEX_TTL = 6 * 3600  # 6 hours


def _poormad_index_cache_file() -> Path:
    return _index_cache_dir() / "poormad-index.json"


def _load_poormad_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for POORMAD_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    poormad_index_cache_file = _poormad_index_cache_file()
    if poormad_index_cache_file.exists():
        try:
            age = time.time() - poormad_index_cache_file.stat().st_mtime
            if age < POORMAD_INDEX_TTL:
                return json.loads(poormad_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("PoorMad index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "PoorMad index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("PoorMad index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        poormad_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        poormad_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class PoorMadIndexSource(SkillSource):
    """Skill source backed by the centralized PoorMad Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_poormad_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "poormad-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "poormad-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        PoorMadIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "poormad-index"
