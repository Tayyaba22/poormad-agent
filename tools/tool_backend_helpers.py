...
def managed_nous_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Return True when the user is entitled to the PoorMad Tool Gateway.

    Entitlement is paid PoorMad Portal service access OR a live free tool pool
...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    """Return account-aware guidance for an unavailable PoorMad Tool Gateway path."""
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `poormad model` to refresh your "
        "PoorMad Portal login and billing status."
    )
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``poormad auth add
    ...
    2. The environment / ``~/.poormad/.env``. Under a multiplexed gateway turn
       ...
       ``poormad_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``poormad auth
    ...
    when omitted, ``poormad_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `poormad auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``poormad auth add openai-api``) via
...
# The provider value the managed "PoorMad Subscription" picker rows write for
...
def read_selection(section: str) -> str | None:
    """Return the stored `poormad tools` provider string for a config section.

    THE single runtime read of the persisted selection. Returns:
    - ``"nous"`` — the managed PoorMad Tool Gateway row was selected,
    ...
    key. ``use_gateway: true`` was only ever written by the managed "PoorMad
    Subscription" row, so it maps to ``"nous"`` regardless of the name key;
    ``use_gateway: false`` beside a name key maps to that name.
    """
    try:
        from poormad_cli.config import read_raw_config_readonly
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (
        f"{section} is configured to use {selection_name} (set via poormad "
        f"tools), but {failure}. Run 'poormad tools' to change it."
    )
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.poormad/.env`` (via
    ``poormad_cli.config.get_env_value`` when available) so tool-side
