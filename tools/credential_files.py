...
def _resolve_poormad_home() -> Path:
...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.poormad",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``POORMAD_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside POORMAD_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because POORMAD_HOME is exactly where
    ...
    """
    poormad_home = _resolve_poormad_home()

    # Reject absolute paths — they bypass the POORMAD_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to POORMAD_HOME)",
            relative_path,
        )
        return False

    host_path = poormad_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape POORMAD_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, poormad_home)
    ...
    # inside POORMAD_HOME and therefore pass the containment check above.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.poormad/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="poormad-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching poormad_constants.get_poormad_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_poormad_dir()`` for backward compatibility with old directory layouts.
    """
    from poormad_constants import get_poormad_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_poormad_dir(new_subpath, old_name)
        if not host_dir.is_dir():
            ...
            # mount costs the feature. get_poormad_dir() already resolved
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.poormad``,
    SSH ``<remote_home>/.poormad``, etc.) and whether translation is wanted.
...
def to_agent_visible_cache_path(
    ...
    backend's PoorMad cache lands):

    * docker / modal — bind-mounted (docker) or per-file-synced (modal) at
      ``/root/.poormad`` (the *container_base* default).
    * ssh / daytona / vercel_sandbox — file-synced under the remote user's
      home; ``~/.poormad`` is shell-expanded by the remote shell, so tool
    ...
    * singularity — NOT translated: Apptainer auto-binds the host home, so
    ...
    if backend in ("docker", "modal"):
        pass  # /root/.poormad default
    elif backend in ("ssh", "daytona", "vercel_sandbox"):
        container_base = "~/.poormad"
    else:
