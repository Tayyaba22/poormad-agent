...
`mcp_tool.py`, and `poormad_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# PoorMadMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_poormad_provider_class() -> Optional[type]:
    ...
    class PoorMadMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._poormad_server_name = server_name
            self._poormad_home = ""
            ...
            self._poormad_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`PoorMadTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, PoorMadTokenStorage)
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            """
            # The SDK's httpx flavour, not PoorMad' — mcp 2.0 builds on httpx2,
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._poormad_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import PoorMadTokenStorage
                        if isinstance(storage, PoorMadTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._poormad_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import PoorMadTokenStorage
            if not isinstance(storage, PoorMadTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``poormad mcp reauth``.
            ...
            back to ``poormad mcp reauth``.
            """
            try:
                if self._poormad_preregistered:
                    return
                ...
                # walking back into the same refusal. `poormad mcp login`
                ...
                if cimd_url and rejected_id == cimd_url:
                    logger.warning(
                        ...
                        self._poormad_server_name, cimd_url,
                    )
        ...
        async def async_auth_flow(self, request):  # type: ignore[override]
            ...
            try:
                await get_manager().invalidate_if_disk_changed(
                    self._poormad_server_name,
                    poormad_home=self._poormad_home,
                )
            except Exception as exc:  # pragma: no cover — defensive
                logger.debug(
    ...
    return PoorMadMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_POORMAD_PROVIDER_CLS: Optional[type] = _make_poormad_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._poormad_home = key[0]
    ...
    def _build_provider(
        ...
        Constructs :class:`PoorMadMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _POORMAD_PROVIDER_CLS is None:
            logger.warning(
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `poormad mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``poormad mcp remove <name>`` and (indirectly) by
        ``poormad mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, poormad_home), None)

        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, poormad_home=poormad_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, poormad_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, poormad_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, poormad_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(poormad_home) / f"{_safe_filename(server_name)}.json"
            try:
