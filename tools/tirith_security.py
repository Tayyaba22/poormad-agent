...
it is automatically downloaded from GitHub releases to $POORMAD_HOME/bin/tirith.
...
def _get_poormad_home() -> str:
    """Return the PoorMad home directory, respecting POORMAD_HOME env var."""
    return str(get_poormad_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_poormad_home(), ".tirith-install-failed")
...
def _poormad_bin_dir() -> str:
    """Return $POORMAD_HOME/bin, creating it if needed."""
    d = os.path.join(_get_poormad_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $POORMAD_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_poormad_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $POORMAD_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $POORMAD_HOME/bin/tirith
    ...
    poormad_bin = os.path.join(_poormad_bin_dir(), "tirith")
    if os.path.isfile(poormad_bin) and os.access(poormad_bin, os.X_OK):
        _resolved_path = poormad_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return poormad_bin
