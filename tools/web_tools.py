...
Backend is selected during ``poormad tools`` setup (web.backend in config.yaml).
When available, PoorMad can route Firecrawl calls through a PoorMad-hosted tool-gateway
for PoorMad Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for PoorMad Subscribers)
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via PoorMad config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through PoorMad' config/.env layer (``poormad config set``, ``poormad tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.poormad/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``poormad tools``). A
    ...
    if configured:
        ...
        # The managed "PoorMad Subscription" selection ("nous") is serviced by
    ...
    # pre-empted by a PoorMad OAuth token whose subscription tier may not
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `poormad tools` repaint.
        try:
...
def _web_requires_env() -> list[str]:
    ...
    refresh against the PoorMad portal on every CLI startup (invoked at
            ...
            if provider is None and backend and selection_exists("web"):
                ...
                if disabled_key:
                    ...
                    error_text = (
                        ...
                        f"Re-enable it with `poormad plugins enable {disabled_key}` "
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `poormad tools` to set one up."
                    ),
                }
        else:
            logger.info(
                ...
                if backend and selection_exists("web"):
                    ...
                    if disabled_key:
                        ...
                        error_text = (
                            ...
                            f"config. Re-enable it with `poormad plugins "
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`poormad plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
def _provider_is_ready(provider) -> bool:
    ...
    require a true availability probe — otherwise ``poormad doctor`` paints a
...
if __name__ == "__main__":
    ...
    from poormad_cli.config import get_env_value as _gev
