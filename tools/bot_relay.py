...
PoorMad Cloud, docker) is a persistent line. This module is the gateway-side
...
file plumbing on the gateway's own POORMAD root — no network. The public
...
# ``bot_mode.envelope_ttl_seconds`` default in poormad_cli/config_defaults.py.
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    if not profile or not connection_id:
        return None
    if not handle:
        handle = "poormad" if profile == "default" else profile
    if (
...
def _poormad_cli() -> str:
    """Resolve the poormad CLI beside this gateway's own interpreter.

    The deliver RPC runs on the target gateway, whose process is the venv
    python — its bin/Scripts directory holds the matching ``poormad``
    entrypoint. A bare ``"poormad"`` relies on PATH, which is exactly what
    ...
    sibling = exe.parent / ("poormad.exe" if sys.platform == "win32" else "poormad")
    if sibling.is_file():
        return str(sibling)
    found = shutil.which("poormad")
    if found:
        return found
    return "poormad"
...
# turns concurrently: deliveries spawn separate ``poormad`` subprocesses, so
