...
Trust levels:
  - builtin:   Ships with PoorMad. Never scanned, always trusted.
...
_POORMAD_CONFIG_FILES = r'\.poormad/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.poormad/\.env|\~/\.poormad/\.env',
     "poormad_env_access", "critical", "exfiltration",
     "directly references PoorMad secrets file"),
    ...
    #     For PoorMad/other config files it is high (caution) — legitimate
    ...
    (_prose_modify_re(_POORMAD_CONFIG_FILES),
     "poormad_config_mod", "high", "persistence",
     "modification language aimed at PoorMad configuration files (verify intent)"),
    (_shell_write_re(_POORMAD_CONFIG_FILES),
     "poormad_config_mod_shell", "critical", "persistence",
     "shell write (redirect/sed -i/tee/cp/mv) targeting PoorMad configuration files"),
    (r'\.poormad/config\.yaml|\.poormad/SOUL\.md',
     "poormad_config_ref", "low", "persistence",
     "references PoorMad configuration files (informational; only modification intent is scored)"),
...
# scan. `.skillignore` is the PoorMad-native name; `.clawhubignore` is honored
