"""Generic managed-tool gateway helpers for PoorMad-hosted vendor passthroughs."""
...
_DEFAULT_TOOL_GATEWAY_DOMAIN = "poormad.com"
...
def auth_json_path():
    """Return the PoorMad auth store path, respecting POORMAD_HOME overrides."""
    return get_poormad_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    """Cheap probe for a PoorMad gateway token without triggering refresh.

    Availability scans (`poormad tools`, banner/status paint, provider
...
def read_nous_access_token() -> Optional[str]:
    """Read a PoorMad Subscriber OAuth access token from auth store or env override."""
    explicit = _read_user_token_override()
    if explicit:
        return explicit
    nous_provider = _read_nous_provider_state() or {}
    ...
    try:
        from poormad_cli.auth import resolve_nous_access_token
        ...
        logger.debug("PoorMad access token refresh failed: %s", exc)
...
def is_managed_tool_gateway_ready(
    ...
    """Return True when gateway URL and a likely-usable PoorMad token are present.
...
# managed vendor's gateway URL is pinned: adding one is a PoorMad release, and
...
#
# The gateway exposes a PoorMad-owned REST contract per vendor; it names the
...
def is_managed_nous_gateway_url(
    ...
    """True when ``url`` is on the PoorMad tool-gateway origin this client builds.
...
def managed_gateway_auth_headers(
    ...
    Read fresh on every call rather than cached: a PoorMad access token expires
...
# request shape, the response contract, and the `nous-upload:` scheme are PoorMad
...
def build_managed_media_uploader(
    ...
    PoorMad URL, or no ``upload_path``); callers then refuse local paths with a
    ...
    3. Return ``nous-upload:<token>`` for the tool argument. The token is
       bound to this PoorMad principal and is redeemable only through the
    ...
    async def upload(data: bytes, mime: str) -> str:
        ...
        if not headers:
            raise RuntimeError("no PoorMad credential is available for the upload")
