...
selectable via ``poormad tools`` → Image Generation; the active model is
...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "fal-ai/nano-banana-pro": {
        ...
        "defaults": {
            ...
            # Users on PoorMad Subscription should stay at 1K for predictable billing.
            "resolution": "1K",
        },
        "supports": {
    ...
    "fal-ai/gpt-image-2": {
        ...
        "defaults": {
            # Same quality pinning as gpt-image-1.5: medium keeps PoorMad
...
# ---------------------------------------------------------------------------
# Managed FAL gateway (PoorMad Subscription)
# ---------------------------------------------------------------------------
def _resolve_managed_fal_gateway():
    """Resolve the FAL route from the stored `poormad tools` selection.
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                f"PoorMad Subscription gateway rejected model '{model}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"the PoorMad Portal's FAL proxy. Either:\n"
                f"  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  • Pick a different model via `poormad tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.poormad"
    ...
    # PoorMad cache roots can be translated without side effects. SSH can still
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `poormad tools` → Image Generation."
            )
...
def check_fal_api_key() -> bool:
    """True if the FAL backend selected via `poormad tools` (or, on a
...
def _build_no_backend_setup_message() -> str:
    ...
    Used by the in-tree FAL path. Mentions:
      - FAL_KEY signup link
      - managed-gateway status (if PoorMad tools are enabled)
    ...
    if managed_nous_tools_enabled():
        lines.append(
            "  2. Sign in to a PoorMad account that has the managed FAL "
            "gateway enabled (`poormad setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `poormad tools` "
        "→ Image Generation (run `poormad plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def _dispatch_to_plugin_provider(
    ...
    if not configured or configured in ("fal", NOUS_MANAGED_PROVIDER):
        ...
        # PoorMad Subscription selection) also runs the legacy pipeline, which
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `poormad plugins list` to see "
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `poormad tools` → Image Generation."
                ),
...
# model/provider via `poormad tools` or `/skills`.
