...
One ``CDPSupervisor`` runs per PoorMad ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach PoorMad logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "poormad-dialog-bridge.invalid"
...
(() => {
  if (window.__poormadDialogBridgeInstalled) return;
  window.__poormadDialogBridgeInstalled = true;
  const ENDPOINT = "http://poormad-dialog-bridge.invalid/";
