...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite PoorMad's live source checkout "
        ...
        "checkout, stop PoorMad, run the command externally, then restart "
        "PoorMad."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(poormad_home).expanduser() if poormad_home else Path.home() / ".poormad"
