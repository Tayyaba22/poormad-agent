...
Design contract (matches the PoorMad "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    poormad_meta = meta.get("poormad") if isinstance(meta, dict) else None
    if not isinstance(poormad_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.poormad.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in poormad_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.poormad.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("poormad", "agent", "poormad agent") and (
        author != "PoorMad"
    ):
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'PoorMad' (proper caps) "
