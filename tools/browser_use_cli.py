...
_PRIVATE_BROWSER_SENTINEL = "_POORMAD_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# poormad: pin this named session to its own tab (once per daemon process)
def _poormad_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "poormad-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_poormad_ensure_own_tab()
del _poormad_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from PoorMad's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at PoorMad's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_poormad_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `poormad tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """PoorMad' own bin dir ($POORMAD_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from poormad_constants import get_poormad_home

        return str(Path(get_poormad_home()) / "bin")
...
def _find_cli() -> Optional[List[str]]:
    ...
    MANAGED-FIRST resolution: PoorMad' own ``$POORMAD_HOME/bin`` copy — the
    one every browser backend selection installs and updates via
    ``install_cli()`` — always wins, so all sessions drive one canonical,
    PoorMad-controlled binary. PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: PoorMad' managed uv (bootstrapped on demand via
    ``poormad_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$POORMAD_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical PoorMad-managed copy, or resolution
    # stays pinned to a binary we don't control (version drift, no updates
    # through poormad tools).
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_poormad_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_backend_cdp(
    ...
    3. A configured cloud browser provider (Browserbase, Firecrawl, PoorMad
    ...
    # PoorMad-gateway variant (use_gateway: true) DOES resolve through the
    ...
    try:
        ...
        return (
            ...
            "switch backends via `poormad tools` → Browser Automation."
        )
...
def _resolve_real_profile_cdp(env: dict, force_local: bool) -> Optional[str]:
    ...
    default Chromium with their logins — a browser PoorMad launches on a
    SNAPSHOT of their real profile (see poormad_cli.browser_connect). Two ways
    in:
...
def browser_exec(
    ...
    # PoorMad gateway, CDP override, local Chrome, …). Named sessions compose
...
def _dynamic_schema_overrides() -> dict:
    ...
    if _real_profile_consented():
        ...
        props["local"] = {
            "type": "boolean",
            "description": (
                "Drive the user's own local browser (a PoorMad-managed copy of "
