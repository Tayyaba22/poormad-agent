...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.poormad/config.yaml``. PoorMad
...
Configuration is loaded from ~/.poormad/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``poormad_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from poormad_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``poormad auth add <provider_id>``).
...
from tools.xai_http import poormad_xai_user_agent
...
# The managed OpenAI audio gateway (PoorMad portal proxy) only proxies these speech
...
# Base URL now resolved via poormad_cli.models.deepinfra_base_url (shared).
...
def _get_default_output_dir() -> str:
    ...
    return str(get_poormad_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.poormad/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.poormad/config.yaml.
    ...
    try:
        ...
        logger.debug("poormad_cli.config not available, using default TTS config")
        return {}
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``poormad tools``); otherwise the historical Edge backend remains active.

    The managed "PoorMad Subscription" selection (``tts.provider: nous``) is
...
# custom voice-cloning scripts, etc.) into PoorMad without any Python code
...
#           output_format: wav
#
# PoorMad writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__POORMAD_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _command_provider_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of PoorMad secrets by
...
def _run_command_tts(
    ...
    Child env is scrubbed of PoorMad secrets (salvage of #56332) while still
    ...
    scrubbed = poormad_subprocess_env(inherit_credentials=False)
...
# file, so retired models disappear from poormad the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``poormad_cli.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `poormad setup` to configure, "
    ...
    from poormad_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `poormad model` or set XAI_API_KEY.")
    ...
    # output_format when PoorMad actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": poormad_xai_user_agent(),
        },
        json=payload,
        timeout=60,
        stream=True,
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import poormad_cli as _poormad_cli

            _poormad_version = str(_poormad_cli.__version__)
        except Exception:
            _poormad_version = "0.0.0"
        # Include PoorMad client context following Gemini's partner
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where PoorMad caches Piper voice models.

    Resolves to ``~/.poormad/cache/piper-voices/`` under the active
    POORMAD_HOME so voice downloads follow profile boundaries.
    """
    from poormad_constants import get_poormad_dir
    root = Path(get_poormad_dir("cache/piper-voices", "piper_voices_cache"))
...
def _text_to_speech_single(
    ...
    *,
    ...
    platform = get_session_env("POORMAD_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `poormad setup` to install Mistral support."
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run poormad setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'poormad setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'poormad tools' and select Piper under TTS, or install manually: "
...
def _resolve_openai_audio_client_config() -> tuple[str, str, bool]:
    ...
    ``is_managed`` is True when the config resolves to the PoorMad managed audio
    ...
    if selected == NOUS_MANAGED_PROVIDER:
        ...
        if managed_gateway is None:
            raise ValueError(selection_error(
                "tts",
                NOUS_MANAGED_PROVIDER,
                "the PoorMad Tool Gateway is not available (not entitled or "
                "unreachable)",
            ))
        return (
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_poormad_home()}/audio_cache/<timestamp>.mp3"
            },
            "speed": {
                "type": "number",
