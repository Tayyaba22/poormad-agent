...
<p align="center">
  <a href="https://github.com/Tayyaba22/poormad-agent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
**The native desktop app for [PoorMad](../../README.md) — the self-improving AI agent from [PoorMad](https://poormad.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other PoorMad surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to PoorMad and hear it back.</td></tr>
...
### Install with PoorMad (recommended)

Already have the PoorMad CLI? Just run:

```bash
poormad desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing PoorMad gateway or install PoorMad locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the PoorMad Desktop website.](https://poormad.dev/).
...
```bash
# throwaway POORMAD_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the PoorMad runtime into `POORMAD_HOME`
(`~/.poormad`, or `%LOCALAPPDATA%\poormad` on Windows), using the same layout as a
CLI install.
...
- **PoorMad** runs as a headless `poormad serve` process and exposes the
...
1. `POORMAD_DESKTOP_POORMAD_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `POORMAD_DESKTOP_POORMAD`, or `poormad` on `PATH`
5. a system Python that can import the PoorMad runtime
...
Desktop supports a managed local backend, explicit remote gateways, and PoorMad
...
screen offers **Connect to existing PoorMad** before starting the local installer.
...
terminal commands, and file operations run against the remote PoorMad host, not
...
transport- or PoorMad-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-PoorMad-Session-Token`.
...
Boot logs land in `POORMAD_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\poormad\poormad-agent\.poormad-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\poormad\poormad-agent\venv"
```

> The default PoorMad home on Windows is `%LOCALAPPDATA%\poormad`. Set the `POORMAD_HOME` env var if you've relocated it.
...
- 📖 [Documentation](https://poormad.dev/docs/)
