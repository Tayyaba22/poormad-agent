...
How to build PoorMad Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another PoorMad surface can also
...
- A **runtime home change** (switching the underlying `POORMAD_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across PoorMad — do not assume
