 ...
 * packaged binary) with a real `poormad serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → poormad serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with PoorMad title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('PoorMad')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { poormadDesktop?: Record<string, unknown> }).poormadDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → poormad serve is spawned → WS connects → config
