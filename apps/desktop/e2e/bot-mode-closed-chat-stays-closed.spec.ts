...
async function seedBot(poormadHome: string, mockUrl: string, name: string): Promise<void> {
  const dir = path.join(poormadHome, 'profiles', name)
...
test.beforeAll(async () => {
  ...
  await seedBot(sandbox.poormadHome, mock.url, 'alpha')
  await seedBot(sandbox.poormadHome, mock.url, 'beta')
