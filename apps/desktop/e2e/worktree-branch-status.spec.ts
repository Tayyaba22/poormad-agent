...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'PoorMad E2E'], { cwd: repo })
...
function configureRepoCwd(poormadHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(poormadHome, mockUrl)
  fs.appendFileSync(path.join(poormadHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(poormadHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.poormadHome, mock.url, repo)
