...
/**
 * E2E smoke tests for the packaged PoorMad desktop app.
 *
 ...
 * (credential stripping, isolated POORMAD_HOME + userData, unique app name).
 *
...
test('window opens with the PoorMad title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      poormadDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).poormadDesktop?.hud?.open({ sessionId: null })
  )
