...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.poormadHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.poormadHome)
  const builder = await RealSessionBuilder.start(sandbox.poormadHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.poormadHome, mock.url)
  fs.appendFileSync(path.join(sandbox.poormadHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.poormadHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
