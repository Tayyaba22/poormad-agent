 ...
 * second gateway, "Homelab", is a REAL second `poormad serve` this spec spawns
 * with its own POORMAD_HOME, profiles and session token, registered in the v2
...
function findPoorMadBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'poormad')

  if (fs.existsSync(venv)) {
    return venv
  }

  const result = spawnSync('which', ['poormad'], { encoding: 'utf8' })

  if (result.status === 0 && result.stdout.trim()) {
    return result.stdout.trim()
  }

  throw new Error('poormad binary not found: create the repo venv (uv sync) or put poormad on PATH')
}
...
/**
 * Spawn a second, fully real `poormad serve` as the remote gateway. Its
 * session token is pinned through POORMAD_DASHBOARD_SESSION_TOKEN so the
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        POORMAD_HOME: home,
        POORMAD_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote poormad serve exited early (${child.exitCode}):\n${log}`)
    }

    try {
      const response = await fetch(`${url}/api/status`, {
        headers: { 'X-PoorMad-Session-Token': REMOTE_TOKEN },
      })
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote poormad serve never became ready:\n${log}`)
  }
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    writeMockProviderConfig(sandbox.poormadHome, mock.url)
    ...
    seedProfiles(sandbox.poormadHome, ['research'])
