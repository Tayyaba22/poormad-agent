/**
 * Shared E2E fixtures for the PoorMad desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → poormad serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `poormad desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `poormad-e2e-${prefix}-${Math.random()}`))
  const poormadHome = path.join(root, 'poormad-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(poormadHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(poormadHome, 'config.yaml')
 ...
 */
export function writeEnvFile(poormadHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(poormadHome, '.env')
 ...
 */
function writeEmptyConfig(poormadHome: string): void {
  const configPath = path.join(poormadHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - POORMAD_HOME → sandbox poormad-home (isolated config/sessions)
 *  - POORMAD_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - POORMAD_DESKTOP_IGNORE_EXISTING=1 → don't pick up `poormad` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - POORMAD_DESKTOP_POORMAD_ROOT → REPO_ROOT (dev checkout resolution)
 *  - POORMAD_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    POORMAD_HOME: sandbox.poormadHome,
    POORMAD_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    POORMAD_DESKTOP_IGNORE_EXISTING: '1',
    POORMAD_DESKTOP_POORMAD_ROOT: REPO_ROOT,
    POORMAD_DESKTOP_APP_NAME: `PoorMadE2E-${Date.now()}`,
    ...
    POORMAD_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated POORMAD_HOME + userData
 * @param env      - the process environment (already has POORMAD_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.poormadHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.poormadHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via POORMAD_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`poormad serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.poormadHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { POORMAD_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to PoorMad backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'PoorMad.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'PoorMad.app', 'Contents', 'MacOS', 'PoorMad')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'poormad')
}
 ...
 * progress without spawning a real PoorMad backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * POORMAD_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    POORMAD_DESKTOP_BOOT_FAKE: '1',
    POORMAD_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + poormad-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).POORMAD_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).POORMAD_DESKTOP_POORMAD
  delete (env as Record<string, string | undefined>).POORMAD_DESKTOP_POORMAD_ROOT
