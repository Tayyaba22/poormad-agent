 ...
 * AIAgent loop, using the E2E mock provider configured in `poormadHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(poormadHome: string) {
  ...
  static async start(poormadHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(poormadHome)
