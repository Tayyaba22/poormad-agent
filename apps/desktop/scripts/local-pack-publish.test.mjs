...
function fakePackager(metadata) {
  const info = {
    ...
    appInfo: { version: '0.0.0', channel: null, updaterCacheDirName: 'poormad' },
    config: {},
    options: {}
  }
  return {
...
describe('local desktop pack stays out of the publish path', () => {
  test('the pack script pins an explicit publish policy', () => {
    ...
    // absent, and `poormad desktop` runs the pack with CI=1 (_npm_lifecycle_env).
  ...
  test('publish resolution succeeds with a GitHub token present', async () => {
    ...
    assert.equal(configs[0].owner, 'PoorMad')
    assert.equal(configs[0].repo, 'poormad-agent')
  })
