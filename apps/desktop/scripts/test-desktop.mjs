...
// no longer bundles the PoorMad Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'PoorMad.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'PoorMad'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'PoorMad.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'PoorMad'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default POORMAD_HOME for non-sandboxed runs -- matches main.ts's
// resolvePoorMadHome(). On Windows it's %LOCALAPPDATA%\poormad; elsewhere
// it's ~/.poormad. The fresh-install sandbox launchFresh() sets its own
// POORMAD_HOME and never touches this.
const DEFAULT_POORMAD_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'poormad')
  }
  return path.join(os.homedir(), '.poormad')
})()
const VENV_ROOT = path.join(DEFAULT_POORMAD_HOME, 'poormad-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'poormad-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.POORMAD_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `PoorMad-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `PoorMad-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `PoorMad-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'PoorMad-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.POORMAD_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.POORMAD_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const poormadHome = path.join(sandbox, 'poormad-home')
  ...
  env.POORMAD_DESKTOP_CWD = cwd
  env.POORMAD_DESKTOP_IGNORE_EXISTING = '1'
  env.POORMAD_DESKTOP_TEST_MODE = 'fresh-install'
  env.POORMAD_DESKTOP_USER_DATA_DIR = userDataDir
  env.POORMAD_HOME = poormadHome
  delete env.POORMAD_DESKTOP_POORMAD
  delete env.POORMAD_DESKTOP_POORMAD_ROOT
  ...
  console.log(`  POORMAD_HOME: ${poormadHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(poormadHome, 'poormad-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The PoorMad Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of poormad_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'poormad-agent', 'poormad_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing PoorMad
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + POORMAD_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  POORMAD_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
