#!/usr/bin/env node
// set-exe-identity.mjs — stamp the PoorMad icon + version metadata onto the
// built PoorMad.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked PoorMad.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `poormad desktop`, the installer's
...
// path (which rebuilds via `poormad desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-PoorMad.exe>
//
...
// Stamp the PoorMad icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'PoorMad',
      FileDescription: 'PoorMad',
      CompanyName: 'PoorMad',
      LegalCopyright: 'Copyright (c) 2026 PoorMad'
    }
  })

  console.log('[set-exe-identity] done — PoorMad icon + identity stamped')
}
