 ...
 * product name (`PoorMad`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/PoorMad'
 *
 * This is a hard failure with no obvious cause for the user — `poormad desktop`
 ...
 * (the mac-unpacked PoorMad.app bundle) and Windows (win-unpacked), so we
 ...
 * PoorMad.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `poormad desktop --build-only` (poormad_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'PoorMad.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'PoorMad'}.exe`
