...
const UPSTREAM_URL = 'https://github.com/Tayyaba22/poormad-agent.git'
...
const LEGACY_STORAGE_KEY = 'poormad.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'poormad.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.poormadDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `poormad-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const poormadHome = join(sandbox, 'poormad-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(poormadHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(poormadHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        POORMAD_DESKTOP_APP_NAME: `PoorMadShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        POORMAD_DESKTOP_POORMAD_ROOT: target.targetRoot,
        POORMAD_DESKTOP_IGNORE_EXISTING: '1',
        POORMAD_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'poormad-short-session-ab-'))
  const prepared = []
