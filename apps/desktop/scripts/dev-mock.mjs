...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `poormad-dev-mock-${Date.now()}`))
  ...
  return { root, poormadHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(poormadHome, mockUrl) {
  fs.writeFileSync(
    path.join(poormadHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(poormadHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.poormadHome, mock.url)
  console.log(`  POORMAD_HOME: ${sandbox.poormadHome}`)
  ...
  const env = {
    ...
    POORMAD_DESKTOP_APP_NAME: `PoorMadDevMock-${Date.now()}`,
  }
