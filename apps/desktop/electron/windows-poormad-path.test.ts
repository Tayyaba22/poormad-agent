// Unit tests for the pure Windows `poormad` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvPoorMadCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `poormad` shim shadows
//      the real poormad.cmd/poormad.exe.
...
//   3. resolveVenvPoorMadCommand() — must probe the venv python via
//      canImportPoorMadCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvPoorMad: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvPoorMad: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvPoorMad: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvPoorMad: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvPoorMad: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvPoorMad: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvPoorMadCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportPoorMadCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    poormadHome: '/fake/poormad-home',
...
test('resolveVenvPoorMadCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvPoorMadCommand('/root/venv/Scripts/poormad.exe', [], deps), null)
})

test('resolveVenvPoorMadCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvPoorMadCommand('/root/venv/Scripts/poormad.cmd', [], deps), null)
})

test('resolveVenvPoorMadCommand: returns null when the basename is not poormad/poormad.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvPoorMadCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvPoorMadCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvPoorMadCommand('/root/venv/bin/poormad.exe', [], deps), null)
})

test('resolveVenvPoorMadCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvPoorMadCommand('/root/venv/Scripts/poormad.exe', [], deps), null)
})

test('resolveVenvPoorMadCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportPoorMadCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvPoorMadCommand('/root/venv/Scripts/poormad.exe', ['serve'], deps)
...
test('resolveVenvPoorMadCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvPoorMadCommand('/root/venv/Scripts/poormad.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'poormad_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvPoorMadCommand: is case-insensitive on poormad.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvPoorMadCommand('/root/venv/Scripts/POORMAD.EXE', [], deps))
  assert.ok(resolveVenvPoorMadCommand('/root/venv/SCRIPTS/poormad.exe', [], deps))
})
