...
import { makePoorMadCloudBackendDownError } from './backend-health'
import {
...
test('normalizeRemoteHeaders keeps safe proxy headers and drops transport/auth headers', () => {
  assert.deepEqual(
    normalizeRemoteHeaders({
      ...
      'X-PoorMad-Session-Token': { encoding: 'plain', value: 'token' },
...
test('remoteRequestMatchesBaseUrl treats HTTPS and WSS as the same gateway origin', () => {
  assert.equal(
    remoteRequestMatchesBaseUrl(
      'wss://poormad.example.com/gateway/api/ws?ticket=abc',
      'https://poormad.example.com/gateway'
    ),
    true
  )
  assert.equal(remoteRequestMatchesBaseUrl('ws://poormad.example.com/api/ws', 'http://poormad.example.com'), true)
  assert.equal(
    remoteRequestMatchesBaseUrl('wss://poormad.example.com/other/api/ws', 'https://poormad.example.com/gateway'),
    false
  )
  assert.equal(
    remoteRequestMatchesBaseUrl('wss://other.example.com/gateway/api/ws', 'https://poormad.example.com/gateway'),
    false
  )
})
...
test('profileRemoteOverride returns the per-profile remote with defaulted auth mode', () => {
  const config = {
    profiles: {
      coder: { mode: 'remote', url: '  https://coder.example.com/poormad  ', token: { value: 'sek' } }
    }
  }

  assert.deepEqual(profileRemoteOverride(config, 'coder'), {
    url: 'https://coder.example.com/poormad',
    authMode: 'token',
    token: { value: 'sek' }
  })
})
...
test('profileRemoteOverride treats a cloud entry as a remote override', () => {
  ...
  const config = {
    profiles: {
      coder: { mode: 'cloud', url: 'https://agent-1.agents.poormad.com', authMode: 'oauth' }
    }
  }

  assert.deepEqual(profileRemoteOverride(config, 'coder'), {
    url: 'https://agent-1.agents.poormad.com',
    authMode: 'oauth',
    token: undefined
  })
})
...
test('localProfileEntry preserves inactive SSH drafts but drops Cloud state', () => {
  const ssh = { mode: 'ssh', host: 'box', user: 'alice', remotePoorMadPath: '/poormad' }
...
// --- translateSelfProfileQuery (registry SSH-scoped poormad:api contract) ---
...
test('resolveProfileApiRequest routes action-status polls with the action-spawning routes', () => {
  ...
  // install): _spawn_poormad_action registers the dynamic action name only in
...
test('normalizeRemoteBaseUrl strips trailing slashes, hash, and query', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/poormad/'), 'https://gw.example.com/poormad')
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/poormad?x=1#frag'), 'https://gw.example.com/poormad')
})

test('normalizeRemoteBaseUrl preserves a path prefix', () => {
  assert.equal(normalizeRemoteBaseUrl('https://host/poormad'), 'https://host/poormad')
})
...
test('normalizeRemoteBaseUrl auto-prepends http:// for scheme-less host:port input', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('gw.example.com/poormad/'), 'http://gw.example.com/poormad')
})
...
test('buildGatewayWsUrl honors a path prefix', () => {
  assert.equal(buildGatewayWsUrl('https://host/poormad', 't'), 'wss://host/poormad/api/ws?token=t')
})
...
test('buildGatewayWsUrlWithTicket uses ?ticket= not ?token=', () => {
  const url = buildGatewayWsUrlWithTicket('https://gw.example.com/poormad', 'tkt-9')
  assert.equal(url, 'wss://gw.example.com/poormad/api/ws?ticket=tkt-9')
...
test('cookiesHaveSession detects the bare access-token cookie', () => {
  assert.equal(cookiesHaveSession([{ name: 'poormad_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession detects the __Host- and __Secure- prefixed variants', () => {
  assert.equal(cookiesHaveSession([{ name: '__Host-poormad_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveSession([{ name: '__Secure-poormad_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession is false for an empty value', () => {
  assert.equal(cookiesHaveSession([{ name: 'poormad_session_at', value: '' }]), false)
})
...
test('cookiesHaveSession ignores unrelated cookies (AT-only by design)', () => {
  ...
  assert.equal(cookiesHaveSession([{ name: 'poormad_session_rt', value: 'x' }]), false)
...
test('AT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(AT_COOKIE_VARIANTS, ['__Host-poormad_session_at', '__Secure-poormad_session_at', 'poormad_session_at'])
})

test('RT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(RT_COOKIE_VARIANTS, ['__Host-poormad_session_rt', '__Secure-poormad_session_rt', 'poormad_session_rt'])
})

// --- cookiesHaveLiveSession (AT or RT — the connectivity check) ---

test('cookiesHaveLiveSession is true for a live access-token cookie', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'poormad_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-poormad_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-poormad_session_at', value: 'x' }]), true)
})
...
test('cookiesHaveLiveSession is true for an RT cookie even with NO access-token cookie', () => {
  ...
  assert.equal(cookiesHaveLiveSession([{ name: 'poormad_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-poormad_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-poormad_session_rt', value: 'x' }]), true)
})

test('cookiesHaveLiveSession is true when both AT and RT are present', () => {
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'poormad_session_at', value: 'a' },
      { name: 'poormad_session_rt', value: 'r' }
    ]),
    true
  )
})
...
test('cookiesHaveLiveSession is false for empty values', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'poormad_session_at', value: '' }]), false)
  assert.equal(cookiesHaveLiveSession([{ name: 'poormad_session_rt', value: '' }]), false)
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'poormad_session_at', value: '' },
      { name: 'poormad_session_rt', value: '' }
    ]),
    false
  )
})
...
// --- cookiesHavePrivySession (PoorMad portal / Privy auth, NOT gateway cookies) ---
...
test('cookiesHavePrivySession does NOT treat poormad gateway cookies as a portal session', () => {
  // The whole point of Q7: a gateway session cookie is NOT a portal sign-in.
  assert.equal(cookiesHavePrivySession([{ name: 'poormad_session_at', value: 'x' }]), false)
  assert.equal(cookiesHavePrivySession([{ name: '__Host-poormad_session_rt', value: 'x' }]), false)
})
...
test('cookiesHavePrivyAccessToken is false for empty values, gateway cookies, and non-arrays', () => {
  ...
  assert.equal(cookiesHavePrivyAccessToken([{ name: 'poormad_session_at', value: 'x' }]), false)
...
test('gateway WS URL IPC result serializes success and the auth-vs-transport matrix', async () => {
  ...
  for (const error of [
    ...
    new Error('Timed out connecting to PoorMad backend after 8000ms'),
...
test('gatewayTicketFailure only copies an integer statusCode, not a message prefix', () => {
  ...
  // classifier (makePoorMadCloudBackendDownError) handles the prefix at the mint
...
// BEFORE waitForPoorMadReady. This mirrors main.ts buildRemoteConnection's
// catch — classify a PoorMad Cloud server fault via the shared factory, else
