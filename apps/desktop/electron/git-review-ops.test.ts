...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'poormad-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'poormad-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'PoorMad Test'], { cwd: dir })
