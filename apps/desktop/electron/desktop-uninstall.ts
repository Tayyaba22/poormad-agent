 ...
 * the three user-facing uninstall modes to the `poormad uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m poormad_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `poormad_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'poormad_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/PoorMad.app/Contents/MacOS/PoorMad  → …/PoorMad.app
 *   Windows: …\PoorMad\PoorMad.exe                 → …\PoorMad  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/PoorMad.app/Contents/MacOS/PoorMad → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/PoorMad.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs PoorMad.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]PoorMad$/i.test(dir) || /[\\/]poormad-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/poormad
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import poormad_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, poormadHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export POORMAD_HOME=${q(poormadHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import poormad_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but PoorMad install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "POORMAD_HOME=${String(poormadHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
