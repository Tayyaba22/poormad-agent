// Launching the PoorMad TUI in the user's OWN terminal emulator.
//
...
// `poormad --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's PoorMad runtime is often a venv Python invoked
//     as `python -m poormad_cli.main`, not a `poormad` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The PoorMad
