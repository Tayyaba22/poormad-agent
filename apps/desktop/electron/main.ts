...
import {
  ...
  resolveVenvPoorMadCommand
} from './windows-poormad-path'
import {
...
const USER_DATA_OVERRIDE = process.env.POORMAD_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.POORMAD_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.POORMAD_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with POORMAD_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[poormad] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[poormad] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. POORMAD_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[poormad] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[poormad] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via POORMAD_DESKTOP_PASSWORD_STORE, bridged by the
// `poormad desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[poormad] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[poormad] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `poormad desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose PoorMad sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[poormad] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[poormad] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[poormad] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[poormad] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[poormad] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('poormad:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized PoorMad burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[poormad] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[poormad] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[poormad] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[poormad] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// POORMAD_HOME — the user-facing root for everything PoorMad-related. Mirrors
// scripts/install.ps1's $PoorMadHome and scripts/install.sh's $POORMAD_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\poormad (matches install.ps1)
//   macOS / Linux: ~/.poormad (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.poormad directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\poormad yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// POORMAD_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// POORMAD_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.poormad / %LOCALAPPDATA%\poormad.
function resolvePoorMadHome() {
  if (process.env.POORMAD_HOME) {
    return normalizePoorMadHomeRoot(process.env.POORMAD_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'poormad-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a POORMAD_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\poormad and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('POORMAD_HOME')

    if (fromRegistry) {
      return normalizePoorMadHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'poormad')
    const legacy = path.join(app.getPath('home'), '.poormad')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.poormad setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.poormad')
}

const POORMAD_HOME = resolvePoorMadHome()

function pathWithPoorMadManagedNode(...entries) {
  const managed = poormadManagedNodePathEntries(POORMAD_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_POORMAD_ROOT — the canonical mutable PoorMad install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_POORMAD_ROOT = path.join(POORMAD_HOME, 'poormad-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_POORMAD_ROOT, 'venv')
...
// boots, going straight to `resolvePoorMadBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_POORMAD_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_POORMAD_ROOT, '.poormad-bootstrap-complete')
...
// active-profile.json records which PoorMad profile the desktop launches its
// local backend as. When set, startPoorMad() passes `poormad --profile <name>
// dashboard …`, which deterministically pins POORMAD_HOME (see
// _apply_profile_override in poormad_cli/main.py) and bypasses the sticky
// ~/.poormad/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors poormad_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// poormadDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under POORMAD_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by poormad_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(POORMAD_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (poormad_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.POORMAD_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.POORMAD_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.POORMAD_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.POORMAD_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.POORMAD_DESKTOP_APP_NAME || 'PoorMad'
...
// 'poormad:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.poormad) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.poormad')
}

// Seed the native About panel with the live PoorMad version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `poormad update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolvePoorMadVersion(),
  copyright: 'Copyright © 2026 PoorMad'
})
...
// startPoorMad(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.POORMAD_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.POORMAD_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
//   * WSL2 IPC stall  = the renderer's `poormad:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.POORMAD_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startPoorMad() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning poormad serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start PoorMad backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  poormadLog.push(...lines)

  if (poormadLog.length > 300) {
    poormadLog.splice(0, poormadLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-poormad-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('poormad:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the poormad:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('poormad:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_POORMAD_ROOT
  })
}
...
// The Tauri updater writes POORMAD_HOME/.poormad-update-in-progress for the whole
...
// (`force_kill_other_poormad`, taskkill /IM poormad.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(POORMAD_HOME)),
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — PoorMad will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(POORMAD_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'PoorMad update did not finish',
        `${result.message}\n\nDetails: ${path.join(POORMAD_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `poormad` — must not shadow `poormad.cmd`/`poormad.exe`.
...
function unwrapWindowsVenvPoorMadCommand(command, backendArgs) {
  return resolveVenvPoorMadCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `poormad serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `poormad` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'poormad_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `poormad --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, POORMAD_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolvePoorMadBackend step 4); execFileSync of a .cmd without
...
function isPoorMadSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'poormad_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.POORMAD_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several PoorMad deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the poormad-import
...
// findGitBash — locate bash.exe on Windows. Resolves POORMAD_GIT_BASH_PATH
...
// normal POORMAD_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\poormad\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'poormad', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'poormad', 'git', 'bin', 'git.exe'))
  }
...
function recentPoorMadLog() {
  return poormadLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's poormad root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_POORMAD_ROOT.
// POORMAD_DESKTOP_POORMAD_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.POORMAD_DESKTOP_POORMAD_ROOT && path.resolve(process.env.POORMAD_DESKTOP_POORMAD_ROOT),
    !IS_PACKAGED && isPoorMadSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isPoorMadSourceRoot(ACTIVE_POORMAD_ROOT) ? ACTIVE_POORMAD_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_POORMAD_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('poormad:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'poormad-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `poormad update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(POORMAD_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `poormad update` can write
// fresh entry points. On Windows this is the file the running backend
// `poormad.exe` holds open; on POSIX it's never mandatory-locked.
function venvPoorMadShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'poormad.exe')
    : path.join(updateRoot, 'venv', 'bin', 'poormad')
}
...
// only signals the direct child, so on Windows a backend `poormad.exe` that
// spawned its own grandchildren (a `poormad` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `PoorMad backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live PoorMad backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.poormadBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the PoorMad backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.poormadBackendIdentity
...
// `poormadProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `poormad.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a poormad
// REPL, a pty terminal, the gateway) keep `poormad.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const poormadProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\poormad.exe, but the
  // backend is `python.exe -m poormad_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (poormadProcess && Number.isInteger(poormadProcess.pid)) {
    initialPids.push(poormadProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(poormadProcess, {
    forceKillProcessTree,
    stopAllPoolBackends
  })

  const shim = venvPoorMadShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentPoorMadProcess = backendConnectionState.getProcess()

        if (currentPoorMadProcess && Number.isInteger(currentPoorMadProcess.pid)) {
          stragglers.push(currentPoorMadProcess.pid)
        }
...
// `poormad update`). Instead we spawn the staged PoorMad-Setup binary with
// --update and quit, so it can run `poormad update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `poormad update` → relaunch),
      ...
      // in: the POORMAD_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `poormad update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `poormad desktop`, never the Tauri installer that self-copies
      // poormad-setup.exe into POORMAD_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `poormad` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `poormad update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `poormad update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, poormadRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(POORMAD_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating PoorMad — this window will close and the updater will open. Don’t reopen PoorMad yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(POORMAD_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // poormad.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running poormad, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the PoorMad install open ' +
        '(a second PoorMad window or a terminal running poormad?). Close it and retry.'
    ...
    // PoorMad processes running from this venv.  The updater normally refuses
    ...
    // `poormad update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `poormad update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          POORMAD_HOME,
          POORMAD_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithPoorMadManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `poormad update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(POORMAD_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another PoorMad update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(POORMAD_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. PoorMad will keep running — try again, or run \`poormad update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvPoorMad = path.join(venvBin, IS_WINDOWS ? 'poormad.exe' : 'poormad')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's PoorMad launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the poormad
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.poormad-bootstrap-complete')),
      hasVenvPoorMad: fileExists(venvPoorMad),
...
// Python-level pre-update snapshot inside `poormad update`.
function preflightStateDb(poormadHome, rememberLog) {
  const stateDbPath = path.join(poormadHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(poormadHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(poormadHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(poormadHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `poormad update`, swaps/relaunches the app bundle, and writes
// .poormad-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'poormad update', percent: null })

    return { ok: true, manual: true, command: 'poormad update', poormadRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      POORMAD_HOME,
      POORMAD_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithPoorMadManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating PoorMad — this window will close. Don’t reopen PoorMad yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_POORMAD_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isPoorMadSourceRoot(ACTIVE_POORMAD_ROOT) &&
    fileExists(venvPython) &&
    canImportPoorMadCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_POORMAD_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `poormad
...
function resolveWebDist() {
  const override = process.env.POORMAD_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set POORMAD_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: poormad desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: poormad desktop --force-build`
  )
...
function resolvePoorMadCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/PoorMad.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.POORMAD_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolvePoorMadCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'poormad_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_POORMAD_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `PoorMad at ${ACTIVE_POORMAD_ROOT}`,
    command,
    args: ['-m', 'poormad_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      poormadHome: POORMAD_HOME,
      pythonPathEntries: [ACTIVE_POORMAD_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_POORMAD_ROOT,
    bootstrap: true,
    shell: false
  }
}
...
function resolvePoorMadBackend(backendArgs) {
  ...
  //    installed `poormad` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isPoorMadSourceRoot.)
  if (!IS_PACKAGED && isPoorMadSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `PoorMad source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_POORMAD_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\poormad\\poormad-agent (Windows) or ~/.poormad/poormad-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active PoorMad runtime at ${ACTIVE_POORMAD_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `poormad` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    POORMAD_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.POORMAD_DESKTOP_IGNORE_EXISTING !== '1') {
    let poormadCommand = null
    const poormadOverride = process.env.POORMAD_DESKTOP_POORMAD

    if (poormadOverride) {
      const resolvedOverride = findOnPath(poormadOverride)

      if (resolvedOverride) {
        poormadCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(poormadOverride, { isWsl: IS_WSL })) {
        poormadCommand = poormadOverride
      } else {
        rememberLog(`Ignoring Windows PoorMad override under WSL: ${poormadOverride}`)
      }
    } else {
      poormadCommand = findOnPath('poormad')
    }

    if (poormadCommand) {
      if (looksLikeDesktopAppBinary(poormadCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving PoorMad CLI: ${poormadCommand}`)
        poormadCommand = null
      }
    }

    if (poormadCommand) {
      const unwrapped = unwrapWindowsVenvPoorMadCommand(poormadCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `poormad` shim
      ...
      const shellForProbe = isCommandScript(poormadCommand)

      // POORMAD_DESKTOP_POORMAD is an explicit deployment override (used by
      ...
      if (shouldTrustPoorMadOverride(poormadOverride) || verifyPoorMadCli(poormadCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvPoorMadCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing PoorMad CLI at ${poormadCommand}`,
          command: poormadCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing PoorMad CLI at ${poormadCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed poormad_cli module via system Python.
  ...
  if (python) {
    ...
    // having poormad_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\poormad\poormad-agent\venv.
    if (canImportPoorMadCli(python)) {
      return {
        kind: 'python',
        label: `installed poormad_cli module via ${python}`,
        command: python,
        args: ['-m', 'poormad_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: poormad_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolvePoorMadBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'PoorMad not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_POORMAD_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolvePoorMadBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no PoorMad install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'PoorMad recovery was handed off to PoorMad Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      poormadHome: POORMAD_HOME,
      logRoot: path.join(POORMAD_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('PoorMad install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `PoorMad bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(POORMAD_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startPoorMad() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // poormad:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolvePoorMadBackend(backend.args))
  }
  ...
  if (!isPoorMadSourceRoot(ACTIVE_POORMAD_ROOT)) {
    throw new Error(
      `PoorMad install at ${ACTIVE_POORMAD_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. PoorMad' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\poormad\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `poormad` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for PoorMad on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch PoorMad.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isPoorMadSourceRoot()
    // plus an importable poormad_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `PoorMad venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `PoorMad at ${ACTIVE_POORMAD_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'PoorMad runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----poormad-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported PoorMad backend URL protocol: ${parsed.protocol}`))
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the PoorMad backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to PoorMad backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-PoorMad-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-PoorMad-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('poormad:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolvePoorMadCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('poormad:preview-file-changed', payload)
}
...
// waitForPoorMadReady how to read a 401 — rejected session vs gated route.
...
async function waitForPoorMad(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForPoorMadReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('poormad:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('poormad:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('poormad:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('poormad:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting PoorMad...".
function sendPowerResume() {
  ...
  webContents.send('poormad:power-resume')
}
...
// 'poormad:power-battery' push below.
ipcMain.handle('poormad:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('poormad:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('poormad:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('poormad:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('poormad:context-menu-spellcheck', {
...
//
// Hosted PoorMad gateways gate the dashboard behind an OAuth provider (e.g.
// PoorMad) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``poormad_session_at``),
...
//   * PoorMad Portal now issues a 24h ROTATING, reuse-detected refresh token
//     alongside the ~15-min access token (Portal NAS #293 / poormad #37247).
//     Both are set as HttpOnly cookies (``poormad_session_at`` ~15 min,
//     ``poormad_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startPoorMad → the renderer's
// single-shot boot() with no retry) surfaced as the "PoorMad couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "PoorMad
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to PoorMad Cloud agent…' : 'Sign in to PoorMad gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to PoorMad backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in poormad_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- PoorMad Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
// The "cloud" connection mode lets a user sign in to the PoorMad portal ONCE in
...
// portal session cookie living in `persist:poormad-remote-oauth`:
...
// Canonical PoorMad portal base URL, overridable for staging/dev. Mirrors the CLI
// convention (poormad_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every PoorMad surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.poormad.dev'

function resolvePortalBaseUrl() {
  const raw = process.env.POORMAD_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL

  return String(raw).trim().replace(/\/+$/, '')
}

// Whether the OAuth partition currently holds a live PoorMad portal session — the
// credential that powers both discovery and the silent cascade. The portal
// authenticates via PRIVY, not the PoorMad gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for poormad_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing PoorMad Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a PoorMad Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to PoorMad Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// Discover the hosted (PoorMad Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to PoorMad Cloud. Open Settings → Gateway, choose PoorMad Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your PoorMad Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your PoorMad Cloud session has expired. Sign in to PoorMad Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the PoorMad Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every poormad:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remotePoorMadPath`/`remoteProfile` —
...
// a valid stored value (pins the root POORMAD_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.POORMAD_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.POORMAD_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted PoorMad Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemotePoorMadPath: (ssh || savedSsh)?.remotePoorMadPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by POORMAD_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the PoorMad Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered PoorMad Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remotePoorMadPath: input.sshRemotePoorMadPath ?? existingBlock.remotePoorMadPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (poormad #37247), and the gateway middleware transparently
    ...
    try {
      ...
      // For a PoorMad-managed Cloud agent, a 502/503/504 from the WS-ticket mint
      ...
      // else (#85335).
      const cloudError = makePoorMadCloudBackendDownError(baseUrl, error)
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote PoorMad gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote PoorMad gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.POORMAD_DESKTOP_REMOTE_TOKEN,
      url: process.env.POORMAD_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      poormadPath: result.poormadPath,
      poormadHome: result.poormadHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { poormadPath: result.poormadPath, poormadHome: result.poormadHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remotePoorMadPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remotePoorMadPath: sshConfig.remotePoorMadPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForPoorMad: (baseUrl, token) => waitForPoorMad(baseUrl, token, lease.signal, 'token'),
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        poormadVersion: result.poormadVersion || '',
        ...
        // per-profile pools also use bare keys. Only startPoorMad' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.poormadVersion || 'poormad (version unknown)'} at ${result.poormadPath || '?'}`
  )
  ...
  return {
    ...connection,
    remotePoorMadVersion: result.poormadVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (POORMAD_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.POORMAD_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startPoorMad() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every PoorMad
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with PoorMad" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remotePoorMadPath: input.sshRemotePoorMadPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remotePoorMadPath || '')
          let poormadPath
          let poormadVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            poormadPath = runtime.poormadPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.poormadPath])
            poormadVersion = inspection.version
            supported = inspection.supported
          } else {
            poormadPath = await remoteLifecycle.locatePoorMad(ssh, sshConfig.remotePoorMadPath || '')
            poormadVersion = await remoteLifecycle.probePoorMadVersion(ssh, poormadPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, poormadPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update PoorMad on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remotePoorMadPath: poormadPath,
            remotePoorMadVersion: poormadVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startPoorMad())
  ...
  // connect to PoorMad gateway". Mirror the renderer's connect here so the test
...
function resetPoorMadConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const poormadProcess = backendConnectionState.invalidate()
  stopBackendChild(poormadProcess)
...
// startPoorMad() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetPoorMadConnection() invalidates it.
  const poormadProcess = backendConnectionState.getProcess()
  const dying = poormadProcess && !poormadProcess.killed ? poormadProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('poormad:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('poormad:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startPoorMad()
  ...
  // spawning, so two children never share one profile's POORMAD_HOME.
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(async error => {
    ...
    rememberLog(
      `PoorMad backend for profile "${key}" failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
...
async function ensureRegistryBackend(connectionId, profile, managedUpdateCorrelation = '') {
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remotePoorMadPath: source.remotePoorMadPath,
  ...
  if (source.kind === 'local') {
    ...
    localEntry.connectionPromise = spawnPoolBackend(profileKey, localEntry, {
      ...
      rememberLog(
        `PoorMad backend for profile "${profileKey}" (forced-local) failed to start: ${error instanceof Error ? error.message : String(error)}`
      )
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. poormad:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: poormadLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForPoorMad(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.poormadPath || !state?.poormadHome) {
  ...
  return {
    ...
    poormadPath: state.poormadPath,
    poormadHome: state.poormadHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remotePoorMadPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.poormadPath ? platform : await probeWindowsRemote(ssh, config.remotePoorMadPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          poormadPath: runtime.poormadPath,
          poormadHome: runtime.poormadHome,
          pythonPath: runtime.python
        }
      }
    }

    const poormadPath = await remoteLifecycle.locatePoorMad(ssh, config.remotePoorMadPath || '')
    const poormadHome = await remoteLifecycle.probeRemotePoorMadHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, poormadPath, poormadHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { poormadPath: state.poormadPath, poormadHome: state.poormadHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForPoorMad(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startPoorMad() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForPoorMad(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited POORMAD_HOME env (see _apply_profile_override
  // step 3 in poormad_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolvePoorMadBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const poormadCwd = resolvePoorMadCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(POORMAD_HOME, 'profiles', key))
  )

  rememberLog(`Starting PoorMad backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: poormadCwd,
        POORMAD_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        POORMAD_WEB_DIST: webDist,
        ...(readyFile ? { POORMAD_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`PoorMad backend for profile "${profile}" failed to start: ${error.message}`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`PoorMad backend for profile "${profile}" exited (${signal || code})`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `PoorMad backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForPoorMad(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `PoorMad backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `PoorMad backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_poormad_home() recreates the deleted profile directory.
//
...
async function startPoorMad() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('PoorMad Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startPoorMad() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving PoorMad backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('PoorMad backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote PoorMad backend at ${remote.baseUrl}`, 24)
      await waitForPoorMad(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote PoorMad backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, poormadLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.poormad/active_profile file) and
    // resolves POORMAD_HOME the same way `poormad -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving PoorMad runtime', 28)

        return resolvePoorMadBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting PoorMad backend via ${backend.label}`, 84)
    rememberLog(`Starting PoorMad backend via ${backend.label}`)
    ...
    const poormadProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin POORMAD_HOME for the child so Python's get_poormad_home()
          // resolves to the SAME location our resolvePoorMadHome() picked. Without
          // this pin, Python falls back to ~/.poormad on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\poormad, which differs from C:\Users\<u>\.poormad.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets POORMAD_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(poormadProcess)
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, poormadProcess)

    if (!processOwner) {
      stopBackendChild(poormadProcess)
      await waitForBackendExit(poormadProcess)
      releaseBackendChild(poormadProcess)
      throw new Error('PoorMad backend start was superseded by a newer connection attempt.')
    }

    poormadProcess.stdout.on('data', rememberLog)
    poormadProcess.stderr.on('data', rememberLog)
    ...
    poormadProcess.once('error', error => {
      releaseBackendChild(poormadProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale PoorMad backend error: ${error.message}`)
        rejectBackendStart?.(new Error('PoorMad backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`PoorMad backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `PoorMad backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    poormadProcess.once('exit', (code, signal) => {
      releaseBackendChild(poormadProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale PoorMad backend exit (${signal || code})`)
      ...
      rememberLog(`PoorMad backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `PoorMad backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `PoorMad backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentPoorMadLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for PoorMad backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(poormadProcess, {
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for PoorMad backend to become ready', 90)
    await Promise.race([waitForPoorMad(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => poormadProcess.exitCode === null && !poormadProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local PoorMad backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'PoorMad backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('poormad:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while PoorMad is
...
// pushes pet state over IPC (poormad:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via poormad:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main PoorMad app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the PoorMad icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see poormad:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep PoorMad in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('poormad:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so PoorMad can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('poormad:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('poormad:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[poormad] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('poormad:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `poormad:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, poormad:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('poormad:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('poormad:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('poormad:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'poormad desktop --force-build',
  ...
  startPoorMad().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('poormad:connection', async (_event, profile) => {
...
ipcMain.handle('poormad:connection:for', async (_event, payload) => {
  const { connectionId, profile } = payload && typeof payload === 'object' ? (payload as any) : ({} as any)
  const registry = readDesktopConnectionsRegistry()
  const id = String(connectionId || '').trim() || registry.primary
  // Same single-owner claim as 'poormad:connection', keyed by the composite
...
ipcMain.on('poormad:connection:active-route', (event, route) => {
...
// "Starting PoorMad…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('poormad:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetPoorMadConnection({ soft: true }),
...
ipcMain.handle('poormad:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('poormad:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('poormad:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('poormad:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('poormad:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('poormad:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolvePoorMadBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'PoorMad is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `poormad-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, POORMAD_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('poormad:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('poormad:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('poormad:zoom:get', event => {
...
ipcMain.on('poormad:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('poormad:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startPoorMad() call restarts the
...
ipcMain.handle('poormad:bootstrap:repair', async () => {
  // Forceful repair: force the next startPoorMad() through the full installer
  ...
  // "bypass active runtime" path inside startPoorMad() and fall through
  ...
  resetPoorMadConnection()

  return { ok: true }
})
ipcMain.handle('poormad:bootstrap:continue-local', async () => {
  rememberLog('[bootstrap] local install selected by renderer; continuing first-launch bootstrap')
  continueFirstRunLocalBootstrap()

  return { ok: true }
})
ipcMain.handle('poormad:bootstrap:cancel', async () => {
...
ipcMain.handle('poormad:boot-progress:get', async () => bootProgressState)
ipcMain.handle('poormad:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('poormad:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('poormad:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('poormad:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('poormad:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('poormad:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('poormad:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('poormad:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('poormad:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('poormad:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('poormad:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('poormad:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('poormad:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('poormad:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('poormad:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemotePoorMadPath: entry.remotePoorMadPath
    })
  ...
  if (entry.kind === 'local') {
    const local = await startPoorMad()
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remotePoorMadPath: connection.remotePoorMadPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemotePoorMadProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root POORMAD_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('poormad:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// poormad:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('poormad:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('poormad:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `poormad update` to every eligible registered connection at once.
...
ipcMain.handle('poormad:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/poormad/update', {}, { timeoutMs: 15_000 })
...
// analogue of the poormad:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('poormad:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('poormad:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startPoorMad() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('poormad:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- PoorMad Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('poormad:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('poormad:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('poormad:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('poormad:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of poormad:profile:set: records the profile the
...
// the gateway (#79886).
ipcMain.handle('poormad:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('poormad:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new POORMAD_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('poormad:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('poormad:requestMicrophoneAccess', async () => {
...
ipcMain.handle('poormad:window:readBelow', async event => {
...
async function handlePoorMadApiRequest(request) {
  ...
  // backend calls ensure_poormad_home() which recreates the profile directory,
...
ipcMain.handle('poormad:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its POORMAD_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handlePoorMadApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handlePoorMadApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('poormad:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('poormad:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('poormad:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as poormad://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('poormad:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('poormad:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('poormad:data-url-read-max:get', () => ({
...
ipcMain.handle('poormad:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('poormad:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('poormad:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('poormad:readFileText', async (_event, filePath) => {
...
// `poormad:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('poormad:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('poormad:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('poormad:writeClipboard', (_event, text) => {
...
ipcMain.handle('poormad:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('poormad:readClipboard', () => clipboard.readText())

ipcMain.handle('poormad:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('poormad:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('poormad:context-menu:edit', (event, command) => {
...
ipcMain.handle('poormad:context-menu:copy-image', event => {
...
ipcMain.handle('poormad:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('poormad:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('poormad:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('poormad:saveClipboardImage', async () => {
...
ipcMain.handle('poormad:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('poormad:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('poormad:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('poormad:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('poormad:active-work', (event, payload) => {
  const id = event.sender.id
...
ipcMain.on('poormad:titlebar-theme', (_event, payload) => {
...
ipcMain.on('poormad:native-theme', (_event, mode) => {
...
ipcMain.on('poormad:translucency:support', event => {
  event.returnValue = { glass: GLASS_SUPPORTED, translucency: TRANSLUCENCY_SUPPORTED }
})

ipcMain.on('poormad:translucency', (_event, payload) => {
...
ipcMain.on('poormad:keep-awake', (_event, on) => {
...
ipcMain.handle('poormad:quick-entry:settings:get', async () => {
...
ipcMain.handle('poormad:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('poormad:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('poormad:quick-entry:submit', {
...
ipcMain.on('poormad:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('poormad:quick-entry:state', payload)
  }
})

ipcMain.on('poormad:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('poormad:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('poormad:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `poormad:window:openSession`).
...
ipcMain.handle('poormad:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('poormad:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('poormad:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('poormad:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolvePoorMadCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('poormad:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolvePoorMadCwd()
}))

ipcMain.handle('poormad:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('poormad:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('poormad:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('poormad:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('poormad:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('poormad:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('poormad:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: poormadLog.slice(-200) }))
...
ipcMain.on('poormad:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (poormad:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('poormad:updates:check', async () =>
...
ipcMain.handle('poormad:updates:apply', async (_event, payload) =>
...
ipcMain.handle('poormad:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('poormad:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical PoorMad version (the one `release.py` bumps in
// poormad_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real PoorMad version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolvePoorMadVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'poormad_cli', '__init__.py')
...
// Renderer-bundle skew: `poormad update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live PoorMad version and push it into the native About panel
// just before showing it, so an in-place `poormad update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolvePoorMadVersion()} — app build out of date, update the desktop app`
        : resolvePoorMadVersion(),
      copyright: 'Copyright © 2026 PoorMad'
    })
    app.showAboutPanel()
  })
...
ipcMain.handle('poormad:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolvePoorMadVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    poormadRoot: resolveUpdateRoot(),
...
// the actual removal via `poormad uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (poormad_cli/uninstall.py + poormad_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_POORMAD_ROOT
  ...
  const fallback = () => ({
    poormad_home: POORMAD_HOME,
    agent_installed: isPoorMadSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'poormad_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, POORMAD_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no PoorMad venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import poormad_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_POORMAD_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_POORMAD_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `poormad-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `poormad-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('poormad:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('poormad:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('poormad:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('poormad:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// poormad:// deep links (e.g. poormad://blueprint/morning-brief?time=08:00,
// poormad://mcp/install?name=NAME&config=B64 — the vendor "Add to PoorMad"
// button, or poormad://plugin/install?repo=owner/repo). Dev
// (`POORMAD_DESKTOP_DEV_SERVER`) registers poormad-dev:// instead — bare
// Electron or a stale OS handler often owns poormad:// on dev machines.
...
// ---------------------------------------------------------------------------
const POORMAD_PROTOCOL = DEV_SERVER ? 'poormad-dev' : 'poormad'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['poormad-dev', 'poormad'] : ['poormad']
...
function handleDeepLink(url) {
  ...
  // poormad://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('poormad:deep-link', payload)
...
ipcMain.handle('poormad:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${POORMAD_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(POORMAD_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(POORMAD_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${POORMAD_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `poormad://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startPoorMad), whose
...
app.whenReady().then(() => {
  ...
  // startPoorMad() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching poormad:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting PoorMad never keeps another app's chord hostage.
