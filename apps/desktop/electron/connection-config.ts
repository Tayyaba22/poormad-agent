 ...
 *     `X-PoorMad-Session-Token` header; WS uses `?token=`.
...
// poormad_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - poormad_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - poormad_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / poormad #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-poormad_session_at', '__Secure-poormad_session_at', 'poormad_session_at']
const RT_COOKIE_VARIANTS = ['__Host-poormad_session_rt', '__Secure-poormad_session_rt', 'poormad_session_rt']

// The PoorMad portal (NAS) does NOT use PoorMad gateway session cookies — it is a
...
// Keep this aligned with poormad_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['poormad', 'test', 'tmp', 'root', 'sudo'])
...
function gatewayTicketFailure(error, authMessage, transportMessage) {
  ...
  // classification (isServerSideHttpError / makePoorMadCloudBackendDownError) and
...
/** Which PoorMad profile the remote SSH dashboard should actually run as.
...
// PoorMad Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remotePoorMadPath = String(entry.remotePoorMadPath || '').trim()

  if (remotePoorMadPath) {
    out.remotePoorMadPath = remotePoorMadPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote PoorMad installation. Preserve an explicit mapping
  // when it is a valid PoorMad profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_poormad_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `POORMAD_HOME` scopes it.
 *
 ...
 * `poormad:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://poormad.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `poormad:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a poormad session ACCESS-token cookie
...
/**
 * True if the cookie jar holds a live PoorMad PORTAL (Privy) session — a non-empty
 ...
 * the PoorMad gateway session cookies, so cloud sign-in / discovery liveness
