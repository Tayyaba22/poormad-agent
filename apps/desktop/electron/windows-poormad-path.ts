/**
 * windows-poormad-path.ts
 *
 * Pure, dependency-injected pieces of Windows `poormad` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvPoorMadCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `poormad` shim shadowed the real
 *      poormad.cmd/poormad.exe; the shim then failed the --version probe and
 ...
 *      and PoorMad launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvPoorMadCommand() — unwrapWindowsVenvPoorMadCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `poormad`) must
 * not shadow `poormad.cmd`/`poormad.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the PoorMad launcher that drives `poormad update`.
...
export interface ResolveVenvPoorMadCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportPoorMadCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `poormad`/`poormad.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/poormad(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m poormad_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportPoorMadCli(). A
 * venv whose update died mid-`pip install` still has python.exe + poormad.exe
 ...
 *
 * Returns null when `command` is not a venv poormad shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvPoorMadCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvPoorMadCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^poormad(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv PoorMad at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing PoorMad Python at ${python}`,
    command: python,
