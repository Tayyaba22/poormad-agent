...
export const ZOOM_STORAGE_KEY = 'poormad:desktop:zoomLevel'
 ...
 * but forgot to emit 'poormad:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('poormad:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `poormad:zoom:changed`, which never fired —
