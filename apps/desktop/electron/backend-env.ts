...
/**
 * PoorMad-managed Node.js directories, in preferred lookup order.
 *
 * There are two on-disk layouts. `scripts/install.ps1` unpacks portable Node
 * straight into `%LOCALAPPDATA%\poormad\node` (node.exe at the root, no `bin\`);
 * `scripts/install.sh` and the node-bootstrap helper use the POSIX
 * `$POORMAD_HOME/node/bin`. Emit BOTH on every platform so mixed and migrated
 ...
 * `iter_poormad_node_dirs()` in poormad_constants.py, which the Electron main
 * process cannot import.
 */
function poormadManagedNodePathEntries(
  poormadHome,
  ...
  const root = pathModule.join(poormadHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const poormadNodeDirs = poormadManagedNodePathEntries(poormadHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([poormadNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizePoorMadHomeRoot(poormadHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!poormadHome) {
    return poormadHome
  }

  const resolved = pathModule.resolve(String(poormadHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). poormad_bootstrap sets this inside the child too, but only
