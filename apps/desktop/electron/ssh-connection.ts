...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(POORMAD_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-PoorMad-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.poormad/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'poormad-desktop-ssh')
  }

  return path.join(os.homedir(), '.poormad', 'desktop-ssh')
}
