...
test('Windows spawn holds the update mutex across marker check and helper spawn', () => {
  const command = atomicWindowsSpawnCommand({
    poormadHome: 'C:\\Users\\andre\\.poormad',
    python: 'C:\\Users\\andre\\.poormad\\python.exe'
  })

  const encoded = command.match(/-EncodedCommand\s+([^\s]+)$/)?.[1]
  const script = encoded ? Buffer.from(encoded, 'base64').toString('utf16le') : ''
  assert.match(script, /\.poormad-update-in-progress/)
...
test('Windows spawn publishes the initial ownership record before releasing the mutex', () => {
  const command = atomicWindowsSpawnCommand(
    ...
    },
    {
      ...
      profile: 'default',
      poormadPath: 'C:\\PoorMad\\poormad.exe',
      poormadHome: 'C:\\Users\\andre\\.poormad',
...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command poormad.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          poormadHome: 'C:\\Users\\alice\\.poormad',
          poormadPath: 'C:\\PoorMad\\poormad.exe',
          python: 'C:\\PoorMad\\python.exe'
        })
      }

      if (script.includes('.poormad-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('poormad_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.poormad\\profiles\\research')
  assert.match(script, /\.poormad-update-in-progress/)
...
test('Windows probe validates PoorMad and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $poormadHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('PoorMad is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /PoorMad is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\PoorMad's\\python.exe" }, 'inspect', [
    'C:\\x y\\poormad.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'poormad_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /PoorMad''s/)
  assert.match(script, /C:\\x y\\poormad\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { poormadPath: lock.poormadPath, poormadHome: lock.poormadHome }
