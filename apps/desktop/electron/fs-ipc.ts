...
// hardening, POORMAD_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('poormad:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('poormad:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('poormad:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('poormad:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<POORMAD_HOME>/desktop-plugins`,
  // resolved from the main-process POORMAD_HOME (see resolvePoorMadHome) — NOT from
  // the connected backend. A remote backend reports its own `poormad_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped poormad_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(poormadHome, 'profiles', profile) : poormadHome
  ...
  ipcMain.handle('poormad:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL logs root (`<POORMAD_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where POORMAD_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('poormad:fs:logsRoot', async () => localPluginsRoot('logs'))

  // The LOCAL agent-plugin root (`<POORMAD_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('poormad:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('poormad:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('poormad:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('poormad:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('poormad:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('poormad:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
