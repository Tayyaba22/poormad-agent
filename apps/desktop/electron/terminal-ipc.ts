...
// registry, and the poormad:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // POORMAD_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.POORMAD_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip PoorMad' TUI into
    ...
    env.TERM_PROGRAM = 'PoorMad'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a poormad/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from POORMAD_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.POORMAD_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `poormad:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('poormad:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('poormad:terminal:write', (_event, id, data) => {
  ...
  ipcMain.handle('poormad:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('poormad:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('poormad:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
