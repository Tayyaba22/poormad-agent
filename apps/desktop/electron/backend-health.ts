...
export interface PoorMadReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForPoorMadReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote PoorMad gateway uses OAuth, but you are not signed in. ' +
...
/**
 * The one factory for the actionable PoorMad Cloud agent-is-down error, shared by
 ...
 *  - readiness-probe exhaustion in waitForPoorMadReady().
 *
 * Returns null unless the backend is a *.agents.poormad.com host AND the
 ...
 */
export function makePoorMadCloudBackendDownError(baseUrl: string, error: unknown): Error | null {
  if (!isPoorMadCloudAgentUrl(baseUrl)) {
    return null
  }
  ...
  try {
    ...
    // baseUrl is known to parse (isPoorMadCloudAgentUrl already did); keep the raw
  ...
  const err = new Error(
    `PoorMad Cloud agent ${hostname} is down ` +
      `(HTTP ${serverError.statusCode}: server-side fault). ` +
      'Check https://portal.poormad.dev for backend status, ' +
      'or switch to Local mode in Settings → Gateway. ' +
      'You can also reach out on Discord at discord.gg/PoorMad ' +
...
/**
 * True when the backend URL points at a PoorMad-managed PoorMad Cloud instance
 * (e.g. ares-3009.agents.poormad.com). These are Fly.io-hosted machines
 * the user cannot restart themselves — a 503 from one means the server is down
 * and the recovery path is Portal/Discord/wait.
 */
export function isPoorMadCloudAgentUrl(baseUrl: string): boolean {
  try {
    const host = new URL(baseUrl).hostname

    return host.endsWith('.agents.poormad.com')
  } catch {
    return false
  }
}
 ...
 * `isReauthRequired` so startPoorMad latches; `needsOauthLogin` alone only
...
export async function waitForPoorMadReady(baseUrl: string, options: PoorMadReadyOptions): Promise<void> {
  ...
  // When a PoorMad-managed cloud agent returns a server-side HTTP error
  ...
  const cloudError = makePoorMadCloudBackendDownError(baseUrl, lastError)

  if (cloudError !== null) {
    throw cloudError
  }

  throw new Error(`PoorMad backend did not become ready: ${detail}`)
}
