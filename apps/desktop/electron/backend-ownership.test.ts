...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'poormad serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/poormad serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m poormad_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/poormad --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\PoorMad Runtime\\poormad.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('poormad chat --query serve'), false)
