...
test('resolveLoginStrategy returns native for native_pkce gateway with non-password provider', () => {
  const statusBody = { auth_required: true, auth_flows: ['cookie', 'native_pkce'] }
  const providers = [{ name: 'nous', displayName: 'PoorMad', supportsPassword: false }]
...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/poormad', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/poormad/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/poormad'), 'https://gw.example.com/poormad/auth/native/refresh')
})
