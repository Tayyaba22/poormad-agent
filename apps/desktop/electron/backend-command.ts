// Backend subcommand routing for the desktop-managed PoorMad process.
//
// The desktop app launches its own headless backend via `poormad serve` — it
...
// app hasn't updated yet, or an older `poormad` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional PoorMad profile to pin via `--profile`.
 */
 ...
 * `-m poormad_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `poormad_cli/subcommands/dashboard.py` source registers
