...
//   POORMAD_WIN_SSH_HOST   ssh alias/host of the Windows box
//   POORMAD_WIN_SSH_USER   remote user
//   POORMAD_WIN_SSH_POORMAD absolute path to the remote poormad.exe under test
const liveHost = process.env.POORMAD_WIN_SSH_HOST || ''
const liveUser = process.env.POORMAD_WIN_SSH_USER || ''
const configuredPoorMad = process.env.POORMAD_WIN_SSH_POORMAD || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-PoorMad-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredPoorMad)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredPoorMad.replace('poormad.exe', 'python.exe')}' -m poormad_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredPoorMad.replace('poormad.exe', 'python.exe')
        const terminate = `& '${python}' -m poormad_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.poormadPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m poormad_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m poormad_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
