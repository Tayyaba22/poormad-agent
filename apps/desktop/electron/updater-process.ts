 ...
 *
 * The staged Tauri `poormad-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `poormad update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `poormad update`, swaps/relaunches
 * the app, and writes .poormad-update-result.json. With the app gone before
 * the update starts, the POORMAD_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `poormad update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into POORMAD_HOME on *every* platform
 * (`poormad-setup.exe` on Windows, `poormad-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_poormad_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `poormad update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(poormadHome, 'poormad-setup.exe')
 ...
 *
 * `copy_self_to_poormad_home` deliberately no-ops during `--update`
 ...
 * "Another PoorMad update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `poormad update`, which
 ...
 * the installer is written to POORMAD_HOME at install/repair time, making mtime
