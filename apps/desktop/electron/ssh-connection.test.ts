...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env POORMAD_DASHBOARD_SESSION_TOKEN=abc123deadbeef POORMAD_DESKTOP=1 poormad dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /POORMAD_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /POORMAD_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-PoorMad-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-PoorMad-Session-Token: hdr_888'), /X-PoorMad-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-PoorMad-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('poormad', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v poormad', 15000)
  assert.equal(args[args.length - 1], 'command -v poormad')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `poormad-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `poormad-ssh-test-${process.pid}-${Date.now()}`)
