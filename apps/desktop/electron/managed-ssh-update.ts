 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `poormad update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.poormadHome, 'PoorMad home')
  const poormadPath = validateRemoteValue(target.poormadPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(poormadPath)

  const updateCommand =
    `env POORMAD_HOME=${homeWord} ` +
    `POORMAD_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'POORMAD_UPDATE_ORIGIN_PROFILE=default ' +
    `POORMAD_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `POORMAD_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:POORMAD_HOME=${psLiteral(home)}`,
    `$env:POORMAD_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:POORMAD_UPDATE_ORIGIN_PROFILE="default"',
    `$env:POORMAD_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:POORMAD_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:POORMAD_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:POORMAD_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:POORMAD_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(poormadPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.poormad-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote PoorMad updated and every managed SSH profile is ready.'
        : restoreOk
