export type ParentWatchdogEnv = {
  POORMAD_PARENT_PID: string
  POORMAD_PARENT_START_MARKER?: string
  POORMAD_PARENT_NONCE?: string
  /** Spawn tag consumed by poormad_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { POORMAD_PARENT_PID: String(pid) }
  env.POORMAD_SPAWN = spawnTag(pid, startMarker)
  ...
  env.POORMAD_PARENT_START_MARKER = startMarker
  env.POORMAD_PARENT_NONCE = nonce

  return env
}

/** Build the `POORMAD_SPAWN` tag mirrored by poormad_cli/process_identity.py.
