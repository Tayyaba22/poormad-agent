 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote PoorMad dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the PoorMad install on the remote (login-shell probe),
 ...
 *     `POORMAD_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForPoorMad in.
 *
 * The minted POORMAD_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^POORMAD_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.poormad/desktop-ssh'
...
// Resolve the remote poormad executable. An EXPLICIT path is honored strictly
...
async function locatePoorMad(ssh, remotePoorMadPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The poormad binary or wrapper script
    ...
    //     the PoorMad version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/Tayyaba22/poormad-agent/issues/74411
    return candidate
  }
  ...
  if (remotePoorMadPath) {
    if (await isExecutable(remotePoorMadPath)) {
      return resolveLauncher(remotePoorMadPath)
    }

    const err: any = new Error(
      `The PoorMad path you set is not an executable on the remote host: "${remotePoorMadPath}". ` +
        'Check the path (it must be the full path to the `poormad` binary on the remote, e.g. ' +
        '~/poormad-agent/.venv/bin/poormad), or clear it to auto-detect.'
    )

    err.kind = 'poormad-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v poormad')}`)).trim()
  ...
  candidates.push('~/.local/bin/poormad')
  candidates.push('/usr/local/bin/poormad')
  candidates.push('~/.poormad/poormad-agent/venv/bin/poormad')
  ...
  const err: any = new Error(
    'PoorMad is not installed on the remote host (could not find a `poormad` executable). ' +
      'Install it on the remote with:  curl -fsSL https://poormad.dev/install.sh | sh  ' +
      '— or set the PoorMad path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<poormad> --version`,
// e.g. "PoorMad v0.18.2 ..."), or '' on failure. Surfaces WHICH poormad a
// connection uses, so a stale/unexpected install is visible.
async function probePoorMadVersion(ssh, poormadPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(poormadPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". PoorMad Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The POORMAD_HOME the remote dashboard will use (explicit env wins, else
// ~/.poormad). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemotePoorMadHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${POORMAD_HOME:-$HOME/.poormad}"')).trim().split('\n').pop()

    return out || '~/.poormad'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote PoorMad home.')
...
marker=home/'.poormad-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing PoorMad checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, poormadHome) {
  const home = assertSafeRemoteHome(poormadHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote PoorMad install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote PoorMad update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote PoorMad update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemotePoorMadProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemotePoorMadHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote PoorMad profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote PoorMad home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'poormadPath', 'poormadHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !poormadPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(poormadPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/poormad`, leaving neither
      // launcher nor POORMAD_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `poormad_home=os.path.expanduser(${shq(poormadHome)}) if ${shq(poormadHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if poormad_home:\n' +
      ' expected_entries.add(os.path.join(poormad_home,"poormad-agent","venv","bin","poormad"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.poormadPath)
  const expectedHome = lock.poormadHome ? expandRemotePath(lock.poormadHome) : "''"
  ...
  const executableMatch = lock.poormadHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.poormadPath === expected.poormadPath &&
    lock.poormadHome === expected.poormadHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.poormadPath)})
poormad_home=os.path.expanduser(${py(lock.poormadHome)})
expected_entries={expected_path,os.path.join(poormad_home,"poormad-agent","venv","bin","poormad")}
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"poormad-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(poormadPath, profile, opts: any = {}) {
  const poormad = expandRemotePath(poormadPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.poormadHome || '~/.poormad')}/.poormad-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.poormadHome || '~/.poormad')}/.poormad-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env POORMAD_DESKTOP=1 ${poormad} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} poormad-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, poormadPath) {
  const poormad = expandRemotePath(poormadPath)

  const out = await ssh.exec(
    `help="$(${poormad} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ssh,
  { poormadPath, profile, token, ownershipId, poormadHome = '~/.poormad', assertInstallClear = async () => {} }
) {
  if (!(await remoteSupportsSshOwnership(ssh, poormadPath))) {
    const err: any = new Error(
      'The remote PoorMad install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update PoorMad on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(poormadPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        poormadHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForPoorMad, a token-gated
...
async function connect(deps) {
  ...
  const poormadHome = await probeRemotePoorMadHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, poormadHome)
  const poormadPath = await locatePoorMad(ssh, remotePoorMadPath)
  log(`located poormad at ${poormadPath}`)
  const poormadVersion = await probePoorMadVersion(ssh, poormadPath)

  if (poormadVersion) {
    log(`remote poormad version: ${poormadVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this PoorMad Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.poormadPath === poormadPath &&
      lock.poormadHome === poormadHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    ownershipId,
    poormadHome,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, poormadHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForPoorMad(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
