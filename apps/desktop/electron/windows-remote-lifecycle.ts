...
const READY_RE = /^POORMAD_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitPoorMadPath = '') {
  const explicit = psLiteral(explicitPoorMadPath)
  ...
  const script = [
    ...
    '$poormadHome=$env:POORMAD_HOME',
    'if(-not $poormadHome){$poormadHome=Join-Path $env:LOCALAPPDATA "poormad"}',
    'Assert-NoReparse $poormadHome $true',
    '$candidate=[IO.Path]::Combine($poormadHome, "poormad-agent\\venv\\Scripts\\poormad.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "poormad-agent\\.venv\\Scripts\\poormad.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $poormadHome "poormad-agent\\venv\\Scripts\\poormad.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "poormad-agent\\.venv\\Scripts\\poormad.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command poormad.exe -ErrorAction SilentlyContinue',
    ...
    '$poormad=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$poormad=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $poormad){throw "PoorMad is not installed on the remote Windows host."}',
    'Assert-NoReparse $poormad $false',
    'if($explicit -and $poormad -ne $explicit){throw "The configured PoorMad path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($poormad), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;poormadHome=$poormadHome;poormadPath=$poormad;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(poormadHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class PoorMadMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(poormadHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".poormad-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[PoorMadMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, poormadHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(poormadHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitPoorMadPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitPoorMadPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'poormad_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'poormad_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'poormad_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.poormadHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".poormad-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};poormadPath=${psLiteral(reservation.poormadPath)};poormadHome=${psLiteral(reservation.poormadHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.poormadPath === 'string' &&
    typeof lock.poormadHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.poormadPath === runtime.poormadPath &&
    lock.poormadHome === runtime.poormadHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remotePoorMadPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.poormadHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.poormadPath])

  if (!inspection.supported) {
    const error: any = new Error('Update PoorMad on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.poormadPath = inspection.path
  const poormadVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located poormad at ${runtime.poormadPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, poormadPath: runtime.poormadPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForPoorMad(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="PoorMad SSH"', 'powershell.exe -NoLogo')
