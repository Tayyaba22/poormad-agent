...
describe('media protocol helpers', () => {
  ...
  it('preserves a configured gateway path prefix', () => {
    const endpoint = new URL(remoteMediaEndpoint('https://gateway.test/poormad/', '/tmp/a b.mp4'))

    expect(endpoint.pathname).toBe('/poormad/api/files/stream')
...
describe('createMediaProtocolHandler', () => {
  it('streams local media through the resolved local-file dependency', async () => {
    ...
    const response = await createMediaProtocolHandler(deps)(
      request('poormad-media://stream/%2Ftmp%2Fclip.mp4', {
        Authorization: 'Bearer renderer-secret',
        Range: 'bytes=1-3'
      })
    )
  ...
  it('preserves explicit HEAD requests through the local stream fetch', async () => {
    ...
    const response = await createMediaProtocolHandler(deps)(
      request('poormad-media://stream/%2Ftmp%2Fclip.mp4', {}, 'HEAD')
    )
  ...
  it('proxies token-auth remote media without placing the token in the URL', async () => {
    const deps = dependencies({
      resolveRemoteConnection: vi.fn(async () => ({
        authMode: 'token' as const,
        baseUrl: 'https://gateway.test/poormad',
        mode: 'remote' as const,
        token: 's e/cret'
      }))
    })
    ...
    const response = await createMediaProtocolHandler(deps)(
      request('poormad-media://remote/%2Froot%2Foutputs%2Frender.mp4?connectionId=work-ssh&profile=reviewer', {
        Range: 'bytes=0-1023'
      })
    )
    ...
    expect(url.pathname).toBe('/poormad/api/files/stream')
    expect(url.searchParams.get('path')).toBe('/root/outputs/render.mp4')
    expect(url.searchParams.has('token')).toBe(false)
    expect(headers.get('x-poormad-session-token')).toBe('s e/cret')
  ...
  it('adds profile scope when one registry backend serves multiple profiles', async () => {
    ...
    await createMediaProtocolHandler(deps)(
      request('poormad-media://remote/%2Froot%2Foutputs%2Frender.mp4?connectionId=cloud&profile=research')
    )
  ...
  it('preserves explicit HEAD requests through the token-auth remote proxy', async () => {
    ...
    const response = await createMediaProtocolHandler(deps)(
      request('poormad-media://remote/%2Froot%2Foutputs%2Frender.mp4', {}, 'HEAD')
    )
  ...
  it('rejects protocol methods other than GET and HEAD', async () => {
    ...
    const response = await createMediaProtocolHandler(deps)(
      request('poormad-media://remote/%2Froot%2Foutputs%2Frender.mp4', {}, 'POST')
    )
  ...
  it('uses a refreshed native bearer for OAuth remote media when available', async () => {
    ...
    const response = await createMediaProtocolHandler(deps)(request('poormad-media://remote/%2Ftmp%2Fclip.mp4'))
  ...
  it('preserves explicit HEAD requests through the native-bearer remote fetch', async () => {
    ...
    const response = await createMediaProtocolHandler(deps)(
      request('poormad-media://remote/%2Ftmp%2Fclip.mp4', {}, 'HEAD')
    )
  ...
  it('fails closed for unsupported extensions and missing remote auth', async () => {
    ...
    expect((await handler(request('poormad-media://remote/%2Ftmp%2Fsecret.txt'))).status).toBe(415)
    expect((await handler(request('poormad-media://remote/%2Ftmp%2Fclip.mp4'))).status).toBe(401)
    expect(deps.fetchRemote).not.toHaveBeenCalled()
  })
})
