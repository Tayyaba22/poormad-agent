...
test('uses lightweight /api/health for current backends', async () => {
  ...
  await waitForPoorMadReady('http://127.0.0.1:9000/', {
    token: 'secret-token',
...
test('does not fall back to heavyweight /api/status for transient health failures', async () => {
  ...
  await assert.rejects(
    waitForPoorMadReady('http://127.0.0.1:9000', {
      fetchPublicJson: async url => {
        calls.push(['public', url])
        throw new Error('Timed out connecting to PoorMad backend after 15000ms')
      },
      fetchJson: async url => {
        calls.push(['token', url])
      },
...
test('recognizes missing-route shapes only', () => {
  ...
  assert.equal(
    isMissingHealthEndpointError(
      new Error('Expected JSON from /api/health but got HTML. The endpoint is likely missing on the PoorMad backend.')
    ),
    true
  )
  assert.equal(isMissingHealthEndpointError(new Error('Timed out connecting to PoorMad backend after 15000ms')), false)
...
test('anonymous gate-shaped 401 falls back to /api/status (backend predates /api/health)', async () => {
  ...
  await waitForPoorMadReady('http://192.168.1.132:9119', {
    token: null,
    fetchPublicJson: async url => {
      calls.push(['public', url])
...
test('a credentialed 401 fails fast for reauth instead of reporting a dead session ready', async () => {
  ...
  await assert.rejects(
    waitForPoorMadReady('https://gateway.example', {
      token: 'session-token',
      fetchPublicJson: async () => {
...
test('unsigned OAuth is a terminal reauth failure; needsOauthLogin alone is not', () => {
  // The unsigned-in throw must set isReauthRequired so startPoorMad latches.
  ...
  assert.equal(isReauthRequiredError(new Error('Could not reach the remote PoorMad gateway')), false)
})
...
test('a credentialed probe still uses the 404 fallback for a genuinely missing route', async () => {
  ...
  await waitForPoorMadReady('https://gateway.example', {
    token: 'session-token',
    fetchPublicJson: async () => {
      throw new Error('public probe must not be used when credentialed')
    },
...
test('isPoorMadCloudAgentUrl detects cloud agent hosts', () => {
  // Positive cases
  assert.equal(isPoorMadCloudAgentUrl('https://ares-3009.agents.poormad.com'), true)
  assert.equal(isPoorMadCloudAgentUrl('https://ares-3009.agents.poormad.com/api/health'), true)
  assert.equal(isPoorMadCloudAgentUrl('http://test.agents.poormad.com'), true)

  // Negative cases
  assert.equal(isPoorMadCloudAgentUrl('http://127.0.0.1:9000'), false)
  assert.equal(isPoorMadCloudAgentUrl('https://gateway.example.com'), false)
  assert.equal(isPoorMadCloudAgentUrl('https://poormad.com'), false)
  assert.equal(isPoorMadCloudAgentUrl('not-a-url'), false)
})

test('waitForPoorMadReady surfaces actionable error for cloud agent 503', async () => {
  let attempts = 0
  const currentTime = { value: 0 }

  try {
    await waitForPoorMadReady('https://ares-3009.agents.poormad.com', {
      fetchPublicJson: async () => {
        attempts++
        // Always return 503
    ...
    assert.ok(error.message.includes('PoorMad Cloud agent'), `unexpected message: ${error.message}`)
    assert.ok(error.message.includes('503'), `should mention status code: ${error.message}`)
    assert.ok(error.message.includes('portal.poormad.dev'), `should mention portal: ${error.message}`)
    assert.ok(error.message.includes('discord.gg/PoorMad'), `should mention Discord: ${error.message}`)
...
test('waitForPoorMadReady does not cloud-wrap non-cloud 503 errors', async () => {
  const currentTime = { value: 0 }
...
test('makePoorMadCloudBackendDownError produces the Cloud shape and preserves cause', () => {
  const err = new Error('upstream unavailable') as any
  err.statusCode = 503
  const result = makePoorMadCloudBackendDownError('https://ares-3009.agents.poormad.com', err)
  assert.ok(result)
  ...
  assert.ok(result?.message.includes('PoorMad Cloud agent ares-3009.agents.poormad.com is down'))
})

test('makePoorMadCloudBackendDownError returns null for a Cloud 401 (routes to reauth)', () => {
  const err = new Error('Unauthorized') as any
  err.statusCode = 401
  assert.equal(makePoorMadCloudBackendDownError('https://ares-3009.agents.poormad.com', err), null)
})

test('makePoorMadCloudBackendDownError returns null for a non-Cloud 503 (generic remote failure)', () => {
  const err = new Error('Service Unavailable') as any
  err.statusCode = 503
  assert.equal(makePoorMadCloudBackendDownError('https://gateway.example.com', err), null)
  assert.equal(makePoorMadCloudBackendDownError('http://127.0.0.1:9000', err), null)
})

test('makePoorMadCloudBackendDownError preserves legacy string-prefix compatibility', () => {
  const result = makePoorMadCloudBackendDownError(
    'https://ares-3009.agents.poormad.com',
