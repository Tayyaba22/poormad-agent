...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'POORMAD_DESKTOP_REMOTE_URL is set but POORMAD_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote PoorMad backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `poormad serve` the desktop never
