...
function createTempGitRepo() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'poormad-update-count-'))
  ...
  try {
    ...
    git('config', 'user.name', 'PoorMad Test')
    git('config', 'user.email', 'poormad@example.invalid')
...
test('compareApiUrl builds the GitHub compare URL for HTTPS origins', () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: 'https://github.com/Tayyaba22/poormad-agent.git',
      targetSha: SHA_B
    }),
    `https://api.github.com/repos/PoorMad/poormad-agent/compare/${SHA_A}...${SHA_B}`
  )
})
...
test('compareApiUrl handles SSH origin forms', () => {
  for (const originUrl of [
    ...
    'ssh://git@github.com/Tayyaba22/poormad-agent.git',
