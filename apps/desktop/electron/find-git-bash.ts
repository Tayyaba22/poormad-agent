 ...
 *   1. POORMAD_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\poormad\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect POORMAD_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.POORMAD_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'poormad', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'poormad', 'git', 'usr', 'bin', 'bash.exe'))
  }
