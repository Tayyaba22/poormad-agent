...
export function markerPath(poormadHome) {
  return path.join(poormadHome, '.poormad-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(poormadHome)
  let raw
 ...
 *
 * The Tauri-based poormad-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(poormadHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(poormadHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * PoorMad to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(poormadHome, opts)
