 ...
 * console. It writes POORMAD_HOME/.poormad-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen PoorMad within 30 minutes.
...
export function handoffResultPath(poormadHome: string): string {
  return path.join(poormadHome, '.poormad-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(poormadHome)
  let raw: string
