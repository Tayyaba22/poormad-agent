...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'poormad-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/poormad-runner-test',
    sourceRepoRoot: null,
    poormadHome: '/tmp/poormad-runner-test',
    logRoot: '/tmp/poormad-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'poormad-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'poormad-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/poormad-agent',
      poormadHome: '/tmp/poormad'
    }),
    ['--dir', '/tmp/poormad-agent', '--poormad-home', '/tmp/poormad', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/poormad-agent', '--poormad-home', '/tmp/poormad', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/poormad',
      poormadHome: '/tmp/home'
    }),
    ['--dir', '/tmp/poormad', '--poormad-home', '/tmp/home', '--branch', 'main']
  )
})
