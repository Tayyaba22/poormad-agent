...
test('parseWorktrees: main checkout + linked worktree', () => {
  const out = [
    ...
    'HEAD def456',
    'branch refs/heads/poormad/feat',
    ''
  ].join('\n')
  ...
  assert.equal(trees[1].branch, 'poormad/feat')
})
...
test('ensureGitRepo: inits a plain dir with a root commit so worktrees branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'poormad-wt-'))
...
test('switchBranch: switches a normal checkout branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'poormad-switch-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBranches: lists locals and flags the checked-out branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'poormad-branches-'))
...
test('listBranches: flags a free default branch as default, not checked out', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'poormad-branches-default-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBranches: a branch claimed by a worktree is flagged checked out', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'poormad-branches-wt-'))
...
test('listBranches: empty on a non-repo path', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'poormad-nonrepo-'))
...
test('addWorktree: existingBranch checks the branch out without a new branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'poormad-convert-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('addWorktree: existing default branch switches the main checkout, not .worktrees/main', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'poormad-convert-default-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBaseBranches: lists local branches and flags the default', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'poormad-base-branches-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBaseBranches: empty on a non-repo path', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'poormad-base-nonrepo-'))
...
test('addWorktree: base param branches off a specified local branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'poormad-base-add-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('addWorktree: base origin/main does not set up upstream tracking', async () => {
  ...
  const remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'poormad-remote-'))
  const cloneDir = fs.mkdtempSync(path.join(os.tmpdir(), 'poormad-clone-'))
...
function seedRemoteAndClone(label, branches) {
  const remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), `poormad-${label}-remote-`))
  const cloneDir = fs.mkdtempSync(path.join(os.tmpdir(), `poormad-${label}-clone-`))
  ...
  execFileSync('git', ['init', '-b', 'main', remoteDir])
  remoteGit('-c', 'user.email=poormad@localhost', '-c', 'user.name=PoorMad', 'commit', '--allow-empty', '-m', 'root')
...
test('switchBranch: non-repo dir short-circuits instead of throwing', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'poormad-sw-'))
