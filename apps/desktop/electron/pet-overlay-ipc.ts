...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('poormad:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('poormad:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('poormad:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('poormad:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('poormad:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('poormad:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('poormad:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('poormad:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('poormad:pet-overlay:control', payload)
  })
}
