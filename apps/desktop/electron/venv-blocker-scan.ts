...
const SCAN_MODULE = 'poormad_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/PoorMad processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'poormad_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another PoorMad process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the PoorMad installation is free.\n' +
    '\n' +
    'Close other PoorMad windows and terminals, then retry.  If the problem\n' +
    'persists, run `poormad update` in a terminal for detailed diagnostics.'
  )
}
