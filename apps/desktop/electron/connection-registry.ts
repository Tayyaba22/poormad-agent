 ...
 * runtime, remote gateways, PoorMad Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@poormad/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remotePoorMadPath || '').trim() !== String(activeSsh.remotePoorMadPath || '').trim() ||
...
/** Turn `ls ~/.poormad/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. PoorMad Cloud
 * instances are platform-managed — we never run `poormad update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remotePoorMadPath: input.remotePoorMadPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/PoorMad-managed names; an empty result stores nothing.
 ...
 * `remotePoorMadPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'PoorMad Cloud' : 'Remote gateway'),
