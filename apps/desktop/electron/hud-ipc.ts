...
export function registerHudIpc({
  ...
  ipcMain.on('poormad:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('poormad:hud:windowing', event => {
  ...
  ipcMain.on('poormad:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('poormad:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('poormad:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('poormad:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('poormad:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('poormad:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('poormad:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('poormad:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('poormad:hud:close', async () => {
    closeHudWindow()
