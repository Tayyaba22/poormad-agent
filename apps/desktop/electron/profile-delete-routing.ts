// Profile-delete routing logic for the `poormad:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_poormad_home() and
...
/** Parse a `poormad:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its POORMAD_HOME.
 *
...
/**
 * Route the next `poormad:api` request away from the primary/window backend
 ...
 * ensure_poormad_home() recreates the directory the delete just removed.
 */
