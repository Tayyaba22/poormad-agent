 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of PoorMad by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.poormad-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(poormadHome) {
  return path.join(poormadHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.poormad/poormad-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(poormadHome) {
  if (!poormadHome) {
    return null
  }

  const candidate = path.join(poormadHome, 'poormad-agent', 'scripts', installScriptName())
...
function cachedScriptPath(poormadHome, commit) {
  return path.join(bootstrapCacheDir(poormadHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/PoorMad/poormad-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(poormadHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(poormadHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, poormadHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass POORMAD_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          POORMAD_HOME: poormadHome || process.env.POORMAD_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, poormadHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, poormadHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--poormad-home', poormadHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, poormadHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, poormadHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, poormadHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(poormadHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, poormadHome, emit })
