import { JsonRpcGatewayClient } from '@poormad/shared'

import type { PoorMadApiRequest } from '@/global'
...
// "Timed out connecting to PoorMad backend" that hangs the UI (#48504).
//
...
export class PoorMadGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'PoorMad gateway connection closed',
      connectErrorMessage: 'Could not connect to PoorMad gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'PoorMad gateway is not connected',
...
// process to route them to that host (poormad:api's registry branch). Null
...
export function poormadApi<T>(request: PoorMadApiRequest): Promise<T> {
  return window.poormadDesktop.api<T>({ ...connectionScoped(), ...request })
}
