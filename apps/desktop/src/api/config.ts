...
import { capabilityScoped, poormadApi, type ProfileScope, profileScoped, STARTUP_REQUEST_TIMEOUT_MS } from './client'

export function getStatus(): Promise<StatusResponse> {
  return poormadApi<StatusResponse>({
    ...profileScoped(),
    path: '/api/status'
  })
}
...
export function getLogs(params: {
  ...
  return poormadApi<LogsResponse>({
    ...profileScoped(),
    path: suffix ? `/api/logs?${suffix}` : '/api/logs'
  })
}

export function getPoorMadConfig(profile?: string): Promise<PoorMadConfig> {
  return poormadApi<PoorMadConfig>({
...
export function getPoorMadConfigRecord(profile?: ProfileScope): Promise<PoorMadConfigRecord> {
  return window.poormadDesktop.api<PoorMadConfigRecord>({
    ...capabilityScoped(profile),
    path: '/api/config'
  })
}

export function getPoorMadConfigDefaults(): Promise<PoorMadConfigRecord> {
  return poormadApi<PoorMadConfigRecord>({
    ...profileScoped(),
...
export function getPoorMadConfigSchema(profile?: null | string): Promise<ConfigSchemaResponse> {
  return poormadApi<ConfigSchemaResponse>({
    ...profileScoped(profile),
    path: '/api/config/schema'
  })
}

export function savePoorMadConfig(config: PoorMadConfigRecord, profile?: null | string): Promise<{ ok: boolean }> {
  return poormadApi<{ ok: boolean }>({
    ...profileScoped(profile),
    path: '/api/config',
    method: 'PUT',
    body: { config }
...
export function getEnvVars(profile?: null | string): Promise<Record<string, EnvVarInfo>> {
  return poormadApi<Record<string, EnvVarInfo>>({
    ...profileScoped(profile),
    path: '/api/env'
  })
}
...
export function setEnvVar(key: string, value: string, profile?: ProfileScope): Promise<{ ok: boolean }> {
  return window.poormadDesktop.api<{ ok: boolean }>({
    ...capabilityScoped(profile),
    path: '/api/env',
    method: 'PUT',
    body: { key, value }
...
export function revealEnvVar(key: string, profile?: ProfileScope): Promise<{ key: string; value: string }> {
  return window.poormadDesktop.api<{ key: string; value: string }>({
    ...capabilityScoped(profile),
    path: '/api/env/reveal',
    method: 'POST',
    body: { key }
...
export function validateProviderCredential(
  ...
  return poormadApi<{ ok: boolean; reachable: boolean; message: string; models?: string[] }>({
    ...profileScoped(),
...
export function getCustomEndpoints(): Promise<CustomEndpointsResponse> {
  return poormadApi<CustomEndpointsResponse>({
    ...profileScoped(),
...
export function validateCustomEndpoint(endpoint: CustomEndpointUpdate): Promise<CustomEndpointValidationResponse> {
  return poormadApi<CustomEndpointValidationResponse>({
...
export function activateCustomEndpoint(id: string): Promise<{ ok: boolean; provider: string; model: string }> {
  return poormadApi<{ ok: boolean; provider: string; model: string }>({
    ...profileScoped(),
...
export function listOAuthProviders(): Promise<OAuthProvidersResponse> {
  return poormadApi<OAuthProvidersResponse>({
    ...profileScoped(),
...
export function disconnectOAuthProvider(providerId: string): Promise<{ ok: boolean; provider: string }> {
  return poormadApi<{ ok: boolean; provider: string }>({
    ...profileScoped(),
...
export function startOAuthLogin(providerId: string, profile?: ProfileScope): Promise<OAuthStartResponse> {
  return window.poormadDesktop.api<OAuthStartResponse>({
    ...capabilityScoped(profile),
...
export function submitOAuthCode(providerId: string, sessionId: string, code: string): Promise<OAuthSubmitResponse> {
  return poormadApi<OAuthSubmitResponse>({
    ...profileScoped(),
...
export function pollOAuthSession(
  ...
  return window.poormadDesktop.api<OAuthPollResponse>({
    ...capabilityScoped(profile),
