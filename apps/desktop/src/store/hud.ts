 ...
 * composer with the reply scrolling above it, so PoorMad can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.poormadDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.poormadDesktop?.hud
...
export function resetHudLayout(): void {
  void window.poormadDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.poormadDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.poormadDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
