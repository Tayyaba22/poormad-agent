...
import type { PoorMadGitBaseBranch, PoorMadGitBranch } from '@/global'
...
import { getPoorMadConfig, poormadApi, type PoorMadGateway } from '@/poormad'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/poormad'
...
const PROJECT_SCOPE_KEY = 'poormad.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('PoorMad gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== projectProfile()) {
    throw new Error('Active PoorMad profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await poormadApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<PoorMadGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<PoorMadGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero PoorMad
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getPoorMadConfig(context.profile))
...
// repo, returning where PoorMad should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<PoorMadGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<PoorMadGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.poormadDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.poormadDesktop?.writeClipboard?.(path)
  }
}
