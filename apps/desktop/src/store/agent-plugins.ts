...
/**
 * Feature store for backend (agent) plugins — the native PoorMad plugins plus
 ...
 * primitives `poormad plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native PoorMad. */
  portable?: boolean
}
 ...
 *  POORMAD_HOME. Always refetches (it's a cheap local disk scan on the
