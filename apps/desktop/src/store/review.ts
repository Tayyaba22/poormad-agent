...
import type { PoorMadReviewFile, PoorMadReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — PoorMad' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'poormad.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'poormad.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'poormad.desktop.reviewTreeMode'
const SELECTED_KEY = 'poormad.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<PoorMadReviewFile[]>([])
...
export const $reviewShipInfo = atom<PoorMadReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['poormadDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: PoorMadReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly PoorMadReviewFile[], path: string): PoorMadReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.poormadDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.poormadDesktop?.openExternal?.(url)
    }
