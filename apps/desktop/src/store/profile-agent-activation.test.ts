...
const agentConn = (over: Partial<PoorMadConnection> = {}): PoorMadConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as PoorMadConnection

const localConn = (over: Partial<PoorMadConnection> = {}): PoorMadConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as PoorMadConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<PoorMadConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<PoorMadConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { poormadDesktop: { getConnection, getConnectionFor } })
})
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<PoorMadConnection>(() => undefined))
