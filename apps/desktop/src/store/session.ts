import type { ConnectionState } from '@poormad/shared'
...
import type { SessionInfo, UsageStats } from '@/types/poormad'
...
const COMPOSER_MODEL_KEY = 'poormad.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'poormad.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'poormad.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'poormad.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'poormad.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'poormad.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'poormad.desktop.lastRoute'
...
function workspaceCwdKey(connection: PoorMadConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.poormadDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.poormadDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<PoorMadConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'poormad.desktop.sessionOwnerHints.v1'
...
export const setConnection = (next: Updater<PoorMadConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by usePoorMadConfig.
