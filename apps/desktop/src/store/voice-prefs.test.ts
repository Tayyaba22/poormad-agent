...
vi.mock('@/poormad', () => ({
  getPoorMadConfigRecord: vi.fn(async () => ({})),
  savePoorMadConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye poormad', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye poormad')
  })
