...
export function setZoomPercent(percent: number): void {
  window.poormadDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.poormadDesktop?.zoom) {
  void window.poormadDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.poormadDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
