 ...
 * app's bounds and stays visible while PoorMad is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'poormad.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'poormad.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.poormadDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.poormadDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.poormadDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.poormadDesktop?.petOverlay?.close()
}
