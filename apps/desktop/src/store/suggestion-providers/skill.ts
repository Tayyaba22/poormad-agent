 ...
 *  Working in `~/www/poormad-agent`, the draft says "poormad-agent" constantly
 *  — a "Use skill: poormad-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`poormad-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/poormad-agent-dev`, `plugin:skill`). */
