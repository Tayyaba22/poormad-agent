import { type ConnectionState, type GatewayEvent, registryBackendScopeKey, resolveGatewayWsUrl } from '@poormad/shared'
import { atom } from 'nanostores'

import type { PoorMadConnection } from '@/global'
import { reconnectBackoffDelayMs } from '@/lib/reconnect-backoff'
import { RECONNECT_ATTEMPT_TIMEOUT_MS, withTimeout } from '@/lib/with-timeout'
import { PoorMadGateway, setApiRequestConnection } from '@/poormad'
...
const isOpen = (gateway: PoorMadGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: PoorMadConnection) => void
  /**
...
interface Secondary {
  ...
  connection: PoorMadConnection | null
  gateway: PoorMadGateway
...
interface GatewayRegistryState {
  ...
  primaryGateway: PoorMadGateway | null
  ...
  $gateway: ReturnType<typeof atom<PoorMadGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('poormad.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<PoorMadGateway | null>(null),
...
export function setPrimaryGateway(gateway: PoorMadGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<PoorMadConnection, 'connectionId'> | null): void {
...
export function activeGateway(): PoorMadGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the poormad module (null
...
function publishActiveConnection(connection: PoorMadConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new PoorMadGateway()
...
async function gatewayForProfile(
  profile: string,
  leaseRequest = false
): Promise<{ gateway: PoorMadGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`PoorMad gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.poormadDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update PoorMad Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<PoorMadGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "PoorMad gateway is not connected" on the Sessions `+` action (#88880).
