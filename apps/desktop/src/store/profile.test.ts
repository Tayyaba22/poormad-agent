...
const { getProfiles } = await import('@/poormad')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/poormad/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<PoorMadConnection> = {}): PoorMadConnection =>
  ({ baseUrl: 'https://poormad-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as PoorMadConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { poormadDesktop: { getConnection } })
