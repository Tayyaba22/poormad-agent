...
import { LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@poormad/shared'
...
const TILES_KEY = 'poormad.desktop.sessionTiles.v2'
const LEGACY_TILES_KEY = 'poormad.desktop.sessionTiles.v1'
...
export const $focusedStoredSessionId = computed(
  ...
  (groupId, tree, selected, workspaceMode) => {
    ...
    // it to the sidebar group, whose active pane ('poormad-bots:pane') is not a
...
// Dev hook for automation (mirrors __POORMAD_LAYOUT_TREE__).
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__POORMAD_SESSION_TILES__ = {
