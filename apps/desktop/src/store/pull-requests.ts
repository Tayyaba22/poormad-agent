...
import type { PoorMadBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/poormad'
...
export const $pullRequestsByBranch = atom<Record<string, PoorMadBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'poormad.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('poormad.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: PoorMadBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
