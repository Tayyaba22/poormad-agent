...
// reaches the gateway); everything else routes through window.poormadDesktop.git,
...
function file(path: string, over: Partial<PoorMadReviewFile> = {}): PoorMadReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as PoorMadReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.poormadDesktop. Any op not supplied defaults
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as PoorMadReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.poormadDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: PoorMadReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as PoorMadReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { poormadDesktop?: unknown }).poormadDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as PoorMadReviewShipInfo)
