...
import type { PoorMadGitWorktree, PoorMadRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: PoorMadGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, PoorMadRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, PoorMadGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<PoorMadRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<PoorMadGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<PoorMadRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<PoorMadGitWorktree[]>>()
const $noRepoStatus = atom<PoorMadRepoStatus | null>(null)
const $noWorktrees = atom<PoorMadGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<PoorMadRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<PoorMadGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: PoorMadRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: PoorMadGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<PoorMadRepoStatus | null>
  seq: number
  target: string
}
