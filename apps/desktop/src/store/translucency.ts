 ...
 * `@poormad/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-poormad-glass]`
 * block in styles.css).
 */
...
} from '@poormad/shared/translucency'
 ...
 * sets `poormadDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.poormadDesktop?.glassSupported === 'boolean'
    ? window.poormadDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.poormadDesktop?.translucencySupported === 'boolean'
    ? window.poormadDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-poormad-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-poormad-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-poormad-glass-on', glassLive)
  root.toggleAttribute('data-poormad-glass', glassOn)
  root.toggleAttribute('data-poormad-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-poormad-glass-scope', scope)
  } else {
    root.removeAttribute('data-poormad-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.poormadDesktop?.setTranslucency?.(state)
  })
