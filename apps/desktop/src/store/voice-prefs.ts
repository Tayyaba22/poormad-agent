...
import { getPoorMadConfigRecord, savePoorMadConfig } from '@/poormad'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getPoorMadConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await savePoorMadConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
