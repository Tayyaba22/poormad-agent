...
// (collect + force-redact + PoorMad-S3 upload) → the modal shows the private
// view link plus the support handoff (GitHub Issues · PoorMad Portal Support ·
// Discord).
//
// Consent is per-upload and explicit — no "always allow", mirroring the CLI's
// `poormad debug share --nous` confirmation contract. On a remote connection
...
interface SharePoorMadResponse {
  error?: string
  expires_at?: string
  ok: boolean
  upload_id?: string
...
async function collectLocalExtras(): Promise<Record<string, string>> {
  try {
    const logs = await window.poormadDesktop?.getRecentLogs?.()
...
export async function confirmSendDiagnostics(): Promise<void> {
  ...
  try {
    ...
    const response = await gateway.request<SharePoorMadResponse>(
