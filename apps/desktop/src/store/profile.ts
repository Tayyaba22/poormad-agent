import { LOCAL_CONNECTION_ID } from '@poormad/shared'
...
import type { ProfileInfo } from '@/types/poormad'
...
// /api/profiles/active `current`). "default" is the root ~/.poormad. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'poormad.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'poormad.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await poormadApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new POORMAD_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.poormadDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<PoorMadConnection | null> {
  const getConnection = window.poormadDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<PoorMadConnection | null> {
  const getConnectionFor = window.poormadDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'poormad.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.poormadDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.poormadDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.poormad) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.poormadDesktop?.touchBackend?.(target).catch(() => undefined)
}
