 ...
 *
 * The archive is the CLI's own `poormad profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/poormad'
...
import type { ProfileDesktopOverlay } from '@/types/poormad'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'PoorMad profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.poormadDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.poormadDesktop?.selectPaths?.({
