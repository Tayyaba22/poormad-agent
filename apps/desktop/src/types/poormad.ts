...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when PoorMad doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in poormad_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `poormad model` uses — instead of
...
export interface PoorMadConfig {
  agent?: {
...
export type PoorMadConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `poormad model` CLI / onboarding OAuth flow. */
  auth_type?: string
  ...
  /** PoorMad only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** PoorMad only: paid models a free-tier user cannot select (shown disabled). */
...
// non-desktop) PoorMad simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (poormad.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors poormad_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with PoorMad, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `poormad tools` active-provider detection). */
  is_active: boolean
  /** Honest readiness computed server-side (keys ∧ PoorMad entitlement ∧
 ...
 *  not PoorMad); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** poormad-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** poormad-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/poormad/update/check` — the backend's own update state.
...
/** One PoorMad-approved MCP catalog entry from `GET /api/mcp/catalog`. */
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the PoorMad Tool Gateway as a result of
   *  switching the main provider to PoorMad. Empty unless provider === 'nous'
