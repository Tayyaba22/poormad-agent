...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to PoorMad gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  const original = window.poormadDesktop
  Object.defineProperty(window, 'poormadDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config, ...overrides }
  })

  return () => Object.defineProperty(window, 'poormadDesktop', { configurable: true, value: original })
}
...
describe('BootFailureOverlay', () => {
  ...
  it('recovers a cloud connection through the portal cascade instead of native OAuth', async () => {
    const gatewayUrl = 'https://agent-1.agents.poormad.com'
    const logout = vi.fn().mockResolvedValue({ ok: true, connected: false })
    const nativeLogin = vi.fn().mockResolvedValue({ ok: true, connected: false })
    const cloudStatus = vi.fn().mockResolvedValue({ portalBaseUrl: 'https://portal.poormad.dev', signedIn: false })

    const cloudLogin = vi.fn().mockResolvedValue({
      ok: true,
      portalBaseUrl: 'https://portal.poormad.dev',
      signedIn: true
    })
  ...
  it('shows the PoorMad Cloud down recovery when the backend flags isCloudBackendDown', async () => {
    const restore = stubDesktop(remoteToken)
    $desktopBoot.set({
      error: 'PoorMad Cloud agent ares-3009.agents.poormad.com is down (HTTP 503: server-side fault).',
      fakeMode: false,
    ...
    try {
      ...
      // remote-failure copy.
      expect(await screen.findByText(/PoorMad Cloud agent is down/i)).toBeTruthy()
      ...
      expect(screen.getByText(/ares-3009\.agents\.poormad\.com/i)).toBeTruthy()
    } finally {
      restore()
    }
  })
