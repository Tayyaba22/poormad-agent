...
import { getGlobalModelOptions } from '@/poormad'
...
import type { ModelOptionProvider, OAuthProvider } from '@/types/poormad'
...
// PoorMad Portal OAuth), ahead of OpenRouter and the rest of the key catalog.
const API_KEY_OPTIONS: ApiKeyOption[] = [
  ...
  },
  {
    ...
    docsUrl: 'https://github.com/Tayyaba22/poormad-agent#bring-your-own-endpoint',
    placeholder: 'http://127.0.0.1:8000/v1'
  }
]

// Build the FULL API-key provider catalog from the backend model options so the
// onboarding / Providers key form lists every `api_key` provider `poormad model`
...
const SHOW_ALL_KEY = 'poormad-onboarding-show-all-v1'
...
export function Picker({ ctx }: { ctx: OnboardingContext }) {
  ...
  // Collapse the secondary providers behind a disclosure whenever PoorMad Portal
        ...
        {showRest ? (
          <>
            {/* Fireworks leads the expanded list, matching CANONICAL_PROVIDERS
                (PoorMad → Fireworks), but stays hidden until the user opens it. */}
