...
export function BootFailureOverlay() {
  ...
  const retry = async () => {
    setBusy('retry')
    await window.poormadDesktop?.resetBootstrap().catch(() => undefined)
    window.location.reload()
  }

  const repair = async () => {
    setBusy('repair')
    await window.poormadDesktop?.repairBootstrap().catch(() => undefined)
    window.location.reload()
  }
  ...
  const switchToLocalGateway = async () => {
    ...
    await window.poormadDesktop?.applyConnectionConfig({ mode: 'local' }).catch(() => undefined)
    setBusy(null)
  }

  // Clear this gateway's stale auth first, then re-establish it through the
  // connection's owning login flow. PoorMad Cloud must reuse its portal session
  ...
  const openLogs = () => void window.poormadDesktop?.revealLogs().catch(() => undefined)
  ...
  let hint: string
  // The electron boot path flags a PoorMad Cloud backend-down (502/503/504) with
  ...
  if (remoteReauth) {
    ...
    // A PoorMad Cloud agent is down — the user cannot restart the managed
    ...
    actions = [
      {
        ...
        onClick: () => openExternalLink('https://portal.poormad.dev'),
        icon: <ExternalLink />
      },
      localAction,
      { ...retryAction, variant: 'secondary' },
      {
        ...
        onClick: () => openExternalLink('https://discord.gg/PoorMad'),
        variant: 'ghost'
      },
