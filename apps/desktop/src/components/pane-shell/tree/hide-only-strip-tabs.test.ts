...
function sessionsBotsTree() {
  ...
  registerPane('poormad-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'poormad-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('poormad-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('poormad-bots:pane')
    expect($hiddenStripTabs.get()).toContain('poormad-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('poormad-bots:pane')

    expect(setStripTabHidden('poormad-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('poormad-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('poormad-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('poormad-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('poormad.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('poormad-bots:pane')
    ...
    revealTreePane('poormad-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('poormad-bots:pane')
    expect(window.localStorage.getItem('poormad.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'poormad-bots:pane', title: 'poormad-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'poormad-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
