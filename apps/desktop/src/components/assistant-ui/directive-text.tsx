...
const POORMAD_REF_TYPES = WIRE_REFERENCE_KINDS
type PoorMadRefType = (typeof POORMAD_REF_TYPES)[number]
...
const POORMAD_DIRECTIVE_RE = referenceRe()
...
export const poormadDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(POORMAD_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...poormadDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing PoorMad directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.poormadDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.poormadDesktop?.readFileDataUrl(id)
