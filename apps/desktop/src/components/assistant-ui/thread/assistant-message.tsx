...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's POORMAD_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.poormadDesktop?.logsRoot?.()
      ...
      const result = await window.poormadDesktop?.openDir?.(root)
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {window.poormadDesktop?.logsRoot && (
