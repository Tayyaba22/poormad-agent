...
describe('delivery command detection', () => {
  it('matches the canonical delivery command', () => {
    const cmd = 'poormad -p turqoise chat --in ~ -c "Bot Chat" -Q -q "Message from 🤖 PoorMad (@poormad): hi there"'

    expect(deliveryTargetFromCommand(cmd)).toBe('turqoise')
  })

  it('matches with a cd prefix and timeout wrapper', () => {
    const cmd = 'cd ~ && timeout 240 poormad -p mr-tester chat --in "~" -Q -q "Message from 🤖 PoorMad: hello"'
  ...
  it('ignores ordinary terminal commands', () => {
    ...
    expect(deliveryTargetFromCommand('poormad -p turqoise chat -q "plain question"')).toBeNull()
    expect(deliveryTargetFromCommand('poormad sessions list')).toBeNull()
  })
})

describe('reply extraction', () => {
  it('strips session_id bookkeeping and keeps the reply', () => {
    const output = 'session_id: 20260813_220347_f69ac6\nHi PoorMad! Good to hear from you.'

    expect(replyTextFromResult({ output })).toBe('Hi PoorMad! Good to hear from you.')
  })
