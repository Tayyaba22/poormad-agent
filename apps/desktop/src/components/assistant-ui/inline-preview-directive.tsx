 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.poormad.send(prompt)` (or declarative
 * `data-poormad-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'poormad-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'poormad-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `poormad.send(prompt)`.
 ...
 *  wires `data-poormad-send` so declarative HTML works with zero script:
 *  `<button data-poormad-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.poormad={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-poormad-send]"):null;' +
    'if(el)send(el.getAttribute("data-poormad-send")||"")},true)})()</script>'
  )
}
...
function InlineHtmlFrame({
  ...
  useEffect(() => {
    ...
    void Promise.resolve(window.poormadDesktop?.readFileText(path))
      .then(result => {
        if (!alive) {
          return
        }
