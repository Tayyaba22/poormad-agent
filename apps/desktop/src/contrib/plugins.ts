 ...
 *    a `PoorMadPlugin` registers automatically (vite glob — drop a folder in).
 *    `poormad-bots` (Bot Mode) ships in-tree and is ON by default; other
 *    reference/demo plugins live in the companion `poormad-example-plugins`
 ...
 *  - RUNTIME: the on-disk doors (`<poormad home>/desktop-plugins/<name>/plugin.js`
 *    and the unified-package half `<poormad home>/plugins/<name>/desktop/plugin.js`)
...
const modules = import.meta.glob<{ default: PoorMadPlugin }>('../plugins/*/plugin.{js,ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default PoorMadPlugin export — skipped`)
