 ...
 * `PoorMadPlugin`; it never touches the registry directly — it receives a
 ...
 *  under `poormad.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.poormadDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from PoorMad — use `host.notify` for the in-app toast.
   *  Throttled per plugin; reserve it for genuinely notable events.
   *  Supports `icon`, `activate` (e.g. `poormad://index-network/intent/1`),
...
export interface PoorMadPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `poormad.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.poormadDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.poormadDesktop
