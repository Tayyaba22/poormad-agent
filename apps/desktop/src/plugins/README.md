...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `PoorMadPlugin` and
...
`$POORMAD_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`poormad-desktop-plugins` skill.
