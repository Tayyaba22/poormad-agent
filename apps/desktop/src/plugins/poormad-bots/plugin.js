/**
 * PoorMad Bot Mode — a "one chat per agent" roster for the PoorMad desktop.
 *
 * Left pane "Bots": one row per PoorMad profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (PoorMad cron jobs) scoped to the
 ...
 * bot-initiated sends use `poormad -p <bot> chat --in ~ -c "Bot Chat"`.
 */

import * as sdk from '@poormad/plugin-sdk'
import {
  atom,
  Button,
  Checkbox,
...
const BOT_ATTENTION_HINTS = {
  ...
  missing_config: 'Provider not configured — run poormad model',
...
/** Mirrors host.paneVisibility('poormad-bots:pane') — wired in register(). */
...
const GROUP_CHAT_SYNC_META_KEY = 'poormad-bots-groups'
 ...
 *  clients (PoorMad Go, headless backends) see rooms regardless of which
...
// to (local, remote URL, SSH, PoorMad Cloud, docker) must be able to find
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['poormad-bots']?.title || profile?.display_name || ''),
...
async function saveBotMeta(owner, patch) {
  ...
  try {
    ...
    const request = route ? requestForBot(bot, 'profiles.configure', { name, ui_meta: { 'poormad-bots': rest } }) :
      host.request('profiles.configure', { name, ui_meta: { 'poormad-bots': rest } })
...
// handoffs (`poormad -p <bot> chat --in ~ -c "Bot Chat" --create-if-missing`)
  ...
  for (const bot of roster) {
    ...
    if (image && typeof image === 'string' && image.startsWith('data:')) {
      ...
      Promise.resolve(request)
        .then(() => queryClient.invalidateQueries({ queryKey: ['poormad-bots', 'roster'] }))
...
function mergeServerMeta(roster, fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['poormad-bots']
...
/** Permanently delete a bot's PoorMad profile, then remove plugin-local state
 ...
 * directory (poormad-agent#52279). That is the "can't delete a bot" error. */
...
if (typeof document !== 'undefined' && !document.getElementById('poormad-bots-roster-css')) {
  const style = document.createElement('style')
  style.id = 'poormad-bots-roster-css'
  style.textContent =
    '.poormad-bots-roster [data-radix-scroll-area-viewport] > div {' +
    ' display: block !important; width: 100%; min-width: 0; }' +
    '.poormad-scroll-cap > [data-radix-scroll-area-viewport] { max-height: inherit; }' +
    '@keyframes poormad-bots-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.35; } }' +
    '.poormad-bots-pulse { animation: poormad-bots-pulse 1.2s ease-in-out infinite; }'
...
// and the row falls back to the "run poormad mcp / Settings" hint. profile is
...
function McpSetupButton({ profile, entry, onDone, ensureProfile }) {
  ...
  const beginOAuth = async () => {
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.poormadDesktop && window.poormadDesktop.openExternal) {
        window.poormadDesktop.openExternal(authUrl)
      } else {
      ...
      tab === 'generate'
        ? imagen
          ...
          : jsx('div', {
              ...
              children:
                imagen === false
                  ? 'No image model available. If you just enabled one (or updated PoorMad), restart the gateway: Ctrl+K → "Restart gateway".'
...
function PetTab({ image, onImage }) {
  ...
  if (!pets.length) {
    return jsx('div', {
      ...
      children: 'No pets in the petdex gallery. Run `poormad pets` to explore.'
    })
  }
 ...
 *  protocol into the system prompt itself (poormad-agent bot_mode_probe).
...
function useRoster() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Multi-source desktops (poormad-agent #86875) also expose the union
 ...
 *  callable alias is 'poormad' — the mention middleware resolves it back to
...
function botHandle(name, bot) {
  ...
  return (name || '').trim().toLowerCase() === 'default' ? 'poormad' : name
}

/** Taggable @-forms derived from a bot's friendly names — the core profile
 *  display name (`poormad profile rename`) and the Bot Mode title. Free text
 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "PoorMad"
 *  can never hijack the primary profile's @poormad alias. */
function mentionNameForms(value) {
  ...
  return [...new Set([slug, collapsed])].filter(
    form => /^[a-z0-9][a-z0-9_-]*$/.test(form) && !['all', 'everyone', 'user', 'default', 'poormad'].includes(form)
  )
}
...
function botFriendlyNames(bot) {
  ...
  return [bot?.ui_meta?.['poormad-bots']?.title, localTitle, aliasTitle, bot?.title, bot?.display_name]
}
...
function newBotChat(bot) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({ kind: 'error', message: 'Update PoorMad Desktop to open another Bot chat.' })
...
function resolveRosterMentions(text, roster, active = {}) {
  ...
  for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)/gi)) {
    ...
    if (token === 'poormad') {
      token = byForm.has('poormad') ? 'poormad' : token
    }
...
// raw Cloud hostname, or to generic 'PoorMad' in Cloud-only mode.
//
...
// grandfather, or recover. Stored-id pins (ui_meta['poormad-bots'].chat) were
...
async function openStoredBotChat(owner, storedId, summary) {
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This PoorMad Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (poormad-agent#89617) — clicking Retry
  ...
  // until an app restart (poormad-agent#93604). A resume is cheap and
...
function createCanonicalChat(owner, { kickoff = false, openingStillCurrent = null } = {}) {
  ...
  const run = (async () => {
    ...
    const res = await requestForBot(bot, 'session.create', {
      ...
      // stale provider — e.g. "out of PoorMad credits" — after a profile switch).
...
async function prepareBotSource(bot) {
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error('Update PoorMad Desktop to chat with agents on other connections.')
  }
...
async function ensureBotMetadata(bot) {
  ...
  const server = row?.ui_meta?.['poormad-bots']
...
function displayName(bot, meta) {
  ...
  // "PoorMad". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `poormad profile rename
  ...
  // never generic "PoorMad" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as PoorMad (the agent it is)
...
function durableGroupChatMembers(bots) {
  return (bots || []).map(bot => {
    ...
    const title = String(botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['poormad-bots']?.title || bot.title || '').trim()
...
// turn in its OWN persistent per-group PoorMad session and is fed only the
 ...
 *  renamed bot never shows up as its raw profile id or a stale "PoorMad"
 *  (community report, Aug 21 2026: renamed default still read "PoorMad is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as PoorMad (matching displayName and the
 *  @poormad handle) so the main agent never loses its name in rooms. */
function groupSpeakerLabel(name) {
  ...
  // Core profile display_name (`poormad profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'PoorMad' : trimmed
}
...
function messagingProtocolSection(name, roster) {
  ...
  return [
    ...
    '',
    '```',
    'poormad -p <agent-name> chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from \uD83E\uDD16 ' + handle + ' (@' + handle + '): your message"',
    '',
    ...
    '',
    'The roster grows over time — run `poormad profile list` for the LIVE',
...
function previewKind(preview) {
  ...
  if (match) {
    ...
    // profile reads @poormad, never @default (#89484).
 ...
 *  (poormad-agent#90268). Older gateways omit worker_session — always false. */
...
function AdvancedProfileConfig({ bot, state, setState }) {
  ...
  if (unsupported) {
    return jsx('div', {
      className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
      children: 'Full configuration needs a newer gateway (restart it after updating PoorMad).'
    })
  }
  ...
  // (poormad-agent#87317): Skills (installed list + one-click hub installs +
...
// https://poormad.dev/docs/skills?embed=picker hides the
// docs chrome and adds "+ Add to this Agent" per card, posting
// {type: 'poormad-skill-pick', ...} to us (poormad-agent#86243). We validate
...
const HUB_ORIGIN = 'https://poormad.dev'
...
function HubSkillsSection({ forProfile, onInstalled }) {
  ...
  useEffect(() => {
    ...
    const onMessage = event => {
      ...
      if (!data || data.type !== 'poormad-skill-pick' || !data.name) {
        return
      }
  ...
  const ensureAgentCreated = () => {
    ...
    const flight = singleFlight(flightRef, async () => {
      ...
      if (remoteTarget) {
        ...
        try {
          void requestForTarget('profiles.configure', { name: slug, ui_meta: { 'poormad-bots': look } }).catch(() => undefined)
                    ...
                    advTab === 'general'
                      ...
                      : capsFailed
                        ? jsx('div', {
                            className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
                            children:
                              'Capability catalog needs a newer gateway (restart it after updating PoorMad).'
                          })
                        : !caps
                          ? jsx('div', {
                              className: 'flex justify-center py-4',
...
// poormad -p <bot> chat delegation wrapper so the run reaches that bot's
...
function routinePrompt(bot, title, instruction, activeProfile) {
  ...
  return (
    ...
    `poormad -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// PoorMad-native schedule string; Advanced exposes it raw.
const FREQUENCIES = [
...
/** Compose the PoorMad schedule string from picker state. */
...
function GroupChatWorkspace({ group, members, onBack, visible = true }) {
  ...
  const renderEntry = (entry, index) => {
                  ...
                  // primary "default" profile renders as PoorMad, remote dupes
...
function BotsHomeView() {
  ...
  const gateway = bot.connectionLabel || (bot.connectionId === 'local' ? 'This device' : 'PoorMad gateway')
...
function GroupRow({ active, group, members, needsYou, onOpen, onDisband }) {
  ...
  // and the group prompt: the primary profile is @poormad, not @default.
...
function BotsPane() {
  ...
  const activityOf = bot => {
    const created = botRosterMeta(bot, allMeta)?.created || bot.ui_meta?.['poormad-bots']?.created || 0
        ...
        : error && !roster.length
          ? jsxs('div', {
              ...
              children: [
                jsx('div', {
                  children: gatewayUp
                    ? `Roster unavailable: ${error instanceof Error ? error.message : 'gateway error'}. If your gateway predates profiles.list, update PoorMad and restart the gateway.`
          ...
          : roster.length === 0
            ...
            : allBotsHidden && !hiddenExpanded
              ...
              : rosterRows.length === 0 && matchingHiddenBots.length === 0
                ...
                : jsx(ScrollArea, {
                    className: 'poormad-bots-roster min-h-0 flex-1',
                    children: jsx('div', {
        ...
        description: deleting
          ? jsxs('span', {
              children: [
                ...
                ' and its associated PoorMad profile at ',
...
export default {
  ...
  register(ctx) {
    ...
    // excluded (a bot doesn't @ itself); 'default' surfaces as @poormad.
    ctx.register({
    ...
    if (!document.getElementById('poormad-bots-keyframes')) {
      const style = document.createElement('style')
      style.id = 'poormad-bots-keyframes'
      style.textContent = '@keyframes poormad-bots-bob { from { transform: translateY(0); } to { transform: translateY(-3px); } }'
      document.head.appendChild(style)
    }
            ...
            try {
              const registry =
                typeof window !== 'undefined'
                  ? await Promise.resolve(window.poormadDesktop?.connections?.list?.()).catch(() => null)
                  : null
    ...
    try {
      if (typeof window !== 'undefined') {
        unbindConnectionsChanged =
          window.poormadDesktop?.connections?.onChanged?.(payload => {
