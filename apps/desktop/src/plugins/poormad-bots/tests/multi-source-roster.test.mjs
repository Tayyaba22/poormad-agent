...
function runtime() {
  ...
  const code = source
    .replace(/^import \* as sdk from '@poormad\/plugin-sdk'\r?\n/m, '')
...
test('default rows use source identity without borrowing another source title', () => {
  ...
  // label. Titled: the title wins. Untitled: it stays "PoorMad". Regression:
  ...
  assert.equal(name(active, undefined), 'PoorMad')
})
...
test('botHandle: precomputed multi-source handle wins; default stays poormad', () => {
  ...
  assert.equal(botHandle('default'), 'poormad')
})
...
test('displayName: local default stays PoorMad; remote default uses the device label', () => {
...
test('resolveRosterMentions: @poormad in this chat is not a handoff to yourself', () => {
  const { __resolveRosterMentions: resolve } = runtime()
  const roster = [
    { name: 'default', connectionId: 'local', handle: 'poormad' },
    {
      name: 'default',
      connectionId: 'mac-mini',
      connectionLabel: 'Mac Mini',
  ...
  const hits = resolve('ask @poormad and @default-mac-mini', roster, {
    name: 'default',
    connectionId: 'local'
  })
