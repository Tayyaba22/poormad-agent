...
// Bot Mode title or `poormad profile rename` display_name — must update what
...
test('mentionNameForms: slugged + collapsed, reserved tokens dropped', () => {
  ...
  // A bot renamed "PoorMad" or "everyone" cannot hijack reserved tags.
  assert.equal(mentionNameForms('PoorMad').length, 0)
...
test('botMentionTag: renamed slug wins, profile handle is the fallback', () => {
  ...
  assert.equal(botMentionTag({ name: 'default' }), 'poormad')
  // display_name (poormad profile rename) drives the tag too.
...
test('parseGroupChatMentions: display_name and ui_meta title forms address a member', () => {
  ...
  const members = [
    ...
    { name: 'ops', title: '', ui_meta: { 'poormad-bots': { title: 'Night Watch' } } }
  ]
