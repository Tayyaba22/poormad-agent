...
import { pathFromPoorMadDeepLink, resolvePoorMadOpenPath } from '@/lib/poormad-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.poormadDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.poormadDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.poormadDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.poormadDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // poormad://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.poormadDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future poormadDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolvePoorMadOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.poormadDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (poormad://index-network/intent/1, poormad://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromPoorMadDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.poormadDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.poormadDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.poormadDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.poormadDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
