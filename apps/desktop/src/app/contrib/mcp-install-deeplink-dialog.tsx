...
import { getPoorMadConfigRecord, saveMcpServers } from '@/poormad'
...
import { setPoorMadConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `poormad://mcp/install` deep links. The payload is
 ...
 * confirming lets PoorMad spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getPoorMadConfigRecord())
      ...
      setPoorMadConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
