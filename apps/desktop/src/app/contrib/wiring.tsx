...
import { getLatestSessionMessages } from '@/poormad'
...
import { usePoorMadConfig } from '../session/hooks/use-poormad-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshPoorMadConfig, sttEnabled, voiceMaxRecordingSeconds } = usePoorMadConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('poormad:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('poormad:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshPoorMadConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshPoorMadConfig])
