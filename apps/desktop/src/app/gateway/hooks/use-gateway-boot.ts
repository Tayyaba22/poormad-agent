import { isGatewayReauthRequired, JsonRpcGatewayError, resolveGatewayWsUrl } from '@poormad/shared'
import { useEffect, useRef } from 'react'

import { shouldApplyPostBootProgressError } from '@/components/boot-failure-reauth'
import type { PoorMadConnection } from '@/global'
import { PoorMadGateway } from '@/poormad'
...
import type { RpcEvent } from '@/types/poormad'
...
export function primaryRuntimeConnectionId(connection: Pick<PoorMadConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.poormadDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: PoorMadGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: PoorMadConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting PoorMad...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting PoorMad…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to PoorMad backend'
        )
        ...
        // as an opaque "Could not connect to PoorMad gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshPoorMadConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshPoorMadConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startPoorMad re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new PoorMadGateway()
    ...
    async function boot() {
      try {
        ...
        // hanging "Starting PoorMad…" forever.
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshPoorMadConfig(),
          ...
          // retry it), not brick boot behind the "PoorMad couldn't start"
