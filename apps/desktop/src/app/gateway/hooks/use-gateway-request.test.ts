...
vi.mock('@/poormad', async importOriginal => {
  const actual = await importOriginal<typeof PoorMadModule>()

  class FakePoorMadGateway {
  ...
  return { ...actual, PoorMadGateway: FakePoorMadGateway }
})
...
const fakeGateway = { connectionState: 'open' } as unknown as PoorMadGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as PoorMadGateway, 'default')
  $gateway.set(primary as unknown as PoorMadGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.poormadDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as PoorMadGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { poormadDesktop: unknown }).poormadDesktop = { getConnection }
    $gateway.set(dropped)
