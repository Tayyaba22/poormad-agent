...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as PoorMadGateway | null
  const gatewayRef = useRef<PoorMadGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.poormadDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<PoorMadGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as PoorMadGateway | null
      }),
    []
  )
