...
import type { SessionInfo } from '@/poormad'
...
// zero PoorMad sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, PoorMadGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, PoorMadGitWorktree[]>>({})
