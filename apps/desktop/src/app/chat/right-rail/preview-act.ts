...
const preamble = () => `  var w = window;
  ...
  w.__poormadActHolder = w.__poormadActHolder || {};
  w.__poormadAct = ${actEngineSource()};
  w.__poormadWatch_fn = (${watchInPage.toString()});
  w.__poormadWatchTag = ${WATCH_TAG};
  var holder = w.__poormadActHolder;
  var act = function (a) { return w.__poormadAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__poormadWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__poormadHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'POORMAD-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
