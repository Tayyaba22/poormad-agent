...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__poormadTourEngine) {
    ${driverIife}
    w.__poormadTourHolder = {};
    w.__poormadTourCollect = (${collectTourTargets.toString()});
    w.__poormadTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__poormad-tour-style')) {
    var style = document.createElement('style');
    style.id = '__poormad-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__poormadTourEngine(
    w.driver.js.driver,
    w.__poormadTourHolder,
    ${JSON.stringify(action)},
    w.__poormadTourCollect,
    document
  ));
})()`
}
