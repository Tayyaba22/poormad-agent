        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.poormadDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask PoorMad to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.poormadDesktop?.watchPreviewFile ||
      !window.poormadDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.poormadDesktop.onPreviewFileChanged(payload => {
    ...
    void window.poormadDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.poormadDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.poormadDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:poormad-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.poormadDesktop?.zoom?.factor?.() || 1
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.poormadDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.poormadDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.poormadDesktop?.openExternal(currentUrl)
                : undefined
            }
