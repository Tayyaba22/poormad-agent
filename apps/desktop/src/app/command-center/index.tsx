...
import { getActionStatus, getLogs, getStatus, getUsageAnalytics, restartGateway, updatePoorMad } from '@/poormad'
import type { ActionStatusResponse, AnalyticsResponse, StatusResponse } from '@/poormad'
...
export function CommandCenterView({ initialSection, onClose, onDeleteSession, onOpenSession }: CommandCenterViewProps) {
  ...
  // Client-side substring filter over the fetched tail (matches `poormad logs --search`).
  ...
  const runSystemAction = useCallback(
    async (kind: 'restart' | 'update') => {
      ...
      try {
        const started = kind === 'restart' ? await restartGateway() : await updatePoorMad()
                    ...
                    <div className="flex items-start justify-between gap-3 max-[47.5rem]:flex-col max-[47.5rem]:gap-2">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                          {cc.poormadActiveSessions(status.version, status.active_sessions)}
                        </div>
                      </div>
