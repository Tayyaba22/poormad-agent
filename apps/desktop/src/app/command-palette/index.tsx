...
import { getPoorMadConfigRecord, listAllProfileSessions } from '@/poormad'
...
// PoorMad session ids: <YYYYMMDD>_<HHMMSS>_<6 hex>. Used to offer a direct
...
function CommandPaletteBody({ onExited }: { onExited: () => void }) {
  ...
  const configQuery = useQuery({
    ...
    queryFn: () => getPoorMadConfigRecord()
  })
        ...
        : []),
      {
        ...
        items: [
          ...
          },
          {
            ...
            icon: Download,
            id: 'cc-update-poormad',
            keywords: ['update', 'upgrade', 'poormad', 'version', 'system', 'restart'],
            label: cc.updatePoorMad,
  ...
  const searchGroups = useMemo<PaletteGroup[]>(() => {
    ...
    // Apply a theme directly from the root search (e.g. "nous" → PoorMad). Live
