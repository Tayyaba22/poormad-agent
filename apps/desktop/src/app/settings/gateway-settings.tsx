...
// PoorMad Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const setKeychainEncryption = async (on: boolean) => {
    ...
    try {
      const res = await window.poormadDesktop.setSecretStorageEncryption(on)
  ...
  // --- PoorMad Cloud (cloud mode) state ---
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.poormadDesktop
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.poormadDesktop?.sshConfigHosts) {
      return
    }
  ...
  const payload = (allowPlainTextToken?: boolean) => ({
    ...
    sshRemotePoorMadPath: state.sshRemotePoorMadPath.trim(),
  ...
  const performSave = async (apply: boolean, allowPlainTextToken: boolean) => {
    ...
    try {
      const next = apply
        ? await window.poormadDesktop.applyConnectionConfig(payload(allowPlainTextToken))
        : await window.poormadDesktop.saveConnectionConfig(payload(allowPlainTextToken))
      ...
      const errors = {
        ...
        'poormad-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.poormadDesktop.saveConnectionConfig({
        mode: state.mode,
      ...
      const result = await window.poormadDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.poormadDesktop.getConnectionConfig(null)
  ...
  const signOut = async () => {
    ...
    try {
      await window.poormadDesktop.oauthLogoutConnectionConfig(trimmedUrl)
      const refreshed = await window.poormadDesktop.getConnectionConfig(null)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.poormadDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.poormadDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.poormadDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.poormadDesktop.testConnectionConfig({
        mode: 'remote',
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
        remoteUrl: trimmedUrl
  ...
  if (!window.poormadDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* PoorMad Cloud panel: one portal sign-in, then a discovered-agent picker
                ...
                {cloudDiscover === 'loading' ? (
                  ...
                  <div className="flex items-start gap-2 py-3 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                    ...
                    <span>
                      ...
                      <ExternalLink href="https://portal.poormad.dev/agents" showExternalIcon={false}>
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/poormad"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemotePoorMadPath: event.target.value }))}
                placeholder={g.sshPoorMadPathPlaceholder}
                value={state.sshRemotePoorMadPath}
              />
            }
            description={g.sshPoorMadPathDesc}
            title={g.sshPoorMadPathTitle}
          />
        </div>
      ) : null}
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          ...
          <ListRow
            action={
              <Button onClick={() => void window.poormadDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
