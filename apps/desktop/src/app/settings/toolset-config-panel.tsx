 ...
 * server-computed `status` (keys ∧ PoorMad entitlement ∧ post-setup install
 ...
 * log inline — the GUI equivalent of the install step `poormad tools` runs
...
/**
 * Backend model catalog — the GUI counterpart of the model picker `poormad
...
export function ToolsetConfigPanel({ toolset, onConfiguredChange, profile }: ToolsetConfigPanelProps) {
  ...
  // Guard the PoorMad Portal sign-in poll loop against unmount/state updates.
  ...
  // panel highlighted the first keyless provider (e.g. PoorMad Portal) even when
  ...
  async function handleSelect(provider: ToolProvider) {
    ...
    try {
      ...
      if (result.needs_nous_auth) {
        // Managed PoorMad row selected without Portal entitlement: the config
        ...
        // sign-in action). Reuses the existing PoorMad Portal device-code flow.
        notify({
          kind: 'warning',
          title: copy.nousAuthNeededTitle,
          message: copy.nousAuthNeededMessage(provider.name),
          action: { label: copy.nousAuthSignIn, onClick: () => void signInToPoorMadPortal() }
        })
  ...
  // Drive the existing PoorMad Portal OAuth device-code flow (the same session
  ...
  async function signInToPoorMadPortal() {
    try {
