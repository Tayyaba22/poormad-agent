...
import type { ToolsetConfig } from '@/types/poormad'
...
const getPoorMadConfigRecord = vi.fn()
const getPoorMadConfigSchema = vi.fn()
const savePoorMadConfig = vi.fn()
...
vi.mock('@/poormad', () => ({
  ...
  getPoorMadConfigRecord: () => getPoorMadConfigRecord(),
  getPoorMadConfigSchema: () => getPoorMadConfigSchema(),
  savePoorMadConfig: (config: unknown) => savePoorMadConfig(config),
...
beforeEach(() => {
  ...
  getPoorMadConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getPoorMadConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
  savePoorMadConfig.mockResolvedValue({ ok: true })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(savePoorMadConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = savePoorMadConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
  ...
  describe('readiness pills', () => {
    it('renders the server status instead of assuming keyless rows are Ready', async () => {
      // The false-Ready bug: a logged-out PoorMad Subscription row and a
  ...
  describe('managed PoorMad provider activation', () => {
    ...
    it('surfaces a sign-in notice when the PUT reports needs_nous_auth', async () => {
      ...
      selectToolsetProvider.mockResolvedValue({
        ok: true,
        name: 'browser',
        provider: 'PoorMad Subscription (Browser Use cloud)',
      ...
      // The single PoorMad row auto-expands; activate via the explicit button.
      await screen.findByRole('button', { name: /PoorMad Subscription/ })
      fireEvent.click(await screen.findByRole('button', { name: /Use this backend/ }))

      await waitFor(() =>
        expect(selectToolsetProvider).toHaveBeenCalledWith('browser', 'PoorMad Subscription (Browser Use cloud)')
      )
      await waitFor(() =>
    ...
    it('drives the existing PoorMad OAuth device-code flow from the sign-in action and refetches', async () => {
      const { notify } = await import('@/store/notifications')
      ...
      startOAuthLogin.mockResolvedValue({
        ...
        session_id: 'sess-1',
        user_code: 'NOUS-1234',
        verification_url: 'https://portal.poormad.dev/device?user_code=NOUS-1234',
        poll_interval: 5,
        expires_in: 600
      })
