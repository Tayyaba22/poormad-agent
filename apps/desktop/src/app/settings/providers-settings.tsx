...
import { disconnectOAuthProvider, listOAuthProviders } from '@/poormad'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/poormad'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.poormadDesktop?.terminal)
...
//      in poormad_cli/provider_catalog.py) — the SAME provider identity
//      `poormad model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // PoorMad can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider PoorMad can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // PoorMad never deletes creds another tool owns behind a silent API call.
