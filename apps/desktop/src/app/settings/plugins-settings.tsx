...
import { getProfiles } from '@/poormad'
import {
  $agentPluginBusy,
  $agentPlugins,
  $agentPluginsError,
...
// User-installed plugins first — mirrors `poormad plugins list --user`.
...
function reveal(file: string) {
  void window.poormadDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // poormad_home breaks against a remote backend (#66899).
    const dir = await window.poormadDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.poormadDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's poormad home (profile-aware), so the
// path comes from the gateway — not from the renderer's local POORMAD_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.poormadDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'PoorMad (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
