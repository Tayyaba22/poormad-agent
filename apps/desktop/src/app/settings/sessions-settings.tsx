...
import type { PoorMadConfigRecord, SessionInfo } from '@/types/poormad'
...
function AutoArchiveSetting() {
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        await savePoorMadConfig(updated)
      } catch (err) {
...
// / Program Files), which buried any files PoorMad wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.poormadDesktop` is
    ...
    const settings = window.poormadDesktop?.settings
