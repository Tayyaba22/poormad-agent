...
import type { ConfigFieldSchema, PoorMadConfigRecord } from '@/types/poormad'

import { poormadConfigCacheWriter, usePoorMadConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<PoorMadConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = usePoorMadConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => poormadConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['poormad-config-schema'] : ['poormad-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getPoorMadConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await savePoorMadConfig(config, scopeProfile)
  ...
  const applyConfig = (next: PoorMadConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: PoorMadConfigRecord) => {
