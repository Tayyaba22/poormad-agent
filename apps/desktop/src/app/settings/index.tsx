...
import { getPoorMadConfigDefaults, getPoorMadConfigRecord, savePoorMadConfig } from '@/poormad'
...
export function SettingsView({ onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getPoorMadConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'poormad-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    ...
    try {
      await savePoorMadConfig(await getPoorMadConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
