...
beforeEach(() => {
  ...
  getPoorMadConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
...
describe('ModelSettings', () => {
  it('loads the current main model and lists configured providers only', async () => {
    ...
    // "PoorMad" shows in both the trigger and the open list.
    expect((await screen.findAllByText('PoorMad')).length).toBeGreaterThan(0)
  ...
  it('replaces the selected provider and model when the active profile changes', async () => {
    ...
    await waitFor(() => expect(screen.getAllByRole('combobox')[0].textContent).toContain('PoorMad'))
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getPoorMadConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(savePoorMadConfig).toHaveBeenCalledWith(
...
describe('ModelSettings MoA preset editor', () => {
  ...
  it('does not clear the model or save when the same provider is re-selected', async () => {
    ...
    try {
      ...
      fireEvent.click(slotSelects().ref1Provider)
      fireEvent.click(await screen.findByRole('option', { name: 'PoorMad' }))
      await vi.advanceTimersByTimeAsync(700)
