...
import { savePoorMadConfig } from '@/poormad'
import { useI18n } from '@/i18n'
import { notifyError } from '@/store/notifications'
import type { PoorMadConfigRecord } from '@/types/poormad'

import { setPoorMadConfigCache, usePoorMadConfigRecord } from '../hooks/use-config-record'
...
function fontFamilyFromConfig(config: PoorMadConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig } = usePoorMadConfigRecord()
  ...
  const [staleConfig, setStaleConfig] = useState<PoorMadConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setPoorMadConfigCache, so
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-3">
            <Input
              ...
              className="flex-1"
              disabled={draft === null}
              list="poormad-terminal-font-families"
          ...
          <datalist id="poormad-terminal-font-families">
