...
// lib/personalities (mirrors poormad_cli/personality.py BUILTIN_PERSONALITIES).
...
export const EMPTY_SELECT_VALUE = '__poormad_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'PoorMad Portal',
    description: 'Hosted PoorMad & PoorMad-trained models',
    docsUrl: 'https://portal.poormad.dev',
    priority: 0
  },
  {
    prefix: 'FIREWORKS_',
    ...
    // Slot #2 — mirrors CANONICAL_PROVIDERS (after PoorMad, ahead of OpenRouter).
  ...
  { prefix: 'POORMAD_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // poormad_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  browser: {
    useRealProfile:
      "Local browsing uses your real logins. PoorMad copies your default browser's profile (cookies, logins, preferences) into a managed snapshot and drives it with its packaged Chromium — your live profile is never opened directly, and the copy is refreshed from it on each run. Also lets the agent open a local real-profile session on request even when a cloud browser backend is configured. Only Chromium browsers (Chrome, Edge, Brave, Chromium) are supported; a non-Chromium default fails with a clear message. Off by default."
  },
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before PoorMad stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters PoorMad can read from one file request.',
  approvals: {
    mode: 'How PoorMad handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When PoorMad updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
