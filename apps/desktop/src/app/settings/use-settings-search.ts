...
import { getEnvVars, getPoorMadConfigSchema } from '@/poormad'
...
import { usePoorMadConfigRecord } from '../hooks/use-config-record'
...
export function useSettingsSearchCatalog(enabled: boolean) {
  const { t } = useI18n()
  const configQuery = usePoorMadConfigRecord()
