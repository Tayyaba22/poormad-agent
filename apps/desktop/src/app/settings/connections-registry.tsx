 ...
 * agent sources (local runtime + any number of remote gateways / PoorMad Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.poormadDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.poormadDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `poormad update` to every eligible source; per-connection results
