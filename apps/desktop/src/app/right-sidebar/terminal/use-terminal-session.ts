...
const POORMAD_PATHS_MIME = 'application/x-poormad-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(POORMAD_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(POORMAD_PATHS_MIME)
  ...
  const getPath = window.poormadDesktop?.getPathForFile
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.poormadDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (poormad --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.poormadDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[poormad-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.poormadDesktop?.terminal?.write(sessionId, `${command}\r`)
