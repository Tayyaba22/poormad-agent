...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { poormadDesktop: { readDir: typeof readDir } }).poormadDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): PoorMadReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { poormadDesktop: unknown }).poormadDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: PoorMadReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<PoorMadReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: PoorMadReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: PoorMadReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: PoorMadReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<PoorMadReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.poormadDesktop is missing', async () => {
    delete (window as unknown as { poormadDesktop?: unknown }).poormadDesktop
