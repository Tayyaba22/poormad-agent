...
import type { PoorMadReadDirEntry, PoorMadReadDirResult } from '@/global'
...
export type ProjectTreeEntry = PoorMadReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: PoorMadReadDirEntry) {
...
async function filterIgnored(entries: PoorMadReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<PoorMadReadDirResult> {
  if (!window.poormadDesktop) {
