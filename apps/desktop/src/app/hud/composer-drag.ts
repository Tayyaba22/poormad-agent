...
function setWorkspaceTransfer(transferring: boolean): void {
  window.poormadDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}
 ...
 * (see poormad:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  useEffect(() => {
    ...
    const onMove = (event: PointerEvent) => {
      ...
      window.poormadDesktop?.hud?.moveBy?.({
        x: dx,
        y: dy,
        width: state.originW,
