...
import type { SkillInfo, ToolsetInfo } from '@/types/poormad'
...
export function SkillsView({
  ...
  const registryBridge = window.poormadDesktop?.connections
  const rosterBridge = window.poormadDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'PoorMad (default)' : p.name,
      value: p.name
    }))
