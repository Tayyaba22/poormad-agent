import type { BillingBlock } from '@poormad/shared'
 ...
 * billing-specific toast — never the generic "PoorMad error" — with a smart CTA
 * (PoorMad → in-app Settings → Billing, other providers → their billing page).
 */
function surfaceBillingBlock(sessionId: string, raw: unknown): void {
  ...
  notify({
    ...
    icon: 'credit-card',
    title: block.is_nous
      ? translateNow('billingBlock.titlePoorMad')
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'PoorMad reported an error',
