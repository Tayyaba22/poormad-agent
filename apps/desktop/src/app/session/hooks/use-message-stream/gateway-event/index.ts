import { registryBackendScopeKey } from '@poormad/shared'
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshPoorMadConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshPoorMadConfig — two REST calls (config + defaults) per event, per
