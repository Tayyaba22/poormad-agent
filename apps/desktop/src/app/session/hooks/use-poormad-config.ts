...
interface PoorMadConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function usePoorMadConfig({ activeSessionIdRef }: PoorMadConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshPoorMadConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getPoorMadConfig(), getPoorMadConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying PoorMad' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshPoorMadConfig, sttEnabled, voiceMaxRecordingSeconds }
}
