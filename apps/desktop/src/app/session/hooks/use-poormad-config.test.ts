...
import { usePoorMadConfig } from './use-poormad-config'

vi.mock('@/poormad', () => ({
  getPoorMadConfig: vi.fn(),
  getPoorMadConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'poormad.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getPoorMadConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getPoorMadConfig>>)

describe('usePoorMadConfig refreshPoorMadConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to PoorMad' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => usePoorMadConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshPoorMadConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => usePoorMadConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getPoorMadConfig>>>()
    vi.mocked(getPoorMadConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => usePoorMadConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshPoorMadConfig(true)
    })
    expect(getPoorMadConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getPoorMadConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getPoorMadConfig>>>()
    vi.mocked(getPoorMadConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => usePoorMadConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshPoorMadConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getPoorMadConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getPoorMadConfig>>>()
    vi.mocked(getPoorMadConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshPoorMadConfig(true)
      refreshC = result.current.refreshPoorMadConfig(true)
    })
