/**
 * @poormad/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__POORMAD_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, poormadApi, type PoorMadGateway } from '@/poormad'
import {
  $gateway,
...
import type { PaginatedSessions, UsageStats } from '@/types/poormad'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend PoorMad profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.poormadDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.poormadDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and PoorMadGateway.connect() has
...
export const host = {
  ...
  connections: async () => {
    const bridge = window.poormadDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update PoorMad Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.poormadDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update PoorMad Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update PoorMad Desktop to open another Bot chat.' })
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('PoorMad Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return poormadApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return poormadApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('PoorMad gateway unavailable')
    }
...
/** PoorMad' reasoning levels + their compact labels, so a plugin surfacing a
...
export type { RpcEvent, StatusResponse } from '@/types/poormad'
