...
vi.mock('@/poormad', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), poormadApi: vi.fn() }))
...
const { deleteProfile, poormadApi } = await import('@/poormad')
...
describe('connection-aware plugin host APIs', () => {
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { poormadDesktop: unknown }).poormadDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(poormadApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(poormadApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(poormadApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
