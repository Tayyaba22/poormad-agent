...
const SKIN_KEY = 'poormad-desktop-theme-v2'
const MODE_KEY = 'poormad-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'poormad-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'poormad-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'poormad-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.poormadTheme = skinName
  root.dataset.poormadMode = rendered
  ...
  window.poormadDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('poormad-boot-background', chromeBg)
    window.localStorage.setItem('poormad-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.poormadThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.poormadDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (PoorMad authoring/activating a skin from a
