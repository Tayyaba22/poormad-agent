/**
 * Live skin sync from the PoorMad backend.
 *
 * The backend resolves the active skin (built-in or `$POORMAD_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (PoorMad
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { PoorMadSkin } from '@poormad/shared/skin'
 ...
 */
export function ingestBackendSkin(skin: PoorMadSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as PoorMadSkin)
