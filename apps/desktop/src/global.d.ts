import type { GatewayWsUrlResult } from '@poormad/shared'
import type { TranslucencyState } from '@poormad/shared/translucency'
...
declare global {
  interface Window {
    poormadDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<PoorMadConnection>
      ...
      // Resume this session in the user's own terminal emulator (`poormad --tui
      ...
      // PoorMad Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new POORMAD_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: PoorMadApiRequest) => Promise<T>
      notify: (payload: PoorMadNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<PoorMadReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<PoorMadReadFileTextResult>
      selectPaths: (options?: PoorMadSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<PoorMadPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<PoorMadPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<PoorMadPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: PoorMadActiveWork) => void
      setTitleBarTheme?: (payload: PoorMadTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<PoorMadReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<POORMAD_HOME>/desktop-plugins),
      ...
      /** LOCAL `<POORMAD_HOME>/logs` (profile-aware) — error card "Open Logs". */
      logsRoot?: () => Promise<string>
      // Local AGENT-plugin root (<POORMAD_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<PoorMadGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<PoorMadGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<PoorMadGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<PoorMadRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: PoorMadReviewScope, baseRef?: null | string) => Promise<PoorMadReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: PoorMadReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<PoorMadReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<PoorMadRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<PoorMadPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: PoorMadTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<PoorMadTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: PoorMadWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: PoorMadPreviewFileChanged) => void) => () => void
...
export interface PoorMadTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface PoorMadTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `poormad update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  poormadRoot?: string
...
export interface PoorMadConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface PoorMadTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface PoorMadActiveWork {
  count: number
  titles: string[]
}

export interface PoorMadWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a PoorMad Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted PoorMad Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected PoorMad Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // Whether the OAuth partition holds a live PoorMad portal (Privy) session — the
  // portal authenticates via Privy, so this reflects the privy-token cookie, NOT
  // the poormad gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered PoorMad Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface DesktopBootProgress {
  error: string | null
  fakeMode: boolean
  /** True when the boot failure is a PoorMad Cloud agent that is down (HTTP 502/503/504). */
...
export interface PoorMadApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface PoorMadNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface PoorMadPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface PoorMadReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface PoorMadPreviewWatch {
  id: string
  path: string
}
...
export interface PoorMadGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface PoorMadGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface PoorMadGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface PoorMadRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface PoorMadRepoStatus {
  branch: null | string
...
// turn began.
export type PoorMadReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface PoorMadReviewFile {
  path: string
  added: number
  removed: number
...
export interface PoorMadReviewList {
  files: PoorMadReviewFile[]
...
export interface PoorMadReviewPr {
  url: string
  state: string
  number: number
}
...
export interface PoorMadBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface PoorMadRepoPullRequests {
  ghReady: boolean
  prs: PoorMadBranchPullRequest[]
}
...
export interface PoorMadPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface PoorMadReviewShipInfo {
  ghReady: boolean
  pr: PoorMadReviewPr | null
}

export interface PoorMadReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface PoorMadReadDirResult {
  entries: PoorMadReadDirEntry[]
  error?: string
}

export interface PoorMadPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface PoorMadSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
