 ...
 * `poormad://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `poormad://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `poormad://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type PoorMadOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const POORMAD_PROTOCOL = 'poormad:'
...
export function normalizePoorMadOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('poormad://') || trimmed.startsWith(`${POORMAD_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // poormad://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // poormad://index-network/intent/1 → /index-network/intent/1
...
export function resolvePoorMadOpenPath(target: PoorMadOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizePoorMadOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizePoorMadOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizePoorMadOpenString(target.path)
...
export function pathFromPoorMadDeepLink(
  ...
  if (kind === 'open') {
    return resolvePoorMadOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolvePoorMadOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `poormad://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromPoorMadDeepLink('open', name, params)
}
