import { poormadApi } from '@/api/client'
import type {
  PoorMadConnection,
  PoorMadReadDirResult,
  PoorMadReadFileTextResult,
  PoorMadSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: PoorMadConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: PoorMadConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<PoorMadReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<PoorMadReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<PoorMadReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<PoorMadReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.poormadDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: PoorMadSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
