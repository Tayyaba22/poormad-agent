...
describe('requestModelOptions', () => {
  ...
  it('recovers an empty gateway catalog through profile-scoped REST without replacing the session selection', async () => {
    const gatewayPayload = { model: 'poormad-local', provider: 'poormad-local' }

    const restPayload = {
      model: 'profile-default',
      provider: 'openai-codex',
      providers: [{ models: ['poormad-local'], name: 'PoorMad Local vLLM', slug: 'poormad-local' }]
    }
  ...
  it('keeps the gateway result when both catalog paths have no selectable models', async () => {
    const gatewayPayload = { model: 'poormad-local', provider: 'poormad-local', providers: [] }
  ...
  it('prefers an owner-routed request over the ambient gateway socket', async () => {
    const gatewayPayload = {
      model: 'chrome-model',
      provider: 'nous',
      providers: [{ models: ['chrome-model'], name: 'PoorMad', slug: 'nous' }]
    }
  ...
  it('scopes REST recovery to the catalog owner profile', async () => {
    const restPayload = {
      model: 'berry-local',
      provider: 'poormad-local',
      providers: [{ models: ['berry-local'], name: 'PoorMad Local', slug: 'poormad-local' }]
    }
...
describe('manualPickRemoved', () => {
  const providers = [
    ...
    { name: 'PoorMad', slug: 'nous', models: [] } // present but unconfigured / re-auth
  ]
