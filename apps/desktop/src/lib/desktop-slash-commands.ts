...
/** Why a known PoorMad command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active PoorMad profile', surface: action('profile') },
  {
    name: '/skin',
...
function isKnownPoorMadSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * PoorMad' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownPoorMadSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with PoorMad and were never asked for. It is
