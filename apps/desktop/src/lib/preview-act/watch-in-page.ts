...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __poormadWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__poormadWatch || (win.__poormadWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('poormad-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('poormad-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __poormadWatchTag?: number }).__poormadWatchTag
