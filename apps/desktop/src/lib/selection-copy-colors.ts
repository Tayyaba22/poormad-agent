...
// PoorMad is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.poormadMode
 ...
 * the only carriers of PoorMad' palette — are dropped so the paste target's
