...
//
// When someone is in a hands-free "Hey PoorMad" voice chat, the natural way to
...
// phrase (optionally addressed to PoorMad), so a real turn that merely contains
...
// Optional address prefixes so "poormad stop" / "ok stop" / "hey poormad, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey poormad', 'hey poormad,', 'poormad', 'poormad,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("poormad") is not a stop command on its own.
      continue
    }
 ...
 * to PoorMad). Returns false for anything that merely contains "stop" as part of
