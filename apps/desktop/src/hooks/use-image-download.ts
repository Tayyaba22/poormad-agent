...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'poormad:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.poormadDesktop?.saveImageFromUrl) {
        if (await window.poormadDesktop.saveImageFromUrl(src)) {
