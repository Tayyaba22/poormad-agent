...
import { getPoorMadConfigRecord, type PoorMadConfigRecord, savePoorMadConfig } from '@/poormad'
...
export interface I18nConfigClient {
  getConfig: () => Promise<PoorMadConfigRecord>
  saveConfig: (config: PoorMadConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.poormadDesktop?.api) {
      return Promise.resolve({})
    }

    return getPoorMadConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.poormadDesktop?.api) {
    ...
    return savePoorMadConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: PoorMadConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: PoorMadConfigRecord, locale: Locale): PoorMadConfigRecord {
