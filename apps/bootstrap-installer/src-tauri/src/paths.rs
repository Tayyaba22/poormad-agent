...
//!
//! Mirrors `poormad_constants.get_poormad_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\poormad
//!   macOS:   ~/.poormad
//!   Linux:   ~/.poormad  (override via $POORMAD_HOME)
//!
//! NOTE (macOS): Python's get_poormad_home(), scripts/install.sh, and the
//! Electron desktop's resolvePoorMadHome() ALL use ~/.poormad on macOS — there
...
/// Returns the canonical PoorMad home directory, respecting $POORMAD_HOME if set.
pub fn poormad_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("POORMAD_HOME") {
    ...
    {
        // %LOCALAPPDATA%\poormad — matches scripts/install.ps1's $PoorMadHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("poormad");
        }
    }

    // macOS + Linux + fallback: ~/.poormad (matches Python get_poormad_home(),
    // install.sh, and the Electron desktop's resolvePoorMadHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".poormad");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".poormad")
}

pub fn log_dir() -> PathBuf {
    poormad_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    poormad_home().join("bootstrap-cache")
}
...
/// POORMAD_HOME so it survives repo checkout deletion (unlike anything under
/// poormad-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\poormad\poormad-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_poormad`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under POORMAD_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves POORMAD_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    poormad_home().join(".poormad-update-in-progress")
}
...
pub fn copy_self_to_poormad_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to POORMAD_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_POORMAD_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".poormad-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under POORMAD_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[poormad-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("POORMAD_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_poormad_home() -> String {
    poormad_home().to_string_lossy().into_owned()
}
