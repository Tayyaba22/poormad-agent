//! PoorMad Setup — Tauri entrypoint.
//!
...
/// desktop app handing off via `PoorMad-Setup.exe --update`).
///
...
pub fn run() {
    // Tracing → bootstrap-installer.log under POORMAD_HOME/logs/ so install
    ...
    // PoorMad is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "PoorMad installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // PoorMad is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "PoorMad" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "PoorMad" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_poormad_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::poormad_home().join("poormad-agent");
                if bootstrap::poormad_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_poormad_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "poormad already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            update::start_update,
            // Hand-off
            bootstrap::launch_poormad_desktop,
            // Diagnostics
            paths::get_log_path,
            paths::get_poormad_home,
            paths::open_log_dir,
        ])
        .run(tauri::generate_context!())
        .expect("error while running PoorMad Setup");
}
