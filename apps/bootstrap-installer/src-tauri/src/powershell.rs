...
/// read side does not see EOF until the last of them closes it. `poormad update`
...
/// stranded with it.
///
/// Same bound `Invoke-PoorMadStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `poormad_home_override` propagates to the child as $POORMAD_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, poormad_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = poormad_home_override {
        cmd.env("POORMAD_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, poormad_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = poormad_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_poormad_home() {
    ...
    /// child. `poormad update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
