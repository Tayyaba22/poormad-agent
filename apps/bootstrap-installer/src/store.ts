...
export const $poormadHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.poormad/logs/bootstrap-installer.log')
    $poormadHome.set('~/.poormad')
  ...
  try {
    const [logPath, poormadHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_poormad_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $poormadHome.set(poormadHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch PoorMad" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (PoorMad-Setup.exe --update);
...
export async function launchPoorMadDesktop(): Promise<void> {
  ...
  await invoke('launch_poormad_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'PoorMad repository' },
