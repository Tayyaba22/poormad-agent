"""Windows UTF-8 bootstrap for PoorMad entry points.
...
should be imported at the very top of every PoorMad entry point
(``poormad``, ``poormad-agent``, ``poormad-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``poormad_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``poormad_bootstrap``, never ``poormad_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing PoorMad modules.

    PoorMad ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real PoorMad source root
    ...
    """
    root = src_root or os.environ.get("POORMAD_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``POORMAD_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("POORMAD_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import poormad_bootstrap``
# (or ``from poormad_bootstrap import apply_windows_utf8_bootstrap``) at
