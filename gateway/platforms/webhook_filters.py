...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.poormad to the active profile home."""
    ...
    if raw == "~/.poormad":
        return poormad_home
    if raw.startswith("~/.poormad/"):
        return poormad_home / raw.removeprefix("~/.poormad/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return poormad_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under POORMAD_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from poormad_constants import get_poormad_home

    scripts_root = (get_poormad_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.poormad" or raw_text.startswith("~/.poormad/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__poormad_ignore__") is True
        ):
            return False, None
