...
There are two ways to add a platform to the PoorMad gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.poormad/plugins/` (or under `plugins/platforms/`
...
**zero changes to core PoorMad code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `poormad gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `poormad_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the PoorMad core.
...
And add it to the `poormad-gateway` composite:

```python
"poormad-gateway": {
    "includes": [..., "poormad-your-platform"]
}
```
...
## 12. Status Display (`poormad_cli/status.py`)
...
## 13. Gateway Setup Wizard (`poormad_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ poormad_cli/ toolsets.py \
  --include="*.py" -l | sort -u
