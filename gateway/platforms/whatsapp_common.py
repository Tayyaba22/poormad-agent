...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "🌑 *PoorMad*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to POORMAD_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/poormad), this function
    mirrors the bridge source to a writable POORMAD_HOME location and returns that
    ...
    # Try POORMAD_HOME location first
    poormad_home = get_poormad_home()
    poormad_home_bridge = poormad_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to POORMAD_HOME if needed
    if poormad_home_bridge.exists():
        return poormad_home_bridge

    # Mirror the bridge source to POORMAD_HOME
    try:
        poormad_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            poormad_home_bridge,
            dirs_exist_ok=False,
