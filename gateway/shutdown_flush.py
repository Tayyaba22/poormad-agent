...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<poormad_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active POORMAD_HOME."""
    from poormad_constants import get_poormad_home

    flush_dir = get_poormad_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<poormad_home>/pending_messages/``), so a runtime cap rotation no
