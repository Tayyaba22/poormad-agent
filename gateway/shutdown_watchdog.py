...
2. An event-loop heartbeat file at ``<POORMAD_HOME>/state/gateway.heartbeat`` so
...
def _process_poormad_home() -> Path:
    """POORMAD_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<POORMAD_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_poormad_home()
...
def get_loop_tick_socket_path(
    ...
    ``<POORMAD_HOME>/state/gateway.loop-tick.<pid>.sock`` — PID-suffixed so a
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from poormad_logging import drain_log_queue
...
async def loop_heartbeat_forever(
    ...
    *initiated* by the loop, so a frozen loop still lets the file age.
    ...
    wedged; see ``poormad_cli.gateway.probe_gateway_loop_liveness`` for the
