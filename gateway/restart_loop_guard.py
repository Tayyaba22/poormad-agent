...
Defenses 1 and 2 (the ``_POORMAD_GATEWAY`` guard on ``poormad gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.poormad.gateway")``,
...
State lives in ``<POORMAD_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_poormad_home() / "gateway" / "restart_loop.json"
