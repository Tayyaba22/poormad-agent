...
Module-level run.py helpers a handler needs (``_poormad_home``,
...
from poormad_cli.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: poormad gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach PoorMad commands.
    ...
    async def _handle_reset_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        try:
            from poormad_cli.lifecycle import invoke_hook as _invoke_hook
        ...
        try:
            from poormad_cli.tips import get_random_tip
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`poormad gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _poormad_home
        ...
        try:
            ...
            await asyncio.to_thread(
                atomic_json_write,
                _poormad_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            await asyncio.to_thread(
                atomic_json_write,
                _poormad_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running PoorMad version."""
        from poormad_cli.slash_exec import CommandContext, execute_command
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _poormad_home, _load_gateway_config
        ...
        from poormad_cli.providers import get_label
        ...
        # via the shared single-owner parser (poormad_cli.model_switch).
        ...
        custom_provs = None
        excluded_provs = []
        config_path = (_command_profile_home or _poormad_home) / "config.yaml"
        try:
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        try:
                            from poormad_cli.context_switch_guard import (
                        ...
                        from poormad_cli.model_switch import format_model_for_display
                        ...
                        if persist_global:
                            try:
                                ...
                                if isinstance(_raw_model, dict):
                                    ...
                                    _persist_cfg["model"] = _persist_model_cfg
                                try:
                                    from poormad_cli.route_identity import should_clear_context_pin_async
                        ...
                        from poormad_cli.model_switch import resolve_display_context_length_async
                        _sw_config_ctx = None
                        _sw_model_cfg = {}
                        try:
        ...
        async def _finish_switch() -> str:
            ...
            # Copilot, and PoorMad-enforced caps win over the raw models.dev entry.
            mi = result.model_info
            from poormad_cli.model_switch import resolve_display_context_length_async
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — PoorMad default runtime
        ...
        from poormad_cli import codex_runtime_switch as crs
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        # Persist the selection only — poormad_cli.personality never writes
    ...
    async def _handle_heartbeat_command(self, event: "MessageEvent") -> str:
        ...
        """
        from poormad_cli.heartbeat import parse_interval, format_interval, MIN_INTERVAL_SECONDS
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `poormad cron` for durable schedules."
        )
    ...
    async def _handle_loop_command(self, event: "MessageEvent") -> str:
        ...
        """
        try:
            from poormad_cli.loops import dispatch_loop_command, goal_blocks_loop_tick
        except Exception as exc:
    ...
    async def _handle_diff_command(self, event: MessageEvent) -> str:
        ...
        PoorMad itself changed. ``--stat`` limits output to the summary.
    ...
    def _save_gateway_config_key(self, key_path: str, value) -> bool:
        ...
        from gateway.run import _poormad_home
        from poormad_cli.config import read_user_config_raw
        config_path = _poormad_home / "config.yaml"
        try:
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.poormad/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``poormad skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _poormad_home
        from poormad_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `poormad skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.poormad/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_fast_command(self, event: MessageEvent) -> Optional[str]:
        ...
        from poormad_cli.models import model_supports_fast_mode
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _poormad_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _poormad_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_save_command(self, event: MessageEvent) -> str:
        ...
        """
        from poormad_cli.session_export import (
            SAVE_USAGE,
        ...
        if redact:
            from poormad_cli.session_export_md import redact_session_data

            export_data = redact_session_data(export_data)

        import tempfile

        temp_dir = tempfile.mkdtemp(prefix="poormad_save_")
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from poormad_cli.session_listing import (
    ...
    async def _handle_branch_command(self, event: MessageEvent) -> str:
        ...
        try:
            await self._session_db.create_session(
                ...
                # path (poormad_state.py:1994-2009) that searches by the complete
    ...
    async def _handle_topup_command(self, event: MessageEvent) -> str:
        """Handle /topup -- show the PoorMad balance and hand off to the portal.
        ...
        lines: list[str] = ["💳 **PoorMad balance**"]
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # ── PoorMad credits magnitudes + monthly-grant % gauge ─────────────
        ...
        # Run off the event loop. The helper gates on "a PoorMad account is logged in"
        # — NOT the inference provider and NOT nested under `if provider:` — so a
        # PoorMad-credentialled user running inference elsewhere (or with none resident)
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `poormad bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``poormad debug share`` from the CLI.
        """
        import asyncio
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update PoorMad to the latest version.

        Spawns ``poormad update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``poormad update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _poormad_home, _resolve_poormad_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        ...
        if is_managed():
            return f"✗ {format_managed_message('update PoorMad')}"
        ...
        poormad_cmd = _resolve_poormad_bin()
        if not poormad_cmd:
            return t("gateway.update.poormad_cmd_not_found")

        pending_path = _poormad_home / ".update_pending.json"
        output_path = _poormad_home / ".update_output.txt"
        exit_code_path = _poormad_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `poormad update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `poormad update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # than through poormad_cmd (venv\Scripts\poormad.exe): the shim
                ...
                poormad_cmd_str = " ".join(shlex.quote(part) for part in poormad_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {poormad_cmd_str} update --gateway"
