...
class PlatformEntry:
    ...
    # status displays (``poormad setup``, ``poormad status``, the dashboard
    ...
    # Env vars this platform needs (for ``poormad setup`` display).
    ...
    # built-ins).  Used by ``poormad gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved POORMAD_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `poormad`
        # invocation -- including plain `poormad chat`, which never touches any
        ...
        # `poormad setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``poormad setup``/``gateway status``, channel
