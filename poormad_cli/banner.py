...
Pure display functions with no PoorMadCLI state dependency.
"""
import json
import logging
import os
...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `poormad ... > file`) raises
...
from poormad_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

POORMAD_AGENT_LOGO = """[bold #C4B5FD]██████╗  ██████╗  ██████╗ ██████╗ ███╗   ███╗ █████╗ ██████╗ [/]
[bold #C4B5FD]██╔══██╗██╔═══██╗██╔═══██╗██╔══██╗████╗ ████║██╔══██╗██╔══██╗[/]
[#8B5CF6]██████╔╝██║   ██║██║   ██║██████╔╝██╔████╔██║███████║██║  ██║[/]
[#8B5CF6]██╔═══╝ ██║   ██║██║   ██║██╔══██╗██║╚██╔╝██║██╔══██║██║  ██║[/]
[#6D28D9]██║     ╚██████╔╝╚██████╔╝██║  ██║██║ ╚═╝ ██║██║  ██║██████╔╝[/]
...
# (e.g. nix-built poormad — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/Tayyaba22/poormad-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/Tayyaba22/poormad-agent"
...
def _github_compare_behind(current_rev: str, target_rev: str) -> Optional[int]:
    ...
    if not (_is_full_sha(current_rev) and _is_full_sha(target_rev)):
        return None
    url = (
        "https://api.github.com/repos/poormad/poormad-agent/"
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "poormad-cli-update-check",
            },
        )
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `poormad update`,
...
def check_for_updates() -> Optional[int]:
    """Check whether a PoorMad update is available.

    Two paths: if ``POORMAD_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = poormad_home / ".update_check"
    embedded_rev = os.environ.get("POORMAD_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # POORMAD_REVISION (that's nix-only). Returning None makes both the Rich
    # banner (build_welcome_banner) and the Ink badge (branding.tsx, guarded
    # on `typeof === 'number' && > 0`) show nothing. The dashboard's REST
    # `/api/poormad/update/check` endpoint short-circuits docker the same way
    # (web_server.py); mirror that here so the banner/TUI surfaces agree.
    try:
        from poormad_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $POORMAD_HOME/poormad-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = poormad_home / "poormad-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active PoorMad git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$POORMAD_HOME/poormad-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``poormad_cli/build_info.py``) and return it as a frozen
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from poormad_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/Tayyaba22/poormad-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    PoorMad checkout. Cached per-process. Release URL always points at the
    canonical PoorMad/poormad-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"PoorMad v{VERSION} ({RELEASE_DATE})"
...
def _format_update_notice(behind: int) -> str:
    ...
    from poormad_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
    ...
    # UPDATE_AVAILABLE_NO_COUNT: nix-built poormad; we know an update
...
def _banner_snapshot_path() -> Path:
    return get_poormad_home() / "cache" / "banner_snapshot.json"
...
def banner_snapshot_fingerprint() -> Optional[str]:
    ...
    try:
        ...
        for p in (get_config_path(), get_poormad_home() / ".env"):
            try:
                st = p.stat()
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]PoorMad[/]")
    else:
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or poormad setup[/]"
            )
        else:
            ...
            ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
            left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]PoorMad[/]")

    if os.getenv("POORMAD_YOLO_MODE"):
    ...
    if not _has_native_mcp:
        try:
            from poormad_cli.plugins import get_portable_mcp_server_names_nowait
    ...
    try:
        from poormad_cli.codex_runtime_switch import get_current_runtime
        from poormad_cli.config import load_config as _load_cfg
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else POORMAD_AGENT_LOGO
        console.print(_logo)
        console.print()
