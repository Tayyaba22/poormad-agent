"""On-demand supply-chain audit for PoorMad installs.

Scans three surfaces a PoorMad user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The PoorMad venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.poormad/plugins``
...
def _discover_plugins(poormad_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.poormad/plugins``.
...
def _discover_components(
    *,
    ...
    home = poormad_home or Path(get_poormad_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `poormad security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, poormad_home=home
    )
