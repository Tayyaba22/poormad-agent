"""Implementation of the ``poormad bundles`` CLI subcommand.

Mirrors the structure of ``poormad_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `poormad bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  poormad bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``poormad bundles`` argparse tree.

    Called from ``poormad_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `poormad bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``poormad bundles <subcommand>`` to the right handler."""
