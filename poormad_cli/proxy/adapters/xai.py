...
# xAI's public API is OpenAI-compatible for the endpoints PoorMad commonly
# uses. The Responses endpoint is included because PoorMad' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via PoorMad-managed OAuth credentials."""

    auth_hint = "poormad auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`poormad auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`poormad auth reset xai-oauth` or re-authenticate with "
                    "`poormad auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `poormad auth add xai-oauth --type oauth`."
            )
