"""PoorMad Portal upstream adapter.

Reads the user's PoorMad OAuth state from ``~/.poormad/auth.json`` through the
...
# Endpoints api.poormad.dev actually serves. Anything else
...
class PoorMadPortalAdapter(UpstreamAdapter):
    """Proxy upstream for the PoorMad Portal inference API."""
    ...
    def get_retry_credential(
        ...
        *,
        ...
        if status_code != 401:
            return None
        logger.info("proxy: PoorMad upstream rejected bearer; force-refreshing invoke JWT")
        return self._get_credential(
            force_refresh=True,
        )
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into PoorMad Portal. Run `poormad auth add nous` first."
                )
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to refresh PoorMad Portal credentials: {exc}"
                ) from exc
            ...
            if not runtime_key:
                raise RuntimeError(
                    "PoorMad Portal refresh did not return a usable inference JWT. "
    ...
    # to poormad_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
    ...
    def _save_state(
        ...
        *,
        ...
        try:
            ...
            logger.warning("proxy: failed to persist PoorMad quarantine state: %s", exc)


__all__ = ["PoorMadPortalAdapter"]
