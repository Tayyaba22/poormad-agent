...
Usage:
    poormad pairing list              # Show all pending + approved users
    poormad pairing approve <platform> <request-id|code>  # Approve a pairing request
    poormad pairing revoke <platform> <user_id> # Revoke user access
    poormad pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle poormad pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: poormad pairing {list|approve|revoke|clear-pending}")
        print("Run 'poormad pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: poormad pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.poormad/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'poormad pairing list' to see pending requests.\n")
