"""poormad webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.poormad/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _poormad_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: poormad gateway run
"""
...
def webhook_command(args):
    """Entry point for 'poormad webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: poormad webhook {subscribe|list|remove|test}")
        print("Run 'poormad webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (poormad gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: poormad webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from poormad webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (poormad gateway run)")
