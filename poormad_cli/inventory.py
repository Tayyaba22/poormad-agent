...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (PoorMad /models returns ~400
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      for PoorMad, ``free_tier``/``unavailable_models`` so the GUI picker can
      show $/Mtok columns and gate paid models on free accounts —
      mirroring the ``poormad model`` CLI picker. Adds network calls
      (pricing fetch + PoorMad tier check); only set for interactive pickers.
    ...
    - ``force_fresh_nous_tier``: bypass the short PoorMad free-tier cache when
      selecting Portal-recommended PoorMad models and applying tier gating. Keep
    ...
    try:
        from poormad_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def build_aux_picker_rows(
    *,
    ...
    back. Mirrors the same filter in ``poormad_cli/moa_cmd.py``.
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`poormad_cli.providers.custom_provider_aliases` — lets the frontend
    do a membership check instead.
    """
    from poormad_cli.providers import custom_provider_aliases
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from poormad_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `poormad model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _anthropic_oauth_credentials_present() -> bool:
    ...
    model.provider / API-key env vars: PoorMad' own Anthropic device flow
    ...
    try:
        ...
        poormad_creds = read_poormad_oauth_credentials() or {}
        if poormad_creds.get("accessToken"):
            return True
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for PoorMad.
    """
    ...
    for row in rows:
        ...
        if slug == "anthropic" and _anthropic_oauth_credentials_present():
            # Anthropic OAuth logins (PoorMad device flow / Claude Code) are
...
def _provider_is_keyless(slug: str) -> bool:
    """True when the provider's PoorMad overlay declares it keyless."""
    try:
        from poormad_cli.providers import POORMAD_OVERLAYS
        overlay = POORMAD_OVERLAYS.get(slug)
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            if auth_type == "api_key" and key_env
            else f"run `poormad model` to configure ({auth_type})"
        )
...
def _apply_pricing(
    ...
    *,
    ...
    """Enrich each provider row with per-model pricing + PoorMad tier gating.
    ...
    # Resolve PoorMad free-tier once (cached in models.py for the TTL window).
    ...
    for row in rows:
        ...
        for mid in models:
            ...
            # Sale chrome is PoorMad Portal-only. Other providers (OpenRouter,
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`poormad_cli.model_switch.list_picker_providers`) so the
