"""poormad import-agent — import Claude Code / Codex CLI setups into PoorMad.

Usage:
    poormad import-agent                       # auto-detect ~/.claude or ~/.codex
    poormad import-agent claude-code           # import from ~/.claude
    poormad import-agent codex                 # import from ~/.codex
    poormad import-agent claude-code --dry-run # preview only, no changes
    poormad import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``poormad claw migrate`` /
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_poormad.py``):
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in POORMAD_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → POORMAD_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in POORMAD_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in POORMAD_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → POORMAD_HOME/skills/codex-imports/<name>/
...
deliberately via ``poormad setup`` or config.yaml.
"""
...
# Same entry delimiter as the PoorMad memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`poormad config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_poormad.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → PoorMad command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a PoorMad glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct PoorMad equivalent — "
        ...
        for name, srv in servers.items():
            ...
            if srv.get("url"):
                ...
                if isinstance(headers, dict):
                    ...
                    if kept_headers:
                        poormad_srv["headers"] = kept_headers
                    for k in headers:
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → POORMAD_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``poormad import-agent`` (invoked from poormad_cli.main)."""
    from poormad_cli.config import get_config_path, load_config, save_config
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: poormad import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: poormad import-agent claude-code   or   poormad import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ⚕ PoorMad — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: poormad import-agent "
    ...
    print_info("Secrets:     never imported — run 'poormad setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: poormad import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'poormad setup' "
               "to configure providers, or add them to ~/.poormad/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from poormad_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'poormad setup' or ~/.poormad/.env.")
        print()
