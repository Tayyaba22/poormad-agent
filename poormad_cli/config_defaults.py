"""Default configuration data for PoorMad.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from poormad_cli/config.py. Must not import from poormad_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["poormad-cli"],
    # SQLite journal mode used by every PoorMad database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running PoorMad server processes.
    ...
    "session": {
        # Per-terminal `poormad -c`: each CLI session drops a breadcrumb file
        # under $POORMAD_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by poormad_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `poormad chat --run-budget N`.
        ...
        # an interactive `poormad gateway restart` must never block for hours
        ...
        # the PoorMad-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps PoorMad
        ...
        # identity slot (SOUL.md). Empty by default. The POORMAD_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, PoorMad adds a coding operating brief
        ...
        # `poormad status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``POORMAD_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        # bot_relay from a short-lived `poormad -p <bot> chat -Q` recipient
        ...
        #             POORMAD_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force POORMAD_HOME/home when it exists (old strict
        ...
        # skipped. When empty, PoorMad auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), PoorMad sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when PoorMad
        ...
        # bundled PoorMad image, which drops to the `poormad` user via
    ...
    "web": {
        ...
        # firecrawl, keenable). Set by the `poormad tools` picker's
    ...
    "browser": {
        ...
        # browser tools) runs on a PoorMad-managed SNAPSHOT of the user's
        ...
        # fresh session launches — driven by PoorMad' packaged Chromium. Only
        ...
        # deletes the snapshot store (~/.poormad/browser-profile/) so copied
        ...
        # approval does it run `poormad browser close-profile` (terminates the
        ...
        "camofox": {
            # When true, PoorMad sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and PoorMad should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``poormad chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.poormad/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: poormad sweeps the checkpoint base at startup
        ...
        # ``poormad checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .poormad.md, or .cursorrules before PoorMad applies
    ...
    # Single-query (``poormad -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, PoorMad truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so PoorMad' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); poormad =
                                      # PoorMad' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. PoorMad' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before PoorMad does.
    ...
    "auxiliary": {
        ...
        # Invoked by ``poormad kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``poormad kanban decompose`` and the kanban
        ...
        # ``poormad profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `poormad model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `poormad` (and `poormad chat`) launches by default:
        ...
        # REPL and `--tui` (or POORMAD_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `poormad --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `poormad -c` muscle memory.  Default off so existing
        # users aren't surprised.  POORMAD_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `poormad --tui` drops a one-time hint
        ...
        # PoorMad credits status-bar notices (usage bands, grant-spent, depleted /
        ...
        # can set `path`/`paths` to POORMAD_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/poormad`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `poormad pets`. Disabled until
        ...
        "pet": {
            ...
            # get_poormad_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # protocol ping (see poormad_cli/web_server.py — an event-loop stall
        ...
        # POORMAD_TUI_WS_ORPHAN_REAP_GRACE_S env var remains an internal
        ...
        # (POORMAD_TUI_SESSION_TTL_S, default 6h) are closed with
        ...
        # ``--insecure`` is not). The bundled PoorMad Portal plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``POORMAD_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``POORMAD_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.poormad.dev).
        "oauth": {
        ...
        # (``POORMAD_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``POORMAD_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``POORMAD_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/poormad``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; PoorMad appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.poormad/cache/piper-voices/
    ...
    # "Hey PoorMad" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey poormad",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_poormad" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.poormad/plugins/.
    "context": {
        "engine": "compressor",
    ...
    # configured providers (OpenRouter, PoorMad, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.poormad/cache/delegation/
    ...
    # response. If not, PoorMad feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before PoorMad auto-pauses the goal and
    ...
    "moa": {
        ...
        # <poormad_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.poormad/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.poormad/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``poormad skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``poormad skills trust`` / ``poormad skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${POORMAD_SKILL_DIR} and ${POORMAD_SESSION_ID} in SKILL.md
        ...
        # (`poormad skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.poormad/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.poormad/.curator_backups/blobs/. Enables `poormad curator ledger`
        # and single-mutation `poormad curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `poormad curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `poormad curator run --consolidate` overrides
        ...
        # normally restored on every `poormad update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `poormad curator purge` deletes
        ...
        # skipped), snapshot ~/.poormad/skills/ into
        # ~/.poormad/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `poormad curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for poormad-specific overrides; everything else
    ...
    # dangerous command. -q runs export POORMAD_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; POORMAD_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace PoorMad' built-in platform hint for a single messaging platform
    ...
    # ``poormad plugins enable|disable`` and intentionally omitted here so an
    ...
    # stored approval from ~/.poormad/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or POORMAD_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .poormad config) always require human
        ...
        # longer trigger the startup banner. Add via `poormad doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``poormad_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow PoorMad to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $POORMAD_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # agent's existing PoorMad Portal token — there is no token key here.
        "chronos": {
            ...
            "portal_url": "https://portal.poormad.dev",
        ...
        # Also overridable via POORMAD_CRON_MAX_PARALLEL env var.
        ...
        # POORMAD_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # POORMAD_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via POORMAD_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `poormad -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `poormad` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `poormad kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     poormad-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core PoorMad tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.poormad/logs/.
    ...
    # curated model lists for OpenRouter and PoorMad Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a poormad-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://poormad.dev/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `poormad model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the PoorMad provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal POORMAD_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.poormad/sessions/sessions.json entirely.
        ...
        # the POORMAD_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.poormad.gateway` that defenses 1-2 don't
        ...
        # `poormad update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``POORMAD_GATEWAY_MAX_STARTS`` / ``POORMAD_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.poormad/.env,
        ...
        # behavior: files must live under the PoorMad cache, under
        ...
        # user. Bridged to POORMAD_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the PoorMad
        # cache (~/.poormad/cache/{documents,images,audio,video,screenshots})
        ...
        # to POORMAD_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # POORMAD_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to POORMAD_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.poormad/state.db.
    ...
    "sessions": {
        ...
        # the default of ``poormad sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.poormad/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `poormad sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `poormad update` prints a one-line notice with
        ...
        # ~/.poormad/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to POORMAD_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # POORMAD_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `poormad sessions export`. Checked
    ...
    # ``poormad doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `poormad doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``poormad update`` behaviour.
    "updates": {
        ...
        #     DBs) into <POORMAD_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``poormad backup``-style zip
        #     of POORMAD_HOME into <POORMAD_HOME>/backups/, restorable with
        #     ``poormad import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `poormad update` does with uncommitted local changes to the
        ...
        # When `poormad update` finds the source checkout parked on a feature
        ...
        #                         merge. `poormad update --switch-branch`
    ...
    "lsp": {
        ...
        #              ``<POORMAD_HOME>/lsp/bin/`` on first use.
    ...
    # `poormad tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.poormad/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.poormad/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.poormad/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, PoorMad may use this encrypted
            ...
            # ~/.poormad/bin/ on first use.  When False you must install
            ...
            # `poormad secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.poormad/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. PoorMad disables it for our users unless they
        # explicitly opt in here. When false (default), PoorMad sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; PoorMad also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each PoorMad computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". PoorMad passes the
    ...
    #
    # Configure with `poormad egress setup`.  Disabled by default — the rest of
    # PoorMad works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.poormad/bin/ on
        ...
        # Together, DeepSeek, PoorMad).  Wildcards (`*.foo.com`) are supported.
        "extra_allowed_hosts": [],
    },

    # PoorMad Desktop (Electron app) launch options. These only affect
    # `poormad desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the POORMAD_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the POORMAD_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "NOUS_BASE_URL": {
        "description": "PoorMad Portal base URL override",
        "prompt": "PoorMad Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials PoorMad mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'poormad model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for PoorMad Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for PoorMad Subscribers only, used to derive vendor hosts, e.g. poormad.com -> firecrawl-gateway.poormad.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for PoorMad Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit PoorMad Subscriber access token for tool-gateway requests (optional; otherwise read from the PoorMad auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey PoorMad' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use PoorMad, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @poormad:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. POORMAD_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #poormad)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: poormad-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'poormad-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote PoorMad API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote PoorMad API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote PoorMad API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # POORMAD_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to POORMAD_TOOL_PROGRESS_MODE for backward
    ...
    # POORMAD_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "POORMAD_PREFILL_MESSAGES_FILE": {
    ...
    "POORMAD_EPHEMERAL_SYSTEM_PROMPT": {
