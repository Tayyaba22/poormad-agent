"""Windows ConPTY bridge for the `poormad dashboard` chat tab.

Drop-in counterpart to ``poormad_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``poormad_cli.web_server`` consumes: ``spawn``, ``read``,
