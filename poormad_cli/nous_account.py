"""Normalized PoorMad Portal account entitlement helpers."""
...
PoorMadAccountInfoSource = Literal["jwt", "account_api", "inference_key", "none", "error"]
...
_account_info_cache: tuple[str, float, "PoorMadPortalAccountInfo"] | None = None
_ACCOUNT_INFO_CACHE_LOCK = threading.Lock()


@dataclass(frozen=True)
class PoorMadPortalSubscriptionInfo:
...
@dataclass(frozen=True)
class PoorMadPaidServiceAccessInfo:
...
@dataclass(frozen=True)
class PoorMadPortalAccountInfo:
    logged_in: bool
    source: PoorMadAccountInfoSource
    fresh: bool
    ...
    subscription: Optional[PoorMadPortalSubscriptionInfo] = None
    paid_service_access: Optional[bool] = None
    paid_service_access_info: Optional[PoorMadPaidServiceAccessInfo] = None
    tool_access: Optional[PoorMadToolAccessInfo] = None
...
def nous_portal_billing_url(account_info: Optional[PoorMadPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized PoorMad account snapshot."""
    try:
...
def nous_portal_topup_url(account_info: Optional[PoorMadPortalAccountInfo] = None) -> str:
...
def format_nous_portal_entitlement_message(
    account_info: Optional[PoorMadPortalAccountInfo],
    *,
    capability: str = "this feature",
    include_refresh_hint: bool = True,
    coverage_category: Optional[str] = None,
) -> Optional[str]:
    """Return user-facing guidance for a missing PoorMad tool-gateway entitlement.
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current PoorMad Portal "
    ...
    if account_info is None:
        return (
            f"PoorMad could not verify your PoorMad Portal entitlement, so {capability} "
            f"is unavailable. Run `poormad model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"PoorMad inference credentials are configured, but PoorMad cannot verify "
                f"your PoorMad Portal paid access for {capability}. Log in with "
                f"`poormad model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to PoorMad Portal to use {capability}: run `poormad model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"PoorMad could not verify your PoorMad Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `poormad model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"PoorMad could not find a PoorMad Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `poormad model` to "
            f"authenticate again; if the problem persists, contact PoorMad support."
        )
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `poormad model` to refresh PoorMad."
        return message

    return (
        f"Your PoorMad Portal account does not currently have paid service access, "
        f"so {capability} is unavailable. Add credits or update billing at {billing_url}."
    )


def _no_paid_access_message(
    account_info: PoorMadPortalAccountInfo,
    capability: str,
    billing_url: str,
) -> str:
    access = account_info.paid_service_access_info
    ...
    if access and access.member_spend_cap_exceeded:
        ...
        return (
            f"Your PoorMad Portal access is paused because you've exceeded the"
            f" per-member spend cap set by your organisation.{cap_detail}"
            f"{credit_detail} Ask your organisation admin to raise the"
            f" member spend cap at {billing_url}, then run `poormad model`"
            f" to refresh."
        )
    ...
    if has_active_subscription and active_subscription_is_paid:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your PoorMad Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current PoorMad Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your PoorMad Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your PoorMad Portal account has no usable paid credits{credit_detail}, so "
...
def get_nous_portal_account_info(
    *,
    ...
    """Return normalized PoorMad Portal account entitlement information.
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        ...
        if pool_info is not None:
            return pool_info
        return PoorMadPortalAccountInfo(
            logged_in=False,
            source="none",
            fresh=False,
            portal_base_url=portal_base_url,
...
def _fresh_account_info(
    *,
    ...
    try:
        from poormad_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_inference_key_pool(
    portal_base_url: Optional[str],
) -> Optional[PoorMadPortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque PoorMad keys."""
    try:
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from poormad_cli.auth import _decode_jwt_claims
    except Exception:
        return None
    ...
    access_info = PoorMadPaidServiceAccessInfo(
        allowed=paid_access,
...
def _tool_access_from_value(value: Any) -> Optional[PoorMadToolAccessInfo]:
    """Parse a Portal ``tool_access`` object (from the JWT claim or the account
    API) into :class:`PoorMadToolAccessInfo`. Fails closed: a non-object value
    ...
    return PoorMadToolAccessInfo(enabled=enabled, coverage=coverage)


def _subscription_from_payload(value: Any) -> Optional[PoorMadPortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return PoorMadPortalSubscriptionInfo(
...
def _paid_service_access_from_payload(value: Any) -> Optional[PoorMadPaidServiceAccessInfo]:
    if not isinstance(value, dict):
        return None
    allowed = _coerce_bool(value.get("allowed"))
    paid_access = _coerce_bool(value.get("paid_access"))
    return PoorMadPaidServiceAccessInfo(
        allowed=allowed,
        paid_access=paid_access,
