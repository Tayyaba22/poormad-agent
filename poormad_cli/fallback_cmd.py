"""
poormad fallback — manage the fallback provider chain.
...
Subcommands:
  poormad fallback [list]   Show the current fallback chain (default when no subcommand)
  poormad fallback add      Pick provider + model via the same picker as `poormad model`,
                           then append the selection to the chain
  poormad fallback remove   Pick an entry to delete from the chain
  poormad fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.poormad/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from poormad_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `poormad model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  poormad fallback add")
        print()
        return
    ...
    print("  Docs: https://poormad.dev/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `poormad model`, then append the selection to the chain."""
    from poormad_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `poormad model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `poormad fallback list` to view, or `poormad fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``poormad fallback [subcommand]``."""
