...
on Japanese locales, etc.).  PoorMad's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``poormad_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. PoorMad is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``POORMAD_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("POORMAD_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the PoorMad-managed Git install directories so
    ...
    # already-running shells don't see the change — which means poormad
    ...
    # install without PoorMad-managed PortableGit).
    ...
    # stdin is re-configured for completeness; PoorMad's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and PoorMad's
    ``poormad config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching PoorMad (User env var in Windows Settings, or
    export in a PowerShell profile) and PoorMad picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known PoorMad-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\poormad\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``poormad`` from right after install) retain their old PATH.

    Any subprocess PoorMad spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known PoorMad-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "poormad", "git", "cmd"),
        os.path.join(local_appdata, "poormad", "git", "bin"),
        os.path.join(local_appdata, "poormad", "git", "usr", "bin"),
        # PoorMad venv Scripts directory — host of the poormad.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes poormad, but harmless to include.
        os.path.join(local_appdata, "poormad", "poormad-agent", "venv", "Scripts"),
