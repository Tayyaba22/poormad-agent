...
the existing module-level functions in poormad_cli.gateway and
poormad_cli.gateway_windows directly. This protocol is a thin facade
...
``poormad gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``poormad gateway start/stop/restart``).
    """
    ...
    # `poormad gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged poormad
    ...
    detection always fails. Since every PoorMad runtime call inside the
    container drops to poormad via ``s6-setuidgid``, that silent failure
...
# in ``poormad_cli.gateway`` (systemd/launchd) and ``poormad_cli.gateway_windows``
...
# active (set via the ``poormad -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in poormad_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from poormad_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from poormad_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from poormad_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from poormad_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in poormad_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from poormad_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from poormad_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from poormad_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from poormad_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``poormad_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `poormad profile create`
# runs inside the container (Phase 4). Static services (main-poormad, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's POORMAD_HOME blindly;
    derive the shared profile root from the current POORMAD_HOME and map the
    ...
    poormad_home = Path(os.environ.get("POORMAD_HOME", "/opt/data"))
    if poormad_home.parent.name == "profiles":
        root = poormad_home.parent.parent
    else:
        root = poormad_home
...
# UID/GID of the in-image ``poormad`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the poormad user.
    ...
    (the ``poormad`` user — UID 10000 — running every PoorMad runtime
    ...
    is dispatched as the poormad user.
    ...
    with poormad ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           poormad:poormad, 0755 (parent must already exist)
    ``svc_dir/event/``                     poormad:poormad, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 poormad:poormad, 0755
    ``svc_dir/supervise/event/``           poormad:poormad, 03730
    ``svc_dir/supervise/control``          poormad:poormad, 0660    (FIFO)
    ...
    slot's supervise/ was poormad-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _POORMAD_UID, _POORMAD_GID)
        except PermissionError:
            # Running as the poormad user already — directory is poormad-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _POORMAD_UID, _POORMAD_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is poormad-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _POORMAD_UID, _POORMAD_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``poormad -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`poormad profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-poormad, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources POORMAD_HOME (and any extra env) via with-contenv —
             so e.g. ``-e POORMAD_HOME=/data/poormad`` is honored at run
          ...
          4. Drops to the poormad user and exec's
             ``poormad -p <profile> gateway run`` (or just ``poormad
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``poormad gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root POORMAD_HOME profile" (the implicit profile that exists at
        the top of $POORMAD_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``poormad_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$POORMAD_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``POORMAD_HOME``,
        ...
        allocator in ``poormad_cli.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export POORMAD_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's POORMAD_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``poormad gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-POORMAD_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The POORMAD_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own POORMAD_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "poormad gateway run --replace"
        else:
            gateway_cmd = f"poormad -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid poormad {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${POORMAD_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the POORMAD_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e POORMAD_HOME=/data/poormad``
        gets its logs under /data/poormad/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``poormad logs`` reads and what
        ...
        return (
            ...
            f': "${{POORMAD_HOME:=/opt/data}}"\n'
            f'log_dir="$POORMAD_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock as poormad when
            # this script starts as root. Never chown or unlink poormad-writable
            # volume paths from this restartable root-context script:
            # log/supervise/control is poormad-owned, so an unprivileged user
            # can race a pathname op through a symlink swap (CWE-59 /
            # CWE-367). Parent logs/gateways is seeded poormad-owned at stage2
            # boot (#45258; tests/docker/test_log_dir_seed.py).
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid poormad mkdir -p "$log_dir"\n'
            f'  s6-setuidgid poormad rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid poormad s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``poormad gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``poormad -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with poormad ownership
            ...
            # the poormad user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is poormad-owned (see ``_seed_supervise_skeleton``), and on
