"""Shared curses-based UI components for PoorMad CLI.

Used by `poormad tools` and `poormad skills` for interactive checklists.
...
_MENU_NAVIGATION_HANDLER: ContextVar[MenuNavigationHandler | None] = ContextVar(
    "poormad_menu_navigation_handler", default=None
)
_NUMBERED_BACK_ENABLED: ContextVar[bool] = ContextVar(
    "poormad_numbered_back_enabled", default=False
)
...
def _read_numbered_input(prompt_text: str) -> str | _NumberedNavigation:
    ...
    from poormad_cli.pt_input_extras import install_modify_other_keys_aliases
