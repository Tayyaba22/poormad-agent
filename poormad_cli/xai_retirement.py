...
Pure logic: walks a PoorMad config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `poormad doctor` and a future `poormad migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a PoorMad config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a PoorMad config and return retirement issues.
...
def apply_migration(
    ...
    from poormad_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `poormad migrate xai` reports nothing to migrate
