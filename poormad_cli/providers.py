"""
Single source of truth for provider identity in PoorMad.
...
2. **PoorMad overlays** — transport type, auth patterns, aggregator flags,
...
# -- PoorMad overlay ----------------------------------------------------------
# PoorMad-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class PoorMadOverlay:
    """PoorMad-specific provider metadata layered on top of models.dev."""
...
POORMAD_OVERLAYS: Dict[str, PoorMadOverlay] = {
    "moa": PoorMadOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": PoorMadOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nous": PoorMadOverlay(
        ...
        base_url_override="https://api.poormad.dev/v1",
    ),
    "openai-codex": PoorMadOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": PoorMadOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": PoorMadOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": PoorMadOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="POORMAD_QWEN_BASE_URL",
    ),
    "lmstudio": PoorMadOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": PoorMadOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": PoorMadOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": PoorMadOverlay(
    ...
    "kimi-for-coding": PoorMadOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": PoorMadOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": PoorMadOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": PoorMadOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": PoorMadOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": PoorMadOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": PoorMadOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": PoorMadOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": PoorMadOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": PoorMadOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": PoorMadOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "opencode-free": PoorMadOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": PoorMadOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": PoorMadOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": PoorMadOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": PoorMadOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": PoorMadOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "arcee": PoorMadOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": PoorMadOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": PoorMadOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "poormad", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. PoorMad overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + PoorMad overlay
    ...
    overlay = POORMAD_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + poormad extra
    ...
    if overlay is not None:
        # PoorMad-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
    ...
    # tencent-tokenhub) are absent from models.dev and POORMAD_OVERLAYS, so
...
def nous_api_mode(model: str = "") -> str:
    """Resolve the wire protocol for a PoorMad Portal model.
    ...
    historical PoorMad transport — so callers that don't yet know the model
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    Resolution order:
      ...
      2. PoorMad Portal dual-wire (model-derived; overlay alone is openai_chat).
      3. Known provider → transport → TRANSPORT_TO_API_MODE.
      4. Direct provider checks (bedrock).
      5. Default: 'chat_completions'.

    *model* is optional but required for dual-wire providers (PoorMad) whose
    ...
    # PoorMad is dual-wire: anthropic/* → Messages, everything else →
    # chat_completions. The PoorMad overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in POORMAD_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact PoorMad provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from poormad_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
