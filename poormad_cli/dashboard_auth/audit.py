...
Profile-aware location: ``$POORMAD_HOME/logs/dashboard-auth.log``.
...
from ``poormad_constants`` or other poormad_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$POORMAD_HOME/logs/dashboard-auth.log``.

    Uses ``poormad_constants.get_poormad_home()`` (a leaf module — no import
    ...
    return get_poormad_home() / "logs" / "dashboard-auth.log"
