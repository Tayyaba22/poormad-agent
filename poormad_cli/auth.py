"""
Multi-provider authentication system for PoorMad.

Supports OAuth device code flows (PoorMad Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.poormad/auth.json with cross-process file locking.
...
# httpx is imported lazily: it costs ~30ms at import time and poormad_cli.auth
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # ("poormad_cli.auth.httpx.Client", ...) keeps working in tests.
...
from poormad_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
# PoorMad Portal defaults
DEFAULT_NOUS_PORTAL_URL = "https://portal.poormad.dev"
DEFAULT_NOUS_INFERENCE_URL = "https://api.poormad.dev/v1"
DEFAULT_NOUS_CLIENT_ID = "poormad-cli"
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from poormad_cli import __version__ as _POORMAD_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _POORMAD_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"poormad-cli/{_POORMAD_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://poormad.dev/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://poormad.dev/docs/guides/oauth-over-ssh"
...
def normalize_actual_base_url(base_url: str) -> str:
    ...
    surface for PoorMad' Responses transport.
    """
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    ...
    "anthropic": ProviderConfig(
        ...
        # from it), so removing it would stop PoorMad finding a setup-token
    ...
    "opencode-free": ProviderConfig(
        ...
        # secret to configure. Select via `poormad model` / `/model free`.
        api_key_env_vars=(),
    ),
...
def get_anthropic_key() -> str:
    ...
    ``~/.poormad/.env`` so a deliberate key rotation isn't shadowed by a stale
    ...
    """
    from poormad_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from poormad_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.poormad/.env over os.environ so a deliberate key rotation
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``poormad auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `poormad model` to re-authenticate."
...
def _format_nous_entitlement_auth_error(error: AuthError) -> str:
    try:
        from poormad_cli.nous_account import (
        ...
        message = format_nous_portal_entitlement_message(
            account_info,
            capability="PoorMad model access",
        )
        if message:
            return message
    except Exception:
        pass
    return f"{error} Check credits or billing in PoorMad Portal, then retry."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("POORMAD_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.poormad/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_poormad_home() / "auth.json"
    # Seat belt: if pytest is running and POORMAD_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch POORMAD_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".poormad" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set POORMAD_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom POORMAD_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_poormad_root()
    except Exception:
        return None
    profile_home = get_poormad_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    (``_global_auth_file_path()`` -> ``get_default_poormad_root()``) + JSON
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".poormad" / "auth.json"
            try:
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared PoorMad store) don't share reentrancy
...
def _auth_store_lock(
    ...
    *,
    ...
    (outer) and the shared PoorMad lock SECOND (inner). All runtime
...
def _save_auth_store(auth_store: Dict[str, Any], target_path: Optional[Path] = None) -> Path:
    ...
    # poormad-agent install tree (#25821, #93050).
...
def _load_provider_state_with_source(
    ...
    global-root fallback state; persisting a rotated PoorMad refresh token only to
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``poormad auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``poormad auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``poormad auth add <provider>`` inside the profile, profile entries
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth or PoorMad device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    try:
        ...
        # ``provider: anthropic`` as a MoA advisor/aggregator has opted PoorMad
    ...
    # not by the user explicitly configuring anthropic in PoorMad.
    ...
    if pconfig is None:
        from poormad_cli.providers import get_provider
    ...
    # the user initiated inside PoorMad (manual add / device-code / PKCE), plus
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "poormad_pkce", "manual"}
    ...
    # explicit for users who never set PoorMad up for it. Only PoorMad-scoped
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `poormad logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from poormad_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'poormad doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'poormad model' for available providers, or run 'poormad doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `poormad auth add openrouter`
    ...
    # `poormad auth list` showing the credential while requests go out with no
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(_scoped_key_env(env_var)):
                ...
                # OAuth but has a stale key in ~/.poormad/.env isn't silently
    ...
    raise AuthError(
        "No inference provider configured. Run 'poormad model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.poormad/.env.",
...
# Allowlist of valid PoorMad Portal hosts. A portal_base_url outside this
...
# Allowlist of hosts the PoorMad Portal proxy is willing to forward inference
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    Mirrors ``_nous_inference_env_override()``: ``POORMAD_PORTAL_BASE_URL`` /
    ``NOUS_PORTAL_BASE_URL`` are the documented dev/staging escape hatch for
    pointing PoorMad at a non-production PoorMad Portal (e.g. a hosted agent
    provisioned on nous-account-service's `staging` environment, which stamps
    ``POORMAD_PORTAL_BASE_URL=https://portal.staging-poormad.com`` into
    ...
    return _optional_base_url(
        os.getenv("POORMAD_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
    )
...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    if reason is None:
        return
    raise AuthError(
        "PoorMad Portal access token is not a usable inference JWT "
        f"({reason}). Re-authenticate with: poormad auth add nous",
        provider="nous",
        code=reason,
...
def _log_nous_invoke_jwt_selected(
    *,
    ...
    logger.debug("PoorMad inference auth: using NAS invoke JWT")
    _oauth_trace(
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("POORMAD_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `poormad model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.poormad/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("POORMAD_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set POORMAD_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("POORMAD_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("POORMAD_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("POORMAD_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `poormad auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `poormad auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `poormad auth spotify` first.",
                provider="spotify",
        ...
        if should_refresh:
            try:
                ...
                if exc.relogin_required and state.get("refresh_token"):
                    ...
                    # Mirrors the PoorMad / xAI-OAuth / Codex-OAuth / MiniMax pattern.
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `poormad auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.poormad/.env, and return it.
    ...
    print("       App name:     anything (e.g. poormad-agent)")
    ...
    # Persist so subsequent `poormad auth spotify` runs skip the wizard.
    save_env_value("POORMAD_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("POORMAD_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved POORMAD_SPOTIFY_CLIENT_ID to ~/.poormad/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "POORMAD_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize PoorMad:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"PoorMad is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.poormad/auth.json (not ~/.codex/)
#
# PoorMad maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from PoorMad auth store (~/.poormad/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `poormad auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `poormad auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `poormad auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `poormad auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``poormad setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``poormad auth add openai-codex``
      ...
      ``poormad auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to PoorMad auth store (~/.poormad/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``poormad auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into PoorMad auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating PoorMad auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run poormad auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `poormad auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to PoorMad auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. PoorMad keeps its OWN
        ...
        # the Codex CLI (or another PoorMad process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from PoorMad's own Codex token store.
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("POORMAD_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("POORMAD_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other PoorMad processes
...
def _probe_codex_quota_restored(
    ...
    *,
    ...
    PoorMad persists a Codex 429's ``reset_at`` locally and freezes the
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.poormad/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `poormad model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `poormad model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `poormad model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `poormad model`.",
            provider="xai-oauth",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.poormad/auth.json even when POORMAD_HOME points at a profile path
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``poormad auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.poormad/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `poormad model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``POORMAD_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `poormad model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older PoorMad (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `poormad model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``poormad model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("POORMAD_XAI_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("POORMAD_XAI_BASE_URL", "").strip().rstrip("/")
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("POORMAD_CA_BUNDLE")
...
def _nous_device_auth_timeout_message(portal_base_url: str) -> str:
    """Actionable timeout text for PoorMad device-code login failures.
...
# =============================================================================
# PoorMad Portal — token refresh and model discovery
# =============================================================================

# -----------------------------------------------------------------------------
# Shared PoorMad token store — lets OAuth credentials persist across profiles
# so a new `poormad --profile <name> auth add nous --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${POORMAD_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<poormad-root>/shared/nous_auth.json`` where ``<poormad-root>`` is what
# ``get_default_poormad_root()`` returns — ``~/.poormad`` on Linux/macOS,
# ``%LOCALAPPDATA%\poormad`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's POORMAD_HOME so named profiles (which
# typically live under ``<poormad-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    """Resolve the directory that holds the shared PoorMad token store.

    Honors ``POORMAD_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<poormad-root>/shared/``, where ``<poormad-root>`` is what
    :func:`poormad_constants.get_default_poormad_root` returns — so
    Linux/macOS classic installs land at ``~/.poormad/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\poormad\\shared\\``, and
    Docker / custom ``POORMAD_HOME`` deployments at
    ``<POORMAD_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("POORMAD_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from poormad_constants import get_default_poormad_root
    return get_default_poormad_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's PoorMad root, refuse rather than silently corrupt cross-profile
    # state. Tests must set POORMAD_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from poormad_constants import get_default_poormad_root
        real_home_shared = (
            get_default_poormad_root() / "shared" / NOUS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared PoorMad auth store during test run: "
                f"{path}. Set POORMAD_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared PoorMad OAuth store.
    ...
    try:
        ...
        # No POORMAD_HOME yet (pre-setup): fall through without locking.
        yield
        return
    ...
    with _file_lock(
        ...
        timeout_seconds,
        "Timed out waiting for shared PoorMad auth lock",
    ):
        yield


def _merge_shared_nous_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local PoorMad state."""
...
def _write_shared_nous_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the PoorMad OAuth state to the shared store.
    ...
    try:
        with _nous_shared_store_lock():
            ...
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed PoorMad
        ...
        logger.debug("Failed to write shared PoorMad auth store: %s", exc)


def _read_shared_nous_state() -> Optional[Dict[str, Any]]:
    """Return the shared PoorMad OAuth state if present and well-formed.
    ...
    try:
        ...
        logger.debug("Shared PoorMad auth store at %s is unreadable: %s", path, exc)
        return None
    if not isinstance(payload, dict):
        return None
...
def _clear_shared_nous_state(reason: str) -> None:
    """Remove the shared PoorMad OAuth store after a terminal token failure."""
    try:
        with _nous_shared_store_lock():
            path = _nous_shared_store_path()
            try:
        ...
        logger.debug("Failed to clear shared PoorMad auth store: %s", exc)


def _is_terminal_nous_refresh_error(exc: Exception) -> bool:
    """True when retrying the same PoorMad refresh token cannot succeed."""
    return (
...
def _quarantine_nous_oauth_state(
    ...
    *,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — PoorMad
    # has a known bug class where credential-shaped literals get corrupted in logs.
    forensic: Dict[str, Any] = {
        "reason": reason,
        "error_code": error.code,
        # No session_id field exists on PoorMad state; provenance is client_id +
    ...
    logger.warning(
        "PoorMad OAuth state quarantined (terminal auth death): %s",
...
def _quarantine_nous_pool_entries(
    ...
    *,
    ...
    """Remove singleton-seeded PoorMad pool entries that contain dead OAuth state."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return False
...
def _try_import_shared_nous_state(
    *,
    ...
    """Attempt to rehydrate PoorMad OAuth state from the shared store.
    ...
    try:
        ...
        logger.debug("Shared PoorMad import failed: %s", exc)
        return None
    except Exception as exc:
        _oauth_trace(
            "nous_shared_import_failed",
...
def _refresh_access_token(
    *,
    ...
    # Detect the OAuth 2.1 "refresh token reuse" signal from the PoorMad portal
    ...
    # called POST /api/oauth/token with PoorMad's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, PoorMad's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "PoorMad Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another PoorMad install sharing "
            "~/.poormad/auth.json) called POST /api/oauth/token with PoorMad's "
            "refresh token without persisting the rotated token back.\n"
            "PoorMad refresh tokens are single-use — only PoorMad may call the "
            "refresh endpoint. For health checks, use `poormad auth status` "
            "instead.\n"
            "Re-authenticate with: poormad auth add nous"
        )
        relogin = True
...
def fetch_nous_models(
    *,
    ...
    """Fetch available model IDs from the PoorMad inference API."""
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip PoorMad models — they're not reliable for agentic tool-calling
            if "poormad" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    """Resolve a refresh-aware PoorMad Portal access token for managed tool gateways."""
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError(
                "PoorMad is not logged into PoorMad Portal.",
                provider="nous",
                relogin_required=True,
            )

        # POORMAD_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted
        ...
        with _nous_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for PoorMad Portal login.",
                    provider="nous",
                    relogin_required=True,
                )
...
def refresh_nous_oauth_pure(
    ...
    *,
    ...
    """Refresh PoorMad OAuth state without mutating auth.json directly.
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise AuthError(
                    "No refresh token is available for PoorMad Portal.",
                    provider="nous",
                    relogin_required=True,
                )
            refreshed = _refresh_access_token(
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    """Refresh PoorMad OAuth from a state dict. Thin wrapper around refresh_nous_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_nous_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "poormad-cli"),
...
def persist_nous_credentials(
    ...
    *,
    ...
    """Persist PoorMad OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    PoorMad credentials are read at runtime from two independent locations:
    ...
    Historically ``poormad auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``poormad auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `poormad auth add nous --type oauth`. Best-
...
def _sync_nous_pool_from_auth_store() -> None:
    ...
    try:
        ...
        logger.debug("Failed to sync PoorMad credential pool from auth store: %s", exc)
...
def resolve_nous_runtime_credentials(
    *,
    ...
    """
    Resolve PoorMad inference credentials for runtime use.
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError("PoorMad is not logged into PoorMad Portal.",
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            portal_url = (
                _optional_base_url(state.get("portal_base_url"))
                or os.getenv("POORMAD_PORTAL_BASE_URL")
        ...
        def _persist_state(reason: str) -> None:
            ...
            # the mtime-keyed PoorMad auth-status cache warm during read paths.
            if (
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for PoorMad Portal login.",
                                provider="nous", relogin_required=True)
    ...
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a PoorMad inference API key",
...
# which does a synchronous OAuth refresh POST to portal.poormad.dev. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`poormad tools`, status panels,
# subscription-feature checks) call it many times per render — `poormad tools` → "All Platforms"
...
# `poormad auth login/logout/add/remove` invalidate naturally on the next call.
...
def invalidate_nous_auth_status_cache() -> None:
    ...
    Call this from any code path that mutates PoorMad auth state without going
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for PoorMad auth.
...
def get_nous_auth_status_local() -> Dict[str, Any]:
    """Refresh-free PoorMad auth snapshot for read-only display surfaces.
...
def get_nous_session_validity() -> str:
    """Classify the PoorMad bootstrap session for the dashboard /api/status probe.

    Returns one of:
      - ``"valid"``    — a usable PoorMad credential is present (login healthy).
      - ``"terminal"`` — the PoorMad session has taken a terminal auth failure
      ...
      - ``"unknown"``  — indeterminate (no PoorMad provider state, or a transient/
    ...
    # into the PoorMad provider state when it clears dead tokens (the exact path
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `poormad auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `poormad auth` and
    # `poormad model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_api_key_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    # the PoorMadOverlay keyless flag — the same source the provider catalog
    # and GUI contract tests use.
    try:
        from poormad_cli.providers import POORMAD_OVERLAYS
        _overlay = POORMAD_OVERLAYS.get(provider_id)
    except Exception:
        _overlay = None
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("POORMAD_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("POORMAD_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``poormad doctor`` runs the live
    ...
    try:
        from poormad_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on PoorMad' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `poormad doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_api_key_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id in {"kimi-coding", "kimi-coding-cn"}:
        ...
        try:
            from poormad_cli.copilot_auth import (
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set POORMAD_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from poormad_cli.config import clear_model_endpoint_credentials
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `poormad logout` historically keyed off auth.json.active_provider only.
...
def _confirm_selection_guards(
    ...
    *,
    ...
    :mod:`poormad_cli.model_selection_guards` and shows one [y/N] confirm with
...
def _prompt_model_selection(
    ...
    # Sale chrome (★ / -N% / was) is PoorMad Portal-only — never for OpenRouter
    ...
    def _confirmed_selection(mid: str) -> Optional[str]:
        if not mid:
            return None
        # Unified guard registry (poormad_cli.model_selection_guards): the cost
    ...
    # Leave room for a leading "★ " on sale rows (PoorMad only).
    ...
    try:
        ...
        from poormad_cli.model_search import model_search_text
    ...
    from poormad_cli.curses_ui import format_radio_item_ansi
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from poormad_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'poormad model' or 'poormad setup' instead."""
    print("The 'poormad login' command has been removed.")
    print("Use 'poormad auth' to manage credentials,")
    print("'poormad model' to select a provider, or 'poormad setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.poormad/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing PoorMad-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in PoorMad auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("PoorMad will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("POORMAD_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("PoorMad will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — PoorMad gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(PoorMad creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to PoorMad auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in PoorMad auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(PoorMad creates its own local OAuth session)")
    print()
    ...
    # xAI credential re-enabled. ``poormad auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``poormad auth list`` would show
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to PoorMad auth store (~/.poormad/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting PoorMad login via MiniMax ({region}) OAuth...")
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    provider. Mirrors the PoorMad / xAI-OAuth / Codex-OAuth quarantine pattern
    so subsequent calls fail fast without a network retry.
    """
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `poormad model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``poormad status`` that just want to know
...
def _nous_device_code_login(
    *,
    ...
    """Run the PoorMad device-code flow and return full OAuth state without persisting."""
    ...
    print(f"Starting PoorMad login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `poormad model` again to finish setup.")
            raise SystemExit(1)
        raise


def nous_token_has_billing_scope() -> bool:
    """Return True if the currently-held PoorMad token carries ``billing:manage``.
...
def step_up_nous_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``POORMAD_PORTAL_BASE_URL`` set
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    """PoorMad Portal device authorization flow."""
    ...
    try:
        ...
        # flow, check the shared store for an existing PoorMad credential
        ...
        if shared:
            ...
            print()
            if shared_path:
                print(f"Found existing PoorMad OAuth credentials at {shared_path}")
            else:
                print("Found existing shared PoorMad OAuth credentials")
            try:
                do_import = input("Import these credentials? [Y/n]: ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                print("Rehydrating PoorMad session from shared credentials...")
        ...
        try:
                ...
                if free_tier:
                    try:
                        ...
                        unavailable_message = (
                            format_nous_portal_entitlement_message(
                                _account_info,
                                capability="paid PoorMad models",
                            )
                            or ""
                        )
                    except Exception:
                    ...
                    # as free so users on older PoorMad builds still see
            ...
            if model_ids:
                ...
                print("No curated models available for PoorMad Portal.")
        except Exception as exc:
        ...
        # them to PoorMad with a mismatched model.  The PoorMad OAuth tokens
        ...
        if not selected_model:
            ...
            with _auth_store_lock():
                ...
                _save_auth_store(auth_store)
            print()
            print("No provider change. PoorMad credentials saved for future use.")
            print("  Run `poormad model` again to switch to PoorMad Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("PoorMad will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `poormad model` or configure an API key to use PoorMad.")
        else:
