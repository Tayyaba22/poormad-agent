...
- Keep data: Remove code but keep ~/.poormad/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove PoorMad PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing poormad-agent or poormad PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# PoorMad" comment and following line
                if '# PoorMad' in line or '# poormad-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('poormad' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing poormad
                if 'poormad' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a PoorMad-owned file, and
...
def remove_wrapper_script():
    """Remove the poormad wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "poormad",
        Path.home() / ".local" / "bin" / "poormad-acp",
        Path.home() / ".local" / "bin" / "poormad-agent",
        Path("/usr/local/bin/poormad"),
        Path("/usr/local/bin/poormad-acp"),
        Path("/usr/local/bin/poormad-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains poormad_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'poormad_cli' in content or 'poormad-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(poormad_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``poormad`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this PoorMad home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (poormad_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``poormad gateway run`` processes
    ...
    try:
        from poormad_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from poormad_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``POORMAD_HOME`` and ``POORMAD_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\poormad\git\cmd``, ``%LOCALAPPDATA%\poormad\git\bin``,
#      ``%LOCALAPPDATA%\poormad\git\usr\bin``, ``%LOCALAPPDATA%\poormad\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\poormad\git\`` and Node to
#      ``%LOCALAPPDATA%\poormad\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``poormad dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\poormad\gateway-service\`` and sometimes
...
def _poormad_path_markers(poormad_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Path-entry substrings that identify PoorMad-owned User-PATH entries.

    ``include_managed_bin`` adds the managed binary dir (``<root>\\bin``,
    holding the poormad launchers and the managed uv) — only wanted when
    ...
    """
    root = str(poormad_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare poormad-agent install dir.
    markers = [root + "\\poormad-agent", root + "\\git", root + "\\node", root + "\\venv"]
    if include_managed_bin:
        markers.append(root + "\\bin")
    # Also match if POORMAD_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "poormad".  We don't want to catch
    # unrelated entries like "cpoormad-foo" or "ephermeral", so we look for
    # backslash-poormad as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(poormad_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip PoorMad-owned entries from User-scope PATH in the registry.
    ...
    ``include_managed_bin`` adds ``<poormad_home>\\bin`` (the managed binary
    dir holding the poormad launchers and the managed uv) to the sweep. Only
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _poormad_path_markers(poormad_home, include_managed_bin=include_managed_bin)
            kept: list[str] = []
...
def remove_poormad_env_vars_windows() -> list[str]:
    """Delete POORMAD_HOME and POORMAD_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("POORMAD_HOME", "POORMAD_GIT_BASH_PATH"):
                try:
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``poormad`` launchers install.ps1 staged in the managed
    binary dir (the default PoorMad root's ``bin``, next to the managed uv).

    Every uninstall mode deletes the code checkout, so the launchers —
    which invoke ``<checkout>\\venv\\Scripts`` — would otherwise dangle:
    ``poormad`` in a new terminal resolves to a launcher whose target is
    ...
    ``_install_repair._quarantine_running_poormad_exe`` relies on), so
    ...
    *windows* is an injectable platform verdict for tests (same pattern as
    ...
    try:
        ...
        from poormad_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from poormad_constants import get_default_poormad_root

        bin_dir = get_default_poormad_root() / "bin"
    except Exception as e:
...
def _is_default_poormad_home(poormad_home: Path) -> bool:
    """Return True when ``poormad_home`` points at the default (non-profile) root."""
    try:
        from poormad_constants import get_default_poormad_root
        return poormad_home.resolve() == get_default_poormad_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its POORMAD_HOME directory.

    We shell out to ``poormad -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current POORMAD_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m poormad_cli.main` so we don't depend on a `poormad`
    #    wrapper that may be half-removed mid-uninstall.
    poormad_invocation = [_sys.executable, "-m", "poormad_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                poormad_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's POORMAD_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``poormad uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$POORMAD_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from poormad_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(poormad_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         🌑 PoorMad Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No PoorMad Chat GUI installation was found.")
        print(f"  Checked: {poormad_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The PoorMad stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(poormad_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The PoorMad at {poormad_home / 'poormad-agent'}")
        print(f"  • Your config, sessions, and secrets under {poormad_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The PoorMad is still installed. Run 'poormad' to use the CLI,")
    print("or 'poormad uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.poormad/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.poormad/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie POORMAD_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_poormad_home(poormad_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.poormad data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            🌑 PoorMad Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {poormad_home / 'config.yaml'}")
    print(f"  Secrets: {poormad_home / '.env'}")
    print(f"  Data:    {poormad_home / 'cron/'}, {poormad_home / 'sessions/'}, {poormad_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their POORMAD_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL PoorMad data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the PoorMad code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, poormad_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • PoorMad PATH entries from shell configs / Windows User PATH")
    print("  • PoorMad wrapper scripts and PoorMad-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • PoorMad config/data: {poormad_home}")
        if _is_default_poormad_home(poormad_home):
        ...
        print(f"  • Keep PoorMad config/data: {poormad_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``poormad`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$POORMAD_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in poormad_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\poormad\git\cmd, not %LOCALAPPDATA%.
        # The managed binary dir (poormad\bin: launchers + managed uv) leaves
        ...
        sweep_managed_bin = full_uninstall and _is_default_poormad_home(poormad_home)
        removed_path_entries = remove_path_from_windows_registry(
            Path(os.path.expandvars(str(poormad_home))),
        ...
        if removed_path_entries:
            ...
            log_info("No PoorMad-owned PATH entries in User environment")

        log_info("Removing POORMAD_HOME / POORMAD_GIT_BASH_PATH User env vars...")
        removed_env = remove_poormad_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No PoorMad-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing poormad command...")
    ...
    #     would dangle — `poormad` in a new terminal would resolve and then
    #     error on its missing venv target, worse than command-not-found.
    if _is_windows():
        log_info("Removing Windows poormad launchers...")
        ...
        if removed_launchers:
            ...
            log_info("No Windows poormad launchers found")

    # 3b. Remove node/npm/npx symlinks the installer left in ~/.local/bin
    #     (only when they still point into this PoorMad home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing PoorMad-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(poormad_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No PoorMad-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(poormad_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE POORMAD_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from poormad_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(poormad_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.poormad/, just remove the poormad-agent subdir
            if poormad_home in project_root.parents or project_root.parent == poormad_home:
    ...
    #     under POORMAD_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(poormad_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(poormad_home)
    ...
    # 5. Optionally remove ~/.poormad/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile POORMAD_HOME dirs live under
        ...
        try:
            if poormad_home.exists():
                shutil.rmtree(poormad_home)
                log_success(f"Removed {poormad_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {poormad_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {poormad_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {poormad_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://poormad.dev/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://poormad.dev/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using PoorMad! 🌑")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m poormad_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import poormad_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``poormad_constants`` + ``poormad_cli.colors``
    (and lazily ``poormad_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m poormad_cli.uninstall")
    parser.add_argument(
        "--mode",
