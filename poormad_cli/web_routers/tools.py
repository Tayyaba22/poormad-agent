...
late-binding seam in :mod:`poormad_cli.web_deps` (``late`` for callables,
...
from poormad_cli.web_deps import late, LateState
from poormad_cli.web_models import (
...
_log = logging.getLogger("poormad_cli.web_server")
...
_spawn_poormad_action = late("_spawn_poormad_action")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from poormad_cli.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `poormad tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `poormad computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``poormad tools`` picker shows
            ...
            if cat:
                ...
                for prov in _visible_providers(cat, config, force_fresh=True):
                    ...
                    row = {
                        ...
                        # including logged-out PoorMad Subscription rows and never-run
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `poormad tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``poormad tools``
    ...
    Managed PoorMad rows (``managed_nous_feature``) additionally report the
    ...
    existing PoorMad Portal OAuth flow (``POST /api/providers/oauth/nous/start``)
    and refetch.
    """
    from poormad_cli.tools_config import (
        TOOL_CATEGORIES,
    ...
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            # Entitlement check for managed PoorMad rows — mirrors the gate the CLI
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.poormad/.env`` via ``save_env_value`` —
    the same store ``poormad tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``poormad tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``poormad -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same POORMAD_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``poormad computer-use permissions grant`` as a background action.
