"""
Baked-in build metadata for PoorMad.

Source installs report their git revision live via ``git rev-parse`` (see
``poormad_cli/dump.py`` and ``poormad_cli/banner.py``).  That doesn't work inside
...
To make ``poormad dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$POORMAD_GIT_SHA``
arg into ``<project_root>/.poormad_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``POORMAD_GIT_SHA`` build-arg fall through to live-git
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".poormad_build_sha"
...
def get_code_identity(refresh: bool = False) -> dict:
    ...
    rev-parse`` for source installs, the baked ``.poormad_build_sha`` for
...
def get_build_sha(short: int = 8) -> Optional[str]:
    ...
    Reads ``<project_root>/.poormad_build_sha`` if present.  The file is
    written by the Dockerfile's ``POORMAD_GIT_SHA`` build-arg and contains
