...
1. **Update receipt** — a machine-readable JSON record of what one
   ``poormad update`` run discovered, did, skipped (and why), written to
   ``<POORMAD_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``poormad_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``poormad update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``poormad update`` run."""
...
def _receipt_dir() -> Path:
    ...
    return get_poormad_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``poormad update`` has many early-termination paths (Windows
