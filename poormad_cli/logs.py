"""``poormad logs`` — view and filter PoorMad log files.
...
Usage examples::
    ...
    poormad logs                    # last 50 lines of agent.log
    poormad logs -f                 # follow agent.log in real time
    poormad logs errors             # last 50 lines of errors.log
    poormad logs gateway -n 100    # last 100 lines of gateway.log
    poormad logs gui -f            # follow gui.log (dashboard/pty/ws)
    poormad logs desktop -f        # follow desktop.log (Electron app boot/backend)
    poormad logs --level WARNING    # only WARNING+ lines
    poormad logs --session abc123   # filter by session ID substring
    poormad logs --component tools  # only tool-related lines
    poormad logs --since 1h         # lines from the last hour
    poormad logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_poormad_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when PoorMad runs — try 'poormad chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_poormad_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_poormad_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_poormad_home()}/logs/")
        return

    print(f"Log files in {display_poormad_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'poormad chat' to generate logs)")
