"""Image-authored deployment provenance for immutable PoorMad runtimes.

The published image bakes ``/etc/poormad/image-provenance.json`` outside both
``$POORMAD_HOME`` and the mutable checkout.  A bind-mounted checkout (including
...
IMAGE_PROVENANCE_PATH = Path("/etc/poormad/image-provenance.json")
