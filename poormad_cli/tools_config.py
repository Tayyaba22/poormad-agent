"""
Unified tool configuration for PoorMad.

`poormad tools` and `poormad setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.poormad/config.yaml under
...
from poormad_cli.nous_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    PoorMadSubscriptionFeatures,
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from poormad_cli.nous_account import format_nous_portal_entitlement_message
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``poormad tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`poormad_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from poormad_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _POORMAD_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `poormad tools` → Video Generation, which walks
...
# `poormad tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `poormad tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from poormad_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `poormad tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``poormad tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from poormad_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from poormad_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``poormad tools`` checklist actually offers
    for ``platform``.
    ...
    so ``poormad tools`` never claims to add or remove a toolset the user was
...
from poormad_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "web": {
        ...
        #   - "PoorMad Subscription" — managed Firecrawl billed via PoorMad
    ...
    "image_gen": {
        ...
        #   - "PoorMad Subscription" — managed FAL billed via the PoorMad
        #     subscription (requires_nous_auth + override_env_vars).
    ...
    "video_gen": {
        ...
        "icon": "🎬",
        # "PoorMad Subscription" row mirrors the image_gen pattern — managed
        # FAL video generation billed via the PoorMad Portal.  Plugin-backed
        ...
        "providers": [
            {
                ...
                # "PoorMad Subscription" we set video_gen.provider = "fal"
    ...
    "x_search": {
        ...
        "setup_note": (
            "PoorMad routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        # backend, never on the paid PoorMad Subscription gateway row:
        #   - "Local Browser" — non-cloud option, no CloudBrowserProvider.
        #   - "PoorMad Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via PoorMad subscription (requires_nous_auth +
        ...
        "providers": [
            ...
            {
                "name": "PoorMad Subscription (Browser Use cloud)",
                "badge": "subscription",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set POORMAD_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "POORMAD_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "POORMAD_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "POORMAD_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "POORMAD_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "POORMAD_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `poormad model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("POORMAD_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    """Reduce a driver's ``--version`` output to one short status line.

    A binary selected by ``POORMAD_CUA_DRIVER_CMD`` is not obliged to answer
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the PoorMad telemetry policy applied.
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports PoorMad' runtime contract."""
    import time
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $POORMAD_HOME/bin is never on PATH, so a bare which()
    # misses the uv PoorMad installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``poormad update`` and
      by ``poormad computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``poormad update`` sets
    ...
    a further ~600s wait on Windows). ``poormad computer-use install
    ...
    ``poormad update`` already prints a contextual line before its update
    ...
    ``poormad update`` and must not inherit the explicit install command's
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `poormad update` calls this
    ...
    override = os.environ.get("POORMAD_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        _print_warning(
            "    POORMAD_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    # An installed driver that fails PoorMad' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # PoorMad' own minimum requirement IS the confirmation that an upgrade is
    ...
    # contract landed, with `poormad update` declining to refresh).
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but PoorMad cannot use its current "
            f"runtime contract: {reason}."
        )
        if os.environ.get("POORMAD_CUA_DRIVER_CMD", "").strip():
            _print_info(
        ...
        if is_windows and require_confirmed_update:
            ...
            _print_info(
                ...
                "poormad computer-use install --upgrade"
            )
            return False
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `poormad computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `poormad update` (require_confirmed_update=True) keeps the
    ...
    # `poormad computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary and not repair_existing:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: poormad computer-use install --upgrade"
            )
            return True
    ...
    if ok and repair_existing:
        ...
        if not repaired.get("ready"):
            ...
            _print_info("    Run: poormad computer-use doctor")
            return False
    if ok and before:
        try:
...
# Optional refreshes launched by ``poormad update`` are quiet and unattended.
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    # Unattended refreshes (installer_timeout set by `poormad update`) fail
    ...
    def _reap_after_timeout(proc):
        ...
        ceiling stops bounding anything and ``poormad update`` hangs past its
    ...
    try:
        # When not verbose (e.g. `poormad update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `poormad update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.poormad/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and installed_binary:
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / PoorMad process.")
            return True
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    Firecrawl, and the PoorMad-managed cloud rows all execute through
    ...
    check — only the PoorMad-managed ``$POORMAD_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary PoorMad installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # — it resolves lazily via npx (or a global/PoorMad-managed install)
        ...
        # (PATH -> Homebrew/PoorMad-managed node -> npx) rather than a bare
        # shutil.which — PoorMad-managed-Node-only setups resolve agent-browser
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/poormad/poormad-agent:latest"
            )
            return

        # browser_cmd was already resolved above (same PATH -> Homebrew ->
        # PoorMad-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — PoorMad-managed-Node-only setups resolve npx only through the
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.poormad/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.poormad/config.yaml")
        ...
        # Run the full `poormad auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.poormad/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from poormad_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: poormad auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `poormad auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: poormad auth spotify")
        except Exception as exc:
        ...
        try:
            from poormad_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: poormad plugins enable observability/langfuse")
        _print_info("    Restart PoorMad for tracing to take effect.")
        _print_info("    Verify: poormad plugins list")
        ...
        try:
            ...
            _print_info("    Run later: poormad auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `poormad auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: poormad auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: poormad auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``poormad tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``poormad tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: poormad tools post-setup <key>")
        return 2
...
def enabled_mcp_server_names(config: dict) -> Set[str]:
    ...
    try:
        from poormad_cli.plugins import (
            get_plugin_manager,
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``poormad-discord``, which contains those
...
#:
#: Saving ``poormad tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[poormad-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``poormad tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"poormad-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (poormad-acp, poormad-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``poormad-discord``) is an opt-in to the platform's native default-off
    ...
    # explicit config like ``[poormad-cli, a2a]`` must survive filtering just
    ...
    # "poormad-cli" (which include all _POORMAD_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[poormad-cli, spotify]`` after enabling Spotify via ``poormad
        ...
        # (e.g. "poormad-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``poormad tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `poormad tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"poormad-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("poormad-")}
    ...
    # shipped as a bundled plugin but user must opt in via `poormad tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `poormad tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by poormad tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # JSON-array string (e.g. "['memory']") from `poormad config set` or a
    ...
    # is invalid (e.g. a migration or hand-edit left `poormad` instead of
    # `poormad-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `poormad update`/`poormad doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `poormad tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (poormad-cli, poormad-telegram, etc.)
    ...
    # Opening `poormad tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    features: Optional[PoorMadSubscriptionFeatures] = None,
) -> bool:
...
def _estimate_tool_tokens() -> Dict[str, int]:
    ...
    from poormad_constants import poormad_home_key

    scope = poormad_home_key()
...
# ("PoorMad Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("PoorMad Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    PoorMad-managed Tool Gateway rows (``managed_nous_feature``) are always
    shown — even to logged-out / unentitled users — so the picker advertises
    that the capability exists.  Selecting one drives an inline PoorMad Portal
    ...
    *activates* the gateway once paid access is confirmed.
    ...
    for provider in cat.get("providers", []):
        # PoorMad-managed Tool Gateway rows stay visible regardless of auth —
    ...
    # later) so the picker lists them alongside FAL / PoorMad Subscription.
    ...
    # remaining hardcoded rows ("PoorMad Subscription", "Firecrawl
    ...
    # the hardcoded "PoorMad Subscription" / "Local Browser" / "Camofox" rows
...
def _hidden_nous_gateway_message(
    ...
    *,
    ...
    """Deprecated: PoorMad Tool Gateway rows are no longer hidden.

    Previously this returned a "log in / upgrade" banner shown above a
    category when its PoorMad-managed rows were filtered out for unentitled
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `poormad tools` gets a silent no-op
...
# Python dependencies installed explicitly through ``poormad tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``poormad tools`` Python dependencies present in this runtime."""
    return [
        name
...
def _agent_browser_installed() -> bool:
    ...
    from poormad_cli.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``poormad tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from poormad_cli.nous_subscription import _has_agent_browser
...
def provider_readiness_status(
    ...
    *,
    ...
    - ``"needs_auth"``  — needs a sign-in: PoorMad Portal login/entitlement for
    ...
    zero-env-var row, including logged-out PoorMad Subscription rows).

    ``features`` (a ``PoorMadSubscriptionFeatures``) can be passed to avoid
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_nous_message = _hidden_nous_gateway_message(
        cat,
        config,
        f"the PoorMad Subscription provider for {name}",
        force_fresh=force_fresh,
    )
    ...
    if len(providers) == 1:
        ...
        # When the user is logged into PoorMad, surface a marker on providers
        # whose access is included in their subscription so it's visually
        # obvious which options cost extra vs. cost nothing on top of PoorMad.
        try:
        ...
        for p in providers:
            ...
            # Mark PoorMad-managed entries. Logged-in paid subscribers get the
            # "included" star; everyone else gets a "via PoorMad Portal" hint so
            ...
            if p.get("managed_nous_feature"):
                if _nous_logged_in:
                    sub_marker = "  ★ Included with your PoorMad subscription"
                else:
                    sub_marker = "  ★ via PoorMad Portal (login on select)"
...
def _is_provider_active(
    ...
    *,
    ...
    if plugin_name and not provider.get("managed_nous_feature"):
        # Managed (PoorMad-subscription) entries fall through to the
...
def _select_plugin_image_gen_provider(plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    ...
    ``use_gateway=True`` marks a provider picked through the PoorMad-managed
...
# Kept in sync with the dashboard selects (poormad_cli/web_server.py
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    post-setup hooks, gate on PoorMad auth, or run interactive model pickers.
    ...
    category. Managed "PoorMad Subscription" rows write ``nous``; BYOK rows
    ...
    # image_gen/video_gen "PoorMad Subscription" rows carry only
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, gate on PoorMad Portal auth, or run interactive model
...
def _configure_provider(
    ...
    *,
    ...
    # PoorMad-managed Tool Gateway backends are always listed (see
    # _visible_providers), but only *activate* once the user has paid PoorMad
    ...
    # "enable all tools" prompt (that lives in `poormad model`).
    if managed_feature:
        from poormad_cli.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
        ...
        if not ensure_nous_portal_access(
            capability=f"{provider.get('name', 'the PoorMad Tool Gateway')}",
            coverage_category=MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature),
        ):
            _print_warning(
                "  Not enabled — PoorMad Portal access is required for this backend."
            )
            return
    ...
    if provider.get("requires_nous_auth") and not managed_feature:
        ...
        if not features.nous_auth_present or not entitled:
            message = format_nous_portal_entitlement_message(
                ...
                capability=f"{provider.get('name', 'PoorMad Subscription')}",
            )
            _print_warning(
                f"  {message or 'PoorMad Subscription is only available after logging into PoorMad Portal.'}"
            )
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your PoorMad subscription.")
    ...
    # PoorMad-managed sibling, show a single dim hint so users know
    # they can avoid the key entirely via a Portal subscription.
    # Suppressed when the user is already authed to PoorMad.
    ...
    if _show_portal_hint:
        _print_info("  Available through PoorMad Portal subscription.")
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``poormad model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``poormad model`` aux-task
    ...
    """
    try:
        from poormad_cli.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `poormad model`, then re-run this."
        )
        return
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline PoorMad Portal login + entitlement gate as _configure_provider:
    ...
    if not env_vars:
        ...
        if backend:
            ...
            if backend == "fal":
                img_cfg = config.setdefault("image_gen", {})
                if isinstance(img_cfg, dict):
                    # A managed (PoorMad Subscription) row also carries
    ...
    if backend:
        ...
        if backend == "fal":
            ...
            if isinstance(img_cfg, dict):
                ...
                # never clobber a PoorMad-managed pick back onto direct keys.
                img_cfg["provider"] = (
                    NOUS_MANAGED_PROVIDER if managed_feature else "fal"
                )
                img_cfg.pop("use_gateway", None)
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `poormad model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `poormad tools` and `poormad setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("🌑 PoorMad Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://poormad.dev/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your PoorMad subscription defaults", Colors.GREEN))
    ...
    print(color(f"  Tool configuration saved to {display_poormad_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'poormad' or gateway restart.", Colors.DIM))
    print()
