...
``$POORMAD_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-poormad-setup
(the stage2 hook) has chowned the volume and seeded $POORMAD_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$POORMAD_HOME``,
    not under ``profiles/``). The dispatcher in ``poormad_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``poormad gateway start`` (no ``-p``) targets. Without it,
    bare ``poormad gateway start`` inside the container would land on
    ...
    ``$POORMAD_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        poormad_home: The container's POORMAD_HOME (typically /opt/data).
            Profiles live under ``<poormad_home>/profiles/<name>/``;
            the default profile lives at ``<poormad_home>`` itself.
    ...
    # ``poormad gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(poormad_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(poormad_home)
    ...
    actions.append(ReconcileAction(
        ...
        prior_exit=_read_prior_exit_label(poormad_home),
    ))

    profiles_root = poormad_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `poormad profile create` (config.yaml
            # is not — that comes later via `poormad setup`). Use it as the
            ...
            # via ``poormad -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(poormad_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = poormad_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("POORMAD_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the poormad args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/poormad/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/poormad/docker/main-wrapper.sh
    ...
    ``poormad`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``poormad <subcommand>``, so an
    explicit leading ``poormad`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `poormad <subcommand>`; peel an explicit poormad.
    if args and Path(args[0]).name == "poormad":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``poormad dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted POORMAD_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `poormad -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit poormad
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the poormad user) won't hit EACCES. See
...
def _write_reconcile_log(
    poormad_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $POORMAD_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted POORMAD_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        poormad_home=poormad_home, scandir=scandir,
    )
    for a in actions:
        print(
