"""``poormad gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh PoorMad Portal access token from the existing login
     (``~/.poormad/auth.json``) — the same path ``poormad dashboard register``
     uses (``resolve_nous_access_token``). This proves *which PoorMad org (tenant)*
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.poormad/.env``. The per-gateway secret authenticates the WS upgrade;
...
def _resolve_identity_token() -> str:
    """Resolve the caller-identity bearer token (generic-OIDC or PoorMad Portal).
    ...
    self-hosted-IdP path; otherwise PoorMad Portal). Raises RuntimeError on failure.
    """
...
def _post_enroll(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Connector rejected the caller identity (401). Your PoorMad Portal "
                "token could not be verified — try `poormad auth add nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    from poormad_cli.config import is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `poormad gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    gapped / self-hosted-IdP, NO PoorMad Portal); otherwise the PoorMad Portal token.
    try:
    ...
    #    save_env_value writes them to ~/.poormad/.env (0600 dir) and never logs.
    to_write = {
    ...
    # process is launched under (load_poormad_dotenv exports that .env), so
...
def _warn_if_secondary_multiplex_profile() -> bool:
    ...
    in poormad_cli/gateway.py. Best-effort: any failure to determine the
    ...
    try:
        ...
        default_root = Path(get_default_poormad_root()).resolve()
        home = Path(get_poormad_home()).resolve()
        try:
