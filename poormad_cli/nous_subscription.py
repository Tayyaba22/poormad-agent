"""Helpers for PoorMad subscription managed-tool capabilities."""
...
from poormad_cli.config import get_env_value, load_config
...
# coverage category (poormad_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `poormad tools` picker scope its entitlement gate to the selected backend, so a
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    ...
    with the runtime resolver is what stops ``poormad status`` from lying.
    """
    if not isinstance(section, dict):
        return None
...
@dataclass(frozen=True)
class PoorMadSubscriptionFeatures:
    subscribed: bool
    nous_auth_present: bool
    provider_is_nous: bool
    features: Dict[str, PoorMadFeatureState]
    account_info: Optional[PoorMadPortalAccountInfo] = None

    @property
    def web(self) -> PoorMadFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> PoorMadFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> PoorMadFeatureState:
        return self.features["tts"]

    @property
    def stt(self) -> PoorMadFeatureState:
        return self.features["stt"]

    @property
    def browser(self) -> PoorMadFeatureState:
        return self.features["browser"]

    @property
    def video_gen(self) -> PoorMadFeatureState:
        return self.features["video_gen"]

    @property
    def modal(self) -> PoorMadFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[PoorMadFeatureState]:
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # PoorMad-managed Node dirs (Windows installer / POSIX $POORMAD_HOME/node)
        ...
        managed_path = with_poormad_node_path().get("PATH", "")
        if managed_path:
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("POORMAD_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # requires `pip install faster-whisper`. For PoorMad subscribers we'd
    ...
    # Keyless Tavily is opt-in: selecting it in `poormad tools` / setup writes
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("POORMAD_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("POORMAD_LOCAL_STT_COMMAND"))
    ...
    features = {
        ...
        "image_gen": PoorMadFeatureState(
            ...
            current_provider="FAL" if (image_selected not in (None, "nous") or (image_selected is None and direct_fal)) else ("PoorMad Subscription" if (image_managed or image_use_gateway) else ""),
            explicit_configured=image_selected is not None or direct_fal,
        ),
        "video_gen": PoorMadFeatureState(
            key="video_gen",
            ...
            current_provider="FAL" if (video_selected not in (None, "nous") or (video_selected is None and direct_fal_video)) else ("PoorMad Subscription" if (video_managed or video_use_gateway) else ""),
            explicit_configured=video_selected is not None or direct_fal_video,
        ),
        "tts": PoorMadFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
    ...
    return PoorMadSubscriptionFeatures(
...
def apply_nous_managed_defaults(
    ...
    *,
    ...
    # (requires `pip install faster-whisper`); for PoorMad subscribers we
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    All lists are empty when the user is not a paid PoorMad subscriber or
    is not using PoorMad as their provider.
    """
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from poormad_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    source_label = "free tool pool" if pool_only else "PoorMad subscription"
    ...
    # pre-checked checklist re-fired on every PoorMad model swap.
    ...
    if pool_only:
        title = "Your free PoorMad tool pool — pick the tools to enable:"
    else:
        title = (
            "Your PoorMad subscription includes the Tool Gateway — "
    ...
    # so the next PoorMad model swap doesn't pre-check it again. Cancel paths
...
# ---------------------------------------------------------------------------
# Inline PoorMad Portal login for the Tool Gateway picker (`poormad tools`)
# ---------------------------------------------------------------------------


def ensure_nous_portal_access(
    *,
    capability: str = "the PoorMad Tool Gateway",
    coverage_category: Optional[str] = None,
) -> bool:
    """Make sure the user is entitled to the PoorMad Tool Gateway, logging in if
    needed.

    Used by ``poormad tools`` when a user selects a PoorMad-managed Tool Gateway
    backend (e.g. "Firecrawl (PoorMad Portal)").  Unlike ``poormad model``'s PoorMad
    login, this:
    ...
    It only performs the PoorMad Portal device-code OAuth (when the user isn't
...
def _run_nous_portal_login_only(*, capability: str) -> bool:
    """Run the PoorMad Portal device-code OAuth and persist credentials only.
    ...
    try:
        ...
        print(f"  Could not start PoorMad Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a PoorMad Portal login.")
    try:
        proceed = input("  Log in to PoorMad Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped PoorMad Portal login.")
        return False

    try:
        # Snapshot the active_provider so a tool-config login never silently
        # switches the user's inference provider to PoorMad.
        ...
        if shared:
            try:
                do_import = input(
                    "  Found existing PoorMad OAuth credentials. Import them? [Y/n]: "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
        ...
        _sync_nous_pool_from_auth_store()
        print("  PoorMad Portal login successful.")
        return True
        ...
        print(f"  PoorMad Portal login failed: {exc}")
        return False
