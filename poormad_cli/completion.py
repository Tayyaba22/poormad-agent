"""Shell completion script generation for poormad CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_poormad_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# PoorMad bash completion
# Add to ~/.bashrc:
#   eval "$(poormad completion bash)"

_poormad_profiles() {{
    local profiles_dir="$HOME/.poormad/profiles"
...
_poormad_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_poormad_profiles)" -- "$cur"))
        return
    fi
...
complete -F _poormad_completion poormad
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef poormad
# PoorMad zsh completion
# Add to ~/.zshrc:
...
_poormad_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.poormad/profiles" ]]; then
        profiles+=($HOME/.poormad/profiles/*(N/:t))
    fi
...
_poormad() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_poormad_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'poormad command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# PoorMad fish completion",
        "# Add to your config:",
        "#   poormad completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __poormad_profiles",
        "    echo default",
        "    if test -d $HOME/.poormad/profiles",
        "        for d in $HOME/.poormad/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c poormad -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c poormad -f -s p -l profile"
        " -d 'Profile name' -xa '(__poormad_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__poormad_profiles)' -d 'Profile name'"
                )
