"""Background keepalive for long-lived PoorMad Portal sessions."""
...
def _timeout_seconds(value: Optional[float]) -> float:
    if value is not None:
        return float(value)
    try:
        return float(os.getenv("POORMAD_NOUS_TIMEOUT_SECONDS", "15"))
...
def _refresh_selected_pool_entry(
    *,
    ...
    """Refresh the current PoorMad credential pool entry when it is stale.

    Returns True when a pool entry exists and is usable/refreshed, False when a
    pool exists but no entry can be used, and None when no PoorMad pool exists.
    """
    try:
        ...
        logger.debug("PoorMad auth keepalive: credential pool unavailable: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("PoorMad auth keepalive: credential pool selection failed: %s", exc)
        return False
    ...
    if access_expiring or not key_usable:
        ...
        if refreshed is None:
            return False
        logger.debug("PoorMad auth keepalive: refreshed credential pool entry")
        return True
...
def refresh_nous_auth_keepalive_once(
    *,
    ...
    """Refresh PoorMad auth once if credentials are configured."""
    ...
    try:
        ...
        logger.debug("PoorMad auth keepalive: refreshed singleton auth state")
        return True
    except AuthError as exc:
        if exc.relogin_required:
            logger.info("PoorMad auth keepalive requires re-login: %s", exc)
        else:
            logger.debug("PoorMad auth keepalive failed: %s", exc)
        return False
    except Exception as exc:
        logger.debug("PoorMad auth keepalive failed: %s", exc)
        return False
...
def start_nous_auth_keepalive(
    *,
    ...
    """Start the process-wide PoorMad auth keepalive thread."""
    ...
    with _keepalive_lock:
        ...
        logger.debug("PoorMad auth keepalive started")
        return _keepalive_thread
