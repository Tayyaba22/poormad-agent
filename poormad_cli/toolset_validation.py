...
the tool registry or launching PoorMad (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``poormad-cli`` to the non-existent ``poormad``. ``resolve_toolset('poormad')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``poormad-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``poormad`` instead of ``poormad-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `poormad tools` to reconfigure."
        )
    return warnings
