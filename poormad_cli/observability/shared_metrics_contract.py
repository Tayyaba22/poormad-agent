"""Bounded product contract for the first PoorMad shared-metrics slice."""
...
SCHEMA_KEY = "poormad.metrics.schema_version"
SCHEMA_VERSION = "poormad.metrics.event.v2"
MODEL_CALL_SCOPE = "poormad.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "poormad.task_run"
TOOL_CALL_SCOPE = "poormad.tool_call"
CLIENT_ACTIVE_MARK = "poormad.client.active"
TOOL_APPROVAL_MARK = "poormad.tool_approval"
SKILL_LIFECYCLE_MARK = "poormad.skill.lifecycle"
SKILL_LOAD_MARK = "poormad.skill.load"
SUBSCRIBER_NAME = "poormad.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "poormad.client.active"
LEGACY_MODEL_CALL_METRIC = "poormad.model_call.count"
MODEL_ROUTE_METRIC = "poormad.model_route.count"
TASK_STARTED_METRIC = "poormad.task_run.started"
TASK_FINISHED_METRIC = "poormad.task_run.finished"
TOOL_CALL_METRIC = "poormad.tool_call.count"
TOOL_APPROVAL_METRIC = "poormad.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "poormad.skill.lifecycle.count"
SKILL_LOAD_METRIC = "poormad.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted PoorMad installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(poormad_version or "").strip()
    ...
    return {
        ...
        "poormad_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("poormad_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its PoorMad logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("poormad.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from poormad_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map PoorMad terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map PoorMad registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "poormad-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal PoorMad tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to PoorMad."""
