"""``poormad pause`` / ``poormad resume`` — the global emergency stop.

``poormad pause`` writes the ESTOP sentinel at ``$POORMAD_HOME/ESTOP``, which
halts cron dispatch, kanban dispatch, and new gateway turns on their next
check. In-flight work is never killed. ``poormad resume`` removes the
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "PoorMad paused"
    ...
    print(
        ...
        "    In-flight work keeps running. Run `poormad resume` to lift the pause."
    )
    return 0
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  PoorMad resumed — dispatch picks up on the next tick.")
    else:
        print(f"PoorMad is not paused (no sentinel at {sentinel_path()}).")
    return 0
...
def build_pause_parser(subparsers) -> None:
    ...
    pause_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "`poormad resume`. In-flight work is never killed."
        ),
    )
    ...
    resume_parser = subparsers.add_parser(
        "resume",
        help="Lift the emergency stop set by `poormad pause`",
