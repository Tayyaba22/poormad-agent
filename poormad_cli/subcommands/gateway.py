"""``poormad gateway`` and ``poormad proxy`` subcommand parsers.

Extracted verbatim from ``poormad_cli/main.py:main()`` (god-file Phase 2).
...
from poormad_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if POORMAD_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy poormad.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy PoorMad gateway unit files "
            "(e.g. poormad.service) left over from older installs. Profile "
            "units (poormad-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "Authenticates as your PoorMad Portal account (the connector derives the "
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.poormad/.env. "
            "Requires being logged in (poormad setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.poormad/.env and forwarded at provision. "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. PoorMad Portal). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `poormad proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
