"""``poormad claw`` subcommand parser.
...
def build_claw_parser(subparsers, *, cmd_claw: Callable) -> None:
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to PoorMad",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to PoorMad",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.poormad/ (by default a "
        "single restore-point archive is written to ~/.poormad/backups/ "
        "before apply; restorable with 'poormad import').",
    )
    claw_migrate.add_argument(
