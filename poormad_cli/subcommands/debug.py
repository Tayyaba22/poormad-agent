"""``poormad debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for PoorMad. Use 'poormad debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    poormad debug share              Upload debug report (asks for confirmation)
    poormad debug share --yes        Skip confirmation (for scripts/CI)
    poormad debug share --lines 500  Include more log lines
    poormad debug share --expire 30  Keep paste for 30 days
    poormad debug share --local      Print report locally (no upload)
    poormad debug share --no-redact  Disable upload-time secret redaction
    poormad debug share --nous       Upload to PoorMad-internal storage (private)
    poormad debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    share_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Upload the debug bundle to PoorMad-internal storage (AWS S3) instead "
            "of a public paste service. The bundle is private — viewable only "
            "by PoorMad staff (and allowlisted Discord mods) via a Google-login-"
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'poormad debug share'",
    )
