"""``poormad sync`` subcommand parser — Skill Sync.

Cloned from ``poormad_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    poormad sync status                 show gate/opt-in/head state
    poormad sync pull                   pull and materialize opted-in skills
    poormad sync push                   push opted-in skills
    poormad sync now                    reconcile: pull then push
    poormad sync enable <skill>         opt a skill into sync
    poormad sync disable <skill>        opt a skill out of sync
    poormad sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    poormad sync propose <skill>        share a skill with your organisation

Sync is INERT unless the resolved PoorMad token carries the access-gate claim
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  poormad sync status            what is synced, and from where\n"
            "  poormad sync enable my-skill   include a skill in your sync\n"
            "  poormad sync now               pull, then push\n"
            "  poormad sync propose my-skill  share a skill with your team\n"
        ),
