"""``poormad dashboard`` / ``poormad serve`` subcommand parsers.
...
``poormad_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `poormad dashboard` / `poormad serve` cmdlines and SIGTERM them directly —
    # the same path `poormad update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running PoorMad web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running PoorMad web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the PoorMad web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older PoorMad desktop app shells (<= 0.15.x) spawn the
    # backend as `poormad dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "PoorMad couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the PoorMad Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the PoorMad backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the PoorMad backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports POORMAD_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `poormad dashboard register` — register a self-hosted dashboard OAuth
    # client with PoorMad Portal and write the client_id into ~/.poormad/.env.
    # Nested subparser so bare `poormad dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        "register",
        help="Register a self-hosted dashboard with PoorMad Portal (writes the OAuth client ID to .env)",
        description=(
            "Register this install as a self-hosted dashboard with your PoorMad "
            "Portal account. Creates an OAuth client, writes "
            "POORMAD_DASHBOARD_OAUTH_CLIENT_ID into ~/.poormad/.env, and prints "
            "how to engage the login gate. Requires being logged in (poormad setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://poormad.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        default=None,
        help=(
            "Override the PoorMad Portal base URL for registration (default: the "
            "portal you logged into). The access token must be valid at this "
            "portal. Also settable via POORMAD_DASHBOARD_PORTAL_URL. Mainly for "
