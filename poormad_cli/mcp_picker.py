"""MCP picker — interactive `poormad mcp picker` (also the default `poormad mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``poormad mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `poormad plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from poormad_cli.cli_output import prompt_yes_no
from poormad_cli.curses_ui import curses_single_select
from poormad_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new PoorMad session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from poormad_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `poormad mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: poormad mcp install <name>    Picker: poormad mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their PoorMad is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer PoorMad — run `poormad update` "
...
def show_catalog() -> None:
    """`poormad mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`poormad mcp picker` (and default `poormad mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`poormad mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `poormad mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
