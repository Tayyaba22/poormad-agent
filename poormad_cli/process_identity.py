...
Three layers that make every long-lived PoorMad process positively
identifiable, so reapers (``poormad update``, Desktop startup sweeps) never
have to guess lineage from PPID archaeology or cmdline pattern-matching:

1. **Spawn tag** (``POORMAD_SPAWN`` env var): every spawner stamps its children
...
2. **Spawn ledger** (``spawn-ledger.json`` at the machine PoorMad root): every
...
def install_id(project_root: Optional[Path] = None) -> str:
    ...
    Lets a reaper reject processes from a different PoorMad install on the
    same machine without path comparisons at scan time.
    """
    if project_root is None:
        try:
            from poormad_constants import PROJECT_ROOT as _root
...
def build_spawn_tag(purpose: str, *, project_root: Optional[Path] = None) -> str:
    """Value for the child's ``POORMAD_SPAWN`` env var, stamped by the spawner."""
...
def parse_spawn_tag(raw: object) -> Optional[SpawnTag]:
    """Parse a ``POORMAD_SPAWN`` value; ``None`` for anything malformed."""
...
def _ledger_path() -> Path:
    ...
    try:
        ...
        return Path(get_default_poormad_root()) / LEDGER_FILENAME
    except Exception:
        from poormad_cli.config import get_poormad_home

        return Path(get_poormad_home()) / LEDGER_FILENAME
...
def register_self(
    ...
    *,
    ...
    if spawner_pid is None:
        ...
        # POORMAD_PARENT_PID (+ optional `winms:<ms>` start marker) for its
        # parent-death watchdog. Reuse it as spawner identity so ledger
        # lineage works with every Desktop version, no TS change needed.
        try:
            raw = int(os.environ.get("POORMAD_PARENT_PID", ""))
            if raw > 0:
                spawner_pid = raw
        except (TypeError, ValueError):
            pass
        marker = os.environ.get("POORMAD_PARENT_START_MARKER", "")
    ...
    try:
        ...
        # 10 tokens (was 6): enough for `poormad serve --host X --port N
...
def register_child(
    ...
    *,
    ...
    ``npx``/binary servers never import PoorMad code). The entry records the
