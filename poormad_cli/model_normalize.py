...
- **Aggregators** (OpenRouter, PoorMad, AI Gateway, Kilo Code) need
...
- **DeepSeek** accepts only the first-class V-series IDs
  ...
  PoorMad revisions folded every non-reasoner input into
...
# slug used by aggregator APIs (OpenRouter, PoorMad, etc.).
#
...
def _normalize_provider_alias(provider_name: str) -> str:
    """Resolve provider aliases to PoorMad' canonical ids."""
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        ...
        target_provider: The canonical PoorMad provider id, e.g.
            ``"openrouter"``, ``"anthropic"``, ``"copilot"``,
            ``"deepseek"``, ``"custom"``.  Should already be normalised
            via ``poormad_cli.models.normalize_provider()``.
    ...
    if provider in {"copilot", "copilot-acp"}:
        try:
            from poormad_cli.models import normalize_copilot_model_id
