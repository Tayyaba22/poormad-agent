"""Pydantic request/response models for the PoorMad dashboard web server.

Extracted verbatim from ``poormad_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from poormad_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``poormad model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `poormad import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`poormad -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the POORMAD_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under POORMAD_HOME.
    output: str = ""
