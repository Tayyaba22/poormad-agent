"""`poormad checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.poormad/checkpoints/``.  Actions:

    poormad checkpoints               # same as `status`
    poormad checkpoints status        # total size, project count, breakdown
    poormad checkpoints list          # per-project checkpoint counts + workdir
    poormad checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    poormad checkpoints clear [-f]    # nuke the entire base (asks first)
    poormad checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    poormad checkpoints
    poormad checkpoints prune --retention-days 3 --max-size-mb 200
    poormad checkpoints clear -f
...
from poormad_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: poormad checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``poormad checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `poormad checkpoints` → status
