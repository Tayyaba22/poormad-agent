"""Per-terminal session breadcrumbs for ``poormad -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$POORMAD_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``poormad -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_poormad_home() / "terminal-sessions"
