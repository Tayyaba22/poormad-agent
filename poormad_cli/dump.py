"""
Dump command for poormad CLI.

Outputs a compact, plain-text summary of the user's PoorMad setup
...
from poormad_cli.config import get_poormad_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.poormad/.env.
    ...
    exports. ``poormad debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.poormad_build_sha`` by the Dockerfile's
    ``POORMAD_GIT_SHA`` build-arg (see ``poormad_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``poormad dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from poormad_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(poormad_home: Path) -> int:
...
def _cron_summary(poormad_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = poormad_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"poormad_home:      {display_poormad_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.poormad/.env: a managed
        ...
        # A credential added via `poormad auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `poormad auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["poormad-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(poormad_home)}")
    lines.append(f"  skills:             {_count_skills(poormad_home)}")
