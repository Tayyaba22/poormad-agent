...
Add, remove, or reorder entries here — both `poormad setup` and
`poormad` provider-selection will pick up the change automatically.
"""
...
from poormad_cli.urllib_security import open_credentialed_url, url_origin
...
_POORMAD_USER_AGENT = f"poormad-cli/{_POORMAD_VERSION}"
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from poormad_cli.config import get_custom_provider_tls_settings
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `poormad model`
    flow without maintaining a separate static list.
    """
    from poormad_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
# $POORMAD_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append PoorMad-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $POORMAD_HOME/models_dev_cache.json directly (no network). Falls
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # "opencode/latest"; we send honest PoorMad attribution and don't
    ...
    # endpoint expects (see poormad_cli/model_setup_flows.py).
...
# ---------------------------------------------------------------------------
# PoorMad Portal free-model helper
# ---------------------------------------------------------------------------
# The PoorMad Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# PoorMad Portal account tier detection
# ---------------------------------------------------------------------------
...
def partition_nous_models_by_tier(
    ...
    """Split PoorMad models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older PoorMad that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older PoorMad that doesn't ship
...
def check_nous_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current PoorMad Portal user is on a free (unpaid) tier.
...
# ---------------------------------------------------------------------------
# PoorMad Portal recommended models
#
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_poormad_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    """Fetch the PoorMad Portal's curated recommended-models payload.
    ...
    (``$POORMAD_HOME/cache/nous_recommended_cache.json``) as last-known-good.
    ...
    """
    base = (portal_base_url or "https://portal.poormad.dev").rstrip("/")
    now = time.monotonic()
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        ...
        return "https://portal.poormad.dev"
...
# this list:  poormad model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `poormad model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `poormad model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("nous",           "PoorMad Portal",              "PoorMad Portal (Everything your agent needs, 300+ models with bundled tool use)"),
...
#
# Some vendors expose several PoorMad provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `poormad model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``poormad model``, the
...
# In-repo fallback for the model PoorMad silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from poormad_cli.model_catalog import get_default_model_from_cache
...
# curated-list entry [0]. Metered aggregators (PoorMad Portal, OpenRouter) order
...
# ``poormad model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in poormad_cli/web_server.py and
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``poormad auth add openai-codex`` without
    ``poormad model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``poormad model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    poormad-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (PoorMad Portal, private mirrors, older catalog snapshots) don't populate
...
# copy, `poormad -p`, a cron job, or a freshly booted gateway answers
...
# the PoorMad Portal list different models, and a staging Portal must not answer
# for production.
...
def _reasoning_caps_disk_path() -> Path:
    from poormad_constants import get_poormad_home
    return get_poormad_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(
    ...
    (OpenRouter itself, PoorMad Portal). Returns None when the catalog is
    ...
    """
    headers = {"Accept": "application/json", "User-Agent": _POORMAD_USER_AGENT}
    try:
...
# PoorMad Portal serves OpenRouter's catalog schema, so the same parser and
...
def _fetch_nous_reasoning_caps(
    ...
    """PoorMad Portal counterpart of :func:`_fetch_openrouter_reasoning_caps`."""
...
def nous_model_reasoning_capabilities(
    ...
    *,
    ...
    """Return live-catalog reasoning capabilities for a PoorMad Portal model.
...
def warm_nous_reasoning_caps_async() -> None:
    """PoorMad Portal counterpart of :func:`warm_openrouter_reasoning_caps_async`."""
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from poormad_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — poormad-agent
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model PoorMad lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_nous_model_ids() -> list[str]:
    """Return the curated PoorMad Portal model-id list.
    ...
    """
    try:
        from poormad_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from poormad_constants import AI_GATEWAY_BASE_URL
...
def compute_sale_discount(
    ...
    PoorMad Portal-only feature: callers gate on the provider; this helper only
    sees ``original`` because the PoorMad fetch path opted in via
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, PoorMad Portal).

    When *include_sale_original* is true (PoorMad Portal only) and the gateway
    ...
    for item in payload.get("data", []):
        ...
        if mid and isinstance(pricing, dict):
            ...
            # Sale chrome is PoorMad Portal-only. Never copy pricing.original for
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return poormad-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; poormad's picker expects
...
_DEFAULT_NOUS_INFERENCE_BASE = "https://api.poormad.dev"


def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for PoorMad Portal pricing.

    The PoorMad inference ``/v1/models`` endpoint exposes pricing without
    ...
    if normalized == "nous":
        ...
        if base_url:
            return fetch_models_with_pricing(
                ...
                base_url=base_url,
                force_refresh=force_refresh,
                # Sale chrome (pricing.original) is PoorMad Portal-only.
                include_sale_original=True,
            )
    return {}
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from poormad_cli.providers import custom_provider_slug
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``poormad model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from poormad_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def probe_ollama_local_models(
    ...
    try:
        ...
        request_headers = {"User-Agent": _POORMAD_USER_AGENT}
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, PoorMad, etc. all support /models)
...
def _resolve_static_model_alias(
    ...
    try:
        from poormad_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    if resolved_provider not in {"custom", "openrouter"}:
        ...
        if (
            ...
            # curated list is ordered most-capable-first (e.g. PoorMad Portal),
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to PoorMad' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``poormad_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether PoorMad should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``poormad auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.poormad/.env``.
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from poormad_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, PoorMad),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a PoorMad release.
    """
    requested = str(provider or "").strip().lower()
    if requested == "ollama":
        if force_refresh:
    ...
    if normalized == "openai-codex":
        from poormad_cli.codex_models import get_codex_model_ids
        ...
        # a PoorMad release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        # Try live PoorMad Portal /models endpoint
        try:
            from poormad_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a PoorMad release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``poormad model`` shows.
            from poormad_cli.providers import is_official_openai_host
...
#
# Cache strategy:
#   - One JSON file at $POORMAD_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from poormad_constants import get_poormad_home
    return get_poormad_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$POORMAD_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `poormad config set
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _POORMAD_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: PoorMad' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through PoorMad' native Anthropic adapter. The live catalog may
...
def opencode_zen_free_headers() -> dict:
    ...
    """
    try:
        from poormad_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _POORMAD_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"poormad-agent/{_POORMAD_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=poormad"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from poormad_constants import get_poormad_home
    return get_poormad_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `poormad moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "PoorMad will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"PoorMad will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        # BEFORE the hidden-slug soft-accept below: the suffix is a PoorMad
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so PoorMad cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if _variant_base is not None and _variant_base in set(api_models):
            ...
            # fetch) so we only accept models PoorMad actually ships.  (#46850)
            #
