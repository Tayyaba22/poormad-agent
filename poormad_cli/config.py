"""
Configuration management for PoorMad.

Config files are stored in ~/.poormad/ for easy access:
- ~/.poormad/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.poormad/.env         - API keys and secrets

This module provides:
- poormad config          - Show current configuration
- poormad config edit     - Open config in editor
- poormad config get      - Print a resolved configuration value
- poormad config set      - Set a specific value
- poormad config unset    - Remove a user configuration value
- poormad config wizard   - Re-run setup wizard
"""
...
from poormad_cli.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``poormad config set`` (which rewrites
    ...
    ``config.yaml`` in place: poormad never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.poormad/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``poormad logs``
    ...
    if fallback == "last-known-good":
        ...
        msg = (
            ...
            f"Fix the YAML (poormad config edit) and retry."
        )
    else:
        msg = (
            f"Failed to parse {config_path}: {exc}. "
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  poormad config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` PoorMad makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. PoorMad itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``poormad update``, the TUI build.
...
#   on every plugin install / ``poormad update``.
...
# * ``POORMAD_HOME`` / ``POORMAD_PROFILE`` / ``POORMAD_CONFIG`` /
#   ``POORMAD_ENV`` — PoorMad runtime location flags. Writing these into
...
#
# IMPORTANT: ``POORMAD_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (POORMAD_LANGFUSE_PUBLIC_KEY,
# POORMAD_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # PoorMad runtime location — never via dashboard env writer.
    # NOT a POORMAD_* blanket: integration credentials (POORMAD_GEMINI_*,
    # POORMAD_LANGFUSE_*, POORMAD_SPOTIFY_*, ...) ARE allowed.
    "POORMAD_HOME", "POORMAD_PROFILE", "POORMAD_CONFIG", "POORMAD_ENV",
    "POORMAD_CONFIG_PATH", "POORMAD_ENV_PATH",
    ...
    "POORMAD_COPILOT_ACP_COMMAND", "POORMAD_COPILOT_ACP_ARGS",
    # PoorMad security policy / approval-routing context. These remain available
    ...
    "POORMAD_YOLO_MODE", "POORMAD_ACCEPT_HOOKS", "POORMAD_REDACT_SECRETS",
    "POORMAD_INTERACTIVE", "POORMAD_EXEC_ASK", "POORMAD_GATEWAY_SESSION",
    "POORMAD_CRON_SESSION", "POORMAD_SINGLE_QUERY_SESSION",
    "POORMAD_SESSION_KEY", "POORMAD_SESSION_PLATFORM",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or PoorMad runtime location "
            "and security policy (POORMAD_HOME, POORMAD_YOLO_MODE, ...) "
            "cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.poormad/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # POORMAD_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `poormad plugins enable
    # observability/langfuse` or `poormad tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "POORMAD_LANGFUSE_ENV",
    "POORMAD_LANGFUSE_RELEASE",
    "POORMAD_LANGFUSE_SAMPLE_RATE",
    "POORMAD_LANGFUSE_MAX_CHARS",
    "POORMAD_LANGFUSE_CAPTURE",
    "POORMAD_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "POORMAD_ACP_AUTH_METHOD",
    "POORMAD_ACP_AUTO_APPROVE",
    "POORMAD_COPILOT_ACP_COMMAND",
    "POORMAD_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from poormad_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set POORMAD_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("POORMAD_MANAGED", "").strip()
    marker = None
    if raw:
        marker = raw.lower()
    else:
        managed_marker = get_poormad_home() / ".managed"
        # An interactive shell reads the marker, because it does not see the
        # POORMAD_MANAGED variable of the service. A marker with content
...
def is_managed() -> bool:
    """Check if PoorMad is running in package-manager-managed mode.

    Two signals: the POORMAD_MANAGED env var (set by the systemd service),
    or a .managed marker file in POORMAD_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update PoorMad through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``poormad_cli/`` — i.e. the git checkout for source
    installs, ``/opt/poormad`` inside the published image. It is a property of
    the running interpreter, NOT of ``$POORMAD_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how PoorMad was installed: 'apt', 'docker', 'nix', 'nixos',
    ...
    2. Legacy home-scoped stamp ``$POORMAD_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. POORMAD_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.poormad``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$POORMAD_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.poormad:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$POORMAD_HOME``, a
    ...
    ``docker`` and ``poormad update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``poormad/poormad-agent`` image bakes a ``docker``
        stamp into ``/opt/poormad`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $POORMAD_HOME.
    try:
    ...
    #    host install whose shared $POORMAD_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``poormad update``.
    try:
        method = (
            (get_poormad_home() / ".install_method")
    ...
    # detect Nix installs that don't set POORMAD_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``poormad_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$POORMAD_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/poormad`` in the published image, which instead bakes the stamp at
...
# Long-form text for ``poormad update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# poormad_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side PoorMad, it doesn't update
...
✗ ``poormad update`` doesn't apply inside the Docker container.

PoorMad runs as a published image (poormad/poormad-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull poormad/poormad-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate poormad-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm poormad/poormad-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/poormad/poormad-agent/tags
  • Your config and session history live under ``$POORMAD_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``poormad update`` inside Docker.
...
def format_managed_message(action: str = "modify this PoorMad installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this PoorMad installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall PoorMad."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from POORMAD_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, poormad_bin
    or None if container mode is not active, we're already inside the
    container, or POORMAD_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("POORMAD_DEV") == "1":
        return None
    ...
    container_mode_file = get_poormad_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "poormad-agent")
    exec_user = info.get("exec_user", "poormad")
    poormad_bin = info.get("poormad_bin", "/data/current-package/bin/poormad")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "poormad_bin": poormad_bin,
    }
...
# Re-export from poormad_constants — canonical definition lives there.
from poormad_constants import get_poormad_home, get_process_poormad_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_poormad_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_poormad_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_poormad_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the POORMAD_UID / POORMAD_GID env vars set by Docker deployments.

    Docker containers running PoorMad commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level POORMAD_HOME once, but
    subdirectories created at runtime by ``ensure_poormad_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("POORMAD_UID", "").strip()
    gid_str = os.environ.get("POORMAD_GID", "").strip()
    try:
...
def _chown_to_poormad_uid(path) -> None:
    """Chown ``path`` to ``POORMAD_UID:POORMAD_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_poormad_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_poormad_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the poormad group can
    share state with the gateway service.

    The mode can be overridden via the POORMAD_HOME_MODE environment variable
    (e.g. POORMAD_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse POORMAD_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``POORMAD_UID``/``POORMAD_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("POORMAD_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_poormad_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When PoorMad runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("POORMAD_CONTAINER") or os.environ.get("POORMAD_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set POORMAD_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into POORMAD_HOME, upgrading legacy empty templates.
...
# ensure_poormad_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_POORMAD_HOME_ENSURED: set = set()


def ensure_poormad_home():
    """Ensure ~/.poormad directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``POORMAD_HOME`` we only re-run
    ...
    ``get_poormad_home()`` and therefore re-run for the new path.
    """
    home = get_poormad_home()
    key = str(home)

    if key in _POORMAD_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``poormad profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old POORMAD_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_poormad_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _POORMAD_HOME_ENSURED.add(key)


def _ensure_poormad_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"POORMAD_HOME {home} does not exist."
        )
...
from poormad_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
# selection step (PoorMad Portal / OpenRouter / Custom endpoint), so this
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `poormad config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.poormad.config`` entries, then checks
    ...
    try:
        ...
        # should never break `poormad update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but PoorMad' own config writer has historically emitted it
    ...
    # PoorMad (``_save_discovered_models_to_config``), not hand-curated.
    # Older PoorMad versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older PoorMad versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``poormad_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (poormad_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (poormad_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (poormad_cli/setup.py)
    "known_plugin_toolsets", # written/read by poormad_cli/tools_config.py toolset-save flow
    "known_builtin_toolsets",  # ditto — which builtin toolsets a platform's checklist has offered
    "tool_gateway_declined_tools",  # per-tool Tool Gateway offer declines (poormad_cli/nous_subscription.py, #92647)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'poormad setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — PoorMad won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, poormad send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'poormad doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars() -> None:
    ...
    if lines:
        ...
        hint_path = display_poormad_home()
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "poormad update / poormad -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from poormad_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ poormad config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in poormad_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from poormad_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from poormad_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: poormad config set <key> <value>")
    ...
    # `poormad update`, but only the version bump is persisted.
    ...
    # metadata.poormad.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.poormad/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_poormad_dotenv()`` populated the process env, or an env var
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``PoorMadCLI`` construction: the model becomes a plain
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``poormad config set``
    ...
    backends) — while display paths (``poormad status``/``dump``) read ``name``
...
#   - survives the ``str() → int()`` round-trip through the POORMAD_MAX_ITERATIONS
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _poormad_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in poormad_cli.personality.
from poormad_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`poormad_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from poormad_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from poormad_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``POORMAD_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`poormad_cli.personality` (single owner).
    """
    from poormad_cli.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.poormad/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_poormad_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.poormad/config.yaml.
    ...
    (which change ``POORMAD_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the PoorMad host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/poormad/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_poormad_dotenv()
...
#   openai-codex (OAuth — poormad auth) — OpenAI Codex
#   nous         (OAuth — poormad auth) — PoorMad Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.poormad/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset PoorMad writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.poormad/.env.
    ...
    interactive menu render (`poormad tools`, `poormad setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `poormad tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.poormad/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.poormad/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``POORMAD_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"POORMAD_CUSTOM_{slug}_API_KEY" if slug else "POORMAD_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.poormad/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.poormad/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to PoorMad — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known PoorMad vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.poormad/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.poormad/.env`` over ``os.environ``.

    Used for PoorMad-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              🌑 PoorMad Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from poormad_cli.auth import get_anthropic_key
    ...
    # Warn on stale POORMAD_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("POORMAD_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale POORMAD_MAX_ITERATIONS={_env_ghost} "
                f"(run 'poormad doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    try:
        from poormad_cli.personality import active_personality_name
    ...
    print(color("  poormad config edit     # Edit config file", Colors.DIM))
    print(color("  poormad config set <key> <value>", Colors.DIM))
    print(color("  poormad setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``POORMAD_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("POORMAD_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `poormad cron list`, then pin the "
        "intended values with `poormad cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (poormad_cli/plugins_cmd.py, poormad_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `poormad config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``poormad config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``poormad config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``poormad config set --force``.
    """
    if is_managed():
    ...
    if not isinstance(_default_value_for_key(key), str):
        ...
        if _lower in {'true', 'yes', 'on'}:
            ...
            #   poormad config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   poormad config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``poormad config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``poormad config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    poormad config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    poormad config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `poormad config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_poormad_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `poormad config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `poormad config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: poormad config get <key> [--json]")
            print()
            print("Examples:")
            print("  poormad config get model")
            print("  poormad config get terminal.backend")
            print("  poormad config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: poormad config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  poormad config set model anthropic/claude-sonnet-4")
            print("  poormad config set terminal.backend docker")
            print("  poormad config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: poormad config unset <key>")
            print()
            print("Examples:")
            print("  poormad config unset model")
            print("  poormad config unset terminal.backend")
            print("  poormad config unset OPENROUTER_API_KEY")
            sys.exit(1)
        try:
        ...
        if missing_config:
            ...
            print("    Run 'poormad config migrate' to add them")
        ...
        print("  poormad config           Show current configuration")
        print("  poormad config edit      Open config in editor")
        print("  poormad config get <key>          Print a resolved config value")
        print("  poormad config set <key> <value>   Set a config value")
        print("  poormad config unset <key>        Remove a config value")
        print("  poormad config check     Check for missing/outdated config")
        print("  poormad config migrate   Update config with new options")
        print("  poormad config path      Show config file path")
        print("  poormad config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``poormad config`` UI so users
