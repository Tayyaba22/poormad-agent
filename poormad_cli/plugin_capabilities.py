...
this. Capabilities govern the *host API surfaces* PoorMad hands out (which
...
until the user re-consents (``poormad plugins update`` surfaces the diff).
...
def parse_declared_capabilities(raw: Any, plugin_name: str = "?") -> List[str]:
    ...
    for a newer PoorMad may declare ids this build doesn't know; they can
