"""
Profile management for multiple isolated PoorMad instances.

Each profile is a fully independent POORMAD_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.poormad/profiles/<name>/`` by default.

The "default" profile is ``~/.poormad`` itself — backward compatible,
zero migration needed.

Usage::

    poormad profile create coder          # fresh profile + bundled skills
    poormad profile create coder --clone  # also copy config, .env, SOUL.md, skills
    poormad profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    poormad -p coder chat                 # or via flag
    poormad profile use coder             # set as sticky default
    poormad profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See poormad_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.poormad``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   poormad-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `poormad backup` archives
...
# Marker file written by `poormad profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `poormad
...
# `poormad skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         PoorMad infrastructure directories that only the default profile
         (``~/.poormad``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_poormad_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.poormad) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "poormad-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when POORMAD_HOME equals the
...
# into the export. The set below identifies the *known PoorMad profile
# artifacts* at the root of POORMAD_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``poormad_constants``.
...
_RESERVED_NAMES = frozenset({
    "poormad", "default", "test", "tmp", "root", "sudo",
})

# PoorMad subcommands that cannot be used as profile names/aliases
_POORMAD_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the poormad root, NOT to the current POORMAD_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where POORMAD_HOME points outside
    ``~/.poormad``, profiles live under ``POORMAD_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_poormad_home() / "profiles"


def _get_default_poormad_home() -> Path:
    """Return the default (pre-profile) POORMAD_HOME path.

    In standard deployments this is ``~/.poormad``.
    In Docker/custom deployments where POORMAD_HOME is outside ``~/.poormad``
    (e.g. ``/opt/data``), returns POORMAD_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_poormad_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the PoorMad installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its POORMAD_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_poormad_home()
...
def profile_matches_home(name: str, home: "Path | None" = None) -> bool:
    ...
    ``home`` defaults to the process's current PoorMad home
    (:func:`poormad_constants.get_poormad_home`).  Used by single-profile
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, poormad subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _POORMAD_SUBCOMMANDS:
        return f"'{canon}' conflicts with a poormad subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            poormad_exe = shutil.which("poormad") or "poormad"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(poormad_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running PoorMad. If we
    ...
    try:
        ...
        # not be migrated. The next `poormad doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``poormad -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``poormad profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``poormad -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``poormad profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``POORMAD_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing PoorMad config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_poormad_home()
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, poormad_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own POORMAD_HOME. When
    ...
    The returned ``poormad_home`` is the path to pass to
    ``set_poormad_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_poormad_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``poormad update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.poormad)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.poormad/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `poormad -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this PoorMad profile.\n"
    ...
    if not soul_path.exists():
        try:
            from poormad_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `poormad update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`poormad profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running PoorMad (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `poormad profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `poormad -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches POORMAD_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``poormad profile create
    ...
    only the essential skills (e.g. ``poormad-agent``), reporting
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "POORMAD_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_poormad_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running PoorMad *backends* bound to this profile.
    ...
    names it, so find it by inspection: a PoorMad backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``POORMAD_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `poormad -p <canon> profile
    ...
    poormad_markers = ("poormad_cli.main", "poormad-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('poormad')` resolution) spawns the
    ...
    # interpreter, not "poormad", so the checks below would otherwise miss it.
    ...
    # interpreter.
    _POORMAD_CONSOLE_SCRIPT_NAMES = frozenset({"poormad", "poormad-agent", "poormad-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a PoorMad process: either an entrypoint marker in argv, a
            # resolved executable named `poormad`, or a python interpreter
            # directly exec'ing a `poormad`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_poormad = (
                any(marker in joined for marker in poormad_markers)
                or exe_name == "poormad"
                or exe_name.startswith("poormad")
            )
            if not is_poormad and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: poormad, poormad-agent,
                # poormad-acp) rather than a bare `startswith("poormad")` --
                ...
                # "poormad-notes.py" or "poormad-unrelated-tool" as the shim,
                ...
                is_poormad = script_stem in _POORMAD_CONSOLE_SCRIPT_NAMES
            if not is_poormad:
                continue
            ...
            # ...or by POORMAD_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("POORMAD_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.poormad).\n"
            "To remove everything, use: poormad uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_poormad_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``POORMAD_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from poormad_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set POORMAD_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("POORMAD_HOME")
    try:
        os.environ["POORMAD_HOME"] = str(profile_dir)
        from poormad_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["POORMAD_HOME"] = old_home
        elif "POORMAD_HOME" in os.environ:
            del os.environ["POORMAD_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.poormad/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: poormad profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from POORMAD_HOME.

    Returns ``"default"`` if POORMAD_HOME is not set or points to ``~/.poormad``.
    Returns the profile name if POORMAD_HOME points into ``~/.poormad/profiles/<name>``.
    Returns ``"custom"`` if POORMAD_HOME is set to an unrecognized path.
    """
    from poormad_constants import get_poormad_home
    poormad_home = get_poormad_home()
    resolved = poormad_home.resolve()

    default_resolved = _get_default_poormad_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      POORMAD_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-PoorMad file the
      user may have lying alongside POORMAD_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``poormad sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``POORMAD_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.poormad itself — its parent is ~/ and its
        # directory name is ".poormad", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: poormad profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.poormad itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.poormad). "
            "Specify a different name: poormad profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="poormad_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"poormad_{old_name}"
    legacy_old_host = f"poormad.{old_name}"
    new_host = f"poormad_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_poormad_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("poormad_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a POORMAD_HOME path string.

    Called early in the CLI entry point, before any poormad modules
    are imported, to set the POORMAD_HOME environment variable.

    When POORMAD_HOME is already set, the configured spelling IS the
