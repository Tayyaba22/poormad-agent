"""
Top-level argparse construction for the poormad CLI.
...
# argparse runs (it sets ``POORMAD_HOME`` and strips itself from ``sys.argv``),
...
# tests/poormad_cli/test_top_level_value_flags_parity.py fails CI if the parser
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    the exact drift that made ``poormad --reasoning high chat …`` misread
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``poormad_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    poormad                        Start interactive chat
    poormad chat -q "Hello"        Single query mode
    poormad --tui                  Launch the modern TUI (or set display.interface: tui)
    poormad --cli                  Force the classic REPL (overrides display.interface: tui)
    poormad -c                     Resume the most recent session
    poormad -c "my project"        Resume a session by name (latest in lineage)
    poormad --resume <session_id>  Resume a specific session by ID
    poormad --resume latest        Resume the most recent session (same as -c)
    poormad --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    poormad setup                  Run setup wizard
    poormad logout                 Clear stored authentication
    poormad auth add <provider>    Add a pooled credential
    poormad auth list              List pooled credentials
    poormad auth remove <p> <t>    Remove pooled credential by index, id, or label
    poormad auth reset <provider>  Clear exhaustion status for a provider
    poormad model                  Select default model
    poormad fallback [list]        Show fallback provider chain
    poormad fallback add           Add a fallback provider (same picker as `poormad model`)
    poormad fallback remove        Remove a fallback provider from the chain
    poormad config                 View configuration
    poormad config edit            Edit config in $EDITOR
    poormad config set model gpt-4 Set a config value
    poormad gateway                Run messaging gateway
    poormad -s poormad-agent-dev,github-auth
    poormad -w                     Start in isolated git worktree
    poormad gateway install        Install gateway background service
    poormad sessions list          List past sessions
    poormad sessions browse        Interactive session picker
    poormad sessions rename ID T   Rename/title a session
    poormad logs                   View agent.log (last 50 lines)
    poormad logs -f                Follow agent.log in real time
    poormad logs errors            View errors.log
    poormad logs --since 1h        Lines from the last hour
    poormad debug share             Upload debug report for support
    poormad console                Open the safe PoorMad command console
    poormad update                 Update to latest version
    poormad dashboard              Start web UI dashboard (port 9119)
    poormad dashboard --stop       Stop running dashboard processes
    poormad dashboard --status     List running dashboard processes

For more help on a command:
    poormad <command> --help
"""
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="poormad",
        description="PoorMad - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `poormad chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via POORMAD_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `poormad setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to POORMAD_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.poormad/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with PoorMad",
    )
    ...
    # top-level parser: when the user writes `poormad -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/poormad_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also POORMAD_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.poormad/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from PoorMad itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
