"""
Status command for poormad CLI.

Shows the status of all PoorMad components.
"""
...
from poormad_cli.auth import AuthError, resolve_provider
from poormad_cli.colors import Colors, color
from poormad_cli.config import get_env_path, get_env_value, get_poormad_home, load_config
from poormad_cli.models import provider_label
from poormad_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from poormad_cli.nous_subscription import get_nous_subscription_features
from poormad_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``poormad config``'s
...
def _estop_status_line():
    """One-line pause banner for `poormad status`, or None when not paused.

    Cheap: a single stat on $POORMAD_HOME/ESTOP via agent.estop.
    """
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `poormad resume` to lift)"


def show_status(args):
    """Show status of all PoorMad components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ PoorMad Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `poormad status`
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (PoorMad inference key configured)"
    else:
        nous_label = "not logged in (run: poormad portal)"
    print(
        f"  {'PoorMad Portal':<12}  {check_mark(nous_logged_in)} "
        f"{nous_label}"
    )
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: poormad model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: poormad auth add minimax-oauth)'}"
    )
    ...
    # disrupt the already-printed PoorMad/Codex/Qwen/MiniMax rows above.
    try:
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: poormad auth add xai-oauth)'}"
    )
    ...
    # =========================================================================
    # PoorMad Subscription Features
    # =========================================================================
    if managed_nous_tools_enabled():
        features = get_nous_subscription_features(config)
        print()
        print(color("◆ PoorMad Tool Gateway", Colors.CYAN, Colors.BOLD))
        if not features.nous_auth_present:
            print("  PoorMad Portal   ✗ not logged in")
        else:
            print("  PoorMad Portal   ✓ managed tools available")
        for feature in features.items():
            if feature.managed_by_nous:
                state = "active via PoorMad subscription"
            elif feature.active:
        ...
        # PoorMad OAuth without entitlement, or an opaque inference key without
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: poormad model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from poormad_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'poormad-agent[vercel]')"
        ...
        # provider's doctor rows (fail-soft — never break `poormad status`).
        try:
    ...
    try:
        from poormad_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   poormad gateway")
    ...
    jobs_file = get_poormad_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_poormad_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'poormad doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'poormad setup' to configure", Colors.DIM))
    print()
