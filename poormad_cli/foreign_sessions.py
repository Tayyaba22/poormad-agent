...
``poormad sessions import`` (and ``--resume @claude`` / ``--resume @codex``)
let a user pull a conversation they started in another agent CLI into
PoorMad and continue it here.
...
role-alternation invariant PoorMad enforces everywhere else:
...
def import_foreign_session(source: str, path, db=None) -> str:
    """Import one foreign session into the PoorMad SessionDB.

    Returns the new PoorMad session id.  The foreign file is only read.
...
def pick_foreign_session(
    ...
    if not sys.stdin.isatty():
        print(
            ...
            "  poormad sessions import --from claude|codex <path>"
        )
        return None
    try:
...
def run_sessions_import(args, db=None) -> Optional[str]:
    """`poormad sessions import` entry point. Returns new session id or None."""
    ...
    print(f"  Continue it with:  poormad --resume {session_id}")
    return session_id
