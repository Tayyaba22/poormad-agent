"""Dashboard process-hygiene helpers — extracted from ``poormad_cli/main.py``.
...
``_detect_concurrent_poormad_instances``) are lifted verbatim. References to
helpers that STAY in ``poormad_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``poormad_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``poormad_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``poormad_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``poormad dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``poormad update`` replaces files on disk, the running
    ...
    ``poormad-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the PoorMad Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``poormad serve`` as
    ...
    environment variable ``POORMAD_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`poormad serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "poormad.*dashboard"`) keeps us consistent
            # with `poormad_cli.gateway._scan_gateway_pids` and avoids the
    ...
    # miss profiled launches — `poormad --profile p serve --host <ip>` contains
    # neither "poormad serve" nor "poormad_cli.main serve". Every serve/
    ...
    # truncated view.
    try:
        from poormad_cli.process_identity import ledger_entries
...
def _poormad_home_for_pid(pid: int) -> str | None:
    """Best-effort ``POORMAD_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("POORMAD_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"POORMAD_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by PoorMad Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``poormad update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    argv: list[str], poormad_home: str | None = None
) -> str:
    """Stable owner key: ``POORMAD_HOME`` when known, else ``--profile`` / ``-p``.

    ``POORMAD_HOME`` ending in ``profiles/<name>`` is normalized to
    ``profile:<name>`` so it shares a cap with an explicit ``--profile``
    flag for the same profile (#78821).  Non-profile homes (including
    distinct ``…/.poormad`` roots) keep a resolved ``home:`` key so
    ...
    if poormad_home:
        try:
            home_path = Path(poormad_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(poormad_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(poormad_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    *,
    ...
    """Select which killed manual backends to respawn after ``poormad update``.

    Each candidate is ``(pid, argv, poormad_home)``.  *own_home* is the
    updating install's home; it defaults to this process's
    ``get_poormad_home()`` and exists as a parameter so tests can pin it.

    Rules (#78821, #94030):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``POORMAD_DESKTOP_CHILD_PID``) owns their
       lifecycle.  These are also the PPID-1 orphans that previously
       multiplied across updates because ``--port 0`` always binds a
       fresh free port.
    2. Never replay a backend from a **foreign** ``POORMAD_HOME``.  The
    ...
    4. Cap at most one managed backend per profile / ``POORMAD_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``poormad update`` respawn detaches with ``start_new_session=True``, so
    ...
    for _pid, argv, poormad_home in candidates:
        if not argv:
            continue
        if _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and poormad_home and _normalized_home_for_compare(poormad_home) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        if norm in seen_cmdlines:
            continue
        profile_key = _profile_key_for_respawn(argv, poormad_home)
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``poormad dashboard`` / ``poormad serve`` processes.

    Called at the end of ``poormad update`` (default ``reason``) and also
    from ``poormad dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``poormad update`` path), a detected
    ``poormad-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``poormad-serve.service``) has its owning unit
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``poormad update``'s systemd
    fleet-restart loop, which restarts ``poormad-serve*`` units before this
    ...
    # When the PoorMad Desktop Electron app spawns this dashboard as a
    # backend child, it sets POORMAD_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] = set()
    raw_pid = os.environ.get("POORMAD_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `poormad update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot POORMAD_HOME before the kill so per-profile caps
                # still work after the process is gone (#78821).
                cmdline = _m()._dashboard_cmdline_for_pid(pid)
                if cmdline:
                    pid_cmdline[pid] = cmdline
                    pid_home[pid] = _poormad_home_for_pid(pid)

        if already_restarted_units:
            # Already handled directly by the caller (e.g. poormad update's
    ...
    #    remote backend (poormad serve) under Restart=on-failure never comes
...
def _detect_concurrent_poormad_instances(
    ...
    The PoorMad Desktop Electron app spawns ``poormad.EXE`` as a backend child,
    so during ``poormad update`` the user-invoked process and the desktop's
    ...
    shims (``poormad.exe`` / ``poormad-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``poormad update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``poormad.exe``) is a separate process from the Python
    ...
    for shim in _m()._poormad_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated poormad.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``poormad update``
    ...
    #      poormad.exe sitting *under* a non-PoorMad parent (e.g. a PoorMad
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        poormad serve --host 127.0.0.1 --port 0
        poormad serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "poormad" not in cmd and "poormad_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (POORMAD_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("POORMAD_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``poormad serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``poormad serve`` on a Mac
...
def _poormad_home_dir() -> Path:
    """Resolved PoorMad home (POORMAD_HOME override or ~/.poormad)."""
    override = os.environ.get("POORMAD_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    for field in ("profile", "poormadPath", "poormadHome", "logPath", "startedAt"):
        value = parsed.get(field)
        if not isinstance(value, str) or len(value) > 1024:
            return False
    # logPath is written as ``{lock_root}/{ownershipId}/{spawnNonce}.log``. We
    # only check the suffix so a relocated POORMAD_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{poormad_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = base_dir if base_dir is not None else (
        _poormad_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local poormad serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    ...
    """Kill leftover Desktop-local ``poormad serve`` backends with no parent.
    ...
    running under POORMAD_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never POORMAD_DESKTOP_CHILD_PID entries
    ...
    - never a candidate younger than ``_REAP_MIN_AGE_SECONDS`` (or whose age
      ...
      only after the backend reports POORMAD_BACKEND_READY, so during
    ...
    for pid, cmd in scanned:
        ...
        # POORMAD_BACKEND_READY, so a sibling spawned seconds ago is not yet
