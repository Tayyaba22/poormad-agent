...
The PoorMad docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
PoorMad Portal). This module fetches, validates, and caches that manifest,
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.poormad/cache/model_catalog.json``.
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/PoorMad/poormad-agent/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def _cache_path() -> Path:
    ...
    return get_poormad_home() / "cache" / "model_catalog.json"
...
def get_curated_nous_models() -> list[str] | None:
    """Return PoorMad Portal's curated list of model ids from the manifest.
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model PoorMad silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`poormad update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``poormad update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.poormad/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``poormad model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
