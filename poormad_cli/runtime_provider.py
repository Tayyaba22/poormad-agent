...
from poormad_cli import config as _config_mod
from poormad_cli.providers import custom_provider_aliases, custom_provider_slug
from poormad_constants import OPENROUTER_BASE_URL
from poormad_cli.providers import is_official_openai_host


def load_config():
    """Late-bound delegate to :func:`poormad_cli.config.load_config`.

    Deliberately NOT a module-level ``from poormad_cli.config import
    load_config``: this module is often imported lazily (inside functions),
    so its first import can happen while a test has
    ``poormad_cli.config.load_config`` patched — a from-import would then
    ...
    patching ``poormad_cli.config.load_config`` OR
    ``poormad_cli.runtime_provider.load_config`` behaves as expected.
    """
...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from poormad_cli.auth import resolve_provider as _resolve_provider
...
def _fallback_api_mode(provider: str, base_url: str, model: str = "") -> str:
    ...
    if detected:
        return detected
    from poormad_cli.providers import determine_api_mode
...
def _copilot_runtime_api_mode(
    ...
    *,
    ...
    try:
        from poormad_cli.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of PoorMad' tool dispatch. Gated behind config key
...
def _parse_api_mode(raw: Any) -> Optional[str]:
    ...
    if isinstance(raw, str):
        from poormad_cli.config import _canonical_api_mode
...
def _nous_inference_base_url_override() -> str:
    """Return the trusted PoorMad runtime base URL override, if configured.
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from poormad_cli.models import azure_foundry_model_api_mode
        ...
        if configured_provider == provider and pool_url_is_default:
            ...
            if cfg_base_url:
                base_url = cfg_base_url
        configured_mode = _parse_api_mode(model_cfg.get("api_mode"))
        from poormad_cli.models import opencode_provider_family
        if opencode_provider_family(provider) is not None:
            ...
            # Refs #16878.
            from poormad_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = _getenv("POORMAD_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the built-in. See tests/poormad_cli/test_runtime_provider_resolution.py
    ...
    if isinstance(custom_providers, dict):
        logger.warning(
            ...
            "Run 'poormad doctor' for details."
        )
        return None
...
def canonical_custom_identity(
    *,
    ...
    3. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``POORMAD_INFERENCE_PROVIDER`` equivalent). When neither
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("POORMAD_INFERENCE_PROVIDER", "").strip()
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'poormad model' or "
    ...
    if provider == "nous":
        ...
        api_key = explicit_api_key or (
            ...
            if _agent_key_is_usable(
                state,
                max(60, env_int("POORMAD_NOUS_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
        ...
        if not api_key:
            creds = resolve_nous_runtime_credentials(
                timeout_seconds=float(_getenv("POORMAD_NOUS_TIMEOUT_SECONDS", "15")),
            )
            api_key = creds.get("api_key", "")
...
def resolve_runtime_provider(
    *,
    ...
    from poormad_cli.config import is_provider_enabled, load_config
    ...
    if pool and pool.has_credentials():
        ...
        # For PoorMad, the pool entry's runtime_api_key is the agent_key
        # compatibility field. It must be an invoke JWT. The pool doesn't
        # refresh it during selection (that would trigger network calls in
        # non-runtime contexts like `poormad auth list`). If the key is
        ...
        if provider == "nous" and entry is not None:
            min_ttl = max(60, env_int("POORMAD_NOUS_MIN_KEY_TTL_SECONDS", 1800))
            nous_state = {
            ...
            if not _agent_key_is_usable(nous_state, min_ttl):
                logger.debug("PoorMad pool entry agent_key expired/missing, refreshing selected pool entry")
                try:
                    refreshed = pool.try_refresh_current()
                except Exception as exc:
                    logger.debug("PoorMad pool entry refresh failed: %s", exc)
                    refreshed = None
                ...
                if not pool_api_key or not _agent_key_is_usable(nous_state, min_ttl):
                    logger.debug("PoorMad pool entry agent_key still unavailable, falling through to runtime resolution")
                    pool_api_key = ""
        if (
            entry is not None
            and pool_api_key
    ...
    if provider == "nous":
        try:
            ...
            if requested_provider != "auto":
                raise
            # Auto-detected PoorMad but credentials are stale/revoked —
            # fall through to env-var providers (e.g. OpenRouter).
            logger.info("Auto-detected PoorMad provider but credentials failed; "
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "poormad-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from poormad_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (PoorMad canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most PoorMad-compatible importers).
