"""Cross-process mutual exclusion for in-flight PoorMad updates.

Three different surfaces can start an update of the same install tree:

* ``poormad update`` from a terminal,
* the dashboard's Update button (``POST /api/poormad/update`` →
  ``_spawn_poormad_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``poormad-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``poormad update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <POORMAD_HOME>/.poormad-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``poormad update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("PoorMad is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``poormad-setup`` binary under ``~/.poormad`` is only refreshed by a full
  installer run (``copy_self_to_poormad_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("PoorMad is still running") on every
  GUI update forever, with no PoorMad process actually running.
"""
...
MARKER_NAME = ".poormad-update-in-progress"

# Set by an orchestrating updater (the Tauri `poormad-setup --update` flow) to
# its own pid before spawning `poormad update` as a child stage. The parent
...
HANDOFF_PID_ENV = "POORMAD_UPDATE_HANDOFF_PID"
...
# to show "PoorMad is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* PoorMad home (never the context-local profile override):
    the Rust updater resolves ``$POORMAD_HOME`` or the platform default, and the
    ...
    return get_process_poormad_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``poormad update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``poormad-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another PoorMad update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `poormad update` as a stage): we run under ITS claim
