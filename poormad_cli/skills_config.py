"""
Skills configuration for PoorMad.
`poormad skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.poormad/config.yaml under:
...
from poormad_cli.config import cfg_get, load_config, save_config
from poormad_cli.colors import Colors, color
from poormad_cli.platforms import PLATFORMS as _PLATFORMS
...
def save_disabled_skills(config: dict, disabled: Set[str], platform: Optional[str] = None):
    ...
    Essential skills (e.g. ``poormad-agent``) are silently dropped from the
...
def skills_command(args=None):
    """Entry point for `poormad skills`."""
    from poormad_cli.curses_ui import curses_checklist
