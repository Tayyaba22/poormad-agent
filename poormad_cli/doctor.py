"""
Doctor command for poormad CLI.

Diagnoses issues with PoorMad setup.
"""
...
from poormad_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
POORMAD_HOME = get_poormad_home()
_DHH = display_poormad_home()  # user-facing display path (e.g. ~/.poormad or ~/.poormad/profiles/coder)

# Load environment variables from ~/.poormad/.env so API key checks work
_env_path = get_env_path()
load_poormad_dotenv(poormad_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from poormad_cli.colors import Colors, color
from poormad_cli.models import _POORMAD_USER_AGENT
from poormad_cli.vercel_auth import describe_vercel_auth
from poormad_constants import OPENROUTER_MODELS_URL
...
from poormad_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all PoorMad containers"
...
def _poormad_database_paths(poormad_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for PoorMad-managed SQLite databases."""
    # backup.py owns the canonical list of per-profile stores; reuse it.
    from poormad_cli.backup import _QUICK_STATE_FILES

    entries = [
        (name, poormad_home / name)
        for name in _QUICK_STATE_FILES
        if name.endswith(".db")
    ]
    # Non-default kanban boards each keep their own kanban.db.
    for board_db in sorted((poormad_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(poormad_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    locks a live connection is holding (see ``poormad_cli.sqlite_safe_read``).
    ...
    """
    from poormad_cli.sqlite_safe_read import (
        has_live_connection,
...
def _format_db_size(db_path: Path) -> str:
    ...
    from poormad_cli.backup import _format_size
...
def _report_database_journal_modes(
    poormad_home: Path | None = None,
    version_info: tuple[int, ...] | None = None,
) -> None:
    """List each database's journal mode; warn on WAL under a vulnerable SQLite."""
    from poormad_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = poormad_home if poormad_home is not None else POORMAD_HOME
    try:
        databases = _poormad_database_paths(home)
    except Exception as exc:
        check_warn(f"Could not list PoorMad databases: {exc}")
        return
    exposed = []
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.poormad/.env contains provider auth/base URL settings."""
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    if item.get("name") != "kanban":
        return False
    if os.environ.get("POORMAD_KANBAN_TASK"):
        return False
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("POORMAD_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from poormad_cli.auth import get_minimax_oauth_auth_status
...
from poormad_cli.sizefmt import format_bytes as _human_bytes
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if isinstance(deferral, dict):
        ...
        lines.append((
            ...
            "(stop the listed processes, then run 'poormad sessions "
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or legacy_trigram:
            detail += (
                "; run 'poormad sessions optimize-storage' offline "
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. POORMAD_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("POORMAD_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("POORMAD_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def collect_relay_plugin_cutover_findings(
    ...
    """Return actionable findings for the removed PoorMad Relay plugin."""
    from poormad_cli.relay_plugin_cutover import (
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches poormad_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``poormad --version`` reporting a stale version while
    ...
    """
    try:
        from poormad_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != poormad_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'poormad update', or set "
            "poormad_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-poormad and dashboard static services are up
    ...
    for static in ("main-poormad", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `poormad profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `poormad doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from poormad_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the POORMAD_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("POORMAD_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via POORMAD_MANAGED_DIR={managed_dir}")
...
def check_macos_tcc_grants() -> None:
    ...
    Post-#73681 builds pin ``designated => identifier "com.poormad.poormad"``
    ...
    if "cdhash" in dr.lower():
        check_warn(
            ...
            "Run `poormad update` to get the stable identifier-pinned signing "
            "identity, then re-grant permissions once.",
        )
        return
    if "certificate" in dr.lower():
        # Certificate-anchored DR (poormad desktop --setup-tcc-identity, or a
        ...
        check_ok(
            ...
            "anchor, see `poormad desktop --setup-tcc-identity`)",
        )
    check_info(
        "If macOS still re-prompts for permissions (toggle shows ON): the stored "
        "grant is stale — run `tccutil reset ScreenCapture com.poormad.poormad` "
        "(repeat per affected service), toggle it ON in System Settings, then "
        "fully quit & relaunch PoorMad once."
    )
...
def _desktop_app_bundle() -> Path | None:
    ...
    (``apps/desktop/release/mac-<arch>/PoorMad.app``) — the only layout whose
    ...
    ``_desktop_packaged_executable``'s selection. ``/Applications/PoorMad.app``
    is deliberately not probed: it is the separately-signed PoorMad-Setup
    launcher (``com.poormad.poormad.setup``, certificate-anchored), whose
    grants are stable by construction and unaffected by rebuilds.
    """
    root = Path(__file__).resolve().parents[1]
    release_dir = root / "apps" / "desktop" / "release"
    candidates = [p for p in release_dir.glob("mac*/PoorMad.app") if p.is_dir()]
    if not candidates:
        return None
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    """
    try:
        from poormad_cli import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    check_info(
        ...
        "app Full Disk Access and PoorMad will never trip per-folder dialogs "
        ...
        "    then enable your terminal (and PoorMad.app if you use Desktop), "
        "and restart them once. With PoorMad' stable signing identities the "
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `poormad`.
    os.environ.setdefault("POORMAD_INTERACTIVE", "1")

    # Handle `poormad doctor --ack <id>` as a fast path. Persist the ack and
    # return without running the rest of the diagnostics — the user has
    # already seen the advisory and just wants to silence it.
    if ack_target:
        from poormad_cli.security_advisories import (
            ADVISORIES,
            ack_advisory,
        )
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.poormad/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 PoorMad Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`poormad doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from poormad_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: PoorMad already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and poormad_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.poormad/.env (primary location for user config)
    env_path = POORMAD_HOME / '.env'
    if env_path.exists():
        ...
        # matches poormad_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'poormad setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'poormad setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'poormad setup' to create one")
                issues.append("Run 'poormad setup' to create .env")
    
    # Check ~/.poormad/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = POORMAD_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from poormad_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from poormad_cli.config import is_provider_enabled
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'poormad config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from poormad_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.poormad/.env or run 'poormad setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'poormad setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'poormad config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from poormad_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from poormad_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'poormad setup' to migrate config")
                else:
                    issues.append("Run 'poormad doctor --fix' or 'poormad setup' to migrate config")
            else:
                ...
                if should_fix:
                    ...
                    for k in stale_root_keys:
                        if not model_section.get(k):
                            ...
                            raw_config.pop(k)
                    from poormad_cli.config import atomic_config_write
                    atomic_config_write(config_path, raw_config)
                    check_ok("Migrated stale root-level keys into model section")
                    fixed_count += 1
                else:
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'poormad doctor --fix'")
        except Exception:
            pass

        # Detect stale POORMAD_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives POORMAD_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from poormad_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("POORMAD_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"POORMAD_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `poormad setup` run)",
                )
                if should_fix:
                    if remove_env_value("POORMAD_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale POORMAD_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove POORMAD_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the POORMAD_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale POORMAD_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'poormad doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from poormad_cli.config import load_env as _load_env_depr
            from poormad_cli.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        ...
        if nous_status.get("logged_in"):
            check_ok("PoorMad Portal auth", "(logged in)")
        else:
            check_warn("PoorMad Portal auth", "(not logged in)")
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses PoorMad' own device-code flow — the Codex CLI is
    ...
    # disrupt the already-printed PoorMad/Codex/Gemini/MiniMax rows above.
    try:
        from poormad_cli.auth import get_xai_oauth_auth_status
    ...
    if poormad_home.exists():
        check_ok(f"{_DHH} directory exists")
    elif should_fix:
        poormad_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = poormad_home / subdir_name
    ...
    soul_path = poormad_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give PoorMad a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# PoorMad Persona\n\n"
                "<!-- Edit this file to customize how PoorMad communicates. -->\n\n"
                "You are PoorMad, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = poormad_home / "memories"
    ...
    state_db_path = poormad_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from poormad_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'poormad doctor --fix' "
                        "(or 'poormad sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from poormad_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'poormad doctor --fix' "
                        "(or 'poormad sessions repair') to recover hidden sessions"
                    )
            else:
                check_warn(f"{_DHH}/state.db exists but has issues: {e}")

        # Health/stats snapshot (#statedb-visibility): a multi-GB state.db
        # with a runaway WAL was previously invisible to every PoorMad
        ...
        try:
            from poormad_state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'poormad sessions optimize-storage' "
    ...
    wal_path = poormad_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'poormad doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "poormad"
        ...
        _cmd_link = _cmd_link_dir / "poormad"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(poormad not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/poormad → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/poormad points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/poormad → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/poormad — run 'poormad doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/poormad exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/poormad not found",
                    "(poormad command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/poormad → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/poormad symlink — run 'poormad doctor --fix'")
    ...
    try:
        from poormad_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "Install the Vercel optional dependency: pip install 'poormad-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `poormad update` does, so a session's first browser call
            ...
            # `poormad update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `poormad doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable POORMAD_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a PoorMad one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'poormad config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": _POORMAD_USER_AGENT,
            }
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``poormad doctor``.
    ...
    # never the legitimate source for `poormad doctor`.
    ...
    try:
        ...
        if api_disabled or web_not_ready:
            issues.append("Run 'poormad setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = POORMAD_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: poormad skills list)")
    ...
    try:
        from poormad_cli.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = POORMAD_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: poormad memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: poormad memory setup",
                    "No Honcho API key — run 'poormad memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run poormad memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: poormad memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: poormad memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from poormad_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "poormad -p" in content:
                            _m = _re.search(r"poormad -p (\S+)", content)
    ...
    # `poormad doctor --live` (real network calls; bounded + read-only).
    try:
        from poormad_cli.doctor_live import maybe_run_live_checks
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'poormad doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
