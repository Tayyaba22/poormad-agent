"""``poormad worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``poormad_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    poormad worktree list             # audit: verdict + reason per tree
    poormad worktree prune            # reap safe trees + merged branches
    poormad worktree prune --dry-run  # show the plan, change nothing
    poormad worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `poormad worktree prune`."
        )
