...
``CommandDef.execute`` (poormad_cli/commands.py) names a key in
...
for a fixed context (enforced by tests/poormad_cli/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``poormad_cli.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    if not home_display:
        ...
        home_display = display_poormad_home()
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: poormad bundles create <name> --skill <s1> --skill <s2>\n"
...
def _exec_help(ctx: CommandContext) -> CommandReply:
    ...
    from poormad_cli.commands import gateway_help_lines
...
def _exec_poormad_id(ctx: CommandContext) -> CommandReply:
    """Core /poormad body — the PoorMad identity card."""
    import platform as _platform
    import sys as _sys

    try:
        from poormad_cli import __version__ as _ver
    except Exception:
        _ver = "unknown"

    lines = [
        "🌑 **PoorMad** — the self-improving AI agent",
        f"Version: {_ver}",
        f"Python: {_sys.version.split()[0]} · Platform: {_platform.system()} {_platform.release()}",
        "Status: ready",
        "",
        "When the light is blocked, the stars align.",
    ]
    return CommandReply("\n".join(lines), format="markdown")
