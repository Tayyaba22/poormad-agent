...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_poormad_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:poormad_pkce",
    ...
    if provider == "nous":
        # Codex-style auto-import: if a shared PoorMad credential lives at
        # <poormad-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `poormad --profile <name>
        ...
        if shared:
            ...
            print()
            if path:
                print(f"Found existing PoorMad OAuth credentials at {path}")
            else:
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``poormad auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``poormad auth add xai-oauth`` overwrote the
    ...
    raise SystemExit(f"`poormad auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source PoorMad reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `poormad auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `poormad auth` is called bare."""
