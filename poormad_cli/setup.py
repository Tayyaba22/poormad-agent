"""
Interactive setup wizard for PoorMad.
...
from poormad_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
from poormad_cli.nous_subscription import get_nous_subscription_features
from tools.tool_backend_helpers import managed_nous_tools_enabled
from poormad_constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https://poormad.dev/docs"
...
# display_poormad_home imported lazily at call sites (stale-module safety during poormad update)
...
from poormad_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from poormad_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("🌑 PoorMad Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure PoorMad using environment variables or config commands:")
    print_info("  poormad config set model.provider custom")
    print_info("  poormad config set model.base_url http://localhost:8080/v1")
    print_info("  poormad config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'poormad setup' in an interactive terminal to use the full wizard.")
    print()
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar(
    "poormad_setup_navigation", default=None
)
...
def is_noninteractive() -> bool:
    ...
    ``POORMAD_NONINTERACTIVE=1`` (see ``poormad_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("POORMAD_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``POORMAD_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from poormad_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'poormad setup')")


def _print_setup_summary(config: dict, poormad_home):
    ...
    try:
        from poormad_cli.auth import resolve_provider
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — PoorMad cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    poormad model            (pick any provider/model)")
        print_info("    poormad setup --portal   (PoorMad Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'poormad setup' to configure"))


    # Web tools (Exa, Parallel, Firecrawl, or Tavily)
    if subscription_features.web.managed_by_nous:
        tool_status.append(("Web Search & Extract (PoorMad subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_nous:
        tool_status.append(("Browser Automation (PoorMad Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via PoorMad), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_nous:
        tool_status.append(("Image Generation (PoorMad subscription)", True, None))
        ...
        try:
            ...
            from poormad_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `poormad tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    if subscription_features.video_gen.managed_by_nous:
        tool_status.append(("Video Generation (FAL via PoorMad subscription)", True, None))
    else:
        try:
            from agent.video_gen_registry import list_providers as _list_video_providers
            from poormad_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        tool_status.append(("Text-to-Speech (OpenAI via PoorMad subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'poormad setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'poormad setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        tool_status.append(("Speech-to-Text (OpenAI via PoorMad subscription)", True, None))
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'poormad tools' → Speech-to-Text")
            )

    if subscription_features.modal.managed_by_nous:
        tool_status.append(("Modal Execution (PoorMad subscription)", True, None))
    elif cfg_get(config, "terminal", "backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'poormad setup terminal'"))
    elif managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        tool_status.append(("Modal Execution (optional via PoorMad subscription)", True, None))

    # Home Assistant
    if get_env_value("HASS_TOKEN"):
        tool_status.append(("Smart Home (Home Assistant)", True, None))

    # Spotify (OAuth via poormad auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'poormad setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {poormad_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('poormad setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('poormad setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('poormad setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('poormad setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('poormad setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('poormad config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('poormad config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('poormad config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('poormad', Colors.GREEN)}              Start chatting")
    print(f"   {color('poormad gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('poormad doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `poormad tools` and `poormad setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``poormad model``)
    ...
    provider added to ``poormad model`` is automatically available here.
    ...
    # Delegate to the shared poormad model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from poormad_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: poormad model")
    ...
    # on demand via `poormad auth add`, `poormad setup` vision, and
    # `poormad setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from poormad_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``poormad model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from poormad_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        choices.append("PoorMad Subscription (managed OpenAI TTS, billed to your subscription)")
    ...
    if selected == "nous-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed PoorMad gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.poormad/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the PoorMad auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via poormad setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'poormad setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where PoorMad runs shell commands and code.")
    ...
    # under ~/.poormad/plugins/). Fail-soft: a broken plugin must not take the
    # setup wizard down.
    ...
    if selected_backend == "local":
        ...
        # configurable later via `poormad setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `poormad setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `poormad egress setup` then `poormad egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `poormad egress setup`."
            )
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed PoorMad gateway and bill to your subscription.")
        ...
        print_info("Requires the optional SDK: pip install 'poormad-agent[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $POORMAD_HOME/bin is never on PATH, so a bare
            # which() misses the uv PoorMad installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'poormad-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into POORMAD_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("POORMAD_MAX_ITERATIONS")
    ...
    print_info("  Run `poormad setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # POORMAD_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.poormad/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from poormad_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_poormad_home(Path(get_poormad_home()))
    except Exception:
        pass
...
def _profile_name_from_poormad_home(poormad_home) -> str | None:
    """Return the active profile name when POORMAD_HOME is a profile dir."""
    if poormad_home.parent.name == "profiles":
        return poormad_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where PoorMad delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects PoorMad to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from poormad_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://poormad.dev/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://poormad.dev/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  poormad config edit")
    print_info("   Open config in your editor:  poormad config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from poormad_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with PoorMad from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'poormad setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     poormad config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `poormad import` or `poormad setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `poormad setup tools` and `poormad tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this PoorMad profile and are not uploaded.")
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from poormad_cli.config
    ...
    if section_key == "model":
        ...
        from poormad_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_poormad.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_poormad migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_poormad", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and PoorMad.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure PoorMad to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point PoorMad at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point PoorMad at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point PoorMad at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point PoorMad at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to PoorMad equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing PoorMad config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in PoorMad.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ PoorMad's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(poormad_home: Path) -> bool:
    ...
    print_info("PoorMad can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: poormad claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=poormad_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: poormad claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing PoorMad configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing PoorMad config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in PoorMad (use poormad claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot PoorMad Portal setup — OAuth + model pick + provider + Tool Gateway.

    Wired into ``poormad setup --portal`` and ``poormad portal``. This is the
    PoorMad-Portal slice of the first-time quick setup, collapsed into a single
    shareable command so a brand-new user goes from zero to a fully working
    PoorMad session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``poormad setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``poormad model`` runs
    when you pick PoorMad. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``poormad portal`` always offers a model picker,
    and there is a single source of truth for the PoorMad onboarding steps.
    """
    ...
    print(color("│     🌑 PoorMad Setup — PoorMad Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your PoorMad Portal sub.")
    print()
    print_info("  Sign up: https://portal.poormad.dev/manage-subscription")
    print()
    ...
    # PoorMad model picker), then offers the Tool Gateway opt-in and sets
    # provider=nous via the login/model save. This is the same routine quick
    # setup calls, so `poormad portal` == quick setup's PoorMad step.
    try:
        from poormad_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `poormad portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `poormad portal`: %s", exc)
        print()
        print_error(f"  PoorMad Portal setup encountered an error: {exc}")
        print_info("  You can retry later with `poormad portal`.")
        return
    ...
    print_info("  Run `poormad portal info` to inspect routing.")
    print_info("  Run `poormad` to start chatting.")


@contextmanager
def _setup_navigation_scope():
    """Install and reliably restore the setup menu navigation context."""
    from poormad_cli.curses_ui import (
...
def run_setup_action_with_navigation(
    ...
    *,
    ...
    Shared commands such as ``poormad model`` use the same provider/model
...
def _run_setup_wizard_impl(args):
    ...
    Supports full, quick, and section-specific setup:
      poormad setup           — full or quick (auto-detected)
      poormad setup model     — just model/provider
      poormad setup tts       — just text-to-speech
      poormad setup terminal  — just terminal backend
      poormad setup gateway   — just messaging platforms
      poormad setup tools     — just tool configuration
      poormad setup telemetry — just local shared metrics
      poormad setup agent     — just agent settings
    """
    from poormad_cli.config import is_managed, managed_error
    if is_managed():
    ...
    # --portal: one-shot PoorMad Portal setup. Skips the rest of the wizard.
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     🌑 PoorMad Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             🌑 PoorMad Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your PoorMad installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps(
                [("Quick Setup", lambda: _run_quick_setup(config, poormad_home))]
            )
            return

        print()
        print_header("Reconfigure")
        print_success("You already have PoorMad configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'poormad setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(poormad_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up PoorMad?",
            [
                "Quick Setup (PoorMad Portal) — free OAuth login, no API keys, model + tools (recommended)",
                ...
                [
                    (
                        ...
                        lambda: _run_first_time_quick_setup(
                            config, poormad_home, is_existing
                        ),
                    )
                ]
            )
    ...
    print_info(f"Data folder:  {poormad_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'poormad config edit'")
    ...
    # Tune later with `poormad setup agent`.
    if not is_existing:
    ...
    def _gateway_step() -> None:
        ...
        from poormad_cli.gateway import ensure_gateway_service
    ...
    _print_setup_summary(config, poormad_home)


def _run_first_time_quick_setup(config: dict, poormad_home, is_existing: bool):
    """Streamlined first-time setup via PoorMad Portal: OAuth, model, terminal & messaging.

    Routes straight to the PoorMad Portal provider — runs the device-code OAuth
    login, picks a PoorMad model, then configures the terminal backend and (optionally)
    a messaging platform. Applies sensible defaults for everything else (agent
    settings, tools); the user can customize later via ``poormad setup <section>``
    or switch providers with ``poormad model``.
    """
    from poormad_cli.config import load_config

    # Step 1: PoorMad Portal — OAuth login + model selection.
    # _model_flow_nous() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # PoorMad model picker). Provider is set to "nous" by the login/model save.
    print()
    print_header("PoorMad Portal")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: https://portal.poormad.dev/manage-subscription")
    print()
    try:
        ...
        print()
        print_info("PoorMad Portal setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_nous error during quick setup: %s", exc)
        print_warning(f"PoorMad Portal setup encountered an error: {exc}")
        print_info("You can try again later with: poormad model")
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'poormad setup gateway'",
        ],
        0,
    )
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `poormad import` from another machine).
        from poormad_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    print()
    print_info("  Configure all settings:    poormad setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  poormad setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    print_info("  your terminal (and PoorMad.app if you use Desktop), or run:")
    print_info("    open \"x-apple.systempreferences:com.apple.preference"
               ".security?Privacy_AllFiles\"")
    print_info("  The grant is permanent — it survives every PoorMad update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``poormad-agent`` skill (the agent's operating manual for driving,
    configuring, and troubleshooting PoorMad) is always seeded — without
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``poormad tools`` (which
    ...
    try:
        ...
        from poormad_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``poormad setup agent`` /
    ``poormad config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, poormad_home, is_existing: bool):
    ...
    print_info("essential `poormad-agent` skill is always kept so the agent")
    print_info("can help you drive and configure PoorMad itself.")
    print()
    ...
    if path == 0:
        ...
        # `poormad update` runs don't re-inject them. Essential skills (the
        # `poormad-agent` operating manual) are still seeded by the sync.
        try:
        ...
        print_info("  Enable tools:        poormad tools")
        print_info("  Seed skills:         poormad skills opt-in --sync")
        print_info("  Add MCP servers:     poormad mcp add")
        print_info("  Enable plugins:      poormad plugins")
        print_info("  Tune agent settings: poormad setup agent")
        print()
        _print_setup_summary(config, poormad_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, poormad_home)


def _blank_slate_walkthrough(config: dict, poormad_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            set_bundled_skills_opt_out(True)
            # Essential skills (the `poormad-agent` operating manual) are
            # still seeded even for an opted-out profile.
            sync_skills(quiet=True)
            print_info("No skills seeded (except the essential `poormad-agent`")
            print_info("skill). A .no-bundled-skills marker keeps future")
            print_info("`poormad update` runs from re-injecting them. Opt back in any")
            print_info("time with `poormad skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `poormad tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `poormad plugins list` / `poormad plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `poormad plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `poormad mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `poormad mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   poormad tools")
    print_info("  Seed skills:         poormad skills opt-in --sync")
    print_info("  Add MCP servers:     poormad mcp add")
    print_info("  Tune agent settings: poormad setup agent")
    print()
...
def _run_quick_setup(config: dict, poormad_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'poormad setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect PoorMad to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'poormad setup gateway'.")
