"""Slash command definitions and autocomplete for the PoorMad CLI.
...
from poormad_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``poormad_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``poormad_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of PoorMad config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal PoorMad works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey PoorMad' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while PoorMad is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.poormad/skills/ for newly installed or removed skills",
    ...
    CommandDef("subscription", "View your PoorMad plan and change it in the browser", "Info",
               cli_only=True, aliases=("upgrade",)),
    CommandDef("topup", "Show your PoorMad balance and manage billing on the portal", "Info"),
    ...
    CommandDef("update", "Update PoorMad to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show PoorMad version", "Info", aliases=("v",),
               busy_policy="dispatch", execute="version"),
    CommandDef("debug", "Upload debug report (system info + logs) and get shareable links", "Info",
               args_hint="[nous|local]"),

    # PoorMad — unique command set (PoorMad-native differentiators)
    CommandDef("poormad", "Show the PoorMad identity card (brand, version, status)", "PoorMad",
               aliases=("pm",), busy_policy="dispatch", execute="poormad_id"),
    CommandDef("obey", "Switch to full-autonomy mode (approvals off) for this session", "PoorMad",
               args_hint="[on|off]", busy_policy="dispatch"),
    CommandDef("boss", "Delegate a task to a subagent fleet with N workers (swarm lite)", "PoorMad",
               args_hint="<goal> --workers N", busy_policy="dispatch"),
    CommandDef("vibe", "Swap agent persona/tone instantly (e.g. /vibe pirate)", "PoorMad",
               args_hint="<mood>", busy_policy="dispatch"),
    CommandDef("reign", "Show PoorMad hierarchy: active agents, cron jobs, sessions, quotas", "PoorMad",
               busy_policy="dispatch"),
    CommandDef("throne", "Promote this chat to the priority lane (fast model, high quota)", "PoorMad",
               busy_policy="dispatch"),
    CommandDef("coin", "Cost-to-date + token burn report for this session", "PoorMad",
               aliases=("cost",), busy_policy="dispatch"),
    CommandDef("scroll", "Compress + summarize a long session into a PoorMad digest", "PoorMad",
               busy_policy="dispatch"),
    CommandDef("forge", "Generate a new skill from the current conversation pattern", "PoorMad",
               args_hint="[name]", busy_policy="dispatch"),
    CommandDef("shadow", "Fork a silent parallel agent to double-check the next answer", "PoorMad",
               busy_policy="dispatch"),
    CommandDef("poormad-theme", "Switch PoorMad skins (poormad-dark, aurora, terminal, ...)", "PoorMad",
               args_hint="<name>", aliases=("theme-pm",), busy_policy="dispatch"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`poormad_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/poormad`` subcommand mapping, and
    ...
    """
    try:
        from poormad_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
# Telegram allows up to 100 BotCommands. PoorMad ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full PoorMad
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``poormad skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``poormad skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``poormad skills list`` and
...
# unrelated command landed. These claim their slots right after /poormad,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /poormad <command>).
...
# native slot, the alias spelling stays reachable via /poormad reset).
...
# ``/poormad <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/poormad" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /poormad topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /poormad moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /poormad debug on Slack.
#   - egress: Docker-only proxy status; reachable as /poormad egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /poormad init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /poormad version on
...
#   - diff: git working-tree diff; reached via /poormad diff on Slack so it
...
#     /poormad update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /poormad heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /poormad refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /poormad pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /poormad whoami on Slack.
...
#     /poormad platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_POORMAD_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "review", "pause", "whoami", "platform"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/poormad <verb>`` subcommand.
    ...
    can still reach them via ``/poormad <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/poormad`` is always reserved as the first entry so the
    legacy ``/poormad <subcommand>`` form keeps working for anything that
    ...
    # Reserve /poormad as the catch-all top-level command.
    entries.append(("poormad", "Talk to PoorMad or run a subcommand", "[subcommand] [args]"))
    seen.add("poormad")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_POORMAD_ONLY:
            # Intentionally Slack-via-/poormad only (see _SLACK_VIA_POORMAD_ONLY).
            return
    ...
    # everything except /poormad, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://poormad-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /poormad handler.

    Maps both canonical names and aliases so /poormad bg do stuff works
    the same as /poormad background do stuff.

    Plugin-registered slash commands are included so ``/poormad <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from poormad_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via poormad_cli.personality."""
        try:
