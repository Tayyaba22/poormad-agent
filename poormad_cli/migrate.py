"""CLI handlers for ``poormad migrate ...``.

Currently exposes only ``poormad migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``poormad migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: poormad migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `poormad migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `poormad doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
