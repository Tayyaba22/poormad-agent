"""PoorMad update pipeline — extracted from ``poormad_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``poormad_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``poormad_cli.main`` reference — so existing call sites and test monkeypatches
that target ``poormad_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``poormad_cli.main``.
...
Imports are one-way: ``poormad_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``poormad_cli.main`` reference.

    Lets callers keep patching ``poormad_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``poormad_cli.main`` -> ``poormad_cli.update_cmd`` stays one-way
    at import time.
    """
    from poormad_cli import main
...
#: ``_purge_stale_poormad_modules`` so any LATER import chain resolves against
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        ...
        "poormad_cli.poormad_logging",
    }
)


def _purge_stale_poormad_modules() -> None:
    """Evict every cached PoorMad module after the checkout changed in-place.

    ``poormad update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from poormad_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``poormad_cli.gateway`` does
    ``from poormad_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the PoorMad
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale PoorMad module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale PoorMad modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``poormad update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``poormad update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``poormad_cli.config_defaults``,
    ``poormad_cli.config``, and ``poormad_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``poormad_cli._subprocess_compat`` and
    ``poormad_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "poormad_cli.config_defaults",
        "poormad_cli.config",
        "poormad_cli.config_migrations",
        "poormad_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from poormad_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from poormad_cli.config import migrate_config
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    shared checkout serves every profile, but ``poormad update`` historically
    ...
    caller): scope config reads/writes via the context-local POORMAD_HOME
    ...
    try:
        ...
        from poormad_cli.profiles import _get_profiles_root, _PROFILE_ID_RE

        active_home = get_process_poormad_home()
        ...
        for entry in sorted(root.iterdir()):
            ...
            token = set_poormad_home_override(entry)
            try:
...
def _check_and_apply_config_migration(
    *,
    ...
    try:
        ...
        print("     Run 'poormad config migrate' to check manually.")
        return
    ...
    if version_bump_only:
        ...
        try:
            ...
            print("     Run 'poormad config migrate' to retry.")
        ...
        if assume_yes:
            ...
            try:
                ...
                print(
                    ...
                    "Run 'poormad config migrate' manually to configure."
                )
                response = "n"
        ...
        if response in {"", "y", "yes", "auto"}:
            ...
            if (gateway_mode or assume_yes or response == "auto") and missing_env:
                print("  ℹ API keys require manual entry: poormad config migrate")
        else:
            print()
            print("Skipped. Run 'poormad config migrate' later to configure.")
    else:
    ...
    # context-local POORMAD_HOME override (never os.environ — other
    ...
    # it and warn loudly.
    try:
        from poormad_cli.backup import restore_cron_jobs_if_emptied
    ...
    try:
        from poormad_cli.backup import restore_cron_jobs_all_profiles
...
# Critical files that PoorMad must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "poormad_cli/main.py",
    "poormad_cli/config.py",
    "poormad_cli/__init__.py",
    "poormad_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "poormad_constants.py",
)
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    On Windows that rewrite is the only reason the running ``poormad.exe`` has
    ...
    Skipping is safe because PoorMad pins its editable finder to a *static*
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``poormad`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="poormad-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``poormad update`` can be driven by a
    ...
    """
    from poormad_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from poormad_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('poormad_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``poormad update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `poormad update`.
    ...
    print("  Preview now:  poormad curator run --dry-run")
    print("  Pause it:     poormad curator pause")
    print(
        "  Docs:         https://poormad.dev/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `poormad update`.
    ...
    try:
        ...
        return
    db_path = get_poormad_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `poormad sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    poormad sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  poormad sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``poormad update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``poormad update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: poormad curator status)"
    )
...
def _reload_process_scan_modules() -> None:
    ...
    ``from poormad_cli._subprocess_compat import bounded_probe_run``. If the
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units* forwards the systemd unit names (no
    ...
    print("    poormad dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``poormad_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.poormad-update-staging"
    backup = f"{dst}.poormad-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `poormad update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``poormad_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``poormad update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && poormad update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("POORMAD_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== poormad-update completed {action_id} ===")
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live PoorMad launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["poormad.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically PoorMad")
    print("  Desktop, a gateway, or another poormad REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `poormad` command to finish it automatically.")
...
def _should_zip_fallback_on_update_error(exc: BaseException) -> bool:
    ...
    A dependency-install failure (locked ``poormad.exe`` / ``uv pip install``
...
def _zip_overlay_block_reason(
    ...
    two-phase replace creates ``*.poormad-update-staging`` siblings inside the
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = (".poormad-update-staging", ".poormad-update-old")
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `poormad update`.")
...
def _clear_stale_sqlite_sidecars(db_path: Path) -> None:
    ...
    database (a crashed writer, or a second PoorMad process the updater's drain
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from poormad_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity

    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (poormad gateway stop), "
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    """Update PoorMad by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `poormad update "
            f"--branch {branch}`, or update against main with `poormad update`."
        )
        _m().sys.exit(1)
    _abort_zip_update_if_dirty_tree()
    zip_url = (
        f"https://github.com/Tayyaba22/poormad-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="poormad-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"poormad-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for poormad-agent itself
        ...
        # GitHub ZIPs extract to poormad-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"poormad-agent-{branch}")
        ...
        try:
            for item in entries:
                ...
                # PoorMad.exe) exists only in the LIVE tree. Swapping `apps`
        ...
        print(
            "  Re-run `poormad update` to retry; if the agent won't start, "
            "reinstall from https://poormad.dev"
        )
        _m().sys.exit(1)
    finally:
    ...
    from poormad_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if uv_bin:
        ...
        from poormad_cli.managed_uv import managed_python_env
    ...
    if not import_ok:
        ...
        print("  Re-run `poormad update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: poormad skills list-modified  "
    ...
    try:
        from poormad_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from poormad_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_poormad_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_poormad_home())
    ...
    try:
        from poormad_cli.update_receipt import finalize_update_receipt
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "poormad-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``poormad update --keep-stash`` (the desktop updater's mode): the
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if PoorMad behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes poormad completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but PoorMad couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but PoorMad couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if PoorMad behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but PoorMad couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but PoorMad couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/Tayyaba22/poormad-agent.git",
    "git@github.com:PoorMad/poormad-agent.git",
    "https://github.com/Tayyaba22/poormad-agent",
    "git@github.com:PoorMad/poormad-agent",
}

OFFICIAL_REPO_URL = "https://github.com/Tayyaba22/poormad-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_poormad_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_poormad_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official PoorMad repository.")
        print("  This means you may miss updates from PoorMad/poormad-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/Tayyaba22/poormad-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/Tayyaba22/poormad-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``poormad update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from poormad_constants import get_default_poormad_root

    default_home = get_default_poormad_root()
...
# ``fleet_restart_pending`` lives under POORMAD_HOME (not next to the venv).
...
def _fleet_restart_pending_marker_path() -> Path:
    """POORMAD_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    return get_poormad_home() / _FLEET_RESTART_PENDING_NAME
...
def _current_checkout_sha() -> str | None:
    ...
    try:
        from poormad_cli.build_info import get_code_identity
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print(
        "⚠ A previous `poormad update` pulled new code but did not "
    ...
    if startup:
        print(
            "  Run `poormad update` or `poormad gateway restart`.",
            file=stream,
        )
...
def _restart_systemd_gateway_units_best_effort(failed: list) -> None:
    """Best-effort ``systemctl restart`` of every poormad-gateway/serve unit."""
...
def _run_pending_fleet_restart() -> bool:
    ...
    try:
        _m()._purge_stale_poormad_modules()
    except Exception:
        pass
    try:
        from poormad_cli.gateway import (
            find_gateway_pids,
            is_macos,
            is_windows,
    ...
    try:
        ...
        if is_windows():
            try:
                from poormad_cli import gateway_windows
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``poormad update``, finish a skipped restart.
    ...
    if _run_pending_fleet_restart():
        ...
        return
    print("  ⚠ Fleet restart incomplete. Recover with: poormad gateway restart")
    sys.exit(1)
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "poormad.exe"
    lines = ["✗ Another poormad.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close PoorMad Desktop, exit any open `poormad` REPLs, and")
    lines.append("  stop the gateway (`poormad gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `poormad update --force` if you've already")
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from poormad_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _filter_non_gateway_concurrent_instances(
    ...
    ``poormad update``. If every concurrent instance is a gateway, the pause
    ...
    TUI shell, a PoorMad Desktop backend child, an unrelated ``poormad`` REPL)
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``poormad tools``."""
    try:
        from poormad_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active PoorMad Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``poormad tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("PoorMad Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} PoorMad Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``poormad update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `poormad update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `poormad update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in PoorMad' editable-install extras or
    ...
    try:
        from poormad_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from poormad_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$POORMAD_HOME/bin/uv``, but on Termux the official
    ...
    """
    from poormad_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `poormad update` is exactly the step
...
def _npm_lockfile_changed(poormad_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `poormad
    ...
    try:
        ...
        cache_file = poormad_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(poormad_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    poormad update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `poormad update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from poormad_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `poormad update`.")
            failed = []
            if any(
    ...
    # PoorMad profile using this checkout. Keep one per-checkout cache under the
    # shared PoorMad root rather than rerunning npm once per named profile.
    shared_poormad_root = get_default_poormad_root()
    ...
    # `poormad update` case. Synchronous and can block ~11s on a true cold
    ...
    if not _m()._npm_lockfile_changed(shared_poormad_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `poormad update`.")
        return list(labels)
    ...
    nixos_env = with_poormad_node_path(_m()._nixos_build_env())
    ...
    # `poormad update` comes from the *desktop* build, not this step; that
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_poormad_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.poormad/logs/update.log`` only, never the terminal.

    During ``poormad update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``poormad update --check``: fetch and report without installing.
    ...
    from poormad_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from poormad_cli.banner import _github_compare_behind
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``poormad update`` without requiring a reinstall.  The
    ...
    ``poormad`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/poormad.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v poormad'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/poormad, code at /usr/local/lib/poormad-agent).
    fhs_link = Path("/usr/local/bin/poormad")
    ...
    path_comment = (
        "# PoorMad — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal: install a ``poormad-acp`` launcher next to the ``poormad`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``poormad update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``poormad-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report PoorMad as not installed even when it is.

    The shim simply delegates to the sibling ``poormad`` launcher with the
    ...
    No-op on Windows (install.ps1 stages the ``poormad`` / ``poormad-acp``
    launchers into the managed binary dir ``$PoorMadHome\bin`` and puts THAT
    ...
    ``poormad_cli._install_repair.ensure_windows_bin_launchers`` re-stages
    them) and wherever a ``poormad-acp`` is already present next to the
    ``poormad`` command.  Unwritable directories (e.g. ``/usr/local/bin`` as
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        poormad_cmd = bin_dir / "poormad"
        acp_cmd = bin_dir / "poormad-acp"
        try:
            if not (poormad_cmd.is_file() or poormad_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of POORMAD_HOME under
      ``backups/`` (restorable via ``poormad import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from poormad_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from poormad_constants import
        # get_poormad_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from poormad_cli.config import get_poormad_home as _get_home
        ...
        try:
            from poormad_cli.backup import create_pre_update_snapshots_all_profiles
            ...
            if _sibling_snaps:
                ...
                try:
                    from poormad_cli.update_receipt import record_step
    ...
    try:
        from poormad_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from poormad_cli.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_poormad_home so the user sees ~/.poormad/...
    try:
        from poormad_constants import get_poormad_home, display_poormad_home

        home = get_poormad_home()
        try:
            display_path = f"{display_poormad_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  poormad import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``poormad update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``poormad update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run poormad from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.poormad-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".poormad-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The poormad.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m poormad_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``poormad
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m poormad_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "poormad_cli.main" in cmdline_low:
        ...
        # the `-m poormad_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``poormad_cli.main`` used to import
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    - The ``poormad.exe`` console shim we were launched from (#88838, #89599).
      The marker cannot help here — every future ``poormad`` launch is also the
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `poormad` launch will complete it")
...
def _poormad_holder_subcommand(cmdline: str) -> str | None:
    """The actual PoorMad SUBCOMMAND a venv-holder argv runs, or None.

    Token-based, never substring (#90778: ``kanban --preserve-cache``
    contained \"serve\" and got labeled as the Desktop backend). Finds the
    ``poormad_cli.main`` / ``poormad(.exe)`` entry token, then returns the
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("poormad_cli.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
        base = low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
        if base in ("poormad", "poormad.exe"):
            entry_idx = i
            break
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``poormad dashboard`` must not be labeled as the
    ...
    """
    lines = [
        "✗ Other PoorMad processes are running from this install's venv:",
    ]
    hint_by_subcommand = {
        "serve": "  ← PoorMad backend (if the Desktop app is open, close it)",
        "dashboard": "  ← poormad dashboard (stop it: poormad dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
    for pid, name, cmdline in matches[:6]:
        sub = _poormad_holder_subcommand(cmdline)
    ...
    lines.append(
        "  Close the PoorMad desktop app / other PoorMad terminals, then re-run:"
    )
    lines.append("    poormad update")
    lines.append("  (or use `poormad update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``poormad update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from poormad_cli._scan_venv_blockers import _is_pausable_gateway
...
def _ledger_manual_serve_holders(
    ...
    PoorMad spawner). Returns the full ledger entries so the relauncher can
    rebuild the launch command from structured host/port/profile instead of
    parsing argv.
    """
    try:
        from poormad_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _serve_relaunch_commands(entries: list[dict]) -> list[list[str]]:
    ...
    try:
        ...
        if scripts_dir is not None:
            for name in ("poormad.exe", "poormad"):
                candidate = scripts_dir / name
                if candidate.is_file():
                    poormad = str(candidate)
                    break
    except Exception:
        poormad = None
    if poormad is None:
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    if skipped or failed:
        print(
            ...
            "restart them manually (poormad serve --host <ip> --port <port>)."
        )
    try:
        from poormad_cli.update_receipt import record_step
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning poormad-setup, and the update-in-progress marker
    ...
    the update with "PoorMad is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a PoorMad backend (``poormad_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.poormad-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "poormad_cli.main" in argv_low and (
...
def _ledger_reapable_backend_pids(
    ...
    (``poormad_cli.process_identity``). A holder qualifies when ALL of:
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of PoorMad ``serve``/``dashboard`` backends safe to reap during a
    ...
    run AND no live Desktop shim (``poormad.exe``) is open, NOTHING legitimate
    ...
    - Only PoorMad backends (``poormad_cli.main`` + ``serve``/``dashboard``)
    ...
    - Only runs when the CALLER has confirmed the hand-off context
      ...
      ``poormad.exe`` shim) — outside that gate this function is never called
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.poormad-runtime`` interpreter child alive and holding the install open
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``poormad serve`` / ``poormad dashboard`` argv.
    ...
    """
    if "poormad_cli.main" not in (cmdline or "").lower():
        return False
    return _poormad_holder_subcommand(cmdline) in ("serve", "dashboard")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    poormad.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating PoorMad...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m poormad_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: poormad gateway run")
...
def _cold_start_windows_gateway_after_update() -> bool:
    ...
    ``poormad gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from poormad_cli import gateway_windows
        from poormad_cli.gateway import find_gateway_pids
    except Exception as exc:
        raise RuntimeError(
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``poormad-gateway*.service``/``poormad-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("poormad-serve")`` alone would also accept the
        # unrelated ``poormad-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "poormad-gateway.service"
            or unit.startswith("poormad-gateway-")
            or unit == "poormad-serve.service"
            or unit.startswith("poormad-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``poormad-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``poormad-serve*`` units (#83438) don't, so sending them
    ...
    (``poormad-gateway-helper`` is fine — profile units are
    ``poormad-gateway-<profile>`` — but ``poormad-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "poormad-gateway" or svc_name.startswith("poormad-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from poormad_cli.gateway import is_macos
    ...
    print("    poormad gateway status")
    if any(not name.startswith("ai.poormad.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.poormad.") for name in ordered):
...
def _restart_launchd_gateway_after_update(
    *, supervision_verify: bool = True
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: poormad gateway restart"
            )
    ...
    print(
        ...
        "    Check logs, then: poormad gateway restart"
    )
    return [], [current_label]
...
def _restart_macos_launchd_gateways(
    ...
    ``ai.poormad.gateway*`` LaunchAgent must reload it — restarting only the
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``poormad_cli.gateway`` itself no longer imports, which is one of the
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    that ``poormad-gateway-foobar.service`` was already restarted.  These are
    ...
    return name in {
        f"poormad-gateway-{profile}",
        f"ai.poormad.gateway-{profile}",
        f"gateway-{profile}",
    }
...
def _gateway_recovery_partition(
    ...
    only safe source here: re-importing ``poormad_cli.gateway`` in the failing
...
def _recover_gateway_restart_after_abort(
    ...
    ``poormad update`` normally performs the fleet restart in the interpreter
    ...
    command = [
        sys.executable,
        "-m",
        "poormad_cli.update_restart_recovery",
        "--stdin",
    ]
    env = os.environ.copy()
    env["POORMAD_UPDATE_RESTART_RECOVERY"] = "1"
    for marker in ("_POORMAD_GATEWAY", "POORMAD_GATEWAY", "POORMAD_GATEWAY_MODE"):
        env.pop(marker, None)
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``poormad_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``poormad update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``poormad-setup.exe``) executes
    ``$POORMAD_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_poormad_home()) / "bootstrap-cache"
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``poormad update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``poormad update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `poormad desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "poormad_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the PoorMad-managed Node on PATH: when
    # `poormad update` runs inside the desktop updater chain (Desktop →
    # poormad-setup → poormad update), the shell PATH customizations are lost,
    # so a bare-PATH child would fail with `node: not found` before cmd_gui can
    # self-heal.
    from poormad_constants import with_poormad_node_path

    build_env = with_poormad_node_path()
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `poormad desktop` to retry)")
        tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
        if tail:
            print(tail)
        from poormad_constants import display_poormad_home as _dhh
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        from poormad_cli.update_receipt import begin_update_receipt
    ...
    # running PoorMad runtime, its supervisor, and its running code version —
    ...
    # On Windows, abort early if another poormad.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        scripts_dir = _m()._venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_poormad_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `poormad serve` backend) will keep .pyd
    ...
    # the poormad.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned PoorMad backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "PoorMad backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .poormad-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "PoorMad is still
        ...
        if _venv_holders:
            ...
            # `poormad serve --host <ip>` powering a REMOTE Desktop holds the
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving PoorMad backends are leaks,
            ...
            try:
                ...
                if _scripts_dir is not None:
                    _no_live_shim = not _m()._detect_concurrent_poormad_instances(_scripts_dir)
            except Exception:
                _no_live_shim = False
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} PoorMad backend process(es) "
    ...
    # process by design (a CLI `poormad update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if handed_off_sync:
                print("→ Finishing the dependency install handed off by poormad.exe...")
            elif not healthy:
            ...
            if handed_off_sync or not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all PoorMad windows/gateways and re-run: poormad update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running PoorMad gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in poormad_cli/config.py bricked
        # every user who ran ``poormad update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``poormad
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``poormad update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``poormad update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``poormad doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && poormad update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # ``poormad update`` can catch up even when git is already up to date.
        ...
        # the old bytecode (e.g. get_poormad_home added to poormad_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``poormad`` launch finishes the install
        ...
        # symbols in poormad_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `poormad` again "
        ...
        if not import_ok:
            ...
            print("    Run `poormad update` again — if it persists, reinstall:")
            print("    https://poormad.dev")
        ...
        if sys.platform == "darwin" and had_desktop_app_before_update:
            print()
            print(
                "  ℹ macOS: if PoorMad re-prompts for permissions you already "
                "granted (toggle shows ON), the stored grant is stale — run "
                "`tccutil reset ScreenCapture com.poormad.poormad` (repeat "
        ...
        try:
            from poormad_cli.macos_tcc_anchor import ensure_tcc_anchor
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_poormad_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.poormad/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit POORMAD_HOME so
        # it is not affected by sync_skills()'s module-level POORMAD_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's POORMAD_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from poormad_cli.profiles import backfill_profile_envs
        ...
        # check `poormad curator status`. Self-stamps after printing so it
        ...
        # Self-heal the poormad-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve PoorMad on PATH without
        # a reinstall.  No-op on Windows (the launcher migration below owns
        # that) and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("poormad-acp launcher self-heal failed: %s", e)

        # Migrate the Windows poormad launchers to the managed binary dir
        # (the default PoorMad root's bin, next to the managed uv) and repair
        # them if they are missing. Earlier layouts put them inside the git
        # checkout (poormad-agent\bin) or put venv\Scripts itself on PATH; the
        # in-checkout copies were swept by this command's own pre-update
        # autostash (git stash push --include-untracked) and, with
        # --keep-stash (the desktop updater), never restored — `poormad`
        ...
        # the managed clone under the default PoorMad root).
        try:
            from poormad_cli._install_repair import migrate_windows_bin_path
        ...
        # don't have it. Tying the refresh to ``poormad update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from poormad_cli.tools_config import install_cua_driver
                ...
                # driver) keeps the installed version — `poormad update`
                # must stay fast; `poormad computer-use install --upgrade`
        ...
        # When running as ``poormad update --gateway`` (spawned by the gateway's
        ...
        #
        # Purge stale cached PoorMad modules FIRST: the import below pulls
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... poormad-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... poormad-gateway*)
            ...
            try:
                from poormad_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all poormad-gateway* units (default + profiles) plus
            # poormad-serve* units (the Desktop app's backend, #83438).
            if supports_systemd_services():
                try:
                    _ensure_user_systemd_env()
                except Exception:
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: poormad gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # poormad-serve has no such handler (it isn't
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `poormad update` restart it automatically, allow\n"
                        ...
                        # path in `poormad gateway restart`
            ...
            # Restart EVERY ai.poormad.gateway* LaunchAgent, not only the
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: poormad gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: poormad -p <profile> gateway run  for each profile)"
                        )

            if failed_or_stale_units:
                gateway_fleet_restart_incomplete = True
                if gateway_mode:
                    _exit_code_path = get_poormad_home() / ".update_exit_code"
                    try:
            ...
            try:
                from poormad_cli.update_receipt import record_gateway_restart
        ...
        # Warn if legacy PoorMad gateway unit files are still installed.
        # When both poormad.service (from a pre-rename install) and the
        # current poormad-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `poormad update` surfaces the issue until the user migrates.
        try:
            from poormad_cli.gateway import (
                has_legacy_poormad_units,
                _find_legacy_poormad_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_poormad_units():
                print()
                print("⚠ Legacy PoorMad gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_poormad_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (poormad.service) fight the current")
                print("  poormad-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    poormad gateway migrate-legacy")
                print()
        ...
        #
        # Forward the systemd units restarted above (includes poormad-serve*,
        ...
        print("  poormad model              # Select provider and model")
        ...
        try:
            from poormad_cli.update_receipt import (
        ...
        try:
            if _pre_update_plan is not None and _pre_update_plan.runtimes:
                ...
                if report_unaccounted_runtimes(_runtime_outcomes):
                    gateway_fleet_restart_incomplete = True
                try:
                    import poormad_cli.update_receipt as _ur
        ...
        if gateway_fleet_restart_incomplete:
            ...
            # ``poormad update`` still runs the catch-up restart.
            sys.exit(1)
        _clear_fleet_restart_pending_marker()
            ...
            if _called_process_error_is_python_dep_install(e):
                ...
                if _m()._is_windows():
                    ...
                    print(
                        ...
                        '"from poormad_cli.main import main; main()" update --yes'
                    )
            try:
                from poormad_cli.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``poormad_cli.gateway`` no longer imports,
