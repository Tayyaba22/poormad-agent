"""
PoorMad Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``poormad desktop`` / ``poormad gui``)
     Built inside the agent checkout under ``$POORMAD_HOME/poormad-agent/``:
       ...
       - ``$POORMAD_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/PoorMad.app`` or ``~/Applications/PoorMad.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\PoorMad`` (NSIS per-user)
...
the app name ("PoorMad"), separate from ``$POORMAD_HOME``:
  - macOS:   ``~/Library/Application Support/PoorMad``
  - Windows: ``%APPDATA%\\PoorMad``
  - Linux:   ``$XDG_CONFIG_HOME/PoorMad`` (default ``~/.config/PoorMad``)
...
``poormad uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(poormad_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return poormad_home / "poormad-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "PoorMad"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "PoorMad"
...
def source_built_gui_artifacts(poormad_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``poormad desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``poormad-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(poormad_home)
    ...
    return [
        ...
        poormad_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "PoorMad" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/PoorMad.app"),
            home / "Applications" / "PoorMad.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\PoorMad).
            local_base / "Programs" / "PoorMad",
            # Older / alternate layout some builds used.
            local_base / "poormad-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "PoorMad")
    else:
        ...
        # ``uninstall_gui``.
        from poormad_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `poormad desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "PoorMad.desktop",
        ]
    return paths


def agent_is_installed(poormad_home: Path) -> bool:
    """Return True when a usable Python agent install exists under POORMAD_HOME.
    ...
    if (agent_root / "poormad_cli").is_dir():
        return True
...
def gui_is_installed(poormad_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(poormad_home):
        if p.exists():
            return True
...
def gui_install_summary(poormad_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = poormad_home if poormad_home is not None else get_poormad_home()
...
def uninstall_gui(poormad_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``poormad-agent/poormad_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$POORMAD_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so PoorMad disappears from the launcher.
        try:
            from poormad_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove poormad' or "
            "'sudo dnf remove poormad'). AppImage builds are a single file you "
