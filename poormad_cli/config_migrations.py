...
``poormad_cli.config.migrate_config``. Each step is a function
...
``poormad_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``poormad_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``poormad_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``poormad_cli.config`` (e.g.
``patch("poormad_cli.config.read_raw_config", ...)``) keep working, because
...
#: POORMAD_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_poormad_home()}/config.yaml and run `poormad setup` to "
...
def _cfg():
    """Return the live ``poormad_cli.config`` module (lazy, cycle-free)."""
    from poormad_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_poormad_home = _c.get_poormad_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$POORMAD_HOME/plugins/`` for currently installed user plugins.
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `poormad plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `poormad curator status` has no
    ...
    #   3. Creates `~/.poormad/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_poormad_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_poormad_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `poormad config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `poormad config set`"
                )
...
# behavior set it to true explicitly via `poormad config set`.
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from poormad_cli.relay_plugin_cutover import legacy_relay_plugin_keys
    ...
    message = (
        ...
        "POORMAD_NEMO_RELAY_PLUGINS_TOML."
    )
...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # period that has since ended server-side, leaving every PoorMad-signed-in
    ...
    # provider surface (`video_generate`, `poormad tools` → Video Generation).
    ...
    if changed:
        ...
        if not quiet:
            print(
                ...
                "lists — video generation now lives under `poormad tools` → "
                "Video Generation (PoorMad Subscription or FAL)."
            )
