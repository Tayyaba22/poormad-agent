"""Client for uploading ``poormad debug share`` bundles to PoorMad-internal S3.

This is the opt-in (``--nous``) destination for ``poormad debug share``.
Unlike the public paste.rs path, bundles uploaded here go to a PoorMad-owned
S3 bucket via a short-lived signed URL minted by the PoorMad account service
(NAS).  The bucket auto-expires objects after 14 days, and the contents are
only viewable by PoorMad staff (and allowlisted Discord mods) through a
...
# Base URL of the PoorMad account service that mints the signed upload URL.
...
NAS_BASE = os.environ.get(
    "POORMAD_DIAGNOSTICS_BASE_URL", "https://portal.poormad.dev"
)
...
_USER_AGENT = "poormad-agent/debug-share"
...
def share_to_nous(report_bundle: bytes) -> dict:
    """Orchestrate the full PoorMad-S3 upload of a gzipped *report_bundle*.
