...
def _root() -> Path:
    ...
    # helper's token, including custom POORMAD_HOME layouts.
    return get_default_poormad_root() / "desktop-ssh"
...
def process_state(pid: int, creation_time_ns: int, poormad_path: str, spawn_nonce: str) -> dict[str, Any]:
    import psutil
    _nonce(spawn_nonce)
    try:
    ...
    expected = os.path.normcase(os.path.abspath(poormad_path))
    arg0 = os.path.normcase(os.path.abspath(argv[0]))
    # argv[0] is either the poormad exe directly, or (normal case) the base Python
    ...
    # `-c <bootstrap that runs poormad_cli.main>`; also accept a plain `-m` launch.
    ...
    launches_module = (
        argv[1:3] == ["-m", "poormad_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "poormad_cli.main" in argv[2])
    )
...
def terminate_owned(pid: int, creation_time_ns: int, poormad_path: str, spawn_nonce: str) -> bool:
    state = process_state(pid, creation_time_ns, poormad_path, spawn_nonce)
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves poormad_cli's own location and prepends its parent, because the
    ...
    # the poormad_cli package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the poormad_cli package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    configured_path = str(payload["poormadPath"])
    if not os.path.isabs(configured_path):
        raise ValueError("PoorMad path must be absolute")
    poormad_path = os.path.abspath(configured_path)
    ...
    venv_dir = os.path.dirname(poormad_path)
    python_entry = os.path.join(venv_dir, "python.exe")
    if not os.path.isfile(python_entry):
        raise ValueError("PoorMad Python runtime was not found")
    ...
    bootstrap = (
        ...
        "runpy.run_module('poormad_cli.main',run_name='__main__',alter_sys=True)"
    )
...
def inspect_poormad(poormad_path: str) -> dict[str, Any]:
    path = os.path.abspath(poormad_path)
    if not os.path.isabs(poormad_path) or not os.path.isfile(path):
        raise ValueError("PoorMad path is not an executable file")
...
def dispatch(argv: list[str]) -> Any:
    ...
    if operation == "probe":
        import platform
        return {"os": "Windows", "arch": platform.machine(), "poormadHome": str(get_default_poormad_root()), "python": sys.executable}
