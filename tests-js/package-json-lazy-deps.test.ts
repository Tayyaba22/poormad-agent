 ...
 *
 * The root ``package.json`` is installed by ``poormad update`` on every user,
 ...
 *   workspace graph entirely. ``poormad update`` and ``poormad doctor --fix``
 ...
 *   ``poormad tools`` → Browser Automation → Camofox, and only used at
 *   runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
 *   Firefox-fork binary, which silently blocked ``poormad update`` for
 ...
 * fails — read the lazy-install guidance in the ``poormad-agent-dev`` skill
...
test('camofox is not in root dependencies (must stay opt-in)', () => {
  ...
  assert.ok(
    ...
    'Camofox is a ~300MB binary-postinstall backend that must stay ' +
      ...
      'Camofox post_setup handler in poormad_cli/tools_config.py so it ' +
      'only installs when the user explicitly selects Camofox via ' +
      '`poormad tools` → Browser Automation → Camofox.'
  )
})
