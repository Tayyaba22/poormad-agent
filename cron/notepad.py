...
Write path is the CLI (``poormad cron notepad <job_id> set <key> <value>``),
...
NOTEPAD_FILE = get_poormad_home().resolve() / "cron" / "notepad.db"
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`poormad cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `poormad cron notepad {job_id} delete "
