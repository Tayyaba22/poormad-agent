...
_POORMAD_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (POORMAD_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `poormad tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "video_gen": {
        "description": (
            ...
            "``poormad tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (POORMAD_KANBAN_TASK env "
    ...
    # Coding posture (base PoorMad — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in poormad_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full PoorMad toolsets (CLI + messaging platforms)
    #
    ...
    # the `poormad send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "poormad-cli": {
        ...
        "tools": _POORMAD_CORE_TOOLS,
        "includes": []
    },

    "poormad-cron": {
        # Mirrors poormad-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `poormad tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as poormad-cli; gated by `poormad tools`",
        "tools": _POORMAD_CORE_TOOLS,
        "includes": []
    },
    ...
    "poormad-email": {
        "description": "Email bot toolset - interact with PoorMad via email (IMAP/SMTP)",
        "tools": _POORMAD_CORE_TOOLS,
        "includes": []
    },
    ...
    "poormad-sms": {
        "description": "SMS bot toolset - interact with PoorMad via SMS (Twilio)",
        "tools": _POORMAD_CORE_TOOLS,
        "includes": []
    },
    ...
    "poormad-webhook": {
        ...
        "tools": _POORMAD_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "poormad-gateway": {
        ...
        "tools": [],
        "includes": ["poormad-telegram", "poormad-discord", "poormad-whatsapp", "poormad-slack", "poormad-signal", "poormad-bluebubbles", "poormad-homeassistant", "poormad-email", "poormad-sms", "poormad-mattermost", "poormad-matrix", "poormad-dingtalk", "poormad-feishu", "poormad-wecom", "poormad-wecom-callback", "poormad-weixin", "poormad-qqbot", "poormad-webhook", "poormad-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``poormad-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_POORMAD_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``poormad-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (poormad-<name>).
        # Gives them _POORMAD_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("poormad-"):
            platform_name = name[len("poormad-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_POORMAD_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
