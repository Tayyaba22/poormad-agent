...
Extracted verbatim from poormad_state.py so the SessionDB mixin modules
(poormad_state_search / poormad_state_schema / poormad_state_portability) can
reference them without importing poormad_state (which would be a cycle).
poormad_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``poormad sessions optimize-storage``. A legacy DB sits at
...
# `poormad sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `poormad db optimize`. This is
...
#
# Several independent PoorMad processes routinely share one state.db (gateway
# service, the Desktop app's `poormad serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `poormad_state._cross_process_repair_lock` (the schema-
...
# rebuilder cannot wedge future rebuilds. It lives here (not poormad_state)
# because the search/schema mixins cannot import poormad_state (cycle).
#
