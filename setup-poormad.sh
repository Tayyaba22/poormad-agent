...
# 5. Symlinks the 'poormad' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/poormad-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink poormad into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up poormad command..."

POORMAD_BIN="$SCRIPT_DIR/venv/bin/poormad"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$POORMAD_BIN" "$COMMAND_LINK_DIR/poormad"
echo -e "${GREEN}✓${NC} Symlinked poormad → $COMMAND_LINK_DISPLAY_DIR/poormad"
...
# ============================================================================
# Seed bundled skills into ~/.poormad/skills/
# ============================================================================

POORMAD_SKILLS_DIR="${POORMAD_HOME:-$HOME/.poormad}/skills"
mkdir -p "$POORMAD_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.poormad/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$POORMAD_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  poormad status        # Check configuration"
if is_termux; then
    echo "  poormad gateway       # Run gateway in foreground"
else
    echo "  poormad gateway install # Install gateway service (messaging + cron)"
fi
echo "  poormad cron list     # View scheduled jobs"
echo "  poormad doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m poormad_cli.main setup
fi
