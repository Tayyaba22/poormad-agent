"""
PoorMad MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (PoorMad-specific extra)

Usage:
    poormad mcp serve
    poormad mcp serve --verbose
...
logger = logging.getLogger("poormad.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using POORMAD_HOME."""
    try:
        from poormad_constants import get_poormad_home
        return get_poormad_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("POORMAD_HOME", Path.home() / ".poormad")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from poormad_constants import get_poormad_home
        directory_file = get_poormad_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("POORMAD_HOME", Path.home() / ".poormad")
...
class EventBridge:
    ...
    This is the PoorMad equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from poormad_constants import get_poormad_home
            db_file = get_poormad_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("POORMAD_HOME", Path.home() / ".poormad")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the PoorMad MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "poormad",
        instructions=(
            "PoorMad messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the PoorMad MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
