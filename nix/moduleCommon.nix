...
#
# `services.poormad-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  poormadHome, programs.poormad-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "poormad-config-attrs";
    description = "PoorMad YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $POORMAD_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "PoorMad";
      ...
      package = mkOption {
        ...
        description = "The poormad-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The PoorMad configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `poormad config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $POORMAD_HOME/.env. PoorMad reads that file at each start, with
          load_poormad_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."poormad/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that PoorMad refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          PoorMad reads SOUL.md and memories/ from POORMAD_HOME, so put those
          files in `poormadHomeFiles`.
      ...
      poormadHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into POORMAD_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          PoorMad reads SOUL.md and the memory files from POORMAD_HOME and not
          from the working directory. Declare those files here, or PoorMad
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the poormad plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. PoorMad discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          poormad_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as poormad (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in poormad-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `poormad gateway`.";
      };
      ...
      # ── The backend: `poormad serve` or `poormad dashboard` ──────────────
      # `poormad serve` and `poormad dashboard` are the same entry point,
      # poormad_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that PoorMad Desktop
      ...
      # controls an external gateway, with `poormad gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that PoorMad
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            POORMAD_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. PoorMad Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."poormad/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `poormad` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.poormad-agent.installPackage was removed. PoorMad now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.poormad-agent.enable = ${lib.boolToString (value != false)};  # the poormad CLI, and POORMAD_HOME for your shells
        programs.poormad-agent.desktop.enable = true;  # the desktop application

      `services.poormad-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "poormad-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "poormad-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "poormad-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "poormad-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "poormad-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # POORMAD_MANAGED variable of the service. The value tells the shell
    ...
    let
      ...
      homeDocumentTree = mkDocumentTree {
        inherit pkgs;
        documents = cfg.poormadHomeFiles;
      };
    ...
    in
    ''
      # Directories. The service units and PoorMad make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${poormadHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. PoorMad
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${poormadHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${poormadHome}/config.yaml
            ${run}chmod ${modes.config} ${poormadHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "poormad-managed" managedSystem} ${poormadHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${poormadHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${poormadHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${poormadHome}/auth.json"
        else
          ''
            if [ ! -e ${poormadHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${poormadHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree poormadHome cfg.poormadHomeFiles}
      ...
      ${run}find ${poormadHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "poormad-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${poormadHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/poormad"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps poormad as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "poormad-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "poormad-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "poormad-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          POORMAD_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export POORMAD_DASHBOARD_SESSION_TOKEN

          if [ -z "$POORMAD_DASHBOARD_SESSION_TOKEN" ]; then
            echo "poormad-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "poormad-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "poormad-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "poormad-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "poormad-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "poormad-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "poormad-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "PoorMad web dashboard and desktop backend"
    else
      "PoorMad backend for PoorMad Desktop";

  # The environment that each PoorMad process needs, from either module.
  #
  # managedSystem gives the value of POORMAD_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give PoorMad an identity and a memory, use
          ${optionPath}.poormadHomeFiles instead. Those files go to
          POORMAD_HOME. PoorMad reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of POORMAD_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
