# nix/nixosModules.nix — the NixOS module for poormad-agent
#
...
#
# Container mode: poormad runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $POORMAD_HOME/.env
# and read by poormad at startup — no container recreation needed for env changes.
#
# Tool resolution: the poormad wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      ...
      poormadHome = "${cfg.stateDir}/.poormad";
      ...
      # POORMAD_HOME via addToSystemPackages, so they can save settings through the
      ...
      # poormad user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "poormad-container-entrypoint" ''
        set -eu

        POORMAD_UID="''${POORMAD_UID:?POORMAD_UID must be set}"
        POORMAD_GID="''${POORMAD_GID:?POORMAD_GID must be set}"

        # ── Group: ensure a group with GID=$POORMAD_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$POORMAD_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="poormad"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$POORMAD_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$POORMAD_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$POORMAD_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$POORMAD_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="poormad"
          TARGET_HOME="/home/poormad"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$POORMAD_UID" -g "$POORMAD_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$POORMAD_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$POORMAD_UID:$POORMAD_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure POORMAD_HOME is owned by the target user.
        ...
        if [ -n "''${POORMAD_HOME:-}" ] && [ -d "$POORMAD_HOME" ]; then
          find "$POORMAD_HOME" \! -user "$POORMAD_UID" -exec chown "$POORMAD_UID:$POORMAD_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/poormad-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/poormad-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/poormad ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/poormad
          chmod 0440 /etc/sudoers.d/poormad
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$POORMAD_UID" --regid="$POORMAD_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $POORMAD_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "poormad-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        poormad_bin=${containerDataDir}/current-package/bin/poormad
      '';

      # Default: /var/lib/poormad/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the poormad group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit poormadHome; };
    ...
    in
    {
      options.services.poormad-agent =
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/poormad";
              description = "State directory. Contains .poormad/ subdir (POORMAD_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the poormad CLI to environment.systemPackages and export
                POORMAD_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.poormad symlink to the service
                  stateDir. These users are automatically added to the poormad group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the poormad CLI to system PATH and export POORMAD_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.poormad/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.POORMAD_HOME = poormadHome;
          })
              ...
              ++ [
                {
                  ...
                  message = "services.poormad-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the poormad user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.poormad-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed poormad
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure poormad is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${poormadHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${poormadHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."poormad-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${poormadHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${poormadHome} ${cfg.workingDirectory}
                  ...
                  find ${poormadHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${poormadHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${poormadHome}/$_subdir"
                    chmod 2770 "${poormadHome}/$_subdir"
                    find "${poormadHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg poormadHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${poormadHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${poormadHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${poormadHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.poormad";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${poormadHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "poormad-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.poormad -> stateDir/.poormad for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.poormad-agent = {
              ...
              # $POORMAD_HOME/.env by the activation script. load_poormad_dotenv()
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.poormad-backend = {
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.poormad-agent = {
              description = "PoorMad Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  POORMAD_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  POORMAD_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env POORMAD_UID="$POORMAD_UID" \
                    --env POORMAD_GID="$POORMAD_GID" \
                    --env POORMAD_HOME=${containerDataDir}/.poormad \
                    --env POORMAD_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/poormad gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
