...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      poormadNpmLib = self'.packages.default.passthru.poormadNpmLib;
      ...
      poormadAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "poormad" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../poormad} $out/bin/poormad
          '')
        ...
        shellHook = ''
          ${poormadAgentDevShellHook}
          ${poormadNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export POORMAD_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "PoorMad dev shell in $POORMAD_PYTHON_SRC_ROOT"
          echo "Ready. Run 'poormad' or 'sandbox poormad' to start."
        '';
      };
    };
}
