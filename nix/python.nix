...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # PoorMad derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            poormad-agent = prev.poormad-agent.overrideAttrs (_old: {
              POORMAD_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $POORMAD_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$POORMAD_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        poormad-agent = prev.poormad-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "poormad-agent-env" {
    poormad-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "poormad-agent-editable-env" {
    poormad-agent = dependency-groups;
  };
}
