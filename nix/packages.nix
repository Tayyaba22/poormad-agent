# nix/packages.nix — PoorMad package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./poormad-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.poormadTui;
        web = full.poormadWeb;
        desktop = full.poormadDesktop;

        update-npm-lockfile = full.poormadNpmLib.updateNpmLockfile;
      };
    };
}
