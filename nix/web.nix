# nix/web.nix — PoorMad Web Dashboard (Vite/React) frontend build
{ poormadNpmLib, ... }:
poormadNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @poormad/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../poormad_cli/web_dist for the
