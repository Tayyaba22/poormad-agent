...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      poormad-agent = self'.packages.default;
      poormadVenv = poormad-agent.poormadVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "poormad-check";
                homeDirectory = "/home/poormad-check";
                stateVersion = "24.11";
              };
            }
            { services.poormad-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.poormad-agent = programs;
              services.poormad-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.poormad-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.poormad-agent);
      ...
      configKeys = pkgs.runCommand "poormad-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${poormadVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "poormad-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "poormad-build-package" { } ''
          echo "PASS: package built at ${poormad-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "poormad-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.POORMAD_TEST = "1";
                environmentFiles = [ "/run/secrets/poormad-env" ];
                poormadHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "poormad" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "poormad" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.poormadAgentSetup.data;

            failures =
              lib.optional (names != [
                "poormad-agent"
                "poormad-backend"
              ]) "expected poormad-agent + poormad-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/poormad gateway" (argvOf "poormad-agent")
              ) "gateway process does not run `poormad gateway`: ${argvOf "poormad-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/poormad serve" (argvOf "poormad-backend")
              ) "backend process does not run `poormad serve`: ${argvOf "poormad-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "poormad-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/poormad-check/.poormad" (envOf n)) names
              ) "gateway and backend must share one POORMAD_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.POORMAD_HOME or null != "/home/poormad-check/.poormad"
              ) "programs.poormad-agent.enable must export POORMAD_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "poormad-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/poormad-check/.poormad/SOUL.md" activation
              ) "poormadHomeFiles must install into POORMAD_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads POORMAD_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "POORMAD_MANAGED=home-manager" (envOf n)) names
              ) "processes must report POORMAD_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "poormad-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "poormad-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/poormad-check";
            ...
            cases = [
              ...
              {
                name = "poormadHomeFiles needs no directory";
                ok = accepts { poormadHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "poormad-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one POORMAD_HOME ───────────────
        # `programs.enable` exports POORMAD_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.poormad
        # while the services use the POORMAD_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/poormad-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                poormadHome = "/home/poormad-check/.poormad-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "poormad-desktop") cfg.home.packages;
            ...
            # second PoorMad runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "poormad-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.poormad-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.poormad-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one poormad-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "POORMAD_HOME" != "/home/poormad-check/.poormad-work"
              ) "the launcher must carry POORMAD_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "POORMAD_HOME")})"
              ++ lib.optional (
                setValue "POORMAD_MANAGED" != "home-manager"
              ) "the launcher must report POORMAD_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "POORMAD_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "POORMAD_DESKTOP_POORMAD" != "${lib.head agentPackages}/bin/poormad"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "POORMAD_DESKTOP_POORMAD")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "POORMAD_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "POORMAD_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "POORMAD_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "POORMAD_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "POORMAD_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "poormad-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares POORMAD_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set POORMAD_HOME" wrapper
              ) "the launcher must carry POORMAD_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "POORMAD_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "POORMAD_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? poormad-backend || cfg.launchd.agents ? poormad-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "poormad-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `poormad`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.poormad-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.poormad-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "poormad-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "poormad-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "poormad" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."poormad-agent-setup".text;

            failures =
              lib.optional (names != [
                "poormad-agent"
                "poormad-backend"
              ]) "expected poormad-agent + poormad-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/poormad gateway" (execOf "poormad-agent")
              ) "gateway unit does not run `poormad gateway`: ${execOf "poormad-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/poormad dashboard" (execOf "poormad-backend")
              ) "backend unit does not run `poormad dashboard`: ${execOf "poormad-backend"}"
              ++ lib.optional (
                units.poormad-agent.environment.POORMAD_HOME != units.poormad-backend.environment.POORMAD_HOME
              ) "gateway and backend must share one POORMAD_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/poormad/.poormad/SOUL.md" activation
              ) "poormadHomeFiles must install into POORMAD_HOME";
          ...
          in
          pkgs.runCommand "poormad-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs poormad, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.poormad-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/poormad serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec poormad directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "poormad-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "poormad-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec poormad, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/poormad dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "poormad-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $POORMAD_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                POORMAD_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "poormad-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'POORMAD_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--poormad-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "poormad-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "poormad-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${poormad-agent}/bin/poormad || (echo "FAIL: poormad binary missing"; exit 1)
          test -x ${poormad-agent}/bin/poormad-agent || (echo "FAIL: poormad-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${poormad-agent}/bin/poormad --version 2>&1 | grep -qi "poormad" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "poormad-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in poormad poormad-agent poormad-acp; do
            test -x ${poormad-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "poormad-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking poormad --help ==="
          ${poormad-agent}/bin/poormad --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${poormad-agent}/bin/poormad --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "poormad-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${poormad-agent}/share/poormad-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${poormad-agent}/share/poormad-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "POORMAD_BUNDLED_SKILLS" ${poormad-agent}/bin/poormad || \
            (echo "FAIL: POORMAD_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: POORMAD_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${poormad-agent}/share/poormad-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${poormad-agent}/share/poormad-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "POORMAD_OPTIONAL_SKILLS" ${poormad-agent}/bin/poormad || \
            (echo "FAIL: POORMAD_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, POORMAD_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "poormad-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${poormad-agent}/share/poormad-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${poormad-agent}/share/poormad-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "POORMAD_BUNDLED_PLUGINS" ${poormad-agent}/bin/poormad || \
            (echo "FAIL: POORMAD_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: POORMAD_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "poormad-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${poormad-agent}/share/poormad-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${poormad-agent}/share/poormad-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${poormad-agent}/share/poormad-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "POORMAD_BUNDLED_LOCALES" ${poormad-agent}/bin/poormad || \
            (echo "FAIL: POORMAD_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: POORMAD_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + POORMAD_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && POORMAD_BUNDLED_LOCALES=${poormad-agent}/share/poormad-agent/locales \
            ${poormadVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with POORMAD_BUNDLED_LOCALES set"; exit 1)
        ...
        # POORMAD_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "poormad-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${poormad-agent}/share/poormad-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${poormad-agent}/share/poormad-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "POORMAD_OPTIONAL_MCPS" ${poormad-agent}/bin/poormad || \
            (echo "FAIL: POORMAD_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: POORMAD_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${poormad-agent}/bin/poormad mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: poormad mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "poormad-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${poormad-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${poormad-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "POORMAD_TUI_DIR" ${poormad-agent}/bin/poormad || \
            (echo "FAIL: POORMAD_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: POORMAD_TUI_DIR set in wrapper"
        ...
        # Verify POORMAD_NODE is set in wrapper and points to Node 26+
        # (PoorMad pins its toolchain to Node 26 everywhere)
        poormad-node = pkgs.runCommand "poormad-node-version" { } ''
          set -e
          echo "=== Checking POORMAD_NODE in wrapper ==="
          grep -q "POORMAD_NODE" ${poormad-agent}/bin/poormad || \
            (echo "FAIL: POORMAD_NODE not set in wrapper"; exit 1)
          echo "PASS: POORMAD_NODE present in wrapper"

          POORMAD_NODE=$(sed -n "s/^export POORMAD_NODE='\(.*\)'/\1/p" ${poormad-agent}/bin/poormad)
          test -x "$POORMAD_NODE" || (echo "FAIL: POORMAD_NODE=$POORMAD_NODE not executable"; exit 1)
          echo "PASS: POORMAD_NODE executable at $POORMAD_NODE"

          NODE_MAJOR=$("$POORMAD_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, PoorMad requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All POORMAD_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify POORMAD_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "poormad-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(POORMAD_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking POORMAD_MANAGED guards ==="
          check_blocked "config set" ${poormad-agent}/bin/poormad config set model foo
          check_blocked "config edit" ${poormad-agent}/bin/poormad config edit
        ...
        extra-python-packages = let
          ...
          poormadWithExtra = poormad-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "poormad-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${poormadWithExtra}/bin/poormad || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${poormadWithExtra}/bin/poormad || \
          ...
          if grep -q "PYTHONPATH" ${poormad-agent}/bin/poormad; then
        ...
        extra-dependency-groups = let
          poormadWithGroups = poormad-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "poormad-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${poormadWithGroups}"
          echo "venv: ${poormadWithGroups.poormadVenv}"
        ...
        messaging-variant = pkgs.runCommand "poormad-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.poormadVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "poormad-config-roundtrip" {
          ...
          merge_and_load() {
            local poormad_home="$1"
            export POORMAD_HOME="$poormad_home"
            ${configMergeScript} ${nixSettings} "$poormad_home/config.yaml"
            ${poormadVenv}/bin/python3 -c '
import json, sys
