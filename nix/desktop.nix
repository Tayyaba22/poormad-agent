# nix/desktop.nix — PoorMad Desktop (Electron) app build + wrapper
#
# `poormadAgent` is the fully-built `.#default` package — it ships the
# `poormad` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `POORMAD_DESKTOP_POORMAD` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing PoorMad CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # POORMAD_HOME and POORMAD_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "poormad-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "poormad-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = poormadNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "poormad-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe poormadNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/poormad-desktop $out/bin
    cp -r ${renderer}/* $out/share/poormad-desktop/
    ...
    substituteInPlace $out/share/poormad-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/poormad-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # POORMAD_DESKTOP_POORMAD to the absolute path of the nix-built `poormad`
    # binary so the desktop's resolver step 4 ("existing PoorMad CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/poormad-desktop \
      --add-flags "$out/share/poormad-desktop" \
      --set POORMAD_DESKTOP_POORMAD "${lib.getExe poormadAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/poormad.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../poormad_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/poormad-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/poormad.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/poormad.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for PoorMad";
    homepage = "https://github.com/Tayyaba22/poormad-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "poormad-desktop";
  };
}
