...
import poormad_cli.auth as auth_mod
...
# un-throttled log fires on *every* selection. On Windows several PoorMad
...
# readiness handshake ("Timed out connecting to PoorMad backend after
...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "nous":
            # PoorMad stores the runtime inference credential in agent_key for
...
def _write_through_provider_state_to_global_root(
    ...
    ``poormad_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``poormad model`` / ``poormad auth`` which
        ...
        Mirrors the PoorMad/Anthropic resync paths above.  Only applies to
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another PoorMad process
    ...
    def _sync_nous_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a PoorMad pool entry from auth.json if tokens differ.

        PoorMad OAuth refresh tokens are single-use.  When another process
        ...
        avoiding a "refresh token reuse" revocation on the PoorMad Portal.
        """
        ...
        try:
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing PoorMad state from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
            ...
            logger.debug("Failed to sync PoorMad entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently PoorMad, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every PoorMad/Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``poormad`` invocation that defaults to the active provider
        (e.g. setup wizard, ``poormad auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across PoorMad
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``POORMAD_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "POORMAD_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "POORMAD_XAI_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("poormad_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another PoorMad
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("poormad_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                # pool. Mirrors the PoorMad quarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another PoorMad process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and PoorMad quarantine paths.
            ...
            if self.provider == "nous":
                synced = self._sync_nous_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("PoorMad refresh failed but auth.json has newer tokens — adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
                ...
                if auth_mod._is_terminal_nous_refresh_error(exc):
                    logger.debug("PoorMad refresh token is terminally invalid; clearing local token state")
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
                        ...
                        logger.debug("Failed to clear terminal PoorMad OAuth state: %s", clear_exc)
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "nous":
            # PoorMad refresh can require network access and should happen when
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other PoorMad profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `poormad model` / `poormad auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `poormad model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``poormad auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `poormad auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `poormad auth remove <provider> <N>` is stable across all source types.
    try:
        from poormad_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, PoorMad PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `poormad setup` ("Claude
        ...
        # Claude Code CLI, poormad_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.poormad/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `poormad setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"poormad_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_poormad_oauth_credentials

        for source_name, creds in (
            ("poormad_pkce", read_poormad_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `poormad auth add nous --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (poormad auth remove copilot gh_cli) must not pay either on
        ...
        # PoorMad auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from poormad_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.poormad/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `poormad auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `poormad auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the PoorMad auth store.  Without this gate the removal is instantly
        ...
        # PoorMad owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``poormad model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``poormad auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from poormad_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
# Prefer ~/.poormad/.env over os.environ — the user's config file is the
# authoritative source for PoorMad credentials. Stale env vars from parent
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_poormad_dotenv() -> apply_onepassword_secrets()).  The raw
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `poormad auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.poormad/.env.
    ...
    try:
        from poormad_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from poormad_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.poormad/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `poormad auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and PoorMad
