...
Resolution order for text tasks (auto mode):
  ...
  3. PoorMad Portal (~/.poormad/auth.json active provider)
...
from poormad_constants import OPENROUTER_BASE_URL
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``POORMAD_CA_BUNDLE`` /
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `poormad update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # PoorMad owns auxiliary retry + provider/model fallback policy (the
    ...
    # PoorMad sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let PoorMad control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="poormad-protected-aux-provider",
        daemon=True,
    ).start()
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before PoorMad triggers summarization.  Higher values delay
...
# "google/gemini-3-flash" kept 404ing here once PoorMad dropped it upstream, and
...
#
# Rolling "-latest" aliases come first where a provider publishes them (PoorMad
...
# PoorMad catalog: gpt-mini-latest 1.40s, claude-haiku-latest 1.55s,
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    """
    try:
        from poormad_cli.auth import resolve_api_key_provider_credentials
        from poormad_cli.models import fetch_models_with_pricing
...
def _get_aux_model_for_provider(provider_id: str, *, prefer_fast: bool = False) -> str:
    ...
    2. ``prefer_fast`` only — the provider's own recommendation hook
       ...
       on long-context side tasks (PoorMad returns its compaction pick), so it
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://poormad.dev",
    "X-Title": "PoorMad",
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    """
    try:
        from poormad_cli.config import cfg_get, load_config
        _cfg = load_config()
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``POORMAD_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``POORMAD_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("POORMAD_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("POORMAD_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "PoorMadAgent",
}
...
from poormad_cli import __version__ as _POORMAD_VERSION

_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://poormad.dev",
    "X-Title": "PoorMad",
    "User-Agent": f"PoorMadAgent/{_POORMAD_VERSION}",
}

# PoorMad Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by PoorMad Portal.
#
# The tags are computed from agent.portal_tags so the client= marker stays
# in lockstep with poormad_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    """Return a fresh PoorMad Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``poormad_cli.__version__`` is
...
# Set at resolve time — True if the auxiliary client points to PoorMad Portal
auxiliary_is_nous: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3.6-flash"
_NOUS_MODEL = "google/gemini-3.6-flash"
_NOUS_DEFAULT_BASE_URL = "https://api.poormad.dev/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_poormad_home() / "auth.json"
...
def _codex_cloudflare_headers(
    ...
    the official endpoint always send PoorMad' originator and version. Custom
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (PoorMad)",
        "originator": "codex_cli_rs",
    }
    if _is_official_codex_base_url(base_url):
        from poormad_cli import __version__

        headers.update({
            "User-Agent": f"PoorMadAgent/{__version__}",
            "originator": "poormad-agent",
        })
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from poormad_cli.auth import _nous_inference_env_override
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("POORMAD_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
    except (TypeError, ValueError):
        return 1800
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Strip the PoorMad-side ``-900k`` large-context picker suffix —
...
class _AnthropicCompletionsAdapter:
    ...
    def __init__(
        ...
        # client's host for PoorMad Portal — a blanket fallback would flip
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private PoorMad plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``poormad_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.poormad/auth.json for an active PoorMad provider.

    Returns the provider state dict if PoorMad is active with tokens,
    otherwise None.
    """
    ...
    try:
        ...
        logger.debug("Could not read PoorMad auth: %s", exc)
        return None


def _nous_api_key(provider: dict) -> str:
    """Extract a usable PoorMad inference JWT from stored auth state."""
    from poormad_cli.auth import _nous_invoke_jwt_is_usable
...
def _nous_base_url() -> str:
    """Resolve the PoorMad inference base URL from env or default."""
    return os.getenv("NOUS_INFERENCE_BASE_URL", _NOUS_DEFAULT_BASE_URL)


def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Resolve PoorMad auxiliary credentials from the selected pool entry."""
    try:
        from poormad_cli.auth import _agent_key_is_usable

        pool = load_pool("nous")
    except Exception as exc:
        logger.debug("Auxiliary PoorMad pool credential resolution failed: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Auxiliary PoorMad pool selection failed: %s", exc)
        return None
    ...
    if force_refresh or not _agent_key_is_usable(state, _nous_min_key_ttl_seconds()):
        try:
            ...
            logger.debug("Auxiliary PoorMad pool refresh failed: %s", exc)
            refreshed = None
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh PoorMad runtime credentials when available.
    ...
    try:
        from poormad_cli.auth import resolve_nous_runtime_credentials

        creds = resolve_nous_runtime_credentials(
            timeout_seconds=env_float("POORMAD_NOUS_TIMEOUT_SECONDS", 15),
            force_refresh=force_refresh,
        )
    except Exception as exc:
        logger.debug("Auxiliary PoorMad runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``poormad auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``poormad_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from poormad_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from PoorMad auth store.
    ...
    try:
        from poormad_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from poormad_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from poormad_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from poormad_cli.models import copilot_default_headers
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    """
    try:
        from poormad_cli.config import cfg_get, load_config_readonly
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Check cross-session rate limit guard before attempting PoorMad —
    # if another session already recorded a 429, skip PoorMad entirely
    ...
    try:
        ...
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping PoorMad Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
    ...
    if runtime is None and not nous:
        logger.warning(
            "Auxiliary PoorMad client unavailable: no PoorMad authentication found "
            "(run: poormad auth)."
        )
    ...
    if runtime is None and nous:
        logger.debug(
            "Auxiliary PoorMad: runtime JWT refresh failed; checking stored "
            "auth.json token."
        )
    global auxiliary_is_nous
    auxiliary_is_nous = True
    logger.debug("Auxiliary client: PoorMad Portal")
    ...
    if not _aux_probe_active():
        ...
        # model is irrelevant to "is PoorMad resolvable?", and the Portal
        # recommended-models fetch below can hit the network.
        try:
            from poormad_cli.models import get_nous_recommended_aux_model
    ...
    if runtime is not None:
        ...
        if not api_key:
            logger.warning(
                "Auxiliary PoorMad client unavailable: no usable inference JWT found "
                "(run: poormad auth add nous)."
            )
            _mark_provider_unhealthy("nous", ttl=60)
            return None, None
...
def _refresh_nous_recommended_model(
    *, vision: bool, stale_model: Optional[str]
    ...
    """Re-fetch the PoorMad Portal's recommended model after a stale-model 404.
    ...
    the PoorMad → OpenRouter catalog, every auxiliary call 404s with
    ...
    try:
        ...
        logger.debug(
            "PoorMad recommended-model refresh failed (%s); using default %s",
            exc, _NOUS_MODEL,
        )
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        from poormad_cli.config import load_config
        from poormad_cli.moa_config import resolve_moa_preset
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `poormad setup` or `poormad model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    from tools.xai_http import poormad_xai_default_headers

    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=poormad_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *,
    ...
    :func:`poormad_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from poormad_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second poormad
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``poormad config aux reset``)."""
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh PoorMad account API says paid access is allowed."""
    try:
        from poormad_cli.nous_account import get_nous_portal_account_info

        account_info = get_nous_portal_account_info(force_fresh=True)
        return account_info.paid_service_access is True
    except Exception as exc:
        logger.debug("Auxiliary PoorMad paid-entitlement refresh check failed: %s", exc)
        return False
...
def _is_model_not_found_error(exc: Exception) -> bool:
    ...
    that has since been dropped from the PoorMad → OpenRouter catalog. The PoorMad
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "api.poormad.dev"):
        return "nous"
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "openai-codex":
            from poormad_cli.auth import resolve_codex_runtime_credentials
...
def _auth_refresh_provider_for_route(
    ...
    Copilot/Codex/Anthropic/PoorMad routes too. (#20832)
    """
    normalized = _normalize_aux_provider(resolved_provider)
    ...
    if base_url_host_matches(client_base_url, "api.anthropic.com"):
        return "anthropic"
    if base_url_host_matches(client_base_url, "api.poormad.dev"):
        return "nous"
    return normalized
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_poormad_fallback_destination", None)
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``poormad_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._poormad_fallback_destination = _fallback_destination_from_entry(
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into PoorMad' built-in discovery
...
def _resolve_auto_route(
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/PoorMad get their chosen chat model; users
      ...
      2. OpenRouter → PoorMad → custom → Codex → API-key providers (fallback
    ...
    #    scenario where a user switches providers via `poormad model` but the
    #    old OPENAI_BASE_URL lingers in ~/.poormad/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: poormad model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.poormad/.env",
    ...
    # on aggregators (OpenRouter, PoorMad) who previously got routed to a
    ...
    if (main_provider and main_model
        ...
        if runtime_base_url and main_provider == "custom":
            ...
            try:
                from poormad_cli.runtime_provider import _get_named_custom_provider
...
def _tag_effective_provider(client: Any, provider: str) -> None:
    ...
    if client is None or not provider:
        return
    try:
        setattr(client, "_poormad_aux_effective_provider", provider)
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_poormad_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from poormad_cli.copilot_auth import copilot_request_headers
        ...
        async_kwargs["default_headers"] = poormad_xai_default_headers()
    else:
    ...
    # See _create_openai_client: disable SDK-internal retries so PoorMad owns
...
def resolve_provider_client(
    ...
    #
    # PoorMad + vision is the one carve-out: the branch below resolves its model
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but PoorMad Portal not configured (run: poormad auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        from poormad_cli.providers import nous_api_mode
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: poormad model)")
                return None, None
    ...
    # OpenRouter / PoorMad bills for side tasks they thought were running on
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: poormad model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: poormad doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("poormad_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        try:
            from poormad_cli.models import opencode_zen_free_runtime as _oc_free_rt
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(poormad_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from poormad_cli.models import _should_use_copilot_responses_api
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → PoorMad → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. PoorMad — skip if already covered by main provider.
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #      :func:`_resolve_provider_vision_default`). PoorMad is the
        ...
        #   3. PoorMad Portal (vision-capable aggregator fallback)
        ...
        if main_provider and main_provider not in {"auto", "", "moa"}:
            ...
            if main_provider == "nous":
                # PoorMad resolves its vision model from the Portal's tier-aware
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes PoorMad Portal product tags when the auxiliary client is backed
    by PoorMad Portal. Returns empty dict otherwise.
    """
...
def _refresh_nous_auxiliary_client(
    *,
    ...
    """Refresh PoorMad runtime creds, rebuild the client, and replace the cache entry."""
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`poormad_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def resolve_compression_fast_lane(
    ...
    *,
    ...
    route is the one PoorMad will call. A requested model covers a compressor
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        ...
        if _provider_norm in {"nous", "nous-portal", "poormad"}:
            from poormad_cli.providers import nous_api_mode
    ...
    # body fields (Gemini ``thinking_config``), and OpenRouter/PoorMad use
    ...
    if _provider_for_portal in {"nous", "nous-portal", "poormad"}:
    ...
    # Anthropic Messages adapters translate PoorMad reasoning into native
    ...
    if reasoning_config and isinstance(reasoning_config, dict):
        ...
        effective_base = base_url or ""
        _nous_on_messages = False
        if provider_norm in {"nous", "nous-portal", "poormad"}:
            from poormad_cli.providers import nous_api_mode
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional PoorMad reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `poormad model`."
                    )
    ...
    try:
        ...
        # ── Stale-model self-heal (PoorMad Portal recommendation drift) ───
        # A long-lived process can pin a Portal-recommended model that has
        # since been dropped from the PoorMad → OpenRouter catalog, so every
        ...
        # known-good default). Only applies to PoorMad-routed calls.
        _heal_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_base_info, "api.poormad.dev")
        )
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            ...
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s: model %r no longer in PoorMad catalog; "
        ...
        # ── PoorMad auth refresh parity with main agent ──────────────────
        client_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_base_info, "api.poormad.dev")
        )
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s: refreshed PoorMad runtime credentials after paid account check, retrying",
                    task or "call",
                )
        ...
        if _is_auth_error(first_err) and client_is_nous:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed PoorMad runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
        ...
        # When the resolved provider returns 401 and neither the PoorMad
...
async def _async_call_llm_impl(
    ...
    *,
    ...
    try:
        ...
        # the PoorMad → OpenRouter catalog, 404'ing every auxiliary call. Force a
        # fresh Portal fetch and retry once with the current recommendation.
        _heal_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_client_base, "api.poormad.dev")
        )
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            healed_model = _refresh_nous_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s (async): model %r no longer in PoorMad catalog; "
                    "retrying with refreshed recommendation %r",
                    task or "call", kwargs.get("model"), healed_model,
                )
                kwargs["model"] = healed_model
        ...
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s (async): refreshed PoorMad runtime credentials after paid account check, retrying",
                    task or "call",
                )
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
        ...
        if _is_auth_error(first_err) and client_is_nous:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed PoorMad runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
