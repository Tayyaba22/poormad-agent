...
cache problem must never block PoorMad startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the PoorMad home used for cache paths.

    ``home_path`` is whatever ``load_poormad_dotenv()`` already resolved;
    falling back to ``$POORMAD_HOME`` / ``~/.poormad`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from poormad_constants import get_poormad_home

        home_path = get_poormad_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<poormad_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if POORMAD_HOME is ever made traversable.
            try:
