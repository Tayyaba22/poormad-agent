"""Credits tracking for PoorMad inference API responses.
...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    """Return True when *model* is a PoorMad free-tier model, using ONLY local data.

    Two signals, both zero-network:

    1. The ``:free`` suffix — the canonical PoorMad free SKU marker (e.g.
       ``nvidia/nemotron-3-ultra:free``). Free by construction on the API side
       (spend is forced to 0 for ``:free`` ids).
    2. The ``stealth/`` prefix — PoorMad stealth-preview SKUs (e.g.
    ...
    3. A peek into the in-process pricing cache in ``poormad_cli.models``
    ...
    if not base_url:
        return False
    try:
        from poormad_cli.models import _is_model_free, _pricing_cache

        # Mirror get_pricing_for_provider's key normalization: the agent's
        # PoorMad base_url is /v1-suffixed (https://api.poormad.dev/v1)
...
def evaluate_credits_notices(
    ...
    *,
    ...
    ``model_is_free``: True when the session's active model is a PoorMad free-tier
    ...
    if target_band != shown_band:
        ...
        if target_band is not None:
            ...
            # only meaningful against a PoorMad subscription cap (no cap → never fires),
...
def parse_credits_headers(
    ...
    try:
        ...
        # sentinel header is absent (the common case for non-PoorMad providers, on
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning(
                    "credits header version %d unsupported, ignoring — update PoorMad",
                    version_val,
                )
            return None
...
# ── Dev test fixtures (POORMAD_DEV_CREDITS_FIXTURE) ───────────────────────────
# Throwaway dev scaffolding: trigger any notice state on demand for testing,
# without real spend or Redis seeding. Set POORMAD_DEV_CREDITS_FIXTURE to either a
...
# POORMAD_DEV_CREDITS scaffolding.
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Return a fixture CreditsState for POORMAD_DEV_CREDITS_FIXTURE, or None.
    ...
    Hard prod-leak guard: a fixture applies ONLY when the dev flag POORMAD_DEV_CREDITS
    is also on, so a stray POORMAD_DEV_CREDITS_FIXTURE (leaked into a shell profile, a
    container env, a launch plist, …) can never surface fabricated balances/notices
    on a real account.
    """
    if not is_truthy_value(os.environ.get("POORMAD_DEV_CREDITS")):
        return None
    raw = os.environ.get("POORMAD_DEV_CREDITS_FIXTURE", "").strip()
    if not raw:
        return None
    name = raw
...
def _credits_state_from_account(info) -> Optional[CreditsState]:
    """Map a PoorMadPortalAccountInfo into a header-shaped CreditsState for the seed.
