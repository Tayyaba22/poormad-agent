...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time PoorMad starts and finds ``~/.openclaw/``.

    Points users at ``poormad claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``poormad claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to PoorMad, run "
        "`poormad claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `poormad claw cleanup` (renames it to ~/.openclaw.pre-migration — "
