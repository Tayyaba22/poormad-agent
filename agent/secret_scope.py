...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the POORMAD_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # PoorMad runtime / deployment
    "POORMAD_HOME", "POORMAD_PROFILE", "POORMAD_GATEWAY_LOCK_DIR",
    "POORMAD_MAX_ITERATIONS", "POORMAD_MAX_TOKENS", "POORMAD_API_TIMEOUT",
    "POORMAD_REDACT_SECRETS", "POORMAD_NOUS_TIMEOUT_SECONDS",
    "_POORMAD_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "POORMAD_KANBAN_DB", "POORMAD_KANBAN_WORKSPACES_ROOT", "POORMAD_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "POORMAD_KANBAN_",
    "POORMAD_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset PoorMad writes
    ...
    ``poormad_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical PoorMad parser: save_env_value
    ...
    from poormad_cli.config import _parse_env_value
...
def build_profile_secret_scope(poormad_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(poormad_home)
    secrets = load_env_file(home / ".env")

    try:
        from poormad_cli.env_loader import get_secret_source_values
