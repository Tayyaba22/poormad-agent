...
Storage
-------
Bundles live in ``~/.poormad/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``POORMAD_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``poormad bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under POORMAD_HOME.

    Honors ``POORMAD_BUNDLES_DIR`` for tests; falls back to
    ``<POORMAD_HOME>/skill-bundles``.
    """
    override = os.environ.get("POORMAD_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_poormad_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `poormad bundles` CLI subcommand.
# ---------------------------------------------------------------------------
