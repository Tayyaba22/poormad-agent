...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into PoorMad with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.poormad/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of PoorMad
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``poormad tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``poormad setup`` for
