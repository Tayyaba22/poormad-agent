"""Fold an agent-as-provider's own activity back into PoorMad' turn state.

Most providers are models: they ask PoorMad to run a tool and PoorMad runs it, so
...
time PoorMad sees the response that work is already done.

Those calls must never come back as pending ``tool_calls`` — PoorMad would re-run
...
* the **skill-review nudge**, whose counter (``_iters_since_skill``) only moves
  on PoorMad tool iterations, of which there are none.

So the provider client hands both back on the completion object and this helper
applies them: ``poormad_projected_messages`` (already-completed
``assistant(tool_calls=[…])`` + ``tool(result)`` history rows) and
``poormad_provider_tool_iterations`` (how many tool iterations happened inside
...
def splice_provider_projection(
    ...
    """
    projected = getattr(response, "poormad_projected_messages", None)
    ...
    raw_iters = getattr(response, "poormad_provider_tool_iterations", 0)
    try:
