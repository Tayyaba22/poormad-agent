...
projection into PoorMad' display, approval bridging, transcript projection into
...
"codex_app_server"`. PoorMad' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 PoorMad secret — gateway
        ...
        spawn_env = poormad_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("POORMAD_KANBAN_TASK"):
            kanban_db = spawn_env.get("POORMAD_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "POORMAD_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("POORMAD_HOME", os.path.expanduser("~/.poormad")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "poormad",
        client_title: str = "PoorMad",
