"""Projects codex app-server events into PoorMad' messages list.

The translator that lets PoorMad' memory/skill review keep working under the
...
PoorMad' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape PoorMad
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way PoorMad' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but PoorMad stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way PoorMad already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — PoorMad' messages store text only.
        text_parts: list[str] = []
