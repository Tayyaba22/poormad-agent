...
Owns one Codex thread per PoorMad session. Drives `turn/start`, consumes
...
# PoorMad' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_POORMAD_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse PoorMad/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per PoorMad session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _POORMAD_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("POORMAD_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="poormad",
            client_title="PoorMad",
            client_version=_get_poormad_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but PoorMad' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into PoorMad' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own poormad-tools callback we
            # auto-accept — the user already approved PoorMad' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO PoorMad
        approval-mode/timeout logic. The PoorMad-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; PoorMad approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map PoorMad approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    PoorMad returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the PoorMad approval mode/timeout resolution
    ...
    # "prompt expired"; the PoorMad-side messaging already distinguishes them.
    return "decline"
...
def _get_poormad_version() -> str:
    """Best-effort PoorMad version string for codex's userAgent line."""
    try:
