...
# Wire-name used when PoorMad keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through PoorMad's configured provider
# (Firecrawl / Tavily / …). Mapped back to ``web_search`` in normalize_response.
_XAI_CLIENT_WEB_SEARCH_ALIAS = "poormad_web_search"
...
# rename on the wire (poormad_<name>), map back in normalize_response so
# PoorMad dispatch is unaffected.
_OPENCODE_RESERVED_TOOL_NAMES = ("web_search", "search_files")
_RESERVED_TOOL_ALIAS_PREFIX = "poormad_"
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        # xAI server-side web search vs PoorMad web providers.
        #
        ...
        #    keep PoorMad dispatch so ``web.backend`` / ``web.search_backend``
        #    is honored, but rename the wire tool to
        #    ``poormad_web_search`` so Grok cannot hijack the name. The alias
        ...
        kwargs = {
            # ``-900k`` large-context picker variants are PoorMad-side aliases
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo the xAI client-path wire alias so PoorMad dispatches
                ...
                # (poormad_web_search / poormad_search_files, #85589).
