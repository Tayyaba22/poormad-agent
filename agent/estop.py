...
``poormad pause`` writes a sentinel file at ``$POORMAD_HOME/ESTOP``;
``poormad resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "PoorMad is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.poormad/ESTOP``.
...
def _poormad_home() -> Path:
    """Resolve the active POORMAD_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active POORMAD_HOME."""
    return _poormad_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on POORMAD_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ PoorMad is paused ({reason}). New work is on hold; "
            "run `poormad resume` to pick things back up."
        )
    return (
        "⏸️ PoorMad is paused. New work is on hold; "
        "run `poormad resume` to pick things back up."
    )
