...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `poormad backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - poormad_home (str): The active POORMAD_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.poormad``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "poormad").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'poormad memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], poormad_home: str) -> None:
        ...
        Called by 'poormad memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``poormad_home`` is the active POORMAD_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE POORMAD_HOME.

        ``poormad backup`` only walks POORMAD_HOME, so any provider state kept
        ...
        of the archive, and ``poormad import`` restores them to their original
