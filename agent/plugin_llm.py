...
Plugins built on PoorMad often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing PoorMad
...
:class:`~poormad_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors PoorMad' main config
...
handles every provider, fallback chain, and per-task override PoorMad
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`poormad_cli.plugins.PluginContext`
