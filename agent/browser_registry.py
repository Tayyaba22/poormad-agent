...
2. Legacy preference order — ``browser-use`` → ``browserbase`` — filtered by
   ...
   because it covers both the managed PoorMad gateway and direct API key path;
...
def registry_generation(*, scope: Optional[str] = None) -> tuple[int, int]:
    ...
    active_scope = scope or poormad_home_key()
    with _lock:
...
def _resolve(configured: Optional[str]) -> Optional[BrowserProvider]:
    ...
    plugins added under ``~/.poormad/plugins/browser/<vendor>/`` are subject
