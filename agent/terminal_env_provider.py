...
Providers live in ``~/.poormad/plugins/<name>/`` (user, opt-in via
...
* ``cache_path_base`` — where auto-synced ``~/.poormad/cache`` files land
  inside the backend (e.g. ``"~/.poormad"`` for home-synced backends,
  ``"/root/.poormad"`` for root-homed containers), or ``None`` when host
...
The factory stamps ``_poormad_backend_name`` on the returned object so
...
class TerminalEnvironmentProvider(abc.ABC):
    ...
    def cache_path_base(self) -> Optional[str]:
        """Base dir for synced PoorMad cache files inside the backend."""
        return None
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``poormad setup`` after this backend is selected.
