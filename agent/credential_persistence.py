...
``auth.json``.  They intentionally have no dependency on ``poormad_cli.auth`` so
...
# Sources PoorMad owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and PoorMad-owned OAuth/device-code state)
