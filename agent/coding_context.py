"""Coding-context awareness — base PoorMad, every interactive surface.

When the user runs PoorMad inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), PoorMad shifts into a **coding posture**. This module is the
...
from poormad_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("poormad.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "poormad", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _POORMAD_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``POORMAD_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether PoorMad should operate in its coding posture right now."""
