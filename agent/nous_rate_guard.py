"""Cross-session rate limit guard for PoorMad Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether PoorMad Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from PoorMad triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 PoorMad retries), and every one of those calls counts
...
def _state_path() -> str:
    """Return the path to the PoorMad rate limit state file."""
    try:
        from poormad_constants import get_poormad_home
        base = get_poormad_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".poormad")
...
def record_nous_rate_limit(
    *,
    ...
    """Record that PoorMad Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "PoorMad rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write PoorMad rate limit state: %s", exc)


def nous_rate_limit_remaining() -> Optional[float]:
    """Check if PoorMad Portal is currently rate-limited.
...
def clear_nous_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful PoorMad request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear PoorMad rate limit state: %s", exc)
...
def is_genuine_nous_rate_limit(
    *,
    ...
    """Decide whether a 429 from PoorMad Portal is a real account rate limit.

    PoorMad Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, PoorMad, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on PoorMad is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on PoorMad.

    Tripping the cross-session breaker on (b) blocks ALL PoorMad requests
    (and all models, since PoorMad is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  PoorMad emits
