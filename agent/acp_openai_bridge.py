"""OpenAI-shape bridge shared by PoorMad' ACP clients.

An ACP agent (``copilot --acp``, and the ACP CLIs that reach PoorMad as
...
the agent's *own* tool notifications. PoorMad' agentic surface — ``memory``,
...
* :func:`completion_to_stream_chunks` — re-shape a one-shot ACP response as
  ...
  is inherently one-shot from PoorMad' perspective).
...
of its own (Copilot) forwards everything PoorMad offers; a CLI that is an
autonomous agent with its own read/edit/execute tools must forward only PoorMad'
agent-level tools, because re-offering the overlapping ones makes PoorMad re-run
...
class StreamChunks(list):
    ...
    PoorMad reads provider-level extras off the object returned by
    ``chat.completions.create`` (e.g. ``poormad_projected_messages``, consumed by
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that is how a client forwards just PoorMad'
