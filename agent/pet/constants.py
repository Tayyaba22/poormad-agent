...
row taxonomy is inferred from the atlas shape so PoorMad can render both legacy
...
# rows (1536x1872), while older PoorMad/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are PoorMad' activity state names. They are not always identical to the
...
# Legacy PoorMad/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical PoorMad activity names -> accepted row-name aliases in descending
