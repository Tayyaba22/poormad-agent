...
_POORMAD_MD_NAMES = (".poormad.md", "POORMAD.md")


def _find_poormad_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.poormad.md`` or ``POORMAD.md``.
    ...
    # could pick up a .poormad.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _POORMAD_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are PoorMad, an intelligent AI assistant created by PoorMad. "
...
POORMAD_AGENT_HELP_GUIDANCE = (
    "You run on PoorMad (by PoorMad). When the user needs help with "
    "PoorMad itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://poormad.dev/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `poormad-agent` skill with skill_view(name='poormad-agent') "
...
POORMAD_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on PoorMad (by PoorMad). When the user needs help with "
    "PoorMad itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://poormad.dev/docs is the "
    "authoritative reference and always holds the latest, most up-to-date "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.poormad/kanban.db`. Your task id is in "
    "`$POORMAD_KANBAN_TASK`; your workspace is `$POORMAD_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $POORMAD_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $POORMAD_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${POORMAD_KANBAN_BRANCH:-wt/$POORMAD_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$POORMAD_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`poormad profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `poormad kanban <verb>` for board operations. Use "
...
#
# The poormad-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to poormad-agent's Python
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that PoorMad "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of PoorMad floating over another application, so the
    user is rarely asking about PoorMad — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating PoorMad "
        ...
        "rather than anything inside PoorMad. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the PoorMad terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the PoorMad desktop app — a graphical chat "
        ...
        "Widgets can talk back: window.poormad.send(\"prompt\") — or a "
        "data-poormad-send=\"prompt\" attribute on any clickable element — sends "
    ...
    "webui": (
        "You are in the PoorMad WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where PoorMad itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host PoorMad runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where PoorMad itself is running. The host OS, home, and cwd "
                f"of the PoorMad process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where PoorMad "
    ...
    # Embedder-supplied environment description. Lets a host that wraps PoorMad
    ...
    extra = (os.getenv("POORMAD_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_poormad_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("POORMAD_PLATFORM") or os.environ.get("POORMAD_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.poormad/skills/`` directory.  External dirs
    ...
    # POORMAD_HOME would otherwise fall back to the launch (default) home and
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_poormad_home_override(str(skills_dir.parent))
    else:
        skills_dir = get_skills_dir()
        _home_token = None
    try:
        external_dirs = get_all_skills_dirs()[1:]  # skip local (index 0)
        # Trusted project-local dirs (./.poormad/skills, ./.agents/skills at
        ...
        if _home_token is not None:
            reset_poormad_home_override(_home_token)
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot PoorMad itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `poormad-agent` skill "
            "first. It has the actual commands (e.g. `poormad config set …`, `poormad tools`, "
            "`poormad setup`) so you don't have to guess or invent workarounds.\n"
...
def load_soul_md(
    ...
    """Load SOUL.md from POORMAD_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the POORMAD_HOME ContextVar falls
    back to the launch home and reads the wrong profile's SOUL.md (#50233,
    same class as the skills-index leak fixed in #86313).
    """
    try:
        from poormad_cli.config import ensure_poormad_home
        ensure_poormad_home()
    except Exception as e:
        logger.debug("Could not ensure POORMAD_HOME before loading SOUL.md: %s", e)

    _home = Path(home_override) if home_override is not None else get_poormad_home()
...
def _load_poormad_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.poormad.md / POORMAD.md — walk to git root."""
    poormad_md_path = _find_poormad_md(cwd_path)
    if not poormad_md_path:
        return ""
    try:
        content = poormad_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = poormad_md_path.name
        try:
            rel = str(poormad_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".poormad.md", context_length=context_length,
            read_path=str(poormad_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", poormad_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .poormad.md / POORMAD.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from POORMAD_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the PoorMad install/source
    ...
    # explicitly configured cwd is honored verbatim — the PoorMad tree is a
    ...
    # their launch dir IS the user's shell cwd (developing PoorMad in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the PoorMad install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_poormad_md(cwd_path, context_length)
    ...
    # SOUL.md from POORMAD_HOME only — skip when already loaded as identity
    if not skip_soul:
