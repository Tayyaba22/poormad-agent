"""Unified removal contract for every credential source PoorMad reads from.

PoorMad seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.poormad/.env
    claude_code   — ~/.claude/.credentials.json
    poormad_pkce   — ~/.poormad/.anthropic_oauth.json
    ...
    manual        — user ran `poormad auth add`
...
unify here is **removal**:
    ...
    ``poormad auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), poormad_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.poormad/.env  → clear it
    ...
    """
    from poormad_cli.config import get_env_path, remove_env_value
    ...
    try:
        ...
        if env_path.exists():
            ...
            # canonical reader in poormad_cli/config.py. read_text() with no
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.poormad/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to PoorMad.",
            f"  The pool entry is now suppressed — PoorMad will ignore "
            f"{env_var} until you run `poormad auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so PoorMad stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `poormad auth add anthropic` to re-enable if needed.",
    ])


def _remove_poormad_pkce(provider: str, removed) -> RemovalResult:
    """~/.poormad/.anthropic_oauth.json is ours — delete it outright."""
    from poormad_constants import get_poormad_home

    result = RemovalResult()
    oauth_file = get_poormad_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared PoorMad Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    """PoorMad OAuth lives in auth.json providers.nous — clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `poormad auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `poormad auth add nous` to re-engage, which is the
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as PoorMad: single-source OAuth state with refresh tokens.
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``poormad auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `poormad model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the PoorMad auth store is not enough — _seed_from_singletons() would
    ...
    via ``poormad auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `poormad auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `poormad auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    PoorMad stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `poormad auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="poormad_pkce",
        remove_fn=_remove_poormad_pkce,
        description="~/.poormad/.anthropic_oauth.json",
    ))
    register(RemovalStep(
