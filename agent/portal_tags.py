"""Centralized PoorMad Portal request tags.

Every PoorMad request that hits the PoorMad Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
PoorMad can attribute usage to PoorMad and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=poormad-agent",
        "client=poormad-client-v<__version__>",
    ]

The version is sourced live from ``poormad_cli.__version__`` so it auto-aligns
...
``poormad_cli.__version__`` is the canonical source of truth.
"""
...
def _poormad_version() -> str:
    """Return the current PoorMad release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``poormad_cli`` cannot be imported (should
...
def poormad_client_tag() -> str:
    """Return the ``client=...`` tag for PoorMad Portal requests.

    Format: ``client=poormad-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=poormad-client-v{_poormad_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a PoorMad session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    PoorMad conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.poormad/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    """Return the canonical list of PoorMad Portal product tags.

    Always returns a fresh list so callers can mutate it freely
    (e.g. ``merged_extra.setdefault("tags", []).extend(nous_portal_tags())``).

    When ``session_id`` is provided, a ``conversation=<session_id>`` tag is
    appended so Portal usage can be attributed to a specific PoorMad
    ...
    """
    tags = ["product=poormad-agent", poormad_client_tag()]
