...
Data source: the NAS account-info fetch (``PoorMadPortalAccountInfo``), whose
...
def usage_model_from_account(account_info: Any) -> UsageModel:
    """Build a :class:`UsageModel` from a ``PoorMadPortalAccountInfo``. Fail-open.
...
def build_usage_model(*, timeout: float = 10.0) -> UsageModel:
    ...
    Dev override: ``POORMAD_DEV_CREDITS_FIXTURE`` short-circuits to a fixture so
...
def _dev_fixture_usage_model() -> Optional[UsageModel]:
    """Map ``POORMAD_DEV_CREDITS_FIXTURE`` to a usage model for offline UX work.

    Recognized names: ``free | healthy | low | topup | depleted``. Returns
    ``None`` when the env var is unset (real portal path runs).
    """
    name = (os.getenv("POORMAD_DEV_CREDITS_FIXTURE") or "").strip().lower()
    if not name:
        return None
