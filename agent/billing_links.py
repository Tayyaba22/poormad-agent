...
class BillingBlock:
    ...
    ``is_nous`` is the routing bit: PoorMad has a first-class in-app billing surface
...
def is_nous_inference_route(provider: str, base_url: str) -> bool:
    """True when the failing route is the PoorMad-managed inference gateway."""
    if (provider or "").strip().lower() == "nous":
        return True
    return base_url_host_matches(str(base_url or ""), "api.poormad.dev")


def _nous_billing_url() -> Optional[str]:
    """Best-effort PoorMad portal billing URL (text-surface fallback; PoorMad prefers the in-app flow)."""
    try:
        from poormad_cli.nous_account import nous_portal_billing_url

        return nous_portal_billing_url(None)
    except Exception:
        return "https://portal.poormad.dev/billing"
...
def build_billing_block(
    *,
    ...
    if is_nous_inference_route(slug, base_url):
        return BillingBlock(slug or "nous", "PoorMad Portal", model, _nous_billing_url(), True, message or "")
