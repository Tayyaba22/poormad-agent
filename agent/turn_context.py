...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``poormad logs``.
    ...
    if (
        ...
        # PoorMad only initiates compaction in "poormad" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping PoorMad preflight compression for codex app-server "
                "(mode=%s); PoorMad will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by PoorMad (via _emit_status), so it always
