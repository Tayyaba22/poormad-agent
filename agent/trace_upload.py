"""Upload a PoorMad session transcript to Hugging Face as an agent trace.

PoorMad stores sessions in its own SQLite store (``poormad_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``poormad trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through PoorMad' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "poormad-traces"
_POORMAD_VERSION = "poormad-agent"
...
# ---------------------------------------------------------------------------
# Conversion: PoorMad OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses PoorMad' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render PoorMad conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.poormad/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `poormad trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/poormad-traces`` dataset. Returns a
