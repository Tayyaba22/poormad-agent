"""Anthropic Messages API adapter for PoorMad.

Translates between PoorMad's internal OpenAI-style message format and
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in poormad_cli/runtime_provider.py.
    """
...
# PoorMad effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _requires_bearer_auth(base_url: str | None) -> bool:
    ...
    Foundry's Anthropic-style endpoint, Palantir Foundry's LLM proxy, and PoorMad
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to poormad's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to poormad's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + PoorMadAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://poormad.dev")
        headers.setdefault("X-Title", "PoorMad")
        headers.setdefault("User-Agent", f"PoorMadAgent/{_POORMAD_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (PoorMad loads that into the process env from
    # ``~/.poormad/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    PoorMad notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    PoorMad historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``poormad models``) never mutates ``~/.poormad/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by PoorMad)
      ...
      5. Anthropic credential_pool OAuth entry (~/.poormad/auth.json)
    ...
    # 1. PoorMad-managed OAuth/setup token env var
    ...
    # 5. PoorMad credential_pool OAuth entry.
...
# ── PoorMad-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.poormad/.anthropic_oauth.json (our own file).
...
def _get_poormad_oauth_file() -> Path:
    return get_poormad_home() / ".anthropic_oauth.json"
...
def run_poormad_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run PoorMad-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize PoorMad with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from poormad_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_poormad_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read PoorMad-managed OAuth credentials from ~/.poormad/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read PoorMad OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See poormad-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See poormad-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See poormad-agent#13848 (Kimi) and
    poormad-agent#16748 (DeepSeek).

    PoorMad Portal's ``/v1/messages`` route is the exception among third-party
...
def build_anthropic_kwargs(
    ...
    # PoorMad Portal routes on its own catalog ids (``anthropic/claude-opus-4.8``);
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("PoorMad", "Claude Code")
                text = text.replace("PoorMad", "Claude Code")
                text = text.replace("poormad-agent", "claude-code")
                text = text.replace("PoorMad", "Anthropic")
                block["text"] = text
        ...
        #      a) bare PoorMad-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that PoorMad surfaces in its CLI. We
...
def create_anthropic_message(
    ...
    *,
    ...
    parsed ``Message`` drops — PoorMad Portal's ``x-nous-credits-*`` balance family
