...
_UNSET = object()
_POORMAD_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_POORMAD_HOME_OVERRIDE", default=_UNSET
)
...
def set_poormad_home_override(path: str | Path | None) -> Token:
    """Set a context-local PoorMad home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _POORMAD_HOME_OVERRIDE.set(value)


def reset_poormad_home_override(token: Token) -> None:
    """Restore the previous context-local PoorMad home override."""
    _POORMAD_HOME_OVERRIDE.reset(token)


def get_poormad_home_override() -> str | None:
    """Return the active context-local PoorMad home override, if any."""
    override = _POORMAD_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_poormad_home() -> Path:
    """Return the platform-native default PoorMad home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "poormad"
    return Path.home() / ".poormad"


def _poormad_home_from_env() -> Path:
    """Resolve POORMAD_HOME from the process environment only.

    Reads the ``POORMAD_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_poormad_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_poormad_home`
    and :func:`get_process_poormad_home` so the two never drift.
    """
    val = os.environ.get("POORMAD_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_poormad_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``POORMAD_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_poormad_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[POORMAD_HOME fallback] POORMAD_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass POORMAD_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_poormad_home() -> Path:
    """Return the PoorMad home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_poormad_home_override`) → ``POORMAD_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``POORMAD_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``POORMAD_HOME`` explicitly (see the systemd
    template in ``poormad_cli/gateway.py`` and the kanban dispatcher in
    ``poormad_cli/kanban_db.py``).  See https://github.com/Tayyaba22/poormad-agent/issues/18594.
    """
    override = get_poormad_home_override()
    if override:
        return Path(override)

    if not os.environ.get("POORMAD_HOME", "").strip():
        _warn_profile_fallback_once()

    return _poormad_home_from_env()


def poormad_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a PoorMad home/profile directory.
    ...
    """
    candidate = Path(path) if path is not None else get_poormad_home()
    resolved = candidate.expanduser().resolve(strict=False)
    return os.path.normcase(str(resolved))


def get_process_poormad_home() -> Path:
    """Return the PoorMad home for the running process, ignoring task overrides.

    Unlike :func:`get_poormad_home`, this never follows the context-local
    override set by :func:`set_poormad_home_override`.  It resolves only the
    process ``POORMAD_HOME`` env var (falling back to the platform default),
...
# Process-level memo for get_default_poormad_root(). The function resolves
# POORMAD_HOME against the native home on every call (~80us of path
...
# Its result depends only on (POORMAD_HOME, platform native home), which are
# compared for free on each call, so the memo is freshness-correct even if a
# test or plugin mutates POORMAD_HOME mid-process.
_default_poormad_root_memo: "tuple[str, str, Path] | None" = None


def get_default_poormad_root() -> Path:
    """Return the root PoorMad directory for profile-level operations.

    In standard deployments this is the platform-native PoorMad home
    (``~/.poormad`` on POSIX, ``%LOCALAPPDATA%\\poormad`` on native Windows).

    In Docker or custom deployments where ``POORMAD_HOME`` points outside
    ``~/.poormad`` (e.g. ``/opt/data``), returns ``POORMAD_HOME`` directly
    — that IS the root.

    In profile mode where ``POORMAD_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.poormad/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    global _default_poormad_root_memo
    native_home = _get_platform_default_poormad_home()
    env_home = os.environ.get("POORMAD_HOME", "")
    if _default_poormad_root_memo is not None:
        memo_native, memo_env, memo_result = _default_poormad_root_memo
    ...
    if not env_home:
        ...
        try:
            ...
            # POORMAD_HOME is under ~/.poormad (normal or profile mode)
            result = native_home
        except ValueError:
            ...
            if env_path.parent.name == "profiles":
                ...
                # Not a profile path — POORMAD_HOME itself is the root
                result = env_path
    _default_poormad_root_memo = (str(native_home), env_home, result)
    return result
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``POORMAD_OPTIONAL_SKILLS``.
    """
    override = os.getenv("POORMAD_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_poormad_home() / "optional-skills"


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers.

    Mirrors :func:`get_optional_skills_dir` for the MCP catalog (PoorMad-approved
    Model Context Protocol servers shipped with the repo but disabled by
    default). Packaged installs may ship ``optional-mcps`` outside the Python
    package tree and expose it via ``POORMAD_OPTIONAL_MCPS``.
    """
    override = os.getenv("POORMAD_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_poormad_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``POORMAD_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<POORMAD_HOME>/skills`` last-resort
    """
    override = os.getenv("POORMAD_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_poormad_home() / "skills"
...
def get_poormad_dir(
    ...
    *,
    ...
    """Resolve a PoorMad subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to POORMAD_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to POORMAD_HOME (e.g. ``"image_cache"``).
        home: Optional explicit PoorMad home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local POORMAD_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_poormad_home()
...
def iter_poormad_node_dirs(home: Path | None = None) -> list[Path]:
    """Return PoorMad-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\poormad\\node``. POSIX installs use
    ``$POORMAD_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_poormad_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with poormadManagedNodePathEntries() in
...
_POORMAD_NODE_TARGET_MAJOR = int(os.environ.get("POORMAD_NODE_TARGET_MAJOR", "22"))
_managed_node_heal_attempted = False
_NODE_BOOTSTRAP_SCRIPT = Path(__file__).resolve().parent / "scripts" / "lib" / "node-bootstrap.sh"

# Install tree root (this file lives at <install_root>/poormad_constants.py).
# Used by secure_parent_dir() to skip chmod on the install dir — chmodding it
# 0700 breaks poormad-user traversal in Docker (UID 10000). See #25821, #93050.
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    PoorMad-managed Node trees live under ``$POORMAD_HOME/node`` (or a profile's
    ``POORMAD_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_poormad_node_executable``
    used to trust file presence alone, so ``poormad update`` would pick that
...
def poormad_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any PoorMad-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_poormad_node_dirs(home):
        for name in names:
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ PoorMad-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `poormad update` afterwards).",
        flush=True,
    )


def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%POORMAD_HOME%\\node`` on Windows.
    ...
    ``FILE_SHARE_DELETE`` — the same mechanism as the poormad.exe quarantine);
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_POORMAD_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_POORMAD_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$POORMAD_HOME/node`` on POSIX.
    ...
    ``POORMAD_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_poormad_managed_node() -> str | None:
    """Install a PoorMad-managed Node tree and return its npm path.
    ...
    PoorMad never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$POORMAD_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_poormad_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_poormad_node_dirs():
...
def heal_poormad_managed_node() -> bool:
    """Redownload PoorMad-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not poormad_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "POORMAD_HOME": str(get_poormad_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_poormad_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_POORMAD_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_poormad_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _POORMAD_NODE_TARGET_MAJOR
    return False


def find_poormad_node_executable(command: str) -> str | None:
    """Return a PoorMad-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_POORMAD_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_poormad_managed_node():
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for PoorMad-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy PoorMad-managed installs.

    This is for PoorMad-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_poormad_node_executable(command)
    if managed:
        return managed
    if poormad_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_poormad_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with PoorMad-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_poormad_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``poormad update`` — leaving a **dangling symlink** that ``which`` still
...
def display_poormad_home() -> str:
    """Return a user-friendly display string for the current POORMAD_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.poormad``
        profile:  ``~/.poormad/profiles/coder``
        custom:   ``/opt/poormad-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.poormad``.  For code that needs a real ``Path``, use
    :func:`get_poormad_home` instead.
    """
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``POORMAD_HOME`` or other path
    env vars resolve to an unexpected location.

    Also refuses to chmod the poormad-agent install tree (the directory this
    ...
    See https://github.com/Tayyaba22/poormad-agent/issues/25821 and
    https://github.com/Tayyaba22/poormad-agent/pull/93050.
    """
    parent = path.parent.resolve()
    # Refuse root and its direct children (/usr, /home, /var, /tmp, …).
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree root. chmodding it 0700 breaks poormad-user
    ...
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        # A credential file inside the install tree usually means POORMAD_HOME
        ...
        logging.getLogger(__name__).warning(
            ...
            "poormad-agent install directory (%s). Credential files are "
            "normally stored under the poormad home directory instead.",
            parent,
            _INSTALL_ROOT,
        )
        return
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{POORMAD_HOME}/home`` when the profile-home directory exists."""
    poormad_home = get_poormad_home_override() or (env or {}).get("POORMAD_HOME") or os.getenv("POORMAD_HOME")
    if not poormad_home:
        return None
    profile_home = os.path.join(poormad_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("POORMAD_REAL_HOME") or os.getenv("POORMAD_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding PoorMad profile HOME.

    ``POORMAD_HOME`` scopes PoorMad state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={POORMAD_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{POORMAD_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{POORMAD_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply PoorMad' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["POORMAD_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when PoorMad itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: PoorMad/poormad-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under POORMAD_HOME.

    Replaces the ``get_poormad_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, poormad_logging.py, poormad_time.py, etc.).
    """
    return get_poormad_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under POORMAD_HOME."""
    return get_poormad_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under POORMAD_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_poormad_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._poormad_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``poormad_cli`` modules using three different Windows
    ...
    A few sites outside ``poormad_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``poormad_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of PoorMad itself. An ImportError
...
# `poormad_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with PoorMad.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("poormad_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `poormad update` is exactly
    ...
    return [
        ...
        "    poormad update",
        "If that also fails, reinstall: https://poormad.dev",
    ]
