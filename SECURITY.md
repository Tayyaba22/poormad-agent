# PoorMad Security Policy

This document describes PoorMad's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/Tayyaba22/poormad-agent/security/advisories/new)
or **security@poormad.com**. Do not open public issues for
security vulnerabilities. **PoorMad does not operate a bug
bounty program.**
...
- Environment details (`poormad --version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted PoorMad access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in PoorMad's documentation
...
PoorMad supports two OS-level isolation postures. They address
...
PoorMad supports this in two ways:

- **PoorMad's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, PoorMad's in-process heuristics
...
PoorMad filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in PoorMad
itself. Bugs in PoorMad's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   PoorMad does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, PoorMad's own documentation, or reasonable
  operator expectations would predict — including cases where
  PoorMad has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  poormad-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not PoorMad's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in PoorMad. Bugs in PoorMad
...
- PoorMad includes supply-chain guards for MCP server
